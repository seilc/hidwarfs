/*
    Compile unit: E:\SD2\Game\flat\xCutscene.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
static class xCutscene sActiveCutscene; // size: 0x138, address: 0x3ABD40
static unsigned int sCutTocCount; // size: 0x4, address: 0x3583A0
static class xCutsceneInfo * sCutTocInfo; // size: 0x4, address: 0x3583A4
static class _xEnt sCutsceneFakeEntity[8]; // size: 0x740, address: 0x3ABE80
void csAsyncReadCB(signed int); // size: 0x0, address: 0x1D04B0
void csDrawFakeEntity(class _xEnt *); // size: 0x0, address: 0x1D10D0
...
void SoundCutsceneCB(class iSndCutsceneCallbackInfo *); // size: 0x0, address: 0x1D04C0
// total size: 0x28
...
public:
    ...
    class _xVec3 Center; // offset 0x10, size 0xC
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D04B0 -> 0x001D04BC
*/
// Range: 0x1D04B0 -> 0x1D04BC
static void csAsyncReadCB() {
    /* anonymous block */ {
        // Range: 0x1D04B0 -> 0x1D04BC
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D04C0 -> 0x001D0558
*/
// Range: 0x1D04C0 -> 0x1D0558
static void SoundCutsceneCB(class iSndCutsceneCallbackInfo * info /* r17 */) {
    /* anonymous block */ {
        // Range: 0x1D04C0 -> 0x1D0558
        void * data; // r2
        unsigned int channel; // r2
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D0560 -> 0x001D0630
*/
// Range: 0x1D0560 -> 0x1D0630
static void * SoundGetData(class xCutscene * csn /* r2 */, unsigned int channel /* r2 */, unsigned int chunk /* r2 */) {
    /* anonymous block */ {
        // Range: 0x1D0560 -> 0x1D0630
        unsigned int channelIndex; // r11
        unsigned int numData; // r10
        unsigned int dataIndex; // r9
        class xCutsceneData * data; // r8
        void * retdata; // r2
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D0630 -> 0x001D0F14
*/
// Range: 0x1D0630 -> 0x1D0F14
void xCutscene_Render(class xCutscene * csn /* r16 */, class _xEnt * * alphaent /* r29+0x16C */, signed int * alphacnt /* r29+0x168 */, float * alphadist /* r29+0x164 */) {
    /* anonymous block */ {
        // Range: 0x1D0630 -> 0x1D0F14
        class FakeEntityInfo * finfo; // r2
        class FakeEntityInfo * finfo; // r2
        unsigned int skipsize; // r4
        signed short weight[4]; // r29+0x11C0
        signed short * v_array[4]; // r29+0x11B0
        class xMorphTargetFile * mphFile; // r21
        class xCutsceneMphFrame * mphFrame; // r29+0x110
        unsigned int numFrame; // r20
        signed int matnum; // r4
        class RwTexture * tex; // r2
        class RpGeometry * geom; // r17
        unsigned int frameIndex; // r5
        unsigned int frameMax; // r9
        unsigned int frameMin; // r8
        unsigned int subIndex; // r7
        unsigned int * currvis; // r17
        unsigned int viscnt; // r2
        unsigned int tempSize; // r29+0x11CC
        class _xVec3 * camVec; // r29+0x100
        class RwMatrixTag animMat[65]; // r29+0x170
        class RpAtomic * model; // r19
        class xCutsceneData * mphdata; // r18
        class xCutsceneData * data; // r29+0xF0
        unsigned int backface; // r29+0xE0
        unsigned int hasAlpha; // r29+0xD0
        unsigned int fakeCount; // r23
        unsigned int visIdx; // r29+0xC0
        unsigned int visFlags; // r2
        unsigned int mphIndex; // r17
        unsigned int animIndex; // r2
        unsigned int dataIndex; // r29+0xB0
        unsigned int i; // r7
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D0F20 -> 0x001D10CC
*/
// Range: 0x1D0F20 -> 0x1D10CC
static void csCalcAnimMatrices(class RwMatrixTag * animMat /* r21 */, class RpAtomic * model /* r22 */, class xCutsceneAnimHdr * ahdr /* r20 */, float time /* r29+0x7A0 */) {
    /* anonymous block */ {
        // Range: 0x1D0F20 -> 0x1D10CC
        class _xVec3 * ttt; // r19
        class _xQuat * qqq; // r18
        unsigned int boneidx; // r17
        unsigned int numbone; // r16
        class _xVec3 tranresult[65]; // r29+0x490
        class _xQuat quatresult[65]; // r29+0x80
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D10D0 -> 0x001D114C
*/
// Range: 0x1D10D0 -> 0x1D114C
static void csDrawFakeEntity(class _xEnt * fakeent /* r2 */) {
    /* anonymous block */ {
        // Range: 0x1D10D0 -> 0x1D114C
        class FakeEntityInfo * finfo; // r16
        class RwMatrixTag animMat[65]; // r29+0x20
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D1150 -> 0x001D13AC
*/
// Range: 0x1D1150 -> 0x1D13AC
void xCutscene_SetCamera(class xCutscene * csn /* r16 */) {
    /* anonymous block */ {
        // Range: 0x1D1150 -> 0x1D13AC
        float lerp; // r3
        float * auxdata; // r7
        void * afile; // r2
        class xCutsceneAnimHdr * ahdr; // r21
        class _xVec3 * tranresult; // r20
        class _xQuat * quatresult; // r19
        class _xMat4x3 camMat; // r29+0xA0
        float camFOV; // r29+0xE0
        unsigned int i; // r6
        unsigned int camAux; // r5
        unsigned int dataIndex; // r18
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D13B0 -> 0x001D15B0
*/
// Range: 0x1D13B0 -> 0x1D15B0
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D15B0 -> 0x001D1A34
*/
// Range: 0x1D15B0 -> 0x1D1A34
signed int xCutscene_LoadStep(class xCutscene * csn /* r17 */) {
    /* anonymous block */ {
        // Range: 0x1D15B0 -> 0x1D1A34
        void * foundModel; // r2
        unsigned int tmpSize; // r29+0x4C
        unsigned int skipAccum; // r16
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D1A40 -> 0x001D1AC0
*/
// Range: 0x1D1A40 -> 0x1D1AC0
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D1AC0 -> 0x001D1BDC
*/
// Range: 0x1D1AC0 -> 0x1D1BDC
signed int xCutscene_Destroy(class xCutscene * csn /* r18 */) {
    /* anonymous block */ {
        // Range: 0x1D1AC0 -> 0x1D1BDC
        enum XFILE_READSECTOR_STATUS cdstat; // r2
        unsigned int i; // r16
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D1BE0 -> 0x001D1D34
*/
// Range: 0x1D1BE0 -> 0x1D1D34
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x001D1D40 -> 0x001D1D98
*/
// Range: 0x1D1D40 -> 0x1D1D98
