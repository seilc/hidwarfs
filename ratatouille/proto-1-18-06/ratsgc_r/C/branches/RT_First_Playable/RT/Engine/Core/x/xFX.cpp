/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xFX.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80066674 -> 0x8006A620
*/
// total size: 0x8
struct RwObject {
    // Members
...
struct RpAtomic * (* gAtomicRenderCallBack)(struct RpAtomic *); // size: 0x4, address: 0x80C07350
// total size: 0x10
struct RwRGBAReal {
    // Members
...
struct RpLight * MainLight; // size: 0x4, address: 0x80C07354
unsigned char ReportUVProb; // size: 0x1, address: 0x80BFCD48
// total size: 0x8
struct fxAtomic {
    // Members
...
static unsigned int num_fx_atomics; // size: 0x4, address: 0x80C07358
// total size: 0x40
struct RwMatrixTag {
    // Members
...
static struct RwFrame * g_matfx_frame; // size: 0x4, address: 0x80C0735C
static int sAuraRequests; // size: 0x4, address: 0x80C07360
unsigned int gFXSurfaceFlags; // size: 0x4, address: 0x80C07DE0
// Range: 0x80066674 -> 0x8006667C
void xFXDebugSetSurfaceFlags(unsigned int flags /* r0 */) {
    // References
    // -> unsigned int gFXSurfaceFlags;
}

// Range: 0x8006667C -> 0x80066684
unsigned int xFXDebugGetSurfaceFlags() {
    // References
    // -> unsigned int gFXSurfaceFlags;
}

unsigned int xfx_initted; // size: 0x4, address: 0x80C07364
// Range: 0x80066684 -> 0x8006672C
void xFXInit() {
    // Local variables
...
struct xFXRing ringlist[8]; // size: 0x200, address: 0x80A79D90
// total size: 0x24
...
// Range: 0x80066798 -> 0x800667E0
struct RwRaster * xFXRingFindRWRaster(unsigned int aid_texture /* r0 */) {
    // Local variables
    struct RwTexture * txtr; // r0
}
...
// Range: 0x800667E0 -> 0x800668CC
struct xFXRing * xFXRingCreate(const struct xVec3 * pos /* r30 */, const struct xFXRing * params /* r4 */) {
    // Local variables
...
// Range: 0x800668CC -> 0x800669D8
static void xFXRingUpdate(float dt /* f1 */) {
    // Local variables
    struct xFXRing * ring; // r4
    int i; // r5
...
float shiny_factor; // size: 0x4, address: 0x80BFCD4C
static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFCD50
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x800669D8 -> 0x80066DD8
void xFX_SceneEnter(struct RpWorld * world /* r26 */) {
    // Local variables
    int i; // r29
    int num; // r28
    const struct xSurface * sp; // r0
    struct zSurfaceProps * pp; // r3
    struct zSurfMatFX * fxp; // r0
    struct RpMaterial * mp; // r27
    struct RwTexture * env; // r4
    struct RwTexture * bump; // r4
    struct RwTexture * env; // r25
    struct RwTexture * bump; // r24
    struct xScene * sc; // r27
    struct xEnt * ent; // r28
    struct RpAtomic * (* tmp)(struct RpAtomic *); // r5
...
// Range: 0x80066DD8 -> 0x80066DDC
void xFX_SceneExit() {}

// Range: 0x80066DDC -> 0x80066E10
void xFXUpdate(float dt /* f31 */) {}

// Range: 0x80066E10 -> 0x80066EB4
unsigned int xFXEnvMap(struct RpMaterial * mat /* r30 */, unsigned int envmapID /* r31 */, float coeff /* f31 */) {
    // Local variables
    struct RwTexture * env; // r4
}

// Range: 0x80066EB4 -> 0x80066F58
unsigned int xFXBumpMap(struct RpMaterial * mat /* r30 */, unsigned int bumpmapID /* r31 */, float coeff /* f31 */) {
    // Local variables
    struct RwTexture * bump; // r4
}
...
// Range: 0x80066F58 -> 0x80066FD8
static void LightResetFrame(struct RpLight * light /* r0 */) {
    // Local variables
    struct RwFrame * frame; // r31
    struct RwV3d xAxis; // r1+0x14
    struct RwV3d yAxis; // r1+0x8
}

// Range: 0x80066FD8 -> 0x8006700C
static struct RpMaterial * MaterialDisableMatFX(struct RpMaterial * material /* r31 */) {}

// Range: 0x8006700C -> 0x80067058
struct RpAtomic * AtomicDisableMatFX(struct RpAtomic * atomic /* r31 */) {
    // Local variables
    struct RpGeometry * geometry; // r3
}
...
// Range: 0x80067058 -> 0x8006714C
static struct RpMaterial * MaterialSetEnvMap(struct RpMaterial * material /* r29 */, void * data /* r4 */) {
    // Local variables
...
// total size: 0x0
struct RpSkin {};
// Range: 0x8006714C -> 0x800671F0
static struct RpMaterial * MaterialSetBumpMap(struct RpMaterial * material /* r29 */, void * data /* r4 */) {
    // Local variables
    struct RwTexture * texture; // r31
    struct RwFrame * frame; // r30
...
// Range: 0x800671F0 -> 0x800672D8
static struct RpMaterial * MaterialSetBumpEnvMap(struct RpMaterial * material /* r29 */, struct RwTexture * env /* r30 */, float env_coeff /* f30 */, struct RwTexture * bump /* r31 */, float bump_coeff /* f31 */) {
    // Local variables
    struct RwFrame * frame; // r5
...
static struct RxPipeline * xFXanimUVPipeline; // size: 0x4, address: 0x80C07368
float xFXanimUVRotMat0[2]; // size: 0x8, address: 0x80BFCD60
float xFXanimUVRotMat1[2]; // size: 0x8, address: 0x80BFCD68
float xFXanimUVTrans[2]; // size: 0x8, address: 0x80BFCD70
float xFXanimUVScale[2]; // size: 0x8, address: 0x80BFCD78
// Range: 0x800672D8 -> 0x800672F0
void xFXanimUVSetTranslation(const struct xVec3 * trans /* r0 */) {
    // References
    // -> float xFXanimUVTrans[2];
}

// Range: 0x800672F0 -> 0x80067308
void xFXanimUVSetScale(const struct xVec3 * scale /* r0 */) {
    // References
    // -> float xFXanimUVScale[2];
}

// Range: 0x80067308 -> 0x80067364
void xFXanimUVSetAngle(float angle /* f30 */) {
    // Local variables
    float s; // f0
    float c; // f0
...
float xFXanimUV2PRotMat0[2]; // size: 0x8, address: 0x80BFCD80
float xFXanimUV2PRotMat1[2]; // size: 0x8, address: 0x80BFCD88
float xFXanimUV2PTrans[2]; // size: 0x8, address: 0x80BFCD90
float xFXanimUV2PScale[2]; // size: 0x8, address: 0x80BFCD98
struct RwTexture * xFXanimUV2PTexture; // size: 0x4, address: 0x80C0736C
// Range: 0x80067364 -> 0x8006737C
void xFXanimUV2PSetTranslation(const struct xVec3 * trans /* r0 */) {
    // References
    // -> float xFXanimUV2PTrans[2];
}

// Range: 0x8006737C -> 0x80067394
void xFXanimUV2PSetScale(const struct xVec3 * scale /* r0 */) {
    // References
    // -> float xFXanimUV2PScale[2];
}

// Range: 0x80067394 -> 0x800673F0
void xFXanimUV2PSetAngle(float angle /* f30 */) {
    // Local variables
    float s; // f0
    float c; // f0
...
// Range: 0x800673F0 -> 0x800673F8
void xFXanimUV2PSetTexture(struct RwTexture * texture /* r0 */) {
    // References
    // -> struct RwTexture * xFXanimUV2PTexture;
}

static struct RwMatrixTag gFXUVs[2]; // size: 0x80, address: 0x80A79F90
static unsigned char done; // size: 0x1, address: 0x80C07370
static signed char init; // size: 0x1, address: 0x80C07371
static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFCDA0
// Range: 0x800673F8 -> 0x80067738
struct RpAtomic * xFXanimUVAtomicSetup(struct RpAtomic * atomic /* r17 */, unsigned char dualPass /* r18 */) {
    // Local variables
    struct RpGeometry * geom; // r21
    int numMaterials; // r20
    int i; // r19
    struct RpMaterial * material; // r15
    float rx; // f5
    float ry; // f1
    float ux; // f6
    float uy; // f4
    float py; // f7
    char text[256]; // r1+0x8
...
class tier_queue_allocator joint_alloc; // size: 0x1C, address: 0x80A7A010
struct activity_data * activities[32]; // size: 0x80, address: 0x80A7A02C
int activities_used; // size: 0x4, address: 0x80C07374
unsigned char need_sort; // size: 0x1, address: 0x80C07378
// total size: 0x48
...
// Range: 0x80067778 -> 0x800677C4
// this: r0
void xFXRibbon::create(int disableAutomaticDebugTweaks /* r0 */) {}

// Range: 0x800677C4 -> 0x80067808
// this: r31
...
static struct curve_node default_curve[2]; // size: 0x18, address: 0x80C0AAF8
// Range: 0x80067808 -> 0x8006787C
void xFXRibbon::load_default_config(struct config_type & cfg /* r31 */) {
    // References
    // -> static struct curve_node default_curve[2];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFCDB8
// Range: 0x8006787C -> 0x80067A08
// this: r30
...
static char __FUNCTION__[7]; // size: 0x7, address: 0x80BFCDC4
char __FUNCTION__$localstatic1$pop_back__36tier_queue<Q29xFXRibbon10joint_data>Fv[9]; // size: 0x9, address: 0x80BFCDCC
char __FUNCTION__$localstatic1$push_front__36tier_queue<Q29xFXRibbon10joint_data>FUi[11]; // size: 0xB, address: 0x80BFCDD8
char __FUNCTION__$localstatic1$front__36tier_queue<Q29xFXRibbon10joint_data>Fv[6]; // size: 0x6, address: 0x80BFCDE4
// Range: 0x80067A08 -> 0x80067B60
// this: r28
void xFXRibbon::insert(const struct xVec3 & loc /* r29 */, const struct xVec3 & norm /* r30 */, float scale /* f30 */, float alpha /* f31 */, int flags /* r31 */) {
    // Local variables
    struct joint_data & joint; // r0
...
// Range: 0x80067BD0 -> 0x80067C34
// this: r0
void xFXRibbon::deactivate() {
    // Local variables
    struct activity_data * * move_act_position; // r6
    struct activity_data * move_act; // r7
    struct activity_data * * tail_act_position; // r8
...
static char __FUNCTION__[7]; // size: 0x7, address: 0x80BFCDEC
char __FUNCTION__$localstatic1$back__36tier_queue<Q29xFXRibbon10joint_data>Fv[5]; // size: 0x5, address: 0x80BFCDF4
// Range: 0x80067C34 -> 0x80067DD0
// this: r31
void xFXRibbon::update(float dt /* f31 */) {
    // Local variables
    struct joint_data & joint; // r0
    const struct joint_data & joint2; // r0

    // References
    // -> static char __FUNCTION__[7];
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80BFCDFC
char __FUNCTION__$localstatic1$__vc__36tier_queue<Q29xFXRibbon10joint_data>Fi[5]; // size: 0x5, address: 0x80BFCE04
// Range: 0x80067DD0 -> 0x80067F48
// this: r31
void xFXRibbon::render() {
    // Local variables
    struct RxObjSpace3DVertex * verts; // r30
    int size; // r3
    int it; // r29
    int subsize; // r28
    int next_it; // r27
...
// Range: 0x80067F48 -> 0x80067F6C
struct RwRaster * xFXRibbon::get_raster(const char * name /* r0 */) {}

// Range: 0x80067F6C -> 0x80067FB4
struct RwRaster * xFXRibbon::get_raster(unsigned int id /* r0 */) {
    // Local variables
    struct RwTexture * texture; // r0
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFCE0C
// Range: 0x80067FB4 -> 0x800681FC
void xFXRibbon::get_normal(struct xVec3 & norm /* r30 */, const struct xVec3 & dir /* r31 */, float orient /* f30 */) {
    // Local variables
    float a; // f0
    float b; // f0
    float ax; // f3
...
// Range: 0x800681FC -> 0x8006839C
// this: r28
void xFXRibbon::refresh_joint(struct joint_data & joint /* r29 */, const class iterator & it /* r30 */) {
    // Local variables
    struct xVec3 offset; // r1+0x3C
    float mag2; // f1
    struct xVec3 dir; // r1+0x30
}

// Range: 0x8006839C -> 0x80068550
// this: r27
void xFXRibbon::eval_joint(const struct joint_data & joint /* r29 */, struct xColor_tag & color /* r30 */, float & width /* r31 */) {
    // Local variables
    float frac; // f1
    const struct curve_node * curve; // r6
...
// Range: 0x80068550 -> 0x80068560
void lerp(float & v /* r0 */, float frac /* f0 */, float v0 /* f0 */, float v1 /* f0 */) {}

// Range: 0x80068560 -> 0x800685CC
void lerp(unsigned char & v /* r0 */, float frac /* f0 */, unsigned char v0 /* r0 */, unsigned char v1 /* r0 */) {}
...
// Range: 0x800685CC -> 0x800689A0
// this: r26
void xFXRibbon::render_strip(struct RxObjSpace3DVertex * verts /* r27 */, class iterator & first /* r28 */, unsigned long size /* r29 */) {
    // Local variables
    struct RxObjSpace3DVertex * v; // r30
    class iterator it; // r1+0x28
    class iterator last; // r1+0x20
    struct joint_data & joint; // r31
    ...
    float ny; // f24
    float nz; // f23
    struct RxObjSpace3DVertex * endv; // r0
...
// Range: 0x800689A0 -> 0x800689AC
static void set_vert_uv(struct RxObjSpace3DVertex & vert /* r0 */, float u /* f0 */, float v /* f0 */) {}

// Range: 0x800689AC -> 0x800689DC
static void set_vert(struct RxObjSpace3DVertex & vert /* r0 */, float x /* f0 */, float y /* f0 */, float z /* f0 */, struct xColor_tag & color /* r0 */) {}

// Range: 0x800689DC -> 0x80068A18
// this: r3
void xFXRibbon::copy_parameters(struct xFXRibbon * ribbon /* r31 */) const {}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFCE18
// Range: 0x80068A18 -> 0x80068BA0
int xFXRibbon::compare_ribbons(void * e1 /* r0 */, void * e2 /* r30 */) {
    // Local variables
...
// Range: 0x80068BA0 -> 0x80068C28
void xFXRibbon::sort_active_ribbons() {
    // Local variables
    struct activity_data * * act; // r4
    struct activity_data * * end_act; // r0
...
static struct raster_pair * raster_map; // size: 0x4, address: 0x80C0737C
static int raster_map_size; // size: 0x4, address: 0x80C07380
static struct xFXRibbon dummy_ribbon; // size: 0x14, address: 0x80C07384
// total size: 0xC
struct curve_node {
    // Members
...
static struct config_type dummy_cfg; // size: 0x20, address: 0x80A7A0AC
// Range: 0x80068C28 -> 0x80068D58
...
static struct tweak_callback cb_config; // size: 0x28, address: 0x80417E80
static signed char init; // size: 0x1, address: 0x80C07398
static struct tweak_callback cb_change_texture; // size: 0x28, address: 0x80417EA8
static signed char init; // size: 0x1, address: 0x80C07399
static struct tweak_callback cb_curve; // size: 0x28, address: 0x80417ED0
static signed char init; // size: 0x1, address: 0x80C0739A
static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFCE28
...
// Range: 0x80068D58 -> 0x8006930C
// this: r31
void xFXRibbon::add_tweaks(const char * debug_name /* r27 */, const char * tweak_prefix /* r28 */, void (* tweaked)(struct xFXRibbon &, void *) /* r29 */, void * context /* r30 */) {
    // Local variables
    const char * prefix; // r30
    char _buffer[128]; // r1+0x500
    char _buffer[128]; // r1+0x480
    char _buffer[128]; // r1+0x400
    char _buffer[128]; // r1+0x380
    char _buffer[128]; // r1+0x300
    char _buffer[128]; // r1+0x280
    char _buffer[128]; // r1+0x200
    char _buffer[128]; // r1+0x180
    char _buffer[128]; // r1+0x100
    char _buffer[128]; // r1+0x80
...
// Range: 0x8006930C -> 0x800693FC
// this: r31
void xFXRibbon::debug_init() {}

// Range: 0x800693FC -> 0x8006967C
// this: r31
void xFXRibbon::debug_update(float dt /* f31 */) {
    // Local variables
    float frac; // f4
    float ang; // f29
    float radius; // f30
...
// Range: 0x8006976C -> 0x80069830
// this: r31
void xFXRibbon::debug_update_curve() {}

static struct tweak_callback cb_spiral; // size: 0x28, address: 0x80417EF8
static signed char init; // size: 0x1, address: 0x80C0739D
static struct tweak_callback cb_activate; // size: 0x28, address: 0x80417F20
static signed char init; // size: 0x1, address: 0x80C0739E
// Range: 0x80069830 -> 0x80069C64
// this: r31
...
static struct tweak_callback cb_node_time; // size: 0x28, address: 0x80417F48
static signed char init; // size: 0x1, address: 0x80C0739F
static struct tweak_callback cb_node_value; // size: 0x28, address: 0x80417F70
static signed char init; // size: 0x1, address: 0x80C073A0
static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFCE34
// Range: 0x80069C64 -> 0x80069F70
// this: r27
...
// Range: 0x8006A114 -> 0x8006A178
void xFXRibbon::on_start_spiral(const struct tweak_info & info /* r0 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// Range: 0x8006A178 -> 0x8006A1C4
void xFXRibbon::on_activate_spiral(const struct tweak_info & info /* r0 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// Range: 0x8006A1C4 -> 0x8006A21C
void xFXRibbon::on_change_curve(const struct tweak_info & info /* r0 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// Range: 0x8006A21C -> 0x8006A270
void xFXRibbon::on_change_config(const struct tweak_info & info /* r0 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// Range: 0x8006A270 -> 0x8006A368
void xFXRibbon::on_change_node_time(const struct tweak_info & info /* r3 */) {
    // Local variables
...
// Range: 0x8006A368 -> 0x8006A3A4
void xFXRibbon::on_change_node_value(const struct tweak_info & info /* r0 */) {
    // Local variables
    struct xFXRibbon & owner; // r3
}

// total size: 0xC
struct /* @class$975xFX_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$978xFX_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$979xFX_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$980xFX_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$981xFX_cpp */ {
    // Members
...
// Range: 0x8006A3A4 -> 0x8006A400
void xFXRibbon::on_change_texture(const struct tweak_info & info /* r0 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// Range: 0x8006A400 -> 0x8006A4B4
void xFXRibbon::scene_enter() {
    // Local variables
    struct activity_data * * act; // r30
    struct activity_data * * end_act; // r29
...
// Range: 0x8006A4B4 -> 0x8006A524
void xFXRibbon::update_all(float dt /* f31 */) {
    // Local variables
...
// Range: 0x8006A524 -> 0x8006A614
void xFXRibbon::render_all() {
    // Local variables
    struct RwRaster * raster; // r31
    unsigned int blend_src; // r30
    unsigned int blend_dst; // r29
    struct activity_data * * act; // r28
    struct activity_data * * end_act; // r27
    struct xFXRibbon * ribbon; // r26
