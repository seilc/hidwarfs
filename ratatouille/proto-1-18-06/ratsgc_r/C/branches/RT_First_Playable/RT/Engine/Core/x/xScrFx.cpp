/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xScrFx.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800B1BB4 -> 0x800B4CB4
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
unsigned char g_debugRenderSafeArea; // size: 0x1, address: 0x80C076D0
// Range: 0x800B1BB4 -> 0x800B1BE0
void xScrFxInit() {}

// Range: 0x800B1BE0 -> 0x800B1C00
void xScrFxReset() {}

// Range: 0x800B1C00 -> 0x800B1C3C
void xScrFxUpdate(float dt /* f31 */) {}

// Range: 0x800B1C3C -> 0x800B1C74
void xScrFxRender() {
    // References
...
// Range: 0x800B1C74 -> 0x800B1CF8
void xScrFxDrawScreenSizeRectangle() {
    // Local variables
    struct RwVideoMode video_mode; // r1+0x8
}
...
// Range: 0x800B1CF8 -> 0x800B1E64
void xScrFxDrawBox(float x1 /* f0 */, float y1 /* f0 */, float x2 /* f0 */, float y2 /* f0 */, unsigned char red /* r0 */, unsigned char green /* r0 */, unsigned char blue /* r0 */, unsigned char alpha /* r0 */, unsigned char clear_z_to_far /* r0 */) {
    // Local variables
    struct rwGameCube2DVertex v[4]; // r1+0x14
    unsigned short indices[4]; // r1+0xC
    float farz; // f0
    int old_z_write; // r1+0x8
    float nearz; // f0
...
struct _xFadeData {
    ...
    struct xColor_tag src; // offset 0x2, size 0x4
    struct xColor_tag dest; // offset 0x6, size 0x4
    float time_passed; // offset 0xC, size 0x4
    float time_total; // offset 0x10, size 0x4
    struct xColor_tag current_color; // offset 0x14, size 0x4
};
static struct _xFadeData mFade; // size: 0x18, address: 0x80C076D4
// Range: 0x800B1E64 -> 0x800B1E90
void xScrFxFadeInit() {
    // References
    // -> static struct _xFadeData mFade;
}

// Range: 0x800B1E90 -> 0x800B1EEC
static unsigned char InterpCol(float t /* f0 */, unsigned char s /* r0 */, unsigned char d /* r0 */) {
    // Local variables
    float val; // f3
}

// Range: 0x800B1EEC -> 0x800B1F90
void xScrFxFade(const struct xColor_tag * base /* r6 */, const struct xColor_tag * dest /* r30 */, float seconds /* f31 */, int hold /* r31 */) {
    // References
    // -> static struct _xFadeData mFade;
}

// Range: 0x800B1F90 -> 0x800B1F9C
void xScrFxStopFade() {
    // References
    // -> static struct _xFadeData mFade;
}
...
// Range: 0x800B1F9C -> 0x800B20C8
static void xScrFxFadeUpdate(float seconds /* f31 */) {
    // Local variables
    float percent; // f31
...
// Range: 0x800B20C8 -> 0x800B215C
static void xScrFxFadeRender() {
    // Local variables
    struct RwVideoMode video_mode; // r1+0x8
...
static float mLetterboxO; // size: 0x4, address: 0x80C076EC
static float mLetterboxTO; // size: 0x4, address: 0x80C076F0
static float sLetterBoxSize; // size: 0x4, address: 0x80C076F4
static unsigned char sLetterBoxAlpha; // size: 0x1, address: 0x80BFE4E0
// Range: 0x800B215C -> 0x800B216C
void xScrFxLetterboxReset() {
    // References
    // -> static float mLetterboxTO;
    // -> static float mLetterboxO;
}

unsigned int FB_YRES; // size: 0x4, address: 0x80C00BE0
// Range: 0x800B216C -> 0x800B21A4
void xScrFxLetterBoxSetSize(float size /* f0 */) {
    // References
    // -> unsigned int FB_YRES;
    // -> static float sLetterBoxSize;
}

// Range: 0x800B21A4 -> 0x800B21D4
float xScrFxLetterBoxGetSizeInScreenPercent() {
    // References
    // -> unsigned int FB_YRES;
    // -> static float sLetterBoxSize;
}

// Range: 0x800B21D4 -> 0x800B21DC
void xScrFxLetterBoxSetAlpha(unsigned char alpha /* r0 */) {
    // References
    // -> static unsigned char sLetterBoxAlpha;
}

// Range: 0x800B21DC -> 0x800B2258
void xScrFxLetterbox(int enable /* r31 */) {
    // References
    // -> static float mLetterboxTO;
    // -> static float mLetterboxO;
    // -> struct xGlobals * xglobals;
...
// Range: 0x800B2408 -> 0x800B2690
void xScrFxDrawSafeArea() {
    // Local variables
    struct RwVideoMode videoMode; // r1+0x8
    unsigned char bHiDef; // r3
    float fSafeArea; // f30
    unsigned int screenWidth; // r31
    unsigned int screenHeight; // r30
    unsigned int xMargin; // r29
    unsigned int yMargin; // r31
    int top; // r0
    int bottom; // r30
    int left; // r0
    int right; // r28
...
struct xGlare {
    ...
    struct RwRGBAReal col; // offset 0x20, size 0x10
    struct RwRaster * raster; // offset 0x30, size 0x4
};
struct xGlare sGlare[10]; // size: 0x208, address: 0x80A95780
// Range: 0x800B2690 -> 0x800B26C4
void xScrFXGlareInit() {
    // Local variables
    int i; // r0

    // References
    // -> struct xGlare sGlare[10];
}

// Range: 0x800B26C4 -> 0x800B26E4
void xScrFXGlareReset() {}

// Range: 0x800B26E4 -> 0x800B26FC
void xScrFXGlareDisable(int id /* r0 */) {
    // References
    // -> struct xGlare sGlare[10];
}

// Range: 0x800B26FC -> 0x800B27A0
void xScrFXGlareChange(int id /* r0 */, const struct xVec3 * pos /* r0 */, float life /* f30 */, float intensity /* f31 */) {
    // References
    // -> struct xGlare sGlare[10];
}

// Range: 0x800B27A0 -> 0x800B2844
void xScrFXGlareUpdate(float dt /* f0 */) {
    // Local variables
    int i; // r5
...
struct xVec3 sFullScreenGlareDir; // size: 0xC, address: 0x80BFE4E4
float sFullScreenGlareIntensity; // size: 0x4, address: 0x80BFE4F0
// total size: 0x4
struct RwRGBA {
    // Members
...
struct RwRGBA sFullScreenGlareColor; // size: 0x4, address: 0x80BFE4F4
int sFullScreenGlareEnabled; // size: 0x4, address: 0x80C076F8
unsigned int sFullScreenGlareTextureID; // size: 0x4, address: 0x80BFE4F8
struct RwTexture * sFullScreenGlareTexturePtr; // size: 0x4, address: 0x80C076FC
...
void xScrFXFullScreenGlareRender() {
    ...
    // -> struct RwTexture * sFullScreenGlareTexturePtr;
    // -> unsigned int sFullScreenGlareTextureID;
    // -> struct RwRGBA sFullScreenGlareColor;
    // -> float sFullScreenGlareIntensity;
    // -> struct xVec3 sFullScreenGlareDir;
    // -> struct xGlobals * xglobals;
...
static struct RxObjSpace3DVertex sStripVert[4]; // size: 0x90, address: 0x80A95988
// Range: 0x800B2A0C -> 0x800B2D9C
void xScrFXGlareRender(const struct xMat4x3 & mat /* r29 */) {
    // Local variables
    int i; // r30
    struct xVec3 w; // r1+0x38
    struct xVec3 h; // r1+0x2C
    struct xVec3 v; // r1+0x20
    struct xVec3 glareDir; // r1+0x14
    float dot; // f0
    float val; // f29
    float glareDirLen; // f0
    struct xVec3 toCam; // r1+0x8
...
static struct rwGameCube2DVertex v[4]; // size: 0x60, address: 0x80A95A18
static unsigned short indices[4]; // size: 0x8, address: 0x80BFE4FC
// Range: 0x800B2D9C -> 0x800B2E8C
void xScrFxDrawBox(float x1 /* f0 */, float y1 /* f0 */, float x2 /* f0 */, float y2 /* f0 */, unsigned char red /* r0 */, unsigned char green /* r0 */, unsigned char blue /* r0 */, unsigned char alpha /* r0 */, float ushift /* f0 */, float vshift /* f0 */) {
    // Local variables
    float oocameraNearClipPlane; // f8

    // References
    // -> static unsigned short indices[4];
    // -> static struct rwGameCube2DVertex v[4];
    // -> void * RwEngineInstance;
}

static struct xMat4x3 sCameraFXMatOld; // size: 0x40, address: 0x80A95A80
// Range: 0x800B2E8C -> 0x800B2ED0
void xCameraFXBegin(const struct xMat4x3 & mat /* r31 */) {
    // References
    // -> static struct xMat4x3 sCameraFXMatOld;
}

static unsigned char anycameraFXActive; // size: 0x1, address: 0x80C07700
// total size: 0x3C
struct cameraFXShake {
    // Members
    float magnitude; // offset 0x0, size 0x4
    struct xVec3 dir; // offset 0x4, size 0xC
    ...
    const struct xVec3 * epicenterP; // offset 0x28, size 0x4
    struct xVec3 epicenter; // offset 0x2C, size 0xC
    const struct xVec3 * player; // offset 0x38, size 0x4
};
// total size: 0x20
struct cameraFXZoom {
    // Members
...
struct cameraFX {
    ...
    union { // inferred
        struct cameraFXShake shake; // offset 0x10, size 0x3C
        struct cameraFXZoom zoom; // offset 0x10, size 0x20
    };
};
// total size: 0xC
struct cameraFXTableEntry {
    // Members
    int type; // offset 0x0, size 0x4
    void (* func)(struct cameraFX *, float, struct xMat4x3 *, struct xMat4x3 *); // offset 0x4, size 0x4
    void (* funcKill)(struct cameraFX *); // offset 0x8, size 0x4
};
struct cameraFXTableEntry sCameraFXTable[3]; // size: 0x24, address: 0x8042DFF8
struct cameraFX sCameraFX[10]; // size: 0x2F8, address: 0x80A95AC0
// Range: 0x800B2ED0 -> 0x800B2F34
static void xCameraFXInit() {
    // Local variables
    int i; // r0

    // References
    // -> struct cameraFX sCameraFX[10];
}

// Range: 0x800B2F34 -> 0x800B2FCC
void xCameraFXKill() {
    // Local variables
    int i; // r5
...
// Range: 0x800B2FCC -> 0x800B3010
struct cameraFX * xCameraFXAlloc() {
    // Local variables
    int i; // r0
...
// Range: 0x800B3010 -> 0x800B3144
void xCameraFXZoomUpdate(struct cameraFX * f /* r3 */, float dt /* f0 */, struct xMat4x3 * m /* r5 */) {}

static float SHAKE_FOREVER; // size: 0x4, address: 0x80C07704
// total size: 0x18
struct xCameraFXShakeParams {
    // Members
    float maxTime; // offset 0x0, size 0x4
    float magnitude; // offset 0x4, size 0x4
    float cycleMax; // offset 0x8, size 0x4
    float rotateMagnitude; // offset 0xC, size 0x4
    float radius; // offset 0x10, size 0x4
    unsigned char shakeY; // offset 0x14, size 0x1
};
// total size: 0x8
struct substr {
    // Members
...
// total size: 0x1
struct /* @class$1083xScrFx_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1084xScrFx_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1085xScrFx_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1086xScrFx_cpp */ {
    // Members
...
// Range: 0x800B3144 -> 0x800B3184
struct cameraFX * xCameraFXShakeForever(float magnitude /* f7 */, float cycleMax /* f6 */, float rotate_magnitude /* f0 */, float radius /* f5 */, const struct xVec3 * epicenter /* r0 */, const struct xVec3 * player /* r0 */, unsigned char shakeY /* r0 */) {
    // References
    // -> static float SHAKE_FOREVER;
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFE504
// Range: 0x800B3184 -> 0x800B324C
void xCameraFXShakeEnd(struct cameraFX * fx /* r31 */, float fadeOutTime /* f31 */) {
    // References
    // -> static char __FUNCTION__[18];
}

// Range: 0x800B324C -> 0x800B338C
struct cameraFX * xCameraFXShake(float maxTime /* f27 */, float magnitude /* f28 */, float cycleMax /* f29 */, float rotate_magnitude /* f30 */, float radius /* f31 */, const struct xVec3 * epicenter /* r28 */, const struct xVec3 * player /* r29 */, unsigned char shakeY /* r30 */) {
    // Local variables
    struct cameraFX * f; // r31
}

// Range: 0x800B338C -> 0x800B35FC
void xCameraFXShakeUpdate(struct cameraFX * f /* r30 */, float dt /* f29 */, struct xMat4x3 * m /* r31 */) {
    // Local variables
    float x; // f31
    float y; // f29
    float scale; // f30
...
// Range: 0x800B35FC -> 0x800B3750
void xCameraFXUpdate(struct xMat4x3 & mat /* r28 */, struct RwCamera * icam /* r29 */, float dt /* f31 */) {
    // Local variables
    int i; // r30

    // References
    // -> static struct xMat4x3 sCameraFXMatOld;
    // -> struct cameraFXTableEntry sCameraFXTable[3];
    // -> static unsigned char anycameraFXActive;
    // -> struct cameraFX sCameraFX[10];
}

// Range: 0x800B3750 -> 0x800B37B4
void xCameraFXEnd(struct xMat4x3 & mat /* r30 */, struct RwCamera * icam /* r31 */) {
    // References
    // -> static struct xMat4x3 sCameraFXMatOld;
}

// total size: 0x3C
struct xScrFxLensFlareElement : public xDynAsset {
    // Members
    unsigned int textureID; // offset 0x10, size 0x4
    struct RwTexture * texture; // offset 0x14, size 0x4
...
// total size: 0x30
struct xScrFxLensFlareSourceAsset : public xDynAsset {
    // Members
    unsigned int object; // offset 0x10, size 0x4
    unsigned int elementID; // offset 0x14, size 0x4
    struct xScrFxLensFlareElement * * elementArray; // offset 0x18, size 0x4
...
struct xScrFxLensFlareSource {
    ...
    struct xVec3 * at; // offset 0x8, size 0x4
    struct xScrFxLensFlareSourceAsset * asset; // offset 0xC, size 0x4
};
struct xScrFxLensFlareSource * sFlareSource; // size: 0x4, address: 0x80C07708
int sNumFlareSources; // size: 0x4, address: 0x80C0770C
// total size: 0x10
...
struct xScrFxLensFlareParams sDefaultLensFlareParams; // size: 0x10, address: 0x80BFE518
struct xScrFxLensFlareParams * sLensFlareParams; // size: 0x4, address: 0x80BFE528
// total size: 0x30
...
struct iScrFxLensFlare sFlare[128]; // size: 0x1800, address: 0x80A95DB8
int sNumFlares; // size: 0x4, address: 0x80C07710
struct RwTexture * gxFlareTexture; // size: 0x4, address: 0x80C07714
struct RwFrame * gxFlareFrame; // size: 0x4, address: 0x80C07718
// Range: 0x800B37B4 -> 0x800B3978
static int ObjRecurse(unsigned int assetID /* r26 */, void (* callback)(void *, unsigned int, void *) /* r30 */, void * data /* r31 */) {
    // Local variables
    int count; // r28
    struct xBase * base; // r4
    struct xGroup * group; // r27
    unsigned int numItems; // r0
    unsigned int i; // r26
    unsigned int memberID; // r0
    unsigned int size; // r1+0x8
    void * obj; // r26
    struct xDynAsset * da; // r0
}
...
// Range: 0x800B3978 -> 0x800B399C
static void LensFlareElementCB(void * obj /* r0 */, void * data /* r0 */) {
    // Local variables
    struct xScrFxLensFlareSourceAsset * asset; // r5
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFE52C
...
// Range: 0x800B399C -> 0x800B3D40
static void LensFlareSourceCB(void * obj /* r26 */, unsigned int type /* r25 */, void * data /* r24 */) {
    // Local variables
    struct xScrFxLensFlareSource * source; // r30
    unsigned int elementID; // r28
    int numElements; // r0
    unsigned int texID; // r29
    int i; // r28
    struct xScrFxLensFlareElement * element; // r27
    int i; // r27
    struct xScrFxLensFlareElement * element; // r29
    struct RwTexture * tex; // r31
    struct RwRaster * raster; // r29
    int width; // r26
    int height; // r25
    struct RwRaster * newRaster; // r25
    struct RwImage * image; // r24
    int format; // r7

    // References
    // -> static char __FUNCTION__[18];
    // -> unsigned int gActiveHeap;
    // -> int sNumFlareSources;
    // -> struct xScrFxLensFlareSource * sFlareSource;
}

static char __FUNCTION__[27]; // size: 0x1B, address: 0x8042E01C
// total size: 0x1
struct iAsyncLockRWSentry {};
// Range: 0x800B3D40 -> 0x800B400C
void xScrFxLensFlare_SceneEnter() {
    // Local variables
    struct iAsyncLockRWSentry rw_sentry; // r1+0x8
    int numDyns; // r0
    int i; // r27
    struct xDynAsset * dyn; // r0
    int numSources; // r30
    int i; // r26
    struct xDynAsset * dyn; // r0
    int i; // r26
    struct xDynAsset * dyn; // r5
    ...
    // -> unsigned int gActiveHeap;
    // -> struct xScrFxLensFlareSource * sFlareSource;
    // -> struct xScrFxLensFlareParams * sLensFlareParams;
    // -> struct xScrFxLensFlareParams sDefaultLensFlareParams;
...
// Range: 0x800B400C -> 0x800B4048
void xScrFxLensFlare_SceneExit() {
    // References
    // -> struct RwFrame * gxFlareFrame;
}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFE540
// Range: 0x800B4048 -> 0x800B4148
static void xScrFxLensFlare_Add(const struct xVec3 * pos /* r31 */, const struct xVec3 * at /* r29 */, float intensity /* f31 */, struct xScrFxLensFlareSource * source /* r30 */) {
    // Local variables
    struct iScrFxLensFlare * flare; // r31

    // References
    // -> int sNumFlares;
    // -> struct iScrFxLensFlare sFlare[128];
    // -> static char __FUNCTION__[20];
}

// Range: 0x800B4148 -> 0x800B417C
static void xScrFxLensFlare_Add(struct xScrFxLensFlareSource * source /* r5 */) {}

// Range: 0x800B417C -> 0x800B41DC
void xScrFxLensFlare_Reset() {
    // Local variables
    int i; // r30
...
static unsigned int override_texture; // size: 0x4, address: 0x80C0771C
static signed char init; // size: 0x1, address: 0x80C07720
static unsigned char _temp_tweaks_registered; // size: 0x1, address: 0x80C07721
static signed char init; // size: 0x1, address: 0x80C07722
...
// Range: 0x800B41DC -> 0x800B4890
static void xScrFxLensFlare_RenderElements(struct RwFrame * frame /* r31 */, unsigned char vCol /* r30 */, void * flr /* r28 */) {
    // Local variables
    struct RxObjSpace3DVertex verts[4]; // r1+0x128
    unsigned short indices[4]; // r1+0x10
    struct RxObjSpace3DVertex qverts[9]; // r1+0x1B8
    unsigned short qindices[10]; // r1+0x54
    struct RwCamera * camera; // r27
    const struct RwV2d * vw; // r0
    struct RwMatrixTag invMtx; // r1+0xE8
    struct RwMatrixTag transform; // r1+0xA8
    struct RwMatrixTag tmp; // r1+0x68
    struct RwMatrixTag * matrix; // r29
    struct RwV3d v; // r1+0x48
    float length; // f0
    struct RwV3d position; // r1+0x3C
    struct RwV3d FlarePosition; // r1+0x30
    const char * prefix; // r3
    struct xScrFxLensFlareElement * ep; // r6
    struct iScrFxLensFlare * flare; // r0
    struct xScrFxLensFlareSource * source; // r28
    int i; // r27
    unsigned char r; // r9
    unsigned char g; // r8
    unsigned char b; // r7
    struct RwV3d flareSize; // r1+0x24
    float uadj; // f5
    float vadj; // f6
    unsigned char r; // r5
    unsigned char g; // r4
    unsigned char b; // r3
    int j; // r0
    struct RwV3d flareSize; // r1+0x18

    // References
    // -> struct RwTexture * gxFlareTexture;
    // -> static unsigned int override_texture;
...
// Range: 0x800B4890 -> 0x800B4988
static void dumpFlare(struct iScrFxLensFlare * flare /* r31 */) {
    // References
    // -> struct xScrFxLensFlareParams * sLensFlareParams;
}

int gxLensFlareDebug; // size: 0x4, address: 0x80C07724
int dumpFlares; // size: 0x4, address: 0x80C07728
struct RwCamera * globalCamera; // size: 0x4, address: 0x80C08D88
// Range: 0x800B4988 -> 0x800B4CB4
void xScrFxLensFlare_Render(struct RwCamera * cam /* r26 */) {
    // Local variables
    struct iScrFxLensFlare * flare; // r27
    struct RwMatrixTag * cammat; // r0
    struct xVec3 * campos; // r29
    struct xVec3 * cam_at; // r28
    int i; // r30
    int i; // r30
    int i; // r30
    int i; // r27
    struct xScrFxLensFlareSource * source; // r25
    float intensity; // f30
