/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xParticleBatch.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801840C0 -> 0x80186880
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
struct block_info {
    ...
    struct block_info * next; // offset 0x8, size 0x4
};
// total size: 0x48
struct internal_system : public xParticleBatchSystem {
    // Members
    struct block_info * block_list; // offset 0x28, size 0x4
    int blocks_used; // offset 0x2C, size 0x4
    int units_used; // offset 0x30, size 0x4
    int units_per_block; // offset 0x34, size 0x4
    struct block_info * best_block; // offset 0x38, size 0x4
    struct internal_system * next; // offset 0x3C, size 0x4
    const char * dbgFile; // offset 0x40, size 0x4
    int dbgLine; // offset 0x44, size 0x4
};
static struct internal_system systems[128]; // size: 0x2400, address: 0x80BAC148
static struct system_bucket buckets[6][128]; // size: 0x3C00, address: 0x80BAE548
static struct block_info * head_free_block; // size: 0x4, address: 0x80C08780
static int free_block_count; // size: 0x4, address: 0x80C08784
static unsigned char * block_buffer; // size: 0x4, address: 0x80C08788
static struct block_info block_info_buffer[32]; // size: 0x180, address: 0x80BB2148
static int systems_size; // size: 0x4, address: 0x80C0878C
static int buckets_size[6]; // size: 0x18, address: 0x80C08790
static char __FUNCTION__[12]; // size: 0xC, address: 0x80C01E10
// Range: 0x801840C4 -> 0x80184148
static struct block_info * alloc_block() {
    // Local variables
    struct block_info * b; // r0

    // References
    // -> static int free_block_count;
    // -> static struct block_info * head_free_block;
    // -> static char __FUNCTION__[12];
}

// Range: 0x80184148 -> 0x80184164
static void free_block(struct block_info * b /* r0 */) {
    // References
    // -> static int free_block_count;
    // -> static struct block_info * head_free_block;
}

// Range: 0x80184164 -> 0x801841C8
static void create_system(struct internal_system & isys /* r31 */, const struct xParticleBatchSystem & sys /* r0 */) {}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80C01E1C
// total size: 0x14
struct system_bucket {
    // Members
    struct RwTexture * texture; // offset 0x0, size 0x4
    union { // inferred
        int rskey; // offset 0x4, size 0x4
        struct { // inferred
    ...
    int order_index; // offset 0xC, size 0x4
    struct internal_system * systems; // offset 0x10, size 0x4
};
// Range: 0x8018421C -> 0x801843BC
static void add_to_bucket(struct internal_system & isys /* r27 */) {
    // Local variables
    int & bsize; // r30
    struct system_bucket * const begin_bucket; // r29
    struct system_bucket * const end_bucket; // r28
    struct system_bucket key; // r1+0x10
    struct system_bucket * bucket; // r26

    // References
    // -> static char __FUNCTION__[14];
    // -> static struct system_bucket buckets[6][128];
    // -> static int buckets_size[6];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01E2C
// Range: 0x801843E8 -> 0x80184564
static void remove_from_bucket(struct internal_system & isys /* r28 */) {
    // Local variables
    int & bsize; // r30
    struct system_bucket * const begin_bucket; // r3
    struct system_bucket * const end_bucket; // r29
    struct system_bucket key; // r1+0xC
    struct system_bucket * bucket; // r26
    struct internal_system * it; // r25
    struct internal_system * * itp; // r4
...
// Range: 0x80184564 -> 0x801845C4
static void clear_system(struct internal_system & system /* r30 */) {
    // Local variables
    struct block_info * block; // r3
    struct block_info * next_block; // r31
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80C01E40
static char __FUNCTION__[13]; // size: 0xD, address: 0x80C01E50
// Range: 0x801845C4 -> 0x80184730
static void merge_blocks(struct internal_system & system /* r30 */) {
    // Local variables
    struct block_info * first; // r4
    struct block_info * second; // r5
    struct block_info * s0; // r29
    struct block_info * s1; // r28
    struct block_info * * ps0; // r31
    struct block_info * b; // r4
    struct block_info * * pb; // r5

    // References
    // -> static char __FUNCTION__[13];
}

// Range: 0x80184730 -> 0x801847C0
static void refresh_blocks(struct internal_system & system /* r30 */) {
    // Local variables
    struct block_info * b; // r3
    struct block_info * * pb; // r31
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80C01E60
// Range: 0x801847C0 -> 0x80184B20
static void validate_system(const struct internal_system & isys /* r31 */) {
    // Local variables
    int blocks_used; // r30
    int units_used; // r29
    const struct block_info * b; // r28

    // References
    // -> static char __FUNCTION__[16];
}

static char __FUNCTION__[25]; // size: 0x19, address: 0x8043CFB8
unsigned int gActiveHeap; // size: 0x4, address: 0x80C07448
// Range: 0x80184B20 -> 0x80184BCC
void xParticleBatchSceneEnter() {
    // References
    // -> static int buckets_size[6];
    // -> static int systems_size;
    // -> static char __FUNCTION__[25];
    // -> static unsigned char * block_buffer;
    // -> unsigned int gActiveHeap;
}

// Range: 0x80184BCC -> 0x80184BD0
static void adjust_batch_size() {}

// Range: 0x80184BD0 -> 0x80184BD4
void xParticleBatchSceneExit() {}

// Range: 0x80184BD4 -> 0x80184C58
void xParticleBatchReset() {
    // Local variables
    struct internal_system * sys; // r4
    struct internal_system * end_sys; // r3
    struct block_info * b; // r3
    struct block_info * endb; // r4
    unsigned char * mem; // r5

    // References
    // -> static unsigned char * block_buffer;
    // -> static struct block_info block_info_buffer[32];
    // -> static int free_block_count;
    // -> static struct block_info * head_free_block;
    // -> static int systems_size;
    // -> static struct internal_system systems[128];
}
...
// total size: 0x4
struct /* @class$1350xParticleBatch_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1488xParticleBatch_cpp */ {
    // Members
...
union /* @class$1015xParticleBatch_cpp */ {
...
// total size: 0x10
struct /* @class$1028xParticleBatch_cpp */ {
    // Members
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x80C01E70
// Range: 0x80184D70 -> 0x80184E4C
int xParticleBatchAddSystem(const struct xParticleBatchSystem & system /* r30 */, const char * dbgFile /* r28 */, int dbgLine /* r29 */) {
    // Local variables
    int system_id; // r31
    struct internal_system & isys; // r30
...
static char __FUNCTION__[28]; // size: 0x1C, address: 0x8043CFD4
// Range: 0x80184E4C -> 0x80184F34
void xParticleBatchReplaceSystem(int system_id /* r31 */, const struct xParticleBatchSystem & system /* r30 */) {
    // Local variables
    struct internal_system & isys; // r31
...
float SCALE_FREE_PERCENT; // size: 0x4, address: 0x80C01E88
float SCALE_SYSTEM_FREE_PERCENT; // size: 0x4, address: 0x80C01E8C
static char __FUNCTION__[22]; // size: 0x16, address: 0x80C01E90
// Range: 0x80184F34 -> 0x80185120
float xParticleBatchRequest(int system_id /* r30 */, float amount /* f31 */, int max_blocks /* r31 */) {
    // Local variables
    struct internal_system & s; // r4
    float newBlockUsage; // f4
    float systemFreePercent; // f1
    float free_percent; // f0
    float newAmount; // f1

    // References
    // -> float SCALE_FREE_PERCENT;
    // -> static int free_block_count;
    // -> float SCALE_SYSTEM_FREE_PERCENT;
    // -> static struct internal_system systems[128];
    // -> static char __FUNCTION__[22];
    // -> static int systems_size;
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01EA8
// Range: 0x80185120 -> 0x80185278
int xParticleBatchEmit(int system_id /* r29 */, int amount /* r30 */, unsigned char * * mem /* r31 */) {
    // Local variables
    struct internal_system & isys; // r29
    struct block_info * block; // r0
    int remain; // r5
...
// Range: 0x80185278 -> 0x8018539C
static struct block_info * grab_best_block(struct internal_system & isys /* r31 */) {
    // Local variables
    int most_used; // r4
    struct block_info * b; // r5
...
class xParticleBatchGroup {
    ...
    int create_system(const struct xParticleBatchSystem & system, const char * dbgFile, int dbgLine);

    void add_tweaks(int * system_id, const char * prefix);

    // Members
    unsigned char * elements; // offset 0x0, size 0x4
    int elements_size; // offset 0x4, size 0x4
};
// Range: 0x8018539C -> 0x801853E4
// this: r31
void xParticleBatchGroup::create() {}

static char __FUNCTION__[5]; // size: 0x5, address: 0x80C01EBC
static char __FUNCTION__[31]; // size: 0x1F, address: 0x8043CFF0
// Range: 0x801853E4 -> 0x80185490
static int compare_group_systems_function(void * e1 /* r0 */, void * e2 /* r0 */) {
    // Local variables
    int sysid1; // r31
    int sysid2; // r30
...
// Range: 0x80185490 -> 0x80185580
static int compare_systems(const struct xParticleBatchSystem & sys1 /* r0 */, const struct xParticleBatchSystem & sys2 /* r0 */) {}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80C01EC4
// Range: 0x80185580 -> 0x80185684
// this: r31
int xParticleBatchGroup::create_system(const struct xParticleBatchSystem & system /* r27 */, const char * dbgFile /* r28 */, int dbgLine /* r29 */) {
    // Local variables
    const unsigned char * end_elements; // r30
    const unsigned char * found; // r0
    int system_id; // r30
...
static struct tweak_callback cb_reload; // size: 0x28, address: 0x803E1D20
static char __FUNCTION__[11]; // size: 0xB, address: 0x80C01ED4
...
// Range: 0x80185684 -> 0x8018590C
// this: r26
void xParticleBatchGroup::add_tweaks(int * system_id /* r27 */, const char * prefix /* r31 */) {
    // Local variables
    struct xParticleBatchSystem & isys; // r29
...
enum /* @enum$1151xParticleBatch_cpp */ {
    FLAG_NOZTEST = 1,
    FLAG_ZWRITE = 2,
    FLAG_ORDERED = 4,
    FLAG_BRIGHT = 8,
...
struct reload_context {
    // Functions
    static reload_context(class xParticleBatchGroup * group, int * system_id, unsigned char privatized);

    // Members
    class xParticleBatchGroup * group; // offset 0x0, size 0x4
    int * system_id; // offset 0x4, size 0x4
    unsigned char privatized; // offset 0x8, size 0x1
    unsigned int texture; // offset 0xC, size 0x4
    unsigned int blend_src; // offset 0x10, size 0x4
    unsigned int blend_dst; // offset 0x14, size 0x4
    int flags; // offset 0x18, size 0x4
};
// Range: 0x8018590C -> 0x8018591C
// this: r0
static reload_context::reload_context(class xParticleBatchGroup * group /* r0 */, int * system_id /* r0 */, unsigned char privatized /* r0 */) {}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80C01EE0
// total size: 0x8
struct substr {
    // Members
...
// total size: 0xC
struct /* @class$964xParticleBatch_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$965xParticleBatch_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$966xParticleBatch_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$967xParticleBatch_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$968xParticleBatch_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$969xParticleBatch_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$970xParticleBatch_cpp */ {
    // Members
...
// Range: 0x8018591C -> 0x80185B04
void xParticleBatchGroup::on_reload(const struct tweak_info & info /* r0 */) {
    // Local variables
    struct reload_context & context; // r31
    int * system_id; // r30
    const struct xParticleBatchSystem & isys; // r29
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x80C01EEC
// Range: 0x80185B04 -> 0x80185CCC
void xParticleBatchDebugCB() {
    // Local variables
    int numFreeBlocks; // r31
    struct block_info * block; // r3
    float free_percent; // f1
    int i; // r30
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01F04
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01F18
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01F2C
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01F40
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01F54
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01F68
// total size: 0x1
struct compare_buckets {
    // Functions
    static int operator()(void * e1, void * e2) const;
};
// Range: 0x80185CCC -> 0x80185D54
static int compare_buckets::operator()(const struct system_bucket & b1 /* r0 */, const struct system_bucket & b2 /* r0 */) {
    // Local variables
    int ordered1; // r0
}

// total size: 0x1
struct compare_group_systems_functor {};
// Range: 0x80185D54 -> 0x80185DF8
static int compare_group_systems_functor::operator()(const struct xParticleBatchSystem & system /* r30 */, const unsigned char & system_id /* r31 */) {
    // References
    // -> static struct internal_system systems[128];
...
// Range: 0x80185DF8 -> 0x80185E18
// this: r0
static int compare_buckets::operator()(void * e1 /* r0 */, void * e2 /* r0 */) const {}
...
// Range: 0x80185E18 -> 0x80185FD4
static void pre_render_buckets(struct system_bucket * first /* r3 */, struct system_bucket * last /* r26 */, float dt /* f31 */) {
    // Local variables
    struct ptank_pool__color_mat pool; // r1+0x8
    struct system_bucket * bucket; // r31
    struct internal_system * system; // r30
    unsigned char started; // r29
    unsigned char lost_any; // r28
    struct block_info * block; // r27
    int newused; // r0
...
// Range: 0x80185FD4 -> 0x80186190
static void pre_render_buckets(struct system_bucket * first /* r3 */, struct system_bucket * last /* r26 */, float dt /* f31 */) {
    // Local variables
    struct ptank_pool__color_mat_uv2 pool; // r1+0x8
    struct system_bucket * bucket; // r31
    struct internal_system * system; // r30
    unsigned char started; // r29
    unsigned char lost_any; // r28
...
// Range: 0x8018634C -> 0x80186508
static void pre_render_buckets(struct system_bucket * first /* r3 */, struct system_bucket * last /* r26 */, float dt /* f31 */) {
    // Local variables
    struct ptank_pool__pos_color_size_uv2 pool; // r1+0x8
    struct system_bucket * bucket; // r31
    struct internal_system * system; // r30
    unsigned char started; // r29
    unsigned char lost_any; // r28
...
// Range: 0x80186508 -> 0x801866C4
static void pre_render_buckets(struct system_bucket * first /* r3 */, struct system_bucket * last /* r26 */, float dt /* f31 */) {
    // Local variables
    struct ptank_pool__pos_color_size_rot pool; // r1+0x8
    struct system_bucket * bucket; // r31
    struct internal_system * system; // r30
    unsigned char started; // r29
    unsigned char lost_any; // r28
...
// Range: 0x801866C4 -> 0x80186880
static void pre_render_buckets(struct system_bucket * first /* r3 */, struct system_bucket * last /* r26 */, float dt /* f31 */) {
    // Local variables
    struct ptank_pool__pos_color_size_rot_uv2 pool; // r1+0x8
    struct system_bucket * bucket; // r31
    struct internal_system * system; // r30
    unsigned char started; // r29
    unsigned char lost_any; // r28
