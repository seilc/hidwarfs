/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\gc\iWater.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801D6478 -> 0x801D7E60
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
struct RpLight * iWaterDirectionalLight; // size: 0x4, address: 0x80C08B8C
struct FxParam * iWaterEMBM; // size: 0x4, address: 0x80C08B90
// total size: 0x20
struct xLinkAsset {
    // Members
...
struct xEnt * waterObjects[8]; // size: 0x20, address: 0x80BB6B78
unsigned int numWaterObjects; // size: 0x4, address: 0x80C08B94
struct xEnt * water2; // size: 0x4, address: 0x80C08B98
struct xEnt * water3; // size: 0x4, address: 0x80C08B9C
struct xEnt * water4; // size: 0x4, address: 0x80C08BA0
struct xEnt * water5; // size: 0x4, address: 0x80C08BA4
struct xEnt * water7; // size: 0x4, address: 0x80C08BA8
float frameCountScale; // size: 0x4, address: 0x80C03180
int bump_mode; // size: 0x4, address: 0x80C03184
unsigned char dff_alpha; // size: 0x1, address: 0x80C03188
int env_clamp; // size: 0x4, address: 0x80C08BAC
int override_width; // size: 0x4, address: 0x80C0318C
int water_frame; // size: 0x4, address: 0x80C03190
float yscale; // size: 0x4, address: 0x80C03194
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
struct xVec3 camscale; // size: 0xC, address: 0x80C03198
static struct RwTexture * saveBump; // size: 0x4, address: 0x80C08BB0
// total size: 0x18
struct bump_layer {
    // Members
    float u; // offset 0x0, size 0x4
...
struct bump_layer bump_layers[4]; // size: 0x60, address: 0x8043E530
int num_bump_layers; // size: 0x4, address: 0x80C031A4
static struct RwTexture * bumpTex; // size: 0x4, address: 0x80C08BB4
static struct RwRaster * bumpRaster; // size: 0x4, address: 0x80C08BB8
// total size: 0x10
struct RwRGBAReal {
    // Members
...
static struct RwCamera * saveCamera; // size: 0x4, address: 0x80C08BBC
static struct RwCamera * bumpCamera; // size: 0x4, address: 0x80C08BC0
static struct RwCamera * clonedShadowCamera; // size: 0x4, address: 0x80C08BC4
...
// Range: 0x801D6500 -> 0x801D6678
static void iWaterBumpGenRenderLayer(struct RwCamera * camera /* r30 */, struct RwTexture * texture /* r4 */, float u /* f29 */, float v /* f30 */, unsigned char alpha /* r31 */, float scale /* f31 */) {
    // Local variables
    struct rwGameCube2DVertex vx[4]; // r1+0x8
    float crw; // f2
    float crh; // f3
    struct RwRaster * camRas; // r3

    // References
    // -> void * RwEngineInstance;
}

// Range: 0x801D6678 -> 0x801D6700
...
static float foo; // size: 0x4, address: 0x80C08BC8
...
// Range: 0x801D6760 -> 0x801D68F4
static struct RwTexture * iWaterBumpGen(struct RwTexture * bump /* r30 */, struct RwTexture * src /* r31 */) {
    // Local variables
    unsigned char genNeeded; // r28
    int i; // r27
    struct xModelInstance * modelInst; // r3
    struct RwRGBA black; // r1+0x8
    struct bump_layer * bp; // r27
    int i; // r28
...
struct RwTexture * iWaterBumpTex; // size: 0x4, address: 0x80C08C38
// Range: 0x801D68F4 -> 0x801D6928
void iWaterBumpGen() {
    // References
...
// Range: 0x801D6928 -> 0x801D6954
void iWaterCreateMTEffect() {
    // References
    // -> int gGCwaterDisable;
}

static struct xLightKit * waterLightKit; // size: 0x4, address: 0x80C08BCC
static struct RwRGBAReal waterHackColor; // size: 0x10, address: 0x80C031A8
// Range: 0x801D6954 -> 0x801D69EC
...
float bw_factor; // size: 0x4, address: 0x80C031B8
static struct RpAtomic * (* defaultRenderCB)(struct RpAtomic *); // size: 0x4, address: 0x80C08BD0
// Range: 0x801D69EC -> 0x801D6A14
static struct RpAtomic * iWaterAtomicRender(struct RpAtomic * atomic /* r0 */) {
    // References
    // -> static struct RpAtomic * (* defaultRenderCB)(struct RpAtomic *);
}

unsigned int waterTexID; // size: 0x4, address: 0x80C031BC
// total size: 0x28
struct PKRAssetType {
    // Members
...
float BumpWidth; // size: 0x4, address: 0x80C031CC
float BumpHeight; // size: 0x4, address: 0x80C031C8
float Shininess; // size: 0x4, address: 0x80C031D0
int EnvMode; // size: 0x4, address: 0x80C08C0C
// total size: 0x0
struct asset_type {};
// total size: 0x0
struct debug_info {};
...
// total size: 0x1
struct /* @class$967iWater_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$968iWater_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$969iWater_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$970iWater_cpp */ {
    // Members
...
// Range: 0x801D6AD4 -> 0x801D71C4
void iWaterSceneEnter() {
    // Local variables
    struct zScene & scene; // r4
    class zWaterBody * it; // r30
    class zWaterBody * end; // r29
    unsigned int body; // r26
    struct zEntSimpleObj * ent; // r0
    int i; // r26
    struct xEnt * ent; // r28
    struct RwMatrixTag newMat; // r1+0x88
    char pfx[64]; // r1+0x48
    char str[64]; // r1+0x8
    struct bump_layer * bp; // r26
    int i; // r27
...
static unsigned int override_bump; // size: 0x4, address: 0x80C08BD4
static signed char init; // size: 0x1, address: 0x80C08BD8
static unsigned int override_env; // size: 0x4, address: 0x80C08BDC
static signed char init; // size: 0x1, address: 0x80C08BE0
static float uvscale; // size: 0x4, address: 0x80C08BE4
static signed char init; // size: 0x1, address: 0x80C08BE8
static float range_scale; // size: 0x4, address: 0x80C08BEC
static signed char init; // size: 0x1, address: 0x80C08BF0
static unsigned char render_texture; // size: 0x1, address: 0x80C08BF1
static signed char init; // size: 0x1, address: 0x80C08BF2
static unsigned char _temp_tweaks_registered; // size: 0x1, address: 0x80C08BF3
static signed char init; // size: 0x1, address: 0x80C08BF4
struct RwTexture * iWaterEnvTex; // size: 0x4, address: 0x80C08C3C
...
struct FxParam EMBM; // size: 0x14, address: 0x80C08BF8
struct RwFrame * FxFrameArray[2]; // size: 0x8, address: 0x80C031D8
...
// Range: 0x801D7270 -> 0x801D78DC
static int iWaterUpdateEMBM(int check /* r0 */) {
    // Local variables
    struct RpMultiTexture * mt; // r28
    struct RpGameCubeMTConfig * config; // r0
    float scale; // f0
    struct RwTexture * bump; // r30
    struct RwTexture * env; // r29
...
unsigned int enableWater; // size: 0x4, address: 0x80C031C0
// Range: 0x801D78DC -> 0x801D7924
void iWaterEnable(struct RpAtomic * atomic /* r0 */) {
    // References
...
// Range: 0x801D7BE8 -> 0x801D7D14
void iWaterReset() {
    // Local variables
    int i; // r28
    struct zEntSimpleObj * ent; // r25
