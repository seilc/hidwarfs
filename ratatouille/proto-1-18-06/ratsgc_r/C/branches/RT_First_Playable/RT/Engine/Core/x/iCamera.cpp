/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\iCamera.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801F38B8 -> 0x801F4AA8
*/
float sCameraNearClip; // size: 0x4, address: 0x80C03928
float sCameraFarClip; // size: 0x4, address: 0x80C0392C
// total size: 0x8
struct RwObject {
    // Members
...
static struct RwCamera * sMainGameCamera; // size: 0x4, address: 0x80C08D8C
static float tweakFarClip; // size: 0x4, address: 0x80C08D90
void * RwEngineInstance; // size: 0x4, address: 0x80C09F34
// Range: 0x801F38B8 -> 0x801F3934
void iCameraSetOrthoProjection(struct RwCamera * cam /* r29 */, unsigned char ortho /* r30 */) {
    // Local variables
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80C03930
// total size: 0x1
...
// Range: 0x801F3934 -> 0x801F3B98
struct RwCamera * iCameraCreate(int width /* r28 */, int height /* r29 */, int mainGameCamera /* r30 */) {
    // Local variables
    struct iAsyncLockRWSentry rw_sentry; // r1+0x8
    struct RwV2d vw; // r1+0xC
    struct RwCamera * camera; // r31
    struct RwFrame * cameraFrame; // r27
    struct RwRaster * cameraRaster; // r0
    struct RwRaster * zbuffer; // r0
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80C03940
...
// Range: 0x801F3B98 -> 0x801F3CD0
void iCameraDestroy(struct RwCamera * camera /* r30 */) {
    // Local variables
    struct iAsyncLockRWSentry rw_sentry; // r1+0x8
    struct RpWorld * pWorld; // r0
...
float iCameraClearTime; // size: 0x4, address: 0x80C08D94
float iCameraClearTimeLast; // size: 0x4, address: 0x80C08D98
// total size: 0x4
struct RwRGBA {
    // Members
...
static struct RwRGBA hotPink; // size: 0x4, address: 0x80C03950
static char __FUNCTION__[13]; // size: 0xD, address: 0x80C03954
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x801F3CD0 -> 0x801F3DC8
void iCameraBegin(struct RwCamera * cam /* r29 */) {
    // Local variables
    long long timeA; // r31
    long long timeB; // r5
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80C03964
// Range: 0x801F3DC8 -> 0x801F3E4C
void iCameraEnd(struct RwCamera * cam /* r31 */) {
    // References
    // -> static char __FUNCTION__[11];
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80C03970
// Range: 0x801F3E4C -> 0x801F3ED0
void iCameraShowRaster(struct RwCamera * cam /* r31 */) {
    // References
    // -> static char __FUNCTION__[18];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80C03984
// Range: 0x801F3ED0 -> 0x801F40CC
void iCameraUpdatePos(struct RwCamera * cam /* r29 */, struct xMat4x3 * pos /* r31 */) {
    // Local variables
    struct RwFrame * f; // r29
...
// Range: 0x801F40CC -> 0x801F40D4
float iCameraGetAspectRatio() {}

// Range: 0x801F40D4 -> 0x801F4140
void iCameraSetFOV(struct RwCamera * cam /* r31 */, float fov /* f0 */) {
    // Local variables
    float view_window; // f0
    float fAspectRatio; // f0
    struct RwV2d vw; // r1+0x8
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80C03998
// Range: 0x801F4140 -> 0x801F4240
...
// Range: 0x801F4240 -> 0x801F42E4
void iCamGetViewMatrix(struct RwCamera * camera /* r30 */, struct xMat4x3 * view_matrix /* r31 */) {
    // Local variables
    struct RwMatrixTag * rw_view; // r0
}

// Range: 0x801F42E4 -> 0x801F4318
...
// Range: 0x801F4318 -> 0x801F432C
void iCameraGetNearFarClip(float & nearPlane /* r0 */, float & farPlane /* r0 */) {
    // References
    // -> float sCameraFarClip;
    // -> float sCameraNearClip;
}

// Range: 0x801F432C -> 0x801F4454
void iCameraSetFogParams(struct iFogParams * fp /* r31 */, float time /* f31 */) {
    // References
    // -> struct xGlobals * xglobals;
}

// Range: 0x801F4490 -> 0x801F487C
void iCameraUpdateFog(long long t /* r30 */) {
    // Local variables
    struct iFogParams * fp; // r0
    struct iFogParams * fa; // r0
    struct iFogParams * fb; // r0
    float a; // f2
...
// Range: 0x801F487C -> 0x801F4974
void iCameraSetFogRenderStates() {
    // Local variables
    struct RwCamera * pCamera; // r30
    struct iFogParams * pFogParams; // r0
...
// Range: 0x801F4990 -> 0x801F49B8
static void iCameraFogOffWrapper() {}

// Range: 0x801F49B8 -> 0x801F4A20
void iCameraSetFog(unsigned char enable /* r0 */) {
    // Local variables
    struct iFogParams * pFogParams; // r0
...
// Range: 0x801F4A20 -> 0x801F4A40
struct RwCamera * RwCameraBeginUpdateWrapper(struct RwCamera * camera /* r0 */) {
    // Local variables
    struct RwCamera * retval; // r0
}

// Range: 0x801F4A40 -> 0x801F4A44
...
// Range: 0x801F4A44 -> 0x801F4AA8
void iCameraPerformRenderWareHack(struct RwCamera * camera /* r30 */) {
    // Local variables
    struct RpWorldSector * * newFrustumSectors; // r0
    int cameraExtOffset; // r0
    struct RpWorldCameraExt * cameraExt; // r3
