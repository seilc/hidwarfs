/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xShadow.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800B61A4 -> 0x800BBA78
*/
float SHADOW_BIAS_AMT; // size: 0x4, address: 0x80BFE560
float SHADOW_MINNORMY; // size: 0x4, address: 0x80BFE564
unsigned int rrs; // size: 0x4, address: 0x80BFE568
unsigned int no_shadows; // size: 0x4, address: 0x80C07738
float SHADOW_EPISLON; // size: 0x4, address: 0x80BFE56C
// total size: 0x8
struct RwObject {
    // Members
...
struct RpLight * ShadowLight; // size: 0x4, address: 0x80C0773C
// total size: 0x10
struct RwRGBAReal {
    // Members
...
struct RwRGBAReal ShadowLightColor; // size: 0x10, address: 0x80BFE570
unsigned int sCacheSrcBlend; // size: 0x4, address: 0x80BFE580
unsigned int sCacheDestBlend; // size: 0x4, address: 0x80BFE584
unsigned char sDrawCameraRaster; // size: 0x1, address: 0x80C07740
static unsigned char drawFillCacheBox; // size: 0x1, address: 0x80C07741
static unsigned char drawCombinedAnimBounds; // size: 0x1, address: 0x80C07742
static unsigned char drawLinesToEnvContacts; // size: 0x1, address: 0x80C07743
static unsigned char drawShadowVolume; // size: 0x1, address: 0x80C07744
static int enableNPCShadowOneEnt; // size: 0x4, address: 0x80BFE588
static int enableNPCShadowAllEnt; // size: 0x4, address: 0x80C07748
static float DefaultShadowDepth; // size: 0x4, address: 0x80BFE58C
int SHADOW_CACHE_POLY_MAX; // size: 0x4, address: 0x80BFE590
char * sCacheBlendLabels[11]; // size: 0x2C, address: 0x8042E160
static unsigned int sCacheBlendValues[11]; // size: 0x2C, address: 0x803B9DC8
static unsigned int sCacheNumBlendModes; // size: 0x4, address: 0x80BFE594
static float SHADOW_BOTH; // size: 0x4, address: 0x80C0774C
// total size: 0x24
...
static struct RxObjSpace3DVertex * Im3DBuffer; // size: 0x4, address: 0x80C07750
static unsigned int Im3DBufferPos; // size: 0x4, address: 0x80C07754
...
struct RwCamera * ShadowCamera; // size: 0x4, address: 0x80C07758
struct RwRaster * ShadowCameraRaster; // size: 0x4, address: 0x80C0775C
static struct RwRaster * ShadowRenderRaster; // size: 0x4, address: 0x80C07760
unsigned char shadow_tri_depth_cull; // size: 0x1, address: 0x80BFE598
float shadow_max_tri_seperation; // size: 0x4, address: 0x80BFE59C
unsigned char shadow_tri_depth_culled; // size: 0x1, address: 0x80C07764
unsigned char shadow_tri_outline; // size: 0x1, address: 0x80C07765
unsigned char shadow_tri_reject; // size: 0x1, address: 0x80C07766
unsigned char shadow_tri_accept; // size: 0x1, address: 0x80C07767
unsigned char shadow_showcam; // size: 0x1, address: 0x80C07768
unsigned char shadow_preview; // size: 0x1, address: 0x80C07769
unsigned char shadow_zone; // size: 0x1, address: 0x80C0776A
unsigned char shadow_normal_flip; // size: 0x1, address: 0x80C0776B
static struct RwV2d ShadowPreviewVerts[2]; // size: 0x10, address: 0x80BFE5A0
// total size: 0x10
struct xSphere {
    // Members
...
struct xVec3 gShadowObjectRadius; // size: 0xC, address: 0x80C0776C
static unsigned int shadow_ent_count; // size: 0x4, address: 0x80C07778
// total size: 0x20
struct xLinkAsset {
    // Members
...
static int sShadowCollJSP; // size: 0x4, address: 0x80C0777C
static struct RwRaster * gc_saveraster; // size: 0x4, address: 0x80C07780
static char __FUNCTION__[12]; // size: 0xC, address: 0x80BFE5B0
// Range: 0x800B61A4 -> 0x800B6434
void xShadowInit() {
    // Local variables
...
// Range: 0x800B64C0 -> 0x800B64EC
void xShadowSetWorld(struct RpWorld * world /* r0 */) {
    // References
...
// Range: 0x800B64EC -> 0x800B657C
void xShadowSetLight(struct xVec3 * target_pos /* r30 */, struct xVec3 * in_vec /* r0 */) {
    // Local variables
    struct xVec3 zvec; // r1+0x10
    struct xMat4x3 matrix; // r1+0x20
...
// Range: 0x800B657C -> 0x800B65C0
int xShadowCameraCreate() {}

// Range: 0x800B65C0 -> 0x800B6620
void xShadowCameraUpdatePreRendered(void * model /* r31 */, void (* renderCB)(void *) /* r12 */, struct xVec3 * center /* r11 */, struct xVec3 * radius /* r10 */, int preRenderedSlot /* r0 */, int shadowMode /* r9 */) {
    // References
    // -> struct RwRaster * ShadowCameraRaster;
...
// Range: 0x800B6620 -> 0x800B6730
void xShadowCameraUpdate(void * model /* r25 */, void (* renderCB)(void *) /* r26 */, struct xVec3 * center /* r27 */, struct xVec3 * radius /* r28 */, int shadowMode /* r29 */) {
    // Local variables
    long long t1; // r0
    long long tt; // r0
    long long tr; // r0
...
// Range: 0x800B6730 -> 0x800B6788
void xShadowCameraFlushPreRendered(void * model /* r31 */, void (* renderCB)(void *) /* r12 */, struct xVec3 * center /* r11 */, struct xVec3 * radius /* r10 */, int preRenderedSlot /* r0 */, int shadowMode /* r9 */) {
    // References
    // -> struct RwCamera * ShadowCamera;
}

unsigned int gEntID; // size: 0x4, address: 0x80C07784
// total size: 0x10
struct RwSphere {
    // Members
...
// Range: 0x800B6788 -> 0x800B68E4
unsigned int xShadowReceiveShadowSetup(struct xEnt * ent /* r27 */) {
    // Local variables
    long long tt; // r0
    long long tt2; // r0
...
unsigned int enable_shadowmap_pipeline; // size: 0x4, address: 0x80C07788
// total size: 0x40
struct RwMatrixTag {
    // Members
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFE5BC
// total size: 0x18
...
// Range: 0x800B68E4 -> 0x800B7304
void xShadowReceiveShadow(struct xEnt * ent /* r15 */, float shadowFactor /* f30 */, int shadowMode /* r18 */, struct RwMatrixTag * shadowMat /* r17 */, struct RwRaster * shadowRast /* r16 */) {
    // Local variables
    struct xModelInstance * m; // r26
    struct RwMatrixTag oldroot; // r1+0xD0
    struct RwCamera * shadowCamera; // r19
    struct RwMatrixTag * shadowMatrix; // r16
    struct RwMatrixTag invMatrix; // r1+0x90
    struct RwV3d at; // r1+0x3C
    struct RwV3d scl; // r1+0x30
    struct RwV3d tr; // r1+0x24
    unsigned int i; // r25
    unsigned int num_verts; // r0
    struct xVec3 * xvert; // r24
    struct RpTriangle * tri; // r23
    struct RpGeometry * geom; // r22
    struct RwMatrixTag * mat; // r17
    struct xVec3 radius; // r1+0x18
    long long t0; // r0
    long long tt; // r0
    int fogstate; // r1+0x8
    struct xModelInstance * model; // r4
    unsigned int max_verts; // r21
    struct RxObjSpace3DVertex * imv; // r20
    unsigned char val; // r19
    struct RwV3d vShad[3]; // r1+0x6C
    struct xVec3 * v0; // r18
    struct xVec3 * v1; // r17
    struct xVec3 * v2; // r16
    struct RwV3d vIn[3]; // r1+0x48
    struct RwV3d c; // r1+0xC
    float len; // f1
...
// Range: 0x800B7304 -> 0x800B7324
void xShadow_ListAdd(struct xEnt * ent /* r0 */) {}

void * RwEngineInstance; // size: 0x4, address: 0x80C09F34
// Range: 0x800B7324 -> 0x800B747C
static void InvertRaster(struct RwCamera * camera /* r0 */) {
    // Local variables
...
// Range: 0x800B747C -> 0x800B750C
static void ShadowCameraDestroy(struct RwCamera * shadowCamera /* r30 */) {
    // Local variables
    struct RwRaster * raster; // r3
...
// Range: 0x800B750C -> 0x800B766C
static struct RwCamera * ShadowCameraUpdate(struct RwCamera * shadowCamera /* r28 */, void * model /* r29 */, void (* renderCB)(void *) /* r30 */, int shadowMode /* r31 */) {
    // Local variables
    struct RwRGBA bgColor; // r1+0xC
...
// Range: 0x800B766C -> 0x800B7878
static struct RwCamera * ShadowCameraUpdatePreRendered(struct RwCamera * shadowCamera /* r29 */, void * model /* r26 */, void (* renderCB)(void *) /* r28 */, int preRenderedSlot /* r30 */, int shadowMode /* r27 */) {
    // Local variables
    struct RwRGBA bgColor; // r1+0xC
...
// Range: 0x800B78B0 -> 0x800B7918
static struct RwCamera * ShadowCameraFlushPreRendered(struct RwCamera * shadowCamera /* r31 */, int preRenderedSlot /* r0 */) {
    // Local variables
    struct RwRaster * raster; // r5
}
...
// Range: 0x800B7918 -> 0x800B7A1C
void GameCubeCameraTextureFlush(struct RwRaster * raster /* r28 */, int boxFilter /* r29 */) {
    // Local variables
    struct RwRaster * dstParent; // r31
...
// Range: 0x800B7A1C -> 0x800B7A48
static struct RwRaster * ShadowRasterCreate(int size /* r3 */) {
    // Local variables
    struct RwRaster * raster; // r0
}

// Range: 0x800B7A48 -> 0x800B7B64
struct RwRaster * ShadowRasterRender(struct RwRaster * shadowRaster /* r30 */, struct RwV2d * vx /* r31 */) {
    // Local variables
    struct RwCamera * camera; // r6
    struct RwRaster * camRas; // r7
    float crw; // f31
    float crh; // f30
    float recipCamZ; // f29
...
// Range: 0x800B7B64 -> 0x800B7BAC
static void RenderShadowPolyOutline(struct xShadowPoly * poly /* r31 */) {}

// Range: 0x800B7BAC -> 0x800B7EF0
static void ShadowRenderCachePolys(struct _ProjectionParam * param /* r29 */, struct xShadowPoly * poly /* r30 */, int count /* r31 */) {
    // Local variables
    int i; // r26
    struct RxObjSpace3DVertex * imv; // r3
    struct RwV3d vShad[3]; // r1+0x2C
    struct RwV3d vIn[3]; // r1+0x8
    float cx; // f4
    float cy; // f5
    float cz; // f3
...
unsigned char debugNoJspsRendering; // size: 0x1, address: 0x80C07274
unsigned char iEnvAllJSPSActive; // size: 0x1, address: 0x80C078F4
float zlim; // size: 0x4, address: 0x80C0778C
float zamt; // size: 0x4, address: 0x80BFE5D4
struct xStats gStats; // size: 0x118, address: 0x8045A210
// Range: 0x800B7EF0 -> 0x800B822C
void xShadowDebug() {
    // Local variables
    float f; // f31
...
// Range: 0x800B822C -> 0x800B8254
void GCSaveFrameBuffer() {
    // References
    // -> static struct RwRaster * gc_saveraster;
}

// Range: 0x800B8254 -> 0x800B839C
...
// Range: 0x800B839C -> 0x800B8468
static struct RwCamera * ShadowCameraCreatePersp(int size /* r30 */) {
    // Local variables
    struct RwCamera * shadowCamera; // r31
    struct RwV3d shadowCameraCenter; // r1+0x10
    struct RwV2d vw; // r1+0x8
    struct RwRaster * zRaster; // r0
}

// Range: 0x800B8468 -> 0x800B8530
static struct RwCamera * ShadowCameraSetSpherePersp(struct RwCamera * camera /* r26 */, struct RwV3d * center /* r27 */, struct xVec3 * radius /* r28 */) {
    // Local variables
    struct RwFrame * camFrame; // r31
    struct RwMatrixTag * camMatrix; // r30
    struct RwV3d * camPos; // r29

    // References
    // -> struct xVec3 gShadowObjectRadius;
}

static int MAX_CACHE_POLYS; // size: 0x4, address: 0x80C07790
// Range: 0x800B8530 -> 0x800B8938
static struct RpCollisionTriangle * shadowCacheEnvCB(struct RpCollisionTriangle * tri /* r29 */, void * data /* r0 */) {
    // Local variables
    struct ShadowCacheContext * context; // r0
...
static int drawExamined; // size: 0x4, address: 0x80C07794
static signed char init; // size: 0x1, address: 0x80C07798
int _rpCollisionGeometryDataOffset; // size: 0x4, address: 0x80C09D94
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x800B8938 -> 0x800B8FA0
static int shadowCacheLeafCB(int numTriangles /* r30 */, int triOffset /* r4 */, void * data /* r31 */) {
    // Local variables
    struct ShadowCBParam * cbparam; // r0
    struct xShadowCache * cache; // r26
    struct RpGeometry * geometry; // r6
    struct RwV3d * vertices; // r25
    struct RpTriangle * triangles; // r24
    int triSlot; // r23
    unsigned short * triIndex; // r22
    int i; // r5
    const struct RpTriangle * const tri; // r7
    struct RwV3d * v0; // r21
    struct RwV3d * v1; // r20
    struct RwV3d * v2; // r19
    struct xVec3 worldV[3]; // r1+0x30
    float posX; // f0
    float posZ; // f1
    float nx; // f5
    float nz; // f4
    float nmag2; // f4
    float pdot; // f3
    struct xVec3 * vert1; // r3
    float dotB; // f2
    struct xVec3 * vert1; // r5
    struct xVec3 * vert2; // r6
    struct xVec3 aa; // r1+0x24
    struct xVec3 bb; // r1+0x18
    struct xVec3 trinorm; // r1+0xC
    float depthtest; // f12
    float dydx; // f0
    float dydz; // f1
    float depth0; // f2
...
static const struct xEnt * sEntSelf; // size: 0x4, address: 0x80C0779C
// total size: 0xC
struct /* @class$1235xShadow_cpp */ {
    // Members
...
// Range: 0x800B8FA0 -> 0x800B9268
static int shadowCacheEntityCB(struct xEnt * ent /* r30 */, void * cbdata /* r31 */) {
    // Local variables
    struct ShadowCBParam * cbparam; // r0
...
static int inittweaks; // size: 0x4, address: 0x80C077A0
static signed char init; // size: 0x1, address: 0x80C077A4
// Range: 0x800B9268 -> 0x800B97F0
void xShadowVertical_FillCache(struct xShadowCache * cache /* r28 */, struct xVec3 * pos /* r29 */, float r /* f30 */, float depth /* f31 */, float minNormY /* f29 */, unsigned char castOnJSP /* r25 */, unsigned char castOnEnts /* r30 */) {
    // Local variables
    int i; // r0
    int j; // r0
    struct xEnv * env; // r31
    struct RpIntersection isx; // r1+0x48
    struct ShadowCacheContext context; // r1+0xC
    int i; // r25
    struct xShadowPoly * polys; // r25
    float limit; // f29
    int i; // r27
    float sortRayDepth[5]; // r1+0x34
    float objMaxDepth; // f2
    struct xQCData qcd; // r1+0x14
    struct ShadowCBParam cbparam; // r1+0x64
...
// Range: 0x800B9874 -> 0x800B9B40
void xShadowVertical_DrawCache(struct xShadowCache * cache /* r31 */, float shadowFactor /* f30 */, float fadeDist /* f31 */, int shadowMode /* r28 */, struct RwMatrixTag * shadowMat /* r30 */, struct RwRaster * shadowRast /* r29 */) {
    // Local variables
    struct _ProjectionParam param; // r1+0x34
    struct RwMatrixTag * shadowMatrix; // r29
    struct RwV3d scl; // r1+0x28
    struct RwV3d tr; // r1+0x1C
    struct xVec3 radius; // r1+0x10
    int old_zwrite; // r1+0xC
    int fogstate; // r1+0x8
...
static int SHADOW_CACHE_MAX; // size: 0x4, address: 0x80BFE5D8
// total size: 0x14
struct xShadowMgr {
    // Members
...
static struct xShadowMgr * sMgrList; // size: 0x4, address: 0x80C077A8
static int sMgrCount; // size: 0x4, address: 0x80C077AC
static int sMgrTotal; // size: 0x4, address: 0x80C077B0
static struct xShadowCache * sCacheList; // size: 0x4, address: 0x80C077B4
unsigned int gActiveHeap; // size: 0x4, address: 0x80C07448
// Range: 0x800B9B40 -> 0x800B9C10
void xShadowManager_Init(int numEnts /* r30 */, int maxPolys /* r31 */, int maxCaches /* r0 */) {
    // Local variables
    struct xShadowPoly * polys; // r3
    int i; // r6
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80BFE5DC
// Range: 0x800B9C10 -> 0x800B9D94
void xShadowManager_Add(struct xEnt * ent /* r31 */) {
    // Local variables
    int i; // r0
...
// Range: 0x800B9DC0 -> 0x800B9EC0
static int CmpShadowMgr(void * a /* r0 */, void * b /* r0 */) {
    // Local variables
    struct xEnt * entA; // r5
    struct xEnt * entB; // r6
    int isPlayerA; // r4
...
// Range: 0x800B9EC0 -> 0x800BA03C
void xShadowModelFileRender(struct RpAtomic * model /* r28 */, struct RwMatrixTag * animMat /* r29 */) {
    // Local variables
    int i; // r7
    struct RpMaterialList * matList; // r0
    struct RwTexture * oldtex[8]; // r1+0x14
    struct RwMatrixTag oldMatrix; // r1+0x34
    struct xSphere * center; // r0
    struct RwV3d scl; // r1+0x8
    struct RwMatrixTag * shadowMatrix; // r0
...
// Range: 0x800BA03C -> 0x800BA3AC
static void xShadowModelRender(struct xModelInstance * minst /* r30 */) {
    // Local variables
...
static char __FUNCTION__[25]; // size: 0x19, address: 0x8042E18C
// Range: 0x800BA3AC -> 0x800BA6CC
void xShadowManager_PreRender() {
    // Local variables
    int i; // r6
    float shadow_strength; // r1+0xC
    struct xEnt * ent_ref; // r30
    int currPreRenderedSlot; // r29
    struct xEnt * ent; // r28
    float dst_depth; // f1
...
static int inittweaks; // size: 0x4, address: 0x80C077B8
static signed char init; // size: 0x1, address: 0x80C077BC
static char __FUNCTION__[22]; // size: 0x16, address: 0x80BFE5F0
unsigned char xClumpColl_FilterFlags; // size: 0x1, address: 0x80BFBA18
// Range: 0x800BA6CC -> 0x800BB03C
void xShadowManager_Render() {
    // Local variables
    int i; // r6
    int cacheUsed[10]; // r1+0x38
    int j; // r29
    float shadow_strength; // r1+0x10
    unsigned char old_xClumpColl_FilterFlags; // r28
    struct xEnt * ent_ref; // r30
    int bestIndex; // r8
    int foundPriority; // r9
    struct xVec3 center; // r1+0x2C
    float radius; // r1+0xC
    struct xShadowMgr * mgr_best; // r29
    float dst_depth; // f2
    const struct xEnt * ep; // r4
    struct xEnt * ent; // r30
    float dst_depth; // f1
    int shadowOutside; // r1+0x8
    struct xVec3 center; // r1+0x20
    struct xVec3 radius; // r1+0x14
    struct RpAtomic * old_model; // r27
    struct xModelInstance * old_mnext; // r26
    struct xEnt * ep; // r26
    struct xShadowMgr * mgr; // r0
    const struct xEntShadow * npcshad; // r3
    int num; // r26
    int a; // r25
    struct xEnt * ep; // r24
    float rad; // f1
    struct xVec3 * lower; // r0
    struct xVec3 * upper; // r0
...
static char __FUNCTION__[23]; // size: 0x17, address: 0x80BFE608
// Range: 0x800BB03C -> 0x800BB3C0
static void xShadow_PickByBaseType(struct xShadowMgr * mgr /* r28 */) {
    // Local variables
    struct xEnt * best_dyn; // r31
    int idx_dyn; // r27
    struct xEnt * best_stat; // r30
    int idx_stat; // r26
    struct xEnt * best_other; // r29
    int idx_other; // r25
    int num; // r0
    int i; // r5
    struct xEnt * ep; // r6
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFE620
// Range: 0x800BB3C0 -> 0x800BB558
static void xShadow_PickByBounds(struct xShadowMgr * mgr /* r30 */) {
    // Local variables
    int num; // r28
    struct xEnt * ent_best; // r31
    int idx_best; // r27
    int i; // r26
    struct xEnt * ep; // r25
    float vol; // f0
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x80BFE638
struct xVec3 g_NY3; // size: 0xC, address: 0x80C0ADD0
// total size: 0x24
struct xRay3 {
    // Members
    struct xVec3 origin; // offset 0x0, size 0xC
...
// Range: 0x800BB558 -> 0x800BB7C8
static void xShadow_PickByRayCast(struct xShadowMgr * mgr /* r25 */) {
    // Local variables
    struct xEnt * ent_best; // r30
    int idx_best; // r29
...
static int logCacheResults; // size: 0x4, address: 0x80C077C0
static signed char init; // size: 0x1, address: 0x80C077C4
static unsigned int logOnlyThisID; // size: 0x4, address: 0x80C077C8
static signed char init; // size: 0x1, address: 0x80C077CC
static int useSelectionType; // size: 0x4, address: 0x80C077D0
static signed char init; // size: 0x1, address: 0x80C077D4
static int inittweaks; // size: 0x4, address: 0x80C077D8
static signed char init; // size: 0x1, address: 0x80C077DC
static char * strz_selectModes[4]; // size: 0x10, address: 0x80BFE650
static char __FUNCTION__[22]; // size: 0x16, address: 0x80BFE660
// Range: 0x800BB7C8 -> 0x800BBA78
