/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zSurface.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8016AB54 -> 0x8016C818
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct zSurfaceProps * zsps; // size: 0x4, address: 0x80C084F8
static int sMapperCount; // size: 0x4, address: 0x80C084FC
// total size: 0x8
...
static struct zMaterialMapAsset * sMapper[20]; // size: 0x50, address: 0x8043C568
// total size: 0x50
...
static struct xSurface sDefaultSurface; // size: 0x28, address: 0x80BAA1D0
// total size: 0x20
struct xLinkAsset {
    // Members
...
static struct zSurfaceProps sDefaultSurfaceProps; // size: 0x118, address: 0x80BAA1F8
// total size: 0x8
struct xBaseAsset {
    // Members
    unsigned int id; // offset 0x0, size 0x4
...
static struct zSurfAssetIN sDefaultSurfaceAsset; // size: 0x1B8, address: 0x80BAA310
static struct xSurface * pDefaultSurface; // size: 0x4, address: 0x80C08500
static char __FUNCTION__[13]; // size: 0xD, address: 0x80C016D8
unsigned int gActiveHeap; // size: 0x4, address: 0x80C07448
// Range: 0x8016AB54 -> 0x8016AE48
void zSurfaceInit() {
    // Local variables
    unsigned int size; // r1+0x8
    unsigned short nsurfs; // r24
    struct zSurfAssetIN * defaultAsset; // r0
    unsigned short i; // r23
    struct zSurfAssetIN * asset; // r26
    struct xSurface * surf; // r22
    struct zSurfaceProps * moprops; // r25
    int j; // r21
...
static char __FUNCTION__[23]; // size: 0x17, address: 0x80C016E8
// Range: 0x8016AEE8 -> 0x8016AFA8
...
// Range: 0x8016AFA8 -> 0x8016AFD0
void zSurfaceExit() {
    // References
    // -> static int sMapperCount;
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80C01700
// Range: 0x8016AFD0 -> 0x8016B058
void zSurfaceResetSurface(struct xSurface * surf /* r31 */) {
    // References
    // -> static char __FUNCTION__[21];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01718
// total size: 0x8
...
// Range: 0x8016B058 -> 0x8016B18C
struct xSurface * zSurfaceGetSurface(unsigned int mat_id /* r27 */) {
    // Local variables
    int map; // r30
    struct zMaterialMapAsset * mapper; // r29
    unsigned short i; // r28
    struct zMaterialMapEntry * entry; // r25
    unsigned short nsurfs; // r0
...
// Range: 0x8016B18C -> 0x8016B1E0
struct xSurface * zSurfaceGetSurface(unsigned int oid /* r0 */, void * optr /* r0 */, struct xModelInstance * mptr /* r0 */) {
    // Local variables
    struct xSurface * surf; // r0
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C0172C
...
// Range: 0x8016B1E0 -> 0x8016B2DC
struct xSurface * zSurfaceGetSurface(const struct xSweptSphere * sphere /* r30 */) {
    // Local variables
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01740
// Range: 0x8016B2DC -> 0x8016B3CC
struct xSurface * zSurfaceGetSurface(const struct xCollis * coll /* r30 */) {
    // Local variables
    struct xSurface * surf; // r31
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80C01754
// Range: 0x8016B3CC -> 0x8016B45C
unsigned char zSurfaceOutOfBounds(const struct xSurface & s /* r0 */) {
    // Local variables
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x80C01768
// Range: 0x8016B45C -> 0x8016B534
int zSurfaceGetDamageType(const struct xSurface * surf /* r31 */) {
    // Local variables
    struct zSurfAssetIN * sass; // r31

    // References
    // -> static char __FUNCTION__[22];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80C01780
// Range: 0x8016B534 -> 0x8016B618
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80C01794
// total size: 0x0
struct xSerial {};
// Range: 0x8016B618 -> 0x8016B6E8
void zSurfaceSave(struct xSurface * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[13];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80C017A4
// Range: 0x8016B6E8 -> 0x8016B7B8
void zSurfaceLoad(struct xSurface * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[13];
}

char __FUNCTION__$localstatic1$CacheTextureRefs__FP19zSurfacePropTexAnim[17]; // size: 0x11, address: 0x80C017B4
static char __FUNCTION__[14]; // size: 0xE, address: 0x80C017C8
// Range: 0x8016B7B8 -> 0x8016B8F8
...
// Range: 0x8016B8F8 -> 0x8016BA1C
void CacheTextureRefs(struct zSurfacePropTexAnim * texanim /* r25 */) {
    // Local variables
    struct xGroup * grp; // r28
    unsigned int num_items; // r0
    unsigned int amt; // r29
    void * mem; // r0
    int i; // r27
    unsigned int aid_item; // r26
    void * memptr; // r0
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80C017D8
// Range: 0x8016BA1C -> 0x8016BB50
void zSurfaceUpdate(struct xBase * to /* r31 */, struct xScene * sc /* r30 */, float dt /* f31 */) {
    // Local variables
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80C017E8
// Range: 0x8016BB50 -> 0x8016C0B8
static void zSurfaceEventCB(struct xBase * to /* r29 */, unsigned int toEvent /* r28 */, const float * toParam /* r30 */, struct xBase * toParamWidget /* r31 */) {
    // Local variables
    struct xSurface * t; // r29
    struct zSurfaceProps * p; // r0
    struct zSurfaceProps * p; // r0
    struct zSurfaceProps * p; // r0
    struct zSurfaceProps * p; // r0
    struct zSurfaceProps * p; // r0
...
// Range: 0x8016C0B8 -> 0x8016C0C0
struct xSurface & zSurfaceGetDefault() {
    // References
    // -> static struct xSurface * pDefaultSurface;
}

char __FUNCTION__$localstatic1$zSurfaceUpdateTextureAnim__FfP8xSurfaceP13zSurfaceProps[26]; // size: 0x1A, address: 0x8043C5B8
...
// Range: 0x8016C0C0 -> 0x8016C818
void zSurfaceUpdateTextureAnim(float dt /* f31 */, struct zSurfaceProps * moprops /* r28 */) {
    // Local variables
    int j; // r30
    struct xVec3 d; // r1+0x8
    struct zSurfacePropUVFX & sfx; // r0
    float uTime; // f3
    int uTimeInt; // r0
    float vTime; // f3
    int vTimeInt; // r0
    struct xGroup * g; // r3
    unsigned int max; // r29
