/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zHangable.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80130738 -> 0x80131724
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80C009C8
// total size: 0x20
struct xLinkAsset {
    // Members
...
// Range: 0x80130738 -> 0x801308BC
void zHangable_Init(struct xBase & data /* r30 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r28 */) {
    // Local variables
...
// total size: 0x40
struct zHangableAsset : public xDynAsset {
    // Members
    unsigned int object; // offset 0x10, size 0x4
    struct xVec3 pivot; // offset 0x14, size 0xC
    struct xVec3 handle; // offset 0x20, size 0xC
...
// total size: 0x18
struct zHangable : public xBase {
    // Members
    struct zHangableAsset * hangableAsset; // offset 0x10, size 0x4
    unsigned int flags; // offset 0x14, size 0x4
};
// Range: 0x801308BC -> 0x80130954
void zHangable_Init(struct zHangable * hangable /* r30 */, struct zHangableAsset * asset /* r31 */) {}
...
// total size: 0x4
struct /* @class$1085zHangable_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1249zHangable_cpp */ {
    // Members
...
struct zHangableObject {
    // Members
    struct zHangable * hangable; // offset 0x0, size 0x4
    struct xEnt * object; // offset 0x4, size 0x4
    float radius; // offset 0x8, size 0x4
    struct xVec3 vel; // offset 0xC, size 0xC
    struct xVec3 accel; // offset 0x18, size 0xC
    struct xVec3 handle; // offset 0x24, size 0xC
    struct xMat3x3 centerMat; // offset 0x30, size 0x30
    struct xVec3 rotAxis; // offset 0x60, size 0xC
    struct xVec3 lastDisp; // offset 0x6C, size 0xC
    struct xVec3 objVel; // offset 0x78, size 0xC
};
static struct zHangableObject * sPlayerHangObj; // size: 0x4, address: 0x80C08178
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
static struct xVec3 playerDPos; // size: 0xC, address: 0x80C0817C
static struct zHangableObject * sHangableObjects; // size: 0x4, address: 0x80C08188
static int sNumHangableObjects; // size: 0x4, address: 0x80C0818C
// total size: 0xC
...
// Range: 0x80130954 -> 0x80130A1C
int zHangable_Recurse(unsigned int id /* r0 */, struct zHangableObject * gpList /* r26 */, struct zHangable * hangable /* r27 */) {
    // Local variables
    unsigned int i; // r30
    int result; // r29
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80C009D8
...
// total size: 0x10
struct /* @class$1788zHangable_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x80130A1C -> 0x80130D54
void zHangable_SceneInit(struct zScene * zsc /* r28 */) {
    // Local variables
    int i; // r30
    int count; // r27
    int numHangables; // r26
    struct zHangable * hangable; // r5
    struct zHangableObject * hangObj; // r29
    struct zHangableAsset * asset; // r28
    struct xMat3x3 rotMat; // r1+0x40
    struct xVec3 pivot; // r1+0x28
    struct xVec3 disp; // r1+0x1C
    struct xVec3 cross; // r1+0x10
    float len; // f0
    float ang; // f1

    // References
    // -> static int sNumHangableObjects;
    // -> struct xGlobals * xglobals;
    // -> static char __FUNCTION__[20];
    // -> static struct zHangableObject * sHangableObjects;
    // -> unsigned int gActiveHeap;
}

// Range: 0x80130D54 -> 0x80130DAC
void zHangable_Reset() {
    // Local variables
    int i; // r5
    struct zHangableObject * hangObj; // r6
    struct zHangable * hangable; // r7
...
// Range: 0x80130DAC -> 0x801314E4
void zHangable_Update(float dt /* f26 */) {
    // Local variables
    int i; // r29
    struct zHangableObject * hangObj; // r28
    struct zHangableAsset * asset; // r27
    struct xVec3 oldHandle; // r1+0xC0
    struct xVec3 hangDisp; // r1+0xB4
    struct xVec3 playerCenter; // r1+0xA8
    struct xVec3 localCenter; // r1+0x9C
    struct xVec3 centerDisp; // r1+0x90
    struct xVec3 disp; // r1+0x84
    float dist; // f0
    struct xVec3 pivot; // r1+0x78
    struct xMat3x3 newMat; // r1+0x130
    struct xMat3x3 rotMat; // r1+0x100
    float comp; // f0
    float comp; // f0
    float maxAng; // f25
    struct xVec3 cross; // r1+0x6C
    float len; // f1
    float ang; // f0
    struct xMat3x3 rotMat; // r1+0xD0
    struct xVec3 actualDisp; // r1+0x60
    struct xVec3 actualPivot; // r1+0x54
    struct xVec3 cross; // r1+0x48
    float len; // f1
    float ang; // f25
    float minArcRadians; // f24
    float height; // f25
    struct xVec3 relVel; // r1+0x3C
    float velSq; // f0
    float total; // f24
    float reqTotal; // f0
    struct xVec3 velAddDir; // r1+0x30
    float velDot; // f0
    struct xVec3 negAddDir; // r1+0x24
    struct xModelInstance * atomic; // r27
    struct xVec3 newPlayerCenter; // r1+0x18

    // References
    // -> static int sNumHangableObjects;
    // -> static struct xVec3 playerDPos;
    // -> static struct zHangableObject * sPlayerHangObj;
...
// total size: 0x890
struct /* @class$2035zHangable_cpp */ {
    // Static members
...
// Range: 0x801314E4 -> 0x801315D4
void zHangable_Recenter(struct zHangableObject * hangObj /* r29 */) {
    // Local variables
    struct zHangableAsset * asset; // r31
    struct xVec3 pivot; // r1+0x14
    struct xVec3 newPivot; // r1+0x8
    struct xMat4x3 * objMat; // r30
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80C009EC
// Range: 0x801315D4 -> 0x80131724
void zHangable_EventCB(struct xBase * to /* r31 */, unsigned int toEvent /* r29 */) {
    // Local variables
    struct zHangable * hang; // r0
    int i; // r29
    struct zHangableObject * hangObj; // r3
