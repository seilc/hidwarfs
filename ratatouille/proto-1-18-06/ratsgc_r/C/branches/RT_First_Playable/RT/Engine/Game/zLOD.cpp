/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zLOD.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801394F4 -> 0x80139D2C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static unsigned int sLODTableCount; // size: 0x4, address: 0x80C081F8
// total size: 0x8
struct RwObject {
    // Members
...
static struct zLODTable * sLODTableList; // size: 0x4, address: 0x80C081FC
static unsigned int sManagerIndex; // size: 0x4, address: 0x80C08200
static unsigned int sManagerCount; // size: 0x4, address: 0x80C08204
static float sLODFadeDistance; // size: 0x4, address: 0x80C00BA8
// total size: 0x10
struct zLODManager {
    // Members
    int numextra; // offset 0x0, size 0x4
    struct zLODTable * lod; // offset 0x4, size 0x4
    struct xModelInstance * model; // offset 0x8, size 0x4
    float adjustNoRenderDist; // offset 0xC, size 0x4
};
static struct zLODManager sManagerList[2900]; // size: 0xB540, address: 0x80B93A38
static unsigned char done; // size: 0x1, address: 0x80C08208
static signed char init; // size: 0x1, address: 0x80C08209
static char __FUNCTION__[13]; // size: 0xD, address: 0x80C00BAC
// total size: 0x20
struct xLinkAsset {
    // Members
...
// Range: 0x801394F4 -> 0x80139770
static void AddToLODList(struct xEnt * ent /* r29 */) {
    // Local variables
    unsigned int i; // r30
    struct xModelInstance * minst; // r28
    float distscale; // f31
    struct xModelInstance * extra; // r6
    struct zDestructible * destructible; // r4

    // References
    // -> static unsigned int sLODTableCount;
    // -> static unsigned int sManagerCount;
    // -> static struct zLODTable * sLODTableList;
    // -> static struct zLODManager sManagerList[2900];
    // -> static float sLODFadeDistance;
    // -> static unsigned char done;
    // -> static char __FUNCTION__[13];
    // -> static signed char init;
}

// Range: 0x80139770 -> 0x801397B0
static struct xEnt * AddToLODList(struct xEnt * ent /* r31 */) {}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80C00BBC
...
// Range: 0x801397B0 -> 0x80139A08
void zLOD_Setup(float fadeDistance /* f0 */) {
    // Local variables
    ...
    struct zLODTable * tableCurr; // r27
    struct RpAtomic * model; // r0
    struct RpAtomic * model; // r0
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80C00BC8
// Range: 0x80139A08 -> 0x80139CBC
void zLOD_Update(unsigned int percent_update /* r0 */) {
    // Local variables
    struct xVec3 * campos; // r0
    unsigned int numUpdates; // r29
    struct zLODManager & lodManager; // r28
    struct zLODTable * lod; // r27
    ...
    // -> static unsigned int sManagerIndex;
    // -> static struct zLODManager sManagerList[2900];
    // -> static unsigned int sManagerCount;
    // -> struct zGlobals globals;
}

// Range: 0x80139CBC -> 0x80139D2C
struct zLODTable * zLOD_Get(struct xEnt * ent /* r0 */) {
    // Local variables
    unsigned int i; // r7
