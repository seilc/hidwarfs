/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xCamSupport.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80013A68 -> 0x80017400
*/
static char __FUNCTION__[7]; // size: 0x7, address: 0x80BFB998
...
// Range: 0x80013A68 -> 0x80013BA4
// this: r30
void xCamSupportPath::create(int max_size /* r29 */, int debug_max_size /* r31 */) {
    // Local variables
...
// Range: 0x80013BA4 -> 0x80013C28
// this: r0
void xCamSupportPath::reset(const struct xVec3 & start_loc /* r29 */) {
    // Local variables
    struct path_node * p; // r31
    struct path_node * endp; // r30
}

// Range: 0x80013C28 -> 0x80013C68
// this: r31
...
// Range: 0x80013C68 -> 0x80013D68
// this: r31
int xCamSupportPath::polar_subdivide(const struct xCamCoordPolar & start /* r28 */, const struct xCamCoordPolar & end /* r29 */) {
    // Local variables
    int path_end; // r0
    int vert_used; // r29
    int subdiv_end; // r30
...
static char __FUNCTION__[6]; // size: 0x6, address: 0x80BFB9A0
// Range: 0x80013D68 -> 0x80013F54
// this: r31
void xCamSupportPath::prune(int start /* r24 */, int size /* r0 */) {
    // Local variables
    int i0; // r30
    int i1; // r29
    int i2; // r28
    int end_i2; // r27
    const struct path_node & A; // r26
    struct path_node & B; // r25
    const struct path_node & C; // r24
    float dx; // f3
    float dy; // f4
    float dz; // f1
    float ABmag; // f0
...
static struct path_node prev_path[33]; // size: 0x18C, address: 0x80459318
static char __FUNCTION__[7]; // size: 0x7, address: 0x80BFB9A8
// Range: 0x80013F70 -> 0x800142F0
// this: r24
void xCamSupportPath::smooth(int start /* r25 */, int size /* r26 */) {
    // Local variables
    const float * w; // r6
    struct path_node * pp; // r21
    struct path_node * end_pp; // r22
    int r; // r27
    int end_r; // r0
    int i; // r31
    int end_i; // r30
    ...
    float Ry; // f30
    float Rz; // f29
    struct path_node & node; // r27
    const struct path_node & B; // r7
    const struct path_node & B; // r3
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80BFB9B0
// Range: 0x800142F0 -> 0x80014674
// this: r28
float xCamSupportPath::traverse(struct xVec3 & rloc /* r29 */, float velmag /* f24 */, float dt /* f25 */) {
    // Local variables
    int i; // r31
    int end; // r30
    const struct path_node & p0; // r27
    const struct path_node & p1; // r26
    float dx; // f28
    float dy; // f27
    float dz; // f26
    float segdist; // f0
    const struct path_node & p2; // r3
    float ndx; // f23
    float ndy; // f22
    float ndz; // f21
    float next_subdist; // f0
    float d; // f2
    float damp; // f1
    float s; // f4
...
static float increase_factor; // size: 0x4, address: 0x80C06E78
static signed char init; // size: 0x1, address: 0x80C06E7C
static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFB9BC
// total size: 0xC
struct path_node {
    // Members
    struct xVec3 loc; // offset 0x0, size 0xC
};
// Range: 0x8001474C -> 0x80014C30
int xCamSupportPath::polar_subdivide(struct path_node * vert /* r29 */, int vert_size /* r30 */, const struct xCamCoordPolar & start /* r31 */, const struct xCamCoordPolar & end /* r25 */, float thresh /* f31 */) {
    // Local variables
    struct path_node head; // r1+0x14
    struct path_node tail; // r1+0x8
    int vert_used; // r27
    float ABx; // f3
    float ABy; // f4
    float ABz; // f1
    float best_total; // f28
    float x; // f27
    float y; // f26
    float z; // f25
    float theta; // f24
    ...
    float ddist; // f21
    float total; // f15
    struct path_node * v; // r26
    struct path_node * endv; // r25
    float cp; // f14
    float ABx; // f3
    float ABy; // f4
    float ABz; // f1
...
// total size: 0x4
struct /* @class$1266xCamSupport_cpp */ {
    // Members
...
union /* @class$1015xCamSupport_cpp */ {
...
// total size: 0x10
struct /* @class$1028xCamSupport_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFB9CC
// Range: 0x80014C30 -> 0x80014F4C
void xCamSupportPath::compute_weights(float * w /* r30 */, int n /* r31 */, float s /* f26 */) {
    // Local variables
    float start_efrac; // f28
    int i; // r25
    float efrac; // f27
    int j; // r24
    float spow; // f0
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFB9DC
// Range: 0x80014F4C -> 0x80015090
// this: r31
void xCamSupportPath::debug_init(int debug_max_size /* r30 */) {
    // Local variables
    unsigned int alloc_size; // r30

    // References
    // -> static char __FUNCTION__[11];
}

// Range: 0x80015090 -> 0x8001515C
// this: r27
...
// total size: 0xC
struct /* @class$964xCamSupport_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$965xCamSupport_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$966xCamSupport_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$967xCamSupport_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$968xCamSupport_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$969xCamSupport_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$970xCamSupport_cpp */ {
    // Members
...
// Range: 0x8001515C -> 0x80015180
static void on_change_config(const struct tweak_info & info /* r3 */) {}
...
static struct tweak_callback cb_config; // size: 0x28, address: 0x803A6A78
// Range: 0x80015180 -> 0x80015320
// this: r29
void xCamSupportPath::add_tweaks(const char * prefix /* r30 */, const char * debug_prefix /* r31 */) {
    // References
    // -> static struct tweak_callback cb_config;
}

enum /* @enum$1152xCamSupport_cpp */ {
    DEBUG_FLAG_RENDER = 1,
};
// total size: 0x4
struct xColor_tag {
...
static unsigned char enable_collision; // size: 0x1, address: 0x80BFB9E7
static unsigned char enable_coll_entity; // size: 0x1, address: 0x80BFB9E8
static unsigned char render_coll_cache_ent; // size: 0x1, address: 0x80C06E98
static unsigned char render_coll_cache_tri; // size: 0x1, address: 0x80C06E99
static unsigned char render_coll_pivot; // size: 0x1, address: 0x80C06E9A
static unsigned char render_coll_compress; // size: 0x1, address: 0x80C06E9B
// Range: 0x80015538 -> 0x80015694
...
static struct ent_cache_entry ent_cache[256]; // size: 0x400, address: 0x804594A4
static int ent_cache_size; // size: 0x4, address: 0x80C06E9C
static char __FUNCTION__[5]; // size: 0x5, address: 0x80BFB9EC
// total size: 0x30
struct xMat3x3 {
    // Members
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFB9F4
// Range: 0x80015694 -> 0x80015804
static float arc_sphere_to_plane(const struct xMat4x3 & circle_mat /* r31 */, float Rc /* f31 */, float Rs /* f30 */, const struct xVec3 & Nplane /* r5 */, float Dplane /* f0 */) {
    // Local variables
    struct xVec3 N; // r1+0x14
    struct xVec3 point_on_plane; // r1+0x8
    float D; // f3
    float D_Rc; // f31
    float Nx2_Ny2; // f30
    float det; // f0
...
// total size: 0x890
struct /* @class$1969xCamSupport_cpp */ {
    // Static members
...
// total size: 0xC
struct /* @class$1404xCamSupport_cpp */ {
    // Members
...
// Range: 0x80015804 -> 0x800159A0
static unsigned char capsule_hits_scene(const struct xCapsule & cap /* r27 */, struct xCollis & coll /* r28 */) {
    // Local variables
    struct xBox box; // r1+0x28
    struct xQCData qcd; // r1+0x8
    struct xRay3 ray; // r1+0x40
    struct ent_cache_entry * ite; // r31
    struct ent_cache_entry * end_ite; // r30
    struct xEnt & ent; // r29
...
// Range: 0x80015C38 -> 0x80015DF8
static unsigned char swept_sphere_hits_scene(struct xSweptSphere & sws /* r28 */, const struct xVec3 & start /* r30 */, const struct xVec3 & end /* r29 */, float radius /* f31 */) {
    // Local variables
    struct xBox box; // r1+0x28
    struct xQCData qcd; // r1+0x8
    struct xRay3 ray; // r1+0x40
    ...
    struct ent_cache_entry * end_ite; // r30
    struct xEnt & ent; // r29
    struct xModelInstance & model; // r5
...
// Range: 0x80015DF8 -> 0x80015F34
static unsigned char sphere_hits_scene(const struct xSphere & o /* r27 */, struct xCollis & coll /* r28 */) {
    // Local variables
    struct xQCData qcd; // r1+0x8
    struct ent_cache_entry * ite; // r31
    struct ent_cache_entry * end_ite; // r30
...
// Range: 0x80015F34 -> 0x800160A8
static void draw_arc(const struct xMat4x3 & circle_mat /* r29 */, float radius /* f28 */, float min_ang /* f29 */, float max_ang /* f0 */, int segments /* r30 */) {
    // Local variables
    float dang; // f30
    float ang; // f29
    int i; // r31
    struct xVec3 loc0; // r1+0x14
    struct xVec3 loc1; // r1+0x8
}
...
// Range: 0x800160A8 -> 0x800160F4
// this: r30
void xCamSupportLOSBar::reset(const struct xVec3 & origin /* r0 */, const struct xVec3 & stern_loc /* r31 */) {
    // References
    // -> struct config cfg;
}

// Range: 0x800160F4 -> 0x8001617C
// this: r30
void xCamSupportLOSBar::begin_update(const struct xVec3 & origin /* r31 */) {
    // References
    // -> struct config cfg;
}
...
// Range: 0x800162F0 -> 0x8001631C
// this: r0
static cache_entity_callback::cache_entity_callback(const struct xVec3 & center /* r0 */, float radius /* f0 */) {
    // References
    // -> static struct ent_cache_entry ent_cache[256];
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80BFBA08
...
// Range: 0x8001631C -> 0x80016E9C
// this: r28
unsigned char xCamSupportLOSBar::collide_pivot() {
    // Local variables
    struct xCollis coll; // r1+0x220
    struct xCapsule to_cap; // r1+0x150
    float to_t; // f30
    float to_dist; // f29
    struct xVec3 to_tohit; // r1+0xF0
    struct xVec3 to_impact; // r1+0xE4
    float new_rail_yoffset; // f26
    struct xCapsule new_cap; // r1+0x134
    struct xVec3 N; // r1+0xD8
    float D; // f25
    struct xMat4x3 vcircle_mat; // r1+0x1E0
    float bar_mag; // f0
    float vrot; // f25
    float oldpitch; // f0
    float newpitch; // f25
    ...
    float xzdist; // f0
    float newy; // f1
    struct xCapsule from_cap; // r1+0x118
    float from_t; // f28
    float from_dist; // f27
    struct xVec3 from_tohit; // r1+0xCC
    float start_s; // f25
    float end_s; // f26
    int tries; // r25
    float s; // f30
    struct xCapsule cap; // r1+0xFC
    struct xVec3 cap_offset; // r1+0xC0
    struct xVec3 impact; // r1+0xB4
    struct xVec3 N; // r1+0xA8
    float D; // f29
    float min_from_t; // f0
    struct xVec3 P; // r1+0x9C
    struct xVec3 CP; // r1+0x90
    struct xVec3 uCP; // r1+0x84
    float Rc; // f25
    struct xMat4x3 circle_mat; // r1+0x1A0
    float max_rot; // f26
    struct xMat3x3 rot_mat; // r1+0x170
    struct xVec3 tohit0; // r1+0x78
    struct xVec3 tohit1; // r1+0x6C
    struct xVec3 depen; // r1+0x60
    const struct xVec3 & P; // r0
    const struct xVec3 & B; // r0
    struct xVec3 BP; // r1+0x54
    struct xVec3 rot_axis; // r1+0x48
    float R; // f0
    float R2; // f25
    struct xVec3 PV; // r1+0x3C
    float det; // f25
    struct xVec3 V; // r1+0x30
    struct xVec3 BV; // r1+0x24
    float offset_mag; // f0
...
// Range: 0x800170F0 -> 0x80017274
void xCamSupportLOSBar::add_tweaks(const char * prefix /* r31 */, const char * debug_prefix /* r5 */) {
    // References
    // -> struct config cfg;
