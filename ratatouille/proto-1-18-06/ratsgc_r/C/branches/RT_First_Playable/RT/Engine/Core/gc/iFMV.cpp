/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\gc\iFMV.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800DAEC8 -> 0x800DC99C
*/
// total size: 0x10
struct BINKRECT {
    // Members
...
struct BINK * hBinkCurrMovie; // size: 0x4, address: 0x80C07920
static unsigned char sARAMtmp; // size: 0x1, address: 0x80C07924
static unsigned int amt; // size: 0x4, address: 0x80C07928
void * RwEngineInstance; // size: 0x4, address: 0x80C09F34
// Range: 0x800DAEC8 -> 0x800DAF7C
static void * BigMovieAlloc(unsigned int uNumBytes /* r31 */) {
    // Local variables
    void * addr; // r3
...
// Range: 0x800DAF7C -> 0x800DAFE4
static void BigMovieFree(void * pMemToFree /* r31 */) {
    // References
    // -> void * RwEngineInstance;
}

// Range: 0x800DAFE4 -> 0x800DB014
unsigned char iFMVSystemInit() {}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80BFF060
enum eGameMode {
    eGameMode_Boot = 0,
    eGameMode_Intro = 1,
    eGameMode_Title = 2,
...
// Range: 0x800DB014 -> 0x800DB310
static struct BINK * iFMVOpen(const char * fullFile /* r27 */, unsigned int uBinkFlags /* r28 */) {
    // Local variables
    struct RADARAMCALLBACKS OurGCCallbacks; // r1+0x8
    struct BINK * hBinkMovie; // r30
    int numRetries; // r29
    struct tag_xFile tfile; // r1+0x10
    enum xRegion reg; // r0
...
// Range: 0x800DB310 -> 0x800DB364
static void iFMVClose(struct BINK * hMovie /* r0 */) {
    // References
    // -> static unsigned char sARAMtmp;
}
...
static struct MOVIE * movie_array; // size: 0x4, address: 0x80C0792C
static int movie_asset_count; // size: 0x4, address: 0x80C07930
static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFF06C
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80BFF080
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x800DB60C -> 0x800DB960
struct MOVIE * iFMVOpen(struct PKRAssetTOCInfo & rAssetInfo /* r31 */) {
    // Local variables
    ...
    int width; // r31
    int height; // r4
    struct RwRaster * pBinkRaster; // r31
    struct RwRaster * pNullOnErr; // r0
...
// Range: 0x800DB960 -> 0x800DB97C
int NextPow2(int in /* r0 */) {
    // Local variables
    int ret; // r0
}

// Range: 0x800DB97C -> 0x800DBA10
...
// Range: 0x800DBA10 -> 0x800DBB70
unsigned int iFMVPlay(const char * filename /* r25 */, unsigned int buttons /* r27 */, float time /* f31 */, unsigned int uSubtitlesAID /* r28 */, unsigned char skippable /* r29 */, unsigned char lockController /* r30 */, unsigned char displayDemoBlink /* r31 */) {
    // Local variables
    char fullname[64]; // r1+0x18
    char BIK[5]; // r1+0x10
    char BIP[5]; // r1+0x8
    enum _GameOstrich old; // r26
    enum xRegion localRegion; // r3
    unsigned int ret; // r26
...
// Range: 0x800DBC2C -> 0x800DBD14
void iFMVResume() {
    // Local variables
    int i; // r30
    struct BINK * hBink; // r0
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80BFF08C
// total size: 0x2
...
// Range: 0x800DBD14 -> 0x800DC878
static unsigned int iFMVPlay(const char * filename /* r14 */, unsigned int buttons /* r15 */, unsigned int uSubtitlesAID /* r16 */, unsigned char skippable /* r1+0x18 */, unsigned char displayDemoBlink /* r17 */, int uAudioTrack /* r1+0x1C */) {
    // Local variables
    int j; // r19
    char fullFile[256]; // r1+0x3C
    struct BINK * hBinkMovie; // r25
    long long delay; // r0
    long long now; // r0
    unsigned char bTrackFound; // r6
    int track; // r0
    struct RwRaster * pCamRaster; // r24
    int iBackBufWidth; // r26
    int iBackBufHeight; // r0
    int iSafeWidth; // r9
    int iSafeHeight; // r3
    struct RwRect MovieRect; // r1+0x2C
    int iHorizPad; // r4
    int iVertPad; // r5
    struct RwCamera * cam; // r14
    struct RwRGBA bg; // r1+0x28
    long long fmvAbortDelay; // r18
    unsigned char aborted; // r23
    long long dwTimeNow; // r0
    void * xfbDisp; // r1+0x24
    void * xfbCopy; // r1+0x20
    void * xfbDest; // r22
    unsigned char bUseYUVPath; // r21
    struct RwRaster * pBinkRaster; // r20
    struct RwGameCubeRasterExtension * pRasterExt; // r19
    int iBinkMovieHalfHeight; // r14
    unsigned char waiting; // r18
    int i; // r18
    int oneIfSkip; // r0
    struct RwRaster * pNullOnErr; // r0
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFF098
// Range: 0x800DC878 -> 0x800DC8D4
void iFMVResetCallback() {
    // References
    // -> static char __FUNCTION__[18];
}

// Range: 0x800DC8D4 -> 0x800DC99C
void iFMVKill() {
    // Local variables
    int i; // r29
