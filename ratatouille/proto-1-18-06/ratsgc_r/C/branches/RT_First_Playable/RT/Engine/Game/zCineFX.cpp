/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zCineFX.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8025FADC -> 0x80260A00
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
// total size: 0x1
struct /* @class$1029zCineFX_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1030zCineFX_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1031zCineFX_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1032zCineFX_cpp */ {
    // Members
...
struct NCINMap {
    ...
    struct NCINEntry * cinfxtab; // offset 0x4, size 0x4
    unsigned int cinhash; // offset 0x8, size 0x4
};
static struct NCINMap * g_cutmap[64]; // size: 0x100, address: 0x80442E38
static char __FUNCTION__[13]; // size: 0xD, address: 0x80C05230
// total size: 0x14
struct NCINBeNosey : public XCSNNosey {
    // Functions
    void CanRenderNow();

    void UpdatedAnimated(struct RpAtomic * model, struct RwMatrixTag * animMat, unsigned int animIndex, unsigned int dataIndex, unsigned int & visFlags);

    void Init(const struct zCutsceneMgr * csnmgr, struct NCINEntry * fxtab, int flags);

    void Done();

    // Members
    struct zCutsceneMgr * use_csnmgr; // offset 0xC, size 0x4
    struct NCINEntry * use_fxtab; // offset 0x10, size 0x4
};
// Range: 0x8025FADC -> 0x8025FBF8
// this: r30
void NCINBeNosey::CanRenderNow() {
    // Local variables
    const struct zCutsceneMgr * csnmgr; // r31
    struct NCINEntry * fxtab; // r30
    struct NCINEntry * nextrec; // r30
    struct NCINEntry * fxrec; // r4

    // References
    // -> static char __FUNCTION__[13];
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80C05240
// Range: 0x8025FBF8 -> 0x8025FD40
// this: r25
void NCINBeNosey::UpdatedAnimated(struct RpAtomic * model /* r26 */, struct RwMatrixTag * animMat /* r27 */, unsigned int animIndex /* r28 */, unsigned int dataIndex /* r29 */, unsigned int & visFlags /* r30 */) {
    // Local variables
    const struct zCutsceneMgr * csnmgr; // r31
    struct NCINEntry * fxtab; // r25
    struct NCINEntry * nextrec; // r25
    struct NCINEntry * fxrec; // r4
...
static struct NCINBeNosey * g_noz_ncin; // size: 0x4, address: 0x80C092B0
static signed char init; // size: 0x1, address: 0x80C092B4
static struct NCINBeNosey nozey_npc_cinematics; // size: 0x14, address: 0x80C092B8
// Range: 0x8025FD40 -> 0x8025FD84
void zCineFXStartup() {
    // References
    // -> static struct NCINBeNosey nozey_npc_cinematics;
    // -> static struct NCINBeNosey * g_noz_ncin;
    // -> static signed char init;
}

// Range: 0x8025FDC8 -> 0x8025FDCC
void zCineFXShutdown() {}

// Range: 0x8025FDCC -> 0x8025FDEC
void zCineFXSceneSetup() {}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80C05250
// Range: 0x8025FDEC -> 0x8025FEEC
void zCineFXRegisterTable(struct NCINMap * maplist /* r3 */) {
    // Local variables
    struct NCINMap * maprec; // r28
    int entry_entered; // r5
    int i; // r4

    // References
    // -> static char __FUNCTION__[21];
    // -> static struct NCINMap * g_cutmap[64];
}

static char __FUNCTION__[25]; // size: 0x19, address: 0x80442F38
// Range: 0x8025FEEC -> 0x8026010C
static struct NCINEntry * zCineFXCutscenePickTable(const struct zCutsceneMgr * csnmgr /* r31 */) {
    // Local variables
    struct NCINEntry * da_fxtab; // r29
    int i; // r28
    struct NCINMap * cutrec; // r27
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80C05268
// Range: 0x8026010C -> 0x80260264
int zCineFXCutscenePrep(const struct zCutsceneMgr * csnmgr /* r30 */) {
    // Local variables
    struct NCINEntry * fxtab; // r31
    struct NCINEntry * nextrec; // r31
    struct NCINEntry * fxrec; // r30
...
// Range: 0x80260264 -> 0x80260274
// this: r0
void NCINBeNosey::Init(const struct zCutsceneMgr * csnmgr /* r0 */, struct NCINEntry * fxtab /* r0 */, int flags /* r0 */) {}
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80C0527C
// Range: 0x80260274 -> 0x80260424
void zCineFXCutsceneDone(const struct zCutsceneMgr * csnmgr /* r30 */) {
    // Local variables
    struct NCINEntry * fxtab; // r31
    struct NCINEntry * nextrec; // r31
    struct NCINEntry * fxrec; // r29
...
// Range: 0x80260424 -> 0x80260438
// this: r0
void NCINBeNosey::Done() {}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80C05290
// Range: 0x80260438 -> 0x80260774
void zCineFXCutscene(const struct zCutsceneMgr * csnmgr /* r26 */) {
    // Local variables
    struct xCutscene * csn; // r31
    struct NCINEntry * fxtab; // r3
    int need_animated; // r30
    int need_render; // r29
    struct NCINEntry * nextrec; // r28
    struct NCINEntry * fxrec; // r27
    int flags; // r0
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80C052A0
// total size: 0x28
struct PKRAssetType {
    // Members
...
// Range: 0x80260774 -> 0x8026086C
static void zCineFXTweakMenu() {
    // Local variables
    int i; // r30
    struct NCINMap * cutrec; // r29
    struct PKRAssetTOCInfo tocdata; // r1+0x8
    int rc; // r0
    char prefix[128]; // r1+0x28
    const struct tweak_info * skipMe; // r0
...
// Range: 0x8026086C -> 0x802609DC
static void zCineFXTweakThisTable(const char * prefix_root /* r31 */, struct NCINEntry * fxtab /* r4 */) {
    // Local variables
    int idx_entry; // r30
    struct NCINEntry * nextrec; // r29
    struct NCINEntry * fxrec; // r28
    char tweakBase[128]; // r1+0x188
    char prefix[128]; // r1+0x108
    char _buffer[128]; // r1+0x88
    char _buffer[128]; // r1+0x8
}

// Range: 0x802609DC -> 0x80260A00
static void AddTables() {}

// total size: 0x10
struct {} XCSNNosey::__vtable; // size: 0x10, address: 0x80C052B4
// total size: 0x8
struct RwObject {
    // Members
...
// total size: 0x10
struct {} NCINBeNosey::__vtable; // size: 0x10, address: 0x80C052C4
