/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xMorph.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8008AB68 -> 0x8008B080
*/
// total size: 0xC
struct RwV3d {
    // Members
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFD5B0
// total size: 0x8
struct RwObject {
    // Members
...
// Range: 0x8008AB68 -> 0x8008AE7C
struct xMorphSeqFile * xMorphSeqSetup(void * data /* r26 */, void * (* FindAssetCB)(unsigned int, char *) /* r29 */) {
    // Local variables
    int i; // r24
    int j; // r28
    struct RpAtomic * model; // r0
    struct xMorphTargetFile * mphtgt; // r27
    struct xMorphSeqFile * header; // r26
    struct xMorphFrame * framelist; // r27
    unsigned int * assetlist; // r25
    char * namelist; // r28
    void * assetPtr; // r0
    unsigned int skipsize; // r3
...
// Range: 0x8008AE7C -> 0x8008B06C
void xMorphRender(struct xMorphSeqFile * seq /* r0 */, struct RwMatrixTag * mat /* r4 */, float time /* f1 */) {
    // Local variables
    float lerp; // f2
    unsigned int tidx; // r6
    unsigned int tcount; // r0
    signed short weight[4]; // r1+0x8
    signed short * wptr; // r6
    float * times; // r7
    struct xMorphFrame * frame; // r5
}

// Range: 0x8008B06C -> 0x8008B080
float xMorphSeqDuration(struct xMorphSeqFile * seq /* r0 */) {}
