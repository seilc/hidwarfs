/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zGrass.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8019E2C0 -> 0x801A2F8C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
// total size: 0x1
struct /* @class$1059zGrass_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1060zGrass_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1061zGrass_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1062zGrass_cpp */ {
    // Members
...
static struct tweak_callback cb_reset; // size: 0x28, address: 0x803E5160
static struct tweak_callback cb_refresh_density; // size: 0x28, address: 0x803E5188
static float default_dist_fade; // size: 0x4, address: 0x80C02448
static float default_dist_cull; // size: 0x4, address: 0x80C0244C
static float density_scale; // size: 0x4, address: 0x80C02450
static float rand_width; // size: 0x4, address: 0x80C02454
static float rand_height; // size: 0x4, address: 0x80C02458
static float rand_intensity; // size: 0x4, address: 0x80C0245C
static int alpha_compare; // size: 0x4, address: 0x80C02460
static float clip_expand; // size: 0x4, address: 0x80C02464
static unsigned char disable_render; // size: 0x1, address: 0x80C088C8
static unsigned char use_game_camera; // size: 0x1, address: 0x80C088C9
static unsigned char render_mesh_wireframe; // size: 0x1, address: 0x80C088CA
static unsigned char render_mesh_bounds; // size: 0x1, address: 0x80C088CB
static unsigned char render_mesh_color; // size: 0x1, address: 0x80C088CC
static unsigned char render_mesh_density; // size: 0x1, address: 0x80C088CD
static unsigned char render_mesh_height; // size: 0x1, address: 0x80C088CE
static int render_cell; // size: 0x4, address: 0x80C02468
static int render_clone_bounds; // size: 0x4, address: 0x80C0246C
static int render_only_clone; // size: 0x4, address: 0x80C02470
static int clones_rendered; // size: 0x4, address: 0x80C088D0
static unsigned char stats_update; // size: 0x1, address: 0x80C088D4
static unsigned char force_dist_default; // size: 0x1, address: 0x80C088D5
// Range: 0x8019E2C0 -> 0x8019E73C
...
class zGrass * grass; // size: 0x4, address: 0x80C088D8
// total size: 0x18
struct analog_data {
    // Members
...
// Range: 0x8019E880 -> 0x8019E8B4
void zGrass::scene_exit() {
    // References
    // -> class zGrass * grass;
}

// Range: 0x8019E8B4 -> 0x8019E8C0
void zGrass::setup() {
    // References
    // -> class zGrass * grass;
}

// Range: 0x8019E8C0 -> 0x8019E9A0
void zGrass::reset() {
    // Local variables
...
// Range: 0x8019E9A0 -> 0x8019E9CC
void zGrass::update(float dt /* f0 */) {
    // References
    // -> class zGrass * grass;
}

// Range: 0x8019E9CC -> 0x8019E9D8
void zGrass::pre_render() {
    // References
    // -> class zGrass * grass;
}

// Range: 0x8019E9D8 -> 0x8019EA64
void zGrass::render() {
    // References
    // -> class zGrass * grass;
    // -> static float default_dist_cull;
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80C02474
// total size: 0x14
struct base_widget : public xBase {
    // Members
    struct knoll_type * knoll; // offset 0x10, size 0x4
};
// Range: 0x8019EA64 -> 0x8019EC54
void zGrass::load(struct xBase & data /* r29 */, struct xDynAsset & asset /* r4 */, unsigned long asset_size /* r30 */) {
    // Local variables
...
// Range: 0x8019EC54 -> 0x8019ECC0
void zGrass::event_handler(struct xBase * to /* r0 */, unsigned int event /* r0 */) {
    // References
    // -> class zGrass * grass;
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80C0247C
// total size: 0x8
struct xModelPipe {
    // Members
    unsigned int Flags; // offset 0x0, size 0x4
...
// Range: 0x8019ECC0 -> 0x8019F1AC
void zGrass::create_knolls(struct knoll_type * & knolls /* r25 */, int & knolls_size /* r26 */) {
    // Local variables
    unsigned int type; // r0
    int asses; // r0
    int i; // r28
    unsigned int asset_size; // r1+0x14
    struct asset_type * asset; // r27
    ...
    unsigned char prelight_only; // r22
...
// Range: 0x8019F2B8 -> 0x8019F2C0
struct RpAtomic * zGrass::setup_atomic(struct RpAtomic * atomic /* r4 */) {}

// Range: 0x8019F2C0 -> 0x8019F31C
void zGrass::destroy_atomic(struct RpAtomic * atomic /* r30 */) {
    // Local variables
    struct RwFrame * frame; // r31
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80C0248C
// Range: 0x8019F31C -> 0x8019F4D4
// this: r29
void zGrass::create_caches() {
    // Local variables
    struct RpAtomic * atomics_found[8]; // r1+0x8
    struct knoll_type * knoll; // r31
    struct knoll_type * end_knoll; // r30
    struct RpAtomic * * t; // r5
    struct RpAtomic * * endt; // r6
    struct grass_cache * cache; // r31
...
// Range: 0x8019F544 -> 0x8019F554
void zGrass::reset_cache(struct grass_cache & cache /* r0 */) {}

// Range: 0x8019F554 -> 0x8019F5B8
// this: r31
void zGrass::reset_view_area() {}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80C0249C
// Range: 0x8019F5B8 -> 0x8019F650
// this: r30
...
// Range: 0x8019F7FC -> 0x8019F864
// this: r29
void zGrass::refresh_density() {
    // Local variables
    union grass_mesh_tri * tri; // r31
    union grass_mesh_tri * end_tri; // r30
}

// Range: 0x8019F864 -> 0x8019F928
// this: r30
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x80C024A8
// Range: 0x8019F928 -> 0x8019FD80
// this: r23
void zGrass::refresh_grid_cell_boxes() {
    ...
    const union grass_mesh_tri * const end_tri; // r24
    const struct knoll_type & knoll; // r7
    float atomic_radius; // f5
    const struct zGrassMeshVertex & v0; // r8
    float xtrilower; // f6
    float xtriupper; // f7
    float ytrilower; // f8
    float ytriupper; // f9
    ...
    float maxheight; // f12
    int vi; // r0
    const struct zGrassMeshVertex & v; // r9
    float height; // f2
    float width; // f0
    float radius; // f0
    struct xBox & box; // r24
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80C024C0
// Range: 0x8019FD80 -> 0x801A040C
// this: r22
void zGrass::partition_meshes() {
    ...
    float zupper; // f31
    int grid_size; // r28
    union grass_mesh_tri * otri; // r29
    union grass_mesh_tri * const end_otri; // r27
    int i; // r26
    struct knoll_type & knoll; // r25
    struct zGrassMeshFace * itri; // r24
    struct zGrassMeshFace * end_itri; // r23
    int grid_index; // r0
    int tri_index; // r23
    int i; // r24
    int tri_size; // r21
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80C024D4
// total size: 0x30
struct xMat3x3 {
    // Members
...
void zGrass::emit_clones(struct clone_node * & clones /* r18 */, struct clone_node * & last_clone /* r19 */, const union grass_mesh_tri & tri /* r20 */) {
    ...
    float rand_intensity_min; // f25
    float rand_intensity_scale; // f24
    float width_min; // f23
    float height_min; // f22
    float animate; // f21
    float s; // r1+0x14
    float t; // r1+0x10
    float v; // f20
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80C024E0
// Range: 0x801A0B50 -> 0x801A0D2C
// this: r29
void zGrass::populate_cell(struct view_cell & vcell /* r30 */, int grid_index /* r0 */) {
    // Local variables
    struct grid_cell & gcell; // r31
    const union grass_mesh_tri * tri; // r31
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80C024F0
// Range: 0x801A0D2C -> 0x801A0DE4
// this: r30
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C024FC
// Range: 0x801A0DE4 -> 0x801A1370
// this: r15
void zGrass::displace_view_area(int new_xlower /* r16 */, int new_xupper /* r17 */, int new_zlower /* r18 */, int new_zupper /* r19 */) {
    ...
    int old_zlower; // r23
    int old_zupper; // r22
    int begin_ix; // r1+0x10
    int end_ix; // r14
    int begin_iz; // r4
    int end_iz; // r1+0xC
    int next_cells; // r1+0x8
    int iz; // r21
    int ix; // r20
    struct view_cell & vcell; // r3
    struct view_cell & vcell; // r0
    struct view_cell & icell; // r26
...
// Range: 0x801A1BAC -> 0x801A1C94
// this: r25
void zGrass::render_clones(struct RpAtomic * atomic /* r26 */, struct clone_node * * output /* r5 */, int size /* r27 */) {
    // Local variables
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80C02510
// Range: 0x801A1C94 -> 0x801A1E9C
// this: r19
void zGrass::render_view_area() {
    ...
    int sizex; // r27
    int sizez; // r26
    struct clone_node * * output_buffer; // r25
    int output_size[8]; // r1+0x8
    int i; // r5
    int viz; // r24
    int vix; // r23
    struct clone_node * clone; // r22
    int cache_index; // r21
    int & size; // r20
...
// Range: 0x801A1E9C -> 0x801A1F18
// this: r31
void zGrass::create_white_light() {}

// Range: 0x801A1F18 -> 0x801A1F40
// this: r3
void zGrass::destroy_white_light() {}

// Range: 0x801A1F40 -> 0x801A2024
// this: r3
...
static char * orient_labels[3]; // size: 0xC, address: 0x80C02524
// Range: 0x801A21D0 -> 0x801A24D4
void zGrass::add_knoll_tweaks(struct knoll_type & knoll /* r30 */) {
    // Local variables
    const char * name; // r5
...
void zGrass::debug_render() {
    ...
    float x; // f28
    float y; // f7
    const union grass_mesh_tri * tri; // r22
    const union grass_mesh_tri * const end_tri; // r21
    const struct xBox & box; // r18
    int tri_passes; // r21
    int total_tris; // r22
    int begin_ix; // r23
    int end_ix; // r24
    ...
    int ix; // r28
    int grid_index; // r0
    struct RxObjSpace3DVertex * vert_buffer; // r21
    struct RxObjSpace3DVertex * vert; // r22
    int iz; // r25
    int ix; // r27
    int grid_index; // r0
    struct grid_cell & gcell; // r3
    const union grass_mesh_tri * tri; // r28
    const union grass_mesh_tri * const end_tri; // r29
    const struct zGrassMeshVertex & v0; // r20
    const struct zGrassMeshVertex & v1; // r19
    const struct zGrassMeshVertex & v2; // r18
    int grid_size; // r3
    int ix; // r21
    struct grid_cell & cell; // r18
    float x; // f28
    float y; // f5
    float z; // f3
    const union grass_mesh_tri * tri; // r18
...
// Range: 0x801A2D64 -> 0x801A2D84
static void set_vert(struct RxObjSpace3DVertex & vert /* r0 */, float x /* f0 */, float y /* f0 */, float z /* f0 */, unsigned char r /* r0 */, unsigned char g /* r0 */, unsigned char b /* r0 */, unsigned char a /* r0 */) {}

// Range: 0x801A2D84 -> 0x801A2DB0
static void set_vert(struct RxObjSpace3DVertex & vert /* r0 */, const struct xVec3 & loc /* r0 */, unsigned char r /* r0 */, unsigned char g /* r0 */, unsigned char b /* r0 */, unsigned char a /* r0 */) {}
...
// Range: 0x801A2DB0 -> 0x801A2DD0
void zGrass::on_reset() {}

// Range: 0x801A2DD0 -> 0x801A2DF8
void zGrass::on_refresh_density() {
    // References
    // -> class zGrass * grass;
}

// Range: 0x801A2DF8 -> 0x801A2E68
void zGrass::on_refresh_wave_dir(const struct tweak_info & info /* r0 */) {
    // Local variables
    struct wave_params & wave; // r31
    float angle; // f31
}

static struct tweak_callback cb_refresh_wave_dir; // size: 0x28, address: 0x803E51B0
// Range: 0x801A2E68 -> 0x801A2F8C
// this: r3
void zGrass::add_wave_tweaks() {
    // Local variables
    const char * root_prefix; // r27
    int i; // r26
