/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zMain.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80139D34 -> 0x8013D174
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct basic_rect screen_bounds; // size: 0x10, address: 0x80C0C0A8
enum eFMVFile {
    eFMVFile_NoMovie = 0,
...
unsigned int FB_DEPTH; // size: 0x4, address: 0x80C00BE4
unsigned int SCENE_ID_MNU_INGAME; // size: 0x4, address: 0x80C08214
...
// Range: 0x80139D34 -> 0x80139D40
void zMainSetPAL() {
    // References
    // -> enum _tagGameVidMode gVidMode;
}

// Range: 0x80139D40 -> 0x80139D4C
void zMainSetNTSC() {
    // References
    // -> enum _tagGameVidMode gVidMode;
}
...
static int sShowMenuOnBoot; // size: 0x4, address: 0x80C00BEC
// total size: 0x8
...
static struct st_SERIAL_PERCID_SIZE g_xser_sizeinfo[2]; // size: 0x10, address: 0x80C00BF0
int gDrawInvisibleWalls; // size: 0x4, address: 0x80C08228
static int gCountryCodeCount; // size: 0x4, address: 0x80C0822C
static char gCountryCodes[4][3]; // size: 0xC, address: 0x80C08230
...
// Range: 0x80139DF8 -> 0x80139F20
static void set_rect_verts(struct rwGameCube2DVertex * vert /* r31 */, float x /* f25 */, float y /* f26 */, float w /* f27 */, float h /* f30 */, float z /* f28 */, float ooz /* f29 */) {}

// Range: 0x80139F20 -> 0x80139F4C
static void set_rect_vert(struct rwGameCube2DVertex & vert /* r0 */, float x /* f0 */, float y /* f0 */, float z /* f0 */, float u /* f0 */, float v /* f0 */) {}

static char __FUNCTION__[26]; // size: 0x1A, address: 0x8043A540
// Range: 0x80139F4C -> 0x8013A06C
static void zMainLanguageSelectionINI(struct xIniFile * ini /* r31 */) {
    // Local variables
    const char * _buf; // r29
    int index; // r1+0x8
...
static float sAttractMode_timer; // size: 0x4, address: 0x80C0823C
static signed char init; // size: 0x1, address: 0x80C08240
static char __FUNCTION__[23]; // size: 0x17, address: 0x80C00C00
...
// Range: 0x8013A06C -> 0x8013A7C4
static void zMainLanguageSelection() {
    // Local variables
    float flagXRES; // f3
    float flagWidth; // f23
    float flagSpacingX; // f24
    float flagSpacingY; // f25
    float flagHeight; // f22
    float flagStartX; // f21
    float flagStartY; // f20
    struct RwTexture * textures[4][2]; // r1+0x38
    int highlighted; // r30
    const char * defaultRegion; // r24
    int i; // r25
    unsigned int countryHash; // r16
    struct RwTexture * texture; // r0
    struct RwRaster * background; // r29
    long long prevTime; // r21
    float dt; // f19
    unsigned char pressedA; // r28
    unsigned char pressedStart; // r27
    unsigned char pressedUp; // r19
    unsigned char pressedDown; // r18
    unsigned char pressedLeft; // r17
    unsigned char pressedRight; // r16
    unsigned char exitingFMV; // r26
    struct RwCamera * cam; // r25
    struct RwRGBA bg; // r1+0xC
    struct basic_rect bounds; // r1+0x28
    unsigned char FMVinput; // r15
    int startidx; // r1+0x8
    int port; // r0
    int i; // r15
    float flayY; // f2
    ...
    long long time; // r15
    struct xCamScreen local_screen; // r1+0x10
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80C00C18
// Range: 0x8013A7C4 -> 0x8013AD3C
void main(int argc /* r31 */, char * * argv /* r24 */) {
    // Local variables
    const char * ini; // r26
    int i; // r25
    const char * arg; // r24
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80C00C20
// Range: 0x8013AD3C -> 0x8013B068
static void zMainOutputMgrSetup() {
    // References
    // -> static char __FUNCTION__[20];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80C00C34
// Range: 0x8013B068 -> 0x8013B0E8
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80C00C48
// Range: 0x8013B0E8 -> 0x8013B9B4
static void zMainParseINIGlobals(struct xIniFile * ini /* r29 */) {
    // Local variables
    const char * SCENE_ID_MNU_START_TEXT; // r0
    const char * videosys; // r30
    char DEFAULT_REGION_NTSC[3]; // r1+0x10
    char DEFAULT_REGION_PAL[3]; // r1+0xC
    const char * defaultregion; // r28
    const char * iniString; // r30
    int idx; // r1+0x8
    int playerCount; // r27
    unsigned int sceneTag; // r26
    unsigned int playerTag; // r4
...
// Range: 0x8013B9B4 -> 0x8013BA18
static void zMainRegisterDebugModeCallbacks() {}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80C00C60
// Range: 0x8013BA18 -> 0x8013BAC8
static void zMainMemLvlChkCB() {
    // References
    // -> static char __FUNCTION__[17];
}

static unsigned int gameSceneID; // size: 0x4, address: 0x80C08244
static signed char init; // size: 0x1, address: 0x80C08248
static unsigned int menuModeID; // size: 0x4, address: 0x80C0824C
static signed char init; // size: 0x1, address: 0x80C08250
static char __FUNCTION__[10]; // size: 0xA, address: 0x80C00C74
// total size: 0x10
struct zVarEntry {
    // Members
...
int sFirstBoot; // size: 0x4, address: 0x80C00C90
...
// Range: 0x8013BAC8 -> 0x8013BF7C
static void zMainLoop() {
    // Local variables
    long long t; // r26
    unsigned int newGameSceneID; // r29
...
// total size: 0x1
struct /* @class$1055zMain_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1056zMain_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1057zMain_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1058zMain_cpp */ {
    // Members
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80C00C80
unsigned int gSoak; // size: 0x4, address: 0x80C07E3C
char * allsoaklevels[]; // size: 0x0, address: 0x80BC90A0
char * * soaklevels; // size: 0x4, address: 0x80C07E68
int enableAudioAnnihilator; // size: 0x4, address: 0x80C07E70
char gScreenDumpPath[]; // size: 0x0, address: 0x8045A0D0
int enableSuperSoaker; // size: 0x4, address: 0x80C07E74
int enableMenuMasher; // size: 0x4, address: 0x80C07E6C
// Range: 0x8013BF7C -> 0x8013C9F8
static void zMainReadINI(const char * iniName /* r0 */) {
    // Local variables
    const char * str; // r0
    void * buf; // r31
    unsigned int size; // r1+0x8
    struct xIniFile * ini; // r30
    unsigned int num_levels; // r0
    char * tok; // r24
    int level_count; // r27
    const char * pbackfile; // r3
    const char * screendumppath; // r27
    unsigned int len; // r0
    const char * watermark; // r3
...
// Range: 0x8013C9F8 -> 0x8013CBB0
static void zMainReadMemReserveINI(const char * iniName /* r25 */) {
    // Local variables
    void * buf; // r0
    unsigned int size; // r1+0x8
    struct xIniFile * ini; // r0
    int j; // r27
    char * disabledList; // r26
    int numScenes; // r0
    int i; // r25
}
...
// Range: 0x8013CBB0 -> 0x8013CBE8
static void zMainLoadFontHIP() {}

static char buffer[128]; // size: 0x80, address: 0x80B9F710
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x8013CD98 -> 0x8013CDA8
char * zStealthDebugGetAssertionHeader() {}

int percentageDone; // size: 0x4, address: 0x80C08254
static class xtextbox backGroundTexture; // size: 0x74, address: 0x8043A55C
static signed char init; // size: 0x1, address: 0x80C08258
int sFirstBoot; // size: 0x4, address: 0x80C00C90
// Range: 0x8013CDA8 -> 0x8013D0D8
void zMainShowProgressBar(int percentage /* r0 */) {
    // Local variables
    const char * loadingText; // r30
    int spotToInsertColor; // r31
