/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xParEmitter.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80092F98 -> 0x80096D4C
*/
// total size: 0x8
struct xBaseAsset {
    // Members
...
static struct xParEmitterAsset sSaveEmmiterSettings; // size: 0x54, address: 0x80A927B8
// total size: 0x138
...
static struct xParEmitterPropsAsset sSaveEmmiterPropSettings; // size: 0x138, address: 0x80A9280C
static struct xParEmitterPropsAsset sDummyProp; // size: 0x138, address: 0x80A92944
// Range: 0x80092F98 -> 0x8009303C
int xParInterpConvertInterpMode(struct xParInterp * p /* r0 */) {}

// Range: 0x8009303C -> 0x8009305C
void xParEmitterInit(void * b /* r0 */, void * tasset /* r0 */) {}
...
// Range: 0x8009305C -> 0x80093278
void xParEmitterInit(struct xBase * b /* r30 */, struct xParEmitterAsset * pea /* r31 */) {
    // Local variables
    struct xParEmitterPropsAsset * prop; // r27
    struct xParEmitter * t; // r0
    int i; // r26
    float mag2; // f0
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFD8C0
// total size: 0x50
...
// total size: 0x4
struct /* @class$1140xParEmitter_cpp */ {
    // Members
...
union /* @class$1279xParEmitter_cpp */ {
...
// total size: 0xC
struct /* @class$1278xParEmitter_cpp */ {
    // Members
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFD8D4
// Range: 0x800933E4 -> 0x800934BC
...
// total size: 0x0
struct xSerial {};
static char __FUNCTION__[19]; // size: 0x13, address: 0x80BFD8E8
// total size: 0x184
...
// Range: 0x800934BC -> 0x800935C4
void xParEmitterEventCB(struct xBase * to /* r4 */, unsigned int toEvent /* r30 */) {
    // Local variables
    struct xParEmitter * t; // r31
    struct xParEmitterCustomSettings fake_settings; // r1+0x8

    // References
    // -> static char __FUNCTION__[19];
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x80BFD8FC
// Range: 0x800935C4 -> 0x8009391C
struct xPar * xParEmitterEmitCustom(struct xParEmitter * p /* r27 */, float dt /* f31 */, struct xParEmitterCustomSettings * info /* r28 */) {
    // Local variables
    unsigned int flags; // r30
    struct xParEmitterAsset * pe; // r29
    struct xPar * emitted_par; // r28
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFD914
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// total size: 0x10
struct /* @class$1543xParEmitter_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x8009391C -> 0x80093A64
static int xParEmitterCull(struct xParEmitter * pe /* r29 */, struct xPar * p /* r30 */) {
    // Local variables
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFD924
// Range: 0x80093A64 -> 0x80093BFC
float xParInterpCompute(int interp_mode /* r0 */, struct xParInterp * r /* r31 */, float time /* f31 */, int time_has_elapsed /* r0 */, float lastVal /* f2 */) {
    // Local variables
    float val; // f1
...
static char __FUNCTION__[26]; // size: 0x1A, address: 0x80419714
// Range: 0x80093BFC -> 0x80093DAC
struct xPar * xParEmitterEmitSetTexIdxs(struct xPar * p /* r30 */, const struct xParSys * ps /* r31 */) {
    // Local variables
    struct xParCmdTex * tex; // r31

    // References
    // -> static char __FUNCTION__[26];
}

static int doDebugDraw; // size: 0x4, address: 0x80C07530
static signed char init; // size: 0x1, address: 0x80C07534
static float rad_cullDebugDraw; // size: 0x4, address: 0x80C07538
static signed char init; // size: 0x1, address: 0x80C0753C
// total size: 0x70
struct xPar {
    // Members
...
static struct xParGroup temp_group; // size: 0x24, address: 0x80A92A7C
static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFD938
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x80093DAC -> 0x80094ADC
struct xPar * xParEmitterEmit(struct xParEmitter * pe /* r15 */, float emit_dt /* f28 */, float par_dt /* f29 */) {
    // Local variables
    struct xPar * last_p; // r1+0x64
    struct xParEmitterAsset * pea; // r29
    struct xParEmitterPropsAsset * prop; // r28
    int rate_has_elapsed; // r14
    float rate; // f0
    int count; // r27
    struct xParSys * ps; // r26
    struct xParGroup * g; // r0
    struct xPar * p; // r25
    float life; // f0
    float size_birth; // f0
    float size_death; // f0
    struct xVec3 emitPosition; // r1+0x4C
    struct xBase * attachObject; // r14
    unsigned int attachGroupIndex; // r24
    unsigned int attachGroupTotal; // r1+0x60
    int emitAgain; // r23
    int emitterIsAttachedToObject; // r22
    struct xBase * emitObj; // r21
    int marker; // r16
    struct xGroup * g; // r14
    unsigned int get_rnd_group_idx; // r17
    const struct xEnt * attach_ent; // r20
    const struct xMat4x3 * bone_mat; // r19
    struct xVec3 bone_vel; // r1+0x40
    struct xVec3 add_vel; // r1+0x34
    struct xVec3 diff; // r1+0x28
    float ds2; // f0
    float last_emit_time; // f30
    const struct xVec3 & camPos; // r18
    int i; // r17
    int c; // r16
    float fc1; // f0
    float fc2; // f0
    struct xBase * obj; // r16
    struct xPar * save_next; // r16
    struct xPar * save_prev; // r30
    float u; // f27
    struct xVec3 pos_diff; // r1+0x1C
    float initial_length; // f25
...
// total size: 0x890
struct /* @class$2051xParEmitter_cpp */ {
    // Static members
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFD948
// Range: 0x80094ADC -> 0x80094BCC
void xParEmitterUpdate(struct xBase * to /* r3 */, float dt /* f31 */) {
    // Local variables
...
static char * region_type_names[17]; // size: 0x44, address: 0x80419774
static char * interp_type_names[8]; // size: 0x20, address: 0x804197B8
static char * render_type_names[7]; // size: 0x1C, address: 0x804197D8
static char buffer[16]; // size: 0x10, address: 0x80C07540
// Range: 0x80094BCC -> 0x80094C30
static char * get_asset_name(struct xParEmitterAsset * asset /* r31 */) {
    // Local variables
    const char * name; // r0
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFD95C
// total size: 0xC
struct /* @class$952xParEmitter_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$953xParEmitter_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$954xParEmitter_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$955xParEmitter_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$956xParEmitter_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$957xParEmitter_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$958xParEmitter_cpp */ {
    // Members
...
// Range: 0x80094C30 -> 0x80094CEC
static void on_change_circle_dir(const struct tweak_info & info /* r31 */) {
    // Local variables
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x80BFD974
// Range: 0x80094CEC -> 0x80094E24
static void on_change_volume_entity(const struct tweak_info & info /* r30 */) {
    // Local variables
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFD98C
// Range: 0x80094E24 -> 0x80094EE8
static void on_change_bone_index(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xParEmitter & emit; // r3
    struct xPEEntBone & region; // r0
    struct xEnt * ent; // r30
...
static struct tweak_callback cb_volume_entity; // size: 0x28, address: 0x804197F4
static signed char init; // size: 0x1, address: 0x80C07550
static struct tweak_callback cb_circle_dir; // size: 0x28, address: 0x8041981C
static signed char init; // size: 0x1, address: 0x80C07551
static char * type_labels[11]; // size: 0x2C, address: 0x80419844
static struct tweak_callback cb_bone_index; // size: 0x28, address: 0x80419870
static signed char init; // size: 0x1, address: 0x80C07552
static char * type_labels[2]; // size: 0x8, address: 0x80BFD9A4
static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFD9AC
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x80BFD9C0
// Range: 0x80095A18 -> 0x80095A9C
static void on_change_region_type(const struct tweak_info & info /* r31 */) {
    // References
    // -> static char __FUNCTION__[22];
}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFD9D8
// Range: 0x80095A9C -> 0x80095C04
static void on_change_cmd_count(const struct tweak_info & info /* r26 */) {
    // Local variables
    struct xParEmitter & emit; // r3
    struct xParSys & sys; // r29
    char prefix[128]; // r1+0x8
    char * end; // r28
    const struct tweak_info * parent; // r0
    unsigned int size; // r30
    unsigned int i; // r27
    struct xParCmd & cmd; // r26
    unsigned int i; // r26
...
// Range: 0x80095C04 -> 0x80095D54
static void add_interp_tweak(const char * cat /* r3 */, const char * title /* r4 */, struct xParInterp & pi /* r31 */) {
    // Local variables
    char prefix[128]; // r1+0x208
...
// Range: 0x80095D54 -> 0x80095D58
static void verify_tables() {}

static char __FUNCTION__[22]; // size: 0x16, address: 0x80BFD9EC
// Range: 0x80095D58 -> 0x80095DE8
static void on_change_render_type(const struct tweak_info & info /* r31 */) {
    // Local variables
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFDA04
// Range: 0x80095DE8 -> 0x80095E88
static void on_change_attach_to(const struct tweak_info & info /* r31 */) {
    // Local variables
...
// Range: 0x80095E88 -> 0x80095ED8
static void on_change_texture(const struct tweak_info & info /* r0 */) {
    // Local variables
    struct xParSys & sys; // r31
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFDA18
// Range: 0x80095ED8 -> 0x80095F54
static void on_expand_emitter(const struct tweak_info & info /* r0 */) {
    // Local variables
    struct xParEmitter * emit; // r31
...
static struct tweak_callback cb_explode; // size: 0x28, address: 0x804198DC
static signed char init; // size: 0x1, address: 0x80C07553
// Range: 0x80095F54 -> 0x80096044
...
static struct tweak_callback cb_emit_type; // size: 0x28, address: 0x80419904
static signed char init; // size: 0x1, address: 0x80C07554
static struct tweak_callback cb_attach_to; // size: 0x28, address: 0x8041992C
static signed char init; // size: 0x1, address: 0x80C07555
static struct tweak_callback cb_render_type; // size: 0x28, address: 0x80419954
static signed char init; // size: 0x1, address: 0x80C07556
static struct tweak_callback cb_change_texture; // size: 0x28, address: 0x8041997C
static signed char init; // size: 0x1, address: 0x80C07557
static struct tweak_callback cb_cmd_count; // size: 0x28, address: 0x804199A4
static signed char init; // size: 0x1, address: 0x80C07558
static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFDA2C
...
static void explode_tweaks(struct xParEmitter & emit /* r31 */) {
    ...
    struct xParEmitterAsset & asset; // r29
