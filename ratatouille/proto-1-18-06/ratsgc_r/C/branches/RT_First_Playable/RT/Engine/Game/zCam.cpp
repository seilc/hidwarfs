/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zCam.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80237B14 -> 0x802399CC
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
// Range: 0x80237B14 -> 0x80237CD8
// this: r31
struct xVec3 & zTargetBlender::Update(float dt /* f31 */) {
    // Local variables
    float t; // f31
}

// Range: 0x80237CD8 -> 0x80237CF8
// this: r0
struct xVec3 & zTargetBlender::operator const xVec3&() const {}

// Range: 0x80237CF8 -> 0x80237D00
// this: r3
struct xVec3 & zTargetBlender::Result() const {}

// Range: 0x80237D00 -> 0x80237D18
void zTargetBlender::blend(float & o /* r0 */, float a /* f0 */, float b /* f0 */, float s /* f0 */) {}

float sBlendTime; // size: 0x4, address: 0x80C048A0
// Range: 0x80237D18 -> 0x80237D20
float zTargetBlender::BlendTime() {
    // References
    // -> float sBlendTime;
}

// Range: 0x80237D20 -> 0x80237D6C
void zTargetBlender::AddTweaks(const char * prefix /* r4 */) {
    // References
    // -> float sBlendTime;
}

// total size: 0xC
struct /* @class$1028zCam_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$1031zCam_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1032zCam_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1033zCam_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1034zCam_cpp */ {
    // Members
...
static unsigned int input_disabled; // size: 0x4, address: 0x80C09030
static unsigned char have_default_orientation; // size: 0x1, address: 0x80C09034
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
static struct xVec3 default_orientation; // size: 0xC, address: 0x80C048A4
static struct zCamPlayer * follow_cam; // size: 0x4, address: 0x80C09038
// total size: 0x18
struct analog_data {
    // Members
...
static struct xCamGroup global_cam; // size: 0xE0, address: 0x80BCADF0
...
static struct xCamScreen global_screen; // size: 0x8, address: 0x80C0903C
static class zTargetBlender blendedLookat; // size: 0x48, address: 0x80BCAED0
static class zTargetBlender blendedMidPoint; // size: 0x48, address: 0x80BCAF18
static class zTargetBlender blendedFloorMount; // size: 0x48, address: 0x80BCAF60
static unsigned char sYTrackingBelowFloorEnabled; // size: 0x1, address: 0x80C09044
static unsigned char firstFrame; // size: 0x1, address: 0x80C048B0
static struct xVec3 lastCameraLookAt; // size: 0xC, address: 0x80C09048
static unsigned char debug_cam_active; // size: 0x1, address: 0x80C09054
static struct xCamGroup debug_cam_group; // size: 0xE0, address: 0x80BCAFB0
// total size: 0x70
...
static struct xCamDebug * debug_cam; // size: 0x4, address: 0x80C09058
static unsigned char debug_cam_triggers; // size: 0x1, address: 0x80C0905C
static unsigned char allow60fpsEmulation; // size: 0x1, address: 0x80C048B1
// total size: 0x10
...
// Range: 0x80237E90 -> 0x80237EB8
static void reset_debug_cam() {
    // References
    // -> static struct xCamGroup debug_cam_group;
}

// Range: 0x80237EB8 -> 0x80238048
...
// Range: 0x80238048 -> 0x802381E8
void zCamSceneEnter(struct zScene & scene /* r31 */) {
    // References
    // -> struct xVec3 m_Null;
    // -> struct zGlobals globals;
...
// Range: 0x80238368 -> 0x80238468
void zCamReset(unsigned char refreshCameraTarget /* r31 */) {
    // References
    // -> struct xGlobals * xglobals;
    // -> struct zGlobals globals;
...
struct xSerial {};
// total size: 0x4
struct /* @class$2353zCam_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2354zCam_cpp */ {
    // Members
...
// Range: 0x80238468 -> 0x80238478
// this: r0
void zTargetBlender::SetBlendTime(float time /* f0 */, unsigned char lazy /* r0 */) {
    // References
    // -> float sBlendTime;
}

// Range: 0x80238478 -> 0x802384BC
// this: r31
void zTargetBlender::Reset() {}

// Range: 0x802384BC -> 0x80238668
...
// Range: 0x80238668 -> 0x80238688
static void zCamRestartDefaultCamera() {}

// Range: 0x80238688 -> 0x802386B8
void zCamSceneSetup() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x802386B8 -> 0x80238860
void zCamHalfAngleIntercept(struct xVec3 & o /* r28 */, const struct xVec3 & A /* r29 */, const struct xVec3 & B /* r30 */, const struct xVec3 & C /* r31 */) {
    // Local variables
    ...
    float angleA2; // f28
    float angleB; // f0
    float lengthBCPrime; // f0
}

// Range: 0x80238860 -> 0x80238870
struct xVec3 & zCamLookPlayer() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x80238870 -> 0x80238880
struct xVec3 & zCamTransitionTarget() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x80238880 -> 0x80238888
void zCamSetYTracking(unsigned char enable /* r0 */) {
    // References
    // -> static unsigned char sYTrackingBelowFloorEnabled;
}

// Range: 0x80238888 -> 0x802388B0
unsigned char zCamIsChangingFocus() {
    // References
    // -> static class zTargetBlender blendedLookat;
}

// Range: 0x802388B0 -> 0x802388B8
// this: r3
unsigned char zTargetBlender::IsBlending() {}

// Range: 0x802388B8 -> 0x80238978
static float GetTargetFloorHeight(int whichTarget /* r3 */) {
    // Local variables
    struct zCommonPlayer * player; // r31
    float mountY; // f31
    float dyFromMount; // f2
...
// Range: 0x802389A4 -> 0x80238B6C
static void zCamCollideTriggersCamera() {
    // Local variables
    struct xScene & s; // r27
    struct xSphere camSphere; // r1+0x14
    struct xVec3 dummyDir; // r1+0x8
    struct zEntTrigger * * it; // r31
    struct zEntTrigger * * end; // r30
    struct zEntTrigger & trig; // r29
    const struct xTriggerAsset & asset; // r0
    unsigned char want_enter; // r3
    unsigned char want_exit; // r4
    const struct xLinkAsset * link; // r5
    const struct xLinkAsset * end_link; // r6
    unsigned char inside; // r0
...
static char __FUNCTION__[29]; // size: 0x1D, address: 0x80440B88
// total size: 0x24
struct xRay3 {
    // Members
    struct xVec3 origin; // offset 0x0, size 0xC
...
// Range: 0x80238B6C -> 0x80238F54
static void zCamCollideTriggersCamTarget(struct xScene * sc /* r24 */) {
    // Local variables
    ...
    int i; // r28
    struct zEntTrigger * trig; // r27
    struct xTriggerAsset * tasset; // r26
    struct xRay3 ray; // r1+0x70
    unsigned char end_inside; // r1+0x8
    unsigned char collide; // r25
    ...
    int numVolumes; // r0
    int i; // r26
    class xCamTransition * camTransition; // r27
...
// Range: 0x80239280 -> 0x802392C0
// this: r30
void zTargetBlender::SetGoal(const struct xVec3 & position /* r0 */, int numTargets /* r31 */) {}

// Range: 0x802392C0 -> 0x80239414
static void zCamDoAnalogInput() {
    // Local variables
    struct zPlayer * human; // r0
    class zPad * pad; // r0
    class zPadAnalogControl * analog; // r31
...
// Range: 0x80239414 -> 0x802395B4
void zCamUpdate(float dt /* f28 */) {
    // Local variables
    float step; // f30
    float remain; // f29
    unsigned char InDebugCam; // r0
...
// Range: 0x802395B4 -> 0x802395E0
void zCamPrepareRender() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x802395E0 -> 0x80239610
void zCamBeginRender() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x80239610 -> 0x8023965C
void zCamEndRender() {
    // References
    // -> struct zGlobals globals;
    // -> static unsigned char debug_cam_active;
...
// Range: 0x8023965C -> 0x80239668
void zCamExitDebugCam() {
    // References
    // -> static unsigned char debug_cam_active;
}

// Range: 0x80239668 -> 0x80239698
void zCamAdd(struct xCam & cam /* r4 */, unsigned char force_cut /* r5 */) {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x80239698 -> 0x802396C8
void zCamRemove(struct xCam & cam /* r4 */, unsigned char force_cut /* r5 */) {
    // References
    // -> struct zGlobals globals;
}

static char __FUNCTION__[24]; // size: 0x18, address: 0x80C048B4
// Range: 0x802396C8 -> 0x802398FC
void zCamSetStartOrientation(unsigned int pointer_id /* r29 */) {
    // Local variables
...
// Range: 0x802398FC -> 0x8023994C
void zCamSetStartOrientation(float yaw /* f31 */, float pitch /* f0 */, float roll /* f0 */) {
    // References
    // -> static struct zCamPlayer * follow_cam;
...
// Range: 0x8023994C -> 0x80239960
struct xCam * zCamGetPrimary() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x80239960 -> 0x80239974
float zCamGetCameraPlayerAudioBias() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x80239974 -> 0x80239998
...
// Range: 0x80239998 -> 0x802399C4
unsigned char zCamGetDebugMode(struct xMat4x3 * * ppDebugCamMtx /* r0 */) {
    // References
    // -> static unsigned char debug_cam_active;
    // -> static struct xCamDebug * debug_cam;
}

// Range: 0x802399C4 -> 0x802399CC
