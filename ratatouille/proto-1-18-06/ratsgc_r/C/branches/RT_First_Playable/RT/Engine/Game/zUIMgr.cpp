/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zUIMgr.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80179E34 -> 0x8017AEEC
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static class zUI * uiElements[1024]; // size: 0x1000, address: 0x80BAA848
static unsigned int uiElementCount; // size: 0x4, address: 0x80C08598
static unsigned char uiHDREnabled; // size: 0x1, address: 0x80C0859C
static unsigned char uiAnyActive; // size: 0x1, address: 0x80C01BD8
static unsigned char screenLetterBoxActive; // size: 0x1, address: 0x80C0859D
static float afterTimer; // size: 0x4, address: 0x80C085A0
// total size: 0x8
struct substr {
    // Members
...
// total size: 0x1
struct /* @class$1029zUIMgr_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1030zUIMgr_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1031zUIMgr_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1032zUIMgr_cpp */ {
    // Members
...
static struct tweak_callback uiCallbacks; // size: 0x28, address: 0x80BAB848
// Range: 0x80179E34 -> 0x80179F54
static char * zUIMgrGetTypeName(class zUI * ui /* r0 */) {}
...
// Range: 0x80179F54 -> 0x80179FDC
static void OnExpandTweaks(const struct tweak_info & info /* r0 */) {
    // Local variables
    class zUI * ui; // r30
    char name[128]; // r1+0x8
    const struct zUIAsset * pAsset; // r0
...
// Range: 0x8017A24C -> 0x8017A254
float zUIGetTimer() {
    // References
    // -> static float afterTimer;
}

// Range: 0x8017A254 -> 0x8017A25C
void zUISetTimer(float value /* f0 */) {
    // References
    // -> static float afterTimer;
}

static struct tweak_callback uimotionCallbacks; // size: 0x28, address: 0x8043C928
static signed char init; // size: 0x1, address: 0x80C085A4
static char __FUNCTION__[11]; // size: 0xB, address: 0x80C01BDC
// total size: 0x30
class xfont {
    // Members
public:
...
// Range: 0x8017A25C -> 0x8017A82C
void zUIMgrInit(struct zScene * s /* r27 */) {
    // Local variables
    ...
    struct zUIMotionCmdAsset * cmd; // r28
    unsigned int j; // r27
    struct zUIMotionCmdColor * color; // r0
    unsigned int i; // r26
    unsigned int i; // r24
    char name[128]; // r1+0xD8
    int count; // r0
    int i; // r27
    struct zUIMotionAsset * asset; // r28
    char name[128]; // r1+0x58
...
// Range: 0x8017AB84 -> 0x8017AB8C
void zUIMgrEnableHDR(unsigned char enabled /* r0 */) {
    // References
    // -> static unsigned char uiHDREnabled;
}

// Range: 0x8017AB8C -> 0x8017ABBC
