/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xGrid.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8006BB80 -> 0x8006DA50
*/
static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFCE48
// total size: 0x38
struct xGrid {
    // Members
...
// Range: 0x8006BB80 -> 0x8006BC2C
void xGridBoundInit(struct xGridBound * gridb /* r30 */, void * data /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFCE58
unsigned int gActiveHeap; // size: 0x4, address: 0x80C07448
// Range: 0x8006BC2C -> 0x8006C0C0
void xGridInit(struct xGrid * grid /* r27 */, const struct xBox * bounds /* r28 */, unsigned short nx /* r29 */, unsigned short nz /* r30 */, unsigned char ingrid_id /* r31 */) {
    // Local variables
    float gsizex; // f6
    float gsizez; // f5
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFCE64
// Range: 0x8006C0C0 -> 0x8006C144
void xGridKill(struct xGrid * grid /* r31 */) {
    // References
    // -> static char __FUNCTION__[10];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFCE70
// Range: 0x8006C144 -> 0x8006C2A0
void xGridEmpty(struct xGrid * grid /* r31 */) {
    // Local variables
    struct xGridBound * * head; // r27
    struct xGridBound * curr; // r3
    int x; // r26
    int z; // r25
    struct xGridBound * currnext; // r24
    struct xGridBound * currnext; // r24
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFCE7C
// Range: 0x8006C2A0 -> 0x8006C408
int xGridAddToCell(struct xGridBound * * cell /* r30 */, struct xGridBound * gridb /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80BFCE8C
// Range: 0x8006C408 -> 0x8006C510
static int xGridAdd(struct xGrid * grid /* r28 */, struct xGridBound * gridb /* r29 */, int x /* r30 */, int z /* r31 */) {
    // References
    // -> static char __FUNCTION__[9];
}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80BFCE98
// total size: 0x20
struct xLinkAsset {
    // Members
...
// Range: 0x8006C510 -> 0x8006CAC0
int xGridAdd(struct xGrid * grid /* r30 */, struct xEnt * ent /* r31 */) {
    // Local variables
    struct xBound * bound; // r0
    struct xVec3 * center; // r29
    float maxr; // f31
    struct xSphere * sph; // r29
    int r; // r3
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80BFCEA4
// Range: 0x8006CAC0 -> 0x8006CC14
int xGridRemove(struct xGridBound * gridb /* r29 */) {
    // Local variables
    struct xGridBound * cur; // r31
    struct xGridBound * * prev; // r30

    // References
    // -> static char __FUNCTION__[12];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80BFCEB0
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x8006CC14 -> 0x8006CDE0
void xGridUpdate(struct xGrid * grid /* r30 */, struct xEnt * ent /* r28 */) {
    // Local variables
    int dx; // r1+0xC
    int dz; // r1+0x8
    struct xGridBound * gridb; // r31
    struct xGridBound * cur; // r29
    struct xGridBound * * prev; // r28
    struct xGridBound * * cell; // r3
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80BFCEBC
// Range: 0x8006CDE0 -> 0x8006CF08
static struct xGridBound * * xGridGetCell(struct xGrid * grid /* r28 */, const struct xEnt * ent /* r31 */, int & grx /* r29 */, int & grz /* r30 */) {
    // Local variables
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80BFCECC
// Range: 0x8006CF08 -> 0x8006D118
void xGridGetCell(struct xGrid * grid /* r29 */, float posx /* f30 */, float posz /* f31 */, int & grx /* r30 */, int & grz /* r31 */) {
    // Local variables
    float pgridx; // f3
    float pgridz; // f4
...
// Range: 0x8006D118 -> 0x8006D17C
struct xGridBound * xGridIterFirstCell(struct xGrid * grid /* r28 */, float posx /* f0 */, float posy /* f0 */, float posz /* f0 */, int & grx /* r29 */, int & grz /* r30 */, struct xGridIterator & it /* r31 */) {}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFCEDC
// Range: 0x8006D17C -> 0x8006D3FC
int xGridEntIsTooBig(struct xGrid * grid /* r30 */, const struct xEnt * ent /* r31 */) {
    // Local variables
    const struct xBound * bound; // r0
    float maxr; // f31
    const struct xBBox * bbox; // r0
    float rx; // f12
    float ry; // f13
    float rz; // f10
    const struct xBBox * bbox; // r0
...
static int offs[4][3][2]; // size: 0x60, address: 0x80417F98
static int k; // size: 0x4, address: 0x80C073A8
// total size: 0xC
...
// Range: 0x8006D3FC -> 0x8006D694
void xGridCheckPosition(struct xGrid * grid /* r26 */, struct xVec3 * pos /* r25 */, struct xQCData * qcd /* r27 */, int (* hitCB)(struct xEnt *, void *) /* r28 */, void * cbdata /* r29 */) {
    // Local variables
...
// Range: 0x8006D694 -> 0x8006D6D0
struct xGridBound * xGridIterFirstCell(struct xGridBound * * head /* r0 */, struct xGridIterator & it /* r0 */) {
    // Local variables
    struct xGridBound * curcell; // r5
}

// Range: 0x8006D6D0 -> 0x8006D744
struct xGridBound * xGridIterFirstCell(struct xGrid * grid /* r0 */, int grx /* r0 */, int grz /* r0 */, struct xGridIterator & it /* r6 */) {}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFCEF0
// Range: 0x8006D744 -> 0x8006D8C0
void xGridIterClose(struct xGridIterator & it /* r31 */) {
    // Local variables
    int & iter_active; // r0
    struct xGridBound * cur; // r5
    struct xGridBound * * prev; // r6

    // References
    // -> static char __FUNCTION__[15];
}

// Range: 0x8006D8C0 -> 0x8006D994
void xGridCheckBoundSetup(struct xBox & box /* r26 */, int & lower_x /* r27 */, int & lower_z /* r28 */, int & upper_x /* r29 */, int & upper_z /* r30 */, struct xGrid & grid /* r31 */, const struct xBound & bound /* r9 */) {
    // Local variables
    float ex; // f3
    float ez; // f1
}

// Range: 0x8006D994 -> 0x8006DA50
static void get_grid_index(int & ox /* r0 */, int & oz /* r0 */, const struct xGrid & grid /* r0 */, float x /* f0 */, float z /* f0 */) {
    // Local variables
    int ix; // r5
    int iz; // r6
    int xmax; // r7
