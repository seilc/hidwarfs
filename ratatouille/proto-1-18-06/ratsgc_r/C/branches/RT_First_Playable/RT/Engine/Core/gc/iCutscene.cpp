/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\gc\iCutscene.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800D633C -> 0x800D703C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80BFEE50
// total size: 0x80
...
// Range: 0x800D633C -> 0x800D65A8
void iCSSoundSetup(struct xCutscene * csn /* r29 */) {
    // Local variables
    unsigned int dataIndex; // r0
    struct xCutsceneData * data; // r31
    unsigned int numData; // r30
    enum xRegion eMyRegion; // r0
    int i; // r5
    unsigned char bLeftFound; // r5
    unsigned int uDefaultLeftTrack; // r0
...
// Range: 0x800D65A8 -> 0x800D6750
void * iCSSoundGetData(unsigned int assetID /* r27 */, unsigned int * len /* r28 */) {
    // Local variables
    struct xCutscene * csn; // r30
    void * retdata; // r29
    struct xCutsceneData * data; // r8
    unsigned int dataIndex; // r0
    unsigned int numData; // r9
    unsigned int foundChannel; // r5
    int channel; // r4
    int chunk; // r31
    int i; // r7
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFEE60
...
// Range: 0x800D6750 -> 0x800D67F4
static void iCSAsyncReadCB(struct tag_xFile * txf /* r31 */) {
    // Local variables
    int bytes; // r1+0x8
    enum IFILE_READSECTOR_STATUS cdstat; // r0
    struct xCutscene * xc; // r0

    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80BFEE70
// total size: 0x28
struct PKRAssetType {
    // Members
...
// Range: 0x800D67F4 -> 0x800D6950
unsigned int iCSFileOpen(struct xCutscene * csn /* r31 */) {
    // Local variables
    unsigned int headerskip; // r30
...
unsigned int read_sizzze; // size: 0x4, address: 0x80C078B8
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFEE7C
// Range: 0x800D6950 -> 0x800D6A74
void iCSFileAsyncRead(struct xCutscene * csn /* r29 */, void * dest /* r30 */, unsigned int size /* r31 */) {
    // References
    // -> static char __FUNCTION__[17];
    // -> unsigned int read_sizzze;
}

// Range: 0x800D6A74 -> 0x800D6AA4
void iCSFileAsyncSkip(struct xCutscene * csn /* r3 */, unsigned int size /* r0 */) {}

// Range: 0x800D6AA4 -> 0x800D6AD8
void iCSFileClose(struct xCutscene * csn /* r31 */) {}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80BFEE90
...
// Range: 0x800D6AD8 -> 0x800D703C
int iCSLoadStep(struct xCutscene * csn /* r29 */) {
    // Local variables
    unsigned int skipAccum; // r30
    unsigned int tmpSize; // r1+0x8
    void * foundModel; // r0
    struct xSndGroup * pLeftGroup; // r0
    enum iSndGroupHandle leftSoundGroup; // r30
    struct xSndGroup * pRightGroup; // r0
    enum iSndGroupHandle rightSoundGroup; // r31
    struct xSndGroup * pLeftGroup; // r0
    enum iSndGroupHandle leftSoundGroup; // r0
    unsigned int i; // r31
