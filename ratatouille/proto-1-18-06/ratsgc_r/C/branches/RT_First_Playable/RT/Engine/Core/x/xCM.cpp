/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xCM.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8001A040 -> 0x8001B8AC
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct basic_rect screen_bounds; // size: 0x10, address: 0x80C0A1B0
static const char * KEY; // size: 0x4, address: 0x80BFBA20
// Range: 0x8001A040 -> 0x8001A144
static void Decrypt(void * buffer_ /* r3 */, unsigned int size /* r4 */, void * key_ /* r0 */, unsigned int keySize /* r0 */) {
    // Local variables
    unsigned char * buffer; // r3
    const unsigned char * key; // r0
    unsigned char last; // r10
    unsigned int keyCurrent; // r8
}

static char __FUNCTION__[8]; // size: 0x8, address: 0x80BFBA24
// total size: 0x4
struct xCreditsData {
    // Members
...
// Range: 0x8001A144 -> 0x8001A30C
static void xCMprep(struct xCreditsData * data /* r28 */) {
    // Local variables
    struct xCMheader * hdr; // r0
    char * dp; // r30
    struct xCMcredits * cp; // r0
    struct xCMpreset * pp; // r29
    struct xCMhunk * hp; // r31
    struct xCMpreset * preset; // r27

    // References
    // -> static char __FUNCTION__[8];
}

static float destX; // size: 0x4, address: 0x80C06EA8
static float destY; // size: 0x4, address: 0x80C06EAC
static float destWidth; // size: 0x4, address: 0x80BFBA2C
static float destHeight; // size: 0x4, address: 0x80BFBA30
// Range: 0x8001A30C -> 0x8001A358
static void xCMScaleBounds(struct basic_rect & r /* r0 */) {
    // References
...
// Range: 0x8001A358 -> 0x8001A3AC
static void xCMScaleBounds(float & x0 /* r0 */, float & y0 /* r0 */, float & x1 /* r0 */, float & y1 /* r0 */) {
    // References
    // -> static float destHeight;
    // -> static float destY;
    // -> static float destWidth;
...
// Range: 0x8001A3AC -> 0x8001A46C
static struct xColor_tag xCMcolor_scale(struct xColor_tag & color /* r0 */, float scale /* f0 */) {
    // Local variables
    struct xColor_tag tmp; // r1+0xC
    struct xColorUnpack tmp2; // r1+0x8
    float a; // f4
}

unsigned int dumpit; // size: 0x4, address: 0x80C06EB0
// Range: 0x8001A46C -> 0x8001A50C
static void xCMdump_header(struct xCMheader * hdr /* r31 */) {}

// Range: 0x8001A50C -> 0x8001A634
static void xCMdump_credits(struct xCMcredits * credits /* r31 */) {}

// Range: 0x8001A634 -> 0x8001A6E0
...
// Range: 0x8001A6E0 -> 0x8001A794
static void xCMdump_texture(struct xCMtexture * tex /* r31 */) {}

static char * preset_names[5]; // size: 0x14, address: 0x80BFBA34
// Range: 0x8001A794 -> 0x8001A864
static void xCMdump_preset(struct xCMpreset * pp /* r31 */) {
    // References
    // -> static char * preset_names[5];
}

// Range: 0x8001A864 -> 0x8001A918
static void xCMdump_hunk(struct xCMhunk * hunk /* r31 */) {}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFBA48
...
// Range: 0x8001A918 -> 0x8001B578
static unsigned int xCMrender(float time /* f28 */, struct xCreditsData * data /* r30 */) {
    // Local variables
    struct xCMheader * hdr; // r30
    char * dp; // r29
    struct xCMcredits * cp; // r0
    struct xCMpreset * pp; // r28
    unsigned int i; // r27
    struct xCMhunk * hp; // r31
    float a; // f27
    struct xCMpreset * preset; // r27
    ...
    float y1; // r1+0x54
    float ca; // f29
    struct xCMtexture * tex; // r0
    enum RwBlendFunction oldSrcBlend; // r1+0x50
    enum RwBlendFunction oldDestBlend; // r1+0x4C
    struct xColor_tag color; // r1+0x48
    struct basic_rect bounds; // r1+0x94
    class xtextbox tb; // r1+0x490
    enum RwBlendFunction oldSrcBlend; // r1+0x44
    enum RwBlendFunction oldDestBlend; // r1+0x40
    struct xColor_tag color; // r1+0x3C
    struct basic_rect bounds; // r1+0x84
    class xtextbox tb; // r1+0x41C
    enum RwBlendFunction oldSrcBlend; // r1+0x38
    enum RwBlendFunction oldDestBlend; // r1+0x34
    unsigned int alignL; // r25
    unsigned int alignR; // r26
    struct xColor_tag colorL; // r1+0x30
    struct basic_rect bounds; // r1+0x74
    class xtextbox tb; // r1+0x3A8
    struct xColor_tag colorR; // r1+0x2C
    struct basic_rect bounds; // r1+0x64
    class xtextbox tb; // r1+0x334
...
float dtscale; // size: 0x4, address: 0x80BFBA54
static float credits_time; // size: 0x4, address: 0x80BFBA58
static struct xCreditsData * credits_data; // size: 0x4, address: 0x80C06EB4
static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFBA5C
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x8001B578 -> 0x8001B754
void xCMupdate(float dt /* f30 */) {
    // Local variables
    float scale; // f31
    int i; // r31
    int port; // r31
...
static unsigned int credits_parentID; // size: 0x4, address: 0x80C06EB8
// Range: 0x8001B77C -> 0x8001B800
void xCMstart(struct xCreditsData * data /* r30 */, struct xBase * parent /* r31 */) {
    // Local variables
    struct xCMheader * hdr; // r0
...
// Range: 0x8001B884 -> 0x8001B898
void xCMsetDest(float x /* f0 */, float y /* f0 */, float width /* f0 */, float height /* f0 */) {
    // References
    // -> static float destHeight;
    // -> static float destWidth;
    // -> static float destY;
