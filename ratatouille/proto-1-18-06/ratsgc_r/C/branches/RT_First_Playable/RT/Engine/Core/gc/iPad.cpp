/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\gc\iPad.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800E5EF0 -> 0x800E7214
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
float g_rumblingThreshold; // size: 0x4, address: 0x80C079D0
// total size: 0xC
struct Vec {
    // Members
    float x; // offset 0x0, size 0x4
...
// total size: 0x18
struct /* @class$1784iPad_cpp */ {
    // Members
    struct Vec2 stick; // offset 0x0, size 0x8
    struct Vec2 substick; // offset 0x8, size 0x8
    float ltrigger; // offset 0x10, size 0x4
    float rtrigger; // offset 0x14, size 0x4
};
union KPADEXStatus {
    // total size: 0x1C
    struct /* @class$1783iPad_cpp */ {
...
struct KPADStatus {
    ...
    unsigned long dev_type; // offset 0x54, size 0x4
    union KPADEXStatus ex_status; // offset 0x58, size 0x1C
    signed char dpd_valid_fg; // offset 0x74, size 0x1
    signed char wpad_err; // offset 0x75, size 0x1
};
static struct KPADStatus kpads[4][10]; // size: 0x12C0, address: 0x80430E18
static int kpad_reads; // size: 0x4, address: 0x80C079D4
static char * inputSources[4]; // size: 0x10, address: 0x80BFF3A0
static unsigned int sPlayerInputSource; // size: 0x4, address: 0x80C079D8
static unsigned int sCameraInputSource; // size: 0x4, address: 0x80BFF3B0
static unsigned char sPlayerFlipX; // size: 0x1, address: 0x80BFF3B4
static unsigned char sPlayerFlipY; // size: 0x1, address: 0x80C079DC
static unsigned char sCameraFlipX; // size: 0x1, address: 0x80C079DD
static unsigned char sCameraFlipY; // size: 0x1, address: 0x80C079DE
static float sAccParamPlayRadius; // size: 0x4, address: 0x80C079E0
static float sAccParamPlaySensitivity; // size: 0x4, address: 0x80BFF3B8
static float sPlayerClampMin; // size: 0x4, address: 0x80C079E4
static float sPlayerClampMax; // size: 0x4, address: 0x80BFF3BC
static unsigned char sPlayerClampRenormalize; // size: 0x1, address: 0x80C079E8
static float sCameraClampMin; // size: 0x4, address: 0x80BFF3C0
static float sCameraClampMax; // size: 0x4, address: 0x80BFF3C4
static unsigned char sCameraClampRenormalize; // size: 0x1, address: 0x80C079E9
// total size: 0xC
struct PADStatus {
    // Members
...
static struct PADStatus sPadData[4]; // size: 0x30, address: 0x804320D8
static unsigned long sPadMotor[4]; // size: 0x10, address: 0x80C079EC
static int sPadCurrLevel[4]; // size: 0x10, address: 0x80C079FC
static int sPadDeltaLevel[4]; // size: 0x10, address: 0x80C07A0C
// Range: 0x800E5EF0 -> 0x800E61C4
int iPadInit() {
    // Local variables
    int i; // r31
    ...
    // References
    // -> static unsigned long sPadMotor[4];
    // -> static int sPadDeltaLevel[4];
    // -> static int sPadCurrLevel[4];
    // -> static unsigned char sCameraClampRenormalize;
    // -> static unsigned char sPlayerClampRenormalize;
    // -> static float sCameraClampMax;
    // -> static float sCameraClampMin;
    // -> static float sPlayerClampMax;
    // -> static float sPlayerClampMin;
    // -> static float sAccParamPlaySensitivity;
    // -> static float sAccParamPlayRadius;
    // -> static unsigned char sCameraFlipY;
    // -> static unsigned char sCameraFlipX;
    // -> static unsigned char sPlayerFlipY;
    // -> static unsigned char sPlayerFlipX;
    // -> static char * inputSources[4];
    // -> static unsigned int sCameraInputSource;
    // -> static unsigned int sPlayerInputSource;
    // -> float g_rumblingThreshold;
}
...
// Range: 0x800E61C4 -> 0x800E6230
struct _tagxPad * iPadEnable(struct _tagxPad * pad /* r31 */, signed short port /* r0 */) {
    // Local variables
    unsigned int new_on; // r1+0x8
}

// Range: 0x800E6230 -> 0x800E6298
static signed char iPadConvStick(float in /* f1 */) {
    // Local variables
    float fin; // f1
}

struct _tagxPad mPad[4]; // size: 0x4E0, address: 0x80A84818
// Range: 0x800E6298 -> 0x800E6344
void iPadWaitForRevolution() {
    // Local variables
    unsigned char ready; // r29
    int i; // r28
    unsigned int dummy; // r1+0xC
    unsigned long type; // r1+0x8
...
// Range: 0x800E6344 -> 0x800E644C
static unsigned short ConvertKPADButtons(unsigned int kbut /* r0 */) {
    // Local variables
    unsigned short pbut; // r3
}

// Range: 0x800E644C -> 0x800E64F8
static void DeadZone(float & x /* r0 */, float flip /* f0 */, float clampMin /* f0 */, float clampMax /* f0 */, unsigned char nrmlz /* r0 */) {}

unsigned int gFrameCount; // size: 0x4, address: 0x80C06F38
// Range: 0x800E64F8 -> 0x800E6940
void iPadUpdateDataAll() {
    // Local variables
    unsigned char spew; // r28
    int i; // r29
    struct xVec2 inputs[4]; // r1+0x10
    float inputX; // r1+0xC
    float inputY; // r1+0x8
    ...
    // -> static unsigned char sPlayerFlipX;
    // -> static unsigned int sPlayerInputSource;
    // -> static int kpad_reads;
    // -> static struct PADStatus sPadData[4];
    // -> static struct KPADStatus kpads[4][10];
    // -> unsigned int gFrameCount;
}

static long long held_start[4]; // size: 0x20, address: 0x80432108
static unsigned char held_through_reset[4]; // size: 0x4, address: 0x80BFF3C8
// Range: 0x800E6940 -> 0x800E6A14
void iPadInHandReset(int port /* r31 */, unsigned int * on /* r0 */) {
    // Local variables
    unsigned char held; // r30

    // References
    // -> static unsigned char held_through_reset[4];
    // -> static long long held_start[4];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFF3CC
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x800E6A14 -> 0x800E6F88
int iPadUpdate(struct _tagxPad * pad /* r26 */, unsigned int * on /* r27 */) {
    // Local variables
    unsigned int read_on; // r28
...
// Range: 0x800E6F88 -> 0x800E6FA0
static unsigned int iPadConvFromGCN(unsigned int buttons /* r0 */, unsigned int gcn_flag /* r0 */, unsigned int x_flag /* r0 */) {}

// Range: 0x800E6FA0 -> 0x800E6FFC
void iPadStopRumble(struct _tagxPad * pad /* r3 */) {
    // References
    // -> static unsigned long sPadMotor[4];
    // -> static int sPadDeltaLevel[4];
    // -> static int sPadCurrLevel[4];
...
// Range: 0x800E707C -> 0x800E7150
void iPadStartRumble(struct _tagxPad * pad /* r0 */, float intensity /* f0 */) {
    // References
    // -> static unsigned long sPadMotor[4];
    // -> static int sPadDeltaLevel[4];
    // -> static int sPadCurrLevel[4];
...
// Range: 0x800E7150 -> 0x800E7210
void iPadUpdateRumble(struct _tagxPad * pad /* r0 */) {
    // References
    // -> static unsigned long sPadMotor[4];
    // -> static int sPadCurrLevel[4];
    // -> static int sPadDeltaLevel[4];
