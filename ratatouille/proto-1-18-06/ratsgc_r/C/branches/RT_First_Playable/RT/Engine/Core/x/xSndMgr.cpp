/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xSndMgr.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800BD934 -> 0x800C217C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
struct SndMixerFadersEntry g_SndMixerFaders[9]; // size: 0x90, address: 0x8042E200
...
enum xSndEffect m_eCurrentEffect; // size: 0x4, address: 0x80C077F8
// total size: 0x8
...
struct SndListenerSpringTableEntry g_SndListenerSpringTable[3]; // size: 0x18, address: 0x80BFE6D4
unsigned char gShowSoundMessages; // size: 0x1, address: 0x80C077FC
unsigned char gDrawSoundDebug; // size: 0x1, address: 0x80C077FD
// total size: 0xC
...
struct SoundControlList IniList[4]; // size: 0x30, address: 0x8042E290
// Range: 0x800BD934 -> 0x800BD980
static unsigned char InIniList(unsigned int id /* r0 */, int whichList /* r0 */) {
    // Local variables
    struct SoundControlList * list; // r6
    unsigned int i; // r0
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFE6EC
...
// Range: 0x800BD980 -> 0x800BDBB0
void xSndMgr_ParseINI(struct xIniFile * ini /* r28 */) {
    // Local variables
    struct SoundControlList * ctl; // r31
    int i; // r30
    const char * str; // r29
    char local_str[1024]; // r1+0x8
    char * tok; // r26
    int n; // r25
...
static class linear_pool faders; // size: 0xC, address: 0x80C07800
// Range: 0x800BDBB0 -> 0x800BDBDC
...
// Range: 0x800BDBDC -> 0x800BDC00
static void reset_faders() {
    // References
    // -> static class linear_pool faders;
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80BFE700
char __FUNCTION__$localstatic1$erase__51linear_pool<Q221@unnamed@xSndMgr_cpp@11sound_fader>FPQ221@unnamed@xSndMgr_cpp@11sound_fader[6]; // size: 0x6, address: 0x80BFE710
// Range: 0x800BDC00 -> 0x800BDD70
static void update_faders(float dt /* f30 */) {
    // Local variables
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFE718
// Range: 0x800BDD70 -> 0x800BDEB8
static struct xSndVoiceInfo * xSndMgrGetVoice(enum xSndHandle & xHandle /* r3 */) {
    // Local variables
    struct iSndHandle * iHandle; // r30
    int iVoiceRef; // r0
    int iVoiceIndex; // r0
    struct xSndVoiceInfo * pVoice; // r3
...
// Range: 0x800BDEB8 -> 0x800BDF28
static struct sound_fader * find_fader(enum xSndHandle handle /* r30 */) {
    // Local variables
    struct sound_fader * it; // r31
    struct sound_fader * end; // r0

    // References
    // -> static class linear_pool faders;
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFE728
// Range: 0x800BDF28 -> 0x800BE050
int xSndMgrGetVoiceIndex(enum xSndHandle & xHandle /* r3 */) {
    // Local variables
    struct iSndHandle * iHandle; // r30
    int iVoiceRef; // r0
    int iVoiceIndex; // r30
...
// Range: 0x800BE050 -> 0x800BE1E8
struct xSndGroup * xSndMgr_AllocGroup(unsigned char nSoundAssets /* r30 */) {
    // Local variables
    unsigned int datasize; // r4
    void * pAllocated; // r3
    struct xSndGroup * pRetGroup; // r3
    unsigned int i; // r7
...
// Range: 0x800BE640 -> 0x800BE680
void xSndMgrSetListenerSpringType(enum eSndListenerSpringType type /* r3 */) {
    // References
...
// total size: 0x1
struct /* @class$990xSndMgr_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$991xSndMgr_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$992xSndMgr_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$993xSndMgr_cpp */ {
    // Members
...
// Range: 0x800BE680 -> 0x800BE690
void xSndMgrSelectListenerMode(enum xSndListener Mode /* r0 */) {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800BE690 -> 0x800BE6C0
void xSndMgrSetEffect(enum xSndEffect Effect /* r0 */) {
    // References
    // -> enum xSndEffect m_eCurrentEffect;
}

// Range: 0x800BE6C0 -> 0x800BE6C8
...
// Range: 0x800BE6C8 -> 0x800BE82C
void xSndMgrProcessSoundPosition(const struct xVec3 * pActual /* r29 */, struct xVec3 * pProcessed /* r30 */) {
    // Local variables
    struct xVec3 playerDelta; // r1+0x20
    struct xVec3 cameraDelta; // r1+0x14
    float playerDist; // f0
    float cameraDist; // f0
    float inwardShift; // f1
...
// Range: 0x800BE82C -> 0x800BE954
void xSndMgr_UpdateListenerPosition(float dt /* f31 */) {
    // Local variables
    struct xMat4x3 * pMat; // r30
...
static char __FUNCTION__[28]; // size: 0x1C, address: 0x8042E2C0
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// total size: 0x4
struct /* @class$2683xSndMgr_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2684xSndMgr_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// Range: 0x800BEA14 -> 0x800BEEB4
unsigned char xSndMgr_UpdateVoicePosition(struct xSndVoiceInfo * pVoice /* r31 */) {
    // Local variables
    struct xVec3 vel; // r1+0x2C
    struct xVec3 vel; // r1+0x20
    struct xVec3 soundDir; // r1+0x14
    float length2; // f1
    float velListener; // f0
    float velSource; // f0
    float newDoppler; // f1
    struct xVec3 soundDirectionXYZ; // r1+0x8
...
// Range: 0x800BEEB4 -> 0x800BEF28
struct xSndVoiceInfo * xSndMgr_GetFreeVirtualVoiceInfo(unsigned int priority /* r3 */, int flags /* r4 */, unsigned char virtualOnly /* r0 */) {
    // Local variables
    int i; // r7
...
// Range: 0x800BEF28 -> 0x800BEF7C
void xSndMgr_Stop(struct xSndVoiceInfo * pVoice /* r0 */, unsigned char bFreeRef /* r0 */) {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800BEF7C -> 0x800BF008
void xSndMgr_Pause(struct xSndVoiceInfo * pVoice /* r30 */, unsigned char bPaused /* r31 */) {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800BF008 -> 0x800BF084
void xSndMgr_CutscenePause(struct xSndVoiceInfo * pVoice /* r30 */, unsigned char bPaused /* r31 */) {}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFE740
// Range: 0x800BF084 -> 0x800BF270
unsigned char xSndMgr_SetInaudible(struct xSndVoiceInfo * pVoice /* r30 */) {
    // Local variables
    struct xSndVoiceInfo * pVoiceStorage; // r31
    int xVoiceId; // r4
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80BFE758
// Range: 0x800BF35C -> 0x800BF474
unsigned char xSndMgr_SetAudible(struct xSndVoiceInfo * pVoiceStorage /* r31 */) {
    // Local variables
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFE76C
// Range: 0x800BF474 -> 0x800BF5EC
...
// Range: 0x800BF5EC -> 0x800BF624
void xSndMgr_SetInitFlag(unsigned char bIsInit /* r0 */) {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800BF624 -> 0x800BF638
unsigned char xSndMgr_IsInited() {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800BF638 -> 0x800BF6A8
...
// Range: 0x800BF6A8 -> 0x800BF728
void xSndMgrUpdate(float dt /* f0 */) {
    // Local variables
    unsigned int i; // r30
...
// Range: 0x800BF728 -> 0x800BF774
void xSndMgrSceneExit() {}

// Range: 0x800BF774 -> 0x800BF794
void xSndMgrExit() {}

// Range: 0x800BF794 -> 0x800BF7F8
unsigned char xSndMgrDoesEffects(signed short eSoundCategory /* r0 */) {}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFE780
float SECS_PER_VBLANK; // size: 0x4, address: 0x80C08210
// Range: 0x800BF7F8 -> 0x800BF994
void xSndMgrStopSounds(signed short eSoundCategory /* r0 */, unsigned char bSkipPaused /* r0 */, unsigned char bInReset /* r0 */) {
    // Local variables
...
// Range: 0x800BF994 -> 0x800BFAD4
void xSndMgrStopPausedSounds(signed short eSoundCategory /* r0 */) {
    // Local variables
    int i; // r30
    int i; // r30
    int i; // r29
...
// Range: 0x800BFAD4 -> 0x800BFBF4
void xSndMgrPauseSounds(signed short eSoundCategory /* r29 */, unsigned char bPaused /* r30 */, unsigned char bPauseFutureSoundsOfThisType /* r31 */) {
    // Local variables
    int i; // r27
    int i; // r27
    int i; // r26
...
// Range: 0x800BFBF4 -> 0x800BFD14
void xSndMgrCutscenePause(signed short eSoundCategory /* r29 */, unsigned char bPaused /* r30 */, unsigned char bPauseFutureSoundsOfThisType /* r31 */) {
    // Local variables
    int i; // r27
    int i; // r27
    int i; // r26
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80BFE794
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80BFE7A8
// Range: 0x800BFE20 -> 0x800BFF1C
...
static char __FUNCTION__[23]; // size: 0x17, address: 0x80BFE7BC
// Range: 0x800BFF1C -> 0x800BFFB8
void xSndMixerSetDampVolume(enum xSndMixerFaderName eMixerFader /* r31 */, float fVolume /* f31 */) {
    // References
    // -> struct SndMixerFadersEntry g_SndMixerFaders[9];
    // -> static char __FUNCTION__[23];
}

// Range: 0x800BFFB8 -> 0x800BFFEC
...
static char __FUNCTION__[25]; // size: 0x19, address: 0x8042E2DC
// Range: 0x800BFFEC -> 0x800C0264
float xSndMixerGetOutputVolume(signed short eSoundCategory /* r30 */) {
    // Local variables
    enum xSndMixerFaderName eMixerFader; // r0
    struct SndMixerFadersEntry * CurFader; // r30
    float outputVolume; // f2
    float CurFaderVolume; // f1
...
// Range: 0x800C0264 -> 0x800C0298
unsigned char xSndMgrGetSoundCount(enum iSndGroupHandle hSound /* r0 */) {
    // Local variables
    struct xSndGroup * pSndGroup; // r0
}

// Range: 0x800C0298 -> 0x800C02F4
unsigned int xSndMgrGetSoundId(enum iSndGroupHandle hSound /* r0 */, unsigned int uIndexOfSound /* r31 */) {
    // Local variables
    struct xSndGroup * pSndGroup; // r0
}

// Range: 0x800C02F4 -> 0x800C039C
...
// Range: 0x800C039C -> 0x800C048C
unsigned char xSndMgrIsPlaying(enum iSndGroupHandle hSoundGroup /* r0 */) {
    // Local variables
    int i; // r5
...
// Range: 0x800C048C -> 0x800C0508
unsigned char xSndMgrGetPriority(enum iSndGroupHandle hSound /* r0 */, unsigned char bStreaming /* r30 */, unsigned char bLooping /* r31 */) {
    // Local variables
    struct xSndGroupHeader * pSnd; // r0
    unsigned int uPriority; // r4
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFE7D4
// Range: 0x800C0508 -> 0x800C0724
struct xSndVoiceInfo * xSndGetFreeVoiceInfo(const struct xSndGroupHeader * pSnd /* r24 */, int iFlags /* r25 */) {
    // Local variables
    float dstFurthest; // f31
    int idxFurthest; // r30
    unsigned int ageOldest; // r29
    int idxOldest; // r28
    unsigned char uInstanceCount; // r27
    int i; // r26
    struct xSndVoiceInfo * vp; // r0
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80BFE7EC
// total size: 0x1C
...
// Range: 0x800C0724 -> 0x800C0FCC
enum xSndHandle xSndMgrPlay(enum iSndGroupHandle hSoundToPlay /* r21 */, int uFlags /* r18 */, const struct xVec3 * pPos_WS /* r26 */, const struct xVec3 * pVel /* r27 */, struct xEnt * pParentEnt /* r28 */, const float * pfOverrideVolume /* r29 */, const float * pfOverridePitch /* r30 */) {
    // Local variables
    int iFlags; // r25
    struct xSndGroupHeader * pSnd; // r24
    unsigned int uInfoIndex; // r0
    struct xSndGroupInfo * pSndInfo; // r23
    struct iSndFileInfo * ip; // r22
    struct xSndVoiceInfo * vp; // r31
    unsigned int uniqueID; // r19
    int voiceRef; // r18
    char buf[128]; // r1+0x8
...
// Range: 0x800C0FCC -> 0x800C1034
unsigned int xSndMgrGetSoundId(enum xSndHandle & hSound /* r0 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r0

    // References
    // -> unsigned char gShowSoundMessages;
}

// Range: 0x800C1034 -> 0x800C1094
unsigned char xSndMgrIsPlaying(enum xSndHandle & hSound /* r0 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r0
...
// Range: 0x800C1094 -> 0x800C1100
unsigned char xSndMgrIsLooping(enum xSndHandle & hSound /* r0 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r0
...
// Range: 0x800C1100 -> 0x800C116C
unsigned char xSndMgrIsStreamed(enum xSndHandle & hSound /* r0 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r0
...
// Range: 0x800C116C -> 0x800C11C4
unsigned int xSndMgrIsReady(enum xSndHandle & hSound /* r0 */) {
    // References
    // -> unsigned char gShowSoundMessages;
}

// Range: 0x800C11C4 -> 0x800C1230
void xSndMgrPause(enum xSndHandle & hSound /* r0 */, unsigned char bPause /* r31 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r0
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFE7F8
// Range: 0x800C12A4 -> 0x800C1438
void xSndMgrSetVolume(enum xSndHandle & hSound /* r31 */, float fVolume /* f31 */, unsigned char assetRelative /* r30 */) {
    // Local variables
...
// Range: 0x800C1438 -> 0x800C1458
void xSndMgrStartStereo(enum xSndHandle hSnd00 /* r0 */, enum xSndHandle hSnd01 /* r0 */) {}

// Range: 0x800C1458 -> 0x800C1490
void xSndMgrChangeAttachedPosition(enum xSndHandle & hSound /* r0 */, const struct xVec3 * pVector /* r31 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r0
}

// Range: 0x800C1490 -> 0x800C1508
void xSndMgrStopChildren(struct xEnt * pParentEnt /* r29 */) {
    // Local variables
    unsigned int i; // r30
...
char __FUNCTION__$localstatic1$push_back__51linear_pool<Q221@unnamed@xSndMgr_cpp@11sound_fader>Fv[10]; // size: 0xA, address: 0x80BFE80C
char __FUNCTION__$localstatic1$back__51linear_pool<Q221@unnamed@xSndMgr_cpp@11sound_fader>Fv[5]; // size: 0x5, address: 0x80BFE818
// Range: 0x800C1508 -> 0x800C15DC
void xSndMgrFade(enum xSndHandle & hSound /* r31 */, float fadeTime /* f29 */, float startVolume /* f30 */, float destVolume /* f31 */) {
    // Local variables
    struct sound_fader * fader; // r3

    // References
    // -> static class linear_pool faders;
}

// Range: 0x800C15DC -> 0x800C1678
void xSndMgrFade(enum xSndHandle & hSound /* r31 */, float fadeTime /* f30 */, float destVolume /* f31 */) {
    // Local variables
    struct xSndVoiceInfo * vp; // r0
}

unsigned int gCurrSoundMemoryUsed; // size: 0x4, address: 0x80C0780C
unsigned int gHighwaterSoundMemoryUsed; // size: 0x4, address: 0x80C07810
unsigned int gNumStreamsOpened; // size: 0x4, address: 0x80C07814
unsigned int gNumStreamsClosed; // size: 0x4, address: 0x80C07818
static char streamPrefix[5]; // size: 0x5, address: 0x80BFE820
static char memoryPrefix[5]; // size: 0x5, address: 0x80BFE828
static char externalPrefix[5]; // size: 0x5, address: 0x80BFE830
int the_heap; // size: 0x4, address: 0x80C07964
// Range: 0x800C1678 -> 0x800C1B0C
void xSndMgrDebugRender() {
    // Local variables
    int size; // r3
    char channelStats[256]; // r1+0x74
    int i; // r26
    struct iSndFileInfo * pSndFileInfo; // r0
    int i; // r26
    char buf[26]; // r1+0x8
    char msg[80]; // r1+0x24
    unsigned char stars; // r25
    int i; // r25
    struct iSndFileInfo * pSndFileInfo; // r0
    char * prefix; // r26
    char pausedState; // r28
    const char * name; // r24
...
// Range: 0x800C1B0C -> 0x800C1B10
void xSndMgrDebugRenderWorld() {}

// Range: 0x800C1B10 -> 0x800C1D34
void xSndMgr_SetPaused(signed short whichCat /* r0 */, unsigned char bPause /* r0 */) {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800C1D34 -> 0x800C1E30
unsigned char xSndMgrIsPaused(signed short eSoundCategory /* r0 */) {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800C1E30 -> 0x800C1EDC
int xSndMgrAllocRef(unsigned int uniqueID /* r27 */, int voiceIndex /* r28 */) {
    // Local variables
    int i; // r30
    int refIndex; // r29
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFE838
// Range: 0x800C1EDC -> 0x800C1FDC
void xSndMgrFreeRef(enum xSndHandle & xHandle /* r3 */) {
    // Local variables
    struct iSndHandle * iHandle; // r31
    int iVoiceRef; // r0
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFE848
// Range: 0x800C1FDC -> 0x800C2120
void xSndMgrUpdateRef(enum xSndHandle & xHandle /* r27 */, int voiceIndex /* r31 */) {
    // Local variables
    int i; // r28
