/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xCutscene.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800251FC -> 0x80027EF0
*/
char __FUNCTION__$localstatic1$Set__13i2dQuadBufferFP7i2dRectP7i2dRect10xColor_tag[4]; // size: 0x4, address: 0x80BFBC68
// total size: 0x80
...
struct xCutscene sActiveCutscene; // size: 0x1A8, address: 0x804598E8
static unsigned int sCutTocCount[16]; // size: 0x40, address: 0x804144B8
static struct xCutsceneInfo * sCutTocInfo[16]; // size: 0x40, address: 0x804144F8
static int sCutTocMax; // size: 0x4, address: 0x80C06EF0
unsigned char gCutsceneDrawBounds; // size: 0x1, address: 0x80C06EF4
unsigned char gCutsceneDrawLineToOrigin; // size: 0x1, address: 0x80C06EF5
// total size: 0xC
struct xModelPool {
    // Members
...
// total size: 0x4
struct /* @class$1080xCutscene_cpp */ {
    // Members
...
static struct xModelInstance * sCutsceneFakeModel; // size: 0x4, address: 0x80C06EF8
static struct xCutsceneModelHack * sCutsceneModelHackBuf; // size: 0x4, address: 0x80C06EFC
// Range: 0x800251FC -> 0x80025298
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFBC6C
// Range: 0x80025298 -> 0x80025348
void xCutscene_InitTOC(void * toc /* r0 */) {
    // References
    // -> static int sCutTocMax;
...
// Range: 0x80025348 -> 0x80025500
void xCutscene_InitEnd() {
    // Local variables
    int i; // r30
    int maxModels; // r7
    int tocnum; // r8
    struct xCutsceneInfo * cnfo; // r9
    unsigned int cnum; // r0
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFBC80
void * RwEngineInstance; // size: 0x4, address: 0x80C09F34
// Range: 0x80025500 -> 0x80025740
struct xCutscene * xCutscene_Create(unsigned int csnID /* r28 */, unsigned int subtitlesID /* r29 */) {
    // Local variables
    struct xCutscene * csn; // r31
    int tocnum; // r6
    struct xCutsceneInfo * cnfo; // r30
    unsigned int i; // r0
    unsigned int maxload; // r3
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFBC94
// Range: 0x800258B4 -> 0x80025954
int xCutscene_LoadStart(struct xCutscene * csn /* r31 */) {
    // References
    // -> static char __FUNCTION__[20];
}

// Range: 0x80025954 -> 0x800259B4
static float xCutsceneConvertBreak(float time /* f1 */, struct xCutsceneBreak * breaklist /* r3 */, unsigned int breakcount /* r4 */, int idx /* r0 */) {
    // Local variables
    unsigned int i; // r7
    float tdiff; // f3
}

static int sRepeatFrames; // size: 0x4, address: 0x80C06F00
static signed char init; // size: 0x1, address: 0x80C06F04
static unsigned char sLastFrame; // size: 0x1, address: 0x80C06F05
static signed char init; // size: 0x1, address: 0x80C06F06
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFBCA8
unsigned int gSoak; // size: 0x4, address: 0x80C07E3C
// Range: 0x800259B4 -> 0x80025CA4
int xCutscene_Update(struct xCutscene * csn /* r31 */, float dt /* f31 */) {
    // Local variables
    struct xCutsceneTime * oldChunk; // r3
...
// Range: 0x80025CA4 -> 0x80025CD8
void xCutscene_SetSpeed(struct xCutscene * csn /* r0 */, float speed /* f1 */) {}
...
// Range: 0x80025CD8 -> 0x80025EF8
static void xcsCalcAnimMatrices(struct RwMatrixTag * animMat /* r26 */, struct RpAtomic * model /* r27 */, struct xCutsceneAnimHdr * ahdr /* r28 */, float time /* f0 */, unsigned int tworoot /* r29 */) {
    // Local variables
    struct xQuat quatresult[101]; // r1+0x54C
    struct xVec3 tranresult[101]; // r1+0x90
    struct xMat4x3 m1; // r1+0x50
    struct xMat4x3 m2; // r1+0x10
    unsigned int numbone; // r0
    unsigned int boneidx; // r4
    struct xQuat * qqq; // r5
    struct xVec3 * ttt; // r6
}

struct xMat4x3 g_I3; // size: 0x40, address: 0x80A7D5C0
static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFBCBC
// total size: 0x8
struct JDeltaRun {
    // Members
...
// Range: 0x80025EF8 -> 0x800268E0
static void JDeltaEval(struct RpAtomic * model /* r23 */, void * deltaModel /* r24 */, void * deltaAnim /* r25 */, float time /* f31 */) {
    // Local variables
    float outweight[128]; // r1+0x8
    float * currweight; // r30
    int i; // r3
    int numFrames; // r22
    int numWeights; // r28
    float * times; // r3
    ...
    float lerp; // f0
    float invlerp; // f1
    struct RwV3d * outverts; // r27
    int numRun; // r0
    int numTargets; // r26
    struct JDeltaTarget * dtgt; // r25
    float scale; // f0
    signed short * svert; // r8
    int j; // r0
    int cmpval; // r3
    ...
    int cmpval; // r19
    float scale; // f4
    signed short * svert; // r9
    int j; // r0
    int cmpval; // r4
    float scale; // f0
    struct RwV3d * vert; // r8
    int j; // r0
    int cmpval; // r4
...
// Range: 0x800268E0 -> 0x80026950
static void CutsceneShadowRender(struct CutsceneShadowModel * smod /* r29 */) {
    // Local variables
    struct RpAtomic * model; // r31
    unsigned int bits; // r30
}

static unsigned int DRAW_SKIP_MASK; // size: 0x4, address: 0x80C06F08
static signed char init; // size: 0x1, address: 0x80C06F0C
static unsigned char done; // size: 0x1, address: 0x80C06F0D
static signed char init; // size: 0x1, address: 0x80C06F0E
static unsigned char done; // size: 0x1, address: 0x80C06F0F
static signed char init; // size: 0x1, address: 0x80C06F10
// total size: 0xC
struct xVec3 {
    // Static members
...
static struct xVec3 shadVec; // size: 0xC, address: 0x80BFBCC8
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFBCD4
// total size: 0x18
...
struct xLightKitBlendInfo gLastLightKit; // size: 0xC, address: 0x80C07428
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x80026950 -> 0x80027A38
void xCutscene_Render(struct xCutscene * csn /* r15 */) {
    // Local variables
    unsigned int i; // r16
    unsigned int dataIndex; // r1+0x1AA8
    unsigned int animIndex; // r26
    unsigned int mphIndex; // r0
    unsigned int visFlags; // r1+0x28
    unsigned int visIdx; // r25
    unsigned int fakeCount; // r24
    unsigned int tworoot; // r17
    unsigned int noshadow; // r1+0x1AA4
    struct xCutsceneData * data; // r23
    struct xCutsceneData * mphdata; // r16
    struct RpAtomic * model; // r22
    struct RpAtomic * shadowModel; // r1+0x1AA0
    struct RwMatrixTag animMat[101]; // r1+0x148
    struct xModelPipe * pipeCurr; // r1+0x24
    unsigned int idx; // r1+0x1A9C
    unsigned int hacked_model; // r21
    int tableIdx; // r1+0x1A98
    struct XCSNNosey * nosey; // r1+0x1A94
    float breakTime; // f31
    struct xBox combinedAnimBound; // r1+0xA4
    unsigned int tempSize; // r1+0x20
    struct xVec3 * scaleVec; // r18
    int table_size; // r0
    int h; // r19
    struct RpAtomic * tmpModel; // r0
    struct zCutsceneHack & cutsceneHackTableData; // r0
    struct zCutsceneHack & cutsceneHackTableData2; // r0
    unsigned int viscnt; // r14
    unsigned int * currvis; // r16
    unsigned int subIndex; // r5
    unsigned int frameMin; // r6
    unsigned int frameMax; // r7
    unsigned int frameIndex; // r8
    unsigned int shadowBits; // r14
    unsigned int atomicNumber; // r1+0x1A90
    struct zCutsceneHack & cutsceneHackTableData2; // r0
    unsigned int morphAnimIndex; // r5
    unsigned int morphModelIndex; // r7
    unsigned int numFrame; // r17
    unsigned int numRun; // r20
    struct xCutsceneMphFrame * mphFrame; // r18
    struct xCutsceneMphRun * mphRun; // r27
    struct xMorphTargetFile * mphFile; // r16
    signed short * v_array[4]; // r1+0x64
    signed short weight[4]; // r1+0x2C
    unsigned int skipsize; // r6
    struct xVec3 * csnTmpArray; // r17
    struct xVec3 * currtmp; // r19
    struct xVec3 * outv; // r18
    unsigned int j; // r17
    unsigned int cmpval; // r16
    void * deltaAnim; // r16
    void * deltaModel; // r17
    int culled; // r16
    struct xBox worldbox; // r1+0x8C
    int culled; // r16
    struct xBox worldbox; // r1+0x74
    struct zCutsceneHack & cutsceneHackTableData2; // r0
    struct CutsceneShadowModel smod; // r1+0x58
    struct xShadowCache scache; // r1+0xBC
    struct xVec3 center; // r1+0x4C
    struct xVec3 radius; // r1+0x40
    float fillCacheRadius; // f31
    struct xLightKitBlendInfo disabledLightKit; // r1+0x34
...
// Range: 0x80027A38 -> 0x80027A44
struct xCutscene * xCutscene_CurrentCutscene() {
    // References
    // -> struct xCutscene sActiveCutscene;
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80BFBCE8
static struct xCamCutscene * cutscene_cam; // size: 0x4, address: 0x80C06F14
// Range: 0x80027A44 -> 0x80027A9C
...
// Range: 0x80027A9C -> 0x80027AF0
// this: r31
static void xCamCutscene::create() {}

// Range: 0x80027B2C -> 0x80027B74
static void cutscene_started(struct xCutscene * csn /* r0 */) {
    // References
    // -> static struct xCamCutscene * cutscene_cam;
}

// Range: 0x80027B74 -> 0x80027BAC
static void cutscene_stopped(struct xCutscene * csn /* r0 */) {
    // References
    // -> static struct xCamCutscene * cutscene_cam;
}

static // total size: 0x4C
struct {} @unnamed@xCutscene_cpp@::xCamCutscene::__vtable; // size: 0x4C, address: 0x80414538
// Range: 0x80027BAC -> 0x80027BFC
// this: r31
...
// total size: 0xC
struct /* @class$1281xCutscene_cpp */ {
    // Members
...
// total size: 0x10
struct /* @class$1558xCutscene_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x80027BFC -> 0x80027C34
// this: r31
static void xCamCutscene::stop() {}

// Range: 0x80027C34 -> 0x80027C54
// this: r3
static struct xCam * xCamCutscene::get_next() {}

// Range: 0x80027C54 -> 0x80027EF0
// this: r27
static void xCamCutscene::update() {
    // Local variables
    struct xCutsceneData * data; // r30
    unsigned int i; // r24
    unsigned int dataIndex; // r29
