/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zAsync.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80183C84 -> 0x801840A4
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
struct RwCamera * cam; // size: 0x4, address: 0x80C08760
// total size: 0x8
struct RwLinkList {
    // Members
...
struct RpLight * light; // size: 0x4, address: 0x80C08764
// total size: 0xA4
struct RwFrame {
    // Members
    struct RwObject object; // offset 0x0, size 0x8
...
struct RwFrame * frame; // size: 0x4, address: 0x80C08768
...
struct RpWorld * world; // size: 0x4, address: 0x80C0876C
// total size: 0x4
class Screen {
    // Members
public:
...
class Screen * screen; // size: 0x4, address: 0x80C08770
// Range: 0x80183C84 -> 0x80183C8C
void Init(class Screen * scr /* r0 */) {
    // References
    // -> class Screen * screen;
}
...
// Range: 0x80183C8C -> 0x80183DA8
void Allocate() {
    // Local variables
    struct RwBBox tmpbbox; // r1+0x24
    struct RwV3d lightpos; // r1+0x18
    struct RwRGBAReal color; // r1+0x8

    // References
    // -> class Screen * screen;
    // -> struct RwCamera * cam;
    // -> struct RpWorld * world;
    // -> struct RpLight * light;
    // -> struct RwFrame * frame;
    // -> unsigned int FB_YRES;
    // -> unsigned int FB_XRES;
}

// Range: 0x80183DA8 -> 0x80183E38
void Free() {
    // References
    // -> struct RpWorld * world;
    // -> struct RwFrame * frame;
...
static float rot; // size: 0x4, address: 0x80C08774
static signed char init; // size: 0x1, address: 0x80C08778
static char __FUNCTION__[13]; // size: 0xD, address: 0x80C01E00
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x80183E38 -> 0x801840A4
void * RenderThread() {
    // Local variables
    long long lastTime; // r30
    unsigned char diskError; // r0
    long long curTime; // r28
    float dt; // f30
    struct RwRGBA bg; // r1+0x8
    struct RwCamera * currentCamera; // r27
