/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zAnimList.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800F5038 -> 0x800F57A0
*/
static int nals; // size: 0x4, address: 0x80C07B70
static unsigned int * aids; // size: 0x4, address: 0x80C07B74
// total size: 0xC
struct RwV3d {
    // Members
...
static struct xAnimTable * * atbls; // size: 0x4, address: 0x80C07B78
static int * anused; // size: 0x4, address: 0x80C07B7C
static char * astnames[20]; // size: 0x50, address: 0x80432AC0
// Range: 0x800F5038 -> 0x800F5040
...
// Range: 0x800F5040 -> 0x800F50EC
static void AddAnimCustom(struct zAnimListAsset * a /* r29 */) {
    // Local variables
    int i; // r30
    unsigned int size; // r1+0x8
    void * buf; // r0
    struct xAnimFile * afile; // r3
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80BFF718
...
// Range: 0x800F50EC -> 0x800F56F8
void zAnimListInit() {
    // Local variables
    int i; // r26
    unsigned int size; // r1+0x18
    struct zAnimListAsset * zala; // r28
    struct PKRAssetTOCInfo ainfo; // r1+0x1C
    struct xAnimTable * atbl; // r25
    void * buf; // r0
    struct xAnimFile * afile; // r29
    int idle_exists; // r0
    int j; // r24
    struct xAnimFile * afile; // r23

    // References
    // -> static int nals;
    // -> static int * anused;
    // -> static char __FUNCTION__[14];
    // -> static char * astnames[20];
    // -> static struct xAnimTable * * atbls;
    // -> static unsigned int * aids;
    // -> unsigned int gActiveHeap;
}

// Range: 0x800F56F8 -> 0x800F5710
void zAnimListExit() {
    // References
    // -> static int * anused;
    // -> static struct xAnimTable * * atbls;
...
// Range: 0x800F5710 -> 0x800F5758
struct xAnimTable * zAnimListGetTable(unsigned int id /* r0 */) {
    // Local variables
    int i; // r5
...
// Range: 0x800F5758 -> 0x800F57A0
int zAnimListGetNumUsed(unsigned int id /* r0 */) {
    // Local variables
    int i; // r5
