/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xJSP.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80074A80 -> 0x80076974
*/
static struct RwV3d * sCurrVert; // size: 0x4, address: 0x80C07408
static unsigned int sAtomicStartCount; // size: 0x4, address: 0x80C0740C
static struct RwV3d * * sAtomicStartVert; // size: 0x4, address: 0x80C07410
// total size: 0x8
struct RwObject {
    // Members
...
// Range: 0x80074A80 -> 0x80074AA8
static struct RpAtomic * CountAtomicCB(struct RpAtomic * atomic /* r0 */, void * data /* r0 */) {
    // References
    // -> static unsigned int sAtomicStartCount;
}
...
// Range: 0x80074AA8 -> 0x80074B38
static struct RpMesh * AddMeshCB(struct RpMesh * mesh /* r28 */, void * pData /* r29 */) {
    // Local variables
    struct RwV3d * * stripVert; // r0
    unsigned int i; // r30

    // References
    // -> static struct RwV3d * sCurrVert;
}

// Range: 0x80074B38 -> 0x80074BA8
static struct RpAtomic * AddAtomicCB(struct RpAtomic * atomic /* r31 */, void * data /* r5 */) {
    // References
    // -> static struct RwV3d * sCurrVert;
    // -> static unsigned int sAtomicStartCount;
    // -> static struct RwV3d * * sAtomicStartVert;
}

// Range: 0x80074BA8 -> 0x80074BE4
static struct RpAtomic * AddAtomicPrecalcedVertCB(struct RpAtomic * atomic /* r0 */, void * data /* r0 */) {
    // References
    // -> static unsigned int sAtomicStartCount;
    // -> static struct RwV3d * * sAtomicStartVert;
}

// Range: 0x80074BE4 -> 0x80074BFC
static struct RpAtomic * ListAtomicCB(struct RpAtomic * atomic /* r0 */, void * data /* r0 */) {
    // Local variables
    struct RpAtomic * * * aList; // r0
}
...
struct xJSPExtraInfo {
    // Members
    char idtag[4]; // offset 0x0, size 0x4
    struct xBox bounds; // offset 0x4, size 0x18
};
// total size: 0x48
struct xJSPHeaderEx : public xJSPHeader {
    // Members
    struct xJSPExtraInfo extraInfo; // offset 0x2C, size 0x1C
};
// total size: 0x10
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFD0D0
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x80074BFC -> 0x80074D74
unsigned int get_matid(const struct xJSPHeader * jsp /* r26 */, unsigned int old_mat_id /* r27 */, unsigned int jspAssetID /* r28 */) {
    // Local variables
    char num[100]; // r1+0x8
    unsigned int matid; // r25
    int count; // r29
    int i; // r28
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFD0DC
// total size: 0xC
struct __rwMark {
    // Members
    unsigned int type; // offset 0x0, size 0x4
...
// Range: 0x80074D74 -> 0x80075538
void xJSP_MultiStreamRead(void * data /* r30 */, unsigned int size /* r27 */, struct xJSPHeader * * jsp /* r31 */, unsigned int assetID /* r28 */) {
    // Local variables
    int i; // r7
    struct RpClump * clump; // r28
    struct xClumpCollBSPTree * colltree; // r0
    int rwresult; // r0
    struct xJSPHeaderEx * hdr; // r29
    struct __rwMark mark; // r1+0x28
    struct __rwMark * mp; // r0
    struct xJSPHeader * tmphdr; // r30
    struct RwV3d * currVec; // r1+0x10
    struct RwV3d * currVec; // r1+0xC
    struct xJSPNodeTree * tmpNodeTree; // r27
    unsigned int * stripvecdata; // r4
    struct __rwMark * mp2; // r27
    struct __rwMark mark2; // r1+0x1C
    struct RwMemory rwmem; // r1+0x14
    struct RwStream * stream; // r27
    int i; // r30
    int atomCount; // r0
    struct RpAtomic * * atomList; // r31
    struct RpAtomic * * atomCurr; // r1+0x8
...
unsigned char needToLock; // size: 0x1, address: 0x80C078B0
// total size: 0x2C8
struct OSContext {
    // Members
...
struct OSSemaphore asyncSema; // size: 0xC, address: 0x80C078A4
static char __FUNCTION__[13]; // size: 0xD, address: 0x80BFD0F4
// Range: 0x80075578 -> 0x80075724
void xJSP_Destroy(struct xJSPHeader * jsp /* r31 */) {
    // Local variables
    int i; // r0
    struct RwLLLink * cur; // r4
    struct RwLLLink * end; // r0
    struct RpAtomic * apAtom; // r0
    unsigned int test; // r1+0x8
...
// Range: 0x80075724 -> 0x800757EC
static void xJSPNodeLightBuild(struct xJSPHeader * jsp /* r31 */) {
    // Local variables
    struct xJSPNodeLight * nlight; // r7
    struct RwLLLink * cur; // r8
    struct RwLLLink * end; // r9
    struct RpClump * clump; // r3
    struct RpAtomic * apAtom; // r10
...
struct TempSplit {
    // Members
    struct TempSplitChild leftChild; // offset 0x0, size 0x8
    struct TempSplitChild rightChild; // offset 0x8, size 0x8
    int leftCount; // offset 0x10, size 0x4
    int rightCount; // offset 0x14, size 0x4
    int axis; // offset 0x18, size 0x4
    float leftValue; // offset 0x1C, size 0x4
    float rightValue; // offset 0x20, size 0x4
};
static struct TempSplit * sSplitList; // size: 0x4, address: 0x80C07414
static struct TempSplit * sSplitCurr; // size: 0x4, address: 0x80C07418
static struct xJSPNodeTreeLeaf * sSplitLeaf; // size: 0x4, address: 0x80C0741C
// total size: 0x18
struct RwBBox {
    // Members
...
// Range: 0x800757EC -> 0x80075A74
static float GetSplitVolumeSum(struct xJSPNodeTreeLeaf * tleaf /* r3 */, int count /* r25 */, int axis /* r26 */, float coord /* f31 */) {
    // Local variables
    int i; // r29
    int numleft; // r28
    int numright; // r27
    struct RwBBox leftbox; // r1+0x20
    struct RwBBox rightbox; // r1+0x8
}
...
// Range: 0x80075A74 -> 0x80075EC8
static int ChooseSplit_MeanMinSum(struct xJSPNodeTreeLeaf * tleaf /* r28 */, int count /* r29 */, int & bestaxis /* r30 */, float & bestcoord /* r31 */) {
    // Local variables
    float bestvolume; // f31
    int i; // r5
    int axis; // r24
    struct RwV3d mean; // r1+0x14
    float testarray[3]; // r1+0x8
    float meancoord; // f1
    float coord; // f2
    float testvol; // f1
}
...
// Range: 0x80075EC8 -> 0x8007613C
static void RecurseSplitJSPNode(struct xJSPNodeTreeLeaf * tleaf /* r27 */, int count /* r28 */, struct TempSplitChild * childptr /* r29 */) {
    // Local variables
    int i; // r25
    int bestaxis; // r1+0xC
    int lastright; // r30
    float bestcoord; // r1+0x8
    struct xJSPNodeTreeLeaf templeaf; // r1+0x10
    struct TempSplit * thisSplit; // r25

    // References
    // -> static struct xJSPNodeTreeLeaf * sSplitLeaf;
    // -> static struct TempSplit * sSplitCurr;
    // -> static struct TempSplit * sSplitList;
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFD104
// Range: 0x80076180 -> 0x80076564
struct xJSPNodeTree * xJSPNodeTreeBuild(struct xJSPHeader * jsp /* r0 */) {
    // Local variables
    struct RpClump * clump; // r24
    int i; // r5
    int j; // r0
    int atomCount; // r30
    struct RpAtomic * * atomList; // r29
    struct RpAtomic * * atomCurr; // r1+0x8
    struct xJSPNodeTreeLeaf * tleaf; // r28
    struct RpGeometry * geom; // r6
    int numVert; // r0
    struct RwV3d * vert; // r3
    struct xJSPNodeTree * jntree; // r27
...
static char __FUNCTION__[36]; // size: 0x24, address: 0x804186D0
// total size: 0x8
struct nodeInfo {
    // Members
    unsigned int type; // offset 0x0, size 0x4
...
// Range: 0x80076564 -> 0x800767A0
struct xJSPHeader * xJSP_ForAllBoxLeafNodeIntersections(struct xJSPHeader * hdr /* r26 */, struct RwBBox * box /* r27 */, int (* callBack)(struct xJSPHeader *, struct xJSPNodeTreeLeaf *, struct RwBBox *, void *) /* r28 */, void * data /* r29 */) {
    // Local variables
    int nStack; // r31
    struct nodeInfo nodeStack[64]; // r1+0x10
    struct nodeInfo node; // r1+0x8
    struct xJSPNodeTree * tree; // r30
    struct xJSPNodeTreeBranch * branch; // r5

    // References
    // -> static char __FUNCTION__[36];
}

// Range: 0x800767A0 -> 0x800768C8
static int JSPApplyLightCB(struct xJSPHeader * jsp /* r0 */, struct xJSPNodeTreeLeaf * leaf /* r4 */, struct RwBBox * box /* r0 */, void * data /* r0 */) {
    // Local variables
    int i; // r8
    struct xJSPNodeLight * nlight; // r9
}

// Range: 0x800768C8 -> 0x8007693C
void xJSP_ApplyLocalLight(struct xJSPHeader * jsp /* r0 */, struct RpLight * light /* r4 */) {
    // Local variables
    struct RwFrame * lframe; // r7
    float lradius; // f4
    struct RwBBox lbox; // r1+0x8
}

// Range: 0x8007693C -> 0x80076974
void xJSP_ClearLocalLight(struct xJSPHeader * jsp /* r0 */) {
    // Local variables
    unsigned int i; // r5
    struct xJSPNodeLight * nlight; // r6
}
