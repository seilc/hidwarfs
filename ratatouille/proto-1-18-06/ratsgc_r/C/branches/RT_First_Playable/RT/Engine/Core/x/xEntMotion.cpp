/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xEntMotion.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80054CBC -> 0x80058DD4
*/
char __FUNCTION__$localstatic1$xSpline3_ArcTotal__FP8xSpline3[18]; // size: 0x12, address: 0x80BFC8A8
// total size: 0x10
struct basic_rect {
    // Static members
...
// total size: 0x4
struct /* @class$1097xEntMotion_cpp */ {
    // Members
...
union /* @class$1236xEntMotion_cpp */ {
...
// total size: 0xC
struct /* @class$1235xEntMotion_cpp */ {
    // Members
...
static struct xEntMotion * * dbg_xems; // size: 0x4, address: 0x80C07268
static unsigned short ent_motion_dbg_num; // size: 0x2, address: 0x80C0726C
static unsigned short ent_motion_dbg_num_allocd; // size: 0x2, address: 0x80C0726E
static signed short ent_motion_dbg_idx; // size: 0x2, address: 0x80BFC8BA
static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFC8BC
// Range: 0x80054CBC -> 0x80055004
void xEntMotionInit(struct xEntMotion * motion /* r29 */, struct xEnt * owner /* r30 */, struct xEntMotionAsset * asset /* r31 */) {
    // Local variables
...
// Range: 0x80055004 -> 0x80055018
static void xEntSpeedDataReset(struct xEntSpeedData * speed_data /* r0 */) {}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFC8CC
// Range: 0x80055018 -> 0x800551EC
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFC8DC
// total size: 0x30
struct xMat3x3 {
    // Members
...
// Range: 0x800551EC -> 0x800558C0
void xEntMotionReset(struct xEntMotion * motion /* r29 */, struct xScene * sc /* r28 */) {
    // Local variables
    struct xEntMotionPenData * aspen; // r0
    float dangle; // f1
    struct xMat3x3 pshrot; // r1+0x40
    struct xVec3 totop; // r1+0x28
    struct xVec3 pshtotop; // r1+0x1C
    struct xVec3 pshdelta; // r1+0x10
    struct xMat3x3 * modlrot; // r31
    struct xVec3 * modlpos; // r28
    struct xEnt * ownr; // r30
    struct xEntMotionMechData * mkasst; // r0
    struct xEntMechData * mech; // r0
    float scale; // f31
    float model_scale; // f30
    float asset_scale; // f29
    float drot; // f29
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFC8EC
// Range: 0x800558C0 -> 0x80055A94
void xEntMotionMove(struct xEntMotion * motion /* r29 */, struct xScene * sc /* r30 */, float dt /* f31 */, struct xEntFrame * frame /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFC8FC
// Range: 0x80055A94 -> 0x80055D1C
...
// Range: 0x80055D1C -> 0x80055E5C
static void xEntOrbitMove(struct xEntMotion * motion /* r30 */, float dt /* f31 */, struct xEntFrame * frame /* r31 */) {
    // Local variables
    float th; // f1
    float u; // f29
    float s; // f0
...
// Range: 0x80055E5C -> 0x80055FA4
static void xEntMPGetNext(struct xEntMotion * motion /* r31 */, struct xMovePoint * prev /* r0 */) {
    // Local variables
    struct xEntMPData * mp; // r0
    struct xVec3 tempdir; // r1+0x10
    struct xMat3x3 mat; // r1+0x20
}

// Range: 0x80055FA4 -> 0x8005602C
static void xEntSpeedDataUpdate(struct xEntSpeedData * speed_data /* r0 */, float dt /* f0 */) {}
...
// Range: 0x8005602C -> 0x80056528
static void xEntMPMove(struct xEntMotion * motion /* r28 */, struct xScene * sc /* r29 */, float dt /* f28 */, struct xEntFrame * frame /* r30 */) {
    // Local variables
    struct xEntMPData * mp; // r31
    float newdist; // f27
    struct xMovePoint * prev; // r4
    float qdot; // f27
    struct xVec3 tgt; // r1+0x54
    struct xVec3 direction; // r1+0x48
    struct xVec3 bank; // r1+0x3C
    struct xQuat quat; // r1+0x2C
    struct xQuat qold; // r1+0x1C
    struct xMat3x3 tmpmat; // r1+0x60
    float u; // f27
    struct xVec3 gravity; // r1+0x10
}

struct xVec3 g_O3; // size: 0xC, address: 0x80C0ADA0
// Range: 0x80056528 -> 0x80056808
static void xEntSplineMove(struct xEntMotion * motion /* r29 */, float dt /* f28 */, struct xEntFrame * frame /* r30 */) {
    // Local variables
    struct xEntSplineData * spline_data; // r31
    unsigned char still_on_spline; // r0
    struct xVec3 target_position; // r1+0x50
    struct xVec3 new_position; // r1+0x44
    struct xVec3 dir; // r1+0x38
    struct xMat3x3 mat; // r1+0x60
...
// Range: 0x80056808 -> 0x80056950
static void xEntPenMove(struct xEntMotion * motion /* r28 */, float dt /* f31 */, struct xEntFrame * frame /* r29 */) {
    // Local variables
    struct xEntPenData * pen; // r30
    struct xEntMotionPenData * aspen; // r0
    float th; // f3
    float v; // f29
    float dangle; // f1
...
// Range: 0x80056950 -> 0x80056D38
static void xEntMechMove(struct xEntMotion * motion /* r29 */, struct xScene * sc /* r27 */, float dt /* f31 */, struct xEntFrame * frame /* r28 */) {
    // Local variables
    struct xEntMechData * mech; // r0
    struct xEntMotionMechData * mkasst; // r0
    unsigned int last; // r30
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFC908
// Range: 0x80056D38 -> 0x80057048
static unsigned int xEntScale(struct xEntMotion * motion /* r28 */, float dt /* f31 */, struct xEntFrame * frame /* r30 */) {
    // Local variables
    struct xEntMechData * mech; // r0
    struct xEntMotionMechData * mkasst; // r0
...
// Range: 0x80057048 -> 0x8005737C
static unsigned int xEntSldMove(struct xEntMotion * motion /* r30 */, float dt /* f30 */, struct xEntFrame * frame /* r31 */) {
    // Local variables
    struct xEntMechData * mech; // r0
    struct xEntMotionMechData * mkasst; // r0
    unsigned int last; // r3
    float tmradj; // f2
    float rem; // f3
    float speed; // f31
}

// Range: 0x8005737C -> 0x80057584
static unsigned int xEntRotMove(struct xEntMotion * motion /* r28 */, float dt /* f1 */, struct xEntFrame * frame /* r29 */) {
    // Local variables
    struct xEntMechData * mech; // r0
    struct xEntMotionMechData * mkasst; // r0
    unsigned int last; // r31
    ...
    float speed; // f2
    struct xEnt * ownr; // r3
    struct xEntAsset * easst; // r30
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFC914
// Range: 0x80057584 -> 0x800577F4
void xEntMechForward(struct xEntMotion * motion /* r31 */) {
    // Local variables
    struct xEntMechData * mech; // r0
    struct xEntMotionMechData * mkasst; // r0
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFC924
// Range: 0x800577F4 -> 0x80057ABC
void xEntMechReverse(struct xEntMotion * motion /* r31 */) {
    // Local variables
    struct xEntMechData * mech; // r0
    struct xEntMotionMechData * mkasst; // r0
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFC934
// Range: 0x80057ABC -> 0x80057D1C
void xEntMotionTranslate(struct xEntMotion * motion /* r29 */, const struct xVec3 * dpos /* r30 */, struct xMat4x3 * dmat /* r31 */) {
    // References
    // -> static char __FUNCTION__[20];
}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFC948
unsigned int gActiveHeap; // size: 0x4, address: 0x80C07448
// Range: 0x80057D1C -> 0x80057DDC
...
// Range: 0x80057DF8 -> 0x80057E20
static void xEntMotionDebugAdd(struct xEntMotion * xem /* r0 */) {
    // References
    // -> static unsigned short ent_motion_dbg_num;
    // -> static struct xEntMotion * * dbg_xems;
    // -> static unsigned short ent_motion_dbg_num_allocd;
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFC95C
// Range: 0x80057E20 -> 0x80057F24
...
// Range: 0x80057F24 -> 0x80058A84
static void xEntMotionDebugWrite(const struct xEntMotion * xem /* r30 */) {
    // Local variables
    char * gps; // r31
...
// Range: 0x80058A84 -> 0x80058CD0
static void xEntMotionDebugDraw(const struct xEntMotion * xem /* r31 */) {
    // Local variables
    struct xMovePoint * xmp; // r28
    unsigned short idx; // r27
    struct xMovePoint * omp; // r26
    unsigned short jdx; // r25
    struct xMovePoint * pmp; // r0
...
// total size: 0x10
struct /* @class$1569xEntMotion_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
