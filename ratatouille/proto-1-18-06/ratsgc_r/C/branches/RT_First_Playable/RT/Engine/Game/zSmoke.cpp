/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zSmoke.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80186D14 -> 0x8018934C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static float magnitude_freq; // size: 0x4, address: 0x80C01F80
static float magnitude_min; // size: 0x4, address: 0x80C01F84
static float magnitude_max; // size: 0x4, address: 0x80C01F88
static float yaw_freq; // size: 0x4, address: 0x80C01F8C
static float yaw_min; // size: 0x4, address: 0x80C01F90
static float yaw_max; // size: 0x4, address: 0x80C01F94
static float pitch_freq; // size: 0x4, address: 0x80C01F98
static float pitch_min; // size: 0x4, address: 0x80C01F9C
static float pitch_max; // size: 0x4, address: 0x80C01FA0
// total size: 0x8
struct substr {
    // Members
...
// total size: 0x1
struct /* @class$1002zSmoke_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1003zSmoke_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1004zSmoke_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1005zSmoke_cpp */ {
    // Members
...
static struct tweak_callback cb_deg_lteq; // size: 0x28, address: 0x803E2198
static struct tweak_callback cb_deg_gteq; // size: 0x28, address: 0x803E21C0
...
static void add_tweaks() {
    ...
    // -> static struct tweak_callback cb_deg_gteq;
    // -> static float pitch_max;
    // -> static struct tweak_callback cb_deg_lteq;
    // -> static float pitch_freq;
    // -> static float yaw_min;
    // -> static float yaw_max;
    // -> static float yaw_freq;
    // -> static float magnitude_min;
    // -> struct tweak_callback cb_tweak_force_gteq;
    // -> static float magnitude_max;
    // -> struct tweak_callback cb_tweak_force_lteq;
    // -> static float magnitude_freq;
}

static float wind_magnitude_time; // size: 0x4, address: 0x80C087B8
static float wind_yaw_time; // size: 0x4, address: 0x80C087BC
static float wind_pitch_time; // size: 0x4, address: 0x80C087C0
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
static struct xVec3 wind_delta; // size: 0xC, address: 0x80C087C4
// total size: 0x8
...
static class xParticleBatchGroup batch_group; // size: 0x8, address: 0x80C087D0
static class zSmokeEmitter * hilighted; // size: 0x4, address: 0x80C087D8
// total size: 0x18
struct analog_data {
    // Members
...
struct smoke_particle {
    ...
    struct xVec2 uv[2]; // offset 0x20, size 0x10
    float age; // offset 0x30, size 0x4
    float age_rate; // offset 0x34, size 0x4
    float growth; // offset 0x38, size 0x4
    float wind; // offset 0x3C, size 0x4
    float dr; // offset 0x40, size 0x4
    float dg; // offset 0x44, size 0x4
    float db; // offset 0x48, size 0x4
    float da; // offset 0x4C, size 0x4
};
// total size: 0x10
struct RwSphere {
    // Members
...
// Range: 0x80186F00 -> 0x80187220
static int update_particles(unsigned char * mem /* r22 */, int count /* r0 */, struct ptank_pool & apool /* r5 */, float dt /* f29 */) {
    // Local variables
    ...
    struct xVec2 * pool_uv; // r25
    struct smoke_particle * p; // r24
    struct smoke_particle * end; // r23
    float oldsize; // f30
    ...
    // -> static struct xVec3 wind_delta;
}
...
// Range: 0x80187220 -> 0x80187280
static void set_system(struct xParticleBatchSystem & system /* r0 */, struct RwTexture * texture /* r0 */, unsigned char behind_fire /* r0 */) {}
...
// Range: 0x80187280 -> 0x80187388
static void perturb_dir(struct xVec3 & dir /* r31 */, float mag /* f0 */) {
    // Local variables
    float rand_scale; // f31
    float ang_min; // f30
    struct xVec3 ang; // r1+0x10
    struct xMat3x3 mat; // r1+0x20
}

// Range: 0x80187388 -> 0x801875D8
static void update_wind(float dt /* f29 */) {
    // Local variables
    float magnitude_range; // f26
    float magnitude; // f31
    float yaw_range; // f26
    float yaw; // f30
    float pitch_range; // f26
    float pitch; // f25
    float sy; // f0
    float cy; // f0
    float sp; // f0
    ...
    // References
    // -> static struct xVec3 wind_delta;
    // -> static float wind_pitch_time;
    // -> static float pitch_min;
    // -> static float pitch_max;
    // -> static float pitch_freq;
    // -> static float wind_yaw_time;
    // -> static float yaw_min;
    // -> static float yaw_max;
    // -> static float yaw_freq;
    // -> static float wind_magnitude_time;
    // -> static float magnitude_min;
    // -> static float magnitude_max;
    // -> static float magnitude_freq;
}
...
// total size: 0x28
struct /* @class$2204zSmoke_cpp */ {
    // Members
    unsigned int attach_to; // offset 0x0, size 0x4
    struct xVec3 loc; // offset 0x4, size 0xC
    struct xVec3 dir; // offset 0x10, size 0xC
    struct xVec3 scale; // offset 0x1C, size 0xC
};
// total size: 0x80
struct asset_type : public xDynAsset {
    ...
    struct /* @class$2204zSmoke_cpp */ {
        ...
        struct xVec3 dir; // offset 0x10, size 0xC
        struct xVec3 scale; // offset 0x1C, size 0xC
    } region; // offset 0x14, size 0x28
    unsigned int texture; // offset 0x3C, size 0x4
    unsigned short texture_rows; // offset 0x40, size 0x2
    unsigned short texture_columns; // offset 0x42, size 0x2
    float rate; // offset 0x44, size 0x4
    float life_min; // offset 0x48, size 0x4
    float life_max; // offset 0x4C, size 0x4
    float size_min; // offset 0x50, size 0x4
    float size_max; // offset 0x54, size 0x4
    float vel_min; // offset 0x58, size 0x4
    float vel_max; // offset 0x5C, size 0x4
    float growth; // offset 0x60, size 0x4
    struct xVec3 vel_dir; // offset 0x64, size 0xC
    float vel_dir_vary; // offset 0x70, size 0x4
    float wind; // offset 0x74, size 0x4
    struct xColor_tag color_birth; // offset 0x78, size 0x4
    struct xColor_tag color_death; // offset 0x7C, size 0x4
};
static struct asset_type dummy_asset; // size: 0x80, address: 0x80BB22D0
// total size: 0x20
struct xLinkAsset {
    // Members
...
struct debug_info {
    // Members
    unsigned char track_player; // offset 0x0, size 0x1
};
// total size: 0xA0
class zSmokeEmitter : private xBase {
    ...
    void prepare_volume(struct zSmokeSphereVolume & volume) const;

    struct RwTexture * get_texture() const;
...
static class zSmokeEmitter dummy_emitter; // size: 0xA0, address: 0x80BB2350
// Range: 0x801875D8 -> 0x8018772C
static void create_dummy_emitter() {
    // References
    // -> static struct asset_type dummy_asset;
    // -> static class zSmokeEmitter dummy_emitter;
}

// Range: 0x8018772C -> 0x80187758
static void debug_init() {
    // References
    // -> static class zSmokeEmitter * hilighted;
}

// Range: 0x80187758 -> 0x80187798
void zSmokeSceneEnter() {
    // References
    // -> static class xParticleBatchGroup batch_group;
    // -> static struct xVec3 wind_delta;
    // -> static float wind_pitch_time;
    // -> static float wind_yaw_time;
    // -> static float wind_magnitude_time;
...
// Range: 0x80187798 -> 0x8018779C
void zSmokeSceneExit() {}

// Range: 0x8018779C -> 0x801877D0
void zSmokeUpdate(float dt /* f31 */) {}
...
struct zSmokeGenom {
    ...
    float growth; // offset 0x34, size 0x4
    float wind; // offset 0x38, size 0x4
};
static char __FUNCTION__[17]; // size: 0x11, address: 0x80C01FA4
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x801877D0 -> 0x80187CBC
void zSmokeEmitSphere(int system_id /* r27 */, const struct zSmokeSphereVolume & volume /* r28 */, int amount /* r29 */) {
    // Local variables
    float size_min; // f19
    float size_rand_scale; // r1+0x88
    float age_rate_min; // f18
    float age_rate_rand_scale; // r1+0x84
    struct xColor_tag color_birth; // r1+0xC
    float dr; // r1+0x80
    float dg; // f17
    float db; // f16
    float da; // f15
    ...
    float cz; // f29
    float radius; // f28
    struct xVec3 vel_dir; // r1+0x10
    float vel_dir_vary; // f27
    float vel_min; // f26
    float vel_rand_scale; // f25
    float growth; // f24
    float wind; // f23
    int texture_rows; // r31
    int max_frame; // r30
    float du; // f22
    float dv; // f14
    unsigned char * mem; // r1+0x8
    int have; // r0
    struct smoke_particle * p; // r25
    struct smoke_particle * endp; // r24
    int frame; // r5
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80C01FB8
// Range: 0x80187DB8 -> 0x80187F38
void zSmokeEmitter::load(struct xBase & data /* r29 */, struct xDynAsset & asset /* r30 */, unsigned long asset_size /* r31 */) {
    // Local variables
...
// Range: 0x80187F38 -> 0x80187F48
char * asset_type::type_name() {}

// Range: 0x80187F48 -> 0x80188158
// this: r31
void zSmokeEmitter::load(const struct asset_type & a /* r0 */) {
    // Local variables
    struct xMat3x3 dir_mat; // r1+0x10
}

// Range: 0x80188158 -> 0x8018831C
// this: r31
void zSmokeEmitter::debug_reset() {
    // Local variables
    struct xMat3x3 dir_mat; // r1+0x10
}

// Range: 0x8018831C -> 0x80188334
// this: r0
void zSmokeEmitter::reset() {}

// Range: 0x80188334 -> 0x801885C8
// this: r26
void zSmokeEmitter::update(float dt /* f28 */) {
    // Local variables
    int emit; // r29
    unsigned char * mem; // r1+0x8
    int have; // r0
    struct smoke_particle * p; // r28
    struct smoke_particle * endp; // r27
    int frame; // r5
}
...
// Range: 0x801885C8 -> 0x80188688
// this: r30
void zSmokeEmitter::prepare_volume(struct zSmokeSphereVolume & volume /* r31 */) const {}

// Range: 0x80188688 -> 0x801886C4
// this: r0
struct RwTexture * zSmokeEmitter::get_texture() const {
    // Local variables
...
// Range: 0x801886C4 -> 0x80188728
// this: r31
void zSmokeEmitter::create_system() {
    // Local variables
...
// Range: 0x80188728 -> 0x801887C4
// this: r31
void zSmokeEmitter::refresh_mat() {}
...
// Range: 0x801887C4 -> 0x80188858
void zSmokeEmitter::update_all(float dt /* f31 */) {
    // Local variables
    struct zScene & scene; // r3
    class zSmokeEmitter * it; // r31
    class zSmokeEmitter * end; // r30
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80C01FC0
// Range: 0x80188858 -> 0x80188980
void zSmokeEmitter::cb_dispatch(struct xBase * to /* r31 */, unsigned int event /* r30 */) {
    // Local variables
    class zSmokeEmitter & e; // r31

    // References
    // -> static char __FUNCTION__[12];
}

// Range: 0x80188980 -> 0x801889C8
// this: r31
void zSmokeEmitter::debug_init() {}
...
// Range: 0x801889C8 -> 0x80188B50
// this: r31
void zSmokeEmitter::debug_update() {
    // Local variables
    struct xBox box; // r1+0x34
    struct xMat3x3 vel_mat; // r1+0x50
    struct xVec3 end_cone; // r1+0x28
    float end_cone_r; // f2
    struct xVec3 wind_vel; // r1+0x1C
...
static struct tweak_callback cb_hilight; // size: 0x28, address: 0x803E21E8
static struct tweak_callback cb_move; // size: 0x28, address: 0x803E2210
static struct tweak_callback cb_move_dir; // size: 0x28, address: 0x803E2238
static struct tweak_callback cb_frame; // size: 0x28, address: 0x803E2260
static struct tweak_callback cb_life; // size: 0x28, address: 0x803E2288
static struct tweak_callback cb_vel_dir; // size: 0x28, address: 0x803E22B0
static struct tweak_callback cb_color; // size: 0x28, address: 0x803E22D8
struct tweak_callback cb_tweak_convert_degrees; // size: 0x28, address: 0x803A9AF0
// Range: 0x80188B50 -> 0x801890B8
// this: r31
void zSmokeEmitter::add_tweaks() {
    // Local variables
    ...
    // References
    // -> static class xParticleBatchGroup batch_group;
    // -> static struct tweak_callback cb_color;
    // -> struct tweak_callback cb_tweak_convert_degrees;
    // -> static struct tweak_callback cb_vel_dir;
    // -> static struct tweak_callback cb_life;
    // -> static struct tweak_callback cb_frame;
    // -> static struct tweak_callback cb_move_dir;
    // -> static struct tweak_callback cb_move;
    // -> static struct tweak_callback cb_hilight;
    // -> static class zSmokeEmitter dummy_emitter;
}

enum /* @enum$2203zSmoke_cpp */ {
    FLAG_ON = 1,
    FLAG_BEHIND_FIRE = 2,
};
// Range: 0x801890B8 -> 0x801890C4
void zSmokeEmitter::enable_hilight(const struct tweak_info & info /* r0 */) {
    // References
    // -> static class zSmokeEmitter * hilighted;
}

// Range: 0x801890C4 -> 0x801890D0
void zSmokeEmitter::disable_hilight() {
    // References
    // -> static class zSmokeEmitter * hilighted;
}

// Range: 0x801890D0 -> 0x80189140
void zSmokeEmitter::on_move(const struct tweak_info & info /* r0 */) {
    // Local variables
    class zSmokeEmitter & e; // r31
    struct asset_type & a; // r30
}

// Range: 0x80189140 -> 0x801891B0
void zSmokeEmitter::on_change_frame(const struct tweak_info & info /* r0 */) {
    // Local variables
    class zSmokeEmitter & e; // r5
}

// Range: 0x801891B0 -> 0x801891E0
void zSmokeEmitter::on_change_life(const struct tweak_info & info /* r0 */) {
    // Local variables
    class zSmokeEmitter & e; // r3
    struct asset_type & a; // r4
}

// Range: 0x801891E0 -> 0x80189270
void zSmokeEmitter::on_change_vel_dir(const struct tweak_info & info /* r0 */) {
    // Local variables
    class zSmokeEmitter & e; // r31
    struct asset_type & a; // r30
    struct xMat3x3 dir_mat; // r1+0x10
...
// Range: 0x80189270 -> 0x8018934C
void zSmokeEmitter::on_change_color(const struct tweak_info & info /* r0 */) {
    // Local variables
    class zSmokeEmitter & e; // r5
    struct asset_type & a; // r6
}
