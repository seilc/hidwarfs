/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zAnimCache.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800F4C18 -> 0x800F5030
*/
static unsigned int cachedAnimations; // size: 0x4, address: 0x80C07B68
static struct zAnimCacheEntry cachedEntry[128]; // size: 0x600, address: 0x80B86BF0
static unsigned int currentCacheIndex; // size: 0x4, address: 0x80C07B6C
static struct xVec3 cache[2560]; // size: 0x7800, address: 0x80B871F0
static char __FUNCTION__[12]; // size: 0xC, address: 0x80BFF6F0
// total size: 0x8
struct RwObject {
    // Members
...
// Range: 0x800F4C18 -> 0x800F4D98
static void fillInCache(struct zAnimCacheEntry * cacheEntry /* r28 */, struct RpAtomic * model /* r29 */, float startTime /* f30 */, float endTime /* f29 */) {
    // Local variables
    struct xVec3 tempPositions[100]; // r1+0x1F50
    struct xQuat tempQuats[100]; // r1+0x1910
    struct xMat4x3 tempMats[100]; // r1+0x10
    struct xVec3 * currentCachePos; // r30
    float time; // f30

    // References
    // -> static unsigned int currentCacheIndex;
    // -> static char __FUNCTION__[12];
}

// Range: 0x800F4D98 -> 0x800F4DA8
void zAnimCacheClear() {
    // References
    // -> static unsigned int currentCacheIndex;
    // -> static unsigned int cachedAnimations;
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFF6FC
// total size: 0x10
struct hitBoneInfo {
    // Members
...
// total size: 0x4
struct /* @class$1080zAnimCache_cpp */ {
    // Members
...
union /* @class$1245zAnimCache_cpp */ {
...
// total size: 0xC
struct /* @class$1488zAnimCache_cpp */ {
    // Members
...
// Range: 0x800F4DA8 -> 0x800F4FE4
void zAnimCacheCreateData(class zCombatAttack * attackTableStates /* r24 */, unsigned int attackTableCount /* r25 */, struct xAnimPlay * animPlay /* r26 */) {
    // Local variables
    unsigned int i; // r29
    unsigned int j; // r28
    struct zAnimCacheEntry newCacheEntry; // r1+0x8
    struct xAnimState * state; // r0
    unsigned int entryIndex; // r27

    // References
    // -> static struct zAnimCacheEntry cachedEntry[128];
    // -> static unsigned int currentCacheIndex;
    // -> static struct xVec3 cache[2560];
    // -> static unsigned int cachedAnimations;
    // -> static char __FUNCTION__[21];
}

// Range: 0x800F5000 -> 0x800F5030
static unsigned char operator!=(const struct zAnimCacheEntry & entry1 /* r0 */, const struct zAnimCacheEntry & entry2 /* r0 */) {}
