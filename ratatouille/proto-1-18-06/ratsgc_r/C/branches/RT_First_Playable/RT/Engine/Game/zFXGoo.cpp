/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zFXGoo.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x802118E8 -> 0x80213768
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static float defaultGooTimes[4]; // size: 0x10, address: 0x80C04088
static float defaultGooWarbc[4]; // size: 0x10, address: 0x80C04098
// total size: 0x8
struct RwObject {
    // Members
...
struct zFXGooInstance {
    ...
    int freezeGroup; // offset 0x4, size 0x4
    struct xVec3 * orig_verts; // offset 0x8, size 0x4
    struct RwRGBA * orig_colors; // offset 0xC, size 0x4
    struct RwTexCoords * orig_uvs; // offset 0x10, size 0x4
    float time; // offset 0x14, size 0x4
    float timer; // offset 0x18, size 0x4
    float w0; // offset 0x1C, size 0x4
    float w2; // offset 0x20, size 0x4
    float warbc[4]; // offset 0x24, size 0x10
    float state_time[4]; // offset 0x34, size 0x10
    struct xVec3 center; // offset 0x44, size 0xC
    enum zFXGooState state; // offset 0x50, size 0x4
    float warb_time; // offset 0x54, size 0x4
    float alpha; // offset 0x58, size 0x4
    float min; // offset 0x5C, size 0x4
    float max; // offset 0x60, size 0x4
    struct xVec3 * ref_parentPos; // offset 0x64, size 0x4
    struct xVec3 pos_parentOnFreeze; // offset 0x68, size 0xC
    unsigned int pipeFlags; // offset 0x74, size 0x4
    struct RpGeometry * geom2; // offset 0x78, size 0x4
};
struct zFXGooInstance zFXGooInstances[24]; // size: 0xBA0, address: 0x80BC3DC0
float min2; // size: 0x4, address: 0x80C040A8
float max2; // size: 0x4, address: 0x80C040AC
float goo_freeze_coeff; // size: 0x4, address: 0x80C040B0
float goo_melt_coeff; // size: 0x4, address: 0x80C040B4
float wpow; // size: 0x4, address: 0x80C040B8
static struct RwTexture * g_txtr_gooFrozen; // size: 0x4, address: 0x80C08E88
static char __FUNCTION__[13]; // size: 0xD, address: 0x80C040BC
...
// Range: 0x802118E8 -> 0x80211DAC
void zFXGooEnable(struct xModelInstance * gooModel /* r27 */, int freezeGroup /* r0 */) {
    // Local variables
    int i; // r5
    struct RpAtomic * atomic; // r31
    struct zFXGooInstance * goo; // r30
    struct RpGeometry * geom; // r29
    int numverts; // r28
    int numtris; // r24
    struct RpMorphTarget * mtp2; // r23
    struct RwV3d * ov; // r22
    struct RwV3d * nv; // r21
    struct RwV3d * on; // r20
    struct RwV3d * nn; // r19
    struct RwRGBA * nc; // r18
    struct RwTexCoords * ouv; // r17
    struct RwTexCoords * nuv; // r16
    struct RpTriangle * ot; // r16
    struct RpTriangle * nt; // r17
    unsigned short v1; // r1+0xC
    unsigned short v2; // r1+0xA
    unsigned short v3; // r1+0x8
    struct RpMaterial * mp; // r5
    struct RwSphere boundingSphere; // r1+0x10
    struct xVec3 * orig_verts; // r16
    struct RwRGBA * orig_colors; // r17
    float * orig_uvs; // r18
    ...
    // -> float max2;
    // -> float min2;
    // -> static float defaultGooTimes[4];
    // -> static float defaultGooWarbc[4];
    // -> unsigned int gActiveHeap;
    // -> static char __FUNCTION__[13];
    // -> static struct RwTexture * g_txtr_gooFrozen;
    // -> struct zFXGooInstance zFXGooInstances[24];
}

// Range: 0x80211DAC -> 0x80211E1C
void zFXGoo_SceneEnter() {
    // Local variables
    int i; // r30
    struct zFXGooInstance * goo; // r29

    // References
    // -> struct zFXGooInstance zFXGooInstances[24];
}

// Range: 0x80211E1C -> 0x80211F04
void zFXGoo_SceneReset() {
    // Local variables
    int i; // r5
    struct zFXGooInstance * goo; // r6

    // References
    // -> struct zFXGooInstance zFXGooInstances[24];
}

// Range: 0x80211F04 -> 0x80211F94
...
unsigned int no_modify_goo; // size: 0x4, address: 0x80C08E8C
static char __FUNCTION__[21]; // size: 0x15, address: 0x80C040CC
enum zFXGooState {
    zFXGooStateNormal = 0,
...
// Range: 0x80211F94 -> 0x80212384
void zFXGooUpdateInstance(struct zFXGooInstance * goo /* r31 */, float dt /* f31 */) {
    // Local variables
    float tmp; // f0
    enum zFXGooState old_state; // r5
    int i; // r30
    struct RpGeometry * geom; // r29
    float t; // f30
    float * warbc; // r0
    ...
    // References
    // -> unsigned int no_modify_goo;
    // -> float wpow;
    // -> float goo_melt_coeff;
    // -> float goo_freeze_coeff;
    // -> static char __FUNCTION__[21];
}

// Range: 0x80212384 -> 0x802123F8
void zFXGooUpdate(float dt /* f31 */) {
    // Local variables
    int i; // r31
    struct zFXGooInstance * goo; // r30
...
static int doDebugDraw; // size: 0x4, address: 0x80C08E90
static signed char init; // size: 0x1, address: 0x80C08E94
static float tad; // size: 0x4, address: 0x80C08E98
static signed char init; // size: 0x1, address: 0x80C08E9C
static int addWorldOffset; // size: 0x4, address: 0x80C08EA0
static signed char init; // size: 0x1, address: 0x80C08EA4
static int addParentOffset; // size: 0x4, address: 0x80C08EA8
static signed char init; // size: 0x1, address: 0x80C08EAC
static int initTweaks; // size: 0x4, address: 0x80C08EB0
static signed char init; // size: 0x1, address: 0x80C08EB4
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C040E4
...
// Range: 0x802123F8 -> 0x80212D28
struct RpAtomic * zFXGooRenderAtomic(struct RpAtomic * atomic /* r20 */) {
    // Local variables
    int i; // r31
    int j; // r31
    struct zFXGooInstance * goo; // r30
    struct xVec3 vec_parMotion; // r1+0x80
    struct RxObjSpace3DVertex * Im3DBuffer; // r29
    unsigned int Im3DBufferPos; // r28
    struct RxObjSpace3DVertex * imv; // r27
    struct RpGeometry * geom; // r26
    struct xVec3 * vert; // r25
    struct xVec3 * normals; // r24
    struct RwRGBA * color; // r23
    float A; // f29
    float B; // f28
    unsigned char * alpha; // r22
    struct xVec3 vec; // r1+0x74
    float d2; // f0
    struct RpTriangle * tri; // r21
    struct xVec3 v0; // r1+0x68
    struct xVec3 v1; // r1+0x5C
    struct xVec3 v2; // r1+0x50
    struct xVec3 n0; // r1+0x44
    struct xVec3 n1; // r1+0x38
    struct xVec3 n2; // r1+0x2C
    struct xVec3 resultV0; // r1+0x20
    struct xVec3 resultV1; // r1+0x14
    struct xVec3 resultV2; // r1+0x8
    char str_buf[128]; // r1+0x8C
    struct zFXGooInstance * goo; // r21
    int a; // r22

    // References
    // -> static float tad;
    // -> static int addParentOffset;
    // -> static int addWorldOffset;
    // -> struct zFXGooInstance zFXGooInstances[24];
    // -> static float defaultGooTimes[4];
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80C040F8
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x80212D28 -> 0x80212F98
int zFXGooIs(struct xEnt * obj /* r29 */, float & depth /* r30 */) {
    // Local variables
    int i; // r5
    struct zFXGooInstance * goo; // r31
    struct xVec3 * pos; // r4
...
// total size: 0x1
struct /* @class$1029zFXGoo_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1030zFXGoo_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1031zFXGoo_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1032zFXGoo_cpp */ {
    // Members
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C04104
// Range: 0x80212F98 -> 0x802131B8
void zFXGooEventSetWarb(struct xEnt * ent /* r29 */, const float * warb /* r30 */) {
    // Local variables
    int i; // r5
    struct zFXGooInstance * goo; // r31
...
static char __FUNCTION__[29]; // size: 0x1D, address: 0x8043F2A8
// Range: 0x802131B8 -> 0x802134A4
void zFXGooEventSetFreezeDuration(struct xEnt * ent /* r29 */, float duration /* f31 */) {
    // Local variables
    int i; // r5
    struct zFXGooInstance * goo; // r31
    int freezeGroup; // r30
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80C04118
// Range: 0x802134A4 -> 0x80213768
void zFXGooEventMelt(struct xEnt * ent /* r30 */) {
    // Local variables
    int i; // r5
    struct zFXGooInstance * goo; // r29
    int freezeGroup; // r31
