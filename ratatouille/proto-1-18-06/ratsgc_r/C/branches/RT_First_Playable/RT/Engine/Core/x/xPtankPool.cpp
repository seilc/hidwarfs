/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xPtankPool.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800A11F8 -> 0x800A22DC
*/
static int MAX_PTANKS_COLOR_MAT; // size: 0x4, address: 0x80BFDE08
static int MAX_PTANKS_COLOR_MAT_UV2; // size: 0x4, address: 0x80BFDE0C
static int MAX_PTANKS_POS_COLOR_SIZE; // size: 0x4, address: 0x80BFDE10
static int MAX_PTANKS_POS_COLOR_SIZE_UV2; // size: 0x4, address: 0x80BFDE14
static int MAX_PTANKS_POS_COLOR_SIZE_ROT; // size: 0x4, address: 0x80BFDE18
static int MAX_PTANKS_POS_COLOR_SIZE_ROT_UV2; // size: 0x4, address: 0x80BFDE1C
static int MAX_PTANKS; // size: 0x4, address: 0x80C075E0
static unsigned char inited; // size: 0x1, address: 0x80C075E4
static unsigned int current_sequence_index; // size: 0x4, address: 0x80C075E8
// total size: 0x8
struct RwObject {
    // Members
...
static struct group_data groups[6]; // size: 0xA8, address: 0x8042BAE8
static int blocks_used[6]; // size: 0x18, address: 0x80C075EC
static char __FUNCTION__[13]; // size: 0xD, address: 0x80BFDE20
// Range: 0x800A11F8 -> 0x800A1374
static void sort_buckets(struct group_data & group /* r25 */) {
    // Local variables
    struct ptank_context * it; // r31
    struct ptank_context * endp; // r30
    const struct RwTexture * texture; // r29
    struct ptank_context * * bucket; // r28
    struct ptank_context * * end; // r27
    struct ptank_context * front; // r26
...
// total size: 0x4
struct /* @class$1350xPtankPool_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1488xPtankPool_cpp */ {
    // Members
...
union /* @class$1015xPtankPool_cpp */ {
...
// total size: 0x10
struct /* @class$1028xPtankPool_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80BFDE30
unsigned int gActiveHeap; // size: 0x4, address: 0x80C07448
// Range: 0x800A1438 -> 0x800A15F0
static void init_groups() {
    // Local variables
    int total; // r30
    struct group_data * it; // r29
    struct group_data * end; // r0
    unsigned char * mem; // r29
    struct group_data * it; // r28
    struct group_data * end; // r0
...
// Range: 0x800A15F0 -> 0x800A1744
static struct RpAtomic * create_ptank(unsigned int flags /* r30 */) {
    // Local variables
...
// Range: 0x800A1744 -> 0x800A17A0
static void destroy_ptank(struct RpAtomic * ptank /* r30 */) {
    // Local variables
    struct RwFrame * frame; // r31
}

// Range: 0x800A17A0 -> 0x800A1864
static void render_ptank(const struct ptank_context & context /* r30 */, int & render_flags /* r0 */) {
    // Local variables
    int xor_flags; // r7
    int noztest; // r8
    int zwrite; // r3
    unsigned int oldtest; // r31
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80BFDE3C
// Range: 0x800A1864 -> 0x800A190C
static int compare_ptanks(void * e1 /* r0 */, void * e2 /* r0 */) {
    // Local variables
    const struct ptank_context * p1; // r0
    const struct ptank_context * p2; // r0
    const struct RwTexture * tex1; // r0
    const struct RwTexture * tex2; // r5
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80BFDE4C
// Range: 0x800A190C -> 0x800A1AC4
void xPTankPoolSceneEnter() {
    // Local variables
    struct group_data * it; // r29
    struct group_data * end; // r31
    int want; // r28
    int size; // r0
...
// total size: 0xC
struct /* @class$964xPtankPool_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$965xPtankPool_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$966xPtankPool_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$967xPtankPool_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$968xPtankPool_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$969xPtankPool_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$970xPtankPool_cpp */ {
    // Members
...
// Range: 0x800A1AC4 -> 0x800A1BBC
static int create_ptanks(struct group_data & group /* r30 */, int want /* r4 */) {
    // Local variables
    int old_group_size; // r0
    struct ptank_context * head; // r3
    struct ptank_context * end; // r28
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFDE64
// Range: 0x800A1BBC -> 0x800A1C98
void xPTankPoolSceneExit() {
    // Local variables
    struct group_data * g; // r31
    struct group_data * endg; // r30
    struct ptank_context * p; // r29
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFDE78
// Range: 0x800A1C98 -> 0x800A1F48
void xPTankPoolRender() {
    // Local variables
    int i; // r30
    char name[128]; // r1+0xC
    struct ptank_context * ptanks; // r31
    struct ptank_context * ptank; // r30
    struct group_data * g; // r29
    struct group_data * endg; // r28
    struct ptank_context * p; // r27
    struct ptank_context * endp; // r26
    int render_flags; // r1+0x8
    struct ptank_context * end_ptank; // r0
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFDE8C
// total size: 0x10
struct render_state {
    // Members
    struct RwTexture * texture; // offset 0x0, size 0x4
...
// Range: 0x800A1F48 -> 0x800A2288
// this: r29
void ptank_pool::grab_block(enum ptank_group_type type /* r30 */) {
    // Local variables
    struct group_data & group; // r31
    struct ptank_context * * end; // r7
    struct ptank_context * * it; // r3
