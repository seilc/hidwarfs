/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\gc\iFile.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800D99F8 -> 0x800DAEC0
*/
static int tbuffer[1032]; // size: 0x1020, address: 0x80B6C500
static int * buffer32; // size: 0x4, address: 0x80C07910
// Range: 0x800D99F8 -> 0x800D9A10
void iFileInit() {
    // References
    // -> static int tbuffer[1032];
    // -> static int * buffer32;
}

// Range: 0x800D9A10 -> 0x800D9A14
void iFileExit() {}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFEFA8
void * RwEngineInstance; // size: 0x4, address: 0x80C09F34
// total size: 0xF0
struct tag_iFile {
    // Members
    unsigned int flags; // offset 0x0, size 0x4
    char path[128]; // offset 0x4, size 0x80
    int fd; // offset 0x84, size 0x4
    struct DVDFileInfo file; // offset 0x88, size 0x3C
    void (* cb)(struct tag_xFile *); // offset 0xC4, size 0x4
    int akey; // offset 0xC8, size 0x4
    int buffer[8]; // offset 0xCC, size 0x20
    int real_pos; // offset 0xEC, size 0x4
};
// total size: 0x114
struct tag_xFile {
    // Members
    char relname[32]; // offset 0x0, size 0x20
    struct tag_iFile ps; // offset 0x20, size 0xF0
    void * user_data; // offset 0x110, size 0x4
};
// Range: 0x800D9A14 -> 0x800D9B38
unsigned int * iFileLoad(const char * name /* r28 */, unsigned int * buffer /* r29 */, unsigned int * size /* r30 */, int flags /* r31 */) {
    // Local variables
    char fullpath[128]; // r1+0x8
    struct tag_xFile file; // r1+0x88
    unsigned int failure; // r0
    unsigned int fsize; // r0

    // References
    // -> static char __FUNCTION__[10];
    // -> void * RwEngineInstance;
}

// Range: 0x800D9B38 -> 0x800D9B64
void iFileFree(unsigned int * buffer /* r0 */) {
    // References
    // -> void * RwEngineInstance;
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFEFB4
// Range: 0x800D9B64 -> 0x800D9EBC
unsigned int iFileOpen(const char * name /* r28 */, int flags /* r29 */, struct tag_xFile * file /* r30 */) {
    // Local variables
    struct tag_iFile * ps; // r31
    char hostIOName[256]; // r1+0x18
    char openFlags[16]; // r1+0x8

    // References
    // -> static char __FUNCTION__[10];
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFEFC0
// Range: 0x800D9EBC -> 0x800DA04C
int iFileSeek(struct tag_xFile * file /* r31 */, int offset /* r29 */, int whence /* r30 */) {
    // Local variables
    struct tag_iFile * ps; // r0
    int real_offset; // r0

    // References
    // -> static char __FUNCTION__[10];
}

unsigned int iFileSyncAsyncReadActive; // size: 0x4, address: 0x80C07914
// Range: 0x800DA04C -> 0x800DA058
static void ifilereadCB() {
    // References
    // -> unsigned int iFileSyncAsyncReadActive;
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFEFCC
float SECS_PER_VBLANK; // size: 0x4, address: 0x80C08210
// Range: 0x800DA058 -> 0x800DA298
unsigned int iFileRead(struct tag_xFile * file /* r29 */, void * buf /* r30 */, unsigned int size /* r31 */) {
    // Local variables
    struct tag_iFile * ps; // r0
    unsigned int numItemsRead; // r0
    int result; // r0

    // References
    // -> unsigned int iFileSyncAsyncReadActive;
    // -> static char __FUNCTION__[10];
    // -> float SECS_PER_VBLANK;
}

// total size: 0x1C
struct file_queue_record {
    // Members
    struct tag_xFile * finfo; // offset 0x0, size 0x4
    void * buf; // offset 0x4, size 0x4
    unsigned int read_size; // offset 0x8, size 0x4
    unsigned int cur_read; // offset 0xC, size 0x4
    enum IFILE_READSECTOR_STATUS status; // offset 0x10, size 0x4
    void (* done)(struct tag_xFile *); // offset 0x14, size 0x4
    unsigned long key; // offset 0x18, size 0x4
};
struct file_queue_record file_queue[4]; // size: 0x70, address: 0x80B6D520
static char __FUNCTION__[9]; // size: 0x9, address: 0x80BFEFD8
// Range: 0x800DA298 -> 0x800DA49C
static void async_cb(long result /* r3 */, struct DVDFileInfo * finfo /* r0 */) {
    // Local variables
    struct file_queue_record * fq; // r31
    char * p; // r4
    long next_read; // r30
    long bytes; // r30

    // References
    // -> static char __FUNCTION__[9];
    // -> struct file_queue_record file_queue[4];
}

// Range: 0x800DA49C -> 0x800DA534
void iFileAsyncService() {
    // Local variables
    int i; // r28

    // References
    // -> struct file_queue_record file_queue[4];
}

static unsigned long fopcount; // size: 0x4, address: 0x80C07918
static signed char init; // size: 0x1, address: 0x80C0791C
// Range: 0x800DA534 -> 0x800DA648
int iFileReadAsync(struct tag_xFile * file /* r31 */, void * buf /* r4 */, unsigned int asize /* r0 */, void (* dcb)(struct tag_xFile *) /* r0 */) {
    // Local variables
    int i; // r28
    long read_size; // r9
    unsigned long key; // r0

    // References
    // -> static unsigned long fopcount;
    // -> struct file_queue_record file_queue[4];
    // -> static signed char init;
}

enum IFILE_READSECTOR_STATUS {
    IFILE_RDSTAT_NOOP = 0,
    IFILE_RDSTAT_INPROG = 1,
    IFILE_RDSTAT_DONE = 2,
    IFILE_RDSTAT_FAIL = 3,
    IFILE_RDSTAT_QUEUED = 4,
    IFILE_RDSTAT_EXPIRED = 5,
};
// Range: 0x800DA648 -> 0x800DA694
enum IFILE_READSECTOR_STATUS iFileReadAsyncStatus(int key /* r0 */, int * bytes_read /* r0 */) {
    // Local variables
    unsigned long i; // r0

    // References
    // -> struct file_queue_record file_queue[4];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFEFE4
// Range: 0x800DA694 -> 0x800DA79C
unsigned int iFileWrite(struct tag_xFile * file /* r29 */, void * buf /* r30 */, unsigned int size /* r31 */) {
    // Local variables
    struct tag_iFile * ps; // r0
    unsigned int n; // r31

    // References
    // -> static char __FUNCTION__[11];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFEFF0
// Range: 0x800DA79C -> 0x800DA854
unsigned int iFileClose(struct tag_xFile * file /* r31 */) {
    // Local variables
    struct tag_iFile * ps; // r0
    int ret; // r0

    // References
    // -> static char __FUNCTION__[11];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80BFEFFC
// Range: 0x800DA854 -> 0x800DA924
unsigned int iFileGetSize(struct tag_xFile * file /* r30 */) {
    // Local variables
    struct tag_iFile * ps; // r0
    int cur; // r31
    long end; // r30

    // References
    // -> static char __FUNCTION__[13];
}

// total size: 0x20
struct DVDDiskID {
    // Members
    char gameName[4]; // offset 0x0, size 0x4
    char company[2]; // offset 0x4, size 0x2
    unsigned char diskNumber; // offset 0x6, size 0x1
    unsigned char gameVersion; // offset 0x7, size 0x1
    unsigned char streaming; // offset 0x8, size 0x1
    unsigned char streamingBufSize; // offset 0x9, size 0x1
    unsigned char padding[22]; // offset 0xA, size 0x16
};
// total size: 0x30
struct DVDCommandBlock {
    // Members
    struct DVDCommandBlock * next; // offset 0x0, size 0x4
    struct DVDCommandBlock * prev; // offset 0x4, size 0x4
    unsigned long command; // offset 0x8, size 0x4
    long state; // offset 0xC, size 0x4
    unsigned long offset; // offset 0x10, size 0x4
    unsigned long length; // offset 0x14, size 0x4
    void * addr; // offset 0x18, size 0x4
    unsigned long currTransferSize; // offset 0x1C, size 0x4
    unsigned long transferredSize; // offset 0x20, size 0x4
    struct DVDDiskID * id; // offset 0x24, size 0x4
    void (* callback)(long, struct DVDCommandBlock *); // offset 0x28, size 0x4
    void * userData; // offset 0x2C, size 0x4
};
// total size: 0x3C
struct DVDFileInfo {
    // Members
    struct DVDCommandBlock cb; // offset 0x0, size 0x30
    unsigned long startAddr; // offset 0x30, size 0x4
    unsigned long length; // offset 0x34, size 0x4
    void (* callback)(long, struct DVDFileInfo *); // offset 0x38, size 0x4
};
static char __FUNCTION__[12]; // size: 0xC, address: 0x80BFF00C
// Range: 0x800DA924 -> 0x800DA9B8
unsigned char iFileExists(const char * name /* r31 */) {
    // Local variables
    char fullname[128]; // r1+0x8
    int fd; // r0

    // References
    // -> static char __FUNCTION__[12];
}

// Range: 0x800DA9B8 -> 0x800DA9DC
unsigned int iFileReadStop() {}

char gHostPath[256]; // size: 0x100, address: 0x80B6D590
static char __FUNCTION__[14]; // size: 0xE, address: 0x80BFF018
// Range: 0x800DA9DC -> 0x800DAB3C
void iFileFullPath(const char * relname /* r30 */, char * fullname /* r31 */) {
    // Local variables
    const char * inchar; // r5
    char * outchar; // r6

    // References
    // -> static char __FUNCTION__[14];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80BFF028
// Range: 0x800DAB3C -> 0x800DAC74
void iFileSetPath(const char * path /* r30 */) {
    // Local variables
    unsigned int len; // r0

    // References
    // -> char gHostPath[256];
    // -> static char __FUNCTION__[13];
}

// total size: 0x8
struct /* @class$1468iFile_cpp */ {
    // Members
    char * bad; // offset 0x0, size 0x4
    char * good; // offset 0x4, size 0x4
};
static // total size: 0x8
struct /* @class$1468iFile_cpp */ {
    // Members
    char * bad; // offset 0x0, size 0x4
    char * good; // offset 0x4, size 0x4
} fixes[2]; // size: 0x10, address: 0x80C0B628
// Range: 0x800DAC74 -> 0x800DAD18
void iFileFixAbsolutePath(char * path /* r29 */) {
    // Local variables
    char tmppath[256]; // r1+0x8
    int i; // r30

    // References
    // -> static // total size: 0x8
struct /* @class$1468iFile_cpp */ {
    // Members
    char * bad; // offset 0x0, size 0x4
    char * good; // offset 0x4, size 0x4
} fixes[2];
}

// Range: 0x800DAD18 -> 0x800DAD38
unsigned int iFileFind(const char * name /* r0 */, int fileflags /* r0 */, struct tag_xFile * file /* r0 */) {}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80BFF038
// Range: 0x800DAD38 -> 0x800DAE30
void iFileGetInfo(struct tag_xFile * file /* r29 */, unsigned int * starting_sector /* r30 */, unsigned int * size_in_bytes /* r31 */) {
    // Local variables
    struct tag_iFile * ps; // r0

    // References
    // -> static char __FUNCTION__[13];
}

enum xFileDriveStatus {
    XFILE_TRAY_OPEN = 0,
    XFILE_STOPPED = 1,
    XFILE_SPINNING = 2,
    XFILE_READING = 3,
    XFILE_PAUSED = 4,
    XFILE_SEEKING = 5,
    XFILE_ERROR = 6,
};
static char __FUNCTION__[19]; // size: 0x13, address: 0x80BFF048
enum xFileDriveType {
    XFILE_DRIVE_CDVD = 0,
    XFILE_DRIVE_CONSOLE_HD = 1,
};
// Range: 0x800DAE30 -> 0x800DAEC0
unsigned int iFileGetSectorSize(enum xFileDriveType drive /* r3 */) {
    // References
    // -> static char __FUNCTION__[19];
}


