/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zNPCCommon.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80246A60 -> 0x8024DF78
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static unsigned char DRAW_ANIM_PHYSICS; // size: 0x1, address: 0x80C09180
// total size: 0x34
struct RwRaster {
    // Members
...
struct RwTexture * whiteTex; // size: 0x4, address: 0x80C09184
static float FREEZING_MUL_R; // size: 0x4, address: 0x80C04BF0
static float FREEZING_MUL_G; // size: 0x4, address: 0x80C04BF4
static float FREEZING_MUL_B; // size: 0x4, address: 0x80C04BF8
float TEST_ALPHA; // size: 0x4, address: 0x80C04BFC
// total size: 0xC
struct xModelPool {
    // Members
...
// total size: 0x4
struct /* @class$1105zNPCCommon_cpp */ {
    // Members
...
union /* @class$1244zNPCCommon_cpp */ {
...
// Range: 0x80246AC0 -> 0x80246AF0
static unsigned char is_conveyor_belt(struct xEnt * ent /* r0 */) {}

static int sNPCHitPoints[3][2]; // size: 0x18, address: 0x80C04C00
enum ezHitReaction hitReactionTable[3][2][65]; // size: 0x618, address: 0x80440E58
// total size: 0x60
...
// total size: 0x4
struct /* @class$2251zNPCCommon_cpp */ {
    // Members
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80C04C18
enum ezHitReactionState {
    zHRS_Normal = 0,
    zHRS_Frozen = 1,
    zHRS_Total = 2,
};
// Range: 0x80246AF0 -> 0x80246D24
// this: r29
enum ezHitReaction zNPCCommon::GetDamageHitReaction(const struct zCombatDamageInfo & damageInfo /* r30 */, enum ezHitReactionState inState /* r31 */) {
    // Local variables
    enum ezHitReaction react; // r3
...
// total size: 0xC
struct /* @class$2465zNPCCommon_cpp */ {
    // Members
...
static struct zSparkEmitterAsset sNPCCommonDestroyShieldGeneratorSparks; // size: 0x94, address: 0x80BCB420
unsigned char draw_npc_bounds; // size: 0x1, address: 0x80C09188
unsigned char draw_npc_combat_bounds; // size: 0x1, address: 0x80C09189
unsigned char m_bDebugAnimation; // size: 0x1, address: 0x80C0918A
unsigned char update_npcs; // size: 0x1, address: 0x80C04C2D
unsigned char m_bShowName; // size: 0x1, address: 0x80C04C2E
unsigned char m_bShowAnimation; // size: 0x1, address: 0x80C04C2F
unsigned char m_bShowBehavior; // size: 0x1, address: 0x80C04C30
unsigned char m_bShowTarget; // size: 0x1, address: 0x80C0918B
unsigned char m_bShowHitTarget; // size: 0x1, address: 0x80C0918C
unsigned char m_bShowFrozen; // size: 0x1, address: 0x80C0918D
unsigned char m_bShowRecentEvents; // size: 0x1, address: 0x80C0918E
unsigned char m_bShowRecentAnimChanges; // size: 0x1, address: 0x80C0918F
unsigned char m_bShowHitPoints; // size: 0x1, address: 0x80C09190
unsigned char m_bShowPosition; // size: 0x1, address: 0x80C09191
unsigned char m_bShowShield; // size: 0x1, address: 0x80C09192
unsigned char m_bShowDetection; // size: 0x1, address: 0x80C09193
// Range: 0x80246D24 -> 0x80246D44
static void on_kill_all() {}

// Range: 0x80246D44 -> 0x80246D48
...
static struct tweak_callback cb_kill_all; // size: 0x28, address: 0x803FBDB8
static struct tweak_callback cb_reset_all; // size: 0x28, address: 0x803FBDE0
unsigned char draw_move_to; // size: 0x1, address: 0x80C08EE2
// Range: 0x80246D48 -> 0x80247088
...
int DM_NPC; // size: 0x4, address: 0x80C09194
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
char * zHitTargetNames[8]; // size: 0x20, address: 0x8043A3BC
...
// Range: 0x80247088 -> 0x80247790
// this: r27
void zNPCCommon::debug_display() {
    // Local variables
    struct xVec3 txtPos; // r1+0x20
    char messageTxt[2048]; // r1+0x2C
    struct zCombat * combat; // r0
    int hp; // r26
    int mhp; // r28
    struct xEnt * target; // r0
    long long lastTime; // r29
    int i; // r31
    struct zDebugStoredAnimChanges * ac; // r0
    float diffSec; // f0
    long long curTime; // r30
    int i; // r28
    struct zDebugStoredEvents * se; // r0
    float diffSec; // f31
    const char * fromName; // r3
    const char * widgetName; // r3
    struct xVec3 * pos; // r0
    struct zCombat * combat; // r0
...
// Range: 0x80247790 -> 0x802477D8
static void bupdate_wrapper(struct xEnt * ent /* r31 */) {}

// Range: 0x802477D8 -> 0x80247850
static void event_wrapper(struct xBase * from /* r26 */, struct xBase * to /* r27 */, unsigned int to_event /* r28 */, const float * to_param /* r29 */, struct xBase * to_param_widget /* r30 */, unsigned int to_param_widget_id /* r31 */) {}

static char __FUNCTION__[22]; // size: 0x16, address: 0x80C04C34
// Range: 0x80247850 -> 0x80247918
struct xModelInstance * zNPCCommonFindFXModel(unsigned int modelID /* r30 */) {
    // Local variables
    struct xModelInstance * minstance; // r31
    struct RpAtomic * imodel; // r0

    // References
    // -> static char __FUNCTION__[22];
}

// Range: 0x80247918 -> 0x802479E0
// this: r24
void zNPCCommon::StoreEvent(struct xBase * from /* r25 */, struct xBase * to /* r26 */, unsigned int to_event /* r27 */, float * to_param /* r28 */, struct xBase * to_param_widget /* r29 */) {
    // Local variables
    struct zDebugStoredEvents * se; // r0
    int i; // r30
}

// Range: 0x80247A1C -> 0x80247AB0
// this: r29
...
// total size: 0x1D4
struct {} zNPC::zNPCCommon::__vtable; // size: 0x1D4, address: 0x804416A8
// Range: 0x80247AE8 -> 0x80247B60
// this: r31
...
// total size: 0x78
struct {} zNPC::zNPCBaseIN2::__vtable; // size: 0x78, address: 0x8044187C
// Range: 0x80247B60 -> 0x80247BE4
// this: r30
zNPCCommon::~zNPCCommon() {
    // References
    // -> struct [anonymous] zNPC::zNPCCommon::__vtable;
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80C04C4C
// Range: 0x80247FD0 -> 0x802480C8
// this: r30
...
// Range: 0x802480C8 -> 0x802480E8
// this: r3
struct xEnt * zNPCCommon::GetAttackingPlayerEnt() {}

// Range: 0x802480E8 -> 0x802480F4
// this: r0
void zNPCCommon::SetAttackingPlayerEnt(struct xEnt * inAttackingPlayer /* r0 */, float inTime /* f0 */) {}
...
void zNPCCommon::check_transient_damage() {
    ...
    int hits_found; // r0
    struct damage_result * hit; // r29
    struct damage_result * end_hit; // r28
    const struct xVec3 & hitcenter; // r27
    const struct xVec3 & npccenter; // r0
...
// Range: 0x80248284 -> 0x80248678
// this: r30
void zNPCCommon::timestep(struct xScene * xscn /* r31 */, float dt /* f29 */) {
    // Local variables
    unsigned int boneIndex; // r3
    struct xMat4x3 boneMat; // r1+0x20
    struct zCombat * combat; // r0
    float originalSpeed; // f31
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80C04C60
...
static unsigned char done; // size: 0x1, address: 0x80C09198
static signed char init; // size: 0x1, address: 0x80C09199
static char __FUNCTION__[17]; // size: 0x11, address: 0x80C04C74
// total size: 0x38
struct xGrid {
    // Members
...
// total size: 0xC
struct /* @class$1243zNPCCommon_cpp */ {
    // Members
...
void zNPCCommon::CheckForDrivable() {
    ...
    int i; // r31
    struct xEnt * floorEntity; // r28
    struct xCollis * collisionRec; // r27
    enum _xCollsIdx collsIndex; // r0
    struct xEnt * driver; // r31
...
// Range: 0x80248AA8 -> 0x80248AC0
// this: r0
StaticObjectsCallBack::StaticObjectsCallBack(const struct xSphere & sphere /* r0 */, struct xCollis * coll /* r0 */, unsigned char & index /* r0 */, unsigned char & startIndex /* r0 */, unsigned char & endIndex /* r0 */) {}
...
// Range: 0x80248AC0 -> 0x80248B64
// this: r31
void zNPCCommon::CheckForSlowCollisionVolume() {
    // Local variables
    struct xVec3 npcPos; // r1+0x8
    struct zEntTrigger * trigger; // r0
}

// Range: 0x80248B64 -> 0x80248B7C
// this: r0
unsigned char zNPCCommon::ShieldHasEnergyToDeploy() {}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80C04C88
// total size: 0x1C
...
// Range: 0x802491E0 -> 0x802491E4
void zNPCCommon::NPCFXUpdate() {}

// Range: 0x802491E4 -> 0x80249214
// this: r3
void zNPCCommon::add_states(struct xAnimTable * table /* r0 */) {}

// Range: 0x80249214 -> 0x80249244
// this: r3
void zNPCCommon::add_transitions(struct xAnimTable * table /* r0 */) {}

// Range: 0x80249244 -> 0x80249264
// this: r0
struct zCombat * zNPCCommon::get_combat() {}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80C04C94
// Range: 0x80249264 -> 0x80249494
// this: r28
void zNPCCommon::deliver_damage(const struct zCombatDamageInfo & damageInfo /* r29 */) {
    // Local variables
    unsigned char npcHit; // r30
    struct zCombat * combat; // r0
    int quarterHealth; // r26

    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80C04CA4
// Range: 0x80249494 -> 0x802495D0
// this: r30
unsigned char zNPCCommon::valid_damage(const struct zCombatDamageInfo & damageInfo /* r31 */) {
    // References
    // -> static char __FUNCTION__[13];
}

// Range: 0x802495D0 -> 0x802496FC
// this: r30
...
// total size: 0x890
struct /* @class$2033zNPCCommon_cpp */ {
    // Static members
...
// total size: 0xC
struct /* @class$1046zNPCCommon_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1047zNPCCommon_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1048zNPCCommon_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$1049zNPCCommon_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1050zNPCCommon_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1051zNPCCommon_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1052zNPCCommon_cpp */ {
    // Members
...
struct xSerial {};
// total size: 0x4
struct /* @class$2210zNPCCommon_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2211zNPCCommon_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// total size: 0x4
struct /* @class$2212zNPCCommon_cpp */ {
    // Members
...
// total size: 0x8
struct /* @class$2209zNPCCommon_cpp */ {
    // Members
    float time; // offset 0x0, size 0x4
...
class zNPCBase {};
// total size: 0x2
struct /* @class$2214zNPCCommon_cpp */ {
    // Members
...
// total size: 0x10
struct /* @class$1579zNPCCommon_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x802496FC -> 0x80249700
void zNPCCommon::PlayDamageSound() {}

// Range: 0x80249700 -> 0x8024979C
// this: r30
void zNPCCommon::damage(const struct zCombatDamageInfo & damageInfo /* r31 */) {
    // Local variables
    struct zCombatDamageInfo newInfo; // r1+0x8
}

// Range: 0x80249810 -> 0x80249828
// this: r0
void zNPCCommon::AboutToBeHitBy(struct xEnt * thrown /* r0 */) {}

// Range: 0x80249828 -> 0x80249858
// this: r3
void zNPCCommon::scene_setup() {}

// Range: 0x80249858 -> 0x802498A4
void zNPCCommon::BeforeAnimMatrices(struct xAnimPlay * play /* r9 */, struct xQuat * quat /* r8 */, struct xVec3 * tran /* r0 */, int boneCount /* r7 */) {}

// Range: 0x802498A4 -> 0x802498D4
// this: r3
void zNPCCommon::before_anim_matrices(struct xAnimPlay * play /* r0 */, struct xQuat * quat /* r0 */, struct xVec3 * tran /* r0 */, int boneCount /* r0 */) {}

static unsigned char __bSetStatics; // size: 0x1, address: 0x80C0919A
static signed char init; // size: 0x1, address: 0x80C0919B
static char __FUNCTION__[5]; // size: 0x5, address: 0x80C04CB4
...
// Range: 0x802498D4 -> 0x80249E68
// this: r30
void zNPCCommon::init(class zNPCInstance * init_inst /* r0 */) {
    // Local variables
    const struct zNPCAssetIN2 * npcAsset; // r0
    unsigned int bufsize; // r1+0x10
    struct xNPCSettingsIN2Asset * settingsAsset; // r0
    unsigned char tempBool; // r1+0x8
    struct xSphere s; // r1+0x14
    const char * group_name; // r1+0xC
...
float zNPCFreezeTimeTable[3][2]; // size: 0x18, address: 0x80C082C4
static unsigned char already_asserted; // size: 0x1, address: 0x80C0919C
static signed char init; // size: 0x1, address: 0x80C0919D
static char __FUNCTION__[19]; // size: 0x13, address: 0x80C04CBC
// Range: 0x80249E68 -> 0x80249FB0
// this: r30
void zNPCCommon::SetDefaultLightKit() {
    // Local variables
    const struct zNPCAssetIN2 * npcAsset; // r0
    struct xLightKit * lightKit; // r31
...
// Range: 0x80249FB0 -> 0x80249FDC
// this: r0
void zNPCCommon::revive() {}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80C04CD0
struct xGrid npcs_grid; // size: 0x38, address: 0x80B931F8
// Range: 0x80249FDC -> 0x8024A16C
// this: r31
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80C04CDC
// Range: 0x8024A16C -> 0x8024A30C
// this: r31
...
// Range: 0x8024A348 -> 0x8024A3C0
// this: r31
void zNPCCommon::deactivate_visible() {}

// Range: 0x8024A3C0 -> 0x8024A3DC
// this: r0
void zNPCCommon::imminent_death() {}

// Range: 0x8024A3DC -> 0x8024A4C0
// this: r31
void zNPCCommon::kill() {}

// Range: 0x8024A4C0 -> 0x8024A4F0
// this: r3
...
// Range: 0x8024A4F0 -> 0x8024A558
// this: r30
unsigned char zNPCCommon::grabbable(enum GrabType grabType /* r31 */) {}

// Range: 0x8024A558 -> 0x8024A5B0
// this: r31
unsigned char zNPCCommon::punchable() {}

// Range: 0x8024A5B0 -> 0x8024A670
// this: r30
void zNPCCommon::GetNukeIconMatrix(struct xMat4x3 * posMat /* r31 */) {
    // Local variables
    int boneIdx; // r0
    struct xMat4x3 boneMat; // r1+0x20
...
// Range: 0x8024A670 -> 0x8024A7A0
static void render_npc_nuke_ring_prepare(struct xModelInstance * ring /* r29 */, int ringNum /* r0 */, float ringEmitStep /* f0 */, float ratio /* f30 */, float sizeMultiplier /* f0 */, struct xMat3x3 * orientMat /* r30 */, struct xMat3x3 * rotMat /* r31 */) {
    // Local variables
    float nukeShockScale; // f1
    float step; // f31
    struct xVec3 scaleVec; // r1+0x10
    struct xMat3x3 scaleMat; // r1+0x50
    struct xMat3x3 finalMat; // r1+0x20
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80C04CE8
float gSceneUpdateTime; // size: 0x4, address: 0x80C08430
struct xVec3 g_X3; // size: 0xC, address: 0x80C0ADAC
// Range: 0x8024A7A0 -> 0x8024AC00
// this: r31
void zNPCCommon::render_npc_nuke() {
    // Local variables
    unsigned char failed; // r28
    int i; // r27
    struct xModelInstance * mi; // r0
    int i; // r27
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80C04CF8
// Range: 0x8024AC00 -> 0x8024AE14
// this: r31
...
// Range: 0x8024B038 -> 0x8024B088
// this: r31
void zNPCCommon::render_extra() {}

// Range: 0x8024B088 -> 0x8024B0E4
// this: r3
void zNPCCommon::post_render_cleanup() {}

// Range: 0x8024B0E4 -> 0x8024B114
// this: r3
void zNPCCommon::debug_render() {}

// Range: 0x8024B114 -> 0x8024B13C
// this: r3
void zNPCCommon::SetDefaultAndEventStates() {}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80C04D08
...
// Range: 0x8024B13C -> 0x8024B6B4
// this: r25
unsigned char zNPCCommon::systemEvent(struct xBase * from /* r26 */, struct xBase * to /* r27 */, unsigned int to_event /* r28 */, const float * to_param /* r29 */, struct xBase * to_param_widget /* r30 */, unsigned int to_param_widget_id /* r31 */) {
    // Local variables
    enum iSndGroupHandle soundGroupHandle; // r4
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80C04D14
// Range: 0x8024B6B4 -> 0x8024B768
void zNPCCommon::GetPosFromBase(struct xBase * inBase /* r30 */, struct xVec3 * outPos /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

// Range: 0x8024B768 -> 0x8024B7B4
// this: r31
void zNPCCommon::ForceEnterDeathState() {}

// Range: 0x8024B7B4 -> 0x8024B7E4
static void on_reset(const struct tweak_info & info /* r3 */) {}

// Range: 0x8024B7E4 -> 0x8024B814
static void on_kill(const struct tweak_info & info /* r3 */) {}

// Range: 0x8024B814 -> 0x8024B874
static void on_damage(const struct tweak_info & info /* r0 */) {
    // Local variables
    class zNPCCommon * npc; // r31
    struct zCombatDamageInfo damage; // r1+0x8
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80C04D24
// Range: 0x8024B874 -> 0x8024B9D0
static void on_atomic_toggle(const struct tweak_info & info /* r0 */) {
    // Local variables
    class zNPCCommon * npc; // r31
    const char * current; // r30
    int index; // r0
    struct xModelInstance * atomic; // r4
    int i; // r6
...
static struct tweak_callback cb_reset; // size: 0x28, address: 0x803FBE08
static struct tweak_callback cb_kill; // size: 0x28, address: 0x803FBE30
static struct tweak_callback cb_damage; // size: 0x28, address: 0x803FBE58
static struct tweak_callback cb_atomic_toggle; // size: 0x28, address: 0x803FBE80
// Range: 0x8024B9D0 -> 0x8024BC18
// this: r30
void zNPCCommon::add_tweaks(const char * prefix /* r31 */) {
    // Local variables
    unsigned int index; // r27
    struct xModelInstance * curr_model; // r26
...
// Range: 0x8024BF10 -> 0x8024C0BC
// this: r30
void zNPCCommon::render_npc() {
    // Local variables
    struct xVec3 worldPos; // r1+0x8
    struct zCombat * combat; // r0
    enum _SDRenderState oldRenderState; // r31
}

// Range: 0x8024C0BC -> 0x8024C1D4
// this: r29
void zNPCCommon::render_npc_freezing() {
    // Local variables
    struct xModelInstance * m; // r31
    struct RwTexture * texture; // r30
    float original_r; // f31
    float original_g; // f30
    float original_b; // f29
    float factor; // f1
...
static unsigned char already_asserted; // size: 0x1, address: 0x80C0919E
static signed char init; // size: 0x1, address: 0x80C0919F
static char __FUNCTION__[11]; // size: 0xB, address: 0x80C04D38
enum xSndHandle {
};
// Range: 0x8024C1D4 -> 0x8024C308
// this: r30
enum xSndHandle zNPCCommon::play_sound(struct zAnimFxSoundGroup * sound /* r0 */) {
    // Local variables
    unsigned int soundID; // r31
    enum iSndGroupHandle GroupToPlay; // r29
...
// Range: 0x8024C308 -> 0x8024C490
// this: r31
void zNPCCommon::initialize_bounds(float modelScale /* f31 */) {
    // Local variables
    unsigned char use_bounding_box; // r1+0x8
...
// Range: 0x8024C490 -> 0x8024C6EC
// this: r31
void zNPCCommon::change_bounds(float scale /* f31 */, struct xVec3 & offset /* r0 */, unsigned char use_bounding_box /* r30 */) {
    // Local variables
    float width; // f5
    float height; // f6
    float length; // f5
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80C04D44
// Range: 0x8024C6EC -> 0x8024C8BC
// this: r31
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x80C04D54
// Range: 0x8024C8BC -> 0x8024C9B4
// this: r29
struct xVec3 zNPCCommon::get_bone_world_position(int bone /* r30 */) {
    // Local variables
...
// Range: 0x8024C9B4 -> 0x8024CA18
// this: r0
unsigned char zNPCCommon::GetParameter(const char * name /* r0 */, unsigned int * value /* r30 */, unsigned int defaultValue /* r31 */) {
    // Local variables
    const char * text; // r1+0x8
}

// Range: 0x8024CA18 -> 0x8024CA7C
// this: r0
unsigned char zNPCCommon::GetParameter(const char * name /* r0 */, int * value /* r30 */, int defaultValue /* r31 */) {
    // Local variables
    const char * text; // r1+0x8
}

// Range: 0x8024CA7C -> 0x8024CAE0
// this: r0
unsigned char zNPCCommon::GetParameter(const char * name /* r0 */, signed short * value /* r30 */, signed short defaultValue /* r31 */) {
    // Local variables
    const char * text; // r1+0x8
}

// Range: 0x8024CAE0 -> 0x8024CB70
// this: r0
unsigned char zNPCCommon::GetParameter(const char * name /* r0 */, unsigned char * value /* r30 */, unsigned char defaultValue /* r31 */) {
    // Local variables
    const char * text; // r1+0x8
}

// Range: 0x8024CB70 -> 0x8024CBD4
// this: r0
unsigned char zNPCCommon::GetParameter(const char * name /* r0 */, float * value /* r31 */, float defaultValue /* f31 */) {
    // Local variables
    const char * text; // r1+0x8
}

// Range: 0x8024CBD4 -> 0x8024CC60
// this: r0
unsigned char zNPCCommon::GetParameter(const char * name /* r0 */, struct xVec3 * value /* r30 */, const struct xVec3 & defaultValue /* r31 */) {
    // Local variables
    const char * text; // r1+0x8
    int size; // r0
}

// Range: 0x8024CC60 -> 0x8024CD0C
// this: r28
unsigned char zNPCCommon::GetParameter(const char * name /* r29 */, const char * * value /* r30 */, const char * defaultValue /* r31 */) {
    // Local variables
    const char * text; // r0
}

// Range: 0x8024CD0C -> 0x8024CD74
// this: r30
...
// Range: 0x8024CD74 -> 0x8024CD7C
unsigned int zNPCCommon::GetNPCIceType() {}

// Range: 0x8024CD7C -> 0x8024CD84
float zNPCCommon::WeaponGetMaxRange() {}

// Range: 0x8024CD84 -> 0x8024CD8C
unsigned char zNPCCommon::HasRangeWeapon() {}

// Range: 0x8024CD8C -> 0x8024CDC0
void init_system() {
    // References
    // -> int DM_NPC;
}

// Range: 0x8024CDC0 -> 0x8024CE18
// this: r31
...
static char __FUNCTION__[7]; // size: 0x7, address: 0x80C04D6C
// Range: 0x8024CE18 -> 0x8024CFD8
// this: r30
unsigned char zNPCCommon::Freeze(float intensity /* f31 */) {
    // Local variables
    class behavior * cur; // r31

    // References
    // -> static char __FUNCTION__[7];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80C04D74
// Range: 0x8024CFD8 -> 0x8024D138
// this: r30
void zNPCCommon::UpdateFrozen(float dt /* f31 */) {
    // Local variables
    class behavior * currBehavior; // r0

    // References
    // -> static char __FUNCTION__[13];
}

// Range: 0x8024D138 -> 0x8024D200
// this: r30
void zNPCCommon::PastUpdateDistCleanup() {}

// Range: 0x8024D200 -> 0x8024D2FC
// this: r30
void zNPCCommon::VisibilityCullCleanup() {}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80C04D84
// Range: 0x8024D2FC -> 0x8024D3A8
// this: r31
void zNPCCommon::SetRandomAnimSpeed(float inMin /* f30 */, float inMax /* f31 */) {
    // Local variables
    float newSpeed; // f0
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80C04D98
// Range: 0x8024D3A8 -> 0x8024D49C
// this: r31
void zNPCCommon::SetRandomAnimTime(float inMin /* f30 */, float inMax /* f31 */) {
    // Local variables
    float newTime; // f1
...
// Range: 0x8024D49C -> 0x8024D4EC
// this: r31
void zNPCCommon::SetRandomAnimSpeedAndTime(float inMin /* f0 */, float inMax /* f0 */) {}

// Range: 0x8024D4EC -> 0x8024D528
// this: r0
void zNPCCommon::SetRandomAnimTime() {}

// Range: 0x8024D528 -> 0x8024D6E8
// this: r31
...
// Range: 0x8024D9B0 -> 0x8024DD48
// this: r29
float zNPCCommon::CalcTranslucency() {
    // Local variables
    const struct xMat4x3 * camMat; // r0
    struct xVec3 delta; // r1+0x50
    float distanceFromPlane; // f0
    struct xVec3 screenCenter; // r1+0x44
    struct xVec3 pointOnSphere; // r1+0x38
    struct xVec3 screenPointOnCircle; // r1+0x2C
    float circleRadius; // f0
    float screenArea; // f27
    float screenMaxX; // f4
    float screenMaxY; // f0
    float squareMinX; // f5
    float squareMaxX; // f6
    ...
    float nearP; // r1+0xC
    float farP; // r1+0x8
    struct xVec3 point; // r1+0x20
    class xPlane nearPlane; // r1+0x10
    float r; // f27
    float distance; // f0
...
// total size: 0x11C
struct {} zNPC::npc_nuke::__vtable; // size: 0x11C, address: 0x80441470
// total size: 0x34
...
// total size: 0x11C
struct {} zBehaviorImplementation<zNPC::zNPCCommon>::__vtable; // size: 0x11C, address: 0x8044158C
float zNPCThawTimeTable[3][2]; // size: 0x18, address: 0x80C082DC
// total size: 0x0
...
// Range: 0x8024DD48 -> 0x8024DF78
// this: r30
unsigned char StaticObjectsCallBack::operator()(struct xEnt & ent /* r31 */) {
    // References
    // -> struct zGlobals globals;
}
