/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zPlatform.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80146064 -> 0x8014B024
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct zParEmitter * sEmitTremble; // size: 0x4, address: 0x80C08328
static struct zParEmitter * sEmitBreakaway; // size: 0x4, address: 0x80C0832C
// total size: 0x10
...
static struct effect * sRumbleBreakaway; // size: 0x4, address: 0x80C08330
static unsigned int entToCheck; // size: 0x4, address: 0x80C08334
static signed char init; // size: 0x1, address: 0x80C08338
...
// total size: 0x4
struct /* @class$1085zPlatform_cpp */ {
    // Members
...
int gDrawInvisibleWalls; // size: 0x4, address: 0x80C08228
// Range: 0x80146108 -> 0x8014610C
void zPlatform_ScenePrepare() {}

// Range: 0x8014610C -> 0x8014612C
void zPlatform_Init(void * plat /* r0 */, void * asset /* r0 */) {}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80C00F48
...
// total size: 0x10
struct /* @class$2050zPlatform_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x8014612C -> 0x801467C0
void zPlatform_Init(struct zPlatform * plat /* r31 */, struct xEntAsset * asset /* r27 */) {
    // Local variables
    struct xPlatformAsset * plat_asset; // r29
    struct xEntMotionAsset * ent_motion_asset; // r28
    struct xAnimFile * spring_file; // r27
    struct xAnimFile * idle_file; // r4
    unsigned int size; // r1+0x18
    unsigned int newid; // r27
    struct xDestructibleAsset * destruct; // r0
...
// Range: 0x801467C0 -> 0x801468A0
void zPlatform_Setup(struct zPlatform * plat /* r30 */, struct xScene * sc /* r31 */) {
    // References
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80C00F58
// Range: 0x801468A0 -> 0x80146970
void zPlatform_Save(struct zPlatform * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80C00F68
// Range: 0x80146970 -> 0x80146A40
void zPlatform_Load(struct zPlatform * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

// Range: 0x80146A40 -> 0x80146A74
void zPlatform_ReReset(struct zPlatform * plat /* r3 */) {}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80C00F78
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x80146A74 -> 0x80147048
void zPlatform_Reset(struct zPlatform * plat /* r27 */, struct xScene * sc /* r28 */) {
    // Local variables
    struct RpAtomic * imodel; // r26
    struct xSurface * surf; // r31
    struct xAnimPlay * tempAnim; // r25
    struct xAnimTable * temptbl; // r24
    unsigned short tempFlags; // r30
    void * tempObject; // r29
    int i; // r0
    struct xMat3x3 normMat; // r1+0x10
...
// Range: 0x80147048 -> 0x80147448
static float SolvePaddleMotion(struct zPlatform * plat /* r0 */, float * time /* r30 */, float tmr /* f26 */) {
    // Local variables
    struct xPlatformPaddleData * paddle; // r31
    float destOrient; // f1
    float srcOrient; // f30
    float absDelta; // f29
    float stutterAmount; // f0
    float A; // f28
    float D; // f27
    float distA; // f3
    float distC; // f1
...
static char __FUNCTION__[28]; // size: 0x1C, address: 0x8043A820
// Range: 0x80147448 -> 0x801475F4
void zPlatform_PaddleStartRotate(struct xEnt * entplat /* r31 */, int direction /* r30 */, int stutter /* r29 */) {
    // Local variables
...
// total size: 0xC
struct /* @class$1249zPlatform_cpp */ {
    // Members
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80C00F88
// total size: 0x8
...
// Range: 0x801475F4 -> 0x801488F4
static void zPlatFM_Update(struct zPlatform * plat /* r31 */, float dt /* f23 */) {
    // Local variables
    struct zPlatFMEventsTable eventsTable[17]; // r1+0x1A0
    struct zPlatFMRunTime * fmrt; // r27
    struct xMat4x3 * pmat; // r26
    struct xMat3x3 platformRot; // r1+0x170
    struct xVec3 platformScale; // r1+0x38
    struct xVec3 platformLoc; // r1+0x2C
    int i; // r28
    float tm; // f31
    float ttm; // f28
    float atm; // f27
    ...
    float p; // f3
    float p; // f4
    struct xMat4x3 * pmat; // r24
    struct xVec3 undoScale; // r1+0x20
    struct xMat3x3 preR; // r1+0x140
    struct xMat3x3 preR; // r1+0x110
    struct xMat3x3 preR; // r1+0xE0
    struct xMat3x3 postR; // r1+0xB0
    struct xMat3x3 postR; // r1+0x80
    struct xMat3x3 postR; // r1+0x50
    struct xQuat curQuat; // r1+0x10
    unsigned char finishedEvent; // r25
    float tm; // f25
    float rts; // f0
    float ups; // f0
    float ats; // f0
...
static char __FUNCTION__[26]; // size: 0x1A, address: 0x8043A880
struct xGlobals * xglobals; // size: 0x4, address: 0x80C00BE8
// Range: 0x801488F4 -> 0x80148F98
void zPlatformBreakaway_Update(struct zPlatform * plat /* r31 */, struct xScene * sc /* r29 */, float dt /* f31 */) {
    // Local variables
...
// Range: 0x80148F98 -> 0x801498A4
void zPlatform_Update(struct xEnt * entplat /* r3 */, struct xScene * sc /* r31 */, float dt /* f31 */) {
    // Local variables
    struct zPlatform * plat; // r30
    unsigned short state; // r31
    int i; // r31
    struct xEntMotionMechData * mkasst; // r0
    float duration; // f0
    struct xQuat * q0; // r29
    struct xVec3 * t0; // r31
    ...
    float itilt; // f2
    float dtilt; // f3
    struct xVec3 * scale; // r0
    struct xVec3 lpos; // r1+0x1C
    float cangle; // f4
    float dangle; // f2
    float mangle; // f5
    float tangle; // f1
    struct xVec3 * scale; // r0
    float time[3]; // r1+0x10
    float orient; // f4
    struct xMat3x3 orientrot; // r1+0x60
    struct xMat3x3 origrot; // r1+0x30
    struct xVec3 * scale; // r0
    struct xMovePoint * src; // r28
    float vecLen; // f0
    int moving; // r28
    struct xVec3 * opos; // r0
    struct xVec3 * pos; // r0
    struct xVec3 * orot; // r0
    struct xVec3 * rot; // r0
...
// total size: 0x890
struct /* @class$2297zPlatform_cpp */ {
    // Static members
...
// total size: 0x54
struct /* @class$2370zPlatform_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1026zPlatform_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1027zPlatform_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1028zPlatform_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$1029zPlatform_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1030zPlatform_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1031zPlatform_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1032zPlatform_cpp */ {
    // Members
...
static int pleaseStopMoving; // size: 0x4, address: 0x80C0833C
static signed char init; // size: 0x1, address: 0x80C08340
static unsigned int aid_haltOnMe; // size: 0x4, address: 0x80C08344
static signed char init; // size: 0x1, address: 0x80C08348
static int beWhereISay; // size: 0x4, address: 0x80C0834C
static signed char init; // size: 0x1, address: 0x80C08350
static struct xVec3 pos_hackstop; // size: 0xC, address: 0x80C00F98
// Range: 0x801498A4 -> 0x8014999C
void zPlatform_Move(struct xEnt * entplat /* r31 */, struct xScene * s /* r30 */, float dt /* f31 */, struct xEntFrame * frame /* r5 */) {
    // Local variables
    struct zPlatform * plat; // r0
...
// Range: 0x8014999C -> 0x80149A64
void zPlatform_Shake(struct zPlatform * plat /* r30 */, float ampl /* f30 */, float freq /* f31 */) {
    // Local variables
...
// Range: 0x80149A64 -> 0x80149B94
static void zPlatform_Tremble(struct zPlatform * plat /* r29 */, float ampl /* f29 */, float freq /* f30 */, float dur /* f31 */) {
    // Local variables
    struct xFFXShakeState * ss; // r31
    struct xFFX * sfkt; // r30
    struct xParEmitterCustomSettings info; // r1+0x8
    int i; // r30
...
// Range: 0x80149B94 -> 0x80149C0C
static void zPlatform_BreakawayFallFX(struct zPlatform * plat /* r0 */) {
    // Local variables
    struct xParEmitterCustomSettings info; // r1+0x8
    int i; // r31
...
// Range: 0x80149C0C -> 0x80149CB8
void zPlatform_Mount(struct zPlatform * plat /* r31 */) {
    // References
    // -> struct xGlobals * xglobals;
}

// Range: 0x80149CB8 -> 0x80149D68
void zPlatform_Dismount(struct zPlatform * plat /* r0 */) {
    // Local variables
    int i; // r1+0x8
...
// Range: 0x80149D68 -> 0x80149DBC
static void zPlatformTranslate(struct xEnt * xent /* r29 */, struct xVec3 * dpos /* r30 */, struct xMat4x3 * dmat /* r31 */) {
    // Local variables
    struct zPlatform * plat; // r0
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80C00FA4
// Range: 0x80149DBC -> 0x80149FAC
static void zPlatFM_EventSetup(struct zPlatform * plat /* r27 */, const float * toParam /* r0 */, int idx /* r28 */, unsigned int mask /* r29 */) {
    // Local variables
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80C00FB8
union FloatAndVoid {
    float f; // offset 0x0, size 0x4
    void * v; // offset 0x0, size 0x4
};
// Range: 0x80149FAC -> 0x8014B024
void zPlatformEventCB(struct xBase * from /* r26 */, struct xBase * to /* r31 */, unsigned int toEvent /* r27 */, const float * toParam /* r30 */, struct xBase * toParamWidget /* r28 */, unsigned int toParamWidgetID /* r29 */) {
    // Local variables
    struct zPlatform * plat; // r31
    float tp[4]; // r1+0x20
    int iParam0; // r4
    ...
    float desired_angle; // f0
    float current_angle; // f0
    float delta_angle; // f1
    float desired_angle; // f0
    float current_angle; // f0
    float delta_angle; // f1
    float desired_angle; // f0
    ...
    float mt; // f1
    float mt; // f1
    union FloatAndVoid dist; // r1+0x10
    struct zShrapnelAsset * shrap; // r0
    struct xVec3 currVel; // r1+0x14
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x80C00FCC
