/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xstransvc.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800CAA5C -> 0x800CC780
*/
static int g_straninit; // size: 0x4, address: 0x80C07840
// total size: 0x28
struct PKRAssetType {
    // Members
...
static struct st_STRAN_DATA g_xstdata; // size: 0x2284, address: 0x8042E408
enum en_LAYER_TYPE {
    PKR_LTYPE_ALL = -1,
    PKR_LTYPE_DEFAULT = 0,
    PKR_LTYPE_TEXTURE = 1,
...
static struct PKRReadFuncs * g_pkrf; // size: 0x4, address: 0x80C07844
static struct PKRAssetType * g_typeHandlers; // size: 0x4, address: 0x80C07848
static char g_localCode[4]; // size: 0x4, address: 0x80BFE9D8
enum xRegion {
    eRegionUnknown = -1,
    eRegionUS = 0,
    eRegionBE = 1,
...
static enum xRegion g_localEnum; // size: 0x4, address: 0x80BFE9DC
static unsigned int g_localDiscLanguageConfig; // size: 0x4, address: 0x80C0784C
// Range: 0x800CAA5C -> 0x800CAA70
unsigned char xSTIsLoaded() {
    // References
    // -> static int g_straninit;
}

// Range: 0x800CAA70 -> 0x800CAAFC
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFE9E0
// Range: 0x800CAB54 -> 0x800CAFEC
int xSTPreLoadScene(unsigned int sid /* r24 */, void * userdata /* r25 */, int flg_hiphop /* r26 */, int fileflags /* r27 */, const char * filename /* r7 */) {
    // Local variables
    int result; // r3
    struct st_STRAN_SCENE * sdata; // r30
    const char * sfile; // r24
    int cltver; // r28
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFE9F0
// Range: 0x800CAFEC -> 0x800CB0C0
int xSTQueueSceneAssets(unsigned int sid /* r0 */, int flg_hiphop /* r0 */) {
    // Local variables
    int result; // r30
    struct st_STRAN_SCENE * sdata; // r0
...
// Range: 0x800CB0C0 -> 0x800CB1D0
void xSTUnLoadSceneMany(unsigned int sceneID /* r29 */, int type /* r30 */) {
    // Local variables
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80BFEA04
// Range: 0x800CB1D0 -> 0x800CB268
float xSTLoadStep() {
    // Local variables
    float pct; // f31
    int rc; // r0
...
// Range: 0x800CB268 -> 0x800CB2A4
void xSTDisconnect(unsigned int sid /* r0 */, int flg_hiphop /* r0 */) {
    // Local variables
    struct st_STRAN_SCENE * sdata; // r0
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFEA10
// Range: 0x800CB2A4 -> 0x800CB3F8
int xSTSwitchScene(unsigned int sid /* r29 */, void * userdata /* r30 */, int (* progmon)(void *, float) /* r31 */) {
    // Local variables
    struct st_STRAN_SCENE * sdata; // r0
    int rc; // r26
    int i; // r25
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80BFEA20
// Range: 0x800CB3F8 -> 0x800CB4C4
char * xSTAssetName(unsigned int aid /* r27 */) {
    // Local variables
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80BFEA30
// Range: 0x800CB4C4 -> 0x800CB5A8
char * xSTAssetName(void * raw_HIP_asset /* r28 */) {
    // Local variables
    const char * aname; // r31
    int scncnt; // r0
    int i; // r29
    struct st_STRAN_SCENE * sdata; // r0
    unsigned int aid; // r4
...
// Range: 0x800CB5A8 -> 0x800CB5F4
char * xSTAssetNameFromPointer(void * model /* r31 */) {
    // Local variables
    struct PKRAssetTOCInfo ainfo; // r1+0x8
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFEA40
...
// Range: 0x800CB5F4 -> 0x800CB6B4
enum en_pkrfindmyass xSTRandMemToAsset(struct PKRAssetTOCInfo * ainfo /* r27 */, void * addr /* r28 */) {
    // Local variables
    int scncnt; // r0
    int i; // r29
    struct st_STRAN_SCENE * sdata; // r0
    enum en_pkrfindmyass rc_cur; // r0

    // References
    // -> static char __FUNCTION__[18];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80BFEA54
// Range: 0x800CB6B4 -> 0x800CB80C
void * xSTFindAsset(unsigned int aid /* r30 */, unsigned int * size /* r31 */) {
    // Local variables
    void * memloc; // r29
    int scncnt; // r0
    int i; // r27
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFEA64
// Range: 0x800CB80C -> 0x800CB950
struct st_PACKER_ATOC_NODE * xSTFindAssetNode(unsigned int aid /* r27 */, unsigned int * size /* r28 */) {
    // Local variables
    struct st_PACKER_ATOC_NODE * memloc; // r31
    int scncnt; // r0
    int i; // r29
    struct st_STRAN_SCENE * sdata; // r0
    int asssize; // r1+0x8
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFEA78
// Range: 0x800CB950 -> 0x800CBA10
int xSTAssetCountByType(unsigned int type /* r27 */) {
    // Local variables
    int sum; // r29
    int cnt; // r0
    struct st_STRAN_SCENE * sdata; // r0
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80BFEA8C
// Range: 0x800CBA10 -> 0x800CBB6C
void * xSTFindAssetByType(unsigned int type /* r24 */, int idx /* r25 */, unsigned int * size /* r26 */) {
    // Local variables
    struct st_STRAN_SCENE * sdata; // r0
    void * memptr; // r29
    int scncnt; // r0
...
// Range: 0x800CBB6C -> 0x800CBBF0
int xSTGetAssetInfo(unsigned int aid /* r27 */, struct PKRAssetTOCInfo * tocainfo /* r28 */) {
    // Local variables
    int found; // r30
    int scncnt; // r0
    int i; // r29
...
// Range: 0x800CBBF0 -> 0x800CBCB8
int xSTGetAssetInfoByType(unsigned int type /* r23 */, int idx /* r24 */, struct PKRAssetTOCInfo * tocainfo /* r25 */) {
    // Local variables
    int found; // r28
    int sum; // r27
    int cnt; // r0
...
// Range: 0x800CBCB8 -> 0x800CBCDC
char * xST_xAssetID_HIPFullPath(unsigned int aid /* r0 */) {}

// Range: 0x800CBCDC -> 0x800CBD70
char * xST_xAssetID_HIPFullPath(unsigned int aid /* r26 */, unsigned int * sceneID /* r27 */) {
    // Local variables
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFEAA0
// Range: 0x800CBD70 -> 0x800CBE98
static int XST_PreLoadScene(struct st_STRAN_SCENE * sdata /* r29 */, const char * sfile /* r30 */, int fileflags /* r31 */) {
    // Local variables
    int cltver; // r1+0x8
...
static char fname[64]; // size: 0x40, address: 0x80AA4400
static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFEAB4
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// total size: 0x4
struct /* @class$1115xstransvc_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1253xstransvc_cpp */ {
    // Members
...
// Range: 0x800CBE98 -> 0x800CBFC4
static char * XST_translate_sid(unsigned int sid /* r29 */, char * exten /* r30 */, unsigned char local /* r31 */) {
    // References
...
static char fname[64]; // size: 0x40, address: 0x80AA4440
// Range: 0x800CBFC4 -> 0x800CC0FC
static char * XST_translate_sid_path(unsigned int sid /* r29 */, char * exten /* r30 */, unsigned char local /* r31 */) {
    // Local variables
    char path_delimiter[2]; // r1+0x8
    char subdir[4]; // r1+0xC
...
// Range: 0x800CC0FC -> 0x800CC12C
static void XST_reset_raw() {
    // References
    // -> static struct st_STRAN_DATA g_xstdata;
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80BFEAC8
// Range: 0x800CC12C -> 0x800CC22C
...
// Range: 0x800CC22C -> 0x800CC284
static void XST_unlock(struct st_STRAN_SCENE * sdata /* r0 */) {
    // References
    // -> static struct st_STRAN_DATA g_xstdata;
}

// Range: 0x800CC284 -> 0x800CC2FC
...
// Range: 0x800CC2FC -> 0x800CC310
static struct st_STRAN_SCENE * XST_get_rawinst(int idx /* r0 */) {
    // References
    // -> static struct st_STRAN_DATA g_xstdata;
}

// Range: 0x800CC310 -> 0x800CC3D8
...
// Range: 0x800CC3D8 -> 0x800CC438
static struct st_STRAN_SCENE * XST_nth_locked(int nth /* r0 */) {
    // Local variables
    struct st_STRAN_SCENE * sdata; // r6
    int cnt; // r7
    int i; // r8
...
// Range: 0x800CC438 -> 0x800CC548
static struct st_STRAN_SCENE * XST_find_bySID(unsigned int sid /* r0 */, int flagHipHop /* r0 */) {
    // Local variables
    struct st_STRAN_SCENE * da_sdata; // r8
    int i; // r9
...
static char __FUNCTION__[23]; // size: 0x17, address: 0x80BFEAD8
// Range: 0x800CC548 -> 0x800CC5E4
...
// Range: 0x800CC5E4 -> 0x800CC5EC
char * xSTGetLocalizationCode() {
    // References
    // -> static char g_localCode[4];
}

// Range: 0x800CC5EC -> 0x800CC5F4
enum xRegion xSTGetLocalizationEnum() {
    // References
    // -> static enum xRegion g_localEnum;
}

// Range: 0x800CC5F4 -> 0x800CC61C
void xSTAddToDiscLanguageConfig(enum xRegion region /* r0 */) {
    // References
    // -> static unsigned int g_localDiscLanguageConfig;
}

// Range: 0x800CC61C -> 0x800CC624
unsigned int xSTGetDiscLanguageConfig() {
    // References
    // -> static unsigned int g_localDiscLanguageConfig;
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFEAF0
// Range: 0x800CC624 -> 0x800CC780
int xSTGetAssetTypes(unsigned int * types /* r31 */, int max_types /* r25 */) {
    // Local variables
    unsigned int * type_buffer; // r29
    unsigned int * type; // r28
    unsigned int * end_type; // r27
    int scenes; // r0
    int i; // r26
    struct st_STRAN_SCENE * sdata; // r0
    const struct PKRAssetType * atype; // r3
    int itotal; // r26
    unsigned int last_type_value; // r3
    unsigned int * otype; // r26
    unsigned int * end_otype; // r4
    const unsigned int * itype; // r5
    const unsigned int * end_itype; // r6
