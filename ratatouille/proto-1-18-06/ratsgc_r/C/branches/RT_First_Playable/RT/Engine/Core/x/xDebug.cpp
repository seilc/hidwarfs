/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xDebug.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80028000 -> 0x8002DBE8
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct basic_rect screen_bounds; // size: 0x10, address: 0x80C0A330
enum eFMVFile {
    eFMVFile_NoMovie = 0,
...
char __FUNCTION__$localstatic1$get_at__36tier_queue<Q29xFXRibbon10joint_data>CFUl[7]; // size: 0x7, address: 0x80BFBCF0
char __FUNCTION__$localstatic1$get_age__9xFXRibbonCFRCQ29xFXRibbon10joint_data[8]; // size: 0x8, address: 0x80BFBCF8
static unsigned char gInRenderLoop; // size: 0x1, address: 0x80C06F18
static int g_STFU; // size: 0x4, address: 0x80C06F1C
static void (* g_msgnotify)(enum en_VERBOSE_MSGLEVEL, char *); // size: 0x4, address: 0x80C06F20
// total size: 0x4
struct __file_modes {
    // Members
...
static struct _FILE * g_userfp; // size: 0x4, address: 0x80C06F24
static struct _FILE * g_errfp; // size: 0x4, address: 0x80C06F28
static int g_dbinit; // size: 0x4, address: 0x80C06F2C
static long long g_lasttym; // size: 0x8, address: 0x80C06F30
static char g_lprbuf[2048]; // size: 0x800, address: 0x80414588
...
static enum en_VERBOSE_MSGLEVEL g_msglvl; // size: 0x4, address: 0x80BFBD00
static int g_prepend_msglevel; // size: 0x4, address: 0x80BFBD04
static char * g_dbml_strz[13]; // size: 0x34, address: 0x80414D88
// total size: 0x18
struct analog_data {
    // Members
...
// Range: 0x80028000 -> 0x80028058
unsigned char xDebugIDontWantYourXprintsOnMyScreen() {
    // References
    // -> struct xGlobals * xglobals;
}

void (* dm_callbacks[20])(); // size: 0x50, address: 0x80414DBC
static char * dm_names[20]; // size: 0x50, address: 0x80414E0C
static char xStatsFieldName[35][32]; // size: 0x460, address: 0x80414E5C
unsigned int gFrameCount; // size: 0x4, address: 0x80C06F38
unsigned int gSceneFrameCount; // size: 0x4, address: 0x80C06F3C
static int sDebugMode; // size: 0x4, address: 0x80C06F40
int last_debug_mode; // size: 0x4, address: 0x80C06F44
...
struct DebugModeData {
    // Members
    void (* callback)(); // offset 0x0, size 0x4
    const char * name; // offset 0x4, size 0x4
    int virtualID; // offset 0x8, size 0x4
};
struct DebugModeData sDebugModeData[68]; // size: 0x330, address: 0x80459A90
int sDebugModeLimit; // size: 0x4, address: 0x80C06F48
int sDebugModeNextVirtualID; // size: 0x4, address: 0x80C06F4C
// Range: 0x80028058 -> 0x8002809C
static int mapVirtualIDToRealID(int virtualID /* r0 */) {
    // Local variables
    int i; // r5

    // References
    // -> int sDebugModeLimit;
    // -> struct DebugModeData sDebugModeData[68];
}
...
static struct xSB screenSB; // size: 0x10, address: 0x80C06F50
static struct xSB relMemInfoSB; // size: 0x10, address: 0x80C06F60
static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFBD10
void * RwEngineInstance; // size: 0x4, address: 0x80C09F34
// Range: 0x8002809C -> 0x800281A8
void xSBCreate(struct xSB * sb /* r30 */, unsigned int size /* r31 */) {
    // References
    // -> void * RwEngineInstance;
    // -> static char __FUNCTION__[10];
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80BFBD1C
// Range: 0x800281A8 -> 0x80028290
char * xSBAppend(struct xSB * sb /* r28 */, const char * str /* r29 */) {
    // Local variables
    char * pRetVal; // r30
...
static class xtextbox tb; // size: 0x74, address: 0x804152BC
static signed char init; // size: 0x1, address: 0x80C06F70
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x80028290 -> 0x8002850C
void xSBRender(struct xSB * sb /* r31 */) {
    // Local variables
    int lineCount; // r6
    int i; // r7
    ...
    // -> struct xColor_tag g_GRAY80;
    // -> struct xColor_tag g_BLACK;
    // -> void * RwEngineInstance;
}

// total size: 0x2
struct /* @class$1780xDebug_cpp */ {
    // Members
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80BFBD28
// Range: 0x8002850C -> 0x80028588
void xSBReset(struct xSB * sb /* r31 */) {
    // References
    // -> static char __FUNCTION__[9];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFBD34
// Range: 0x80028588 -> 0x80028618
void xSBDestroy(struct xSB * sb /* r31 */) {
    // References
    // -> void * RwEngineInstance;
    // -> static char __FUNCTION__[11];
}

// Range: 0x80028618 -> 0x800286D8
void DBStartup(enum en_VERBOSE_MSGLEVEL setlvl /* r0 */, char * logfile /* r4 */, char * errfile /* r31 */, void (* msgnotify)(enum en_VERBOSE_MSGLEVEL, char *) /* r0 */) {
    // References
    // -> static struct xSB relMemInfoSB;
    // -> static struct xSB screenSB;
    // -> static long long g_lasttym;
    // -> static struct _FILE * g_errfp;
    // -> static struct _FILE * g_userfp;
    // -> static void (* g_msgnotify)(enum en_VERBOSE_MSGLEVEL, char *);
    // -> static enum en_VERBOSE_MSGLEVEL g_msglvl;
    // -> static int g_dbinit;
}

// Range: 0x800286D8 -> 0x80028748
void DBShutdown() {
    // References
    // -> static struct xSB relMemInfoSB;
    // -> static struct xSB screenSB;
...
// Range: 0x80028748 -> 0x800287C8
int DBChkMsgLvl(enum en_VERBOSE_MSGLEVEL msglvl /* r0 */) {
    // References
    // -> static enum en_VERBOSE_MSGLEVEL g_msglvl;
    // -> struct _FILE __files[];
}

static long long elap; // size: 0x8, address: 0x80C06F78
static signed char init; // size: 0x1, address: 0x80C06F80
// total size: 0xC
...
// Range: 0x800287C8 -> 0x80028ADC
void DBprintf(enum en_VERBOSE_MSGLEVEL msglvl /* r31 */, const char * fmt /* r30 */) {
    // Local variables
    struct __va_list_struct marker[1]; // r1+0x68
    char * skipptr; // r3
    long long tym; // r29
    ...
    // -> static struct xSB relMemInfoSB;
    // -> static struct xSB screenSB;
    // -> static char * g_dbml_strz[13];
    // -> static int g_prepend_msglevel;
    // -> static long long elap;
    // -> static long long g_lasttym;
    // -> static int g_STFU;
    // -> static signed char init;
}

// Range: 0x80028ADC -> 0x80028AFC
void xDebugVSync() {}

// Range: 0x80028AFC -> 0x80028C00
void xprintf(const char * fmt /* r29 */) {
    // Local variables
    struct __va_list_struct marker[1]; // r1+0x68
    char buf[512]; // r1+0x274
    char buf2[512]; // r1+0x74
    char * s; // r4
    char * d; // r5
}

static const char * _assert2func; // size: 0x4, address: 0x80C06F84
static const char * _assert2file; // size: 0x4, address: 0x80C06F88
static unsigned int _assert2line; // size: 0x4, address: 0x80C06F8C
static const char * _assert2expr; // size: 0x4, address: 0x80C06F90
// Range: 0x80028C00 -> 0x80028C14
void xDebug_assert2_info(const char * func /* r0 */, const char * file /* r0 */, unsigned int line /* r0 */, const char * expr /* r0 */) {
    // References
    // -> static const char * _assert2expr;
    // -> static unsigned int _assert2line;
    // -> static const char * _assert2file;
    // -> static const char * _assert2func;
}

static unsigned char first; // size: 0x1, address: 0x80C06F94
static signed char init; // size: 0x1, address: 0x80C06F95
// total size: 0x20
struct DVDDiskID {
    // Members
...
// Range: 0x80028C14 -> 0x80028E24
void xDebugReportAssertion(const char * sfile /* r27 */, int line /* r28 */, const char * func /* r29 */, const char * expr /* r30 */, const char * message /* r31 */) {
    // Local variables
    struct tag_xFile file; // r1+0x208
    char workingBuffer[512]; // r1+0x8
    unsigned int result; // r26
    const char * platform; // r25
    char * it; // r5
...
char * region_array[21]; // size: 0x54, address: 0x8042BBD0
enum xRegion {
    eRegionUnknown = -1,
    eRegionUS = 0,
    eRegionBE = 1,
...
// Range: 0x80028E24 -> 0x80028F40
void xDebug_assert2(const char * fmt /* r31 */) {
    // Local variables
    struct __va_list_struct argp[1]; // r1+0x68
    char buf1[512]; // r1+0x274
    char buf2[512]; // r1+0x74
    char pretty_buf[640]; // r1+0x474

    // References
    // -> unsigned int gzAsyncRenderDisable;
    // -> static const char * _assert2expr;
    // -> static const char * _assert2func;
    // -> static unsigned int _assert2line;
    // -> static const char * _assert2file;
...
static struct RwCamera * g_visass_cam; // size: 0x4, address: 0x80C06F98
static float s_visass_y[2]; // size: 0x8, address: 0x80C06F9C
struct _tagxPad mPad[4]; // size: 0x4E0, address: 0x80A84818
// Range: 0x80028F40 -> 0x80028F98
static int SkipButtonPressedAnyPad() {
    // Local variables
    int pressed; // r3
    int i; // r0
...
int LetItSkip_and_LetItCrash; // size: 0x4, address: 0x80C06FA4
unsigned int gSoak; // size: 0x4, address: 0x80C07E3C
// Range: 0x80028F98 -> 0x80029134
unsigned int xDebugBoing() {
    // Local variables
    struct RwCamera * cam; // r30
    struct basic_rect top; // r1+0x24
    struct basic_rect bot; // r1+0x14
    struct xColor_tag bg; // r1+0x10

    // References
    // -> unsigned int gFrameCount;
    // -> static float s_visass_y[2];
    // -> static struct RwCamera * g_visass_cam;
    // -> static unsigned char gInRenderLoop;
    // -> int LetItSkip_and_LetItCrash;
    // -> struct xGlobals * xglobals;
    // -> unsigned int gSoak;
}

// Range: 0x80029134 -> 0x80029288
int xDebugModeAdd(const char * debugModeName /* r30 */, void (* func)() /* r31 */) {
    // Local variables
    int i; // r27
    int positionToAdd; // r27
    int i; // r26
    int virtualID; // r0

    // References
    // -> struct DebugModeData sDebugModeData[68];
    // -> int sDebugModeNextVirtualID;
    // -> static int sDebugMode;
    // -> int sDebugModeLimit;
}

// Range: 0x800292A4 -> 0x80029324
void xDebugModeSet(int mode /* r31 */) {
    // Local variables
    int oldmode; // r0
    int realMode; // r0
...
// Range: 0x80029324 -> 0x8002937C
static void xDebugModeNext() {
    // References
    // -> struct DebugModeData sDebugModeData[68];
    // -> int sDebugModeLimit;
    // -> static int sDebugMode;
...
// Range: 0x8002937C -> 0x800293C8
static void xDebugModePrev() {
    // References
    // -> struct DebugModeData sDebugModeData[68];
    // -> static int sDebugMode;
    // -> int sDebugModeLimit;
...
// Range: 0x800293E4 -> 0x80029470
void xDebugModeSetByName(const char * name /* r29 */) {
    // Local variables
    int i; // r30
...
static char __FUNCTION__[27]; // size: 0x1B, address: 0x80415330
// Range: 0x80029470 -> 0x8002950C
void xDebugModeRegisterCallback(int mode /* r0 */, void (* func)() /* r30 */) {
    // Local variables
    int realID; // r0
...
// Range: 0x8002950C -> 0x80029518
void xDebugRenderLoopStart() {
    // References
    // -> static unsigned char gInRenderLoop;
}

// Range: 0x80029518 -> 0x80029524
void xDebugRenderLoopEnd() {
    // References
    // -> static unsigned char gInRenderLoop;
}
...
static struct ScreenCapture sScreenCaptureModes[15]; // size: 0x78, address: 0x8041534C
static char * sScreenCaptureNames[15]; // size: 0x3C, address: 0x80459DC0
static char sScreenCaptureNamesBuffer[210]; // size: 0xD2, address: 0x80459DFC
static unsigned int sScreenCaptureMode; // size: 0x4, address: 0x80C06FA8
static int sDumpFileNumber; // size: 0x4, address: 0x80C06FAC
static unsigned char sDumpFileValid; // size: 0x1, address: 0x80C06FB0
static char sDumpFile[512]; // size: 0x200, address: 0x80459ED0
char gScreenDumpPath[256]; // size: 0x100, address: 0x8045A0D0
enum xScreenDump {
    exSD_NONE = 0,
    exSD_NATIVE = 1,
    exSD_MASSIVE = 2,
...
// Range: 0x80029524 -> 0x800295D4
char * xDebugGetNextDumpFile() {
    // Local variables
    struct tag_xFile file; // r1+0x8

    // References
    // -> static char sDumpFile[512];
    // -> static unsigned char sDumpFileValid;
    // -> static int sDumpFileNumber;
    // -> char gScreenDumpPath[256];
}

// Range: 0x800295D4 -> 0x800295E0
void xDebugAdvanceDumpFile() {
    // References
    // -> static unsigned char sDumpFileValid;
}

// total size: 0x18
struct gMassiveDumpParams {
    // Members
    int width; // offset 0x0, size 0x4
    int height; // offset 0x4, size 0x4
    int tile_width; // offset 0x8, size 0x4
    int tile_height; // offset 0xC, size 0x4
    int cur_x_tile; // offset 0x10, size 0x4
    int cur_y_tile; // offset 0x14, size 0x4
};
struct gMassiveDumpParams gMassiveDump; // size: 0x18, address: 0x80C06FB8
static unsigned char * sMassiveBuffer; // size: 0x4, address: 0x80C06FD0
static int sMassiveBufferMax; // size: 0x4, address: 0x80C06FD4
int sSeekMin; // size: 0x4, address: 0x80C06FD8
int sSeekMax; // size: 0x4, address: 0x80C06FDC
static char __FUNCTION__[25]; // size: 0x19, address: 0x804153C4
// Range: 0x800295E0 -> 0x800296E4
static void xDebugBufferedWriteFlush(struct tag_xFile * file /* r31 */) {
    // Local variables
    int n; // r0

    // References
    // -> static char __FUNCTION__[25];
    // -> static int sMassiveBufferMax;
    // -> static unsigned char * sMassiveBuffer;
    // -> int sSeekMin;
    // -> int sSeekMax;
}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80BFBD40
// Range: 0x800296E4 -> 0x800297F0
static void xDebugBufferedWrite(struct tag_xFile * file /* r0 */, int seek /* r29 */, unsigned char * data /* r30 */, int size /* r31 */) {
    // References
    // -> int sSeekMin;
    // -> static unsigned char * sMassiveBuffer;
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80BFBD54
// total size: 0xC
...
// Range: 0x800297F0 -> 0x800299A4
static unsigned char xDebugCreateBMP(const char * fname /* r29 */, int width /* r0 */, int height /* r0 */, struct tag_xFile * file /* r31 */) {
    // Local variables
    struct BmpMegaHeader mhdr; // r1+0xC
    unsigned int n; // r0
    unsigned char zero; // r1+0x8

    // References
    // -> static char __FUNCTION__[16];
    // -> struct gMassiveDumpParams gMassiveDump;
}

// Range: 0x800299A4 -> 0x80029B88
static void xDebugWriteBMPTile(struct tag_xFile * file /* r26 */, int width /* r0 */, int height /* r0 */, int x /* r0 */, int y /* r0 */, unsigned char * pixels /* r8 */) {
    // Local variables
    int seek; // r29
    unsigned char * sp; // r28
    int yy; // r27
    unsigned char rgb[1920]; // r1+0x8
    int xx; // r5
...
// Range: 0x80029B88 -> 0x80029C94
static struct RwCamera * RenderTile(struct RwCamera * camera /* r29 */, int tx /* r30 */, int ty /* r31 */) {
    // Local variables
    struct RwRGBA bkcol; // r1+0x8
...
// Range: 0x80029C94 -> 0x80029DBC
static struct RwImage * SaveTile(struct RwImage * image /* r27 */, int tx /* r28 */, int ty /* r29 */, void * pData /* r30 */) {
    // Local variables
    int y; // r0
...
struct xMat4x3 gMassiveDumpCameraMatrix; // size: 0x40, address: 0x8045A1D0
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
struct xVec2 gMassiveDumpViewWindow; // size: 0x8, address: 0x80C06FE0
static char __FUNCTION__[22]; // size: 0x16, address: 0x80BFBD64
// Range: 0x80029DBC -> 0x8002A130
static void xDebugTakeMassiveDump(struct RwCamera * camera /* r29 */) {
    // Local variables
    struct RwRaster * camras; // r4
    int nx; // r6
    int ny; // r7
    const char * file; // r0
    struct tag_xFile fileObject; // r1+0x8
...
// Range: 0x8002A130 -> 0x8002A1C8
static void TakeScreenShotCallback() {
    // Local variables
    struct RwCamera * cam; // r31

    // References
    // -> struct xVec2 gMassiveDumpViewWindow;
    // -> struct xMat4x3 gMassiveDumpCameraMatrix;
    // -> struct zGlobals globals;
    // -> enum xScreenDump gScreenDump;
    // -> static struct ScreenCapture sScreenCaptureModes[15];
    // -> struct gMassiveDumpParams gMassiveDump;
    // -> static unsigned int sScreenCaptureMode;
}

// Range: 0x8002A1C8 -> 0x8002A250
static void NextNameScreenShotCallback(void * value /* r30 */) {
    // Local variables
    const char * * str; // r0
    const char * f; // r0
    const char * it; // r3
...
static int envmode; // size: 0x4, address: 0x80C06FE8
static int delay_switch; // size: 0x4, address: 0x80C06FEC
static signed char init; // size: 0x1, address: 0x80C06FF0
static float coeff; // size: 0x4, address: 0x80C06FF4
static signed char init; // size: 0x1, address: 0x80C06FF8
static int matno; // size: 0x4, address: 0x80C06FFC
static signed char init; // size: 0x1, address: 0x80C07000
static int texno; // size: 0x4, address: 0x80C07004
static signed char init; // size: 0x1, address: 0x80C07008
static int drawsize; // size: 0x4, address: 0x80C0700C
static signed char init; // size: 0x1, address: 0x80C07010
static unsigned char showbadtextures; // size: 0x1, address: 0x80C07011
static signed char init; // size: 0x1, address: 0x80C07012
static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFBD7C
...
// Range: 0x8002A250 -> 0x8002B144
void xDebugModeService() {
    // Local variables
    unsigned int num; // r0
    struct RwCamera * cam; // r27
    struct RwCamera * cam; // r27
    char str[8]; // r1+0x8
    struct xVec3 * p; // r0
    unsigned int usage; // r3
    struct RpWorld * world; // r31
    struct RpMaterial * mat; // r30
    int nmat; // r29
    int ntex; // r28
    struct RwTexture * tex; // r5
    int i; // r29
    struct PKRAssetTOCInfo xinfo; // r1+0x50
    struct PKRAssetTOCInfo xinfo; // r1+0x30
    int fx; // r0
    ...
    float dy; // f27
    float recipCamZ; // f31
    struct PKRAssetTOCInfo xinfo; // r1+0x10
    float x; // f27
    float y; // f28
    float dx; // f29
    float dy; // f30
    ...
    // -> static int envmode;
    // -> static unsigned char showbadtextures;
    // -> static signed char init;
    // -> static signed char init;
    // -> static signed char init;
    // -> static signed char init;
...
// total size: 0x1
struct /* @class$1038xDebug_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1039xDebug_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1040xDebug_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1041xDebug_cpp */ {
    // Members
...
unsigned int XDG_XMIN; // size: 0x4, address: 0x80BFBD90
unsigned int XDG_XMAX; // size: 0x4, address: 0x80BFBD94
unsigned int XDG_YHEIGHT; // size: 0x4, address: 0x80BFBD98
float XDG_XSCALE; // size: 0x4, address: 0x80C07014
// total size: 0x2C
struct DGstate {
    // Members
    unsigned int flags; // offset 0x0, size 0x4
    unsigned int draw; // offset 0x4, size 0x4
    char * name[6]; // offset 0x8, size 0x18
    unsigned int pick; // offset 0x20, size 0x4
    signed char field[6]; // offset 0x24, size 0x6
};
struct DGstate gDG; // size: 0x2C, address: 0x8045A440
// total size: 0x1B0
struct XDG {
    // Members
    unsigned short flags; // offset 0x0, size 0x2
    unsigned short idx; // offset 0x2, size 0x2
    float value[101]; // offset 0x4, size 0x194
    float scale; // offset 0x198, size 0x4
    struct xColor_tag graphcolor; // offset 0x19C, size 0x4
    float tickmin; // offset 0x1A0, size 0x4
    float tickmax; // offset 0x1A4, size 0x4
    float tickstep; // offset 0x1A8, size 0x4
    struct xColor_tag tickcolor; // offset 0x1AC, size 0x4
};
static struct XDG dg[6]; // size: 0xA20, address: 0x8045A46C
static struct xColor_tag dg_color[6]; // size: 0x18, address: 0x80C07018
static char dg_color_str[6][8]; // size: 0x30, address: 0x804153E0
static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFBD9C
// Range: 0x8002B144 -> 0x8002B394
void xDebugGraphSetup(struct XDG * gp /* r29 */, float scale /* f28 */, struct xColor_tag & graphcolor /* r31 */, float tickmin /* f29 */, float tickmax /* f30 */, float tickstep /* f31 */, struct xColor_tag & tickcolor /* r30 */) {
    // References
    // -> unsigned int XDG_XMAX;
    // -> float XDG_XSCALE;
    // -> unsigned int FB_YRES;
    // -> unsigned int XDG_YHEIGHT;
    // -> unsigned int XDG_XMIN;
    // -> unsigned int FB_XRES;
    // -> static char __FUNCTION__[17];
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFBDB0
// Range: 0x8002B394 -> 0x8002B458
void xDebugGraphUpdate(struct XDG * gp /* r31 */, float new_value /* f31 */) {
    // References
    // -> static char __FUNCTION__[18];
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80BFBDC4
// Range: 0x8002B458 -> 0x8002B658
void xDebugGraphRender(struct XDG * gp /* r31 */) {
    // Local variables
    int i; // r8
    int idx; // r9
    float f; // f30
    struct xVec2 pt[101]; // r1+0x10
    struct xVec2 * vp; // r10
...
// Range: 0x8002B658 -> 0x8002B84C
void xDebugGraphInit() {
    // Local variables
    unsigned int i; // r26
    struct xColor_tag gray; // r1+0x10

    // References
    // -> struct DGstate gDG;
    // -> unsigned int FB_YRES;
    // -> float VBLANKS_PER_SEC;
    // -> static struct XDG dg[6];
    // -> static struct xColor_tag dg_color[6];
}

// Range: 0x8002B84C -> 0x8002B898
static int dgcmp(void * A /* r0 */, void * B /* r0 */) {}

// total size: 0x10
struct xDGsort {
    // Members
    long long val; // offset 0x0, size 0x8
    unsigned int idx; // offset 0x8, size 0x4
};
// Range: 0x8002B898 -> 0x8002B9E0
static void xDebugGraphSort() {
    // Local variables
    unsigned int i; // r11
    struct xDGsort * list; // r31
    long long * xp; // r12
...
static int g_xgdb_usage; // size: 0x4, address: 0x80C07030
// Range: 0x8002B9E0 -> 0x8002BC14
void xDebugGraphUsageMsg() {
    // References
    // -> struct DGstate gDG;
    // -> static int g_xgdb_usage;
}

// Range: 0x8002BC14 -> 0x8002C39C
void xDebugGraphService() {
    // Local variables
    unsigned int i; // r5
    unsigned int j; // r6
    float x; // r1+0x14
    ...
    float angle; // r1+0xC
    float mag; // r1+0x8
    unsigned int stack; // r24
    unsigned int val; // r7
    unsigned int stack_it; // r0
    unsigned int n; // r23

    // References
    // -> struct xStats gStats;
    // -> static char xStatsFieldName[35][32];
    // -> unsigned int gFrameCount;
    // -> struct DGstate gDG;
    // -> static char dg_color_str[6][8];
    // -> static struct XDG dg[6];
    // -> static int g_xgdb_usage;
}
...
static struct tweak_callback cb; // size: 0x28, address: 0x80415410
static signed char init; // size: 0x1, address: 0x80C07034
// Range: 0x8002C39C -> 0x8002C578
void xDebugInit() {
    // Local variables
    int i; // r29
    char * p; // r28
    ...
    // -> static signed char init;
    // -> static char * sScreenCaptureNames[15];
    // -> static unsigned int sScreenCaptureMode;
    // -> static struct ScreenCapture sScreenCaptureModes[15];
    // -> static char sScreenCaptureNamesBuffer[210];
    // -> static char * dm_names[20];
    // -> void (* dm_callbacks[20])();
    // -> int sDebugModeNextVirtualID;
    // -> int sDebugModeLimit;
    // -> struct DebugModeData sDebugModeData[68];
}

// Range: 0x8002C578 -> 0x8002C5A0
void xDebugUpdate() {}

// Range: 0x8002C5A0 -> 0x8002C5A4
void xDebugExit() {}

struct xColor_tag g_WHITE; // size: 0x4, address: 0x80C0A27C
char timestamp[]; // size: 0x0, address: 0x80430710
// Range: 0x8002C5A4 -> 0x8002C7B8
void xDebugTimestampScreen() {
    // Local variables
    struct RwCamera * cam; // r31
    struct RwRGBA bg; // r1+0x14
    class xtextbox tb; // r1+0xBC
    unsigned int i; // r30
...
struct OldRenderState sOldRenderState; // size: 0x24, address: 0x8045AE8C
// Range: 0x8002C7B8 -> 0x8002C7E8
void xDebugAssertStartup() {
    // References
    // -> unsigned int FB_YRES;
    // -> unsigned int FB_XRES;
    // -> static struct RwCamera * g_visass_cam;
...
// Range: 0x8002C7E8 -> 0x8002C814
void xDebugAssertShutdown() {
    // References
    // -> static struct RwCamera * g_visass_cam;
}

static unsigned char in_this_function; // size: 0x1, address: 0x80C07035
static signed char init; // size: 0x1, address: 0x80C07036
// Range: 0x8002C814 -> 0x8002CCB0
void xDebugAssertScreen(const char * str1 /* r21 */, const char * str2 /* r20 */, const char * pretty_str /* r19 */) {
    // Local variables
    struct RwCamera * cam; // r25
    struct RwCamera * ccam; // r24
    struct RwRGBA bg; // r1+0x14
    char str[2048]; // r1+0x130
    char * c; // r5
    class xtextbox tb; // r1+0xBC
    unsigned int i; // r19

    // References
    // -> static unsigned char in_this_function;
    // -> struct OldRenderState sOldRenderState;
    // -> struct xColor_tag g_BLACK;
    // -> static float s_visass_y[2];
    // -> static struct basic_rect screen_bounds;
    // -> struct xColor_tag g_WHITE;
...
// Range: 0x8002CCB0 -> 0x8002CCD4
void xDebugStackTrace() {}

static int page; // size: 0x4, address: 0x80C07038
static signed char init; // size: 0x1, address: 0x80C0703C
static int selected; // size: 0x4, address: 0x80C07040
static signed char init; // size: 0x1, address: 0x80C07044
static int delay_switch; // size: 0x4, address: 0x80C07048
static signed char init; // size: 0x1, address: 0x80C0704C
// Range: 0x8002CCD4 -> 0x8002D1E8
void debug_mode_page() {
    ...
    int aItem; // r29
    char a[18]; // r1+0x1C
    unsigned char aSel; // r29
    int bItem; // r28
    char b[18]; // r1+0x8
    unsigned char bSel; // r0
...
struct TexInfo {
    ...
    const char * name; // offset 0x0, size 0x4
    unsigned int size; // offset 0x4, size 0x4
    unsigned int firstPixel; // offset 0x8, size 0x4
    unsigned int width : 13; // offset 0xC, size 0x4
    unsigned int height : 13; // offset 0xC, size 0x4
    unsigned int depth : 6; // offset 0xC, size 0x4
};
// Range: 0x8002D1E8 -> 0x8002D234
static int texcmp(void * A /* r3 */, void * B /* r4 */) {
    // Local variables
    struct TexInfo * a; // r0
    struct TexInfo * b; // r0
}

// Range: 0x8002D234 -> 0x8002D5CC
void xDebugTextureReport() {
    // Local variables
    int i; // r29
    int ntex; // r30
    struct TexInfo * texlist; // r28
    struct TexInfo * tp; // r27
    struct PKRAssetTOCInfo xinfo; // r1+0x10
    struct RwTexture * tex; // r3
    struct RwRaster * raster; // r26
    unsigned int palette_size; // r26
    char str[8]; // r1+0x8
...
struct SFXInfo {
    // Members
    const char * name; // offset 0x0, size 0x4
    const char * sound; // offset 0x4, size 0x4
    unsigned int size; // offset 0x8, size 0x4
    unsigned int calcSize; // offset 0xC, size 0x4
};
enum xTexDumpType {
    xTexDump_BMP = 0,
    xTexDump_RAW = 1,
...
// total size: 0x18
struct _xDrawTextInfo_t {
    // Members
    char * pszText; // offset 0x0, size 0x4
    struct xVec3 vPosWS; // offset 0x4, size 0xC
    float fWidth; // offset 0x10, size 0x4
    float fHeight; // offset 0x14, size 0x4
};
static struct _xDrawTextInfo_t * _pInfo; // size: 0x4, address: 0x80C07050
static struct xSB _xDrawTextSB; // size: 0x10, address: 0x80C07054
static unsigned int _nDrawTextInfo; // size: 0x4, address: 0x80C07064
static unsigned int _nDrawTextInfoMaxLines; // size: 0x4, address: 0x80BFBDD8
// Range: 0x8002D5CC -> 0x8002D644
void xDrawTextInit(unsigned int uBufferSize /* r3 */, unsigned int uNumLinesMax /* r0 */) {
    // References
    // -> static unsigned int _nDrawTextInfo;
    // -> static unsigned int _nDrawTextInfoMaxLines;
    // -> void * RwEngineInstance;
    // -> static struct _xDrawTextInfo_t * _pInfo;
    // -> static struct xSB _xDrawTextSB;
}

// Range: 0x8002D644 -> 0x8002D684
void xDrawTextExit() {
    // References
    // -> static struct _xDrawTextInfo_t * _pInfo;
    // -> void * RwEngineInstance;
...
// Range: 0x8002D684 -> 0x8002D84C
void xDrawTextRender() {
    // Local variables
    struct xColor_tag color; // r1+0x10
    struct xVec2 vUnitScreen; // r1+0x14
    unsigned int i; // r29
    class xfont font; // r1+0x6C
    struct basic_rect bounds; // r1+0x2C
...
// Range: 0x8002D84C -> 0x8002D908
void xDrawText(const struct xVec3 & vWorld /* r31 */, const char * fmt /* r4 */) {
    // Local variables
    struct __va_list_struct marker[1]; // r1+0x68
...
// Range: 0x8002D908 -> 0x8002D9A0
void xDrawText(const struct xVec3 & vWorld /* r3 */, float fWidth /* f0 */, float fHeight /* f0 */, const char * pszText /* r30 */) {
    // Local variables
    struct _xDrawTextInfo_t * pIn; // r31
...
struct xMtx4x3 {
    ...
    union { // inferred
        ...
        struct xMat3x3 m_xMat3x3; // offset 0x0, size 0x30
        struct xMat4x3 m_xMat4x3; // offset 0x0, size 0x40
        struct RwMatrixTag m_RwMatrix; // offset 0x0, size 0x40
        struct { // inferred
            struct xVec3 right; // offset 0x0, size 0xC
            int flags; // offset 0xC, size 0x4
            struct xVec3 up; // offset 0x10, size 0xC
...
// Range: 0x8002D9A0 -> 0x8002DB64
unsigned char xDrawTextComputeScreenPt(struct xVec2 & vUnitScreen /* r29 */, const struct xVec3 & vWorldSpace /* r30 */) {
    // Local variables
    struct RwCamera * pCam; // r31
    float fNearClip; // f31
    float fFarClip; // f30
    struct xMtx4x3 mtxView; // r1+0x20
    struct xVec3 vCameraSpace; // r1+0x10
    float fRecipZ; // f2
