/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zUIText.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8017BAF0 -> 0x8017C674
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static unsigned char isCJK; // size: 0x1, address: 0x80C085B0
static float HSCALE; // size: 0x4, address: 0x80C085B4
static float VSCALE; // size: 0x4, address: 0x80C085B8
// total size: 0x80
class zUIText : public zUI {
    ...
    void setup_textbox(class xtextbox & tb, const struct zUITextAsset * asset) const;
...
// Range: 0x8017BBB8 -> 0x8017BDE0
// this: r29
void zUIText::setup_textbox(class xtextbox & tb /* r30 */, const struct zUITextAsset * asset /* r31 */) const {}
...
// Range: 0x8017BDE0 -> 0x8017C09C
// this: r31
void zUIText::DoRender() const {
    // Local variables
    const struct zUITextAsset * asset; // r30
    class xtextbox tb; // r1+0x10
    float hscale; // f31
    float vscale; // f30
    float oldWidth; // f29
    float oldHeight; // f28
    const class layout & layout; // r0
    float actualWidth; // r1+0xC
    float actualHeight; // r1+0x8

    // References
    // -> static unsigned char isCJK;
    // -> static float VSCALE;
    // -> static float HSCALE;
}

// Range: 0x8017C09C -> 0x8017C100
// this: r31
void zUIText::DoHandleEvent(struct xBase * from /* r0 */, unsigned int toEvent /* r0 */, const float * toParam /* r0 */, struct xBase * toParamWidget /* r0 */, unsigned int toParamWidgetID /* r8 */) {}

// Range: 0x8017C100 -> 0x8017C16C
// this: r30
void zUIText::DoApplyMotionFrame(const struct zUIMotionFrame * frame /* r31 */) {}

static char * fontNames[4]; // size: 0x10, address: 0x80C01C18
static char * justifyXNames[3]; // size: 0xC, address: 0x80C01C28
static char * justifyYNames[3]; // size: 0xC, address: 0x80C01C34
// Range: 0x8017C16C -> 0x8017C488
// this: r31
void zUIText::DoAddTweaks(const char * baseName /* r30 */) {
    // Local variables
    struct zUITextAsset * asset; // r0
    char buffer[128]; // r1+0x8

    // References
    // -> static char * justifyYNames[3];
    // -> static char * justifyXNames[3];
    // -> static char * fontNames[4];
}

// Range: 0x8017C488 -> 0x8017C4A8
void zUIText_Init(struct xBase & data /* r0 */, struct xDynAsset & asset /* r0 */) {}

// Range: 0x8017C4A8 -> 0x8017C528
void zUIText_Init(class zUIText * text /* r30 */, struct zUITextAsset * asset /* r31 */) {}
...
// total size: 0x64
struct {} zUIText::__vtable; // size: 0x64, address: 0x8043C9D8
