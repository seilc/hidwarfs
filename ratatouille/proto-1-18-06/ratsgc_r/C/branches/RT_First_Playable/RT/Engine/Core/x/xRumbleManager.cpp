/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\x\xRumbleManager.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800A5F5C -> 0x800A6CE4
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
struct effectInternal {
    // Functions
    void updateCameraShake(float intensity);

    void EndCameraShake();

    float GetIntensity(const struct xVec3 & playerPos);

    // Members
    const struct emitterBase * pEmitter; // offset 0x0, size 0x4
    float timeToLive; // offset 0x4, size 0x4
    float prevMagFraction; // offset 0x8, size 0x4
    struct cameraFX * cameraEffect; // offset 0xC, size 0x4
    unsigned char rumbleForever; // offset 0x10, size 0x1
};
// Range: 0x800A5F5C -> 0x800A6048
// this: r0
void effectInternal::updateCameraShake(float intensity /* f0 */) {
    // Local variables
    const struct effectAsset * pAsset; // r5
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80BFE098
// Range: 0x800A6048 -> 0x800A60DC
// this: r31
...
// Range: 0x800A60DC -> 0x800A623C
// this: r30
float effectInternal::GetIntensity(const struct xVec3 & playerPos /* r0 */) {
    // Local variables
    float intensity; // r1+0x8
    const struct effectAsset * pAsset; // r31
    float magFraction; // f31
}
...
struct padInfo {
    ...
    // Members
    struct effectInternal m_effects[16]; // offset 0x0, size 0x140
    struct effectInternal * m_currentExclusive; // offset 0x140, size 0x4
};
// Range: 0x800A623C -> 0x800A635C
// this: r3
void padInfo::FindNewExclusive() {
    // Local variables
    unsigned int maxPriority; // r6
    unsigned int i; // r7
    unsigned int currentPriority; // r0
}

// total size: 0x510
class Manager {
    // Functions
    unsigned char Add(struct _tagxPad * pad, struct emitterBase * pEmitter);

    unsigned char Stop(struct _tagxPad * pad, struct emitterBase * pEmitter);

    void Update(struct _tagxPad * pad, const struct xVec3 & playerPos, float timeElapsed, unsigned char bPauseState);

    void Reset();

    Manager();

    void StopCameraShake(int padNum);

    void StopCameraShake();

    // Members
    struct padInfo m_padInfo[4]; // offset 0x0, size 0x510
};
// Range: 0x800A635C -> 0x800A6390
void Manager::store(struct padInfo * pInfo /* r0 */, const struct emitterBase * pEmitter /* r0 */, struct effectInternal * pEffectInternal /* r0 */, float timeToLive /* f0 */, unsigned char rumbleForever /* r0 */) {}

static unsigned char done; // size: 0x1, address: 0x80C07628
static signed char init; // size: 0x1, address: 0x80C07629
static char __FUNCTION__[4]; // size: 0x4, address: 0x80BFE0A8
// total size: 0x30
struct xMat3x3 {
    // Members
...
// total size: 0x4
struct /* @class$1097xRumbleManager_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1235xRumbleManager_cpp */ {
    // Members
...
union /* @class$1236xRumbleManager_cpp */ {
...
// total size: 0x10
struct /* @class$1797xRumbleManager_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x800A6390 -> 0x800A6804
// this: r28
unsigned char Manager::Add(struct _tagxPad * pad /* r30 */, struct emitterBase * pEmitter /* r29 */) {
    // Local variables
    int counter; // r31
    int i; // r27
    struct padInfo * pInfo; // r31
    float timeToLive; // f31
    unsigned char rumbleForever; // r30
    struct effectInternal * currentExclusive; // r0
    const struct emitterBase * pCurrentEmitter; // r5
    unsigned int effectPriority; // r4
    struct effectInternal * freeEffect; // r27
    unsigned int i; // r7
...
// total size: 0x890
struct /* @class$2044xRumbleManager_cpp */ {
    // Static members
...
// Range: 0x800A6804 -> 0x800A6870
// this: r0
unsigned char Manager::Stop(struct _tagxPad * pad /* r0 */, struct emitterBase * pEmitter /* r0 */) {
    // Local variables
    struct padInfo * pInfo; // r7
    unsigned int i; // r8
}

// Range: 0x800A6870 -> 0x800A6B2C
// this: r0
void Manager::Update(struct _tagxPad * pad /* r29 */, const struct xVec3 & playerPos /* r30 */, float timeElapsed /* f0 */, unsigned char bPauseState /* r31 */) {
    // Local variables
    struct padInfo * pInfo; // r28
    unsigned int i; // r6
    float intensity; // f31
    unsigned int i; // r27
}
...
static signed char init; // size: 0x1, address: 0x80C0762A
static class Manager rumbleManager; // size: 0x510, address: 0x80A938E0
// Range: 0x800A6B2C -> 0x800A6B70
class Manager & Manager::Get() {
    // References
    // -> static class Manager rumbleManager;
    // -> static signed char init;
}

// Range: 0x800A6B70 -> 0x800A6BB4
// this: r31
void Manager::Reset() {}

// Range: 0x800A6BB4 -> 0x800A6BEC
// this: r31
Manager::Manager() {}

struct _tagxPad mPad[4]; // size: 0x4E0, address: 0x80A84818
// Range: 0x800A6BEC -> 0x800A6C3C
...
// Range: 0x800A6C3C -> 0x800A6C98
// this: r0
void Manager::StopCameraShake(int padNum /* r0 */) {
    // Local variables
    unsigned int j; // r30
}

// Range: 0x800A6C98 -> 0x800A6CE4
// this: r30
