/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Game\zDust.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801B7D6C -> 0x801BA118
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static char * ring_chain_labels[3]; // size: 0xC, address: 0x80C02AB0
static float emit_rate; // size: 0x4, address: 0x80C02ABC
static float size_min; // size: 0x4, address: 0x80C02AC0
static float size_max; // size: 0x4, address: 0x80C02AC4
static float vel_min; // size: 0x4, address: 0x80C02AC8
static float vel_max; // size: 0x4, address: 0x80C02ACC
static float life_min; // size: 0x4, address: 0x80C02AD0
static float life_max; // size: 0x4, address: 0x80C02AD4
static float growth; // size: 0x4, address: 0x80C02AD8
static float intensity_min; // size: 0x4, address: 0x80C02ADC
static float intensity_max; // size: 0x4, address: 0x80C02AE0
static float vel_decay; // size: 0x4, address: 0x80C02AE4
static float rot_vel_min; // size: 0x4, address: 0x80C02AE8
static float rot_vel_max; // size: 0x4, address: 0x80C02AEC
static float base_alpha; // size: 0x4, address: 0x80C02AF0
static float alpha_coeff1; // size: 0x4, address: 0x80C02AF4
static float alpha_coeff2; // size: 0x4, address: 0x80C02AF8
static float alpha_coeff3; // size: 0x4, address: 0x80C02AFC
static float fade_in_time; // size: 0x4, address: 0x80C02B00
static float fade_out_time; // size: 0x4, address: 0x80C02B04
// total size: 0x1C
struct ring_chain_data {
    // Members
    float emit_rate; // offset 0x0, size 0x4
    int type; // offset 0x4, size 0x4
    int segments; // offset 0x8, size 0x4
    float rot_vel; // offset 0xC, size 0x4
    float rot_mag; // offset 0x10, size 0x4
    float dcos; // offset 0x14, size 0x4
    float dsin; // offset 0x18, size 0x4
};
static struct ring_chain_data chain[2]; // size: 0x38, address: 0x8043DB68
static unsigned int texture; // size: 0x4, address: 0x80C02B08
static int texture_rows; // size: 0x4, address: 0x80C02B0C
static int texture_columns; // size: 0x4, address: 0x80C02B10
// total size: 0x8
struct substr {
    // Members
...
// total size: 0x1
struct /* @class$967zDust_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$968zDust_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$969zDust_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$970zDust_cpp */ {
    // Members
...
// Range: 0x801B7D6C -> 0x801B7DE4
static void on_change_ring_fade(const struct tweak_info & info /* r0 */) {
    // References
...
// Range: 0x801B7DE4 -> 0x801B7E04
static void on_refresh_dirt_system() {}

// Range: 0x801B7E04 -> 0x801B7ECC
static void on_reset_ring_chains() {
    // Local variables
    int i; // r29

    // References
    // -> static struct ring_chain_data chain[2];
}
...
static struct tweak_callback cb_ring_fade; // size: 0x28, address: 0x803E8410
static struct tweak_callback cb_refresh_dirt_system; // size: 0x28, address: 0x803E8438
static struct tweak_callback cb_reset_ring_chains; // size: 0x28, address: 0x803E8460
// Range: 0x801B7ECC -> 0x801B8478
static void add_tweaks() {
    ...
    // References
    // -> static struct tweak_callback cb_reset_ring_chains;
    // -> static char * ring_chain_labels[3];
    // -> static struct ring_chain_data chain[2];
    // -> static struct tweak_callback cb_refresh_dirt_system;
    // -> static unsigned int texture;
    // -> static int texture_columns;
    // -> static int texture_rows;
    // -> static struct tweak_callback cb_ring_fade;
    // -> static float fade_out_time;
    // -> static float fade_in_time;
    // -> static float alpha_coeff3;
    // -> static float alpha_coeff2;
    // -> static float alpha_coeff1;
    // -> static float base_alpha;
    // -> static float vel_decay;
...
enum /* @enum$2053zDust_cpp */ {
    RING_CHAIN_RANDOM = 0,
    RING_CHAIN_ROTATE = 1,
    RING_CHAIN_WAVE = 2,
    RING_CHAIN_TYPES = 3,
};
static int dirt_system_id; // size: 0x4, address: 0x80C089D0
static float ring_vel_decay; // size: 0x4, address: 0x80C089D4
static struct xVec2 * dirt_uvs; // size: 0x4, address: 0x80C089D8
static unsigned char refreshed_ring_chains; // size: 0x1, address: 0x80C089DC
static float ring_chain_ang[2]; // size: 0x8, address: 0x80C089E0
static struct xVec2 ring_chain[2][8]; // size: 0x80, address: 0x80BB67B0
// Range: 0x801B8478 -> 0x801B84DC
...
// Range: 0x801B84DC -> 0x801B85F8
static void set_uv_frames(struct xVec2 * uv /* r27 */, int rows /* r28 */, int columns /* r29 */) {
    // Local variables
    float du; // f30
    float dv; // f29
    float v; // f28
...
// Range: 0x801B85F8 -> 0x801B86F4
static void create_dirt_system() {
    // Local variables
    int uvpairs; // r31
    struct xParticleBatchSystem system; // r1+0x8

    // References
    // -> static int dirt_system_id;
    // -> static unsigned int texture;
    // -> static int texture_columns;
    // -> static int texture_rows;
    // -> static struct xVec2 * dirt_uvs;
    // -> unsigned int gActiveHeap;
}

// Range: 0x801B86F4 -> 0x801B87B8
static void refresh_dirt_system() {
    // Local variables
    int uvpairs; // r31
    struct xParticleBatchSystem system; // r1+0x8
...
struct dust_particle {
    ...
    float dsize; // offset 0x28, size 0x4
    struct xColor_tag color; // offset 0x2C, size 0x4
    float rot_vel; // offset 0x30, size 0x4
    const struct xVec2 * uv; // offset 0x34, size 0x4
    float pad1; // offset 0x38, size 0x4
...
// Range: 0x801B87B8 -> 0x801B8A9C
static int update_dust_particles(unsigned char * mem /* r28 */, int count /* r0 */, struct ptank_pool & apool /* r5 */, float dt /* f25 */) {
    // Local variables
    ...
    float a2; // f28
    float a3; // f27
    struct dust_particle * p; // r30
    struct dust_particle * end; // r29
    float hsize; // f26
...
// Range: 0x801B8A9C -> 0x801B8DA0
static int update_dust_particles_uv2(unsigned char * mem /* r28 */, int count /* r0 */, struct ptank_pool & apool /* r5 */, float dt /* f25 */) {
    // Local variables
...
// Range: 0x801B8DA0 -> 0x801B8DDC
static void move_ring_chain(struct xVec2 * chain /* r5 */, struct xVec2 * end_chain /* r0 */, float x /* f1 */, float z /* f2 */, float dcos /* f0 */, float dsin /* f0 */) {}
...
// Range: 0x801B8DDC -> 0x801B8F10
static void refresh_ring_chains() {
    // Local variables
    float dt; // f29
    int i; // r28

    // References
    // -> static struct xVec2 ring_chain[2][8];
    // -> static float ring_chain_ang[2];
    // -> static struct ring_chain_data chain[2];
    // -> struct zGlobals globals;
    // -> static unsigned char refreshed_ring_chains;
}

// Range: 0x801B8F10 -> 0x801B8FAC
static void reset_ring_chains() {
    // Local variables
    int i; // r29
...
// Range: 0x801B8FAC -> 0x801B9084
static void get_ring_dir(float & dx /* r28 */, float & dz /* r29 */, const struct ring_chain_data & data /* r30 */, const struct xVec2 * chain /* r31 */) {
    // Local variables
    float rand_ang; // f31
    int seg; // r0
}
...
struct activity_data {
    // Members
    class zDustRingEmitter * owner; // offset 0x0, size 0x4
    float emitted[2]; // offset 0x4, size 0x8
    float intensity_min; // offset 0xC, size 0x4
    float intensity_delta; // offset 0x10, size 0x4
    unsigned char alpha; // offset 0x14, size 0x1
    unsigned char pad1; // offset 0x15, size 0x1
    unsigned char pad2; // offset 0x16, size 0x1
    unsigned char pad3; // offset 0x17, size 0x1
};
// total size: 0x2C
struct debug_data {
    // Members
    struct config_type cfg; // offset 0x0, size 0x14
    unsigned char track_player; // offset 0x14, size 0x1
    struct xVec3 center; // offset 0x18, size 0xC
    float force; // offset 0x24, size 0x4
    float yoffset; // offset 0x28, size 0x4
};
// total size: 0xC
class zDustRingEmitter {
    ...
    void kickup(const struct xVec3 & center, float force, float dt);

    void refresh_activity();

    void debug_init(const char * tweak_prefix);

    void debug_update(float dt);

    void add_tweaks(const char * tweak_prefix);

    // Static members
    static int activities_used; // size: 0x4
    static struct activity_data * activities; // size: 0x4

    // Members
    const struct config_type * cfg; // offset 0x0, size 0x4
    struct activity_data * data; // offset 0x4, size 0x4
    struct debug_data * debug; // offset 0x8, size 0x4
};
static class zDustRingEmitter dummy_emitter; // size: 0xC, address: 0x80C089E8
// Range: 0x801B9084 -> 0x801B90CC
void zDustSceneEnter() {
    // References
    // -> static class zDustRingEmitter dummy_emitter;
}

// Range: 0x801B90CC -> 0x801B90F0
void zDustReset() {}

// Range: 0x801B90F0 -> 0x801B9110
void zDustUpdate(float dt /* f0 */) {}

// Range: 0x801B9110 -> 0x801B911C
void zDustRender() {
    // References
    // -> static unsigned char refreshed_ring_chains;
}

int activities_used; // size: 0x4, address: 0x80C089F4
struct activity_data * activities; // size: 0x4, address: 0x80C089F8
// Range: 0x801B911C -> 0x801B912C
// this: r0
void zDustRingEmitter::create() {}

// Range: 0x801B912C -> 0x801B914C
void zDustRingEmitter::load_default_config(struct config_type & cfg /* r0 */) {}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80C02B14
// Range: 0x801B914C -> 0x801B9220
// this: r31
unsigned char zDustRingEmitter::activate() {
    // References
    // -> int activities_used;
    // -> static char __FUNCTION__[9];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80C02B20
// Range: 0x801B9220 -> 0x801B92B8
// this: r31
...
static char __FUNCTION__[7]; // size: 0x7, address: 0x80C02B2C
// Range: 0x801B92B8 -> 0x801B9900
// this: r20
void zDustRingEmitter::kickup(const struct xVec3 & center /* r21 */, float force /* r1+0x8 */, float dt /* r1+0xC */) {
    // Local variables
    float cx; // r1+0x70
    float cy; // r1+0x6C
    float cz; // r1+0x68
    float radius_min; // f22
    float radius_delta; // r1+0x64
    float size_min; // f21
    float size_delta; // r1+0x60
    float vmag_min; // f20
    float vmag_delta; // f19
    float life_min; // f0
    float life_delta; // f18
    float rot_vel_min; // f0
    float rot_vel_delta; // f17
    float growth; // f16
    float intensity_min; // f15
    float intensity_delta; // f14
    unsigned char alpha; // r26
    int dirt_frames; // r25
    int i; // r24
    int emit; // r23
    unsigned int rval; // r0
    unsigned char * mem; // r1+0x18
    int have; // r0
    struct dust_particle * p; // r22
    struct dust_particle * endp; // r21
    float dx; // r1+0x14
    float dz; // r1+0x10
    float rr; // f2
    float rand_radius; // f2
    float vmag; // f1
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80C02B34
// Range: 0x801B9900 -> 0x801B9A08
// this: r31
void zDustRingEmitter::refresh_activity() {
    // Local variables
    float intensity_min; // f3
    float intensity_max; // f0
...
// Range: 0x801B9A08 -> 0x801B9A44
void zDustRingEmitter::scene_enter() {
    // References
    // -> int activities_used;
    // -> struct activity_data * activities;
}

// Range: 0x801B9A44 -> 0x801B9A6C
void zDustRingEmitter::reset_all() {
    // Local variables
    struct activity_data * act; // r4
    struct activity_data * const end_act; // r0
...
// Range: 0x801B9A6C -> 0x801B9AF0
void zDustRingEmitter::update_all(float dt /* f31 */) {
    // Local variables
    struct activity_data * activity; // r31
    struct activity_data * end_activity; // r30
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80C02B48
// Range: 0x801B9AF0 -> 0x801B9B78
struct activity_data * zDustRingEmitter::start_activity() {
    // Local variables
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80C02B58
// Range: 0x801B9B78 -> 0x801B9C44
void zDustRingEmitter::stop_activity(struct activity_data * activity /* r31 */) {
    // Local variables
    struct activity_data * tail_activity; // r4
...
// Range: 0x801B9C90 -> 0x801B9D18
// this: r31
void zDustRingEmitter::debug_init(const char * tweak_prefix /* r0 */) {}

// Range: 0x801B9D18 -> 0x801B9DE4
// this: r31
void zDustRingEmitter::debug_update(float dt /* f31 */) {
    // Local variables
...
// Range: 0x801B9DE4 -> 0x801B9F1C
void zDustRingEmitter::add_config_tweaks(const char * prefix /* r28 */, struct config_type & cfg /* r29 */, const struct tweak_callback * cb /* r30 */, void * context /* r31 */) {}

static struct tweak_callback cb_check_active; // size: 0x28, address: 0x803E8488
static struct tweak_callback cb_refresh_config; // size: 0x28, address: 0x803E84B0
// Range: 0x801B9F1C -> 0x801BA094
// this: r30
void zDustRingEmitter::add_tweaks(const char * tweak_prefix /* r29 */) {
    // Local variables
    char buffer[128]; // r1+0x10
    const char * prefix; // r31

    // References
    // -> static struct tweak_callback cb_check_active;
    // -> static struct tweak_callback cb_refresh_config;
}

// Range: 0x801BA094 -> 0x801BA0D8
void zDustRingEmitter::check_active(const struct tweak_info & info /* r0 */) {
    // Local variables
    class zDustRingEmitter & owner; // r3
}

// Range: 0x801BA0D8 -> 0x801BA118
void zDustRingEmitter::on_refresh_config(const struct tweak_info & info /* r0 */) {
    // Local variables
    class zDustRingEmitter & owner; // r31
}
