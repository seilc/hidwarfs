/*
    Compile unit: C:\branches\RT_First_Playable\RT\Engine\Core\gc\iDebug.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800D7044 -> 0x800D783C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct RwRaster * mCharset; // size: 0x4, address: 0x80C078C0
// total size: 0x4
struct RwRGBA {
    // Members
...
static struct RwRGBA mForegroundColor; // size: 0x4, address: 0x80BFEEA0
static struct RwRGBA mBackgroundColor; // size: 0x4, address: 0x80BFEEA4
// total size: 0x2C8
struct OSContext {
    // Members
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80BFEEA8
void (* old_dsc)(unsigned short); // size: 0x4, address: 0x80C07AA0
// Range: 0x800D7044 -> 0x800D70F0
void iDebugInit() {
    // References
...
// Range: 0x800D70F0 -> 0x800D70F4
void iDebugUpdate() {}

// Range: 0x800D70F4 -> 0x800D70F8
void iDebugWriteProfile() {}

// Range: 0x800D70F8 -> 0x800D7118
void iDebugVSync() {}

// Range: 0x800D7118 -> 0x800D711C
void iDebugStackTrace() {}

// Range: 0x800D711C -> 0x800D7140
void iDebugIDEOutput(char * buf /* r0 */) {}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80BFEEB4
// Range: 0x800D7140 -> 0x800D7288
static void SWAP16(int n /* r30 */, void * vaddr /* r31 */) {
    // Local variables
    unsigned char * addr; // r4
    unsigned char tmp; // r5

    // References
    // -> static char __FUNCTION__[7];
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80BFEEBC
// Range: 0x800D7288 -> 0x800D73E0
static void SWAP32(int n /* r30 */, void * vaddr /* r31 */) {
    // Local variables
    unsigned char * addr; // r5
    unsigned char tmp; // r4

    // References
    // -> static char __FUNCTION__[7];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80BFEEC4
...
// Range: 0x800D73E0 -> 0x800D7748
void iDebugScreenDump() {
    // Local variables
    struct tag_xFile file; // r1+0x84
    class zUIImage * picImage; // r0
    unsigned int size; // r24
    unsigned int n; // r0
    unsigned int x; // r24
    unsigned int y; // r31
    char fname[64]; // r1+0x44
    unsigned char * bp; // r30
    unsigned char * buf; // r29
    long long t0; // r28
    long long t1; // r24
    struct BmpMegaHeader mhdr; // r1+0xC
    unsigned int bufsize; // r26
    unsigned long pixval; // r1+0x8
...
// Range: 0x800D7748 -> 0x800D783C
void iDebugDumpEFB(char * name /* r29 */) {
    // Local variables
    char filename[32]; // r1+0xC
    struct tag_xFile file; // r1+0x2C
    unsigned char buf[4096]; // r1+0x140
    unsigned short x; // r31
    unsigned short y; // r30
    unsigned char * bp; // r29
    unsigned long pixval; // r1+0x8
}
