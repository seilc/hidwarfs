/*
    ...
    Language: C++
    Code range: 0x80088E48 -> 0x8008A938
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static void * sSndStaticBuffer; // size: 0x4, address: 0x803C17B8
// total size: 0x0
...
struct ExternalStreamInfo externStreamInfo[2]; // size: 0x20, address: 0x803C17BC
// total size: 0x0
...
struct gcSoundInfoAsset sinfoArray[12]; // size: 0x180, address: 0x80350300
unsigned int sinfoArrayLoaded; // size: 0x4, address: 0x803C17DC
// total size: 0x8
struct xBaseAsset {
    // Members
...
static struct xSndVoiceInfo * pTempStreams[4]; // size: 0x10, address: 0x803C17E0
static unsigned char sOldPauseState[7]; // size: 0x7, address: 0x803C17F0
static unsigned char sReverbSetup; // size: 0x1, address: 0x803C17F7
// total size: 0x4
...
// total size: 0x1
struct /* @class$753iSnd_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$754iSnd_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$755iSnd_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$756iSnd_cpp */ {
    // Members
...
// Range: 0x80088E48 -> 0x80088F84
void iSndInit() {
    // Local variables
    struct xSndVoiceInfo * voice; // r5
    int i; // r0
...
// Range: 0x80089030 -> 0x800892D8
static unsigned char loadSoundInfoAsset(unsigned int uInfoArrayIndex /* r27 */, struct PKRAssetTOCInfo & rAssetInfo /* r25 */) {
    // Local variables
    unsigned char retval; // r30
    struct gcSoundInfoAsset & rSoundAsset; // r29
    const char * hipname; // r28
    struct tag_xFile hopFile; // r1+0xC
    int uSoundInfoOffsetBytes; // r31
    unsigned int dataBufferSize; // r4
    unsigned char * dataBuffer; // r25
    int i; // r5
    int i; // r5
    int uStaticBufferLength; // r5
...
// Range: 0x800892D8 -> 0x80089340
static unsigned char unloadSoundInfoAsset(unsigned int uInfoArrayIndex /* r0 */) {
    // Local variables
...
// Range: 0x800893A4 -> 0x800893C4
void iSndExit() {}

// Range: 0x800893C4 -> 0x80089444
struct iSndFileInfo * iSndLookup(unsigned int id /* r0 */) {
    // Local variables
    struct gcSoundInfoAsset * pSoundBankInfo; // r7
    int i; // r0
    int lowerIndex; // r8
    int upperIndex; // r9
    int thisIndex; // r10
...
// Range: 0x80089444 -> 0x800894B0
static struct sDSPADPCM * iSndFindCutsceneHeader(unsigned int id /* r0 */) {
    // Local variables
    struct gcSoundInfoAsset * pSoundBankInfo; // r8
    int i; // r9
    struct sDSPADPCM * pHeader; // r10
    int j; // r0
...
// Range: 0x800894B0 -> 0x80089504
unsigned char initBytesToConsume(struct FSOUND_STREAM * stream /* r0 */) {
    // Local variables
    int i; // r0
...
// Range: 0x80089504 -> 0x80089558
unsigned char releaseBytesToConsume(struct FSOUND_STREAM * stream /* r0 */) {
    // Local variables
    int i; // r0
...
// Range: 0x80089558 -> 0x80089594
int getBytesToConsume(struct FSOUND_STREAM * stream /* r0 */) {
    // Local variables
    int i; // r0
...
// Range: 0x80089594 -> 0x800895D0
int getFrontBuffer(struct FSOUND_STREAM * stream /* r0 */) {
    // Local variables
    int i; // r0
...
// Range: 0x800895D0 -> 0x8008960C
void * getDataPtr(struct FSOUND_STREAM * stream /* r0 */) {
    // Local variables
    int i; // r0
...
// Range: 0x8008960C -> 0x8008964C
unsigned char setBytesToConsume(struct FSOUND_STREAM * stream /* r0 */, int bytesToConsume /* r0 */) {
    // Local variables
    int i; // r0
...
// Range: 0x8008964C -> 0x8008968C
unsigned char setFrontBuffer(struct FSOUND_STREAM * stream /* r0 */, int frontBuffer /* r0 */) {
    // Local variables
    int i; // r0
...
// Range: 0x8008968C -> 0x800896CC
unsigned char setDataPtr(struct FSOUND_STREAM * stream /* r0 */, void * pData /* r0 */) {
    // Local variables
    int i; // r0
...
// Range: 0x800896CC -> 0x800897EC
signed char iSndExternalCB(struct FSOUND_STREAM * stream /* r25 */, void * buff /* r26 */, int len /* r27 */, void * userdata /* r28 */) {
    // Local variables
    int bytesToConsume; // r30
    int frontbuffer; // r0
    void * pData; // r29
    unsigned int datasize; // r1+0x8
}

// Range: 0x800897EC -> 0x800898D8
static void iSndCloseStream(int idx /* r0 */, unsigned char bFreeRef /* r28 */) {
    // Local variables
    int nOpenState; // r0
    int nIndex; // r0
...
// Range: 0x800898D8 -> 0x80089CF4
unsigned char iSndPlay(struct xSndVoiceInfo * vp /* r27 */) {
    // Local variables
    struct gcSoundInfoAsset * pSoundBankInfo; // r31
    struct iSndFileInfo * pSndInfo; // r30
    struct FSOUND_SAMPLE * pSample; // r0
    unsigned int uOpenMode; // r29
    struct sDSPADPCM * pHeader; // r0
    struct FSOUND_GCADPCMINFO * info; // r28
    int nChannel; // r0
    struct xSndVoiceInfo * pVoice; // r28
    int nOffsetToStartStream; // r30
    int nStreamSize; // r26
    const char * hipname; // r0
    unsigned int nIndex; // r5
    unsigned int samplemode; // r0
    int nChannel; // r0
    int nRawChanZero; // r26
    int savePriority; // r30
...
// Range: 0x80089CF4 -> 0x80089EC0
static unsigned char iSndPlayDone(struct xSndVoiceInfo * vp /* r29 */) {
    // Local variables
    int nRawChanZero; // r0
...
// Range: 0x80089EC0 -> 0x80089EC4
void iSndStartStereo() {}

// Range: 0x80089EC4 -> 0x80089F30
void iSndSetMode() {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x80089F30 -> 0x80089F74
void iSndSetEnvironmentalEffect(int effectType /* r0 */) {}

// Range: 0x80089F74 -> 0x8008A04C
unsigned char iSndGetFreeStreamInfo(unsigned int priority /* r0 */) {
    // Local variables
    int numStreams; // r6
    int i; // r0
    unsigned int lo_priority; // r7
    unsigned int lo_voice; // r8
    unsigned int i; // r9
...
// Range: 0x8008A04C -> 0x8008A1DC
struct xSndVoiceInfo * iSndGetFreeVoiceInfo(unsigned int priority /* r27 */, int flags /* r0 */) {
    // Local variables
    unsigned int lo_priority; // r6
    unsigned int lo_voice; // r7
    int voiceIsUnused; // r8
    unsigned int i; // r9
    unsigned int loopMode; // r0
...
// Range: 0x8008A1DC -> 0x8008A5B0
void iSndUpdate() {
    // Local variables
    int i; // r29
    int nOpenState; // r22
    unsigned char bCloseStream; // r24
    int nChannel; // r0
    int nRawChanZero; // r25
    unsigned char bDialogActive; // r28
    int i; // r27
    signed char playing; // r0
    signed char paused; // r0
    unsigned char is3d; // r0
    unsigned int loopMode; // r0
    struct xVec3 vPosInvX; // r1+0x18
    int nSampleFreq; // r1+0x8
    struct FSOUND_SAMPLE * pSample; // r0
    float iShiftedFreq; // f0
    struct xVec3 vListenerPosInvX; // r1+0xC
...
// Range: 0x8008A5B0 -> 0x8008A5B8
unsigned char iSndIsReady() {}

// Range: 0x8008A5B8 -> 0x8008A6B4
void iSndStop(struct xSndVoiceInfo * vp /* r30 */, unsigned char bFreeRef /* r31 */) {
    // Local variables
    unsigned int loopMode; // r0
    int nRawChanZero; // r0
...
// Range: 0x8008A6B4 -> 0x8008A6E8
void iSndPause(struct xSndVoiceInfo * vp /* r3 */, unsigned int paused /* r0 */) {}

// Range: 0x8008A6E8 -> 0x8008A774
void iSndSetVol(struct xSndVoiceInfo * vp /* r30 */, float xVolume /* f31 */) {
    // Local variables
    float fVolume; // f1
    unsigned char iVolume; // r0
}

// Range: 0x8008A774 -> 0x8008A7F4
void iSndSetPitch(struct xSndVoiceInfo * pVoice /* r31 */, float xpitch /* f31 */) {
    // Local variables
    int nSampleFreq; // r1+0x8
    struct FSOUND_SAMPLE * pSample; // r0
    float fShiftedFreq; // f0
}

// Range: 0x8008A7F4 -> 0x8008A868
void iSndKill() {
    // Local variables
    unsigned char oneOpen; // r4
    unsigned int i; // r0
...
// Range: 0x8008A868 -> 0x8008A86C
void iSndSetExternalCallback() {}

// Range: 0x8008A86C -> 0x8008A870
void iSndSuspendCD() {}

// Range: 0x8008A870 -> 0x8008A8D8
void iSndSuspend() {
    // Local variables
    signed short i; // r30
