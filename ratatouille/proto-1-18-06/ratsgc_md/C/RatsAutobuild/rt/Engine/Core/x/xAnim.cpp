/*
    ...
    Language: C++
    Code range: 0x80005840 -> 0x80009F48
*/
// total size: 0x1C
struct xMemPool {
    // Members
...
// Range: 0x80006078 -> 0x800060A0
void xAnimTempTransitionFree(struct xAnimTransition * tran /* r4 */) {
    // References
    // -> static struct xMemPool sxAnimTempTranPool;
}

// Range: 0x800060A0 -> 0x800060FC
...
// Range: 0x800062A8 -> 0x80006354
static void _xAnimTransitionListInsert(struct xAnimTransitionList * * list /* r0 */, struct xAnimTransitionList * toInsert /* r0 */) {
    // Local variables
    struct xAnimTransitionList * curr; // r8
}

// Range: 0x80006354 -> 0x80006404
...
// Range: 0x8000656C -> 0x80006A48
void xAnimCalcPhysicsData(struct xAnimFile * afile /* r31 */, struct xAnimFile * endPose /* r28 */) {
    // Local variables
    int numAnims; // r27
...
// Range: 0x80006D04 -> 0x80007230
void xAnimFileEval(struct xAnimFile * data /* r22 */, float time /* f1 */, float * bilinear /* r26 */, unsigned int flags /* r23 */, struct xVec3 * tran /* r24 */, struct xQuat * quat /* r25 */) {
    // Local variables
    int i; // r28
    unsigned int numBones; // r27
    unsigned int fileflags; // r19
    float evalTime; // f31
...
// Range: 0x80007718 -> 0x80007B20
static void _xAnimTableAddTransition(struct xAnimTable * table /* r24 */, struct xAnimTransition * tran /* r31 */, const char * source /* r14 */, const char * dest /* r25 */) {
    // Local variables
    unsigned char * buffer; // r30
    struct xAnimState * * stateList; // r29
    struct xAnimState * state; // r3
    unsigned int i; // r18
    unsigned int stateCount; // r1+0xC
    unsigned int allocCount; // r1+0x8
    const char * stateName; // r20
    struct xAnimTransitionList * tlist; // r28
    struct xAnimTransition * substTransitionList[32]; // r1+0x110
    unsigned int substTransitionCount; // r27
    unsigned char hasSubst; // r26
    int i; // r0
    unsigned char isComplex; // r15
    const char * COMPLEX_PATTERNS; // r17
    const char * search; // r16
    struct xAnimState * state; // r19
    char extra[128]; // r1+0x90
    char tempName[128]; // r1+0x10
    char * tempIterator; // r18
    char * extraIterator; // r17
    unsigned char allowMissingState; // r16
    int i; // r15
...
// Range: 0x80007D8C -> 0x80007EA8
struct xAnimState * xAnimTableAddFileID(struct xAnimTable * table /* r28 */, struct xAnimFile * file /* r29 */, unsigned int stateID /* r5 */, unsigned int subStateID /* r30 */, unsigned int subStateCount /* r27 */) {
    // Local variables
    struct xAnimState * state; // r31
...
// Range: 0x80008598 -> 0x800086D8
void xAnimPhysicsEval(struct xAnimPhysicsData * phys /* r27 */, float time /* f1 */, struct xVec3 * disp /* r28 */, float * yaw /* r31 */) {
    // Local variables
    float interp; // f31
    int index; // r29
}

// Range: 0x800086D8 -> 0x80008700
...
struct _FILE __files[]; // size: 0x0, address: 0x802EAF88
// Range: 0x800089E8 -> 0x80009240
...
// Range: 0x8000932C -> 0x80009424
void xAnimPlaySetup(struct xAnimPlay * play /* r29 */, void * object /* r0 */, struct xAnimTable * table /* r30 */, struct xModelInstance * modelInst /* r0 */) {
    // Local variables
    int i; // r31
}

static unsigned char * tmpBuffer; // size: 0x4, address: 0x803C0C5C
static signed char init; // size: 0x1, address: 0x803C0C60
// Range: 0x80009424 -> 0x80009578
void xAnimPlayChooseTransition(struct xAnimPlay * play /* r28 */) {
    // Local variables
    unsigned int i; // r31
    void * object; // r30
    struct xAnimTransition * * found; // r29
    struct xAnimTransitionList * curr; // r26
...
void xAnimPlayStartTransition(struct xAnimPlay * play /* r29 */, struct xAnimTransition * transition /* r1+0x8 */) {
    ...
    struct xAnimSingle * bl; // r28
    unsigned char transferredState; // r27
    unsigned char tookOwnership; // r30
}

// Range: 0x800097B8 -> 0x80009848
...
// Range: 0x80009848 -> 0x80009AF8
void xAnimPlayEval(struct xAnimPlay * play /* r25 */) {
    // Local variables
    unsigned int i; // r31
    unsigned int bone; // r22
    struct xQuat * quatresult; // r30
    struct xVec3 * tranresult; // r29
    struct xQuat * quatblend; // r28
    struct xVec3 * tranblend; // r27
    struct xAnimSingle * si; // r26
    float blendF; // f31
    float blendR; // f30
    unsigned short * blendO; // r22

    // References
    // -> unsigned char * g_iAnim_workBuffer;
}

// Range: 0x80009AF8 -> 0x80009C00
void xAnimPoolCB(struct xMemPool * pool /* r0 */, void * data /* r0 */) {
    // Local variables
    int i; // r11
    struct xAnimPlay * clone; // r5
    struct xAnimPlay * play; // r0
    struct xAnimSingle * clonesingle; // r10
    struct xAnimSingle * currsingle; // r12
    struct xAnimActiveEffect * curract; // r11
}

// Range: 0x80009C00 -> 0x80009E18
void xAnimPoolInit(struct xMemPool * pool /* r26 */, unsigned int count /* r27 */, unsigned int singles /* r28 */, unsigned int blendFlags /* r29 */, unsigned int effectMax /* r30 */) {
    // Local variables
...
// Range: 0x80009E68 -> 0x80009F04
void xAnimPoolFree(struct xAnimPlay * play /* r29 */) {
    // Local variables
    unsigned int i; // r30
}

// Range: 0x80009F04 -> 0x80009F48
void xAnimSetMultifileFromCode(struct xAnimState * state /* r0 */, unsigned int animCode /* r0 */) {
    // Local variables
    unsigned int i; // r0
}
