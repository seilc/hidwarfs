/*
    ...
    Language: C++
    Code range: 0x8004AA28 -> 0x8004C48C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct xModelPool * sxModelPoolList; // size: 0x4, address: 0x803C10BC
...
static struct RwCamera * subcamera; // size: 0x4, address: 0x803C10C0
int xModelPipeNumTables; // size: 0x4, address: 0x803C10C4
int xModelPipeCount[16]; // size: 0x40, address: 0x803C10C8
// total size: 0x10
...
struct xModelPipeTable * xModelPipeData[16]; // size: 0x40, address: 0x803C1108
...
int xModelBucketEnabled; // size: 0x4, address: 0x803C1154
int xModelAlphaBucketEnabled; // size: 0x4, address: 0x803BD818
// total size: 0x8
struct xModelPipe {
    // Members
    unsigned int Flags; // offset 0x0, size 0x4
...
// Range: 0x8004AA28 -> 0x8004AA70
struct xModelPipe xModelGetPipe(struct RpAtomic * model /* r0 */) {
    // Local variables
    int i; // r0
...
// Range: 0x8004AAD8 -> 0x8004ABC8
void xModelPoolInit(unsigned int count /* r29 */, unsigned int numMatrices /* r31 */) {
    // Local variables
    int i; // r7
    unsigned char * buffer; // r3
    struct RwMatrixTag * mat; // r8
    struct xModelPool * pool; // r0
    struct xModelPool * curr; // r4
    struct xModelPool * * prev; // r6
    struct xModelInstance * inst; // r9
...
// Range: 0x8004ABC8 -> 0x8004AE7C
struct xModelInstance * xModelInstanceAllocReuse(struct RpAtomic * data /* r23 */, void * object /* r24 */, unsigned short flags /* r25 */, unsigned char boneIndex /* r26 */, unsigned char * boneRemap /* r27 */, struct xModelInstance * reuse_instance /* r28 */) {
    // Local variables
    int i; // r23
    unsigned int boneCount; // r0
    unsigned int matCount; // r29
    struct xModelPool * curr; // r3
    struct xModelPool * found; // r31
    struct xModelInstance * dude; // r28
    struct RwMatrixTag * allocmats; // r0
    unsigned char nosrcblend; // r4
    unsigned char nodestblend; // r5
...
// Range: 0x8004AE7C -> 0x8004AEAC
static struct xModelInstance * FindChild(struct xModelInstance * inst /* r0 */) {
    // Local variables
    struct xModelInstance * curr; // r4
}

// Range: 0x8004AEAC -> 0x8004AF64
void xModelInstanceFree(struct xModelInstance * modelInst /* r30 */, unsigned char animFree /* r31 */) {
    // Local variables
    struct xModelInstance * child; // r3
    struct xModelInstance * curr; // r4
    struct xModelInstance * * prev; // r3
}

// Range: 0x8004AF64 -> 0x8004AFB0
void xModelInstanceAttach(struct xModelInstance * inst /* r0 */, struct xModelInstance * parent /* r4 */) {
    // Local variables
    struct xModelInstance * curr; // r5
}

// Range: 0x8004AFB0 -> 0x8004B060
...
// Range: 0x8004B060 -> 0x8004B268
void xModelEvalSingle(struct xModelInstance * modelInst /* r28 */, struct xBox * combinedAnimBound /* r29 */) {
    // Local variables
    int i; // r26
    struct xModelInstance * dad; // r0
    unsigned char * remap; // r26
...
// Range: 0x8004B268 -> 0x8004B2B8
void xModelEval(struct xModelInstance * modelInst /* r30 */) {
    // Local variables
...
// Range: 0x8004B2B8 -> 0x8004B530
void xModelRenderSingle(struct xModelInstance * modelInst /* r30 */) {
    // Local variables
    unsigned char reset; // r31
    struct xMat3x3 tmpmat; // r1+0x10
    unsigned char alpha; // r4
    struct xAnimPlay * a; // r28
    unsigned short i; // r5
...
// Range: 0x8004B58C -> 0x8004B7B8
void xModelRender2D(const struct xModelInstance & model /* r26 */, const struct basic_rect & r /* r27 */, const struct xVec3 & from /* r28 */, const struct xVec3 & to /* r29 */) {
    // Local variables
    struct RwCamera * camera; // r31
    struct RwMatrixTag * cammat; // r30
    struct xMat4x3 objmat; // r1+0x120
    struct xMat4x3 shearmat; // r1+0xE0
    struct xMat4x3 temp1; // r1+0xA0
    struct xMat4x3 temp2; // r1+0x60
    const struct RwV2d * camvw; // r0
    float viewscale; // f3
    float shearX; // f7
    float shearY; // f1
...
// Range: 0x8004B7B8 -> 0x8004B87C
static struct RwCamera * CameraCreate(int width /* r28 */, int height /* r29 */, int zBuffer /* r30 */) {
    // Local variables
    struct RwCamera * camera; // r31
}

// Range: 0x8004B87C -> 0x8004B904
static void CameraDestroy(struct RwCamera * camera /* r30 */) {
    // Local variables
...
// Range: 0x8004B904 -> 0x8004B928
void xModel_SceneEnter(struct RpWorld * world /* r0 */) {
    // References
    // -> static struct RwCamera * subcamera;
}

// Range: 0x8004B928 -> 0x8004B950
void xModel_SceneExit(struct RpWorld * world /* r0 */) {
    // References
    // -> static struct RwCamera * subcamera;
}

// Range: 0x8004B950 -> 0x8004B9B8
...
// Range: 0x8004B9B8 -> 0x8004BA64
void xModelAnimCollRefresh(const struct xModelInstance & cm /* r29 */) {
    // Local variables
    unsigned int size; // r31
    struct xMat4x3 & mat; // r30
    struct xMat4x3 old_mat; // r1+0x10

    // References
    // -> struct xMat4x3 g_I3;
}

// Range: 0x8004BA64 -> 0x8004BAA8
void xModelGetBoneLocation(struct xVec3 & loc /* r0 */, const struct xModelInstance & model /* r0 */, unsigned long index /* r0 */) {
    // Local variables
    struct xMat4x3 & root_mat; // r0
    struct xMat4x3 & anim_mat; // r5
}

// Range: 0x8004BAA8 -> 0x8004BAE8
void xModelGetBoneMat(struct xMat4x3 & mat /* r0 */, const struct xModelInstance & model /* r0 */, unsigned long index /* r0 */) {
    // Local variables
    const struct xMat4x3 & root_mat; // r0
}

// Range: 0x8004BAE8 -> 0x8004BB94
void xModelGetBoneLocationScaled(struct xVec3 & loc /* r28 */, const struct xModelInstance & model /* r29 */, unsigned long index /* r30 */) {
    // Local variables
    struct xMat4x3 temp_mat; // r1+0x10
    const struct xMat4x3 * root_mat; // r31
}

// Range: 0x8004BB94 -> 0x8004BBD8
int xModelSphereCull(struct xSphere * sphere /* r3 */) {
    // References
    // -> void * RwEngineInstance;
}

// Range: 0x8004BBD8 -> 0x8004BCF8
int xModelSphereCullPlusShadow(struct xSphere * sphere /* r0 */, struct xVec3 * shadowVec /* r0 */, int * shadowOutside /* r0 */) {
    // Local variables
    struct RwSphere * worldsph; // r0
    const struct RwFrustumPlane * frustumPlane; // r6
    int numPlanes; // r7
    float nDot; // f0
    float nDot; // f2
    float sDot; // f0
...
// Range: 0x8004BCF8 -> 0x8004BDDC
int xModelBoxCull(struct xBox * box /* r0 */) {
    // Local variables
    const struct RwFrustumPlane * frustumPlane; // r3
    int numPlanes; // r0
...
// Range: 0x8004BDDC -> 0x8004BFE0
int xModelBoxCullPlusShadow(struct xBox * box /* r0 */, struct xVec3 * shadowVec /* r0 */, int * shadowOutside /* r0 */) {
    // Local variables
    const struct RwFrustumPlane * frustumPlane; // r6
    int numPlanes; // r3
...
// Range: 0x8004BFE0 -> 0x8004C02C
int xModelCullGroup(struct xModelInstance * minst /* r4 */) {}

// Range: 0x8004C02C -> 0x8004C0F8
int xModelCullGroupPlusShadow(struct xModelInstance * minst /* r6 */, float shadowDepth /* f0 */, int * shadowOutside /* r31 */) {
    // Local variables
    struct xVec3 shadVec; // r1+0x8
    struct xBox * bbox; // r3
}

// Range: 0x8004C0F8 -> 0x8004C164
int xModelCullSingle(struct xModelInstance * minst /* r30 */) {
    // Local variables
    int result; // r31
}

// Range: 0x8004C164 -> 0x8004C1C0
void xModelSetLightKit(struct xModelInstance * minst /* r3 */, struct xLightKit * lightKit /* r0 */, float transitionTime /* f0 */) {}
...
// Range: 0x8004C1C0 -> 0x8004C1FC
unsigned char xModelIsPreinstanced(const struct xModelInstance * model /* r0 */) {
    // Local variables
    struct RpGeometry * geom; // r3
    struct RpMorphTarget * mt; // r4
}

// Range: 0x8004C1FC -> 0x8004C308
unsigned char xModelGetPipe(unsigned int modelID /* r30 */, unsigned int atomicNumber /* r0 */, struct xModelPipe * * pipe /* r31 */) {
    // Local variables
    int j; // r23
    int k; // r22
    unsigned int subAtomicBits; // r3
...
// Range: 0x8004C308 -> 0x8004C390
static struct RpAtomic * find_first_atomic(struct xModelAssetInfo * minf /* r0 */) {
    // Local variables
    struct xModelAssetInst * inst; // r31
    struct xModelAssetInst * end_inst; // r30
    void * data; // r0
    struct RpAtomic * child_atomic; // r0
}

// Range: 0x8004C390 -> 0x8004C44C
struct RpAtomic * xModelFindAtomic(unsigned int id /* r28 */, unsigned int * actual_id /* r29 */) {
    // Local variables
...
// Range: 0x8004C44C -> 0x8004C48C
int xModelGetNumAtomics(struct RpAtomic & model /* r3 */) {
    // Local variables
    int numAtomics; // r31
    struct RpAtomic * myModel; // r3
}
