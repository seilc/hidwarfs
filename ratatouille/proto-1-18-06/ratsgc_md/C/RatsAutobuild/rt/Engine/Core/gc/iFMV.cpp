/*
    ...
    Language: C++
    Code range: 0x8007CF84 -> 0x8007E454
*/
// total size: 0x10
struct BINKRECT {
    // Members
...
struct BINK * hBinkCurrMovie; // size: 0x4, address: 0x803C1668
static unsigned char sARAMtmp; // size: 0x1, address: 0x803C166C
static unsigned int amt; // size: 0x4, address: 0x803C1670
void * RwEngineInstance; // size: 0x4, address: 0x803C4584
// Range: 0x8007CF84 -> 0x8007D038
...
// Range: 0x8007D038 -> 0x8007D0A0
static void BigMovieFree(void * pMemToFree /* r31 */) {
    // References
    // -> void * RwEngineInstance;
}

// Range: 0x8007D0A0 -> 0x8007D0A8
...
// Range: 0x8007D0A8 -> 0x8007D2D8
static struct BINK * iFMVOpen(const char * fullFile /* r30 */, unsigned int uBinkFlags /* r31 */) {
    // Local variables
    struct RADARAMCALLBACKS OurGCCallbacks; // r1+0x8
    struct BINK * hBinkMovie; // r28
    int numRetries; // r27
...
static struct MOVIE * movie_array; // size: 0x4, address: 0x803C1674
static int movie_asset_count; // size: 0x4, address: 0x803C1678
...
// Range: 0x8007D654 -> 0x8007D670
int NextPow2(int in /* r0 */) {
    // Local variables
    int ret; // r0
}

// Range: 0x8007D670 -> 0x8007D704
...
// Range: 0x8007D9F0 -> 0x8007E370
static unsigned int iFMVPlay(const char * filename /* r14 */, unsigned int buttons /* r28 */, unsigned int uSubtitlesAID /* r29 */, unsigned char skippable /* r30 */, unsigned char displayDemoBlink /* r31 */, int uAudioTrack /* r1+0x18 */) {
    // Local variables
    int j; // r17
    char fullFile[256]; // r1+0x38
    struct BINK * hBinkMovie; // r23
    long long delay; // r0
    long long now; // r0
    unsigned char bTrackFound; // r6
    int track; // r0
    struct RwRaster * pCamRaster; // r22
    int iBackBufWidth; // r24
    int iBackBufHeight; // r0
    int iSafeWidth; // r5
    int iSafeHeight; // r3
    struct RwRect MovieRect; // r1+0x28
    int iHorizPad; // r4
    int iVertPad; // r6
    struct RwCamera * cam; // r14
    struct RwRGBA bg; // r1+0x24
    long long fmvAbortDelay; // r14
    unsigned char aborted; // r21
    long long dwTimeNow; // r0
    void * xfbDisp; // r1+0x20
    void * xfbCopy; // r1+0x1C
    void * xfbDest; // r20
    unsigned char bUseYUVPath; // r19
    struct RwRaster * pBinkRaster; // r18
    struct RwGameCubeRasterExtension * pRasterExt; // r17
    int iBinkMovieHalfHeight; // r15
    unsigned char waiting; // r16
    int i; // r16
    struct _tagxPad * pad; // r15
    int oneIfSkip; // r0
    unsigned char * pBits; // r0
    float fTime; // f30
...
// Range: 0x8007E370 -> 0x8007E39C
void iFMVResetCallback() {}

// Range: 0x8007E39C -> 0x8007E454
void iFMVKill() {
    // Local variables
    int i; // r29
