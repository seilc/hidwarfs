/*
    ...
    Language: C++
    Code range: 0x80065808 -> 0x80069590
*/
// total size: 0x8
struct RwObject {
    // Members
...
struct RpLight * ShadowLight; // size: 0x4, address: 0x803C1408
// total size: 0x10
struct RwRGBAReal {
    // Members
...
struct RwRGBAReal ShadowLightColor; // size: 0x10, address: 0x803BD9D8
int SHADOW_CACHE_POLY_MAX; // size: 0x4, address: 0x803BD9E8
static float SHADOW_BOTH; // size: 0x4, address: 0x803C140C
// total size: 0x24
...
static struct RxObjSpace3DVertex * Im3DBuffer; // size: 0x4, address: 0x803C1410
static unsigned int Im3DBufferPos; // size: 0x4, address: 0x803C1414
...
struct RwCamera * ShadowCamera; // size: 0x4, address: 0x803C1418
struct RwRaster * ShadowCameraRaster; // size: 0x4, address: 0x803C141C
static struct RwRaster * ShadowRenderRaster; // size: 0x4, address: 0x803C1420
// total size: 0xC
struct xVec3 {
    // Static members
...
struct xVec3 gShadowObjectRadius; // size: 0xC, address: 0x803C1424
static unsigned int shadow_ent_count; // size: 0x4, address: 0x803C1430
// total size: 0x20
struct xLinkAsset {
    // Members
...
static int sShadowCollJSP; // size: 0x4, address: 0x803C1434
static struct RwRaster * gc_saveraster; // size: 0x4, address: 0x803C1438
// Range: 0x80065808 -> 0x80065870
void xShadowInit() {
    // Local variables
...
// Range: 0x800659B0 -> 0x800659D8
int xShadowCameraCreate() {}

// Range: 0x800659D8 -> 0x80065A38
...
// Range: 0x80065A38 -> 0x80065AA0
void xShadowCameraUpdate(void * model /* r27 */, void (* renderCB)(void *) /* r28 */, struct xVec3 * center /* r29 */, struct xVec3 * radius /* r30 */, int shadowMode /* r31 */) {
    // References
    // -> struct RwRaster * ShadowCameraRaster;
    // -> static struct RwRaster * ShadowRenderRaster;
    // -> struct RwCamera * ShadowCamera;
...
// Range: 0x80065B60 -> 0x80066354
void xShadowReceiveShadow(struct xEnt * ent /* r31 */, float shadowFactor /* f30 */, int shadowMode /* r23 */, struct RwMatrixTag * shadowMat /* r22 */, struct RwRaster * shadowRast /* r21 */) {
    // Local variables
    struct xModelInstance * m; // r27
    struct RwMatrixTag oldroot; // r1+0xD0
    struct RwCamera * shadowCamera; // r0
    struct RwMatrixTag * shadowMatrix; // r21
...
// Range: 0x80066354 -> 0x80066374
void xShadow_ListAdd(struct xEnt * ent /* r0 */) {}

// Range: 0x80066374 -> 0x800664E0
...
// Range: 0x80066568 -> 0x80066700
static struct RwCamera * ShadowCameraUpdate(struct RwCamera * shadowCamera /* r27 */, void * model /* r28 */, void (* renderCB)(void *) /* r29 */, int shadowMode /* r30 */) {
    // Local variables
...
int _RwGameCubeRasterExtOffset; // size: 0x4, address: 0x803C4628
enum VITVMode {
...
// Range: 0x80066B00 -> 0x80066D90
static void ShadowRenderCachePolys(struct _ProjectionParam * param /* r26 */, struct xShadowPoly * poly /* r27 */, int count /* r28 */) {
    // Local variables
    int i; // r29
...
// Range: 0x80066D90 -> 0x80066DB8
void GCSaveFrameBuffer() {
    // References
    // -> static struct RwRaster * gc_saveraster;
}

// Range: 0x80066DB8 -> 0x80066F14
...
int _rpCollisionGeometryDataOffset; // size: 0x4, address: 0x803C43E4
// Range: 0x8006747C -> 0x80067A28
static int shadowCacheLeafCB(int numTriangles /* r20 */, int triOffset /* r4 */, void * data /* r21 */) {
    // Local variables
    struct ShadowCBParam * cbparam; // r0
    struct xShadowCache * cache; // r29
    ...
    float nmag2; // f4
    float pdot; // f3
    struct xVec3 * vert0; // r6
    struct xVec3 * vert1; // r4
    float dotB; // f2
    struct xVec3 * vert0; // r9
    struct xVec3 * vert1; // r10
    struct xVec3 * vert2; // r9
    struct xVec3 aa; // r1+0x20
    struct xVec3 bb; // r1+0x14
    struct xVec3 trinorm; // r1+0x8
    float depthtest; // f12
    float dydx; // f0
    float dydz; // f1
    float depth0; // f2
...
static const struct xEnt * sEntSelf; // size: 0x4, address: 0x803C143C
// total size: 0xC
struct /* @class$804xShadow_cpp */ {
    // Members
...
// Range: 0x80067CD8 -> 0x80068018
void xShadowVertical_FillCache(struct xShadowCache * cache /* r29 */, struct xVec3 * pos /* r30 */, float r /* f30 */, float depth /* f31 */, float minNormY /* f29 */, unsigned char castOnJSP /* r25 */, unsigned char castOnEnts /* r31 */) {
    // Local variables
    int i; // r0
    int j; // r0
    struct xEnv * env; // r26
    struct RpIntersection isx; // r1+0x44
    struct ShadowCacheContext context; // r1+0x8
    int i; // r25
    struct xShadowPoly * polys; // r25
    float limit; // f29
    int i; // r26
    float sortRayDepth[5]; // r1+0x30
    float objMaxDepth; // f2
    struct xQCData qcd; // r1+0x10
    struct ShadowCBParam cbparam; // r1+0x60
...
// Range: 0x80068060 -> 0x80068354
void xShadowVertical_DrawCache(struct xShadowCache * cache /* r31 */, float shadowFactor /* f30 */, float fadeDist /* f31 */, int shadowMode /* r30 */, struct RwMatrixTag * shadowMat /* r29 */, struct RwRaster * shadowRast /* r6 */) {
    // Local variables
...
static int SHADOW_CACHE_MAX; // size: 0x4, address: 0x803BD9EC
// total size: 0x14
struct xShadowMgr {
    // Members
...
static struct xShadowMgr * sMgrList; // size: 0x4, address: 0x803C1440
static int sMgrCount; // size: 0x4, address: 0x803C1444
static int sMgrTotal; // size: 0x4, address: 0x803C1448
static struct xShadowCache * sCacheList; // size: 0x4, address: 0x803C144C
unsigned int gActiveHeap; // size: 0x4, address: 0x803C1074
// Range: 0x80068354 -> 0x80068418
...
// Range: 0x80068418 -> 0x800684C4
void xShadowManager_Add(struct xEnt * ent /* r0 */) {
    // Local variables
    int i; // r0
...
// Range: 0x8006874C -> 0x80068A80
static void xShadowModelRender(struct xModelInstance * minst /* r30 */) {
    // Local variables
    struct xVec3 radius; // r1+0x1C
    struct RwV3d scl; // r1+0x10
    struct xMat3x3 tmpmat; // r1+0x90
    int i; // r7
    struct RpMaterialList * matList; // r0
    struct RwTexture * oldtex[8]; // r1+0x28
    struct RwMatrixTag oldMatrix; // r1+0x48
    struct xSphere * center; // r0
    struct RwMatrixTag * shadowMatrix; // r0
...
unsigned char xClumpColl_FilterFlags; // size: 0x1, address: 0x803BD568
// Range: 0x80068D38 -> 0x80069438
void xShadowManager_Render() {
    ...
    unsigned char old_xClumpColl_FilterFlags; // r30
    struct xEnt * ent_ref; // r29
    int bestIndex; // r8
    int foundPriority; // r9
    struct xVec3 center; // r1+0x2C
    float radius; // r1+0xC
    struct xShadowMgr * mgr_best; // r27
    float dst_depth; // f2
...
// Range: 0x80069550 -> 0x80069590
static void xShadow_PickEntForNPC(struct xShadowMgr * mgr /* r0 */) {}
