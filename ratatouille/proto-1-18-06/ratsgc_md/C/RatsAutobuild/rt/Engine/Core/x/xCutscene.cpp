/*
    ...
    Language: C++
    Code range: 0x8001BB98 -> 0x8001DCA0
*/
// total size: 0x80
...
struct xCutscene sActiveCutscene; // size: 0x1A8, address: 0x802F4E90
static unsigned int sCutTocCount[16]; // size: 0x40, address: 0x803BD588
static struct xCutsceneInfo * sCutTocInfo[16]; // size: 0x40, address: 0x803BD5C8
static int sCutTocMax; // size: 0x4, address: 0x803C0D00
// total size: 0xC
struct xModelPool {
    // Members
...
static struct xModelInstance * sCutsceneFakeModel; // size: 0x4, address: 0x803C0D04
static struct xCutsceneModelHack * sCutsceneModelHackBuf; // size: 0x4, address: 0x803C0D08
// Range: 0x8001BB98 -> 0x8001BBF0
...
// Range: 0x8001BC34 -> 0x8001BDC8
void xCutscene_InitEnd() {
    // Local variables
    int i; // r31
    int maxModels; // r8
    int tocnum; // r9
    struct xCutsceneInfo * cnfo; // r10
...
// Range: 0x8001C088 -> 0x8001C0D0
int xCutscene_LoadStart(struct xCutscene * csn /* r31 */) {}

// Range: 0x8001C0D0 -> 0x8001C124
static float xCutsceneConvertBreak(float time /* f1 */, struct xCutsceneBreak * breaklist /* r0 */, unsigned int breakcount /* r0 */, int idx /* r0 */) {
    // Local variables
    unsigned int i; // r0
    float tdiff; // f4
}

static int sRepeatFrames; // size: 0x4, address: 0x803C0D0C
static signed char init; // size: 0x1, address: 0x803C0D10
static unsigned char sLastFrame; // size: 0x1, address: 0x803C0D11
static signed char init; // size: 0x1, address: 0x803C0D12
unsigned int gSoak; // size: 0x4, address: 0x803C1BD4
// Range: 0x8001C124 -> 0x8001C3C8
...
// Range: 0x8001C61C -> 0x8001CA8C
static void JDeltaEval(struct RpAtomic * model /* r25 */, void * deltaModel /* r26 */, void * deltaAnim /* r0 */, float time /* f0 */) {
    // Local variables
    ...
    float lerp; // f2
    float invlerp; // f3
    struct RwV3d * outverts; // r28
    int numRun; // r0
    struct JDeltaTarget * dtgt; // r27
    float scale; // f0
    signed short * svert; // r8
    int j; // r9
    int cmpval; // r10
    struct RwV3d * vert; // r22
    int j; // r21
    int cmpval; // r20
    float scale; // f4
    ...
    int cmpval; // r20
    float scale; // f0
    struct RwV3d * vert; // r10
    int j; // r8
    int cmpval; // r11
}

// Range: 0x8001CA8C -> 0x8001CAEC
...
static struct xVec3 shadVec; // size: 0xC, address: 0x803BD608
// total size: 0x18
...
struct xLightKitBlendInfo gLastLightKit; // size: 0xC, address: 0x803C0FF0
...
void xCutscene_Render(struct xCutscene * csn /* r15 */) {
    ...
    unsigned int visFlags; // r1+0x10
    unsigned int visIdx; // r25
    unsigned int fakeCount; // r24
    unsigned int tworoot; // r17
    unsigned int noshadow; // r1+0x1A94
    struct xCutsceneData * data; // r23
    struct xCutsceneData * mphdata; // r14
    struct RpAtomic * model; // r22
    struct RpAtomic * shadowModel; // r1+0x1A90
    struct RwMatrixTag animMat[101]; // r1+0x130
    struct xModelPipe * pipeCurr; // r1+0xC
    unsigned int idx; // r21
    unsigned int hacked_model; // r20
    int tableIdx; // r1+0x1A8C
    struct XCSNNosey * nosey; // r19
    float breakTime; // f31
    struct xBox combinedAnimBound; // r1+0x8C
    unsigned int tempSize; // r1+0x8
    ...
    unsigned int subIndex; // r0
    unsigned int frameMin; // r4
    unsigned int frameMax; // r5
    unsigned int frameIndex; // r8
    unsigned int shadowBits; // r18
    unsigned int atomicNumber; // r1+0x1A88
    struct zCutsceneHack & cutsceneHackTableData2; // r0
    unsigned int morphAnimIndex; // r5
    unsigned int morphModelIndex; // r7
    unsigned int numFrame; // r4
    unsigned int numRun; // r1+0x1A84
    struct xCutsceneMphFrame * mphFrame; // r5
    struct xCutsceneMphRun * mphRun; // r1+0x1A80
    struct xMorphTargetFile * mphFile; // r6
    signed short * v_array[4]; // r1+0x4C
    signed short weight[4]; // r1+0x14
    unsigned int skipsize; // r7
    struct xVec3 * csnTmpArray; // r14
    struct xVec3 * currtmp; // r17
    struct xVec3 * outv; // r1+0x1A7C
    unsigned int j; // r16
    unsigned int cmpval; // r1+0x1A78
    void * deltaAnim; // r0
    void * deltaModel; // r0
    int culled; // r14
    struct xBox worldbox; // r1+0x74
    int culled; // r14
    struct xBox worldbox; // r1+0x5C
    struct zCutsceneHack & cutsceneHackTableData2; // r0
    struct CutsceneShadowModel smod; // r1+0x40
    struct xShadowCache scache; // r1+0xA4
    struct xVec3 center; // r1+0x34
    struct xVec3 radius; // r1+0x28
    float fillCacheRadius; // f31
    struct xLightKitBlendInfo disabledLightKit; // r1+0x1C
...
// Range: 0x8001D838 -> 0x8001D844
struct xCutscene * xCutscene_CurrentCutscene() {
    // References
    // -> struct xCutscene sActiveCutscene;
}

static struct xCamCutscene * cutscene_cam; // size: 0x4, address: 0x803C0D14
// Range: 0x8001D844 -> 0x8001D89C
...
struct xCamCutscene : public xCam {
    ...
    // Members
    struct xCutscene * csn; // offset 0x130, size 0x4
    struct xMat4x3 original_mat; // offset 0x140, size 0x40
    struct xCam * previous_cam; // offset 0x180, size 0x4
};
// Range: 0x8001D89C -> 0x8001D8F0
// this: r31
static void xCamCutscene::create() {}

// Range: 0x8001D928 -> 0x8001D970
static void cutscene_started(struct xCutscene * csn /* r0 */) {
    // References
    // -> static struct xCamCutscene * cutscene_cam;
}

// Range: 0x8001D970 -> 0x8001D9A8
...
struct {} @unnamed@xCutscene_cpp@::xCamCutscene::__vtable; // size: 0x40, address: 0x803BD614
// Range: 0x8001D9A8 -> 0x8001D9F8
// this: r31
...
// Range: 0x8001D9F8 -> 0x8001DA30
// this: r31
static void xCamCutscene::stop() {}

// Range: 0x8001DA30 -> 0x8001DA50
// this: r3
static struct xCam * xCamCutscene::get_next() {}

// Range: 0x8001DA50 -> 0x8001DCA0
// this: r27
