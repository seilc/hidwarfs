/*
    ...
    Language: C++
    Code range: 0x8000F3B4 -> 0x80011A34
*/
...
// Range: 0x8000F3B4 -> 0x8000F43C
// this: r30
void xCamSupportPath::create(int max_size /* r4 */, int debug_max_size /* r31 */) {
    // References
    // -> unsigned int gActiveHeap;
}

// Range: 0x8000F43C -> 0x8000F4B0
// this: r0
...
// Range: 0x8000F5C0 -> 0x8000F750
// this: r31
void xCamSupportPath::prune(int start /* r4 */) {
    // Local variables
    int i0; // r30
    int i1; // r29
    int i2; // r28
...
static struct path_node prev_path[33]; // size: 0x18C, address: 0x802F4900
// Range: 0x8000F76C -> 0x8000F984
// this: r24
...
// Range: 0x8000F984 -> 0x8000FCB8
// this: r26
float xCamSupportPath::traverse(struct xVec3 & rloc /* r27 */, float velmag /* f24 */, float dt /* f25 */) {
    // Local variables
    int i; // r31
    int end; // r30
    const struct path_node & p0; // r29
    const struct path_node & p1; // r28
    float dx; // f28
    float dy; // f27
    float dz; // f26
    float segdist; // f0
...
// Range: 0x8000FD80 -> 0x800101AC
int xCamSupportPath::polar_subdivide(struct path_node * vert /* r25 */, int vert_size /* r26 */, const struct xCamCoordPolar & start /* r27 */, const struct xCamCoordPolar & end /* r28 */, float thresh /* f31 */) {
    // Local variables
    ...
    float ddist; // f18
    float total; // f15
    struct path_node * v; // r29
    struct path_node * endv; // r28
    float cp; // f14
    float ABx; // f3
    float ABy; // f4
    float ABz; // f1
...
// total size: 0x4
struct /* @class$872xCamSupport_cpp */ {
    // Members
...
union /* @class$746xCamSupport_cpp */ {
...
// Range: 0x800101AC -> 0x800103DC
void xCamSupportPath::compute_weights(float * w /* r31 */, int n /* r25 */, float s /* f26 */) {
    // Local variables
...
static struct ent_cache_entry ent_cache[256]; // size: 0x400, address: 0x802F4A8C
static int ent_cache_size; // size: 0x4, address: 0x803C0C88
// total size: 0x30
struct xMat3x3 {
    // Members
...
// total size: 0xC
struct /* @class$890xCamSupport_cpp */ {
    // Members
...
// Range: 0x80010E30 -> 0x800117D4
// this: r28
unsigned char xCamSupportLOSBar::collide_pivot() {
    // Local variables
    struct xCollis coll; // r1+0x1F0
    struct xCapsule to_cap; // r1+0x124
    float to_t; // f30
    float to_dist; // f29
    struct xVec3 to_tohit; // r1+0xC4
    struct xVec3 to_impact; // r1+0xB8
    float new_rail_yoffset; // f26
    struct xCapsule new_cap; // r1+0x108
    struct xVec3 N; // r1+0xAC
    float D; // f25
    struct xMat4x3 vcircle_mat; // r1+0x1B0
    float bar_mag; // f0
    float vrot; // f25
    float oldpitch; // f0
    float newpitch; // f25
    ...
    float xzdist; // f0
    float newy; // f1
    struct xCapsule from_cap; // r1+0xEC
    float from_t; // f28
    float from_dist; // f27
    struct xVec3 from_tohit; // r1+0xA0
    float start_s; // f25
    float end_s; // f26
    int tries; // r25
    float s; // f30
    struct xCapsule cap; // r1+0xD0
    struct xVec3 cap_offset; // r1+0x94
    struct xVec3 impact; // r1+0x88
    struct xVec3 N; // r1+0x7C
    float D; // f29
    float min_from_t; // f0
    struct xVec3 P; // r1+0x70
    struct xVec3 CP; // r1+0x64
    struct xVec3 uCP; // r1+0x58
    float Rc; // f25
    struct xMat4x3 circle_mat; // r1+0x170
    float max_rot; // f0
    struct xMat3x3 rot_mat; // r1+0x140
    struct xVec3 depen; // r1+0x4C
    struct xVec3 BP; // r1+0x40
    struct xVec3 rot_axis; // r1+0x34
    float R; // f0
    float R2; // f25
    struct xVec3 PV; // r1+0x28
    float det; // f1
    struct xVec3 V; // r1+0x1C
    struct xVec3 BV; // r1+0x10
    float offset_mag; // f0
...
// Range: 0x800117D4 -> 0x80011970
// this: r30
unsigned char xCamSupportLOSBar::collide_compress() {
    // Local variables
    struct xSweptSphere sws; // r1+0x90
    struct xVec3 start; // r1+0x30
    struct xVec3 end; // r1+0x24
