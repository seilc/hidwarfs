/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xScrFx.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80061F70 -> 0x8006494C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
unsigned char g_debugRenderSafeArea; // size: 0x1, address: 0x803C1370
// Range: 0x80061F70 -> 0x80061F9C
void xScrFxInit() {}

// Range: 0x80061F9C -> 0x80061FBC
void xScrFxReset() {}

// Range: 0x80061FBC -> 0x80061FF8
void xScrFxUpdate(float dt /* f31 */) {}

// Range: 0x80061FF8 -> 0x80062030
void xScrFxRender() {
    // References
...
static float mLetterboxO; // size: 0x4, address: 0x803C138C
static float mLetterboxTO; // size: 0x4, address: 0x803C1390
static float sLetterBoxSize; // size: 0x4, address: 0x803C1394
static unsigned char sLetterBoxAlpha; // size: 0x1, address: 0x803BD950
// Range: 0x800624E4 -> 0x800624F4
...
struct xGlare sGlare[10]; // size: 0x208, address: 0x80327328
// Range: 0x800629D4 -> 0x800629FC
...
struct xVec3 sFullScreenGlareDir; // size: 0xC, address: 0x803BD954
float sFullScreenGlareIntensity; // size: 0x4, address: 0x803BD960
// total size: 0x4
struct RwRGBA {
    // Members
...
struct RwRGBA sFullScreenGlareColor; // size: 0x4, address: 0x803BD964
int sFullScreenGlareEnabled; // size: 0x4, address: 0x803C1398
unsigned int sFullScreenGlareTextureID; // size: 0x4, address: 0x803BD968
struct RwTexture * sFullScreenGlareTexturePtr; // size: 0x4, address: 0x803C139C
...
static struct RxObjSpace3DVertex sStripVert[4]; // size: 0x90, address: 0x80327530
// Range: 0x80062D08 -> 0x800630AC
void xScrFXGlareRender(const struct xMat4x3 & mat /* r27 */) {
    // Local variables
    int i; // r29
    struct xGlare * g; // r28
...
static struct rwGameCube2DVertex v[4]; // size: 0x60, address: 0x803275C0
static unsigned short indices[4]; // size: 0x8, address: 0x803BD96C
// Range: 0x800630AC -> 0x8006319C
...
static struct xMat4x3 sCameraFXMatOld; // size: 0x40, address: 0x803C13A0
// Range: 0x8006319C -> 0x800631D8
void xCameraFXBegin(const struct xMat4x3 & mat /* r31 */) {
    // References
    // -> static struct xMat4x3 sCameraFXMatOld;
}

static unsigned char anycameraFXActive; // size: 0x1, address: 0x803C13E0
// total size: 0x3C
...
struct cameraFXTableEntry sCameraFXTable[3]; // size: 0x24, address: 0x803BD974
struct cameraFX sCameraFX[10]; // size: 0x2F8, address: 0x80327620
// Range: 0x800631D8 -> 0x80063230
...
// Range: 0x8006326C -> 0x800632B8
struct cameraFX * xCameraFXAlloc() {
    // Local variables
    int i; // r0
    struct cameraFX * f; // r3

    // References
    // -> struct cameraFX sCameraFX[10];
}

// Range: 0x800632B8 -> 0x800633EC
void xCameraFXZoomUpdate(struct cameraFX * f /* r3 */, float dt /* f0 */, struct xMat4x3 * m /* r5 */) {}

static float SHAKE_FOREVER; // size: 0x4, address: 0x803C13E4
// Range: 0x800633EC -> 0x8006342C
...
// Range: 0x8006342C -> 0x80063448
void xCameraFXShakeEnd(struct cameraFX * fx /* r0 */, float fadeOutTime /* f0 */) {}

// Range: 0x80063448 -> 0x80063570
...
// Range: 0x800637D4 -> 0x800638F0
void xCameraFXUpdate(struct xMat4x3 & mat /* r27 */, struct RwCamera * icam /* r28 */, float dt /* f31 */) {
    // Local variables
    int i; // r29
    struct cameraFX * f; // r3
...
// Range: 0x800638F0 -> 0x8006392C
void xCameraFXEnd(struct xMat4x3 & mat /* r0 */, struct RwCamera * icam /* r31 */) {
    // References
    // -> static struct xMat4x3 sCameraFXMatOld;
}
...
static struct xScrFxLensFlareSource * sFlareSource; // size: 0x4, address: 0x803C13E8
static int sNumFlareSources; // size: 0x4, address: 0x803C13EC
// total size: 0x10
...
static struct xScrFxLensFlareParams sDefaultLensFlareParams; // size: 0x10, address: 0x803BD998
static struct xScrFxLensFlareParams * sLensFlareParams; // size: 0x4, address: 0x803BD9A8
// total size: 0x30
...
static struct iScrFxLensFlare sFlare[128]; // size: 0x1800, address: 0x80327918
static int sNumFlares; // size: 0x4, address: 0x803C13F0
struct RwTexture * gxFlareTexture; // size: 0x4, address: 0x803C13F4
struct RwFrame * gxFlareFrame; // size: 0x4, address: 0x803C13F8
// Range: 0x8006392C -> 0x80063AEC
...
// Range: 0x80063B10 -> 0x80063D88
static void LensFlareSourceCB(void * obj /* r26 */, unsigned int type /* r25 */, void * data /* r24 */) {
    // Local variables
    struct xScrFxLensFlareSource * source; // r31
    unsigned int elementID; // r29
    int numElements; // r0
    unsigned int texID; // r6
    int i; // r7
    struct xScrFxLensFlareElement * element; // r5
    int i; // r30
    struct xScrFxLensFlareElement * element; // r28
    struct RwTexture * tex; // r27
    ...
    // -> static int sNumFlareSources;
    // -> static struct xScrFxLensFlareSource * sFlareSource;
}

// total size: 0x1
struct iAsyncLockRWSentry {};
// Range: 0x80063D88 -> 0x80063F2C
void xScrFxLensFlare_SceneEnter() {
    ...
    // -> unsigned int gActiveHeap;
    // -> static struct xScrFxLensFlareSource * sFlareSource;
    // -> static struct xScrFxLensFlareParams * sLensFlareParams;
    // -> static struct xScrFxLensFlareParams sDefaultLensFlareParams;
...
// Range: 0x80063F2C -> 0x80063F68
void xScrFxLensFlare_SceneExit() {
    // References
    // -> struct RwFrame * gxFlareFrame;
}

// Range: 0x80063F68 -> 0x80064004
static void xScrFxLensFlare_Add(const struct xVec3 * pos /* r3 */, const struct xVec3 * at /* r29 */, float intensity /* f31 */, struct xScrFxLensFlareSource * source /* r30 */) {
    // Local variables
    struct iScrFxLensFlare * flare; // r31

    // References
    // -> static int sNumFlares;
    // -> static struct iScrFxLensFlare sFlare[128];
}

// Range: 0x80064004 -> 0x80064038
static void xScrFxLensFlare_Add(struct xScrFxLensFlareSource * source /* r5 */) {}

// Range: 0x80064038 -> 0x80064090
...
// Range: 0x80064090 -> 0x80064644
static void xScrFxLensFlare_RenderElements(struct RwFrame * frame /* r28 */, unsigned char vCol /* r31 */, void * flr /* r29 */) {
    // Local variables
    struct RxObjSpace3DVertex verts[4]; // r1+0x120
    unsigned short indices[4]; // r1+0x8
    struct RxObjSpace3DVertex qverts[9]; // r1+0x1B0
    unsigned short qindices[10]; // r1+0x4C
    struct RwCamera * camera; // r27
    const struct RwV2d * vw; // r0
    struct RwMatrixTag invMtx; // r1+0xE0
    struct RwMatrixTag transform; // r1+0xA0
    struct RwMatrixTag tmp; // r1+0x60
    struct RwMatrixTag * matrix; // r30
    struct RwV3d v; // r1+0x40
    float length; // f0
    struct RwV3d position; // r1+0x34
    struct RwV3d FlarePosition; // r1+0x28
    struct xScrFxLensFlareElement * ep; // r6
    struct iScrFxLensFlare * flare; // r0
    struct xScrFxLensFlareSource * source; // r29
    int i; // r28
    unsigned char r; // r9
    unsigned char g; // r8
    unsigned char b; // r7
    struct RwV3d flareSize; // r1+0x1C
    float uadj; // f5
    float vadj; // f6
    unsigned char r; // r8
    unsigned char g; // r7
    unsigned char b; // r9
    int j; // r0
    struct RwV3d flareSize; // r1+0x10
...
// Range: 0x80064644 -> 0x80064648
static void dumpFlare() {}

int dumpFlares; // size: 0x4, address: 0x803C13FC
struct RwCamera * globalCamera; // size: 0x4, address: 0x803C2E70
// Range: 0x80064648 -> 0x8006494C
void xScrFxLensFlare_Render(struct RwCamera * cam /* r27 */) {
    // Local variables
    struct iScrFxLensFlare * flare; // r26
    struct RwMatrixTag * cammat; // r0
    struct xVec3 * campos; // r30
    struct xVec3 * cam_at; // r29
    int i; // r28
    int i; // r31
    int i; // r31
    int i; // r28
