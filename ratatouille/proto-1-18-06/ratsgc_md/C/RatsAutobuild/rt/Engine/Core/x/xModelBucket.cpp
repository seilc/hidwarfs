/*
    ...
    Language: C++
    Code range: 0x8004C5E4 -> 0x8004DDE0
*/
// total size: 0x8
struct RwObject {
    // Members
...
static struct RpWorld * sBucketDummyWorld; // size: 0x4, address: 0x803C1158
// total size: 0x14
...
static struct RwCamera * sBucketDummyCamera; // size: 0x4, address: 0x803C115C
static int sBucketCurr; // size: 0x4, address: 0x803C1160
static int sBucketCount; // size: 0x4, address: 0x803C1164
// total size: 0x10
struct RwSphere {
    // Members
...
// total size: 0x4
struct /* @class$785xModelBucket_cpp */ {
    // Members
...
static struct xModelBucket * sBucketList; // size: 0x4, address: 0x803C1168
static int sBucketClipCullCurr; // size: 0x4, address: 0x803C116C
static int sBucketClipCullCount; // size: 0x4, address: 0x803C1170
static struct xModelBucket * * sBucketClipCullList; // size: 0x4, address: 0x803C1174
static int sAlphaCurr; // size: 0x4, address: 0x803C1178
static int sAlphaStart; // size: 0x4, address: 0x803C117C
static int sAlphaCount; // size: 0x4, address: 0x803C1180
// total size: 0x14
...
static struct xModelAlphaBucket * sAlphaList; // size: 0x4, address: 0x803C1184
static int sBucketOpaqueRenderStart; // size: 0x4, address: 0x803C1188
// Range: 0x8004C5E4 -> 0x8004C66C
static int compare_opaque_buckets(void * e1 /* r0 */, void * e2 /* r0 */) {
    // Local variables
    const struct xModelBucket & b1; // r0
    const struct xModelBucket & b2; // r0
    int use_layer1; // r0
    int use_layer2; // r5
}

// Range: 0x8004C66C -> 0x8004C69C
...
// Range: 0x8004C69C -> 0x8004C778
void xModelBucket_PreCountBucket(struct RpAtomic * data /* r28 */, struct xModelPipe & pipe /* r0 */, unsigned int subObjects /* r29 */) {
    // Local variables
    int numbuckets; // r31
    int modelcount; // r30
...
// Range: 0x8004C778 -> 0x8004C83C
void xModelBucket_PreCountAlloc(int maxAlphaModels /* r0 */) {
    // Local variables
...
// Range: 0x8004C83C -> 0x8004C844
static struct RpAtomic * GimmeAtomicCallback(struct RpAtomic * atomic /* r0 */, void * tmp /* r0 */) {}
...
// total size: 0x0
struct RpSkin {};
// Range: 0x8004C844 -> 0x8004CAA4
void FullAtomicDupe(struct RpAtomic * atomic /* r25 */, int count /* r29 */, struct RpAtomic * * output /* r30 */) {
    // Local variables
    int i; // r31
    struct RwStream * stream; // r26
    ...
    struct RpClump * tempClump; // r25
    struct RwMemory rwmemB; // r1+0xC
    struct RpClump * outClump; // r0
    struct RpHAnimHierarchy * pHier; // r25
...
// Range: 0x8004CACC -> 0x8004CB9C
void xModelBucket_SetMaterialFX(struct RpAtomic * model /* r26 */, struct xModelPipe & pipe /* r27 */) {
    // Local variables
    struct RpGeometry * geom; // r30
    unsigned char skinned; // r0
    int numMaterials; // r29
    int i; // r28
    struct RpMaterial * material; // r3
}

// Range: 0x8004CB9C -> 0x8004CE1C
void xModelBucket_InsertBucket(struct RpAtomic * data /* r26 */, struct xModelPipe & pipe /* r27 */, unsigned int subObjects /* r28 */) {
    // Local variables
    int numbuckets; // r29
...
// Range: 0x8004CE1C -> 0x8004CEEC
void xModelBucket_Init() {
    // Local variables
    int i; // r6
    struct xModelBucket * bucket; // r7
    struct xModelBucket * * backref; // r8
...
// Range: 0x8004CEEC -> 0x8004CF34
struct xModelBucket * * xModelBucket_GetBuckets(struct RpAtomic * data /* r0 */) {
    // Local variables
    int i; // r0
...
// total size: 0xC
struct /* @class$805xModelBucket_cpp */ {
    // Members
...
union /* @class$806xModelBucket_cpp */ {
...
// Range: 0x8004CF70 -> 0x8004D22C
void xModelBucket_Add(struct xModelInstance * minst /* r30 */) {
    // Local variables
    struct RpAtomic * model; // r31
    struct xMat4x3 * cammat; // r3
    float camdist2; // f1
    struct xModelBucket * bucket; // r31
    float camdot; // f30
    float alphaFade; // f1
    unsigned char alphaFlags; // r5
    unsigned char alphaLOD; // r6
    unsigned char alphaModel; // r7
...
// Range: 0x8004D22C -> 0x8004D4A4
void xModelBucket_AddAlphaFade(struct xModelInstance * minst /* r31 */, float alphaFade /* f31 */) {
    // Local variables
    struct RpAtomic * model; // r30
    struct xMat4x3 * cammat; // r3
    struct xModelBucket * bucket; // r5
    float camdot; // f3
    unsigned char alphaFlags; // r6
    unsigned char alphaLOD; // r7
    unsigned char alphaModel; // r8
...
// Range: 0x8004D4A4 -> 0x8004D4B0
void xModelBucket_RenderOpaqueBegin() {
    // References
    // -> static int sBucketOpaqueRenderStart;
}

// Range: 0x8004D4B0 -> 0x8004D628
void xModelBucket_RenderOpaque(int maxLayer /* r31 */) {
    // Local variables
    int i; // r25
    struct xModelBucket * bucket; // r24
    struct RpAtomic * data; // r23
    struct xModelInstance * minst; // r22
    struct RpAtomic * oldmodel; // r21
    int flags; // r20
...
// Range: 0x8004D678 -> 0x8004DB04
static void xModelBucket_RenderAlphaLayerInternal(int maxLayer /* r24 */) {
    // Local variables
    unsigned int pipeMask; // r31
    unsigned int curPipeFlags; // r30
    unsigned int lastPipeFlags; // r29
    unsigned char curAlphaDiscard; // r28
    enum RwFogType oldfogtype; // r0
    struct xModelInstance * minst; // r27
    struct RpAtomic * oldmodel; // r26
    float oldAlpha; // f31
    unsigned int xorPipeFlags; // r25
    enum RwBlendFunction srcBlend; // r0
    enum RwBlendFunction destBlend; // r5
...
// Range: 0x8004DB04 -> 0x8004DB08
void ValidateRenderStateExpectations() {}

// Range: 0x8004DB08 -> 0x8004DB98
...
// Range: 0x8004DB98 -> 0x8004DC00
unsigned char xModelBucket_RenderAny(int maxLayer /* r0 */) {
    // References
    // -> static struct xModelBucket * sBucketList;
    // -> static int sBucketCount;
    // -> static int sBucketOpaqueRenderStart;
...
// Range: 0x8004DC84 -> 0x8004DD1C
static void xModelBucket_SetBlendAndZModes(int flags /* r0 */) {
    // Local variables
    enum RwBlendFunction srcBlend; // r0
    enum RwBlendFunction destBlend; // r0
}

// Range: 0x8004DD1C -> 0x8004DD9C
static void xModelBucket_ResetBlendAndZModes(int flags /* r0 */) {}

// total size: 0x1
class CmpAlphaBucket {};
// Range: 0x8004DD9C -> 0x8004DDE0
static int CmpAlphaBucket::operator()(void * _a /* r0 */, void * _b /* r0 */) {
    // Local variables
    const struct xModelAlphaBucket * a; // r0
    const struct xModelAlphaBucket * b; // r0
}
