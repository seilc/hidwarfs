/*
    ...
    Language: C++
    Code range: 0x800409B0 -> 0x80042198
*/
static struct RwV3d * sCurrVert; // size: 0x4, address: 0x803C0F60
static unsigned int sAtomicStartCount; // size: 0x4, address: 0x803C0F64
static struct RwV3d * * sAtomicStartVert; // size: 0x4, address: 0x803C0F68
// total size: 0x8
struct RwObject {
    // Members
...
// Range: 0x80040B14 -> 0x80040BD8
unsigned int get_matid(const struct xJSPHeader * jsp /* r31 */, unsigned int old_mat_id /* r0 */, unsigned int jspAssetID /* r27 */) {
    // Local variables
    char num[100]; // r1+0x8
    unsigned int matid; // r3
    int count; // r29
...
// Range: 0x80040BD8 -> 0x80041128
void xJSP_MultiStreamRead(void * data /* r28 */, unsigned int size /* r26 */, struct xJSPHeader * * jsp /* r27 */) {
    // Local variables
    int i; // r7
    ...
    struct xJSPHeader * tmphdr; // r0
    struct RwV3d * currVec; // r1+0x10
    struct RwV3d * currVec; // r1+0xC
    struct xJSPNodeTree * tmpNodeTree; // r28
    unsigned int * stripvecdata; // r5
    struct __rwMark * mp2; // r28
    struct __rwMark mark2; // r1+0x1C
    struct RwMemory rwmem; // r1+0x14
    struct RwStream * stream; // r28
    int i; // r31
    int atomCount; // r0
    struct RpAtomic * * atomList; // r27
...
unsigned char needToLock; // size: 0x1, address: 0x803C1614
// total size: 0x2C8
struct OSContext {
    // Members
...
struct OSSemaphore asyncSema; // size: 0xC, address: 0x803C1608
// Range: 0x80041168 -> 0x800412CC
...
static struct TempSplit * sSplitList; // size: 0x4, address: 0x803C0F6C
static struct TempSplit * sSplitCurr; // size: 0x4, address: 0x803C0F70
static struct xJSPNodeTreeLeaf * sSplitLeaf; // size: 0x4, address: 0x803C0F74
// total size: 0x18
struct RwBBox {
    // Members
...
// Range: 0x80041394 -> 0x80041620
static float GetSplitVolumeSum(struct xJSPNodeTreeLeaf * tleaf /* r23 */, int count /* r24 */, int axis /* r25 */, float coord /* f31 */) {
    // Local variables
    int i; // r28
    int numleft; // r27
    int numright; // r26
...
static void RecurseSplitJSPNode(struct xJSPNodeTreeLeaf * tleaf /* r27 */, int count /* r28 */, struct TempSplitChild * childptr /* r29 */) {
    ...
    struct TempSplit * thisSplit; // r24
...
// Range: 0x80041FC0 -> 0x800420EC
static int JSPApplyLightCB(struct xJSPHeader * jsp /* r0 */, struct xJSPNodeTreeLeaf * leaf /* r0 */, struct RwBBox * box /* r0 */, void * data /* r0 */) {
    // Local variables
    int i; // r7
    struct xJSPNodeLight * nlight; // r8
}

// Range: 0x800420EC -> 0x80042160
