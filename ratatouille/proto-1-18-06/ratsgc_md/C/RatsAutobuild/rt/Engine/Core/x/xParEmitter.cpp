/*
    ...
    Language: C++
    Code range: 0x800519F4 -> 0x80052DA4
*/
// total size: 0x8
struct xBaseAsset {
    // Members
...
static struct xParEmitterAsset sSaveEmmiterSettings; // size: 0x54, address: 0x80324420
// total size: 0x138
...
static struct xParEmitterPropsAsset sSaveEmmiterPropSettings; // size: 0x138, address: 0x80324474
static struct xParEmitterPropsAsset sDummyProp; // size: 0x138, address: 0x803245AC
// Range: 0x800519F4 -> 0x800519F8
static void add_tweaks() {}

// Range: 0x800519F8 -> 0x80051A98
int xParInterpConvertInterpMode(struct xParInterp * p /* r0 */) {}

// Range: 0x80051A98 -> 0x80051AB8
...
// total size: 0x4
struct /* @class$820xParEmitter_cpp */ {
    // Members
...
union /* @class$839xParEmitter_cpp */ {
...
// total size: 0xC
struct /* @class$838xParEmitter_cpp */ {
    // Members
...
// Range: 0x80051CD4 -> 0x80051DA4
void xParEmitterSetup(struct xParEmitter * t /* r31 */) {
    // Local variables
    struct xEnt * ent; // r0
}

// Range: 0x80051DA4 -> 0x80051DE0
...
// Range: 0x80051DE0 -> 0x80051E90
void xParEmitterEventCB(struct xBase * to /* r4 */, unsigned int toEvent /* r0 */) {
    // Local variables
    struct xParEmitter * t; // r31
    struct xParEmitterCustomSettings fake_settings; // r1+0x8
}

// Range: 0x80051E90 -> 0x80052100
...
// Range: 0x800521A4 -> 0x800522F0
float xParInterpCompute(int interp_mode /* r0 */, struct xParInterp * r /* r31 */, float time /* f0 */, int time_has_elapsed /* r0 */, float lastVal /* f2 */) {
    // Local variables
    float val; // f0
}

// Range: 0x800522F0 -> 0x800523A4
struct xPar * xParEmitterEmitSetTexIdxs(struct xPar * p /* r30 */, const struct xParSys * ps /* r0 */) {
    // Local variables
...
static struct xParGroup temp_group; // size: 0x24, address: 0x803C11A0
// Range: 0x800523A4 -> 0x80052D6C
struct xPar * xParEmitterEmit(struct xParEmitter * pe /* r31 */, float emit_dt /* f28 */, float par_dt /* f29 */) {
    // Local variables
    struct xPar * last_p; // r1+0x48
    struct xParEmitterAsset * pea; // r28
    struct xParEmitterPropsAsset * prop; // r27
    int rate_has_elapsed; // r0
    float rate; // f0
    int count; // r26
    struct xParSys * ps; // r25
    struct xParGroup * g; // r0
    struct xPar * p; // r24
    float life; // f0
    float size_birth; // f0
    float size_death; // f0
    struct xVec3 emitPosition; // r1+0x2C
    struct xBase * attachObject; // r23
    unsigned int attachGroupIndex; // r22
    unsigned int attachGroupTotal; // r21
    int emitAgain; // r20
    int emitterIsAttachedToObject; // r1+0x44
    struct xBase * emitObj; // r19
    int marker; // r14
    struct xGroup * g; // r15
    unsigned int get_rnd_group_idx; // r16
    const struct xEnt * attach_ent; // r18
    const struct xMat4x3 * bone_mat; // r17
    struct xVec3 bone_vel; // r1+0x20
    struct xVec3 add_vel; // r1+0x14
    float last_emit_time; // f30
    const struct xVec3 & camPos; // r1+0x40
    int i; // r16
    int c; // r15
    float fc1; // f0
    float fc2; // f0
    struct xBase * obj; // r0
    struct xPar * save_next; // r14
    struct xPar * save_prev; // r15
    float u; // f27
    struct xVec3 pos_diff; // r1+0x8
...
// Range: 0x80052D6C -> 0x80052DA4
void xParEmitterUpdate(struct xBase * to /* r0 */, float dt /* f0 */) {
    // Local variables
    struct xParEmitter * pe; // r0
}
