/*
    ...
    Language: C++
    Code range: 0x800B4D64 -> 0x800B5A1C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
// Range: 0x800B4D64 -> 0x800B4D84
void zHangable_Init(struct xBase & data /* r0 */, struct xDynAsset & asset /* r0 */) {}
...
static struct zHangableObject * sPlayerHangObj; // size: 0x4, address: 0x803C1F28
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
static struct xVec3 playerDPos; // size: 0xC, address: 0x803C1F2C
static struct zHangableObject * sHangableObjects; // size: 0x4, address: 0x803C1F38
static int sNumHangableObjects; // size: 0x4, address: 0x803C1F3C
// total size: 0xC
...
// Range: 0x800B4EDC -> 0x800B515C
void zHangable_SceneInit(struct zScene * zsc /* r31 */) {
    // Local variables
    int i; // r28
    int count; // r27
    int numHangables; // r26
    struct zHangable * hangable; // r5
    struct zHangableObject * hangObj; // r26
...
// Range: 0x800B51B4 -> 0x800B5898
void zHangable_Update(float dt /* f26 */) {
    // Local variables
    int i; // r30
    struct zHangableObject * hangObj; // r29
    struct zHangableAsset * asset; // r28
    struct xVec3 oldHandle; // r1+0xAC
    struct xVec3 hangDisp; // r1+0xA0
    struct xVec3 playerCenter; // r1+0x94
    struct xVec3 localCenter; // r1+0x88
    struct xVec3 centerDisp; // r1+0x7C
    struct xVec3 disp; // r1+0x70
    float dist; // f0
    struct xVec3 pivot; // r1+0x64
    struct xMat3x3 newMat; // r1+0x120
    struct xMat3x3 rotMat; // r1+0xF0
    float comp; // f0
    float comp; // f0
    float maxAng; // f25
    struct xVec3 cross; // r1+0x58
    float len; // f1
    float ang; // f0
    struct xMat3x3 rotMat; // r1+0xC0
    struct xVec3 actualDisp; // r1+0x4C
    struct xVec3 actualPivot; // r1+0x40
    struct xVec3 cross; // r1+0x34
    float len; // f1
    float ang; // f25
    float minArcRadians; // f24
    float height; // f25
    struct xVec3 relVel; // r1+0x28
    float velSq; // f0
    float total; // f24
    float reqTotal; // f0
    struct xVec3 velAddDir; // r1+0x1C
    float velDot; // f0
    struct xModelInstance * atomic; // r28
    struct xVec3 newPlayerCenter; // r1+0x10
...
// Range: 0x800B5978 -> 0x800B5A1C
void zHangable_EventCB(struct xBase * to /* r29 */, unsigned int toEvent /* r0 */) {
    // Local variables
