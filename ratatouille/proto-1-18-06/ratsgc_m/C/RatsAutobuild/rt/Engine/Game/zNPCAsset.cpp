/*
    ...
    Language: C++
    Code range: 0x80135630 -> 0x8013597C
*/
// total size: 0x8
struct xBaseAsset {
    // Members
...
// Range: 0x80135630 -> 0x80135680
static int ValidThief(const struct xDynAsset * dynAsset /* r31 */) {
    // Local variables
    char fullName[256]; // r1+0x8
}

// Range: 0x80135680 -> 0x801356D0
static int ValidChef(const struct xDynAsset * dynAsset /* r31 */) {
    // Local variables
    char fullName[256]; // r1+0x8
}
...
static struct DynNPCInfo g_DynNPCInfo[3]; // size: 0x3C, address: 0x803BD518
// total size: 0xC
struct RwV3d {
    // Members
...
static int needToHash; // size: 0x4, address: 0x803C1948
static signed char init; // size: 0x1, address: 0x803C194C
// Range: 0x80135718 -> 0x80135790
...
// Range: 0x80135790 -> 0x8013585C
int zNPCAsset_CountNPCs(int * typeCount /* r27 */) {
    // Local variables
    int count_NPC; // r29
    int num_alldyn; // r0
    int i; // r28
    ...
    int isOurs; // r5
    struct DynNPCInfo * npcinf_next; // r6
    struct DynNPCInfo * npcinf; // r7

    // References
    // -> static struct DynNPCInfo g_DynNPCInfo[3];
}

// Range: 0x8013585C -> 0x80135900
void zNPCAsset_CreateNPCs() {
    // Local variables
    int num_alldyn; // r0
    int i; // r29
    unsigned int dynAssetSize; // r1+0x8
    const struct xDynAsset * dynAsset; // r3
