/*
    Compile unit: c:\SourceTreeTools\compile\dolphin\build\libraries\card\src\CARDBlock.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x8039E868 -> 0x8039EDE0
*/
// total size: 0x50
...
// Range: 0x8039E868 -> 0x8039E8BC
unsigned short * __CARDGetFatBlock(struct CARDControl * card /* r31 */) {}

struct CARDControl __CARDBlock[2]; // size: 0x220, address: 0x80D57600
// Range: 0x8039E8BC -> 0x8039E9D4
static void WriteCallback(long chan /* r25 */, long result /* r26 */) {
    // Local variables
    struct CARDControl * card; // r31
    void (* callback)(long, long); // r27
    unsigned short * fat0; // r29
    unsigned short * fat1; // r28

    // References
    // -> struct CARDControl __CARDBlock[2];
}

// Range: 0x8039E9D4 -> 0x8039EAA0
static void EraseCallback(long chan /* r27 */, long result /* r30 */) {
    // Local variables
    struct CARDControl * card; // r31
    void (* callback)(long, long); // r29
    unsigned short * fat; // r28
...
// Range: 0x8039EAA0 -> 0x8039EC24
long __CARDAllocBlock(long chan /* r25 */, unsigned long cBlock /* r28 */, void (* callback)(long, long) /* r1+0x10 */) {
    // Local variables
    struct CARDControl * card; // r29
    unsigned short * fat; // r30
    unsigned short iBlock; // r31
    unsigned short startBlock; // r26
    unsigned short prevBlock; // r24
    unsigned short count; // r23

    // References
    // -> struct CARDControl __CARDBlock[2];
}

// Range: 0x8039EC24 -> 0x8039ED18
long __CARDFreeBlock(long chan /* r28 */, unsigned short nBlock /* r31 */, void (* callback)(long, long) /* r1+0x10 */) {
    // Local variables
    struct CARDControl * card; // r29
    unsigned short * fat; // r30
    unsigned short nextBlock; // r27

    // References
    // -> struct CARDControl __CARDBlock[2];
}

// Range: 0x8039ED18 -> 0x8039EDE0
long __CARDUpdateFatBlock(long chan /* r29 */, unsigned short * fat /* r31 */, void (* callback)(long, long) /* r1+0x10 */) {
    // Local variables
    struct CARDControl * card; // r30
    unsigned long addr; // r28
