/*
    Compile unit: c:\SourceTreeTools\compile\dolphin\build\libraries\card\src\CARDOpen.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x803A0DB0 -> 0x803A13B4
*/
// total size: 0x40
struct CARDDir {
    // Members
...
// Range: 0x803A0DB0 -> 0x803A0E2C
int __CARDCompareFileName(struct CARDDir * ent /* r3 */, const char * fileName /* r4 */) {
    // Local variables
    char * entName; // r31
    char c1; // r1+0x10
    char c2; // r30
...
// Range: 0x803A0E2C -> 0x803A0EC0
long __CARDAccess(struct CARDControl * card /* r1+0x8 */, struct CARDDir * ent /* r30 */) {
    // Local variables
    const struct DVDDiskID * diskID; // r31

    // References
    // -> struct DVDDiskID __CARDDiskNone;
}

unsigned char __CARDPermMask; // size: 0x1, address: 0x80D6801A
// Range: 0x803A0EC0 -> 0x803A0FA8
long __CARDIsWritable(struct CARDControl * card /* r27 */, struct CARDDir * ent /* r31 */) {
    // Local variables
    const struct DVDDiskID * diskID; // r28
    long result; // r30
    unsigned char perm; // r29

    // References
    // -> struct DVDDiskID __CARDDiskNone;
    // -> unsigned char __CARDPermMask;
}

// Range: 0x803A0FA8 -> 0x803A1004
long __CARDIsReadable(struct CARDControl * card /* r1+0x8 */, struct CARDDir * ent /* r30 */) {
    // Local variables
    long result; // r31
}

// Range: 0x803A1004 -> 0x803A10B0
long __CARDGetFileNo(struct CARDControl * card /* r29 */, const char * fileName /* r1+0xC */, long * pfileNo /* r1+0x10 */) {
    // Local variables
...
// Range: 0x803A10B0 -> 0x803A11E8
long CARDFastOpen(long chan /* r26 */, long fileNo /* r29 */, struct CARDFileInfo * fileInfo /* r27 */) {
    // Local variables
    struct CARDControl * card; // r1+0x14
    struct CARDDir * dir; // r25
    struct CARDDir * ent; // r28
    long result; // r31
}

// Range: 0x803A11E8 -> 0x803A12F0
long CARDOpen(long chan /* r28 */, const char * fileName /* r1+0xC */, struct CARDFileInfo * fileInfo /* r30 */) {
    // Local variables
    struct CARDControl * card; // r1+0x18
    long fileNo; // r1+0x14
    long result; // r31
    struct CARDDir * dir; // r27
    struct CARDDir * ent; // r29
}

// Range: 0x803A12F0 -> 0x803A13AC
long CARDClose(struct CARDFileInfo * fileInfo /* r31 */) {
    // Local variables
    struct CARDControl * card; // r1+0xC
    long result; // r29
}

// Range: 0x803A13AC -> 0x803A13B4
int __CARDIsOpened() {}
