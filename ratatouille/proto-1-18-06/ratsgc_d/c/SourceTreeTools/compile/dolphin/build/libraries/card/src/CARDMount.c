/*
    Compile unit: c:\SourceTreeTools\compile\dolphin\build\libraries\card\src\CARDMount.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x8039FD48 -> 0x803A0894
*/
static long SectorSizeTable[8]; // size: 0x20, address: 0x8053B6A8
static long LatencyTable[8]; // size: 0x20, address: 0x8053B6C8
unsigned short __CARDVendorID; // size: 0x2, address: 0x80D68018
// Range: 0x8039FD48 -> 0x8039FE24
static int IsCard(unsigned long id /* r3 */) {
    // Local variables
    unsigned long size; // r31
    long sectorSize; // r30

    // References
    // -> static long SectorSizeTable[8];
    // -> unsigned short __CARDVendorID;
}
...
// Range: 0x8039FE24 -> 0x8039FFA8
long CARDProbeEx(long chan /* r30 */, long * memSize /* r27 */, long * sectorSize /* r28 */) {
    // Local variables
    unsigned long id; // r1+0x14
...
// Range: 0x8039FFA8 -> 0x803A0384
static long DoMount(long chan /* r29 */) {
    // Local variables
    struct CARDControl * card; // r31
    unsigned long id; // r1+0x10
    unsigned char status; // r1+0xC
    long result; // r30
    struct OSSramEx * sram; // r26
    int i; // r27
    unsigned char checkSum; // r25
    int step; // r24
    unsigned short vendorID; // r23

    // References
    // -> unsigned short __CARDVendorID;
    // -> static long LatencyTable[8];
    // -> static long SectorSizeTable[8];
    // -> struct CARDControl __CARDBlock[2];
}

// Range: 0x803A0384 -> 0x803A04F4
void __CARDMountCallback(long chan /* r29 */, long result /* r31 */) {
    // Local variables
    struct CARDControl * card; // r30
    void (* callback)(long, long); // r27

    // References
    // -> struct CARDControl __CARDBlock[2];
}

// Range: 0x803A04F4 -> 0x803A06F4
long CARDMountAsync(long chan /* r30 */, void * workArea /* r27 */, void (* detachCallback)(long, long) /* r1+0x10 */, void (* attachCallback)(long, long) /* r26 */) {
    // Local variables
    struct CARDControl * card; // r31
    int enabled; // r29
...
// Range: 0x803A06F4 -> 0x803A0758
long CARDMount(long chan /* r30 */, void * workArea /* r1+0xC */, void (* detachCallback)(long, long) /* r1+0x10 */) {
    // Local variables
    long result; // r31
}

// Range: 0x803A0758 -> 0x803A0810
static void DoUnmount(long chan /* r30 */, long result /* r1+0xC */) {
    // Local variables
    struct CARDControl * card; // r31
    int enabled; // r29
...
// Range: 0x803A0810 -> 0x803A0894
long CARDUnmount(long chan /* r31 */) {
    // Local variables
    struct CARDControl * card; // r1+0xC
    long result; // r30
}
