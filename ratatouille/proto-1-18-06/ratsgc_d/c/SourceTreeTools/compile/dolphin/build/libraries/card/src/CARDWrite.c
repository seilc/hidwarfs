/*
    Compile unit: c:\SourceTreeTools\compile\dolphin\build\libraries\card\src\CARDWrite.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x803A1ED0 -> 0x803A2338
*/
// total size: 0x50
...
// Range: 0x803A1ED0 -> 0x803A206C
static void WriteCallback(long chan /* r27 */, long result /* r29 */) {
    // Local variables
    struct CARDControl * card; // r31
    void (* callback)(long, long); // r28
    unsigned short * fat; // r26
    struct CARDDir * dir; // r25
    struct CARDDir * ent; // r24
    struct CARDFileInfo * fileInfo; // r30

    // References
    // -> struct CARDControl __CARDBlock[2];
}

// Range: 0x803A206C -> 0x803A215C
static void EraseCallback(long chan /* r26 */, long result /* r29 */) {
    // Local variables
    struct CARDControl * card; // r31
    void (* callback)(long, long); // r28
    struct CARDFileInfo * fileInfo; // r27

    // References
    // -> struct CARDControl __CARDBlock[2];
}

// Range: 0x803A215C -> 0x803A2338
long CARDWriteAsync(struct CARDFileInfo * fileInfo /* r27 */, void * buf /* r28 */, long length /* r29 */, long offset /* r26 */, void (* callback)(long, long) /* r23 */) {
    // Local variables
