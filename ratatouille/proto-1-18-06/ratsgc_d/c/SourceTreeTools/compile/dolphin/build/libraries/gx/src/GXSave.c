/*
    Compile unit: c:\SourceTreeTools\compile\dolphin\build\libraries\gx\src\GXSave.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x803C6D58 -> 0x803C7D24
*/
static const unsigned char * dlist; // size: 0x4, address: 0x80D6BFD8
static unsigned long dlistSize; // size: 0x4, address: 0x80D6BFDC
static unsigned long bytesRead; // size: 0x4, address: 0x80D6BFE0
// Range: 0x803C6D58 -> 0x803C6DD0
static unsigned char __ReadMem(void * ptr /* r3 */, unsigned long sz /* r4 */) {
    // Local variables
    const unsigned char * src; // r30
    unsigned char * dst; // r29
    unsigned long i; // r31

    // References
    // -> static const unsigned char * dlist;
    // -> static unsigned long bytesRead;
    // -> static unsigned long dlistSize;
}
...
// Range: 0x803C6DD0 -> 0x803C6F44
static void __SaveCPRegs(unsigned char reg /* r28 */, unsigned char vatIdx /* r29 */, unsigned long data /* r30 */) {
    // Local variables
    long idx; // r31
...
// Range: 0x803C6F44 -> 0x803C6F94
void DPF() {}

// Range: 0x803C6F94 -> 0x803C7004
static void __ReconstVtxStatus(unsigned char vatIdx /* r3 */) {
    // Local variables
    unsigned long vat; // r31

    // References
    // -> struct __GXData_struct * const __GXData;
}

static unsigned long vtxCompSize[5]; // size: 0x14, address: 0x80546EBC
static int clrCompSize[6]; // size: 0x18, address: 0x80546ED0
// Range: 0x803C7004 -> 0x803C7730
static unsigned long GetAttrSize(unsigned char vatIdx /* r3 */, unsigned long attrIdx /* r4 */) {
    // Local variables
    unsigned long vcd; // r30
    unsigned long vat; // r31
    unsigned long nc; // r28

    // References
    // -> static unsigned long vtxCompSize[5];
    // -> struct __GXData_struct * const __GXData;
    // -> static int clrCompSize[6];
}
...
// Range: 0x803C7730 -> 0x803C77BC
static void __ParseVertexData(unsigned char vatIdx /* r1+0x8 */) {
    // Local variables
    unsigned short vcnt; // r1+0xA
    enum _GXAttr attrIdx; // r31
    unsigned long vsize; // r30
...
// Range: 0x803C77BC -> 0x803C7B8C
void __GXShadowDispList(void * list /* r1+0x8 */, unsigned long nbytes /* r1+0xC */) {
    // Local variables
    unsigned char cmd; // r1+0x1C
    unsigned char cmdOp; // r27
    unsigned char vatIdx; // r28
    unsigned long reg32; // r1+0x18
    unsigned long d32; // r1+0x14
    unsigned char reg8; // r1+0x10
    unsigned char cpAddr; // r24
    unsigned long i; // r26
    unsigned long addr; // r30
    unsigned long cnt; // r25
    long xfAddr; // r31
...
// Range: 0x803C7B8C -> 0x803C7D24
void __GXShadowIndexState(unsigned long idx_reg /* r1+0x8 */, unsigned long reg_data /* r25 */) {
    // Local variables
    unsigned long * basePtr; // r24
    unsigned long * memAddr; // r29
    unsigned long cnt; // r28
    unsigned long stride; // r27
    unsigned long addr; // r30
    unsigned long data; // r1+0x10
    unsigned long index; // r23
    unsigned long i; // r26
    long xfAddr; // r31
