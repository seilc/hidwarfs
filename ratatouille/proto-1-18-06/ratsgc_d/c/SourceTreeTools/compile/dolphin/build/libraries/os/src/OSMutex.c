/*
    Compile unit: c:\SourceTreeTools\compile\dolphin\build\libraries\os\src\OSMutex.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x803D419C -> 0x803D4438
*/
// total size: 0x2C8
struct OSContext {
    // Members
...
// Range: 0x803D4240 -> 0x803D437C
int __OSCheckMutex(struct OSMutex * mutex /* r3 */) {
    // Local variables
    struct OSThread * thread; // r31
    struct OSThreadQueue * queue; // r30
    long priority; // r29
}

// Range: 0x803D437C -> 0x803D43CC
int __OSCheckDeadLock(struct OSThread * thread /* r3 */) {
    // Local variables
    struct OSMutex * mutex; // r31
}

// Range: 0x803D43CC -> 0x803D4438
int __OSCheckMutexes(struct OSThread * thread /* r30 */) {
    // Local variables
    struct OSMutex * mutex; // r31
}
