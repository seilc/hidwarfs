/*
    Compile unit: c:\SourceTreeTools\compile\dolphin\build\libraries\card\src\CARDUnlock.c
    Producer: MW EABI PPC C-Compiler
    Language: C89
    Code range: 0x8039DA78 -> 0x8039E484
*/
static unsigned char CardData[352]; // size: 0x160, address: 0x8053B240
static unsigned long next; // size: 0x4, address: 0x80D68010
// Range: 0x8039DA78 -> 0x8039DA9C
static int CARDRand() {
    // References
    // -> static unsigned long next;
}

// Range: 0x8039DA9C -> 0x8039DAA4
static void CARDSrand(unsigned int seed /* r3 */) {
    // References
    // -> static unsigned long next;
}

// Range: 0x8039DAA4 -> 0x8039DAF8
static unsigned long exnor_1st(unsigned long data /* r3 */, unsigned long rshift /* r4 */) {
    // Local variables
    unsigned long wk; // r29
    unsigned long work; // r31
    unsigned long i; // r30
}

// Range: 0x8039DAF8 -> 0x8039DB4C
static unsigned long exnor(unsigned long data /* r3 */, unsigned long lshift /* r4 */) {
    // Local variables
    unsigned long wk; // r29
    unsigned long work; // r31
    unsigned long i; // r30
...
// Range: 0x8039DB4C -> 0x8039DBDC
static unsigned long bitrev(unsigned long data /* r3 */) {
    // Local variables
    unsigned long wk; // r30
    unsigned long i; // r31
    unsigned long k; // r29
    unsigned long j; // r28
}
...
// Range: 0x8039DBDC -> 0x8039DD5C
static long ReadArrayUnlock(long chan /* r31 */, unsigned long data /* r30 */, void * rbuf /* r1+0x10 */, long rlen /* r1+0x14 */, int mode /* r1+0x18 */) {
    // Local variables
    struct CARDControl * card; // r28
    int err; // r29
    unsigned char cmd[5]; // r1+0x1C
...
// Range: 0x8039DD5C -> 0x8039DDA8
static unsigned long GetInitVal() {
    // Local variables
    unsigned long tmp; // r31
    unsigned long tick; // r30
}

// Range: 0x8039DDA8 -> 0x8039DE50
static long DummyLen() {
    // Local variables
    unsigned long tick; // r29
    unsigned long wk; // r30
    long tmp; // r31
    unsigned long max; // r28
}

// total size: 0x10
struct CARDDecParam {
    // Members
    unsigned char * inputAddr; // offset 0x0, size 0x4
    unsigned long inputLength; // offset 0x4, size 0x4
    unsigned long aramAddr; // offset 0x8, size 0x4
    unsigned char * outputAddr; // offset 0xC, size 0x4
};
// Range: 0x8039DE50 -> 0x8039E1E0
long __CARDUnlock(long chan /* r1+0x8 */, unsigned char * flashID /* r1+0xC */) {
    // Local variables
    unsigned long init_val; // r16
    unsigned long data; // r1+0x5C
    long dummy; // r23
    long rlen; // r18
    unsigned long rshift; // r29
    unsigned char fsts; // r1+0x58
    unsigned long wk; // r31
    unsigned long wk1; // r28
    unsigned long Ans1; // r17
    unsigned long Ans2; // r1+0x54
    unsigned long * dp; // r26
    unsigned char rbuf[64]; // r1+0x14
    unsigned long para1A; // r22
    unsigned long para1B; // r21
    unsigned long para2A; // r20
    unsigned long para2B; // r19
    struct CARDControl * card; // r30
    struct STRUCT_DSP_TASK * task; // r27
    struct CARDDecParam * param; // r24
    unsigned char * input; // r25
    unsigned char * output; // r1+0x10

    // References
    // -> static unsigned char CardData[352];
    // -> struct CARDControl __CARDBlock[2];
}

// Range: 0x8039E1E0 -> 0x8039E294
static void InitCallback(void * _task /* r1+0x8 */) {
    // Local variables
    long chan; // r31
    struct CARDControl * card; // r30
    struct STRUCT_DSP_TASK * task; // r29
    struct CARDDecParam * param; // r28

    // References
    // -> struct CARDControl __CARDBlock[2];
}

// Range: 0x8039E294 -> 0x8039E484
static void DoneCallback(void * _task /* r1+0x8 */) {
    // Local variables
    unsigned char rbuf[64]; // r1+0x10
    unsigned long data; // r27
    long dummy; // r28
    long rlen; // r26
    unsigned long rshift; // r22
    unsigned char fsts; // r1+0xC
    unsigned long wk; // r29
    unsigned long wk1; // r21
    unsigned long Ans2; // r23
    long chan; // r31
    struct CARDControl * card; // r30
    long result; // r25
    struct STRUCT_DSP_TASK * task; // r20
    struct CARDDecParam * param; // r19
    unsigned char * input; // r24
    unsigned char * output; // r18
