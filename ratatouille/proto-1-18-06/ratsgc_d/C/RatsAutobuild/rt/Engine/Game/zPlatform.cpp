/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zPlatform.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8018DBD0 -> 0x80193B3C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct zParEmitter * sEmitTremble; // size: 0x4, address: 0x80D6A6A0
static struct zParEmitter * sEmitBreakaway; // size: 0x4, address: 0x80D6A6A4
// total size: 0x10
...
static struct effect * sRumbleBreakaway; // size: 0x4, address: 0x80D6A6A8
static unsigned int entToCheck; // size: 0x4, address: 0x80D6A6AC
static signed char init; // size: 0x1, address: 0x80D6A6B0
...
// Range: 0x8018DBD0 -> 0x8018DC98
void genericPlatRender(struct xEnt * ent /* r31 */) {
    // Local variables
    int assdf; // r1+0x8

    // References
    // -> int gDrawInvisibleWalls;
    // -> static unsigned int entToCheck;
    // -> static signed char init;
}

int gDrawInvisibleWalls; // size: 0x4, address: 0x80D6A5C0
// Range: 0x8018DC98 -> 0x8018DC9C
void zPlatform_ScenePrepare() {}

// Range: 0x8018DC9C -> 0x8018DCCC
void zPlatform_Init(void * plat /* r1+0x8 */, void * asset /* r1+0xC */) {}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D63300
...
// total size: 0x10
struct /* @class$1893zPlatform_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x8018DCCC -> 0x8018E450
void zPlatform_Init(struct zPlatform * plat /* r31 */, struct xEntAsset * asset /* r21 */) {
    // Local variables
    struct xPlatformAsset * plat_asset; // r30
    struct xEntMotionAsset * ent_motion_asset; // r26
    void * buf; // r1+0x20
    unsigned int size; // r1+0x1C
    void * spring_anim; // r29
    void * idle_anim; // r28
    struct xAnimFile * spring_file; // r27
    struct xAnimFile * idle_file; // r25
    unsigned int size; // r1+0x18
    unsigned int newid; // r24
    struct xDestructibleAsset * destruct; // r20
...
// Range: 0x8018E450 -> 0x8018E52C
void zPlatform_Setup(struct zPlatform * plat /* r31 */, struct xScene * sc /* r1+0x8 */) {
    // References
    // -> static struct effect * sRumbleBreakaway;
    // -> static struct zParEmitter * sEmitBreakaway;
    // -> static struct zParEmitter * sEmitTremble;
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D6330F
// Range: 0x8018E52C -> 0x8018E61C
void zPlatform_Save(struct zPlatform * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D6331E
// Range: 0x8018E61C -> 0x8018E70C
void zPlatform_Load(struct zPlatform * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

// Range: 0x8018E70C -> 0x8018E754
void zPlatform_ReReset(struct zPlatform * plat /* r31 */) {
    // Local variables
    unsigned int size; // r1+0x8
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D6332D
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x8018E754 -> 0x8018EDD0
void zPlatform_Reset(struct zPlatform * plat /* r31 */, struct xScene * sc /* r1+0x10 */) {
    // Local variables
    unsigned int size; // r1+0x14
    struct RpAtomic * imodel; // r27
    struct xSurface * surf; // r28
    struct xAnimPlay * tempAnim; // r26
    struct xAnimTable * temptbl; // r25
    unsigned short tempFlags; // r24
    void * tempObject; // r23
    int i; // r30
    struct xMat4x3 * pmat; // r21
    struct xMat3x3 normMat; // r1+0x20
    struct _zEnv * zen; // r22
    struct xEnvAsset * easset; // r29
    struct xLightKit * objLightKit; // r20
...
// Range: 0x8018EDD0 -> 0x8018F30C
static float SolvePaddleMotion(struct zPlatform * plat /* r29 */, float * time /* r30 */, float tmr /* f31 */) {
    // Local variables
    struct xPlatformPaddleData * paddle; // r31
    float destOrient; // f28
    float srcOrient; // f27
    float absDelta; // f30
    float stutterAmount; // f24
    float A; // f29
    float D; // f26
    float distA; // f23
    float distC; // f25
...
static char __FUNCTION__[28]; // size: 0x1C, address: 0x8051F888
// Range: 0x8018F30C -> 0x8018F4E8
void zPlatform_PaddleStartRotate(struct xEnt * entplat /* r30 */, int direction /* r29 */, int stutter /* r1+0x8 */) {
    // Local variables
    struct zPlatform * plat; // r31
    float time[3]; // r1+0xC
...
// total size: 0xC
struct /* @class$1053zPlatform_cpp */ {
    // Members
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D6333D
// total size: 0x8
struct zPlatFMEventsTable {
    // Members
    unsigned int sourceEvent; // offset 0x0, size 0x4
    unsigned int drivenEvent; // offset 0x4, size 0x4
};
// total size: 0x10
struct xQuat {
    // Members
...
// Range: 0x8018F4E8 -> 0x80190C98
static void zPlatFM_Update(struct zPlatform * plat /* r28 */, float dt /* f29 */) {
    // Local variables
    struct zPlatFMEventsTable eventsTable[17]; // r1+0x240
    struct zPlatFMRunTime * fmrt; // r30
    struct xMat4x3 * pmat; // r31
    struct xMat3x3 platformRot; // r1+0x210
    struct xVec3 platformScale; // r1+0xD8
    struct xVec3 platformLoc; // r1+0xCC
    int i; // r29
    float tm; // f30
    float ttm; // f25
    float atm; // f27
    ...
    float cfcs; // r1+0x64
    float cfdec; // r1+0x60
    struct xMat4x3 * pmat; // r27
    struct xVec3 undoScale; // r1+0xC0
    struct xMat3x3 preR; // r1+0x1E0
    struct xMat3x3 preR; // r1+0x1B0
    struct xMat3x3 preR; // r1+0x180
    struct xMat3x3 postR; // r1+0x150
    struct xMat3x3 postR; // r1+0x120
    struct xMat3x3 postR; // r1+0xF0
    struct xQuat curQuat; // r1+0xB0
    float scale_adjust; // r1+0x5C
    float scale_adjust; // r1+0x58
    float scale_adjust; // r1+0x54
    float scale_adjust; // r1+0x50
    unsigned char finishedEvent; // r26
    float tm; // r1+0x4C
    float rts; // r1+0x48
    float ups; // r1+0x44
    float ats; // r1+0x40
    ...
    float len; // f24
    float len2; // r1+0x3C
    float len_inv; // r1+0x38
    float inv_t; // f21
    float len2; // r1+0x34
    float len_inv; // r1+0x30
    float rdotu; // r1+0x2C
    float len2; // r1+0x28
    float len_inv; // r1+0x24

    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[26]; // size: 0x1A, address: 0x8051F8E8
struct xGlobals * xglobals; // size: 0x4, address: 0x80D63020
// Range: 0x80190C98 -> 0x801913B4
void zPlatformBreakaway_Update(struct zPlatform * plat /* r31 */, struct xScene * sc /* r1+0x8 */, float dt /* f31 */) {
    // Local variables
    struct xAnimState * ast; // r26
    struct xAnimSingle * single; // r30
    struct xAnimState * ast; // r25
    struct xAnimSingle * single; // r28
    struct xAnimState * ast; // r24
...
static unsigned int watch_plat; // size: 0x4, address: 0x80D6334C
static int been_here; // size: 0x4, address: 0x80D6A6B4
static int been_here; // size: 0x4, address: 0x80D6A6B8
static int been_here; // size: 0x4, address: 0x80D6A6BC
static int been_here; // size: 0x4, address: 0x80D6A6C0
static int been_here; // size: 0x4, address: 0x80D6A6C4
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D63350
unsigned char * g_iAnim_workBuffer; // size: 0x4, address: 0x80D61DF8
// Range: 0x801913B4 -> 0x80192078
void zPlatform_Update(struct xEnt * entplat /* r1+0x10 */, struct xScene * sc /* r1+0x14 */, float dt /* f31 */) {
    // Local variables
    struct zPlatform * plat; // r31
    int i; // r1+0x3C
    unsigned char use_world_up; // r1+0x19
    unsigned short state; // r24
    int i; // r25
    struct xEntMotionMechData * mkasst; // r1+0x38
    float duration; // f24
    struct xQuat * q0; // r17
    struct xVec3 * t0; // r1+0x34
    struct xEntDrive * drv; // r30
    float s; // r1+0x30
    struct xEnt * p; // r28
    struct xEntDrive * drv; // r29
    float ctilt; // f29
    float itilt; // f28
    float dtilt; // f27
    struct xVec3 * scale; // r22
    float s; // r1+0x2C
    struct xEnt * p; // r1+0x28
    struct xVec3 lpos; // r1+0x4C
    float cangle; // f23
    float dangle; // f30
    float mangle; // f22
    float eangle; // r1+0x24
    float tangle; // f25
    struct xVec3 * scale; // r21
    float time[3]; // r1+0x40
    float orient; // r1+0x20
    struct xMat3x3 orientrot; // r1+0x90
    struct xMat3x3 origrot; // r1+0x60
    struct xVec3 * scale; // r20
    struct xMovePoint * src; // r1+0x1C
    float vecLen; // f26
    int moving; // r23
    struct xVec3 * opos; // r19
    struct xVec3 * pos; // r18
    struct xVec3 * orot; // r27
    struct xVec3 * rot; // r26
    ...
    // -> struct zGlobals globals;
    // -> static int been_here;
    // -> static unsigned int watch_plat;
}
...
// total size: 0x890
struct /* @class$2136zPlatform_cpp */ {
    // Static members
...
// total size: 0x54
struct /* @class$2209zPlatform_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$833zPlatform_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$834zPlatform_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$835zPlatform_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$836zPlatform_cpp */ {
    // Members
...
static int pleaseStopMoving; // size: 0x4, address: 0x80D6A6C8
static signed char init; // size: 0x1, address: 0x80D6A6CC
static unsigned int aid_haltOnMe; // size: 0x4, address: 0x80D6A6D0
static signed char init; // size: 0x1, address: 0x80D6A6D4
static int beWhereISay; // size: 0x4, address: 0x80D6A6D8
static signed char init; // size: 0x1, address: 0x80D6A6DC
static struct xVec3 pos_hackstop; // size: 0xC, address: 0x80D63364
static int been_here; // size: 0x4, address: 0x80D6A6E0
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D63370
// Range: 0x80192078 -> 0x801921D0
void zPlatform_Move(struct xEnt * entplat /* r30 */, struct xScene * s /* r29 */, float dt /* f31 */, struct xEntFrame * frame /* r1+0x8 */) {
    // Local variables
    struct zPlatform * plat; // r31
    ...
    // -> static int been_here;
    // -> static char __FUNCTION__[15];
    // -> static struct xVec3 pos_hackstop;
    // -> static int beWhereISay;
    // -> static signed char init;
    // -> static unsigned int aid_haltOnMe;
    // -> static int pleaseStopMoving;
    // -> static signed char init;
    // -> static signed char init;
}
...
struct xFFXShakeState {
    // Members
    struct xVec3 disp; // offset 0x0, size 0xC
    ...
    struct xFFXShakeState * next; // offset 0x20, size 0x4
};
// Range: 0x801921D0 -> 0x8019228C
void zPlatform_Shake(struct zPlatform * plat /* r1+0x8 */, float ampl /* r1+0xC */, float freq /* r1+0x10 */) {
    // Local variables
    struct xFFXShakeState * ss; // r31
    struct xFFX * sfkt; // r30
}
...
// Range: 0x8019228C -> 0x801923AC
static void zPlatform_Tremble(struct zPlatform * plat /* r28 */, float ampl /* r1+0x8 */, float freq /* r1+0xC */, float dur /* f31 */) {
    // Local variables
    struct xFFXShakeState * ss; // r31
    struct xFFX * sfkt; // r30
    struct xParEmitterCustomSettings info; // r1+0x10
    int i; // r29
...
// Range: 0x801923AC -> 0x80192430
static void zPlatform_BreakawayFallFX(struct zPlatform * plat /* r1+0x8 */) {
    // Local variables
    struct xParEmitterCustomSettings info; // r1+0xC
    int i; // r31
...
// Range: 0x80192430 -> 0x801924E4
void zPlatform_Mount(struct zPlatform * plat /* r31 */) {
    // References
    // -> struct xGlobals * xglobals;
}

// Range: 0x801924E4 -> 0x801925AC
void zPlatform_Dismount(struct zPlatform * plat /* r31 */) {
    // Local variables
    int i; // r1+0x8
...
// Range: 0x801925AC -> 0x80192608
static void zPlatformTranslate(struct xEnt * xent /* r1+0x8 */, struct xVec3 * dpos /* r29 */, struct xMat4x3 * dmat /* r30 */) {
    // Local variables
    struct zPlatform * plat; // r31
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D6337F
// Range: 0x80192608 -> 0x80192878
static void zPlatFM_EventSetup(struct zPlatform * plat /* r28 */, const float * toParam /* r30 */, int idx /* r29 */, unsigned int mask /* r1+0x10 */) {
    // Local variables
    struct zPlatFMRunTime * fmrt; // r31
    float ds; // f28
    float ttm; // f30
    float atm; // f31
    float dtm; // f29
...
static int been_here; // size: 0x4, address: 0x80D6A6E4
static int been_here; // size: 0x4, address: 0x80D6A6E8
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D63392
union FloatAndVoid {
    float f; // offset 0x0, size 0x4
    void * v; // offset 0x0, size 0x4
};
// Range: 0x80192878 -> 0x80193B3C
void zPlatformEventCB(struct xBase * from /* r19 */, struct xBase * to /* r24 */, unsigned int toEvent /* r20 */, const float * toParam /* r30 */, struct xBase * toParamWidget /* r28 */, unsigned int toParamWidgetID /* r1+0x10 */) {
    // Local variables
    struct zPlatform * plat; // r31
    float tp[4]; // r1+0x2C
    int iParam0; // r27
    int destctr; // r26
    struct zPlatFMRunTime * fmrt; // r25
    float yaw; // f26
    float pitch; // f25
    float roll; // f24
    struct xMat3x3 rotMat; // r1+0x40
    float desired_angle; // f23
    float current_angle; // f22
    float delta_angle; // f31
    float desired_angle; // f21
    float current_angle; // f20
    float delta_angle; // f30
    float desired_angle; // f19
    float current_angle; // f18
    float delta_angle; // f29
    float mt; // f28
    float mt; // f27
    union FloatAndVoid dist; // r1+0x1C
    struct zShrapnelAsset * shrap; // r29
    struct xVec3 currVel; // r1+0x20
    struct xVec3 * offset; // r1+0x18
    struct xVec3 * offset; // r1+0x14
    class zMeshCircle * circle; // r23
    class zMeshCircle * circle; // r22
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x8051F93C
