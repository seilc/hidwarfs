/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zCombat.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x802CA8E8 -> 0x802CD804
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static unsigned char hasAddedToCount; // size: 0x1, address: 0x80D6B600
static char * combatBoundsDrawString[4]; // size: 0x10, address: 0x80D668D0
static int drawCombatBounds; // size: 0x4, address: 0x80D6B604
static unsigned char drawCombatAttack; // size: 0x1, address: 0x80D6B608
// total size: 0x8
struct substr {
    // Members
...
// total size: 0x1
struct /* @class$737zCombat_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$738zCombat_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$739zCombat_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$740zCombat_cpp */ {
    // Members
...
// Range: 0x802CA8E8 -> 0x802CA920
static void ExplodeCombatTweaks(const struct tweak_info & info /* r31 */) {}

// Range: 0x802CA920 -> 0x802CA998
void zCombatInit() {
    // References
    // -> static unsigned char drawCombatAttack;
    // -> static char * combatBoundsDrawString[4];
    // -> static int drawCombatBounds;
}
...
// total size: 0x10
class CheckForCollisionWrapper {
    // Functions
    unsigned char operator()(struct xEnt & ent) const;

    // Members
    struct xEnt * attacker; // offset 0x0, size 0x4
    struct xSweptSphere * sphere; // offset 0x4, size 0x4
    struct zCombat * combat; // offset 0x8, size 0x4
    const struct xVec3 * move_dir; // offset 0xC, size 0x4
};
// Range: 0x802CA998 -> 0x802CA9FC
// this: r31
unsigned char CheckForCollisionWrapper::operator()(struct xEnt & ent /* r30 */) const {}
...
struct zCombat {
    // Functions
    int Setup(const struct xHierarchyBoundInitData * initHierarchy, unsigned int boundsNumber);

    void Init(const struct xHierarchyBoundInitData * initHierarchy, float masterRadius, const class zCombatAttack * stateTable, unsigned short stateTableSize, int hitPoints, float scale, const char * baseName, const char * * boneNames, int boneCount);

    unsigned char ShouldRunEffect();

    class zCombatAttack * FindAttackState(unsigned int id);

    void PostUpdate(struct xEnt * ent, float dt);

    void SetupTweaks(const char * base, struct xEnt * ent, const char * * bones, int boneCount);

    void ExplodeCombatTweaks(const char * base, int baseSize);

    void UpdateBounds(struct xEnt * ent);

    void SendObjectHit(struct xEnt * attacker, struct xEnt * target, struct xSurface * surface, enum zHitTarget hitTarget, const struct xVec3 * direction, const struct xVec3 * hit_loc, const struct xVec3 * hit_norm, const struct xVec3 * move_dir, unsigned char hitEffect);

    void CheckForCollision(struct xEnt * attacker, struct xEnt * target, struct xSweptSphere * sphere, const struct xVec3 * hitDirection);

    void CheckForCollisionEnv(struct xEnt * attacker, struct xEnv * env, struct xSweptSphere * sphere, const struct xVec3 * hitDirection);

    void SweepSphere(struct xEnt * attacker, struct xVec3 * pos1, struct xVec3 * pos2, float radius);

    void CheckForHit(struct xEnt * attacker);

    void StartEffect(struct xEnt * attacker);

    void StopEffect();

    void RenderEffectModel();

    void RenderAfterEffect();

    void UpdateEffect(struct xEnt * attacker, float dt);

    void ContinueEffect();

    void StartBlur(struct xEnt * attacker);

    void StopBlur(struct xEnt * attacker);

    void UpdateBlur(struct xEnt * attacker);
...
// Range: 0x802CA9FC -> 0x802CAA5C
// this: r30
int zCombat::Setup(const struct xHierarchyBoundInitData * initHierarchy /* r31 */, unsigned int boundsNumber /* r1+0x8 */) {}
...
// Range: 0x802CAA5C -> 0x802CAB64
// this: r31
void zCombat::Init(const struct xHierarchyBoundInitData * initHierarchy /* r30 */, float masterRadius /* r1+0x8 */, const class zCombatAttack * stateTable /* r1+0xC */, unsigned short stateTableSize /* r1+0x10 */, int hitPoints /* r26 */, float scale /* r1+0x14 */, const char * baseName /* r27 */, const char * * boneNames /* r28 */, int boneCount /* r29 */) {
    // Local variables
    char name[128]; // r1+0x18
}

// Range: 0x802CAB64 -> 0x802CAB68
void zCombat::PreUpdate() {}

// Range: 0x802CAB68 -> 0x802CAC1C
// this: r3
unsigned char zCombat::ShouldRunEffect() {}

// Range: 0x802CAC1C -> 0x802CAC78
// this: r3
class zCombatAttack * zCombat::FindAttackState(unsigned int id /* r4 */) {
    // Local variables
    const class zCombatAttack * result; // r30
    int i; // r31
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D668E0
// total size: 0x18
struct analog_data {
    // Members
...
// Range: 0x802CAC78 -> 0x802CB400
// this: r31
void zCombat::PostUpdate(struct xEnt * ent /* r30 */, float dt /* f31 */) {
    // Local variables
    int i; // r27
    struct xHierarchyNode & node; // r26
    struct effect * rumble; // r20
    unsigned char needToContinue; // r25
    unsigned char resetSphere; // r24
    const struct xAnimState * curState; // r23
    const class zCombatAttack * temp; // r22
    float newTime; // f30
    int i; // r29
    struct xMat4x3 * mat; // r19
    unsigned char ranAttack; // r21
    int i; // r28

    // References
    // -> static unsigned char drawCombatAttack;
    // -> static unsigned char hasAddedToCount;
...
// total size: 0x4
struct /* @class$2113zCombat_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2114zCombat_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// Range: 0x802CB400 -> 0x802CB47C
static void GetBonePos(struct xVec3 & pos /* r31 */, int boneNumber /* r29 */, const struct xMat4x3 * mats /* r30 */) {}
...
static struct tweak_callback cb; // size: 0x28, address: 0x80528590
static signed char init; // size: 0x1, address: 0x80D6B609
// Range: 0x802CB47C -> 0x802CB56C
// this: r31
void zCombat::SetupTweaks(const char * base /* r1+0x8 */, struct xEnt * ent /* r1+0xC */, const char * * bones /* r1+0x10 */, int boneCount /* r1+0x14 */) {
    // References
    // -> static struct tweak_callback cb;
    // -> static signed char init;
}

// Range: 0x802CB56C -> 0x802CBC78
// this: r31
void zCombat::ExplodeCombatTweaks(const char * base /* r1+0x8 */, int baseSize /* r30 */) {
    // Local variables
    char buffer[128]; // r1+0x4C
    int i; // r27
    class zCombatAttack * asset; // r29
    char statename[64]; // r1+0xC
    struct xAnimState * state; // r26
    int i; // r28
}

static struct zCombatDamageInfo * damage_info_stack[4]; // size: 0x10, address: 0x80D6B60C
static int damage_info_stack_count; // size: 0x4, address: 0x80D6B61C
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D668EB
// Range: 0x802CBC78 -> 0x802CBD2C
static void PushDamageInfo(const struct zCombatDamageInfo * info /* r1+0x8 */) {
    // References
    // -> static int damage_info_stack_count;
    // -> static struct zCombatDamageInfo * damage_info_stack[4];
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D668FA
// Range: 0x802CBD2C -> 0x802CBDB8
static void PopDamageInfo() {
    // References
    // -> static int damage_info_stack_count;
    // -> static char __FUNCTION__[14];
}

// Range: 0x802CBDB8 -> 0x802CBFD0
void zCombatDamage(struct xEnt * ent /* r30 */, const struct zCombatDamageInfo & damageInfo /* r31 */) {
    // Local variables
    float params[4]; // r1+0x8
    unsigned int toEvent; // r29
}
...
// Range: 0x802CBFD0 -> 0x802CC024
unsigned int zCombatGetHitTarget(struct xEnt * attacker /* r30 */, struct xEnt * target /* r1+0x8 */) {
    // Local variables
    enum zHitTarget hitTarget; // r31
}
...
// Range: 0x802CC024 -> 0x802CC1A8
unsigned int zCombatGetHitTarget(struct xVec3 * attackerPos /* r29 */, struct xEnt * target /* r31 */) {
    // Local variables
    enum zHitTarget hitTarget; // r30
    struct xVec3 attack_dir; // r1+0x8
    float dot; // f31
...
// Range: 0x802CC1A8 -> 0x802CC240
void zCombat::UpdateSphere(struct xSphere * sphere /* r30 */, int boneNumber /* r29 */, const struct xMat4x3 * mats /* r31 */, const struct xVec3 * boneOffset /* r1+0x8 */) {
    // Local variables
    struct xVec3 offset; // r1+0x18
    struct xVec3 bone; // r1+0xC
}

struct xGlobals * xglobals; // size: 0x4, address: 0x80D63020
// Range: 0x802CC240 -> 0x802CC3F8
// this: r31
void zCombat::UpdateBounds(struct xEnt * ent /* r1+0x20 */) {
    // Local variables
    const struct xVec3 * pos; // r23
    const struct xVec3 * playerCenter; // r22
    struct xVec3 objectDirection; // r1+0x24
    float length2; // f31
    unsigned char rs; // r30
...
static unsigned char done; // size: 0x1, address: 0x80D6B620
static signed char init; // size: 0x1, address: 0x80D6B621
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D66908
...
// Range: 0x802CC3F8 -> 0x802CC774
// this: r31
void zCombat::SendObjectHit(struct xEnt * attacker /* r27 */, struct xEnt * target /* r30 */, struct xSurface * surface /* r1+0x10 */, enum zHitTarget hitTarget /* r1+0x14 */, const struct xVec3 * direction /* r23 */, const struct xVec3 * hit_loc /* r26 */, const struct xVec3 * hit_norm /* r24 */, const struct xVec3 * move_dir /* r1+0x98 */, unsigned char hitEffect /* r25 */) {
    // Local variables
    int i; // r29
    unsigned char connect; // r28
    struct xVec3 dir; // r1+0x18
    struct zCombatDamageInfo params; // r1+0x24
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D66916
// Range: 0x802CC774 -> 0x802CCC1C
// this: r26
void zCombat::CheckForCollision(struct xEnt * attacker /* r30 */, struct xEnt * target /* r31 */, struct xSweptSphere * sphere /* r29 */, const struct xVec3 * hitDirection /* r1+0x10 */) {
    // Local variables
    unsigned char isPlayer; // r23
    unsigned char hitIt; // r28
    enum zHitTarget hitTarget; // r27
    struct zCombat * targetCombat; // r25
    signed char subsphere; // r1+0x14
    unsigned short hitTargetU16; // r1+0x16
    struct xVec3 attack_dir; // r1+0x3C
    float dot; // f31
    struct xVec3 direction; // r1+0x30
    struct xVec3 hitLoc; // r1+0x24
    struct xVec3 hitNorm; // r1+0x18
    unsigned char hitEffect; // r24
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x805285B8
// total size: 0x1C
...
// Range: 0x802CCC1C -> 0x802CCE10
// this: r31
void zCombat::CheckForCollisionEnv(struct xEnt * attacker /* r27 */, struct xEnv * env /* r21 */, struct xSweptSphere * sphere /* r30 */, const struct xVec3 * hitDirection /* r1+0x10 */) {
    // Local variables
    struct xVec3 hdng; // r1+0x14
    unsigned char oldHitEnv; // r24
    unsigned char firstContact; // r23
    struct xSurface * pXSurface; // r26
    struct zSurfaceProps * prop; // r29
    struct zSurfAssetIN * pZSurface; // r28
    enum iSndGroupHandle pSmackSound; // r22
...
// Range: 0x802CCE10 -> 0x802CD00C
// this: r26
void zCombat::SweepSphere(struct xEnt * attacker /* r27 */, struct xVec3 * pos1 /* r28 */, struct xVec3 * pos2 /* r29 */, float radius /* f30 */) {
    // Local variables
    struct xVec3 moveDirection; // r1+0x38
    float moveMagnitude2; // f31
    struct xVec3 * moveDirectionPtr; // r30
    struct xSweptSphere swept; // r1+0x90
    struct xSphere previousSphere; // r1+0x28
    struct xSphere sphere; // r1+0x18
    int i; // r31
    struct xBound tempBound; // r1+0x44
    class CheckForCollisionWrapper callback; // r1+0x8
...
// Range: 0x802CD00C -> 0x802CD0D0
// this: r31
void zCombat::CheckForHit(struct xEnt * attacker /* r1+0x8 */) {
    // Local variables
    int i; // r30
    float radius; // f31
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D66928
...
// Range: 0x802CD0D0 -> 0x802CD294
// this: r31
void zCombat::StartEffect(struct xEnt * attacker /* r30 */) {
    // Local variables
    enum zCommonPlayerUpgradeLevel meleeUpgrade; // r29
...
// Range: 0x802CD294 -> 0x802CD2E4
// this: r31
void zCombat::StopEffect() {}

// Range: 0x802CD2E4 -> 0x802CD330
// this: r31
void zCombat::RenderEffectModel() {}

// Range: 0x802CD330 -> 0x802CD35C
// this: r1+0x8
void zCombat::RenderAfterEffect() {}
...
static struct xFXRing sBodySlamRing[3]; // size: 0xC0, address: 0x805285D0
// Range: 0x802CD35C -> 0x802CD518
// this: r31
void zCombat::UpdateEffect(struct xEnt * attacker /* r29 */, float dt /* f30 */) {
    // Local variables
    float startDiff; // f29
    const struct xVec3 * center; // r30
    float radius; // f31

    // References
    // -> static struct xFXRing sBodySlamRing[3];
}

// Range: 0x802CD518 -> 0x802CD55C
// this: r31
void zCombat::ContinueEffect() {}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D66934
// Range: 0x802CD55C -> 0x802CD654
// this: r31
void zCombat::StartBlur(struct xEnt * attacker /* r30 */) {
    // References
    // -> static char __FUNCTION__[10];
}

// Range: 0x802CD654 -> 0x802CD6D4
// this: r31
void zCombat::StopBlur(struct xEnt * attacker /* r30 */) {}

// Range: 0x802CD6D4 -> 0x802CD804
// this: r31
void zCombat::UpdateBlur(struct xEnt * attacker /* r30 */) {
    // Local variables
    unsigned char run; // r29
}
