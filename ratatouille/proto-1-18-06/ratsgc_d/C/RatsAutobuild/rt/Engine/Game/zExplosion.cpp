/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zExplosion.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80158618 -> 0x8015F848
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static char ini_filename[12]; // size: 0xC, address: 0x80D6DF30
static float decal_cast_dist; // size: 0x4, address: 0x80D628C0
static int type; // size: 0x4, address: 0x80D6A080
static float repeat_every; // size: 0x4, address: 0x80D6A084
static float emit_height; // size: 0x4, address: 0x80D628C4
static float emit_random_offset; // size: 0x4, address: 0x80D628C8
static int type; // size: 0x4, address: 0x80D6A088
static unsigned char active; // size: 0x1, address: 0x80D6A08C
static unsigned char bind_to_player; // size: 0x1, address: 0x80D628CC
static float height; // size: 0x4, address: 0x80D628D0
static float radius_inner; // size: 0x4, address: 0x80D628D4
static float radius_outer; // size: 0x4, address: 0x80D628D8
static float freq; // size: 0x4, address: 0x80D628DC
static float duration; // size: 0x4, address: 0x80D628E0
static int type; // size: 0x4, address: 0x80D6A090
static float rate; // size: 0x4, address: 0x80D6A094
static float life; // size: 0x4, address: 0x80D628E4
static float width; // size: 0x4, address: 0x80D628E8
static float length; // size: 0x4, address: 0x80D628EC
static float vel; // size: 0x4, address: 0x80D628F0
static float radius; // size: 0x4, address: 0x80D6A098
static float gravity; // size: 0x4, address: 0x80D628F4
static float alpha; // size: 0x4, address: 0x80D628F8
static float height_above_player; // size: 0x4, address: 0x80D628FC
static int type; // size: 0x4, address: 0x80D6A09C
static float rate; // size: 0x4, address: 0x80D6A0A0
static float life; // size: 0x4, address: 0x80D62900
static float scale; // size: 0x4, address: 0x80D62904
static float vel_min; // size: 0x4, address: 0x80D62908
static float vel_max; // size: 0x4, address: 0x80D6290C
static float radius; // size: 0x4, address: 0x80D6A0A4
static float gravity; // size: 0x4, address: 0x80D62910
static float alpha; // size: 0x4, address: 0x80D62914
static float height_above_player; // size: 0x4, address: 0x80D62918
static unsigned char render_fire; // size: 0x1, address: 0x80D6291C
static unsigned char render_smoke_backface; // size: 0x1, address: 0x80D6291D
static unsigned char render_smoke_frontface; // size: 0x1, address: 0x80D6291E
static unsigned char render_streaks; // size: 0x1, address: 0x80D6291F
static unsigned char render_sparks; // size: 0x1, address: 0x80D62920
static unsigned char render_presence; // size: 0x1, address: 0x80D6A0A8
static unsigned char invert_front_back; // size: 0x1, address: 0x80D6A0A9
// total size: 0x8
struct substr {
    // Members
...
// total size: 0xC
struct /* @class$791zExplosion_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$792zExplosion_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$793zExplosion_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$794zExplosion_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$795zExplosion_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$796zExplosion_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$797zExplosion_cpp */ {
    // Members
...
static struct tweak_callback cb_debug_emit; // size: 0x28, address: 0x804A6D98
// Range: 0x80158618 -> 0x80158D50
static void add_tweaks(const char * prefix /* r31 */) {
    // References
    // -> static unsigned char invert_front_back;
    // -> static unsigned char render_presence;
    // -> static unsigned char render_sparks;
    // -> static unsigned char render_streaks;
    // -> static unsigned char render_smoke_frontface;
    // -> static unsigned char render_smoke_backface;
    // -> static unsigned char render_fire;
    // -> static float height_above_player;
    // -> static float alpha;
    ...
    // -> static float freq;
    // -> static float radius_outer;
    // -> static float radius_inner;
    // -> static float height;
    // -> static unsigned char bind_to_player;
    // -> static unsigned char active;
    // -> static float emit_random_offset;
    // -> static float emit_height;
    // -> static float repeat_every;
    // -> static struct tweak_callback cb_debug_emit;
    // -> static float decal_cast_dist;
}

static unsigned int decal_type_ids[4]; // size: 0x10, address: 0x80D6DF4C
static int decal_blend_mode[3]; // size: 0xC, address: 0x80D6DF5C
static int decal_flags[3]; // size: 0xC, address: 0x80D6DF68
// total size: 0x18
struct animix {
    // Members
...
static struct curve_node decal_anim_impact[3]; // size: 0x54, address: 0x804A6DC0
static struct curve_node decal_anim_burn[7]; // size: 0xC4, address: 0x804A6E14
static struct curve_node decal_anim_plasma[3]; // size: 0x54, address: 0x804A6ED8
static struct xResponseCurve decal_curve[3]; // size: 0x3C, address: 0x80CF13E0
// total size: 0x14
struct anim_data {
    // Members
    float alpha; // offset 0x0, size 0x4
    float scale; // offset 0x4, size 0x4
    float heat; // offset 0x8, size 0x4
    float rise; // offset 0xC, size 0x4
    float glow; // offset 0x10, size 0x4
};
// total size: 0x40
struct mote {
    // Members
    unsigned char type; // offset 0x0, size 0x1
    unsigned char material_index; // offset 0x1, size 0x1
    ...
    float age_rate; // offset 0x20, size 0x4
    float scale; // offset 0x24, size 0x4
    float vel; // offset 0x28, size 0x4
    struct anim_data anim; // offset 0x2C, size 0x14
};
static struct mote * motes; // size: 0x4, address: 0x80D6A0AC
static int motes_used; // size: 0x4, address: 0x80D6A0B0
// total size: 0x40
struct streak_particle {
    ...
    unsigned char group_index; // offset 0x20, size 0x1
    unsigned char pad1; // offset 0x21, size 0x1
    unsigned char pad2; // offset 0x22, size 0x1
    unsigned char pad3; // offset 0x23, size 0x1
    float life; // offset 0x24, size 0x4
    float alpha; // offset 0x28, size 0x4
    float alpha_vel; // offset 0x2C, size 0x4
    float length_vel; // offset 0x30, size 0x4
    float length_max; // offset 0x34, size 0x4
    float gravity; // offset 0x38, size 0x4
    unsigned int pad4; // offset 0x3C, size 0x4
};
static struct streak_particle * streaks; // size: 0x4, address: 0x80D6A0B4
static int streaks_used; // size: 0x4, address: 0x80D6A0B8
// total size: 0x1C
struct streak_emitter {
    // Members
...
static struct streak_emitter * streak_emitters; // size: 0x4, address: 0x80D6A0BC
static int streak_emitters_used; // size: 0x4, address: 0x80D6A0C0
// total size: 0x34
struct RwRaster {
    // Members
...
static struct streak_group * streak_groups; // size: 0x4, address: 0x80D6A0C4
static int streak_groups_used; // size: 0x4, address: 0x80D6A0C8
// total size: 0x30
struct spark_particle {
    ...
    float alpha_vel; // offset 0x28, size 0x4
    float gravity; // offset 0x2C, size 0x4
};
static struct spark_particle * sparks; // size: 0x4, address: 0x80D6A0CC
static int sparks_used; // size: 0x4, address: 0x80D6A0D0
// total size: 0x1C
struct spark_emitter {
    // Members
    int type; // offset 0x0, size 0x4
    struct xVec3 loc; // offset 0x4, size 0xC
    float emitted; // offset 0x10, size 0x4
...
static struct spark_emitter * spark_emitters; // size: 0x4, address: 0x80D6A0D4
static int spark_emitters_used; // size: 0x4, address: 0x80D6A0D8
// total size: 0x8
struct spark_group {
    // Members
    struct RwTexture * texture; // offset 0x0, size 0x4
...
static struct spark_group * spark_groups; // size: 0x4, address: 0x80D6A0DC
static int spark_groups_used; // size: 0x4, address: 0x80D6A0E0
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
struct curve_node {
    // Members
    float t; // offset 0x0, size 0x4
    struct anim_data anim; // offset 0x4, size 0x14
};
// total size: 0xC
struct glare_config {
    // Members
    ...
    float scale; // offset 0x4, size 0x4
    struct xColor_tag color; // offset 0x8, size 0x4
};
// total size: 0x14
struct /* @class$2092zExplosion_cpp */ {
    // Members
...
struct streak_config {
    ...
    float emit_delay; // offset 0x8, size 0x4
    float emit_time; // offset 0xC, size 0x4
...
struct shake_config {
    ...
    float mag; // offset 0x4, size 0x4
    float cycle; // offset 0x8, size 0x4
    float rot_mag; // offset 0xC, size 0x4
    float radius; // offset 0x10, size 0x4
};
// total size: 0x24
struct presence_config {
    // Members
    unsigned int hurt_whom; // offset 0x0, size 0x4
    float delay; // offset 0x4, size 0x4
    float life; // offset 0x8, size 0x4
    float knockback; // offset 0xC, size 0x4
    float radius_begin; // offset 0x10, size 0x4
    float radius_end; // offset 0x14, size 0x4
    float damage_player; // offset 0x18, size 0x4
    float damage_npc; // offset 0x1C, size 0x4
    float damage_other; // offset 0x20, size 0x4
};
// total size: 0x14
struct decal_config {
    // Members
...
// total size: 0x18
struct /* @class$2093zExplosion_cpp */ {
    // Members
    unsigned int fire; // offset 0x0, size 0x4
    unsigned int smoke; // offset 0x4, size 0x4
    unsigned int streak; // offset 0x8, size 0x4
    unsigned int spark; // offset 0xC, size 0x4
    unsigned int glare; // offset 0x10, size 0x4
    unsigned int decal; // offset 0x14, size 0x4
};
// total size: 0x150
struct ini_config {
    // Members
    char name[64]; // offset 0x0, size 0x40
    float vel_decay; // offset 0x40, size 0x4
    float emit_radius; // offset 0x44, size 0x4
    unsigned int motes_min; // offset 0x48, size 0x4
    unsigned int motes_max; // offset 0x4C, size 0x4
    float life_min; // offset 0x50, size 0x4
    float life_max; // offset 0x54, size 0x4
    float scale_min; // offset 0x58, size 0x4
    float scale_max; // offset 0x5C, size 0x4
    float scale_fire; // offset 0x60, size 0x4
    float scale_smoke; // offset 0x64, size 0x4
    float vel_min; // offset 0x68, size 0x4
    float vel_max; // offset 0x6C, size 0x4
    unsigned int sound; // offset 0x70, size 0x4
    struct curve_node * anim; // offset 0x74, size 0x4
    unsigned int anim_size; // offset 0x78, size 0x4
    struct glare_config glare; // offset 0x7C, size 0xC
    // total size: 0x14
    struct /* @class$2092zExplosion_cpp */ {
        // Members
        unsigned int type; // offset 0x0, size 0x4
        ...
        float intensity; // offset 0xC, size 0x4
        float freq; // offset 0x10, size 0x4
    } warp; // offset 0x88, size 0x14
    struct streak_config streak; // offset 0x9C, size 0x28
    struct spark_config spark; // offset 0xC4, size 0x28
    struct shake_config shake; // offset 0xEC, size 0x14
    struct presence_config presence; // offset 0x100, size 0x24
    struct decal_config decal; // offset 0x124, size 0x14
    // total size: 0x18
    struct /* @class$2093zExplosion_cpp */ {
        // Members
        unsigned int fire; // offset 0x0, size 0x4
        ...
        unsigned int glare; // offset 0x10, size 0x4
        unsigned int decal; // offset 0x14, size 0x4
    } texture; // offset 0x138, size 0x18
};
static struct ini_config * ini_configs; // size: 0x4, address: 0x80D6A0E4
enum iSndGroupHandle {
};
// total size: 0x14
struct /* @class$2094zExplosion_cpp */ {
    // Members
    int type; // offset 0x0, size 0x4
    float radius; // offset 0x4, size 0x4
    float duration; // offset 0x8, size 0x4
...
// total size: 0x18
struct /* @class$2095zExplosion_cpp */ {
    // Members
    struct RwTexture * fire; // offset 0x0, size 0x4
    struct RwTexture * smoke; // offset 0x4, size 0x4
    struct RwTexture * streak; // offset 0x8, size 0x4
    struct RwTexture * spark; // offset 0xC, size 0x4
    struct RwTexture * glare; // offset 0x10, size 0x4
    struct RwTexture * decal; // offset 0x14, size 0x4
};
// total size: 0x130
struct runtime_config {
    // Members
    unsigned int id; // offset 0x0, size 0x4
    float vel_decay; // offset 0x4, size 0x4
    float emit_radius; // offset 0x8, size 0x4
    unsigned int motes_min; // offset 0xC, size 0x4
    unsigned int motes_max; // offset 0x10, size 0x4
    float life_min; // offset 0x14, size 0x4
    float life_max; // offset 0x18, size 0x4
    float scale_min; // offset 0x1C, size 0x4
    float scale_max; // offset 0x20, size 0x4
    float scale_fire; // offset 0x24, size 0x4
    float scale_smoke; // offset 0x28, size 0x4
    float vel_min; // offset 0x2C, size 0x4
    float vel_max; // offset 0x30, size 0x4
    enum iSndGroupHandle sound; // offset 0x34, size 0x4
    int material_index; // offset 0x38, size 0x4
    int streak_group_index; // offset 0x3C, size 0x4
    int spark_group_index; // offset 0x40, size 0x4
    struct xResponseCurve anim; // offset 0x44, size 0x14
    struct glare_config glare; // offset 0x58, size 0xC
    // total size: 0x14
    struct /* @class$2094zExplosion_cpp */ {
        // Members
        int type; // offset 0x0, size 0x4
        ...
        float intensity; // offset 0xC, size 0x4
        float freq; // offset 0x10, size 0x4
    } warp; // offset 0x64, size 0x14
    struct streak_config streak; // offset 0x78, size 0x28
    struct spark_config spark; // offset 0xA0, size 0x28
    struct shake_config shake; // offset 0xC8, size 0x14
    struct presence_config presence; // offset 0xDC, size 0x24
    struct decal_config decal; // offset 0x100, size 0x14
    // total size: 0x18
    struct /* @class$2095zExplosion_cpp */ {
        // Members
        struct RwTexture * fire; // offset 0x0, size 0x4
        ...
        struct RwTexture * glare; // offset 0x10, size 0x4
        struct RwTexture * decal; // offset 0x14, size 0x4
    } texture; // offset 0x114, size 0x18
    float vel_decay_mult; // offset 0x12C, size 0x4
};
static struct runtime_config * runtime_configs; // size: 0x4, address: 0x80D6A0E8
// total size: 0x8
struct material_group {
    // Members
    struct RwTexture * texture_fire; // offset 0x0, size 0x4
    struct RwTexture * texture_smoke; // offset 0x4, size 0x4
};
static struct material_group * materials; // size: 0x4, address: 0x80D6A0EC
static unsigned int configs_size; // size: 0x4, address: 0x80D6A0F0
static int materials_used; // size: 0x4, address: 0x80D6A0F4
// total size: 0x20
...
// total size: 0x3C
struct presence_node : public presence_volume {
    // Members
    int flags; // offset 0x20, size 0x4
    float delay; // offset 0x24, size 0x4
    float time; // offset 0x28, size 0x4
    float end_time; // offset 0x2C, size 0x4
    float radius_vel; // offset 0x30, size 0x4
    union entry_data * space_entry; // offset 0x34, size 0x4
    struct trail_emitter * trail; // offset 0x38, size 0x4
};
static struct presence_node presi[32]; // size: 0x780, address: 0x80CF141C
static int presi_size; // size: 0x4, address: 0x80D6A0F8
// Range: 0x80158D50 -> 0x80158DA8
static int find_decal_type(unsigned int id /* r3 */) {
    // Local variables
    const unsigned int * it; // r31
    const unsigned int * end; // r30

    // References
    // -> static unsigned int decal_type_ids[4];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D62921
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6292E
static char __FUNCTION__[21]; // size: 0x15, address: 0x8051EA48
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D6293A
// Range: 0x80158DA8 -> 0x80158E64
static void spawn_streak_emitter(int type /* r29 */, const struct xVec3 & loc /* r1+0x8 */) {
    // Local variables
    const struct runtime_config & cfg; // r31
    struct streak_emitter * emitter; // r30

    // References
    // -> static struct runtime_config * runtime_configs;
    // -> static int streak_emitters_used;
}

// Range: 0x80158E64 -> 0x80158F10
static struct streak_emitter * alloc_streak_emitter() {
    // Local variables
    struct streak_emitter * se; // r31

    // References
    // -> static int streak_emitters_used;
    // -> static struct streak_emitter * streak_emitters;
    // -> static char __FUNCTION__[21];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6294E
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D6295A
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D62965
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D62979
// Range: 0x80158F10 -> 0x80158FCC
static void spawn_spark_emitter(int type /* r29 */, const struct xVec3 & loc /* r1+0x8 */) {
    // Local variables
    const struct runtime_config & cfg; // r31
    struct spark_emitter * emitter; // r30

    // References
    // -> static struct runtime_config * runtime_configs;
    // -> static int spark_emitters_used;
}

// Range: 0x80158FCC -> 0x80159074
static struct spark_emitter * alloc_spark_emitter() {
    // Local variables
    struct spark_emitter * se; // r31

    // References
    // -> static int spark_emitters_used;
    // -> static struct spark_emitter * spark_emitters;
    // -> static char __FUNCTION__[20];
}

// Range: 0x80159074 -> 0x8015912C
static void random_direction(struct xVec3 & dir /* r1+0x8 */) {
    // Local variables
    float ang; // f30
    float uz; // f31
    float r; // f29
...
// Range: 0x8015912C -> 0x8015921C
static void random_sphere_offset(struct xVec3 & offset /* r1+0x8 */, float radius /* r1+0xC */) {
    // Local variables
    float ang; // f29
    float uz; // f30
    float r; // f28
...
// Range: 0x8015921C -> 0x80159378
static void launch_sphere(int type /* r26 */, int size /* r27 */, const struct xVec3 & loc /* r29 */) {
    // Local variables
    const struct runtime_config & cfg; // r30
    struct mote * m; // r31
    struct mote * endm; // r28
    float r; // f31
    float rv; // f30
    float life; // f29

    // References
    // -> static int motes_used;
    // -> static struct mote * motes;
    // -> static struct runtime_config * runtime_configs;
}
...
// Range: 0x80159378 -> 0x80159638
static void launch_line(int type /* r25 */, int size /* r26 */, const struct xVec3 & loc0 /* r28 */, const struct xVec3 & offset /* r29 */, float age_delta /* r1+0x10 */) {
    // Local variables
    const struct runtime_config & cfg; // r30
    struct xMat3x3 lookmat; // r1+0x50
    struct mote * m; // r31
    struct mote * endm; // r27
    float s; // f31
    float theta; // f27
...
static unsigned char already_asserted; // size: 0x1, address: 0x80D6A0FC
static signed char init; // size: 0x1, address: 0x80D6A0FD
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D6298C
// Range: 0x80159638 -> 0x80159828
static void emit_streaks(const struct runtime_config & cfg /* r30 */, const struct xVec3 & loc /* r1+0x8 */, int amount /* r26 */) {
    // Local variables
    int remaining; // r28
    int group; // r27
    float alpha_vel; // f30
    int i; // r29
    struct streak_particle * streak; // r31
    float r; // f31
    ...
    // -> static int streaks_used;
}

// Range: 0x80159828 -> 0x80159968
static struct streak_particle * alloc_streak(int group_index /* r26 */) {
    // Local variables
    struct streak_group & group; // r30
    struct streak_particle * streak; // r29
    struct streak_particle * tail; // r28
    struct streak_group * mg; // r31
    struct streak_group * endmg; // r25
    struct streak_particle * next; // r27

    // References
    // -> static int streaks_used;
    // -> static struct streak_particle * streaks;
    // -> static struct streak_group * streak_groups;
    // -> static int streak_groups_used;
    // -> static char __FUNCTION__[13];
}

// Range: 0x80159A04 -> 0x80159B90
static void emit_sparks(const struct runtime_config & cfg /* r31 */, const struct xVec3 & loc /* r1+0x8 */, int amount /* r26 */) {
    // Local variables
    int remaining; // r28
    int group; // r27
    float alpha_vel; // f30
    float velrange; // f29
    int i; // r29
    struct spark_particle * spark; // r30
    float r; // f31
    float velmag; // f28

    // References
    // -> static int sparks_used;
}

// Range: 0x80159B90 -> 0x80159CD0
static struct spark_particle * alloc_spark(int group_index /* r26 */) {
    // Local variables
    struct spark_group & group; // r30
    struct spark_particle * spark; // r29
    struct spark_particle * tail; // r28
    struct spark_group * mg; // r31
    struct spark_group * endmg; // r25
    struct spark_particle * next; // r27

    // References
    // -> static int sparks_used;
    // -> static struct spark_particle * sparks;
    // -> static struct spark_group * spark_groups;
    // -> static int spark_groups_used;
...
// total size: 0x4
struct /* @class$1063zExplosion_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1227zExplosion_cpp */ {
    // Members
    float t; // offset 0x0, size 0x4
...
// total size: 0x10
struct /* @class$934zExplosion_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
struct ordered_mote {
    // Members
    struct xVec3 loc; // offset 0x0, size 0xC
    float scale_fire; // offset 0xC, size 0x4
    unsigned int sort_key; // offset 0x10, size 0x4
    float scale_smoke; // offset 0x14, size 0x4
    unsigned char type; // offset 0x18, size 0x1
    unsigned char material_index; // offset 0x19, size 0x1
    unsigned char alpha_fire; // offset 0x1A, size 0x1
    unsigned char alpha_smoke; // offset 0x1B, size 0x1
    unsigned char dirsign; // offset 0x1C, size 0x1
    unsigned char glow; // offset 0x1D, size 0x1
...
// Range: 0x80159D4C -> 0x8015A15C
static struct ordered_mote * transform_motes(struct ordered_mote * om /* r31 */) {
    // Local variables
    struct RwFrame * cam_frame; // r26
    ...
    float cam_depth; // f27
    struct mote * m; // r30
    struct mote * endm; // r25
    const struct runtime_config & cfg; // r28
    float scale; // f31
    struct RwSphere worldsph; // r1+0x1C
    float alpha; // f26
    struct RwSphere worldsph; // r1+0xC
    float alpha; // f25
    float glow; // f24
    float alpha; // f23
    float alpha; // f22
...
// Range: 0x8015A15C -> 0x8015A1C0
static unsigned char fast_cull(struct ordered_mote * om /* r31 */) {
    // Local variables
...
// total size: 0x30
struct ptank_pool__color_mat : public ptank_pool {
    // Members
    struct xColor_tag * color; // offset 0x20, size 0x4
    struct xMat4x3 * mat; // offset 0x24, size 0x4
    int stride_color; // offset 0x28, size 0x4
    int stride_mat; // offset 0x2C, size 0x4
};
// Range: 0x8015A1C0 -> 0x8015A594
static void ptank_render_streaks() {
    // Local variables
    struct RwFrame * cam_frame; // r22
    const struct xMat4x3 & cam_mat; // r27
    struct ptank_pool__color_mat pool; // r1+0x2C
    unsigned char min_alpha; // r1+0x8
    float Ax; // f23
    float Ay; // f22
    float Az; // f21
    struct streak_group * group; // r29
    struct streak_group * end_group; // r21
    struct ptank_pool__color_mat & _fptank_pool; // r31
    int _fptank_used; // r26
    struct xColor_tag * pool_color; // r25
    struct xMat4x3 * pool_mat; // r28
    struct streak_particle * streak; // r30
    struct streak_particle * end_streak; // r20
    struct RwSphere worldsph; // r1+0x1C
    float Vx; // f31
    float Vy; // f30
    float Vz; // f29
...
// Range: 0x8015A594 -> 0x8015A75C
static void ptank_render_sparks() {
    // Local variables
    struct RwFrame * cam_frame; // r25
    const struct xMat4x3 & cam_mat; // r1+0x8
    struct ptank_pool__pos_color_size pool; // r1+0x1C
    unsigned char min_alpha; // r28
    struct spark_group * group; // r30
    struct spark_group * end_group; // r24
    struct spark_particle * spark; // r31
    struct spark_particle * end_spark; // r23
    struct RwSphere worldsph; // r1+0xC
...
// Range: 0x8015A75C -> 0x8015A86C
static void ptank_render() {
    // Local variables
    struct ordered_mote * om_buffer; // r31
    const struct ordered_mote * end_om; // r30
    int total; // r29
    struct ptank_pool__pos_color_size pool; // r1+0x8
...
// Range: 0x8015A86C -> 0x8015A9F8
static void ptank_render_smoke_frontface(struct ptank_pool__pos_color_size & pool /* r31 */, const struct ordered_mote * om /* r30 */, const struct ordered_mote * end_om /* r1+0x8 */) {
    // Local variables
    int material_index; // r28
    const struct runtime_config & cfg; // r29
...
// Range: 0x8015A9F8 -> 0x8015AB64
static void ptank_render_fire(struct ptank_pool__pos_color_size & pool /* r31 */, const struct ordered_mote * om /* r30 */, const struct ordered_mote * end_om /* r1+0x8 */) {
    // Local variables
    int material_index; // r28
    unsigned char min_glow; // r27
    const struct runtime_config & cfg; // r29
...
// Range: 0x8015AB64 -> 0x8015ACF0
static void ptank_render_smoke_backface(struct ptank_pool__pos_color_size & pool /* r31 */, const struct ordered_mote * om /* r30 */, const struct ordered_mote * end_om /* r1+0x8 */) {
    // Local variables
    int material_index; // r28
    const struct runtime_config & cfg; // r29
...
// Range: 0x8015ACF0 -> 0x8015AD38
static int compare_ini_sections(void * e1 /* r1+0x8 */, void * e2 /* r1+0xC */) {
    // Local variables
    const struct xIniSection & s1; // r31
    const struct xIniSection & s2; // r30
}

// Range: 0x8015AD38 -> 0x8015AD80
static int compare_ini_values(void * e1 /* r1+0x8 */, void * e2 /* r1+0xC */) {
    // Local variables
    const struct xIniValue & v1; // r31
    const struct xIniValue & v2; // r30
}

// Range: 0x8015AD80 -> 0x8015AFD0
static void reset_config(struct ini_config & cfg /* r31 */) {}
...
// Range: 0x8015AFD0 -> 0x8015B85C
static void load_config(struct ini_config & cfg /* r30 */, struct xIniSection & sec /* r31 */, struct xIniFile & ini /* r25 */) {
    // Local variables
    struct xIniValue * values; // r29
    unsigned int decal_type_hash; // r1+0xC
    char buffer[32]; // r1+0x10
    unsigned int total; // r28
    unsigned int i; // r27
    struct curve_node & n; // r1+0x8
}

// Range: 0x8015B890 -> 0x8015B970
static unsigned int read_list(float * v /* r28 */, unsigned int vsize /* r1+0x8 */, const char * name /* r1+0xC */, const struct xIniValue * values /* r1+0x10 */, int values_size /* r1+0x14 */) {
    // Local variables
    const struct xIniValue * iv; // r30
    char * nextoken; // r1+0x18
    char * s; // r29
    unsigned int total; // r31
}
...
// Range: 0x8015B970 -> 0x8015B9D4
static struct xIniValue * get_value(const char * name /* r1+0x8 */, const struct xIniValue * values /* r1+0xC */, int values_size /* r1+0x10 */) {
    // Local variables
    struct xIniValue key; // r1+0x14
}

// Range: 0x8015B9D4 -> 0x8015BA9C
static void read_value(struct xColor_tag & v /* r1+0x8 */, const char * name /* r1+0xC */, const struct xIniValue * values /* r1+0x10 */, int values_size /* r1+0x14 */) {
    // Local variables
    const struct xIniValue * iv; // r30
    char * nextoken; // r1+0x18
    char * s; // r29
    unsigned char * c; // r31
    unsigned char * endc; // r28
}

// Range: 0x8015BA9C -> 0x8015BAFC
static void read_hashed_text(unsigned int & v /* r30 */, const char * name /* r1+0x8 */, const struct xIniValue * values /* r1+0xC */, int values_size /* r1+0x10 */) {
    // Local variables
    const struct xIniValue * iv; // r31
}

// Range: 0x8015BAFC -> 0x8015BB5C
static void read_value(unsigned int & v /* r30 */, const char * name /* r1+0x8 */, const struct xIniValue * values /* r1+0xC */, int values_size /* r1+0x10 */) {
    // Local variables
    const struct xIniValue * iv; // r31
}

// Range: 0x8015BB5C -> 0x8015BBC0
static void read_value(float & v /* r30 */, const char * name /* r1+0x8 */, const struct xIniValue * values /* r1+0xC */, int values_size /* r1+0x10 */) {
    // Local variables
    const struct xIniValue * iv; // r31
}

static struct curve_node anim[4]; // size: 0x60, address: 0x8051EA60
// Range: 0x8015BBC0 -> 0x8015BE68
static void load_default_config_fire(struct ini_config & cfg /* r31 */) {
    // References
    // -> static struct curve_node anim[4];
}

static struct curve_node anim[4]; // size: 0x60, address: 0x8051EAC0
// Range: 0x8015BE68 -> 0x8015C0F8
static void load_default_config_plasma(struct ini_config & cfg /* r31 */) {
    // References
    // -> static struct curve_node anim[4];
}

static struct curve_node anim[5]; // size: 0x78, address: 0x8051EB20
// Range: 0x8015C0F8 -> 0x8015C388
static void load_default_config_fire_trail(struct ini_config & cfg /* r31 */) {
    // References
    // -> static struct curve_node anim[5];
}

// total size: 0x8
struct default_type_loader {
    // Members
    const char * name; // offset 0x0, size 0x4
    void (* load)(struct ini_config &); // offset 0x4, size 0x4
};
static struct default_type_loader default_types[3]; // size: 0x18, address: 0x804A6F2C
...
// Range: 0x8015C388 -> 0x8015C668
static void load_ini_configs() {
    // Local variables
    struct xIniFile * ini; // r31
    struct xIniSection * sections; // r29
    unsigned int memsize; // r1+0x14
    void * mem; // r23
    unsigned int max_sections; // r20
    struct xIniSection * isec; // r30
    struct xIniSection * end_isec; // r19
    struct xIniSection * osec; // r22
    unsigned char is_default; // r21
    const struct default_type_loader * dtl; // r28
    const struct default_type_loader * end_dtl; // r18
    struct ini_config * cfg; // r24
    struct ini_config * end_cfg; // r17
    int i; // r27
    struct ini_config * cfg; // r25
    struct ini_config * end_cfg; // r1+0x10
    struct xIniSection * sec; // r26

    // References
    // -> void * RwEngineInstance;
    // -> static unsigned int configs_size;
    // -> static struct ini_config * ini_configs;
    // -> static struct default_type_loader default_types[3];
    // -> unsigned int _rwMemoryLastAllocLine;
    // -> const char * _rwMemoryLastAllocFile;
    // -> static char ini_filename[12];
}

// Range: 0x8015C684 -> 0x8015C800
static void load_config_textures(struct runtime_config & ocfg /* r30 */, const struct ini_config & icfg /* r31 */) {}

// Range: 0x8015C800 -> 0x8015C8EC
static void refresh_materials() {
    // Local variables
    struct runtime_config * cfg; // r31
    struct runtime_config * end_cfg; // r29
    int mat_index; // r30

    // References
    // -> static int materials_used;
    // -> static struct material_group * materials;
    // -> static unsigned int configs_size;
    // -> static struct runtime_config * runtime_configs;
}

// Range: 0x8015C8EC -> 0x8015C9D8
static void refresh_streak_groups() {
    // Local variables
    struct runtime_config * cfg; // r31
    struct runtime_config * end_cfg; // r28
    int i; // r30
    struct streak_group * sg; // r29
    struct streak_group * endsg; // r27
...
// Range: 0x8015C9D8 -> 0x8015CAC4
static void refresh_spark_groups() {
    // Local variables
    struct runtime_config * cfg; // r31
    struct runtime_config * end_cfg; // r28
    int i; // r30
    struct spark_group * sg; // r29
    struct spark_group * endsg; // r27
...
// Range: 0x8015CAC4 -> 0x8015CB2C
static void update_configs(float dt /* r1+0x8 */) {
    // Local variables
    struct runtime_config * cfg; // r31
    struct runtime_config * end_cfg; // r30
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D62999
// Range: 0x8015CB2C -> 0x8015CCA0
static struct presence_node * alloc_presence() {
    // Local variables
    struct presence_node * presence; // r31

    // References
    // -> static char __FUNCTION__[15];
    // -> static int presi_size;
    // -> static struct presence_node presi[32];
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D629A8
// Range: 0x8015CCA0 -> 0x8015CE18
static void free_presence(struct presence_node * presence /* r31 */) {
    // Local variables
    struct presence_node * tail_presence; // r30
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D629B6
union entry_node {
...
// Range: 0x8015CED0 -> 0x8015CF94
static void destroy_presence(struct presence_node * presence /* r31 */) {
    // References
    // -> class xSpacePartitionTree2D space_tree;
    // -> static char __FUNCTION__[17];
}

// Range: 0x8015CF94 -> 0x8015D17C
static struct presence_node * create_presence(const struct xVec3 & center /* r30 */, float delay /* r1+0x8 */, float life /* f28 */, float knockback /* r1+0xC */, float radius_begin /* f31 */, float radius_end /* f29 */, float damage_player /* f24 */, float damage_npc /* f25 */, float damage_other /* f26 */, unsigned char infinite /* r1+0x10 */, struct trail_emitter * trail /* r1+0x14 */) {
    // Local variables
    float total_damage; // f27
    struct presence_node * presence; // r31
...
// Range: 0x8015D17C -> 0x8015D1F0
static void move_presence(struct presence_node * presence /* r30 */, const struct xVec3 & center /* r31 */, float radius /* f31 */) {
    // References
    // -> class xSpacePartitionTree2D space_tree;
}
...
// Range: 0x8015D1F0 -> 0x8015D27C
static void damage_reactives(struct presence_node & presence /* r30 */) {
    // Local variables
    struct zEntSimpleObj * reactive_hits[16]; // r1+0x18
    struct xSphere sphere; // r1+0x8
    int reactive_hits_size; // r29
    int i; // r31
}
...
// Range: 0x8015D27C -> 0x8015D3B4
static void update_presence(float dt /* f31 */) {
    // Local variables
    struct presence_node * presence; // r31
    struct presence_node * end; // r30
...
// Range: 0x8015D3B4 -> 0x8015D4F0
static void update_motes(float dt /* f30 */) {
    // Local variables
    struct mote * m; // r31
    struct mote * endm; // r30
    const struct runtime_config & cfg; // r29
    float dloc; // f31
...
// Range: 0x8015D4F0 -> 0x8015D648
static void update_streak_emitters(float dt /* f31 */) {
    // Local variables
    struct streak_emitter * emitter; // r31
    struct streak_emitter * end_emitter; // r30
    float time; // f30
    const struct runtime_config & cfg; // r29
    int emit; // r28
...
// Range: 0x8015D648 -> 0x8015D714
static void free_streak_emitter(struct streak_emitter * se /* r31 */) {
    // References
    // -> static int streak_emitters_used;
    // -> static struct streak_emitter * streak_emitters;
    // -> static char __FUNCTION__[20];
...
// Range: 0x8015D714 -> 0x8015D81C
static void update_streaks(float dt /* f31 */) {
    // Local variables
    struct streak_particle * streak; // r31
    struct streak_particle * end_streak; // r30
...
// Range: 0x8015D81C -> 0x8015D96C
static void free_streak(struct streak_particle * streak /* r29 */) {
    // Local variables
    struct streak_group & group; // r30
    struct streak_particle * tail; // r28
    struct streak_group * mg; // r31
    struct streak_group * endmg; // r26
    struct streak_particle * next; // r27
...
// Range: 0x8015D96C -> 0x8015DAC4
static void update_spark_emitters(float dt /* f31 */) {
    // Local variables
    struct spark_emitter * emitter; // r31
    struct spark_emitter * end_emitter; // r30
    float time; // f30
    const struct runtime_config & cfg; // r29
    int emit; // r28
...
// Range: 0x8015DAC4 -> 0x8015DB90
static void free_spark_emitter(struct spark_emitter * se /* r31 */) {
    // References
    // -> static int spark_emitters_used;
    // -> static struct spark_emitter * spark_emitters;
...
// Range: 0x8015DB90 -> 0x8015DC70
static void update_sparks(float dt /* f31 */) {
    // Local variables
    struct spark_particle * spark; // r31
    struct spark_particle * end_spark; // r30
...
// Range: 0x8015DC70 -> 0x8015DDC0
static void free_spark(struct spark_particle * spark /* r29 */) {
    // Local variables
    struct spark_group & group; // r30
    struct spark_particle * tail; // r28
    struct spark_group * mg; // r31
    struct spark_group * endmg; // r26
    struct spark_particle * next; // r27
...
// Range: 0x8015DDC0 -> 0x8015DFD0
static void load_runtime_configs() {
    // Local variables
    struct ini_config * icfg; // r31
    struct ini_config * end_icfg; // r27
    struct runtime_config * ocfg; // r30
...
// Range: 0x8015DFD0 -> 0x8015DFD4
static void add_config_tweaks() {}

char * animix_labels[6]; // size: 0x18, address: 0x804F2360
// Range: 0x8015E154 -> 0x8015E24C
static void init_decals() {
    // References
    // -> char * animix_labels[6];
    // -> static struct xResponseCurve decal_curve[3];
    // -> static struct curve_node decal_anim_plasma[3];
    // -> static struct curve_node decal_anim_burn[7];
    // -> static struct curve_node decal_anim_impact[3];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D629C7
// total size: 0x5C
struct emit_context {
    // Members
...
// Range: 0x8015E24C -> 0x8015E3B4
static void paint_decals(const struct xVec3 & loc /* r1+0x8 */, const struct runtime_config & cfg /* r31 */) {
    // Local variables
    struct emit_context ec; // r1+0xC

    // References
    // -> static struct xResponseCurve decal_curve[3];
    // -> static int decal_blend_mode[3];
    // -> static int decal_flags[3];
    // -> static char __FUNCTION__[13];
}

static float repeat_time; // size: 0x4, address: 0x80D6A100
// Range: 0x8015E3B4 -> 0x8015E450
...
// total size: 0x890
struct /* @class$2056zExplosion_cpp */ {
    // Static members
...
// Range: 0x8015E450 -> 0x8015E470
static void debug_emit() {}

// Range: 0x8015E470 -> 0x8015E684
static void debug_init() {
    // Local variables
    const char * prefix; // r28
    unsigned int labels_size; // r30
    const char * * labels; // r31
    unsigned int i; // r29

    // References
    // -> static float repeat_time;
    // -> static int type;
    // -> static int type;
    // -> static int type;
    // -> static int type;
...
// Range: 0x8015E684 -> 0x8015E6E8
static void debug_update_test(float dt /* r1+0x8 */) {
    // References
    // -> static float repeat_time;
    // -> static float repeat_every;
}
...
struct trail_emitter {
    // Functions
    void start(int type, const struct xVec3 & loc);

    void stop();

    void update(const struct xVec3 & loc, float dt, float intensity);
...
static struct trail_emitter te; // size: 0x30, address: 0x80CF1B9C
static unsigned char was_active; // size: 0x1, address: 0x80D6A104
static signed char init; // size: 0x1, address: 0x80D6A105
static float spiral_time; // size: 0x4, address: 0x80D6A108
static signed char init; // size: 0x1, address: 0x80D6A10C
static struct xVec3 center; // size: 0xC, address: 0x80D629D4
// Range: 0x8015E6E8 -> 0x8015E964
static void debug_update_test_spiral(float dt /* f27 */) {
    // Local variables
    unsigned int insert_flags; // r1+0x8
    const struct xMat4x3 & mat; // r31
    float frac; // f31
    float ang; // f30
    float radius; // f29
    struct xVec3 norm; // r1+0x18
    struct xVec3 loc; // r1+0xC

    // References
    // -> static unsigned char was_active;
    // -> static struct trail_emitter te;
    // -> static unsigned int configs_size;
    // -> static int type;
    // -> static struct xVec3 center;
    // -> static float radius_inner;
    // -> static float radius_outer;
    // -> static float freq;
    // -> static float duration;
    // -> static float spiral_time;
    // -> static float height;
    // -> struct xGlobals * xglobals;
    // -> static unsigned char bind_to_player;
    // -> static signed char init;
...
static float emitted; // size: 0x4, address: 0x80D6A110
static signed char init; // size: 0x1, address: 0x80D6A114
// Range: 0x8015E964 -> 0x8015EB70
static void debug_update_test_streaks(float dt /* r1+0x8 */) {
    // Local variables
    int emit; // r30
    int remaining; // r27
    int type; // r29
    int group; // r26
    struct xVec3 center; // r1+0xC
    int i; // r28
    struct streak_particle * streak; // r31
    float ilife; // f31
    ...
    // -> static unsigned int configs_size;
    // -> static int type;
    // -> static float emitted;
    // -> static int streaks_used;
    // -> static float rate;
    // -> static signed char init;
}

static float emitted; // size: 0x4, address: 0x80D6A118
static signed char init; // size: 0x1, address: 0x80D6A11C
// Range: 0x8015EB70 -> 0x8015ED8C
static void debug_update_test_sparks(float dt /* r1+0x8 */) {
    // Local variables
    int emit; // r30
    int remaining; // r27
    int type; // r29
...
// Range: 0x8015ED8C -> 0x8015EDD0
static void debug_update(float dt /* f31 */) {}

// Range: 0x8015EDD0 -> 0x8015EDD4
static void debug_render() {}

// Range: 0x8015EDD4 -> 0x8015EDF4
void startup() {}

// Range: 0x8015EDF4 -> 0x8015EEB0
void scene_enter() {
    // References
    // -> static int presi_size;
    // -> static struct spark_emitter * spark_emitters;
...
// Range: 0x8015EEB0 -> 0x8015EEB4
void scene_exit() {}

// Range: 0x8015EEB4 -> 0x8015EF60
void reset() {
    // Local variables
    struct presence_node * pres; // r31
    struct presence_node * end_pres; // r30
...
// Range: 0x8015EF60 -> 0x8015EFC4
void update(float dt /* f31 */) {}

// Range: 0x8015EFC4 -> 0x8015EFE4
void pre_render() {}

// Range: 0x8015EFE4 -> 0x8015F004
void render() {}

// Range: 0x8015F004 -> 0x8015F050
int find_type(unsigned int id /* r3 */) {
    // Local variables
    int i; // r31
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D629E0
// Range: 0x8015F050 -> 0x8015F268
void emit(int type /* r29 */, const struct xVec3 & loc /* r30 */) {
    // Local variables
    const struct ini_config & icfg; // r26
    const char * name; // r1+0x8
    const struct runtime_config & cfg; // r31
    unsigned int motes_available; // r27
    unsigned int motes_required; // r28
...
static char __FUNCTION__[6]; // size: 0x6, address: 0x80D629E5
// Range: 0x8015F268 -> 0x8015F3D4
// this: r30
void trail_emitter::start(int type /* r29 */, const struct xVec3 & loc /* r28 */) {
    // Local variables
    const struct runtime_config & cfg; // r31
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D629EB
// Range: 0x8015F3D4 -> 0x8015F4B4
// this: r31
void trail_emitter::stop() {
    // References
    // -> static char __FUNCTION__[5];
    // -> static unsigned int configs_size;
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80D629F0
// Range: 0x8015F4B4 -> 0x8015F848
// this: r31
void trail_emitter::update(const struct xVec3 & loc /* r26 */, float dt /* f31 */, float intensity /* r1+0x8 */) {
    // Local variables
    const struct runtime_config & cfg; // r30
    int emit_rand; // r22
    int emit_total; // r27
    int motes_available; // r25
    const struct xVec3 & loc0; // r29
    const struct xVec3 & loc1; // r28
    struct xVec3 offset; // r1+0xC
    int streak_amount; // r24
    int spark_amount; // r23
