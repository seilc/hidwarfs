/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zDestructible.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80142B3C -> 0x801466A8
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static float DEG_TO_RAD; // size: 0x4, address: 0x80D624F0
static float LAUNCH_ANGLE; // size: 0x4, address: 0x80D624F4
static float LAUNCH_VEL; // size: 0x4, address: 0x80D624F8
static float DAMAGE_RADIUS; // size: 0x4, address: 0x80D624FC
static float MAX_TIME_NO_COLLISIONS; // size: 0x4, address: 0x80D62500
static float HIT_TO_DESTROY_TIME; // size: 0x4, address: 0x80D62504
static float Y_VELOCITY; // size: 0x4, address: 0x80D62508
static float RESPAWN_THRESHOLD; // size: 0x4, address: 0x80D6250C
// total size: 0x20
struct xLinkAsset {
    // Members
...
// total size: 0x4
struct /* @class$901zDestructible_cpp */ {
    // Members
...
union /* @class$1040zDestructible_cpp */ {
...
// Range: 0x80142B3C -> 0x80142BB0
static void UpdateModelBound(struct xEnt * ent /* r31 */, struct xModelInstance * model /* r30 */, unsigned char gridUpdate /* r1+0x8 */) {}
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D62510
// total size: 0x8
...
// Range: 0x80142BB0 -> 0x80142D10
static void LoadModel(struct xEnt * ent /* r31 */, struct zDestructible * dest /* r30 */, unsigned char gridUpdate /* r1+0x8 */) {
    // Local variables
    unsigned int modelID; // r29
    struct RpAtomic * imodel; // r28
    struct RwV3d temp_pos; // r1+0xC

    // References
    // -> static char __FUNCTION__[10];
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x8051D500
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// total size: 0x10
struct /* @class$1901zDestructible_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x80142D10 -> 0x80142F94
static void SwapModelThroughState(struct xEnt * ent /* r31 */, struct zDestructibleState * state /* r30 */, struct zDestructible * dest /* r28 */) {
    // Local variables
    struct xModelInstance * curr; // r29
...
// Range: 0x80143164 -> 0x80143264
static void zDestructible_Launch(struct xEnt * ent /* r31 */, struct xVec3 & direction /* r30 */) {
    // Local variables
    struct xVec3 vec; // r1+0x1C
    struct xVec3 y_axis; // r1+0x10
    struct xMat3x3 rot; // r1+0x30

    // References
    // -> static float LAUNCH_VEL;
    // -> static float DEG_TO_RAD;
    // -> static float LAUNCH_ANGLE;
}

static float DOBJ_STD_BOUND_VAR; // size: 0x4, address: 0x80D6251C
// Range: 0x80143264 -> 0x80143318
static void DOBJ_alwaysUseSphere(struct xEnt * ent /* r30 */) {
    // Local variables
    struct xEnt * dobj; // r31
    struct xVec3 bndcent; // r1+0x8

    // References
    // -> static float DOBJ_STD_BOUND_VAR;
}
...
// Range: 0x80143318 -> 0x801433FC
static void zDestructible_SendEvent(struct xEnt * ent /* r31 */, struct xBase * dest /* r27 */) {
    // Local variables
    int hit_sourcetype; // r29
    const struct zHitSourceMapEntry * damage_type_map; // r30
    float params[6]; // r1+0x8

    // References
    // -> struct zHitSourceMapEntry zHitSourceMap[66];
}

// total size: 0xC
struct /* @class$1039zDestructible_cpp */ {
    // Members
...
// Range: 0x801433FC -> 0x80143744
static void zDestructible_AffectAll(struct xEnt * ent /* r31 */, struct zDestructible * dest /* r30 */, struct xScene * sc /* r25 */, float dt /* f31 */) {
    // Local variables
    int i; // r29
    struct xVec3 dist; // r1+0x8
    void (* bak_bupdate)(struct xEnt *, struct xVec3 *); // r26
    struct xEntCollis ent_collis_rec; // r1+0x14
    int i; // r28
    int i; // r27

    // References
    // -> static float MAX_TIME_NO_COLLISIONS;
    // -> static float DAMAGE_RADIUS;
    // -> static float DOBJ_STD_BOUND_VAR;
    // -> struct xGlobals * xglobals;
}
...
// Range: 0x80143744 -> 0x80143794
static unsigned int BurnSinRateCallback(float time /* r1+0x8 */) {}

static unsigned int check_ent_id; // size: 0x4, address: 0x80D69F20
static signed char init; // size: 0x1, address: 0x80D69F24
static char __FUNCTION__[25]; // size: 0x19, address: 0x8051D516
// Range: 0x80143794 -> 0x8014424C
void zDestructibleAsset_Reset(struct zDestructible * & dest /* r31 */, struct xEnt * ent /* r28 */) {
    // Local variables
    int asdf; // r1+0x8
    struct xDestructibleAssetState * d_states; // r30
    unsigned int i; // r29
    struct RpAtomic * imodel; // r19
    unsigned int fx_count; // r24
    unsigned int j; // r22
    struct zDestructibleStateFXParams * fx_Param; // r27
    struct xDestructibleAssetAttachedAnimList * ds_state_anim_list; // r26
    unsigned int * animListIDs; // r23
    unsigned int j; // r25
    void * tmp_ptr; // r21

    // References
    // -> static char __FUNCTION__[25];
    // -> static unsigned int check_ent_id;
    // -> static signed char init;
}

static char __FUNCTION__[27]; // size: 0x1B, address: 0x8051D52F
// Range: 0x8014424C -> 0x80144380
void zDestructibleAsset_ReReset(struct zDestructible * & dest /* r30 */) {
    // Local variables
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x8051D54A
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x80144380 -> 0x8014495C
void zDestructibleAsset_Init(unsigned int assetID /* r21 */, struct zDestructible * & dest /* r31 */, struct xEnt * ent /* r23 */) {
    // Local variables
    struct xDestructibleAssetState * d_states; // r30
    unsigned int i; // r29
    struct xModelInstance * baseModel; // r26
    unsigned int fx_count; // r28
    struct xDestructibleAssetAttachedAnimList * ds_state_anim_list; // r27
...
static unsigned int check_ent_id; // size: 0x4, address: 0x80D69F28
static signed char init; // size: 0x1, address: 0x80D69F2C
// Range: 0x8014495C -> 0x801449F0
void zDestructibleAsset_CopyLODDistances(struct xEnt * ent /* r3 */, struct zDestructible * destructible /* r4 */) {
    // Local variables
    int asdf; // r1+0x8
...
static char __FUNCTION__[31]; // size: 0x1F, address: 0x8051D562
// Range: 0x801449F0 -> 0x80144ACC
static struct zEntAnimData * zDestructible_GetOwnerAnimData(struct xEnt * ent /* r29 */, int ownerID /* r1+0x8 */) {
    // References
    // -> static char __FUNCTION__[31];
}

// Range: 0x80144ACC -> 0x80144B5C
static void zDestructible_SetAnimation(struct xEnt * ent /* r1+0x8 */, struct zDestructible * dest /* r29 */, struct zDestructibleState * state /* r30 */, int idx /* r1+0xC */) {
    // Local variables
    struct zEntAnimData * ent_animdata; // r31
}
...
static char __FUNCTION__[29]; // size: 0x1D, address: 0x8051D581
// Range: 0x80144B5C -> 0x80144C80
static void zDestructible_SwapAnimations(struct xEnt * ent /* r1+0x8 */, struct zDestructible * dest /* r31 */) {
    // Local variables
    struct zDestructibleState * state; // r30

    // References
    // -> static char __FUNCTION__[29];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D62520
// Range: 0x80144C80 -> 0x80144E8C
static void zDestructible_Swap(struct xEnt * ent /* r29 */, struct zDestructible * dest /* r30 */, struct xVec3 & initial_velocity /* r1+0x8 */, int pad_port /* r1+0xC */, unsigned char state_change /* r1+0x10 */) {
    // Local variables
    struct zDestructibleState * state; // r31
...
// total size: 0x20
struct xSoundFXAsset : public xBaseAsset {
    // Members
    union { // inferred
    ...
    struct xVec3 pos; // offset 0x10, size 0xC
    unsigned int uFlags; // offset 0x1C, size 0x4
};
// total size: 0x18
struct xSoundFX : public xBase {
    // Members
    struct xSoundFXAsset * asset; // offset 0x10, size 0x4
    enum xSndHandle sndHandle; // offset 0x14, size 0x4
};
static char __FUNCTION__[38]; // size: 0x26, address: 0x8051D59E
// Range: 0x80144E8C -> 0x80145054
static void zDestructible_StartSwitchStatesSounds(struct xEnt * ent /* r30 */, struct zDestructible * dest /* r31 */) {
    // Local variables
    struct xSoundFX * soundfxswitch; // r29

    // References
    // -> static char __FUNCTION__[38];
}

static char __FUNCTION__[26]; // size: 0x1A, address: 0x8051D5C4
// Range: 0x80145054 -> 0x80145240
static void zDestructible_StateChange(struct xEnt * ent /* r29 */, struct zDestructible * dest /* r30 */, int pad_port /* r1+0x8 */) {
    // Local variables
    struct zDestructibleState * state; // r31
...
static char __FUNCTION__[25]; // size: 0x19, address: 0x8051D5DE
// Range: 0x80145240 -> 0x801453DC
static void zDestructible_StopSounds(struct xEnt * ent /* r28 */, struct zDestructible * dest /* r31 */) {
    // Local variables
    int lastidx; // r30
    struct xSoundFX * soundfx; // r29

    // References
    // -> static char __FUNCTION__[25];
}

static char __FUNCTION__[32]; // size: 0x20, address: 0x8051D5F7
// Range: 0x801453DC -> 0x801454E0
static void zDestructible_StopAllIdleSounds(struct xEnt * ent /* r1+0x8 */, struct zDestructible * dest /* r31 */) {
    // Local variables
    unsigned int i; // r30
...
static char __FUNCTION__[26]; // size: 0x1A, address: 0x8051D617
// Range: 0x801454E0 -> 0x8014570C
static void zDestructible_StartSounds(struct xEnt * ent /* r30 */, struct zDestructible * dest /* r31 */) {
    // Local variables
    struct xSoundFX * soundfx; // r29
...
static char __FUNCTION__[27]; // size: 0x1B, address: 0x8051D631
// Range: 0x8014570C -> 0x801459C8
static void zDestructible_StateProcess(struct xEnt * ent /* r30 */, struct zDestructible * dest /* r31 */, unsigned int damage /* r27 */, unsigned int idx /* r1+0x8 */, const float * toParam /* r25 */, struct xVec3 & initial_velocity /* r23 */, int pad_port /* r28 */, unsigned char state_change /* r24 */) {
    // Local variables
    struct xModelInstance * curr; // r29
    unsigned int _percent; // r26

    // References
    // -> static char __FUNCTION__[27];
}

// Range: 0x801459F0 -> 0x80145A44
static void zDestructible_PlayAnim(struct xEnt * ent /* r1+0x8 */, struct zDestructible * dest /* r31 */, struct zDestructibleState * state /* r1+0xC */, int idx /* r1+0x10 */) {}

static char __FUNCTION__[27]; // size: 0x1B, address: 0x8051D64C
// total size: 0x8
struct AnalogStick {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x80145A44 -> 0x80146334
void zDestructibleAsset_EventCB(struct xBase * from /* r27 */, struct xBase * root_base /* r1+0x8 */, struct zDestructible * to /* r1+0xC */, unsigned int toEvent /* r1+0x10 */, const float * toParam /* r28 */) {
    // Local variables
    struct xEnt * ent; // r30
    struct xEnt * root_ent; // r17
    struct zDestructible * dest; // r31
    struct zDestructibleState * dstates; // r23
    struct zDestructibleState * cstate; // r29
    int low; // r22
    int up; // r1+0x20
    int idx; // r26
    int idx; // r25
    ...
    int padport; // r21
    class zPlayerInput pinput; // r1+0x78
    struct xVec3 loc; // r1+0x3C
    struct xVec3 loc; // r1+0x30
    struct xSurface * surf; // r1+0x1C
    struct zEntAnimData * ent_animdata; // r1+0x18
    unsigned int hit_source_flag; // r20
    unsigned int damage; // r1+0x14
    int i; // r24
    struct xVec3 kickback; // r1+0x24
    int padport; // r19
    class zPlayerInput pinput; // r1+0x54
    unsigned char isEnabled; // r18

    // References
    // -> struct xGlobals * xglobals;
    // -> static float RESPAWN_THRESHOLD;
...
// total size: 0x890
struct /* @class$2144zDestructible_cpp */ {
    // Static members
...
// total size: 0xC
struct /* @class$842zDestructible_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$843zDestructible_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$844zDestructible_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$845zDestructible_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$846zDestructible_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$847zDestructible_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$848zDestructible_cpp */ {
    // Members
...
// Range: 0x80146334 -> 0x801464CC
void zDestructibleAsset_Update(struct xEnt * ent /* r30 */, struct zDestructible * dest /* r31 */, struct xScene * sc /* r1+0x8 */, float dt /* f31 */) {
    // References
    // -> static float HIT_TO_DESTROY_TIME;
}

// Range: 0x801464CC -> 0x80146500
struct zDestructible * zDestructibleGetFromEntity(struct xEnt * ent /* r3 */) {}

// Range: 0x80146500 -> 0x801466A8
void add_tweaks_destructible(const char * prefix /* r31 */) {
    // Local variables
