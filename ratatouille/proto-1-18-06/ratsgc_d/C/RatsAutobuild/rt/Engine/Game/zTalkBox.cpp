/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zTalkBox.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801BBEC0 -> 0x801C0A10
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct shared_type shared; // size: 0x2A0, address: 0x80D0BF08
static unsigned int zero; // size: 0x4, address: 0x80D6A9D8
// Range: 0x801BBEC0 -> 0x801BBF38
static void speak_stop() {
    // References
    // -> static struct shared_type shared;
}

char __FUNCTION__$localstatic1$push_back__56fixed_queue<Q222@unnamed@zTalkBox_cpp@12trigger_pair,32>Fv[10]; // size: 0xA, address: 0x80D639C0
char __FUNCTION__$localstatic1$back__56fixed_queue<Q222@unnamed@zTalkBox_cpp@12trigger_pair,32>Fv[5]; // size: 0x5, address: 0x80D639CA
char __FUNCTION__$localstatic1$__ml__Q256fixed_queue<Q222@unnamed@zTalkBox_cpp@12trigger_pair,32>8iteratorCFv[5]; // size: 0x5, address: 0x80D639CF
// Range: 0x801BBF38 -> 0x801BBFD4
static void trigger(unsigned int event /* r30 */) {
    // Local variables
    struct trigger_pair & t; // r31
...
char __FUNCTION__$localstatic1$front__56fixed_queue<Q222@unnamed@zTalkBox_cpp@12trigger_pair,32>Fv[6]; // size: 0x6, address: 0x80D639D4
char __FUNCTION__$localstatic1$pop_front__56fixed_queue<Q222@unnamed@zTalkBox_cpp@12trigger_pair,32>Fv[10]; // size: 0xA, address: 0x80D639DA
// total size: 0x8
struct trigger_pair {
    // Members
...
static struct substr positive[6]; // size: 0x30, address: 0x804B3648
static struct substr negative[6]; // size: 0x30, address: 0x804B3678
// Range: 0x801BC04C -> 0x801BC118
static unsigned char read_bool(const struct substr & s /* r29 */, unsigned char def /* r1+0x8 */) {
    // Local variables
    unsigned long i; // r31
    unsigned long i; // r30
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D639E4
// Range: 0x801BC118 -> 0x801BC2FC
...
static int v[32]; // size: 0x80, address: 0x80D0C1A8
// Range: 0x801BC2FC -> 0x801BC568
static void load_wait_context(struct wait_context & c /* r31 */, const struct tag_entry_list & el /* r27 */) {
    // Local variables
    const struct tag_entry * e; // r30
    const struct tag_entry * prompt; // r29
    unsigned long r; // r26
    unsigned long i; // r28
...
// Range: 0x801BC568 -> 0x801BC574
// this: r3
static void wait_context::reset_type() {}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80D639F4
// total size: 0x8
...
// Range: 0x801BC574 -> 0x801BC77C
static void parse_tag_auto_wait(struct jot & j /* r29 */, const class xtextbox & ctb /* r1+0x8 */, const struct split_tag & ti /* r1+0xC */) {
    // Local variables
    struct wait_context & c; // r30
...
// Range: 0x801BC7A8 -> 0x801BC834
static void reset_tag_auto_wait(struct jot & j /* r31 */, const class xtextbox & ctb /* r1+0x8 */) {
    // Local variables
    struct wait_context & c; // r30
...
// Range: 0x801BC834 -> 0x801BC874
static unsigned char trigger_auto_wait(const struct jot & j /* r31 */) {
    // References
    // -> static struct shared_type shared;
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D63A08
// total size: 0x4
struct signal_context {
    // Members
    unsigned int flags; // offset 0x0, size 0x4
};
// Range: 0x801BC874 -> 0x801BCB5C
static void parse_tag_signal(struct jot & j /* r1+0x8 */, const class xtextbox & ctb /* r1+0xC */, const struct split_tag & ti /* r1+0x10 */) {
    // Local variables
    unsigned long il; // r27
    unsigned long ir; // r26
    struct signal_context & c; // r30
    const struct tag_entry * e; // r31
    struct tag_entry_list el; // r1+0x24
    int v[20]; // r1+0x2C
    unsigned long r; // r28
    unsigned long i; // r29
...
static unsigned int signals[20]; // size: 0x50, address: 0x804B36A8
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D63A19
// Range: 0x801BCB5C -> 0x801BCCA4
static unsigned char trigger_signal(const struct jot & j /* r1+0x8 */) {
    // Local variables
    const struct signal_context & c; // r31
    unsigned long i; // r30
...
enum /* @enum$1968zTalkBox_cpp */ {
    ACTION_SET = 0,
    ACTION_PUSH = 1,
    ACTION_POP = 2,
};
enum /* @enum$1969zTalkBox_cpp */ {
    TYPE_INVALID = 0,
    TYPE_VOLUME = 1,
    TYPE_TARGET = 2,
    TYPE_ORIGIN = 3,
...
// Range: 0x801BCCA4 -> 0x801BCD74
static int load_sound_type(struct sound_context & c /* r31 */, const struct tag_entry_list & el /* r1+0x8 */) {
    // Local variables
    const struct tag_entry * e; // r30
    unsigned long r; // r29
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D63A28
// Range: 0x801BCD74 -> 0x801BD190
static void parse_tag_sound(struct jot & j /* r28 */, const class xtextbox & ctb /* r1+0x8 */, const struct split_tag & ti /* r1+0xC */) {
    // Local variables
    struct sound_context & c; // r31
    const struct tag_entry * e; // r30
    struct tag_entry_list el; // r1+0x54
    const struct substr & action; // r27
    const struct substr & action; // r29
    int anim; // r1+0x10
...
// Range: 0x801BD190 -> 0x801BD1F0
static void reset_tag_sound(struct jot & j /* r3 */, const class xtextbox & ctb /* r5 */) {
    // Local variables
    struct sound_context & c; // r31
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D63A38
char __FUNCTION__$localstatic1$pop__14sound_queue<4>Fv[4]; // size: 0x4, address: 0x80D63A46
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x801BD1F0 -> 0x801BD538
static unsigned char trigger_sound(const struct jot & j /* r27 */) {
    // Local variables
    const struct sound_context & c; // r31
    float fVolume; // r1+0x8
    struct ztalkbox & talk; // r26
    class zNPCBaseIN2 * npc; // r29
    struct xEnt * player; // r28
    struct xBase * obj; // r30
...
// Range: 0x801BD538 -> 0x801BD53C
static void parse_tag_pause() {}

// Range: 0x801BD53C -> 0x801BD540
static void reset_tag_pause() {}

// Range: 0x801BD540 -> 0x801BD548
static unsigned char trigger_pause() {}

// Range: 0x801BD548 -> 0x801BD5E0
static void parse_tag_allow_quit(struct jot & j /* r1+0x8 */, const struct split_tag & ti /* r31 */) {
    // Local variables
    unsigned char & c; // r30
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80521A70
// Range: 0x801BD5E0 -> 0x801BD6E0
static void reset_tag_allow_quit(struct jot & j /* r1+0x8 */, const class xtextbox & ctb /* r1+0xC */) {
    // Local variables
    unsigned char & c; // r31
...
// Range: 0x801BD6E0 -> 0x801BD704
static unsigned char trigger_allow_quit(const struct jot & j /* r3 */) {
    // References
    // -> static struct shared_type shared;
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63A4A
// total size: 0x14
...
// Range: 0x801BD704 -> 0x801BD99C
static void parse_tag_teleport(struct jot & j /* r25 */, const struct split_tag & ti /* r31 */) {
    // Local variables
    struct teleport_context & c; // r30
    unsigned int id; // r28
    unsigned char value_used; // r27
    struct xDynAsset * a; // r29
    struct location_asset & ta; // r18
    struct pointer_asset & ta; // r26
...
// Range: 0x801BD99C -> 0x801BDA18
static void move_player(const struct xVec3 & loc /* r1+0x8 */) {
    // Local variables
    struct xVec3 & oloc1; // r31
    struct xVec3 & oloc2; // r30
...
// Range: 0x801BDA18 -> 0x801BDA78
static void turn_player(float yaw /* r1+0x8 */) {
    // Local variables
    struct xMat4x3 & mat; // r31
    struct xVec3 ypr; // r1+0xC

    // References
    // -> struct xGlobals * xglobals;
}

// Range: 0x801BDA78 -> 0x801BDAEC
static unsigned char trigger_teleport(const struct jot & j /* r30 */) {
    // Local variables
    const struct teleport_context & c; // r31
}

// Range: 0x801BDAEC -> 0x801BDB84
static void parse_tag_trap(struct jot & j /* r1+0x8 */, const struct split_tag & ti /* r31 */) {
    // Local variables
    unsigned char & c; // r30
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D63A5D
// Range: 0x801BDB84 -> 0x801BDC80
static void reset_tag_trap(struct jot & j /* r1+0x8 */, const class xtextbox & ctb /* r1+0xC */) {
    // Local variables
    unsigned char & c; // r31
...
// Range: 0x801BDC80 -> 0x801BDC88
static unsigned char trigger_trap() {}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D63A6C
// Range: 0x801BDC88 -> 0x801BDE4C
static void parse_tag_wait(struct jot & j /* r31 */, const class xtextbox & ctb /* r1+0x8 */, const struct split_tag & ti /* r1+0xC */) {
    // Local variables
    struct wait_context & c; // r30
    struct tag_entry_list el; // r1+0x18
...
// Range: 0x801BDE4C -> 0x801BDED4
static unsigned char trigger_wait(const struct jot & j /* r30 */) {
    // Local variables
    const struct wait_context & c; // r31

    // References
    // -> static struct shared_type shared;
}

// Range: 0x801BDED4 -> 0x801BDF2C
static unsigned char triggerTagShow(const struct jot & j /* r29 */) {
    // Local variables
    unsigned int * id; // r31
    struct ztalkbox * talkBoxToShow; // r30
}

// Range: 0x801BDF2C -> 0x801BDFC4
static void parseTagShow(struct jot & j /* r30 */, const struct split_tag & splitTag /* r31 */) {
    // Local variables
    unsigned int * id; // r29
...
// Range: 0x801BDFC4 -> 0x801BE01C
static unsigned char triggerTagTriggerPads(const struct jot & j /* r29 */) {
    // Local variables
    struct TriggerPadsContext * context; // r30
    struct ztalkbox * talkBoxTriggerPads; // r31
}

// Range: 0x801BE01C -> 0x801BE08C
static unsigned char triggerTagResetHide(const struct jot & j /* r29 */) {
    // Local variables
    unsigned int * id; // r30
    struct ztalkbox * talkBoxToResetHide; // r31
}

// Range: 0x801BE08C -> 0x801BE1B0
static void parseTagTriggerPads(struct jot & j /* r26 */, const struct split_tag & splitTag /* r31 */) {
    // Local variables
    char * c; // r30
    unsigned char forceActiveTag; // r28
    const char * forceActiveKeyword; // r27
    unsigned int lengthToCompare; // r24
    struct TriggerPadsContext * context; // r29
    unsigned int talkBoxTriggerPadsIDSize; // r23
    unsigned int talkBoxTriggerPadsID; // r22

    // References
    // -> static struct shared_type shared;
}

// Range: 0x801BE1B0 -> 0x801BE248
static void parseTagResetHide(struct jot & j /* r30 */, const struct split_tag & splitTag /* r31 */) {
    // Local variables
    unsigned int * id; // r29
...
static struct tag_type new_tags[12]; // size: 0xF0, address: 0x80521A88
static unsigned long new_tags_size; // size: 0x4, address: 0x80D63A7C
static float music_fade; // size: 0x4, address: 0x80D6A9DC
static float music_fade_delay; // size: 0x4, address: 0x80D63A80
static unsigned char registered; // size: 0x1, address: 0x80D6A9E0
static signed char init; // size: 0x1, address: 0x80D6A9E1
// Range: 0x801BE248 -> 0x801BE314
...
// Range: 0x801BE374 -> 0x801BE3E0
static void deactivate() {
    // Local variables
    struct ztalkbox & e; // r31

    // References
    // -> static struct shared_type shared;
}

// Range: 0x801BE3E0 -> 0x801BE458
static void activate(struct ztalkbox & e /* r31 */) {
    // References
    // -> static struct shared_type shared;
}

// Range: 0x801BE458 -> 0x801BE49C
static unsigned char is_wait_jot(const struct jot & j /* r3 */) {}
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D63A84
// Range: 0x801BE49C -> 0x801BE678
static void update_prompts() {
    // Local variables
    struct ztalkbox & active; // r31
    const char * message; // r30
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D63A93
// Range: 0x801BE678 -> 0x801BE73C
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D63AA0
char __FUNCTION__$localstatic1$recent__14sound_queue<4>CFi[7]; // size: 0x7, address: 0x80D63AAC
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D63AB3
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D63AB8
// Range: 0x801BE73C -> 0x801BE83C
static void stop_wait(struct ztalkbox & e /* r27 */, const float * args /* r28 */, unsigned long args_size /* r1+0x8 */) {
    // Local variables
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D63AC2
// Range: 0x801BE83C -> 0x801BEAF8
static void cb_dispatch(struct xBase * to /* r28 */, unsigned int event /* r1+0x8 */, const float * argf /* r30 */) {
    // Local variables
    struct ztalkbox & e; // r31
...
// Range: 0x801BEAF8 -> 0x801BEB54
static char * load_text(unsigned int id /* r30 */) {
    // Local variables
    const char * text; // r31
}

static char __FUNCTION__[5]; // size: 0x5, address: 0x80D63ACE
// total size: 0x20
struct xLinkAsset {
    // Members
...
// total size: 0x1
struct /* @class$761zTalkBox_cpp */ {
    // Members
    unsigned char visible : 1; // offset 0x0, size 0x1
};
// total size: 0x1
struct /* @class$807zTalkBox_cpp */ {
    // Members
...
enum /* @enum$812zTalkBox_cpp */ {
    XJ_LEFT = 0,
    XJ_CENTER = 1,
    XJ_RIGHT = 2,
};
enum /* @enum$813zTalkBox_cpp */ {
    YJ_TOP = 0,
    YJ_CENTER = 1,
    YJ_BOTTOM = 2,
};
enum /* @enum$814zTalkBox_cpp */ {
    EX_UP = 0,
    EX_CENTER = 1,
    EX_DOWN = 2,
    MAX_EX = 3,
...
// Range: 0x801BEB54 -> 0x801BEE50
// this: r31
void ztalkbox::load(const struct asset_type & a /* r30 */) {
    // References
    // -> static char __FUNCTION__[5];
}

// Range: 0x801BEE50 -> 0x801BEEE8
// this: r31
void ztalkbox::reset() {
    // References
    // -> static struct shared_type shared;
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D63AD3
// Range: 0x801BEEE8 -> 0x801BF054
// this: r30
void ztalkbox::set_text(const char * s /* r29 */) {
    // Local variables
...
// Range: 0x801BF054 -> 0x801BF058
static void state_type::start() {}

// Range: 0x801BF058 -> 0x801BF05C
static void state_type::stop() {}

// Range: 0x801BF05C -> 0x801BF0C8
// this: r29
void ztalkbox::set_text(unsigned int id /* r30 */) {
    // Local variables
    const char * text; // r31
}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D63ADC
// Range: 0x801BF0C8 -> 0x801BF190
// this: r31
void ztalkbox::add_text(const char * s /* r1+0x8 */) {
    // References
    // -> static struct shared_type shared;
    // -> static char __FUNCTION__[9];
}

// Range: 0x801BF190 -> 0x801BF1E8
// this: r1+0x8
void ztalkbox::add_text(unsigned int id /* r30 */) {
    // Local variables
    const char * text; // r31
}

// Range: 0x801BF1E8 -> 0x801BF214
// this: r1+0x8
void ztalkbox::clear_text() {}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D63AE5
// Range: 0x801BF214 -> 0x801BF400
// this: r31
void ztalkbox::start_talk(const char * s /* r28 */, struct callback * cb /* r29 */, class zNPCBaseIN2 * npc /* r1+0x8 */) {
    // Local variables
...
// Range: 0x801BF400 -> 0x801BF478
// this: r27
void ztalkbox::start_talk(unsigned int text_id /* r28 */, struct callback * cb /* r29 */, class zNPCBaseIN2 * npc /* r30 */) {
    // Local variables
    const char * text; // r31
}

// Range: 0x801BF478 -> 0x801BF4B4
// this: r1+0x8
...
// Range: 0x801BF5E4 -> 0x801BF610
// this: r3
void ztalkbox::stop_wait(unsigned int mask /* r4 */) {
    // References
    // -> static struct shared_type shared;
}

// Range: 0x801BF610 -> 0x801BF6A4
// this: r31
void ztalkbox::show() {
    // References
    // -> static struct shared_type shared;
...
// Range: 0x801BF6A4 -> 0x801BF704
// this: r31
void ztalkbox::hide() {}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D63AF0
// Range: 0x801BF704 -> 0x801BF860
// this: r30
void ztalkbox::MasterTellSlaves(int isBeginning /* r1+0x8 */) {
    // Local variables
    int i; // r29
    const struct xLinkAsset * link; // r31
    struct xBase * mychild; // r28
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D63B01
// Range: 0x801BF860 -> 0x801BF954
// this: r1+0x8
void ztalkbox::MasterLoveSlave(const struct xBase * slave /* r28 */, int starting /* r1+0xC */) {
    // Local variables
    struct xGroup * grp; // r29
    unsigned int cnt; // r27
    unsigned int i; // r31
    struct xBase * grpitem; // r30
...
// Range: 0x801BF954 -> 0x801BF9CC
void ztalkbox::load_settings(struct xIniFile & ini /* r1+0x8 */) {
    // References
    // -> static struct shared_type shared;
}

static signed char init; // size: 0x1, address: 0x80D6A9E2
// total size: 0x8
class start_state_type : public state_type {
    // Functions
    static start_state_type();
};
static class start_state_type start_state; // size: 0x8, address: 0x80D6A9E4
static signed char init; // size: 0x1, address: 0x80D6A9EC
// total size: 0xC
...
static class next_state_type next_state; // size: 0xC, address: 0x80D6A9F0
static signed char init; // size: 0x1, address: 0x80D6A9FC
// total size: 0xC
...
static class wait_state_type wait_state; // size: 0xC, address: 0x80D6AA00
static signed char init; // size: 0x1, address: 0x80D6AA0C
// total size: 0x8
class stop_state_type : public state_type {
    // Functions
    static stop_state_type();
};
static class stop_state_type stop_state; // size: 0x8, address: 0x80D6AA10
// Range: 0x801BF9CC -> 0x801BFAAC
void ztalkbox::init() {
    // References
...
struct {} @unnamed@zTalkBox_cpp@::stop_state_type::__vtable; // size: 0x14, address: 0x80D63B7C
// Range: 0x801BFAAC -> 0x801BFAE8
// this: r31
...
static // total size: 0x14
struct {} @unnamed@zTalkBox_cpp@::state_type::__vtable; // size: 0x14, address: 0x80D63B68
enum state_enum {
    STATE_INVALID = -1,
    BEGIN_STATE = 0,
    STATE_START = 1,
...
// Range: 0x801BFAE8 -> 0x801BFAF8
// this: r3
static state_type::state_type(enum state_enum type /* r4 */) {
    // References
    // -> static struct [anonymous] @unnamed@zTalkBox_cpp@::state_type::__vtable;
}

static // total size: 0x14
struct {} @unnamed@zTalkBox_cpp@::wait_state_type::__vtable; // size: 0x14, address: 0x80D63B54
// Range: 0x801BFAF8 -> 0x801BFB34
// this: r31
...
static // total size: 0x14
struct {} @unnamed@zTalkBox_cpp@::next_state_type::__vtable; // size: 0x14, address: 0x80D63B40
// Range: 0x801BFB34 -> 0x801BFB70
// this: r31
...
static // total size: 0x14
struct {} @unnamed@zTalkBox_cpp@::start_state_type::__vtable; // size: 0x14, address: 0x80D63B2C
// Range: 0x801BFB70 -> 0x801BFBAC
// this: r31
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D63B11
// Range: 0x801BFBAC -> 0x801BFD64
void ztalkbox::load(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r29 */) {
    // Local variables
...
// Range: 0x801BFD64 -> 0x801BFD74
char * asset_type::type_name() {}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D63B16
...
// Range: 0x801BFD74 -> 0x801BFF4C
void ztalkbox::update_all(struct xScene & s /* r1+0x8 */, float dt /* r1+0xC */) {
    // Local variables
    enum state_enum newtype; // r30
    struct ztalkbox & talk; // r31
    enum trigger_pads_enum tp; // r1+0x10
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D63B21
// Range: 0x801BFF4C -> 0x801C0080
...
// Range: 0x801C04FC -> 0x801C0568
static unsigned char trigger_jot(const struct jot & j /* r31 */) {
    // Local variables
    unsigned char (* trigger)(struct jot &); // r30
}

// Range: 0x801C0568 -> 0x801C056C
static void next_state_type::stop() {}

// Range: 0x801C056C -> 0x801C059C
static enum state_enum next_state_type::update() {
    // References
    // -> static struct shared_type shared;
}

// Range: 0x801C059C -> 0x801C05D0
// this: r31
static void wait_state_type::start() {}

// Range: 0x801C05D0 -> 0x801C0694
// this: r30
...
// Range: 0x801C0694 -> 0x801C08EC
// this: r30
static enum state_enum wait_state_type::update(float dt /* r1+0x8 */) {
    // Local variables
    unsigned int & pressed; // r31
...
// Range: 0x801C099C -> 0x801C09A0
static void stop_state_type::start() {}

// Range: 0x801C09A0 -> 0x801C09A4
static void stop_state_type::stop() {}

// Range: 0x801C09A4 -> 0x801C09AC
