/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zParticleLocator.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80227308 -> 0x8022C170
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64C08
static unsigned short popper_weight_sizes[8]; // size: 0x10, address: 0x80D6F230
static struct activity_data * activities; // size: 0x4, address: 0x80D6AF50
static int activities_used; // size: 0x4, address: 0x80D6AF54
static struct popper_data * poppers; // size: 0x4, address: 0x80D6AF58
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D64C14
// Range: 0x80227308 -> 0x802273AC
static struct activity_data * alloc_activity() {
    // Local variables
    struct activity_data * activity; // r31

    // References
    // -> static int activities_used;
    // -> static struct activity_data * activities;
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D64C23
// Range: 0x802273AC -> 0x802274A4
static void free_activity(struct activity_data * activity /* r31 */) {
    // Local variables
    struct activity_data * tail_activity; // r30
...
// Range: 0x802275D4 -> 0x80227640
static unsigned char setup_attach_fixed(struct activity_data & activity /* r31 */, const struct zParticleAsset & asset /* r30 */) {}

// Range: 0x80227640 -> 0x80227680
static void update_attach_entity(struct activity_data & activity /* r31 */) {}
...
// Range: 0x80227680 -> 0x802276DC
static void update_attach_entity_bone(struct activity_data & activity /* r31 */) {
    // Local variables
    struct xMat4x3 bone_mat; // r1+0x10
}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80D64C31
// Range: 0x802276DC -> 0x8022785C
static unsigned char setup_attach_entity(struct activity_data & activity /* r31 */, const struct zParticleAsset & asset /* r30 */) {
    // Local variables
    struct xEnt * attach_to; // r29
    unsigned int need_update; // r27

    // References
    // -> static char __FUNCTION__[20];
}

// Range: 0x8022785C -> 0x80227864
static unsigned char setup_attach_entity_tag() {}

// Range: 0x80227864 -> 0x80227890
static void get_offset_point(struct xVec3 & offset /* r1+0x8 */) {}
...
// Range: 0x80227890 -> 0x802278A4
static unsigned char setup_volume_point(struct activity_data & activity /* r3 */) {}

// Range: 0x802278A4 -> 0x802278AC
static void edge_emit_point(struct activity_data & activity /* r3 */, unsigned char edge_only /* r4 */) {}

// Range: 0x802278AC -> 0x802279F0
static void get_offset_sphere(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    float ang; // f29
    float uz; // f30
    float r; // f28
...
// Range: 0x802279F0 -> 0x80227AF0
static void get_offset_sphere_edge(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    float ang; // f30
    float uz; // f31
    float r; // f29
...
// Range: 0x80227AF0 -> 0x80227B5C
static unsigned char setup_volume_sphere(struct activity_data & activity /* r3 */, const struct zParticleAsset & asset /* r4 */) {
    // Local variables
    int edge_only; // r31
}

// Range: 0x80227B5C -> 0x80227B94
static void edge_emit_sphere(struct activity_data & activity /* r3 */, unsigned char edge_only /* r4 */) {}

// Range: 0x80227B94 -> 0x80227C74
static void get_offset_circle(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    float ang; // f31
    float rr; // f30
}

// Range: 0x80227C74 -> 0x80227D24
static void get_offset_circle_edge(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    float ang; // f31
    float rr; // f30
}

// Range: 0x80227D24 -> 0x80227E3C
static void get_offset_circle_cylinder(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    float ang; // f31
    float rr; // f30
    float z; // f29
...
// Range: 0x80227E3C -> 0x80227EB0
static void edge_emit_circle(struct activity_data & activity /* r3 */, unsigned char edge_only /* r4 */) {}

// Range: 0x80227EB0 -> 0x80227F60
static unsigned char setup_volume_circle(struct activity_data & activity /* r30 */, const struct zParticleAsset & asset /* r31 */) {
    // Local variables
    unsigned char edge_only; // r28
}

// Range: 0x80227F60 -> 0x80227FDC
static void get_offset_line(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    float d; // f31
}

// Range: 0x80227FDC -> 0x80228228
static void get_offset_capsule(struct xVec3 & offset /* r30 */, struct volume_data & data /* r31 */) {
    // Local variables
    float ang; // f30
    float uz; // f29
    float r; // f27
...
// Range: 0x80228228 -> 0x80228410
static void get_offset_capsule_edge(struct xVec3 & offset /* r30 */, struct volume_data & data /* r31 */) {
    // Local variables
    float ang; // f31
    float uz; // f30
    float r; // f27
...
// Range: 0x80228410 -> 0x80228554
static void get_offset_cylinder(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    float ang; // f31
    float d; // f29
    float rr; // f30
...
// Range: 0x80228554 -> 0x80228668
static void get_offset_cylinder_edge(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    float ang; // f31
    float d; // f29
    float rr; // f30
...
// Range: 0x80228668 -> 0x802287B8
static unsigned char setup_volume_line(struct activity_data & activity /* r30 */, const struct zParticleAsset & asset /* r24 */) {
    // Local variables
    const struct volume_line_data & line; // r31
    int edge_only; // r28
    struct volume_data & ovolume; // r29
    float r; // f29
    float d; // f28
    float rr; // f31
}

// Range: 0x802287B8 -> 0x8022885C
static void edge_emit_line(struct activity_data & activity /* r3 */, unsigned char edge_only /* r4 */) {
    // Local variables
    const struct volume_line_data & line; // r31
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x805239A0
// Range: 0x8022885C -> 0x802289D4
static void get_offset_model_vert(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    int vert_index; // r28
    struct xModelInstance * * models; // r30
    const int * vertices; // r29

    // References
    // -> static char __FUNCTION__[22];
}

// Range: 0x802289D4 -> 0x80228A04
static void get_offset_model_vert_expand(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r1+0xC */) {}

// Range: 0x80228A04 -> 0x80228A58
static void get_offset_model_surface(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    struct xVec3 vel; // r1+0xC
}

// Range: 0x80228A58 -> 0x80228BB8
static unsigned char setup_volume_model(struct activity_data & activity /* r31 */, const struct zParticleAsset & asset /* r1+0x8 */) {
    // Local variables
    struct xEnt * attach_to; // r29
    const struct volume_model_data & model; // r28
    int model_index; // r25
    int models_size; // r27
    int total_vertices; // r26
    struct xModelInstance * m; // r30
    struct RpGeometry * geom; // r22
    int size; // r24
}

// Range: 0x80228BB8 -> 0x80228BC0
static void edge_emit_model(struct activity_data & activity /* r3 */, unsigned char edge_only /* r4 */) {}

// Range: 0x80228BC0 -> 0x80228CC0
static void get_offset_box(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    float x; // f31
    float y; // f30
    float z; // f29
...
// Range: 0x80228CC0 -> 0x80228D80
static void get_offset_box_edge(struct xVec3 & offset /* r1+0x8 */, struct volume_data & data /* r31 */) {
    // Local variables
    float x; // f31
    float z; // f30
}

// Range: 0x80228D80 -> 0x80228E14
static unsigned char setup_volume_box(struct activity_data & activity /* r31 */, const struct zParticleAsset & asset /* r27 */) {
    // Local variables
    int edge_only; // r30
}

// Range: 0x80228E14 -> 0x80228E4C
static void edge_emit_box(struct activity_data & activity /* r3 */, unsigned char edge_only /* r4 */) {}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64C45
...
// total size: 0x4
struct /* @class$884zParticleLocator_cpp */ {
    // Members
...
union /* @class$1040zParticleLocator_cpp */ {
...
// total size: 0xC
struct /* @class$1039zParticleLocator_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1289zParticleLocator_cpp */ {
    // Members
...
union attach_data {
    struct attach_fixed_data fixed; // offset 0x0, size 0x1
    struct attach_entity_data entity; // offset 0x0, size 0x8
    struct attach_entity_tag_data entity_tag; // offset 0x0, size 0x10
};
union motion_data {
    struct motion_none_data none; // offset 0x0, size 0x1
    struct motion_spiral_data spiral; // offset 0x0, size 0x14
};
union volume_data {
    struct volume_point_data point; // offset 0x0, size 0x1
    struct volume_sphere_data sphere; // offset 0x0, size 0x4
    struct volume_circle_data circle; // offset 0x0, size 0xC
    struct volume_line_data line; // offset 0x0, size 0xC
    struct volume_model_data model; // offset 0x0, size 0x8
    struct volume_box_data box; // offset 0x0, size 0xC
};
// total size: 0x6C
struct zParticleAsset : public xDynAsset {
    // Members
    ...
    float emitDistance; // offset 0x1C, size 0x4
    unsigned char attach_type; // offset 0x20, size 0x1
    unsigned char motion_type; // offset 0x21, size 0x1
    unsigned char volume_type; // offset 0x22, size 0x1
    unsigned char system_type; // offset 0x23, size 0x1
    struct xVec3 location; // offset 0x24, size 0xC
    // total size: 0xC
    struct /* @class$1289zParticleLocator_cpp */ {
        // Members
        float yaw; // offset 0x0, size 0x4
        float pitch; // offset 0x4, size 0x4
        float roll; // offset 0x8, size 0x4
    } axis; // offset 0x30, size 0xC
    union attach_data attach; // offset 0x3C, size 0x10
    union motion_data motion; // offset 0x4C, size 0x14
    union volume_data volume; // offset 0x60, size 0xC
};
// total size: 0x100
struct debug_info {
    // Members
    char prefix[128]; // offset 0x0, size 0x80
    struct zParticleAsset * asset; // offset 0x80, size 0x4
    struct xEnt * attach_to; // offset 0x84, size 0x4
    struct xEnt * original_attach_to; // offset 0x88, size 0x4
    int attach_tweaking; // offset 0x8C, size 0x4
    int motion_tweaking; // offset 0x90, size 0x4
    int volume_tweaking; // offset 0x94, size 0x4
    struct attach_fixed_data attach_fixed; // offset 0x98, size 0x1
    struct attach_entity_data attach_entity; // offset 0x9C, size 0x8
    struct attach_entity_tag_data attach_entity_tag; // offset 0xA4, size 0x10
    struct motion_none_data motion_none; // offset 0xB4, size 0x1
    struct motion_spiral_data motion_spiral; // offset 0xB8, size 0x14
    struct volume_point_data volume_point; // offset 0xCC, size 0x1
    struct volume_sphere_data volume_sphere; // offset 0xD0, size 0x4
    struct volume_circle_data volume_circle; // offset 0xD4, size 0xC
    struct volume_line_data volume_line; // offset 0xE0, size 0xC
    struct volume_model_data volume_model; // offset 0xEC, size 0x8
    struct volume_box_data volume_box; // offset 0xF4, size 0xC
};
// total size: 0xC
class zParticleLocator {
    ...
    unsigned char activate(const struct zParticleAsset & asset, struct xEnt * attach_to);

    void deactivate();

    void setEmitFromEdge(unsigned char edgeEmit);

    void update(float dt);

    void make_locations(void * data, int stride, int elements, int loc_offset);

    struct xMat4x3 & get_frame() const;

    void get_center(struct xVec3 & center) const;

    void get_bounding_sphere(struct xSphere & sphere) const;

    float get_height() const;

    float get_area() const;

    void add_tweaks(const char * prefix, struct zParticleAsset & asset, struct xEnt * attach_to);

    void debug_render();

    void debug_init();

    struct xEnt * get_attach_to() const;

    void on_tweaked();

    void refresh_attach_tweaks();

    void refresh_motion_tweaks();

    void refresh_volume_tweaks();

    // Members
public:
    struct activity_data * activity; // offset 0x0, size 0x4
    struct xEnt * ent; // offset 0x4, size 0x4
    struct debug_info * debug; // offset 0x8, size 0x4
};
// Range: 0x80228E4C -> 0x80228F48
void zParticleLocator::scene_enter() {
    // Local variables
    unsigned int index; // r30
    struct popper_data * it; // r31
    struct popper_data * end; // r29

    // References
    // -> static unsigned short popper_weight_sizes[8];
    // -> static char __FUNCTION__[12];
    // -> unsigned int gActiveHeap;
    // -> static struct popper_data * poppers;
    // -> static int activities_used;
    // -> static struct activity_data * activities;
}

// Range: 0x80228F48 -> 0x80228F64
void zParticleLocator::scene_exit() {
    // References
    // -> static struct popper_data * poppers;
    // -> static int activities_used;
    // -> static struct activity_data * activities;
...
// Range: 0x80228F64 -> 0x80228F70
// this: r3
void zParticleLocator::create() {}

// Range: 0x80228F70 -> 0x80228F74
void zParticleLocator::restart() {}

static unsigned char (* const setup_attach_table[3])(struct activity_data &, struct zParticleAsset &); // size: 0xC, address: 0x80D6F274
static unsigned char (* const setup_volume_table[6])(struct activity_data &, struct zParticleAsset &); // size: 0x18, address: 0x804BF1A0
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D64C51
// Range: 0x80228F74 -> 0x80229134
// this: r31
unsigned char zParticleLocator::activate(const struct zParticleAsset & asset /* r30 */, struct xEnt * attach_to /* r29 */) {
    // References
    // -> static unsigned char (* const setup_volume_table[6])(struct activity_data &, struct zParticleAsset &);
    // -> static char __FUNCTION__[9];
    // -> static unsigned char (* const setup_attach_table[3])(struct activity_data &, struct zParticleAsset &);
    // -> static int activities_used;
}

// Range: 0x80229134 -> 0x802291B8
// this: r31
void zParticleLocator::deactivate() {
    // Local variables
    const struct volume_model_data & model; // r30
}

static void (* const edge_emit_table[6])(struct activity_data &, unsigned char); // size: 0x18, address: 0x804BF1B8
// Range: 0x802291B8 -> 0x80229214
// this: r31
void zParticleLocator::setEmitFromEdge(unsigned char edgeEmit /* r1+0x8 */) {
    // References
    // -> static void (* const edge_emit_table[6])(struct activity_data &, unsigned char);
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80D64C5A
// Range: 0x80229214 -> 0x802292C8
// this: r31
void zParticleLocator::update(float dt /* r1+0x8 */) {
    // References
    // -> static char __FUNCTION__[7];
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D64C61
// Range: 0x802292C8 -> 0x802293E8
// this: r31
void zParticleLocator::make_locations(void * data /* r1+0x8 */, int stride /* r24 */, int elements /* r1+0xC */, int loc_offset /* r1+0x10 */) {
    // Local variables
    const struct xMat4x3 & amat; // r28
    void (* const get_offset)(struct xVec3 &, struct volume_data &); // r27
    struct volume_data & volume; // r26
    unsigned char * p; // r30
    unsigned char * endp; // r25
    struct xVec3 & loc; // r29
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D64C70
// Range: 0x802293E8 -> 0x80229478
// this: r31
struct xMat4x3 & zParticleLocator::get_frame() const {
    // References
    // -> static char __FUNCTION__[10];
}

// Range: 0x80229478 -> 0x80229580
// this: r30
void zParticleLocator::get_center(struct xVec3 & center /* r29 */) const {
    // Local variables
    const struct xMat4x3 & mat; // r31
    float hlen; // f31
}

// Range: 0x80229580 -> 0x802296E0
// this: r29
void zParticleLocator::get_bounding_sphere(struct xSphere & sphere /* r31 */) const {
    // Local variables
    const struct xMat4x3 & mat; // r30
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D64C7A
// Range: 0x802296E0 -> 0x80229834
// this: r31
float zParticleLocator::get_height() const {
    // Local variables
    const struct xMat4x3 & mat; // r30
    float yend; // f31
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D64C85
// Range: 0x80229834 -> 0x80229A50
// this: r31
float zParticleLocator::get_area() const {
    // Local variables
    float area; // f31
    float area; // r1+0x8
    const struct volume_model_data & model; // r30
...
char __FUNCTION__$localstatic1$SkinXformVertAndNormal__FP5xVec3P5xVec3PC5xVec3PC5xVec3PC7xMat4x3PC7xMat4x3PCfPCUiPCUsUi[23]; // size: 0x17, address: 0x805239B6
// total size: 0x0
struct RpSkin {};
// Range: 0x80229A50 -> 0x80229BC0
void eval_tri(struct xVec3 * vert /* r25 */, struct xVec3 * norm /* r29 */, const struct xMat4x3 * mat /* r30 */, const struct RpGeometry * geom /* r20 */, const struct RpTriangle * tri /* r31 */) {
    // Local variables
    const struct RpSkin * skin; // r28
    const struct RpMorphTarget * mt; // r24
    const struct xVec3 * in_vert; // r27
    const struct xVec3 * in_norm; // r26
    const struct xMat4x3 * skinmat; // r23
    const float * vert_bone_weight; // r22
    const unsigned int * vert_bone_index; // r21
}

// Range: 0x80229BC0 -> 0x80229EA0
void SkinXformVertAndNormal(struct xVec3 * vert /* r1+0x8 */, struct xVec3 * norm /* r1+0xC */, const struct xVec3 * in_vert /* r1+0x10 */, const struct xVec3 * in_norm /* r1+0x14 */, const struct xMat4x3 * mat /* r21 */, const struct xMat4x3 * skinmat /* r1+0x18 */, const float * wt /* r1+0x1C */, const unsigned int * idx /* r1+0x20 */, const unsigned short * vert_index /* r22 */, unsigned int count /* r17 */) {
    // Local variables
    unsigned int catMatFlags[2]; // r1+0x34
    struct xMat4x3 * catmat; // r27
    const struct xMat4x3 * rootmat; // r20
    unsigned short i; // r26
    const struct xVec3 * cur_in_vert; // r1+0x30
    const struct xVec3 * cur_in_norm; // r1+0x2C
    unsigned int cur_idx; // r24
    const float * cur_wt; // r19
    unsigned int i; // r25
    unsigned int midx; // r30
    const struct xMat4x3 * pMatrix; // r23
    const float * fwt; // r31
    unsigned int wtidx; // r29
    unsigned int maxwt; // r28
    struct xVec3 accumV; // r1+0x54
    struct xVec3 v; // r1+0x48
    struct xVec3 v; // r1+0x3C

    // References
    // -> char __FUNCTION__$localstatic1$SkinXformVertAndNormal__FP5xVec3P5xVec3PC5xVec3PC5xVec3PC7xMat4x3PC7xMat4x3PCfPCUiPCUsUi[23];
}

// Range: 0x80229EA0 -> 0x8022A034
void random_point_on_triangle(struct xVec3 & loc /* r28 */, struct xVec3 & norm /* r29 */, const struct xVec3 * v /* r30 */, const struct xVec3 * n /* r31 */) {
    // Local variables
    float s; // f31
    float t; // f30
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x805239CD
// Range: 0x8022A034 -> 0x8022A16C
void random_surface_point(struct xVec3 & loc /* r1+0x8 */, struct xVec3 & norm /* r1+0xC */, const struct popper_data & popper /* r31 */) {
    // Local variables
    const struct xMat4x3 * mat; // r26
    int which; // r29
    struct RpGeometry * geom; // r28
    struct RpAtomic * const * atomic; // r30
    struct RpAtomic * const * end; // r25
    int size; // r27
    struct RpTriangle * tri; // r24
    struct xVec3 v[3]; // r1+0x34
    struct xVec3 n[3]; // r1+0x10
...
struct popper_data {
    // Functions
    static int find_weight(float v) const;

    // Members
    enum state_enum state; // offset 0x0, size 0x4
    struct xEnt * ent; // offset 0x4, size 0x4
    struct RpAtomic * atomic[4]; // offset 0x8, size 0x10
    unsigned int atomic_size; // offset 0x18, size 0x4
    float emitted; // offset 0x1C, size 0x4
    float radius; // offset 0x20, size 0x4
    float area; // offset 0x24, size 0x4
    int faces; // offset 0x28, size 0x4
    int max_faces; // offset 0x2C, size 0x4
    float * weight; // offset 0x30, size 0x4
};
// Range: 0x8022A16C -> 0x8022A2F8
// this: r31
static int popper_data::find_weight(float v /* f31 */) const {
    // Local variables
    int start; // r30
    int end; // r29
    int index; // r28
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x805239E2
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// total size: 0x10
struct /* @class$1345zParticleLocator_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x8022A2F8 -> 0x8022A4E0
unsigned char setup_popper_emitter(struct popper_data & popper /* r31 */) {
    // Local variables
    float * weight; // r28
    struct xModelInstance * model; // r30
    struct RpGeometry * geom; // r27
    int size; // r25
    struct RpMorphTarget * mt; // r24
    const struct xVec3 * vert; // r26
    const struct RpTriangle * tri; // r29
    float * end; // r23
    const struct xVec3 & a; // r22
    const struct xVec3 & b; // r21
    const struct xVec3 & c; // r20
...
// Range: 0x8022A4E0 -> 0x8022A540
int count_faces(struct xModelInstance * model /* r31 */) {
    // Local variables
    int total; // r30
    struct RpGeometry * geom; // r29
}

// Range: 0x8022A540 -> 0x8022A6BC
float get_triangle_area(const struct xVec3 & a /* r31 */, const struct xVec3 & b /* r29 */, const struct xVec3 & c /* r30 */) {
    // Local variables
    struct xVec3 ab; // r1+0x20
    struct xVec3 ac; // r1+0x14
    struct xVec3 abXac; // r1+0x8
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64C8E
// Range: 0x8022A6BC -> 0x8022A79C
struct popper_data * grab_popper(struct xEnt * ent /* r30 */) {
    // Local variables
    struct popper_data * popper; // r31
    int numFaces; // r29
...
// Range: 0x8022A79C -> 0x8022A7E8
struct popper_data * find_popper(struct xEnt * ent /* r3 */) {
    // Local variables
    struct popper_data * it; // r31
    struct popper_data * end; // r30

    // References
    // -> static struct popper_data * poppers;
}

// Range: 0x8022A7E8 -> 0x8022A85C
struct popper_data * find_free_popper(int facesNeeded /* r3 */) {
    // Local variables
    struct popper_data * found; // r30
    struct popper_data * it; // r31
    struct popper_data * end; // r29
...
static struct tweak_callback cb_tweaked; // size: 0x28, address: 0x804BF1D0
static struct tweak_callback cb_tweaked_degrees; // size: 0x28, address: 0x804BF1F8
// total size: 0xC
struct range_type {
    // Members
...
static struct range_type attach_range[3]; // size: 0x24, address: 0x804BF220
static struct range_type motion_range[2]; // size: 0x18, address: 0x804BF244
static struct range_type volume_range[6]; // size: 0x48, address: 0x804BF25C
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64C9A
// Range: 0x8022A85C -> 0x8022A93C
static void copy_attach(struct debug_info & debug /* r1+0x8 */, const struct zParticleAsset & asset /* r1+0xC */, int type /* r31 */) {
    // References
    // -> static struct range_type attach_range[3];
    // -> static char __FUNCTION__[12];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64CA6
// Range: 0x8022A93C -> 0x8022AA1C
static void copy_attach(struct zParticleAsset & asset /* r1+0x8 */, const struct debug_info & debug /* r1+0xC */, int type /* r31 */) {
    // References
    // -> static struct range_type attach_range[3];
    // -> static char __FUNCTION__[12];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64CB2
// Range: 0x8022AA1C -> 0x8022AAFC
static void copy_motion(struct debug_info & debug /* r1+0x8 */, const struct zParticleAsset & asset /* r1+0xC */, int type /* r31 */) {
    // References
    // -> static struct range_type motion_range[2];
    // -> static char __FUNCTION__[12];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64CBE
// Range: 0x8022AAFC -> 0x8022ABDC
static void copy_motion(struct zParticleAsset & asset /* r1+0x8 */, const struct debug_info & debug /* r1+0xC */, int type /* r31 */) {
    // References
    // -> static struct range_type motion_range[2];
    // -> static char __FUNCTION__[12];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64CCA
// Range: 0x8022ABDC -> 0x8022ACBC
static void copy_volume(struct debug_info & debug /* r1+0x8 */, const struct zParticleAsset & asset /* r1+0xC */, int type /* r31 */) {
    // References
    // -> static struct range_type volume_range[6];
    // -> static char __FUNCTION__[12];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64CD6
// Range: 0x8022ACBC -> 0x8022AD9C
static void copy_volume(struct zParticleAsset & asset /* r1+0x8 */, const struct debug_info & debug /* r1+0xC */, int type /* r31 */) {
    // References
    // -> static struct range_type volume_range[6];
    // -> static char __FUNCTION__[12];
}

static char * attach_labels[3]; // size: 0xC, address: 0x80D64CE4
static char * motion_labels[2]; // size: 0x8, address: 0x80D64CF0
static char * volume_labels[6]; // size: 0x18, address: 0x805239F8
// Range: 0x8022AD9C -> 0x8022B12C
// this: r31
void zParticleLocator::add_tweaks(const char * prefix /* r29 */, struct zParticleAsset & asset /* r28 */, struct xEnt * attach_to /* r1+0x10 */) {
    // Local variables
    struct zParticleAsset & a; // r30

    // References
    // -> static struct tweak_callback cb_tweaked;
    // -> static char * volume_labels[6];
    // -> static char * motion_labels[2];
    // -> static char * attach_labels[3];
    // -> static struct tweak_callback cb_tweaked_degrees;
}
...
// Range: 0x8022B12C -> 0x8022B42C
// this: r31
void zParticleLocator::debug_render() {
    // Local variables
    const struct xMat4x3 & amat; // r30
    const struct xVec3 & ref; // r26
    const struct xVec3 & center; // r28
    float radius; // f31
    const struct xVec3 & start; // r25
    struct xVec3 end; // r1+0x28
    const struct volume_line_data & line; // r29
    const struct xVec3 & start; // r27
    struct xVec3 end; // r1+0x1C
    struct xVec3 dimensions; // r1+0x10
    struct xBox box; // r1+0x34
...
// total size: 0x890
struct /* @class$1842zParticleLocator_cpp */ {
    // Static members
...
static struct attach_fixed_data default_attach_fixed; // size: 0x1, address: 0x80D6F298
// total size: 0x8
...
static struct attach_entity_data default_attach_entity; // size: 0x8, address: 0x80D6F29C
// total size: 0x10
struct attach_entity_tag_data {
    // Members
    unsigned int entity; // offset 0x0, size 0x4
    struct xVec3 tag; // offset 0x4, size 0xC
};
static struct attach_entity_tag_data default_attach_entity_tag; // size: 0x10, address: 0x80D6F2A4
// total size: 0x1
struct motion_none_data {};
static struct motion_none_data default_motion_none; // size: 0x1, address: 0x80D6F2B4
// total size: 0x14
...
static struct motion_spiral_data default_motion_spiral; // size: 0x14, address: 0x80D6F2B8
// total size: 0x1
struct volume_point_data {};
static struct volume_point_data default_volume_point; // size: 0x1, address: 0x80D6F2CC
// total size: 0x4
struct volume_sphere_data {
    // Members
    float radius; // offset 0x0, size 0x4
};
static struct volume_sphere_data default_volume_sphere; // size: 0x4, address: 0x80D6F2D0
// total size: 0xC
...
static struct volume_circle_data default_volume_circle; // size: 0xC, address: 0x80D6F2D4
// total size: 0xC
...
static struct volume_line_data default_volume_line; // size: 0xC, address: 0x80D6F2E0
// total size: 0x8
...
static struct volume_model_data default_volume_model; // size: 0x8, address: 0x80D6F2EC
// total size: 0xC
struct volume_box_data {
    // Members
    struct xVec3 dimensions; // offset 0x0, size 0xC
};
static struct volume_box_data default_volume_box; // size: 0xC, address: 0x80D6F2F4
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D64CF8
// Range: 0x8022B42C -> 0x8022B578
// this: r31
void zParticleLocator::debug_init() {
    // References
    // -> static struct volume_box_data default_volume_box;
    // -> static struct volume_model_data default_volume_model;
    // -> static struct volume_line_data default_volume_line;
    // -> static struct volume_circle_data default_volume_circle;
    // -> static struct volume_sphere_data default_volume_sphere;
    // -> static struct volume_point_data default_volume_point;
    // -> static struct motion_spiral_data default_motion_spiral;
    // -> static struct motion_none_data default_motion_none;
    // -> static struct attach_entity_tag_data default_attach_entity_tag;
    // -> static struct attach_entity_data default_attach_entity;
    // -> static struct attach_fixed_data default_attach_fixed;
    // -> static char __FUNCTION__[11];
}

// Range: 0x8022B6BC -> 0x8022B780
// this: r31
struct xEnt * zParticleLocator::get_attach_to() const {
    // Local variables
    unsigned int id; // r29
    struct xEnt * ent; // r30
}
...
// total size: 0xC
struct /* @class$825zParticleLocator_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$826zParticleLocator_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$827zParticleLocator_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$828zParticleLocator_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$829zParticleLocator_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$830zParticleLocator_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$831zParticleLocator_cpp */ {
    // Members
...
// Range: 0x8022B780 -> 0x8022B7B8
void zParticleLocator::on_tweaked(const struct tweak_info & info /* r30 */) {
    // Local variables
    class zParticleLocator * owner; // r31
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D64D03
// total size: 0x8
struct /* @class$1935zParticleLocator_cpp */ {
    // Members
    float ang_delta; // offset 0x0, size 0x4
    float height; // offset 0x4, size 0x4
};
// total size: 0x8
struct /* @class$1936zParticleLocator_cpp */ {
    // Members
    float length; // offset 0x0, size 0x4
    unsigned int cap_weight; // offset 0x4, size 0x4
};
// total size: 0x48
struct /* @class$1937zParticleLocator_cpp */ {
    // Members
    int vertices[8]; // offset 0x0, size 0x20
    struct xModelInstance * models[8]; // offset 0x20, size 0x20
    int models_size; // offset 0x40, size 0x4
    int total_vertices; // offset 0x44, size 0x4
};
// total size: 0x4
struct /* @class$1938zParticleLocator_cpp */ {
    // Members
    struct popper_data * popper; // offset 0x0, size 0x4
};
// total size: 0x18
struct /* @class$1939zParticleLocator_cpp */ {
    // Members
    struct xVec3 dim; // offset 0x0, size 0xC
    struct xVec3 halfDim; // offset 0xC, size 0xC
};
// total size: 0x4C
struct volume_data {
    // Members
    ...
    union { // inferred
        ...
        struct /* @class$1935zParticleLocator_cpp */ {
            ...
            float ang_delta; // offset 0x0, size 0x4
            float height; // offset 0x4, size 0x4
        } circle; // offset 0x4, size 0x8
        // total size: 0x8
        struct /* @class$1936zParticleLocator_cpp */ {
            // Members
            float length; // offset 0x0, size 0x4
            unsigned int cap_weight; // offset 0x4, size 0x4
        } line; // offset 0x4, size 0x8
        // total size: 0x48
        struct /* @class$1937zParticleLocator_cpp */ {
            // Members
            int vertices[8]; // offset 0x0, size 0x20
            ...
            int models_size; // offset 0x40, size 0x4
            int total_vertices; // offset 0x44, size 0x4
        } model_vert; // offset 0x4, size 0x48
        // total size: 0x4
        struct /* @class$1938zParticleLocator_cpp */ {
            // Members
            struct popper_data * popper; // offset 0x0, size 0x4
        } model_surface; // offset 0x4, size 0x4
        // total size: 0x18
        struct /* @class$1939zParticleLocator_cpp */ {
            // Members
            struct xVec3 dim; // offset 0x0, size 0xC
            struct xVec3 halfDim; // offset 0xC, size 0xC
        } box; // offset 0x4, size 0x18
    };
};
// total size: 0x50
struct /* @class$1941zParticleLocator_cpp */ {
    // Members
    struct xMat4x3 mat; // offset 0x0, size 0x40
    int bone; // offset 0x40, size 0x4
};
// total size: 0x50
struct attach_data {
    // Members
    ...
    struct /* @class$1941zParticleLocator_cpp */ {
        ...
        struct xMat4x3 mat; // offset 0x0, size 0x40
        int bone; // offset 0x40, size 0x4
    } entity; // offset 0x0, size 0x50
};
// total size: 0x100
struct activity_data {
    // Members
    class zParticleLocator * owner; // offset 0x0, size 0x4
    const struct zParticleAsset * asset; // offset 0x4, size 0x4
    void (* update_attach)(struct activity_data &, float); // offset 0x8, size 0x4
    void (* get_offset)(struct xVec3 &, struct volume_data &); // offset 0xC, size 0x4
    struct attach_data attach; // offset 0x10, size 0x50
    struct volume_data volume; // offset 0x60, size 0x4C
    struct xEnt * attach_to; // offset 0xAC, size 0x4
    struct xMat4x3 buffer_mat; // offset 0xB0, size 0x40
    struct xMat4x3 * attach_mat; // offset 0xF0, size 0x4
    unsigned char edge_only; // offset 0xF4, size 0x1
};
// Range: 0x8022B7B8 -> 0x8022B8F4
// this: r31
void zParticleLocator::on_tweaked() {
    // Local variables
    struct zParticleAsset & a; // r30
    struct xEnt * attach_to; // r29
    struct activity_data old_activity; // r1+0x10
...
// Range: 0x8022B8F4 -> 0x8022BB18
// this: r31
void zParticleLocator::refresh_attach_tweaks() {
    // Local variables
    char prefix[128]; // r1+0x18
    const char * * ent_labels; // r1+0x14
    const unsigned int * ent_ids; // r1+0x10
    unsigned int ents; // r30
    struct attach_entity_data & d; // r29
    struct attach_entity_tag_data & d; // r28
...
// Range: 0x8022BB18 -> 0x8022BD44
// this: r31
void zParticleLocator::refresh_motion_tweaks() {
    // Local variables
    char prefix[128]; // r1+0x10
    struct motion_spiral_data & d; // r30

    // References
    // -> static struct tweak_callback cb_tweaked;
}

enum /* @enum$1283zParticleLocator_cpp */ {
    FLAG_REVERSE_REPEAT = 1,
};
// Range: 0x8022BD44 -> 0x8022C170
// this: r31
void zParticleLocator::refresh_volume_tweaks() {
    // Local variables
    char prefix[128]; // r1+0x8
    struct volume_sphere_data & d; // r27
    struct volume_circle_data & d; // r28
    struct volume_line_data & d; // r29
    struct volume_model_data & d; // r30
...
enum /* @enum$1287zParticleLocator_cpp */ {
    FLAG_SURFACE = 1,
};
enum /* @enum$1286zParticleLocator_cpp */ {
    FLAG_CYLINDRICAL = 1,
};
