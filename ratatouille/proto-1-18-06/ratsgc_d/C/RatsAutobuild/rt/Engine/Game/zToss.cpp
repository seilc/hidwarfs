/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zToss.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8024D284 -> 0x8024D728
*/
// total size: 0x20
struct xLinkAsset {
    // Members
...
class toss : public zLightweight {
    // Functions
    toss(struct xEnt * ent, struct pointer_asset * dest_pointer, float arc_height, float speed, struct xBase * toss_event_sender);
...
// Range: 0x8024D284 -> 0x8024D288
void toss::Setup() {}

char __FUNCTION__$localstatic1$GetIndex__32zLightweightSystem<Q21z4toss,10>FUi[9]; // size: 0x9, address: 0x80D65258
// Range: 0x8024D288 -> 0x8024D300
class toss * toss::find(struct xEnt * ent /* r1+0x8 */) {
    // Local variables
    unsigned int i; // r31
    class toss * cur; // r30
}
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D65261
// total size: 0x8
struct xVec2 {
    // Members
...
// Range: 0x8024D300 -> 0x8024D614
// this: r31
toss::toss(struct xEnt * ent /* r29 */, struct pointer_asset * dest_pointer /* r30 */, float arc_height /* f31 */, float speed /* r1+0x10 */, struct xBase * toss_event_sender /* r1+0x14 */) {
    // Local variables
    struct xVec2 toDest2d; // r1+0x20
    float a; // f30
    float b; // f29
    float c; // f28
    float m0; // r1+0x1C
    ...
    struct xMat3x3 * curMat; // r27

    // References
    // -> static char __FUNCTION__[5];
    // -> struct [anonymous] z::toss::__vtable;
}
...
// Range: 0x8024D614 -> 0x8024D728
// this: r31
void toss::Update(float dt /* r1+0x8 */) {
    // Local variables
    float pct; // f31
    struct xVec3 newLoc; // r1+0x1C
    float ox; // f30
    struct xQuat curOrient; // r1+0xC
}
