/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zMain.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8017FBA0 -> 0x80183720
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct basic_rect screen_bounds; // size: 0x10, address: 0x80D6E388
enum eFMVFile {
    eFMVFile_NoMovie = 0,
...
unsigned int FB_DEPTH; // size: 0x4, address: 0x80D6301C
unsigned int SCENE_ID_MNU_INGAME; // size: 0x4, address: 0x80D6A5AC
...
// Range: 0x8017FBA0 -> 0x8017FBAC
void zMainSetPAL() {
    // References
    // -> enum _tagGameVidMode gVidMode;
}

// Range: 0x8017FBAC -> 0x8017FBB8
void zMainSetNTSC() {
    // References
    // -> enum _tagGameVidMode gVidMode;
}
...
float ShortbreadCookieJar; // size: 0x4, address: 0x80D6A5BC
static int sShowMenuOnBoot; // size: 0x4, address: 0x80D63024
// total size: 0x8
...
static struct st_SERIAL_PERCID_SIZE g_xser_sizeinfo[2]; // size: 0x10, address: 0x80D63028
int gDrawInvisibleWalls; // size: 0x4, address: 0x80D6A5C0
static int gCountryCodeCount; // size: 0x4, address: 0x80D6A5C4
static char gCountryCodes[4][3]; // size: 0xC, address: 0x80D6A5C8
// total size: 0x18
...
// Range: 0x8017FBB8 -> 0x8017FC94
static void render_rect(const struct basic_rect & bounds /* r31 */) {
    // Local variables
    float z; // f31
    float cooz; // f29
...
// Range: 0x8017FC94 -> 0x8017FDA4
static void set_rect_verts(struct rwGameCube2DVertex * vert /* r31 */, float x /* f28 */, float y /* f29 */, float w /* f26 */, float h /* f27 */, float z /* f30 */, float ooz /* f31 */) {}

// Range: 0x8017FDA4 -> 0x8017FDDC
static void set_rect_vert(struct rwGameCube2DVertex & vert /* r3 */, float x /* f1 */, float y /* f2 */, float z /* f3 */, float u /* f4 */, float v /* f5 */) {}

static char __FUNCTION__[26]; // size: 0x1A, address: 0x8051F520
// Range: 0x8017FDDC -> 0x8017FF40
static void zMainLanguageSelectionINI(struct xIniFile * ini /* r30 */) {
    // Local variables
    const char * _buf; // r31
    int index; // r1+0x8

    // References
    // -> static int gCountryCodeCount;
    // -> static char gCountryCodes[4][3];
...
static float sAttractMode_timer; // size: 0x4, address: 0x80D6A5D4
static signed char init; // size: 0x1, address: 0x80D6A5D8
static int been_here; // size: 0x4, address: 0x80D6A5DC
static char __FUNCTION__[23]; // size: 0x17, address: 0x8051F53A
...
// Range: 0x8017FF40 -> 0x80180884
static void zMainLanguageSelection() {
    // Local variables
    float flagXRES; // f28
    float flagWidth; // f31
    float flagSpacingX; // f27
    float flagSpacingY; // f26
    float flagHeight; // f30
    float flagStartX; // f24
    float flagStartY; // f23
    unsigned char pressedXAfterRemove; // r1+0xB
    unsigned char ignoreNextRelease; // r1+0xA
    int memBaseIdx; // r1+0x30
    struct RwTexture * textures[4][2]; // r1+0x68
    int highlighted; // r30
    const char * defaultRegion; // r1+0x2C
    int i; // r31
    unsigned int countryHash; // r17
    struct RwTexture * texture; // r1+0x28
    struct RwRaster * background; // r1+0x24
    struct RwRaster * noController; // r1+0x20
    long long prevTime; // r1+0x40
    float dt; // f29
    float flashTimer; // r1+0x1C
    unsigned char pressedA; // r27
    unsigned char pressedStart; // r26
    unsigned char pressedUp; // r25
    unsigned char pressedDown; // r24
    unsigned char pressedLeft; // r23
    unsigned char pressedRight; // r22
    unsigned char exitingFMV; // r20
    struct RwCamera * cam; // r21
    struct RwRGBA bg; // r1+0x18
    struct basic_rect bounds; // r1+0x58
    unsigned char FMVinput; // r1+0x9
    int startidx; // r1+0x14
    int port; // r29
    unsigned char noPad; // r1+0x8
    int i; // r28
    struct RwRaster * raster; // r1+0x10
    float flagX; // f22
    float flayY; // f21
    struct basic_rect bounds; // r1+0x48
    long long time; // r18
    struct xCamScreen local_screen; // r1+0x34
    int new_memBaseIdx; // r1+0xC
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D63038
...
// Range: 0x80180884 -> 0x80180F24
void main(int argc /* r21 */, char * * argv /* r22 */) {
    // Local variables
    unsigned int options; // r26
    const char * ini; // r25
    int i; // r29
    const char * arg; // r30
    int j; // r27
    int i; // r28
    struct xCamScreen local_screen; // r1+0xC
    int port; // r1+0x8
    int secs; // r24
    int i; // r31
    ...
    // -> unsigned int FB_YRES;
    // -> unsigned int FB_XRES;
    // -> static int sShowMenuOnBoot;
    // -> static struct st_SERIAL_PERCID_SIZE g_xser_sizeinfo[2];
}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80D6303D
// Range: 0x80180F24 -> 0x80181258
static void zMainOutputMgrSetup() {
    // References
    // -> static char __FUNCTION__[20];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D63051
// Range: 0x80181258 -> 0x801812F4
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x8051F551
// Range: 0x801812F4 -> 0x80181D0C
static void zMainParseINIGlobals(struct xIniFile * ini /* r31 */) {
    // Local variables
    const char * SCENE_ID_MNU_START_TEXT; // r29
    const char * videosys; // r27
    char DEFAULT_REGION_NTSC[3]; // r1+0xF
    char DEFAULT_REGION_PAL[3]; // r1+0xC
    const char * defaultregion; // r28
    const char * iniString; // r30
    int idx; // r1+0x8
    int playerCount; // r26
    unsigned int sceneTag; // r25
    unsigned int playerTag; // r22
    ...
    // -> unsigned int SCENE_ID_MNU_INGAME;
    // -> int GCN_DEMO_PARAM;
...
// Range: 0x80181D0C -> 0x80181DDC
static void zMainRegisterDebugModeCallbacks() {
    // Local variables
    int DM_BOULDER; // r31
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D63062
// Range: 0x80181DDC -> 0x80181E94
static void zMainMemLvlChkCB() {
    // References
    // -> static char __FUNCTION__[17];
}

static unsigned int gameSceneID; // size: 0x4, address: 0x80D6A5E0
static signed char init; // size: 0x1, address: 0x80D6A5E4
static unsigned int menuModeID; // size: 0x4, address: 0x80D6A5E8
static signed char init; // size: 0x1, address: 0x80D6A5EC
static int been_here; // size: 0x4, address: 0x80D6A5F0
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D63073
// total size: 0x10
struct zVarEntry {
    // Members
...
int sFirstBoot; // size: 0x4, address: 0x80D63090
...
// Range: 0x80181E94 -> 0x8018245C
static void zMainLoop() {
    // Local variables
    long long t; // r28
    int loopMemBaseIdx; // r27
    int new_memBaseIdx; // r26
    unsigned int newGameSceneID; // r30
    unsigned char stop; // r23
    unsigned char showLoadingScreen; // r25
    class zLoadingScreen screen; // r1+0x8C
    int force_draw_black; // r22
    int i; // r31
    unsigned int pbacksize; // r1+0x8
    char pbfilename[128]; // r1+0xC
    ...
    // -> struct zGlobals globals;
    // -> static int sShowMenuOnBoot;
    // -> static unsigned int menuModeID;
...
// total size: 0x1
struct /* @class$880zMain_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$881zMain_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$882zMain_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$883zMain_cpp */ {
    // Members
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D6307D
unsigned int gSoak; // size: 0x4, address: 0x80D6A1E0
char * allsoaklevels[]; // size: 0x0, address: 0x80D2AB80
char * * soaklevels; // size: 0x4, address: 0x80D6A20C
int enableAudioAnnihilator; // size: 0x4, address: 0x80D6A214
char gScreenDumpPath[]; // size: 0x0, address: 0x8055C52E
int enableSuperSoaker; // size: 0x4, address: 0x80D6A218
int enableMenuMasher; // size: 0x4, address: 0x80D6A210
// Range: 0x8018245C -> 0x8018304C
static void zMainReadINI(const char * iniName /* r1+0x8 */) {
    // Local variables
    const char * str; // r29
    void * buf; // r26
    unsigned int size; // r1+0xC
    struct xIniFile * ini; // r31
    unsigned int num_levels; // r25
    char * tok; // r27
    int level_count; // r28
    const char * pbackfile; // r23
    const char * screendumppath; // r22
    unsigned int len; // r30
    const char * watermark; // r24
    int arraySize; // r21
...
void zMainMemCardSpaceQuery() {
    ...
    int do_chk; // r23
    int fullCard; // r20
    unsigned char formatInProgress; // r28
    unsigned char formatFailed; // r22
    enum eStartupErrors startupError; // r29
    int activeCard; // r31
    char * workArea; // r24
    long resultCode; // r30
    unsigned char A_pressed; // r19
    unsigned char B_pressed; // r18
    unsigned char D_pressed; // r17
    int i; // r21
...
// Range: 0x80183070 -> 0x801830B0
static void zMainLoadFontHIP() {
    // Local variables
    int bootVersionNumber; // r1+0x8
}

static char buffer[128]; // size: 0x80, address: 0x80D00EC0
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x801830B0 -> 0x801832B8
char * zStealthDebugGetAssertionInfo() {
    // Local variables
    const char * sceneStr; // r31
    struct xVec3 pos; // r1+0x20
    char playerstring[16]; // r1+0x10
    int i; // r30
    const char * config; // r28
...
// Range: 0x801832B8 -> 0x801832C8
char * zStealthDebugGetAssertionHeader() {}

int percentageDone; // size: 0x4, address: 0x80D6A5F4
static class xtextbox backGroundTexture; // size: 0x74, address: 0x8051F568
static signed char init; // size: 0x1, address: 0x80D6A5F8
int sFirstBoot; // size: 0x4, address: 0x80D63090
// Range: 0x801832C8 -> 0x80183684
void zMainShowProgressBar(int percentage /* r25 */) {
    // Local variables
    const char * loadingText; // r31
    int spotToInsertColor; // r29
    char textBuffer[128]; // r1+0xC4
    char * working; // r1+0x1C
    const char * restOfString; // r1+0x18
    struct RwCamera * cam; // r30
    struct RwRGBA bg; // r1+0x14
    struct xColor_tag c1; // r1+0x10
    const char * str; // r28

    // References
    // -> static class xtextbox backGroundTexture;
