/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zNPCCommon.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x802B56A0 -> 0x802BD854
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static unsigned char DRAW_ANIM_PHYSICS; // size: 0x1, address: 0x80D6B560
// total size: 0x34
struct RwRaster {
    // Members
...
struct RwTexture * whiteTex; // size: 0x4, address: 0x80D6B564
static float FREEZING_MUL_R; // size: 0x4, address: 0x80D66490
static float FREEZING_MUL_G; // size: 0x4, address: 0x80D66494
static float FREEZING_MUL_B; // size: 0x4, address: 0x80D66498
float TEST_ALPHA; // size: 0x4, address: 0x80D6649C
// total size: 0xC
struct xModelPool {
    // Members
...
// total size: 0x4
struct /* @class$916zNPCCommon_cpp */ {
    // Members
...
// Range: 0x802B56A0 -> 0x802B5700
static void scale_model(struct xModelInstance * modelInst /* r31 */, float scale /* f31 */) {}
...
union /* @class$1055zNPCCommon_cpp */ {
...
// Range: 0x802B5700 -> 0x802B5758
static unsigned char is_conveyor_belt(struct xEnt * ent /* r3 */) {}

static int sNPCHitPoints[3][2]; // size: 0x18, address: 0x80526A18
enum ezHitReaction hitReactionTable[3][2][65]; // size: 0x618, address: 0x80526A30
// total size: 0x60
...
// total size: 0x4
struct /* @class$2090zNPCCommon_cpp */ {
    // Members
...
class zNPCCommon : public zNPCBaseIN2 {
    // Functions
    enum ezHitReaction GetDamageHitReaction(const struct zCombatDamageInfo & damageInfo, enum ezHitReactionState inState);

    void debug_display();

    void StoreEvent(struct xBase * from, struct xBase * to, unsigned int to_event, float * to_param, struct xBase * to_param_widget);

    void StoreAnimChange();
    ...
    unsigned char CanHelperAITarget();

    struct xEnt * GetAttackingPlayerEnt();

    void SetAttackingPlayerEnt(struct xEnt * inAttackingPlayer, float inTime);

    void check_transient_damage();

    void timestep(struct xScene * xscn, float dt);

    void ExitingSpawnState();

    unsigned char IsGrabbable();

    void CheckForDrivable();

    void CheckForSlowCollisionVolume();

    unsigned char ShieldHasEnergyToDeploy();

    void update_npc(float dt);

    void add_states(struct xAnimTable * table);

    void add_transitions(struct xAnimTable * table);

    struct zCombat * get_combat();

    void deliver_damage(const struct zCombatDamageInfo & damageInfo);

    unsigned char valid_damage(const struct zCombatDamageInfo & damageInfo);

    void AdjustDamage(struct zCombatDamageInfo & newInfo);

    void PlayDamageSound();

    void damage(const struct zCombatDamageInfo & damageInfo);

    void AboutToBeHitBy(struct xEnt * thrown);

    void scene_setup();

    void before_anim_matrices(struct xAnimPlay * play, struct xQuat * quat, struct xVec3 * tran, int boneCount);

    void init(class zNPCInstance * init_inst);

    void SetDefaultLightKit();
    ...
    void activate_visible();

    void deactivate_visible();
    ...
    unsigned char grabbable(enum GrabType grabType);

    unsigned char punchable();

    void GetNukeIconMatrix(struct xMat4x3 * posMat);

    void render_npc_nuke();

    void render_npc_stun();

    void render_npc_shield_generator();
    ...
    void SetDefaultAndEventStates();

    unsigned char systemEvent(struct xBase * from, struct xBase * to, unsigned int to_event, const float * to_param, struct xBase * to_param_widget, unsigned int to_param_widget_id);

    void ForceEnterDeathState();
    ...
    void render_npc_freezing();

    enum xSndHandle play_sound(struct zAnimFxSoundGroup * sound);

    void initialize_bounds(float modelScale);

    void change_bounds(float scale, struct xVec3 & offset, unsigned char use_bounding_box);

    void update_bounds();

    struct xVec3 get_bone_world_position(int bone);

    unsigned char GetParameter(const char * name, unsigned int * value, unsigned int defaultValue);

    unsigned char GetParameter(const char * name, int * value, int defaultValue);

    unsigned char GetParameter(const char * name, signed short * value, signed short defaultValue);

    unsigned char GetParameter(const char * name, unsigned char * value, unsigned char defaultValue);

    unsigned char GetParameter(const char * name, float * value, float defaultValue);

    unsigned char GetParameter(const char * name, struct xVec3 * value, const struct xVec3 & defaultValue);

    unsigned char GetParameter(const char * name, const char * * value, const char * defaultValue);

    int GetMaxHitPoints();

    void TellTargetAboutUs();

    unsigned char Freeze(float intensity);

    void UpdateFrozen(float dt);

    void PastUpdateDistCleanup();

    void VisibilityCullCleanup();

    void SetRandomAnimSpeed(float inMin, float inMax);

    void SetRandomAnimTime(float inMin, float inMax);

    void SetRandomAnimSpeedAndTime(float inMin, float inMax);

    void SetRandomAnimTime();

    void BuildStateMachine();

    float DistanceToPriorityTarget();

    float CalcTranslucency();
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80527048
enum ezHitReactionState {
    zHRS_Normal = 0,
    zHRS_Frozen = 1,
    zHRS_Total = 2,
};
// Range: 0x802B5758 -> 0x802B59A8
// this: r29
enum ezHitReaction zNPCCommon::GetDamageHitReaction(const struct zCombatDamageInfo & damageInfo /* r31 */, enum ezHitReactionState inState /* r30 */) {
    // Local variables
    enum ezHitReaction react; // r28
    struct zCombat * combat; // r27

    // References
    // -> enum ezHitReaction hitReactionTable[3][2][65];
...
// total size: 0xC
struct /* @class$2304zNPCCommon_cpp */ {
    // Members
...
struct glintInfo {
    ...
    float length_min; // offset 0x4, size 0x4
    float length_rand; // offset 0x8, size 0x4
    float length_max; // offset 0xC, size 0x4
};
// total size: 0x10
struct _xFColor {
    // Members
...
struct lightInfo {
    ...
    float up; // offset 0x14, size 0x4
    float max; // offset 0x18, size 0x4
    float down; // offset 0x1C, size 0x4
};
// total size: 0x94
struct zSparkEmitterAsset : public xDynAsset {
    // Static members
    static int VERSION; // size: 0x4

    // Members
    float update_distance; // offset 0x10, size 0x4
    unsigned int texture; // offset 0x14, size 0x4
    unsigned int flags; // offset 0x18, size 0x4
    unsigned int attach_obj_id; // offset 0x1C, size 0x4
    struct xVec3 position; // offset 0x20, size 0xC
    int num_sparks; // offset 0x2C, size 0x4
    float period; // offset 0x30, size 0x4
    float period_rand; // offset 0x34, size 0x4
    float lifetime_min; // offset 0x38, size 0x4
    float lifetime_rand; // offset 0x3C, size 0x4
    float yaw; // offset 0x40, size 0x4
    float pitch; // offset 0x44, size 0x4
    float dir_vary; // offset 0x48, size 0x4
    float velocity_min; // offset 0x4C, size 0x4
    float velocity_rand; // offset 0x50, size 0x4
    float gravity; // offset 0x54, size 0x4
    float size_min; // offset 0x58, size 0x4
    float size_rand; // offset 0x5C, size 0x4
    struct glintInfo glint; // offset 0x60, size 0x10
    struct lightInfo light; // offset 0x70, size 0x20
    unsigned int soundAsset; // offset 0x90, size 0x4
};
static struct zSparkEmitterAsset sNPCCommonDestroyShieldGeneratorSparks; // size: 0x94, address: 0x80D2CF48
unsigned char draw_npc_bounds; // size: 0x1, address: 0x80D6B568
unsigned char draw_npc_combat_bounds; // size: 0x1, address: 0x80D6B569
unsigned char m_bDebugAnimation; // size: 0x1, address: 0x80D6B56A
unsigned char update_npcs; // size: 0x1, address: 0x80D664A0
unsigned char m_bShowName; // size: 0x1, address: 0x80D664A1
unsigned char m_bShowAnimation; // size: 0x1, address: 0x80D664A2
unsigned char m_bShowBehavior; // size: 0x1, address: 0x80D664A3
unsigned char m_bShowTarget; // size: 0x1, address: 0x80D6B56B
unsigned char m_bShowHitTarget; // size: 0x1, address: 0x80D6B56C
unsigned char m_bShowFrozen; // size: 0x1, address: 0x80D6B56D
unsigned char m_bShowRecentEvents; // size: 0x1, address: 0x80D6B56E
unsigned char m_bShowRecentAnimChanges; // size: 0x1, address: 0x80D6B56F
unsigned char m_bShowHitPoints; // size: 0x1, address: 0x80D6B570
unsigned char m_bShowPosition; // size: 0x1, address: 0x80D6B571
unsigned char m_bShowShield; // size: 0x1, address: 0x80D6B572
unsigned char m_bShowDetection; // size: 0x1, address: 0x80D6B573
// Range: 0x802B59A8 -> 0x802B59C8
static void on_kill_all() {}

// Range: 0x802B59C8 -> 0x802B59CC
static void on_reset_all() {}
...
static struct tweak_callback cb_kill_all; // size: 0x28, address: 0x804D0250
static struct tweak_callback cb_reset_all; // size: 0x28, address: 0x804D0278
unsigned char draw_move_to; // size: 0x1, address: 0x80D6B30A
// Range: 0x802B59CC -> 0x802B5D24
unsigned char zNPCCommon::static_setup() {
    // Local variables
    char prefix[1]; // r1+0x8

    // References
    // -> static struct tweak_callback cb_reset_all;
    // -> static struct tweak_callback cb_kill_all;
    // -> float TEST_ALPHA;
    // -> static float FREEZING_MUL_B;
    // -> static float FREEZING_MUL_G;
    // -> static float FREEZING_MUL_R;
    // -> unsigned char m_bShowDetection;
    // -> unsigned char m_bShowPosition;
    // -> unsigned char m_bShowHitPoints;
    // -> unsigned char m_bShowShield;
    // -> unsigned char m_bShowRecentAnimChanges;
    // -> unsigned char m_bShowRecentEvents;
    // -> unsigned char m_bShowFrozen;
    // -> unsigned char m_bShowHitTarget;
    // -> unsigned char m_bShowTarget;
    // -> unsigned char m_bShowBehavior;
    // -> unsigned char m_bShowAnimation;
    // -> unsigned char m_bShowName;
    // -> unsigned char update_npcs;
    // -> unsigned char m_bDebugAnimation;
    // -> unsigned char draw_npc_combat_bounds;
    // -> unsigned char draw_npc_bounds;
    // -> unsigned char draw_move_to;
}

int DM_NPC; // size: 0x4, address: 0x80D6B574
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x802B5D24 -> 0x802B6534
// this: r31
void zNPCCommon::debug_display() {
    // Local variables
    struct xVec3 txtPos; // r1+0x40
    char messageTxt[2048]; // r1+0x4C
    struct zCombat * combat; // r28
    int hp; // r22
    int mhp; // r21
    struct xEnt * target; // r24
    long long lastTime; // r19
    int i; // r26
    struct zDebugStoredAnimChanges * ac; // r27
    float diffSec; // f31
    long long curTime; // r1+0x38
    int i; // r25
    struct zDebugStoredEvents * se; // r30
    float diffSec; // f30
    const char * fromName; // r18
    const char * widgetName; // r17
    struct xVec3 * pos; // r29
    struct zCombat * combat; // r23
...
// Range: 0x802B6534 -> 0x802B657C
static void bupdate_wrapper(struct xEnt * ent /* r31 */) {}

// Range: 0x802B657C -> 0x802B65F4
static void event_wrapper(struct xBase * from /* r1+0x8 */, struct xBase * to /* r31 */, unsigned int to_event /* r1+0xC */, const float * to_param /* r1+0x10 */, struct xBase * to_param_widget /* r1+0x14 */, unsigned int to_param_widget_id /* r1+0x18 */) {}

static char __FUNCTION__[22]; // size: 0x16, address: 0x8052705D
// Range: 0x802B65F4 -> 0x802B66D4
struct xModelInstance * zNPCCommonFindFXModel(unsigned int modelID /* r29 */) {
    // Local variables
    struct xModelInstance * minstance; // r31
...
// Range: 0x802B66D4 -> 0x802B67A8
// this: r29
void zNPCCommon::StoreEvent(struct xBase * from /* r24 */, struct xBase * to /* r25 */, unsigned int to_event /* r26 */, float * to_param /* r27 */, struct xBase * to_param_widget /* r28 */) {
    // Local variables
    struct zDebugStoredEvents * se; // r31
    int i; // r30
}

// Range: 0x802B67E4 -> 0x802B6894
// this: r31
void zNPCCommon::StoreAnimChange() {
    // Local variables
    int i; // r30
    int idx; // r29
}
...
// total size: 0x1D4
struct {} zNPC::zNPCCommon::__vtable; // size: 0x1D4, address: 0x805272C4
// Range: 0x802B68CC -> 0x802B6960
// this: r31
zNPCCommon::zNPCCommon() {
    // References
    // -> struct [anonymous] zNPC::zNPCCommon::__vtable;
}
...
// total size: 0x78
struct {} zNPC::zNPCBaseIN2::__vtable; // size: 0x78, address: 0x80527498
// Range: 0x802B6960 -> 0x802B69E0
// this: r31
zNPCCommon::~zNPCCommon() {
    // References
    // -> struct [anonymous] zNPC::zNPCCommon::__vtable;
...
// Range: 0x802B69E0 -> 0x802B6A78
void zNPCCommon::SceneSetup() {
    // References
    // -> static struct zSparkEmitterAsset sNPCCommonDestroyShieldGeneratorSparks;
    // -> struct RwTexture * whiteTex;
}

// Range: 0x802B6A78 -> 0x802B6B44
void zNPCCommon::create_anim_table(struct xAnimTable * table /* r31 */) {}
...
// Range: 0x802B6B44 -> 0x802B6E50
// this: r31
void zNPCCommon::apply_anim_physics() {
    // Local variables
    float yawDisp; // f30
    struct xMat3x3 rot; // r1+0x80
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D664A4
// Range: 0x802B6E50 -> 0x802B6F64
// this: r31
unsigned char zNPCCommon::CanHelperAITarget() {
    // Local variables
...
// Range: 0x802B6F64 -> 0x802B6F84
// this: r3
struct xEnt * zNPCCommon::GetAttackingPlayerEnt() {}

// Range: 0x802B6F84 -> 0x802B6F90
// this: r3
void zNPCCommon::SetAttackingPlayerEnt(struct xEnt * inAttackingPlayer /* r4 */, float inTime /* f1 */) {}
...
// Range: 0x802B6F90 -> 0x802B713C
// this: r28
void zNPCCommon::check_transient_damage() {
    // Local variables
    struct damage_result hits[8]; // r1+0x64
    struct xBox box; // r1+0x14
    int hits_found; // r27
    struct damage_result * hit; // r31
    struct damage_result * end_hit; // r26
    const struct xVec3 & hitcenter; // r30
    const struct xVec3 & npccenter; // r29
    struct xVec3 hitdir; // r1+0x8
    struct zCombatDamageInfo info; // r1+0x2C
...
// Range: 0x802B713C -> 0x802B75E0
// this: r31
void zNPCCommon::timestep(struct xScene * xscn /* r1+0x20 */, float dt /* f29 */) {
    // Local variables
    unsigned int boneIndex; // r30
    struct xMat4x3 boneMat; // r1+0x30
    struct zCombat * combat; // r29
    float originalSpeed; // f28
    float frozenSpeed; // f30
    float factor; // f31
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D664B6
...
// Range: 0x802B75E0 -> 0x802B76BC
// this: r31
void zNPCCommon::ExitingSpawnState() {
    // References
    // -> struct xGlobals * xglobals;
    // -> static char __FUNCTION__[18];
}

// Range: 0x802B76BC -> 0x802B7734
// this: r31
unsigned char zNPCCommon::IsGrabbable() {
    // References
    // -> struct xGlobals * xglobals;
}
...
static unsigned char done; // size: 0x1, address: 0x80D6B578
static signed char init; // size: 0x1, address: 0x80D6B579
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D664C8
// total size: 0x38
struct xGrid {
    // Members
...
// total size: 0xC
struct /* @class$1054zNPCCommon_cpp */ {
    // Members
...
// total size: 0x14
class StaticObjectsCallBack {
    // Functions
    StaticObjectsCallBack(const struct xSphere & sphere, struct xCollis * coll, unsigned char & index, unsigned char & startIndex, unsigned char & endIndex);
    ...
    unsigned char & startIndex; // offset 0xC, size 0x4
    unsigned char & endIndex; // offset 0x10, size 0x4
};
enum _xCollsIdx {
...
// Range: 0x802B7734 -> 0x802B7A34
// this: r26
void zNPCCommon::CheckForDrivable() {
    // Local variables
    class zNPCCommon & npc; // r31
    struct xEntCollis collisionRecords; // r1+0x178
    struct xBound tempBound; // r1+0x2C
    struct xSphere sphere; // r1+0x8
    class StaticObjectsCallBack checkPlatforms; // r1+0x18
    int i; // r27
    struct xEnt * floorEntity; // r30
    struct xCollis * collisionRec; // r29
    enum _xCollsIdx collsIndex; // r25
    struct xEnt * driver; // r28
    char tempString[256]; // r1+0x78
...
// Range: 0x802B7A34 -> 0x802B7A4C
// this: r3
StaticObjectsCallBack::StaticObjectsCallBack(const struct xSphere & sphere /* r4 */, struct xCollis * coll /* r5 */, unsigned char & index /* r6 */, unsigned char & startIndex /* r7 */, unsigned char & endIndex /* r8 */) {}
...
// Range: 0x802B7A4C -> 0x802B7B00
// this: r31
void zNPCCommon::CheckForSlowCollisionVolume() {
    // Local variables
    struct xVec3 npcPos; // r1+0x8
    struct zEntTrigger * trigger; // r30
}

// Range: 0x802B7B00 -> 0x802B7B18
// this: r3
unsigned char zNPCCommon::ShieldHasEnergyToDeploy() {}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D664D9
// total size: 0x1C
...
// Range: 0x802B7B18 -> 0x802B81F0
// this: r31
void zNPCCommon::update_npc(float dt /* f31 */) {
    // Local variables
    int i; // r28
    struct zPlatform * belt; // r27
    float beltSpeed; // f25
    struct xVec3 disp; // r1+0x24
    struct xVec3 initialDisp; // r1+0x18
    struct xSphere sphere; // r1+0x8
    float maxScale; // f30
    float maxChange; // f29
...
// Range: 0x802B81F0 -> 0x802B81F4
void zNPCCommon::NPCFXUpdate() {}

// Range: 0x802B81F4 -> 0x802B8234
// this: r1+0x8
void zNPCCommon::add_states(struct xAnimTable * table /* r1+0xC */) {}

// Range: 0x802B8234 -> 0x802B8274
// this: r1+0x8
void zNPCCommon::add_transitions(struct xAnimTable * table /* r1+0xC */) {}

// Range: 0x802B8274 -> 0x802B829C
// this: r1+0x8
struct zCombat * zNPCCommon::get_combat() {}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D664E4
// Range: 0x802B829C -> 0x802B8514
// this: r29
void zNPCCommon::deliver_damage(const struct zCombatDamageInfo & damageInfo /* r30 */) {
    // Local variables
    unsigned char npcHit; // r26
    struct zCombat * combat; // r31
    signed short beforeHitPoints; // r27
    struct zPlayer * fromPlayer; // r25
    int quarterHealth; // r28

    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D664F3
// Range: 0x802B8514 -> 0x802B866C
// this: r31
unsigned char zNPCCommon::valid_damage(const struct zCombatDamageInfo & damageInfo /* r30 */) {
    // References
    // -> static char __FUNCTION__[13];
}

// Range: 0x802B866C -> 0x802B87B8
// this: r29
void zNPCCommon::AdjustDamage(struct zCombatDamageInfo & newInfo /* r31 */) {
    // Local variables
    const struct zPlayer & player; // r30
...
// total size: 0x890
struct /* @class$1872zNPCCommon_cpp */ {
    // Static members
...
// total size: 0xC
struct /* @class$857zNPCCommon_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$858zNPCCommon_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$859zNPCCommon_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$860zNPCCommon_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$861zNPCCommon_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$862zNPCCommon_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$863zNPCCommon_cpp */ {
    // Members
...
struct xSerial {};
// total size: 0x4
struct /* @class$2049zNPCCommon_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2050zNPCCommon_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// total size: 0x4
struct /* @class$2051zNPCCommon_cpp */ {
    // Members
...
// total size: 0x8
struct /* @class$2048zNPCCommon_cpp */ {
    // Members
    float time; // offset 0x0, size 0x4
...
class zNPCBase {};
// total size: 0x2
struct /* @class$2053zNPCCommon_cpp */ {
    // Members
...
// total size: 0x10
struct /* @class$1390zNPCCommon_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x802B87B8 -> 0x802B88B8
// this: r31
void zNPCCommon::PlayDamageSound() {
    // Local variables
    unsigned int sndID; // r30
    unsigned int sndID2; // r29
    unsigned int loopSndID; // r28
    enum iSndGroupHandle soundGroupHandle; // r27
    enum iSndGroupHandle soundGroupHandle2; // r26
    enum iSndGroupHandle soundGroupHandle; // r25
}

// Range: 0x802B88B8 -> 0x802B8954
// this: r31
void zNPCCommon::damage(const struct zCombatDamageInfo & damageInfo /* r30 */) {
    // Local variables
    struct zCombatDamageInfo newInfo; // r1+0x8
}

// Range: 0x802B89C8 -> 0x802B89E0
// this: r3
void zNPCCommon::AboutToBeHitBy(struct xEnt * thrown /* r4 */) {}

// Range: 0x802B89E0 -> 0x802B8A18
// this: r1+0x8
void zNPCCommon::scene_setup() {}

// Range: 0x802B8A18 -> 0x802B8A7C
void zNPCCommon::BeforeAnimMatrices(struct xAnimPlay * play /* r31 */, struct xQuat * quat /* r1+0x8 */, struct xVec3 * tran /* r1+0xC */, int boneCount /* r1+0x10 */) {
    // Local variables
    class zNPCCommon * pBase; // r30
}

// Range: 0x802B8A7C -> 0x802B8AD4
// this: r1+0x8
void zNPCCommon::before_anim_matrices(struct xAnimPlay * play /* r1+0xC */, struct xQuat * quat /* r1+0x10 */, struct xVec3 * tran /* r1+0x14 */, int boneCount /* r1+0x18 */) {}

static unsigned char __bSetStatics; // size: 0x1, address: 0x80D6B57A
static signed char init; // size: 0x1, address: 0x80D6B57B
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D66500
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
struct xVec3 g_O3; // size: 0xC, address: 0x80D6D198
// total size: 0xC
struct xNPCSettingsIN2Asset {
    // Members
    unsigned int count; // offset 0x0, size 0x4
    struct xModelAssetParam param; // offset 0x4, size 0x8
};
// Range: 0x802B8AD4 -> 0x802B9118
// this: r31
void zNPCCommon::init(class zNPCInstance * init_inst /* r1+0x8 */) {
    // Local variables
    const struct zNPCAssetIN2 * npcAsset; // r30
    unsigned int bufsize; // r1+0x14
    struct xNPCSettingsIN2Asset * settingsAsset; // r29
    unsigned char tempBool; // r1+0xC
    float scale; // f31
    struct xSphere s; // r1+0x18
    const char * group_name; // r1+0x10

    // References
    // -> struct xVec3 g_O3;
    // -> static char __FUNCTION__[5];
    // -> unsigned int gActiveHeap;
    // -> static unsigned char __bSetStatics;
    // -> static signed char init;
    // -> struct xGlobals * xglobals;
}

float zNPCFreezeTimeTable[3][2]; // size: 0x18, address: 0x80D01188
static unsigned char already_asserted; // size: 0x1, address: 0x80D6B57C
static signed char init; // size: 0x1, address: 0x80D6B57D
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D66505
// Range: 0x802B9118 -> 0x802B925C
// this: r29
void zNPCCommon::SetDefaultLightKit() {
    // Local variables
    const struct zNPCAssetIN2 * npcAsset; // r28
    struct xLightKit * lightKit; // r30
    struct _zEnv * zen; // r27
    struct xEnvAsset * easset; // r31
...
// Range: 0x802B925C -> 0x802B9290
// this: r1+0x8
void zNPCCommon::revive() {}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D66518
struct xGrid npcs_grid; // size: 0x38, address: 0x80CF49B0
// Range: 0x802B9290 -> 0x802B9420
// this: r31
void zNPCCommon::activate() {
    // Local variables
    int i; // r30
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D66521
// Range: 0x802B9420 -> 0x802B95D8
// this: r31
void zNPCCommon::deactivate() {
    // Local variables
    int i; // r30
...
// Range: 0x802B95D8 -> 0x802B9620
// this: r31
void zNPCCommon::activate_visible() {
    // References
    // -> struct xGrid npcs_grid;
}

// Range: 0x802B9620 -> 0x802B96A4
// this: r31
void zNPCCommon::deactivate_visible() {}

// Range: 0x802B96A4 -> 0x802B96CC
// this: r3
void zNPCCommon::imminent_death() {}

// Range: 0x802B96CC -> 0x802B97BC
// this: r31
void zNPCCommon::kill() {}

// Range: 0x802B97BC -> 0x802B9800
// this: r30
unsigned char zNPCCommon::alive() {}
...
// Range: 0x802B9800 -> 0x802B9860
// this: r31
unsigned char zNPCCommon::grabbable(enum GrabType grabType /* r1+0x8 */) {}

// Range: 0x802B9860 -> 0x802B98B8
// this: r31
unsigned char zNPCCommon::punchable() {}

// Range: 0x802B98B8 -> 0x802B9984
// this: r31
void zNPCCommon::GetNukeIconMatrix(struct xMat4x3 * posMat /* r29 */) {
    // Local variables
    int boneIdx; // r30
    struct xMat4x3 boneMat; // r1+0x20
    struct xVec3 boneOffset; // r1+0x10
}

// Range: 0x802B9984 -> 0x802B9AE0
static void render_npc_nuke_ring_prepare(struct xModelInstance * ring /* r31 */, int ringNum /* r1+0x10 */, float ringEmitStep /* r1+0x14 */, float ratio /* f30 */, float sizeMultiplier /* f28 */, struct xMat3x3 * orientMat /* r1+0x18 */, struct xMat3x3 * rotMat /* r1+0x1C */) {
    // Local variables
    float nukeShockScale; // f29
    float step; // f31
    struct xVec3 scaleVec; // r1+0x20
    struct xMat3x3 scaleMat; // r1+0x60
    struct xMat3x3 finalMat; // r1+0x30
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D6652C
float gSceneUpdateTime; // size: 0x4, address: 0x80D6A8D8
struct xVec3 g_X3; // size: 0xC, address: 0x80D6D1A4
// Range: 0x802B9AE0 -> 0x802B9F74
// this: r31
void zNPCCommon::render_npc_nuke() {
    // Local variables
    unsigned char failed; // r28
    int i; // r29
    struct xModelInstance * mi; // r27
    int i; // r30
    struct xMat4x3 posMat; // r1+0xB0
    struct xMat4x3 camMat; // r1+0x70
    struct xMat3x3 rotMat; // r1+0x40
    struct xMat3x3 orientMat; // r1+0x10
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D6653C
// Range: 0x802B9F74 -> 0x802BA1A4
// this: r31
void zNPCCommon::render_npc_stun() {
    // Local variables
    struct xMat3x3 rotMat1; // r1+0x60
    int boneIdx; // r30
    struct xMat4x3 boneMat; // r1+0x20
    struct xVec3 boneOffset; // r1+0x10
...
// Range: 0x802BA1A4 -> 0x802BA3DC
// this: r31
void zNPCCommon::render_npc_shield_generator() {
    // Local variables
    int boneID; // r30
    float rotation; // f31
    struct xVec3 boneOffset; // r1+0x10
    struct xMat4x3 boneMat; // r1+0x60
    struct xMat4x3 rotMat; // r1+0x20
...
// Range: 0x802BA3DC -> 0x802BA42C
// this: r31
void zNPCCommon::render_extra() {}

// Range: 0x802BA42C -> 0x802BA49C
// this: r31
void zNPCCommon::post_render_cleanup() {}

// Range: 0x802BA49C -> 0x802BA4D4
// this: r1+0x8
void zNPCCommon::debug_render() {}

// Range: 0x802BA4D4 -> 0x802BA508
// this: r31
void zNPCCommon::SetDefaultAndEventStates() {}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6654C
// total size: 0x30
...
// Range: 0x802BA508 -> 0x802BAAF0
// this: r31
unsigned char zNPCCommon::systemEvent(struct xBase * from /* r22 */, struct xBase * to /* r23 */, unsigned int to_event /* r26 */, const float * to_param /* r30 */, struct xBase * to_param_widget /* r29 */, unsigned int to_param_widget_id /* r24 */) {
    // Local variables
    enum iSndGroupHandle soundGroupHandle; // r28
    class zNPCCustomAV * customAV; // r25
    struct zTransientNodeData transientData; // r1+0x74
    struct zTransientNodeData transientData; // r1+0x44
    float taunt; // f31
    float times; // f30
    union FloatAndVoid dist; // r1+0x8
    struct zCombat * combat; // r27
    struct zCombatDamageInfo info; // r1+0xC
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D66558
// Range: 0x802BAAF0 -> 0x802BABB4
void zNPCCommon::GetPosFromBase(struct xBase * inBase /* r31 */, struct xVec3 * outPos /* r30 */) {
    // References
    // -> static char __FUNCTION__[15];
}

// Range: 0x802BABB4 -> 0x802BAC00
// this: r31
void zNPCCommon::ForceEnterDeathState() {}

// Range: 0x802BAC00 -> 0x802BAC48
static void on_reset(const struct tweak_info & info /* r30 */) {
    // Local variables
    class zNPCCommon * npc; // r31
}

// Range: 0x802BAC48 -> 0x802BAC90
static void on_kill(const struct tweak_info & info /* r30 */) {
    // Local variables
    class zNPCCommon * npc; // r31
}

// Range: 0x802BAC90 -> 0x802BACFC
static void on_damage(const struct tweak_info & info /* r30 */) {
    // Local variables
    class zNPCCommon * npc; // r31
    struct zCombatDamageInfo damage; // r1+0x8
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D66567
// Range: 0x802BACFC -> 0x802BAE24
static void on_atomic_toggle(const struct tweak_info & info /* r29 */) {
    // Local variables
    class zNPCCommon * npc; // r27
    const char * current; // r30
    int index; // r26
    struct xModelInstance * atomic; // r31
    int i; // r28
...
static struct tweak_callback cb_reset; // size: 0x28, address: 0x804D02A0
static struct tweak_callback cb_kill; // size: 0x28, address: 0x804D02C8
static struct tweak_callback cb_damage; // size: 0x28, address: 0x804D02F0
static struct tweak_callback cb_atomic_toggle; // size: 0x28, address: 0x804D0318
// Range: 0x802BAE24 -> 0x802BB0A0
// this: r31
void zNPCCommon::add_tweaks(const char * prefix /* r30 */) {
    // Local variables
    unsigned int index; // r28
    struct xModelInstance * curr_model; // r29
    char buffer[128]; // r1+0x10

    // References
    // -> static struct tweak_callback cb_atomic_toggle;
    // -> static struct tweak_callback cb_damage;
    // -> static struct tweak_callback cb_kill;
    // -> static struct tweak_callback cb_reset;
}

// Range: 0x802BB0A0 -> 0x802BB3F4
// this: r31
void zNPCCommon::reset() {
    // Local variables
    float scale; // r1+0xC
    struct xSphere s; // r1+0x10
    struct xModelInstance * m; // r30
...
// Range: 0x802BB3F4 -> 0x802BB5B0
// this: r31
void zNPCCommon::render_npc() {
    // Local variables
    struct xVec3 worldPos; // r1+0x8
    struct zCombat * combat; // r30
    enum _SDRenderState oldRenderState; // r29
}

// Range: 0x802BB5B0 -> 0x802BB700
// this: r31
void zNPCCommon::render_npc_freezing() {
    // Local variables
    struct xModelInstance * m; // r30
    struct RwTexture * texture; // r29
    float original_r; // f30
    float original_g; // f29
    float original_b; // f28
    float factor; // f31
...
static unsigned char already_asserted; // size: 0x1, address: 0x80D6B57E
static signed char init; // size: 0x1, address: 0x80D6B57F
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D66578
enum xSndHandle {
};
// Range: 0x802BB700 -> 0x802BB848
// this: r31
enum xSndHandle zNPCCommon::play_sound(struct zAnimFxSoundGroup * sound /* r30 */) {
    // Local variables
    unsigned int soundID; // r29
    enum iSndGroupHandle GroupToPlay; // r28
...
// Range: 0x802BB848 -> 0x802BB9D8
// this: r31
void zNPCCommon::initialize_bounds(float modelScale /* r1+0x8 */) {
    // Local variables
    unsigned char use_bounding_box; // r1+0xC
    struct xVec3 offset; // r1+0x20
    float scale; // r1+0x10
...
// Range: 0x802BB9D8 -> 0x802BBC80
// this: r31
void zNPCCommon::change_bounds(float scale /* f31 */, struct xVec3 & offset /* r1+0x8 */, unsigned char use_bounding_box /* r1+0xC */) {
    // Local variables
    float width; // f30
    float height; // f29
    float length; // f28
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D66583
// Range: 0x802BBC80 -> 0x802BBE74
// this: r31
void zNPCCommon::update_bounds() {
    // References
    // -> static char __FUNCTION__[14];
}

static char __FUNCTION__[24]; // size: 0x18, address: 0x80527073
// Range: 0x802BBE74 -> 0x802BBF88
// this: r31
struct xVec3 zNPCCommon::get_bone_world_position(int bone /* r29 */) {
    // Local variables
...
// Range: 0x802BBF88 -> 0x802BBFFC
// this: r1+0x8
unsigned char zNPCCommon::GetParameter(const char * name /* r1+0xC */, unsigned int * value /* r31 */, unsigned int defaultValue /* r1+0x10 */) {
    // Local variables
    const char * text; // r1+0x14
}

// Range: 0x802BBFFC -> 0x802BC070
// this: r1+0x8
unsigned char zNPCCommon::GetParameter(const char * name /* r1+0xC */, int * value /* r31 */, int defaultValue /* r1+0x10 */) {
    // Local variables
    const char * text; // r1+0x14
}

// Range: 0x802BC070 -> 0x802BC0E4
// this: r1+0x8
unsigned char zNPCCommon::GetParameter(const char * name /* r1+0xC */, signed short * value /* r31 */, signed short defaultValue /* r1+0x10 */) {
    // Local variables
    const char * text; // r1+0x14
}

// Range: 0x802BC0E4 -> 0x802BC188
// this: r1+0x8
unsigned char zNPCCommon::GetParameter(const char * name /* r1+0xC */, unsigned char * value /* r31 */, unsigned char defaultValue /* r1+0x10 */) {
    // Local variables
    const char * text; // r1+0x14
}

// Range: 0x802BC188 -> 0x802BC1F8
// this: r1+0x8
unsigned char zNPCCommon::GetParameter(const char * name /* r1+0xC */, float * value /* r31 */, float defaultValue /* r1+0x10 */) {
    // Local variables
    const char * text; // r1+0x14
}

// Range: 0x802BC1F8 -> 0x802BC2A0
// this: r1+0x8
unsigned char zNPCCommon::GetParameter(const char * name /* r1+0xC */, struct xVec3 * value /* r31 */, const struct xVec3 & defaultValue /* r29 */) {
    // Local variables
    const char * text; // r1+0x10
    int size; // r30
}

// Range: 0x802BC2A0 -> 0x802BC354
// this: r31
unsigned char zNPCCommon::GetParameter(const char * name /* r28 */, const char * * value /* r29 */, const char * defaultValue /* r1+0x8 */) {
    // Local variables
    const char * text; // r30
}

// Range: 0x802BC354 -> 0x802BC3D0
// this: r30
int zNPCCommon::GetMaxHitPoints() {
    // Local variables
    int hp; // r31

    // References
    // -> static int sNPCHitPoints[3][2];
}

// Range: 0x802BC3D0 -> 0x802BC3D8
unsigned int zNPCCommon::GetNPCIceType() {}

// Range: 0x802BC3D8 -> 0x802BC3E0
float zNPCCommon::WeaponGetMaxRange() {}

// Range: 0x802BC3E0 -> 0x802BC3E8
unsigned char zNPCCommon::HasRangeWeapon() {}

// Range: 0x802BC3E8 -> 0x802BC41C
void init_system() {
    // References
    // -> int DM_NPC;
}

// Range: 0x802BC41C -> 0x802BC48C
// this: r29
void zNPCCommon::TellTargetAboutUs() {
    // Local variables
    struct xEnt * ent; // r31
    struct zPlayer * playerEnt; // r30
}
...
static char __FUNCTION__[7]; // size: 0x7, address: 0x80D66591
// Range: 0x802BC48C -> 0x802BC668
// this: r31
unsigned char zNPCCommon::Freeze(float intensity /* r1+0x8 */) {
    // Local variables
    class behavior * cur; // r30

    // References
    // -> static char __FUNCTION__[7];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D66598
// Range: 0x802BC668 -> 0x802BC7FC
// this: r31
void zNPCCommon::UpdateFrozen(float dt /* f31 */) {
    // Local variables
    class behavior * currBehavior; // r30

    // References
    // -> static char __FUNCTION__[13];
}

// Range: 0x802BC7FC -> 0x802BC8C8
// this: r31
void zNPCCommon::PastUpdateDistCleanup() {}

// Range: 0x802BC8C8 -> 0x802BC9E0
// this: r31
void zNPCCommon::VisibilityCullCleanup() {
    // Local variables
    unsigned char activatedFlag; // r30
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D665A5
// Range: 0x802BC9E0 -> 0x802BCAA0
// this: r31
void zNPCCommon::SetRandomAnimSpeed(float inMin /* r1+0x8 */, float inMax /* r1+0xC */) {
    // Local variables
    float newSpeed; // f31
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D665B8
// Range: 0x802BCAA0 -> 0x802BCBAC
// this: r31
void zNPCCommon::SetRandomAnimTime(float inMin /* r1+0x8 */, float inMax /* r1+0xC */) {
    // Local variables
    float newTime; // f31
...
// Range: 0x802BCBAC -> 0x802BCC0C
// this: r31
void zNPCCommon::SetRandomAnimSpeedAndTime(float inMin /* r1+0x8 */, float inMax /* r1+0xC */) {}

// Range: 0x802BCC0C -> 0x802BCC54
// this: r31
void zNPCCommon::SetRandomAnimTime() {}

// Range: 0x802BCC54 -> 0x802BCE2C
// this: r31
void zNPCCommon::BuildStateMachine() {}
...
// Range: 0x802BD048 -> 0x802BD11C
// this: r30
float zNPCCommon::DistanceToPriorityTarget() {
    // Local variables
    struct xVec3 * targetPos; // r31
    float dist; // f30
    float bound_r; // f31
    float dist2; // f29
}
...
// Range: 0x802BD11C -> 0x802BD5D0
// this: r31
float zNPCCommon::CalcTranslucency() {
    // Local variables
    const struct xMat4x3 * camMat; // r30
    struct xVec3 delta; // r1+0x60
    float distanceFromPlane; // r1+0x1C
    struct xVec3 screenCenter; // r1+0x54
    struct xVec3 pointOnSphere; // r1+0x48
    struct xVec3 screenPointOnCircle; // r1+0x3C
    float circleRadius; // f29
    float screenArea; // r1+0x18
    float screenMinX; // f28
    float screenMaxX; // f27
    float screenMinY; // f26
    float screenMaxY; // f25
    float squareMinX; // f24
    float squareMaxX; // f23
    float squareMinY; // f22
    float squareMaxY; // f21
    float overlapArea; // r1+0x14
    float nearP; // r1+0x10
    float farP; // r1+0xC
    struct xVec3 point; // r1+0x30
    class xPlane nearPlane; // r1+0x20
    float r; // f31
    float distance; // f30
...
// total size: 0x11C
struct {} zNPC::npc_nuke::__vtable; // size: 0x11C, address: 0x8052708C
// total size: 0x34
...
// total size: 0x11C
struct {} zBehaviorImplementation<zNPC::zNPCCommon>::__vtable; // size: 0x11C, address: 0x805271A8
float zNPCThawTimeTable[3][2]; // size: 0x18, address: 0x80D011A0
// total size: 0x0
...
// Range: 0x802BD5D0 -> 0x802BD854
// this: r31
unsigned char StaticObjectsCallBack::operator()(struct xEnt & ent /* r30 */) {
    // Local variables
    struct xModelInstance * collmod; // r28
