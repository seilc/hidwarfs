/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zHangable.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80174CE8 -> 0x80175E94
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D62E18
// total size: 0x20
struct xLinkAsset {
    // Members
...
// Range: 0x80174CE8 -> 0x80174E94
void zHangable_Init(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r29 */) {
    // Local variables
...
// total size: 0xC
struct /* @class$1053zHangable_cpp */ {
    // Members
...
static struct zHangableObject * sPlayerHangObj; // size: 0x4, address: 0x80D6A510
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
static struct xVec3 playerDPos; // size: 0xC, address: 0x80D6A514
static struct zHangableObject * sHangableObjects; // size: 0x4, address: 0x80D6A520
static int sNumHangableObjects; // size: 0x4, address: 0x80D6A524
// total size: 0xC
...
// Range: 0x80174F30 -> 0x80175028
int zHangable_Recurse(unsigned int id /* r1+0x8 */, struct zHangableObject * gpList /* r30 */, struct zHangable * hangable /* r22 */) {
    // Local variables
    unsigned int i; // r28
    int result; // r31
    struct xBase * base; // r29
    struct xGroup * group; // r26
    unsigned int count; // r23
    unsigned int memberID; // r25
    int incr; // r24
    struct zEntSimpleObj * simp; // r27
}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80D62E27
...
// total size: 0x10
struct /* @class$1631zHangable_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x80175028 -> 0x80175394
void zHangable_SceneInit(struct zScene * zsc /* r26 */) {
    // Local variables
    int i; // r30
    int count; // r25
    int numHangables; // r29
    struct zHangable * hangable; // r28
    struct zHangableObject * hangObj; // r31
...
// Range: 0x80175394 -> 0x80175410
void zHangable_Reset() {
    // Local variables
    int i; // r30
    struct zHangableObject * hangObj; // r31
    struct zHangable * hangable; // r29
...
// Range: 0x80175410 -> 0x80175C24
void zHangable_Update(float dt /* f28 */) {
    // Local variables
    int i; // r28
    struct zHangableObject * hangObj; // r31
    struct zHangableAsset * asset; // r30
    struct xVec3 oldHandle; // r1+0xCC
    struct xVec3 hangDisp; // r1+0xC0
    struct xVec3 playerCenter; // r1+0xB4
    struct xVec3 localCenter; // r1+0xA8
    struct xVec3 centerDisp; // r1+0x9C
    struct xVec3 disp; // r1+0x90
    float dist; // f27
    struct xVec3 pivot; // r1+0x84
    struct xMat3x3 newMat; // r1+0x140
    struct xMat3x3 rotMat; // r1+0x110
    float comp; // f23
    float comp; // f22
    float maxAng; // f29
    struct xVec3 cross; // r1+0x78
    float len; // f26
    float ang; // f31
    struct xMat3x3 rotMat; // r1+0xE0
    struct xVec3 actualDisp; // r1+0x6C
    struct xVec3 actualPivot; // r1+0x60
    struct xVec3 cross; // r1+0x54
    float len; // f25
    float ang; // f30
    float minArcRadians; // f24
    float height; // f21
    struct xVec3 relVel; // r1+0x48
    float velSq; // f20
    float kinetic; // f19
    float potential; // f18
    float total; // r1+0x20
    float reqTotal; // r1+0x1C
    struct xVec3 velAddDir; // r1+0x3C
    float velDot; // r1+0x18
    struct xVec3 negAddDir; // r1+0x30
    struct xModelInstance * atomic; // r29
    struct xVec3 newPlayerCenter; // r1+0x24
...
// total size: 0x890
struct /* @class$1874zHangable_cpp */ {
    // Static members
...
// Range: 0x80175C24 -> 0x80175D14
void zHangable_Recenter(struct zHangableObject * hangObj /* r31 */) {
    // Local variables
    struct zHangableAsset * asset; // r29
    struct xVec3 pivot; // r1+0x14
    struct xVec3 newPivot; // r1+0x8
    struct xMat4x3 * objMat; // r30
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D62E3B
// Range: 0x80175D14 -> 0x80175E94
void zHangable_EventCB(struct xBase * to /* r28 */, unsigned int toEvent /* r1+0x8 */) {
    // Local variables
    struct zHangable * hang; // r31
    int i; // r30
