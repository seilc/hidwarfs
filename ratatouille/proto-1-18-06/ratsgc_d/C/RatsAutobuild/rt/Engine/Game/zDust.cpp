/*
    ...
    Language: C++
    Code range: 0x80213C40 -> 0x802163D8
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static char * ring_chain_labels[3]; // size: 0xC, address: 0x80D64858
static float emit_rate; // size: 0x4, address: 0x80D64864
static float size_min; // size: 0x4, address: 0x80D64868
static float size_max; // size: 0x4, address: 0x80D6486C
static float vel_min; // size: 0x4, address: 0x80D64870
static float vel_max; // size: 0x4, address: 0x80D64874
static float life_min; // size: 0x4, address: 0x80D64878
static float life_max; // size: 0x4, address: 0x80D6487C
static float growth; // size: 0x4, address: 0x80D64880
static float intensity_min; // size: 0x4, address: 0x80D64884
static float intensity_max; // size: 0x4, address: 0x80D64888
static float vel_decay; // size: 0x4, address: 0x80D6488C
static float rot_vel_min; // size: 0x4, address: 0x80D64890
static float rot_vel_max; // size: 0x4, address: 0x80D64894
static float base_alpha; // size: 0x4, address: 0x80D64898
static float alpha_coeff1; // size: 0x4, address: 0x80D6489C
static float alpha_coeff2; // size: 0x4, address: 0x80D648A0
static float alpha_coeff3; // size: 0x4, address: 0x80D648A4
static float fade_in_time; // size: 0x4, address: 0x80D648A8
static float fade_out_time; // size: 0x4, address: 0x80D648AC
// total size: 0x1C
...
static struct ring_chain_data chain[2]; // size: 0x38, address: 0x80523338
static unsigned int texture; // size: 0x4, address: 0x80D648B0
static int texture_rows; // size: 0x4, address: 0x80D648B4
static int texture_columns; // size: 0x4, address: 0x80D648B8
// total size: 0x8
struct substr {
    // Members
...
// total size: 0x1
struct /* @class$771zDust_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$772zDust_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$773zDust_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$774zDust_cpp */ {
    // Members
...
// Range: 0x80213C40 -> 0x80213CE4
static void on_change_ring_fade(const struct tweak_info & info /* r31 */) {
    // Local variables
...
// Range: 0x80213CE4 -> 0x80213D04
static void on_refresh_dirt_system() {}

// Range: 0x80213D04 -> 0x80213DE0
...
static struct tweak_callback cb_ring_fade; // size: 0x28, address: 0x804BC588
static struct tweak_callback cb_refresh_dirt_system; // size: 0x28, address: 0x804BC5B0
static struct tweak_callback cb_reset_ring_chains; // size: 0x28, address: 0x804BC5D8
// Range: 0x80213DE0 -> 0x802143FC
...
enum /* @enum$1892zDust_cpp */ {
    RING_CHAIN_RANDOM = 0,
    RING_CHAIN_ROTATE = 1,
    RING_CHAIN_WAVE = 2,
    RING_CHAIN_TYPES = 3,
};
static int dirt_system_id; // size: 0x4, address: 0x80D6AE38
static float ring_vel_decay; // size: 0x4, address: 0x80D6AE3C
static struct xVec2 * dirt_uvs; // size: 0x4, address: 0x80D6AE40
static unsigned char refreshed_ring_chains; // size: 0x1, address: 0x80D6AE44
static float ring_chain_ang[2]; // size: 0x8, address: 0x80D6AE48
static struct xVec2 ring_chain[2][8]; // size: 0x80, address: 0x80D18230
// Range: 0x802143FC -> 0x802144A0
...
// Range: 0x802144A0 -> 0x802145D8
static void set_uv_frames(struct xVec2 * uv /* r31 */, int rows /* r27 */, int columns /* r28 */) {
    // Local variables
    float du; // f29
    float dv; // f28
    float v; // f31
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D648BC
...
// Range: 0x802145D8 -> 0x80214714
static void create_dirt_system() {
    // Local variables
    int uvpairs; // r31
    struct RwTexture * dirt_texture; // r27
...
// Range: 0x802147FC -> 0x80214B38
static int update_dust_particles(unsigned char * mem /* r28 */, int count /* r1+0x8 */, struct ptank_pool & apool /* r1+0xC */, float dt /* f31 */) {
    // Local variables
    struct ptank_pool__pos_color_size_rot & pool; // r30
    float vel_decay; // f26
    float max_alpha; // r1+0x10
    float a1; // f25
    float a2; // f24
    float a3; // f23
...
// Range: 0x80214B38 -> 0x80214E94
static int update_dust_particles_uv2(unsigned char * mem /* r28 */, int count /* r1+0x8 */, struct ptank_pool & apool /* r1+0xC */, float dt /* f31 */) {
    // Local variables
...
// Range: 0x80214E94 -> 0x80214ED0
static void move_ring_chain(struct xVec2 * chain /* r3 */, struct xVec2 * end_chain /* r4 */, float x /* f1 */, float z /* f2 */, float dcos /* f3 */, float dsin /* f4 */) {}
...
// Range: 0x80214ED0 -> 0x80215038
static void refresh_ring_chains() {
    // Local variables
    float dt; // f29
    int i; // r30
    const struct ring_chain_data & data; // r31
    float ang; // f31
...
static class zDustRingEmitter dummy_emitter; // size: 0xC, address: 0x80D6AE50
// Range: 0x80215170 -> 0x802151B8
void zDustSceneEnter() {
    // References
    // -> static class zDustRingEmitter dummy_emitter;
}

// Range: 0x802151B8 -> 0x802151DC
void zDustReset() {}

// Range: 0x802151DC -> 0x80215204
void zDustUpdate(float dt /* r1+0x8 */) {}

// Range: 0x80215204 -> 0x80215210
void zDustRender() {
    // References
    // -> static unsigned char refreshed_ring_chains;
}

int activities_used; // size: 0x4, address: 0x80D6AE5C
struct activity_data * activities; // size: 0x4, address: 0x80D6AE60
// Range: 0x80215210 -> 0x80215224
// this: r3
void zDustRingEmitter::create() {}

// Range: 0x80215224 -> 0x8021524C
void zDustRingEmitter::load_default_config(struct config_type & cfg /* r3 */) {}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D648CF
// Range: 0x8021524C -> 0x8021534C
// this: r31
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D648D8
// Range: 0x8021534C -> 0x802153F4
// this: r31
...
static char __FUNCTION__[7]; // size: 0x7, address: 0x80D648E3
// Range: 0x802153F4 -> 0x80215ADC
// this: r30
void zDustRingEmitter::kickup(const struct xVec3 & center /* r26 */, float force /* f30 */, float dt /* r1+0x8 */) {
    // Local variables
    float cx; // f23
    float cy; // f22
    float cz; // f21
    float radius_min; // f29
    float radius_delta; // f20
    float size_min; // f28
    float size_max; // f19
    float size_delta; // f18
    float vmag_min; // r1+0x40
    float vmag_delta; // r1+0x3C
    float life_min; // f27
    float life_delta; // r1+0x38
    float rot_vel_min; // f26
    float rot_vel_delta; // r1+0x34
    float growth; // r1+0x30
    float intensity_min; // r1+0x2C
    float intensity_delta; // r1+0x28
    unsigned char alpha; // r24
    int dirt_frames; // r23
    int i; // r28
    float emit_rate; // r1+0x24
    int emit; // r27
    unsigned int rval; // r29
    unsigned char * mem; // r1+0x20
    int have; // r25
    struct dust_particle * p; // r31
    struct dust_particle * endp; // r22
    float dx; // r1+0x1C
    float dz; // r1+0x18
    int chain_index; // r1+0x14
    float rr; // f31
    float rand_radius; // f25
    float vmag; // f24
    float life; // r1+0x10
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D648EA
// Range: 0x80215ADC -> 0x80215C34
// this: r31
void zDustRingEmitter::refresh_activity() {
    // Local variables
    float intensity_min; // f31
    float intensity_max; // f30
...
// Range: 0x80215C70 -> 0x80215CBC
void zDustRingEmitter::reset_all() {
    // Local variables
    struct activity_data * act; // r31
    struct activity_data * const end_act; // r30
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D648FB
// Range: 0x80215D3C -> 0x80215DE4
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D6490A
// Range: 0x80215DE4 -> 0x80215ED0
...
// Range: 0x80215F1C -> 0x80215FB0
// this: r31
void zDustRingEmitter::debug_init(const char * tweak_prefix /* r1+0x8 */) {}

// Range: 0x80215FB0 -> 0x8021607C
// this: r31
void zDustRingEmitter::debug_update(float dt /* r1+0x8 */) {
    // Local variables
...
static struct tweak_callback cb_check_active; // size: 0x28, address: 0x804BC600
static struct tweak_callback cb_refresh_config; // size: 0x28, address: 0x804BC628
// Range: 0x802161B4 -> 0x8021632C
// this: r31
...
// Range: 0x8021632C -> 0x8021638C
void zDustRingEmitter::check_active(const struct tweak_info & info /* r30 */) {
    // Local variables
    class zDustRingEmitter & owner; // r31
}

// Range: 0x8021638C -> 0x802163D8
void zDustRingEmitter::on_refresh_config(const struct tweak_info & info /* r30 */) {
    // Local variables
    class zDustRingEmitter & owner; // r31
}
