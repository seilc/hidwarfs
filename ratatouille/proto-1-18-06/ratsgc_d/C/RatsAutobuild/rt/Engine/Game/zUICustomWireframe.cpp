/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zUICustomWireframe.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80297060 -> 0x802979C8
*/
float WIREFRAME_ROTATE_SPEED; // size: 0x4, address: 0x80D65F98
float WIREFRAME_UPDATE_RATE; // size: 0x4, address: 0x80D65F9C
unsigned int loadSceneID; // size: 0x4, address: 0x80D6B410
// total size: 0x54
struct {} zUICustomWireframe::__vtable; // size: 0x54, address: 0x805259B8
// total size: 0xC
...
// total size: 0x20
class zUICustomWireframe : public zUICustom {
    // Functions
    zUICustomWireframe(class zUI * ui);
    ...
    // Members
    struct zWireframeAsset * wireframeAsset; // offset 0x8, size 0x4
    struct rwGameCube2DVertex * vertices; // offset 0xC, size 0x4
    float dX; // offset 0x10, size 0x4
    float dY; // offset 0x14, size 0x4
    float rotation; // offset 0x18, size 0x4
    float scale; // offset 0x1C, size 0x4
};
// Range: 0x80297060 -> 0x802970B4
// this: r31
zUICustomWireframe::zUICustomWireframe(class zUI * ui /* r1+0x8 */) {
    // References
    // -> struct [anonymous] zUICustomWireframe::__vtable;
}
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D65FA0
// total size: 0x28
struct PKRAssetType {
    // Members
...
// Range: 0x802970B4 -> 0x802974BC
// this: r31
void zUICustomWireframe::Init() {
    // Local variables
    char wireframeName[1024]; // r1+0x144
    char sceneName[5]; // r1+0x8
    char * ID; // r29
    struct PKRAssetTOCInfo info; // r1+0x10
    int found; // r24
    unsigned int size; // r25
    const char * hipFilename; // r26
    unsigned char error; // r27
    struct tag_xFile hipFile; // r1+0x30
    int err; // r28
    float dx; // f31
    float dy; // f30
    unsigned int i; // r30
...
// Range: 0x802974BC -> 0x80297948
// this: r31
unsigned char zUICustomWireframe::PreRender() {
    // Local variables
    float scaleX; // f29
    float scaleY; // f28
    float z; // f31
    float zRecip; // r1+0x14
    struct zWireframeVertex * assetVertices; // r29
    unsigned short * indices; // r28
    float WIREFRAME_ROTATE_UNIT; // f27
    int rotationIndex; // r27
    float phi; // f26
    float sinPhi; // f25
    float cosPhi; // f24
    float sinTheta; // f23
    float cosTheta; // f22
    unsigned int i; // r30
    struct xVec3 currVert; // r1+0x18
    float newX; // f19
    float newY; // f18
    float newZ; // f30
    enum RwShadeMode oldShadeMode; // r1+0x10
    int oldAlphaEnable; // r1+0xC
    enum RwCullMode oldCullMode; // r1+0x8

    // References
    // -> unsigned int FB_YRES;
    // -> unsigned int FB_XRES;
    // -> float WIREFRAME_UPDATE_RATE;
    // -> float WIREFRAME_ROTATE_SPEED;
}

// Range: 0x80297948 -> 0x802979C4
// this: r3
void zUICustomWireframe::PreUpdate(float dt /* f1 */) {
    // References
    // -> float WIREFRAME_ROTATE_SPEED;
}

// Range: 0x802979C4 -> 0x802979C8
void zUICustomWireframe::Exit() {}
