/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zShrapnel.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801B44E8 -> 0x801B96FC
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static unsigned char GOO_DRAW_BOUNDING_SPHERE; // size: 0x1, address: 0x80D6A9A0
static float GOO_SLIDE_Y_THRESHOLD; // size: 0x4, address: 0x80D63860
static float GOO_DECAL_EMIT_TIME; // size: 0x4, address: 0x80D63864
static float GOO_DECAL_SCALE; // size: 0x4, address: 0x80D63868
static float GOO_STICK_TIME; // size: 0x4, address: 0x80D6386C
static float GOO_FADE_TIME; // size: 0x4, address: 0x80D63870
static float GOO_FADE_DELAY; // size: 0x4, address: 0x80D63874
static float GOO_SLIDE_ACCEL; // size: 0x4, address: 0x80D63878
static float GOO_MAX_SLIDE_SPEED; // size: 0x4, address: 0x80D6387C
static float GOO_FLY_GRAVITY; // size: 0x4, address: 0x80D63880
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
static struct xMat4x3 tmpMat; // size: 0x40, address: 0x80D01FD0
static int sNumActiveFrags; // size: 0x4, address: 0x80D6A9A4
static struct zFrag sFragPool[150]; // size: 0x9150, address: 0x80D02010
// total size: 0x54
struct zFragGroup {
    // Members
...
// total size: 0x54
struct /* @class$1628zShrapnel_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$828zShrapnel_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$829zShrapnel_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$830zShrapnel_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$831zShrapnel_cpp */ {
    // Members
...
static struct zFrag sFirstFreeFrag; // size: 0xF8, address: 0x80D0B160
static struct zFrag sFirstActiveFrag; // size: 0xF8, address: 0x80D0B258
static struct zFrag sProjectileList; // size: 0xF8, address: 0x80D0B350
static struct zFrag sLightningList; // size: 0xF8, address: 0x80D0B448
static struct zFrag sParticleList; // size: 0xF8, address: 0x80D0B540
static struct zFrag sSoundList; // size: 0xF8, address: 0x80D0B638
static struct zFrag sExplosionList; // size: 0xF8, address: 0x80D0B730
static struct zFrag sDistortionList; // size: 0xF8, address: 0x80D0B828
static struct zFrag sGooList; // size: 0xF8, address: 0x80D0B920
float cFragThrottleCeiling; // size: 0x4, address: 0x80D63884
float cFragThrottleThresholdPercent; // size: 0x4, address: 0x80D63888
// total size: 0x68
...
static struct _tagLightningAdd sLightningAddInfo; // size: 0x68, address: 0x80D0BA18
// total size: 0xC
struct zShrapnelInitTable {
    // Members
    char * name; // offset 0x0, size 0x4
    void (* initCB)(struct zShrapnelAsset *, struct xModelInstance *, struct xVec3 *, struct xVec3 *, void (*)(struct zFrag *, struct zFragAsset *)); // offset 0x4, size 0x4
    unsigned int ID; // offset 0x8, size 0x4
};
static struct zShrapnelInitTable sShrapnelTable[1]; // size: 0xC, address: 0x80D6388C
// Range: 0x801B44E8 -> 0x801B45C0
void zFrag_Throttler_AddTweaks(const char * prefix /* r31 */) {
    // Local variables
    char _buffer[128]; // r1+0x90
    char _buffer[128]; // r1+0x10

    // References
    // -> static int sNumActiveFrags;
    // -> float cFragThrottleThresholdPercent;
    // -> float cFragThrottleCeiling;
}
...
// Range: 0x801B45C0 -> 0x801B46D4
unsigned char zFrag_Throttler_Permission(enum zFragType type /* r30 */) {
    // Local variables
    int cFragThrottleThreshold; // r31
    float range; // f31
    float t; // f30
    float probability; // f29
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D63898
// Range: 0x801B46D4 -> 0x801B48DC
struct zFrag * zFrag_Alloc(enum zFragType type /* r30 */, unsigned char useThrottling /* r1+0x8 */) {
    // Local variables
    struct zFrag * result; // r31

    // References
    // -> static int sNumActiveFrags;
    // -> static struct zFrag sFirstActiveFrag;
    // -> static struct zFrag sFirstFreeFrag;
    // -> static char __FUNCTION__[12];
    // -> float cFragThrottleCeiling;
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D638A4
// Range: 0x801B48DC -> 0x801B4A88
void zFrag_Free(struct zFrag * frag /* r31 */) {
    // References
    // -> static struct zFrag sFirstFreeFrag;
...
// Range: 0x801B4A88 -> 0x801B4A8C
void zFrag_CheckDuplicateModel() {}
...
// Range: 0x801B4A8C -> 0x801B4B94
void zShrapnel_GameInit() {
    // Local variables
    int i; // r30
    struct zShrapnelInitTable * curr; // r31

    // References
    // -> static struct _tagLightningAdd sLightningAddInfo;
    // -> struct xColor_tag g_YELLOW;
    // -> static struct zShrapnelInitTable sShrapnelTable[1];
}

// Range: 0x801B4B94 -> 0x801B4BD8
void zShrapnel_FragShrapnelSceneInit(struct zFragShrapnelAsset * asset /* r31 */) {}

// Range: 0x801B4BD8 -> 0x801B4C4C
void zShrapnel_ProjectileSceneInit(struct zFragProjectileAsset * asset /* r31 */) {}
...
static struct smoke_config smoke_config; // size: 0x38, address: 0x80D0BA80
struct smoke_config default_config; // size: 0x38, address: 0x80D18198
// Range: 0x801B4C4C -> 0x801B4D44
void zShrapnel_ParticleSceneInit(struct zFragParticleAsset * asset /* r31 */) {
    // References
    // -> static struct smoke_config smoke_config;
    // -> struct smoke_config default_config;
}

static char __FUNCTION__[25]; // size: 0x19, address: 0x80521878
// Range: 0x801B4D44 -> 0x801B4DEC
void zShrapnel_SmokeSceneInit(struct zFragSmokeAsset * asset /* r31 */) {
    // References
    // -> static char __FUNCTION__[25];
}

// Range: 0x801B4DEC -> 0x801B4E88
void zShrapnel_GooSceneInit(struct zFragGooAsset * asset /* r31 */) {
    // References
    // -> static unsigned char GOO_DRAW_BOUNDING_SPHERE;
}

// Range: 0x801B4E88 -> 0x801B4EF4
void zShrapnel_SetShrapnelAssetInitCB(struct zShrapnelAsset * sasset /* r3 */) {
    // Local variables
    int i; // r30
    struct zShrapnelInitTable * curr; // r31
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D638AF
// Range: 0x801B4EF4 -> 0x801B5200
void zShrapnel_SceneInit() {
    // Local variables
    int i; // r30
    int numShrapnel; // r22
    int j; // r28
    struct zShrapnelAsset * sa; // r29
    struct zFragAsset * fa; // r31
    struct zFragProjectileAsset * projAss; // r27
    struct zFragParticleAsset * partAss; // r26
    struct zFragSoundAsset * soundAss; // r21
    struct zFragLightningAsset * lightAss; // r20
    struct zFragExplosionAsset * ass; // r19
    struct zFragDistortionAsset * ass; // r18
    struct zFragFireAsset * ass; // r17
    struct zFragLightAsset * ass; // r1+0x8
    struct zFragSmokeAsset * ass; // r25
    struct zFragGooAsset * ass; // r24
    struct zFragShrapnelAsset * shrapAss; // r23

    // References
    // -> static char __FUNCTION__[20];
    // -> static struct zFrag sGooList;
    // -> static struct zFrag sDistortionList;
    // -> static struct zFrag sExplosionList;
    // -> static struct zFrag sSoundList;
    // -> static struct zFrag sParticleList;
    // -> static struct zFrag sLightningList;
    // -> static struct zFrag sProjectileList;
    // -> static int sNumActiveFrags;
    // -> static struct zFrag sFragPool[150];
    // -> static struct zFrag sFirstFreeFrag;
    // -> static struct zFrag sFirstActiveFrag;
}

// Range: 0x801B5200 -> 0x801B5378
void zShrapnel_Update(float dt /* f31 */) {
    // Local variables
    struct zFrag * curr; // r31
    struct zFrag * next; // r30
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D638C3
// Range: 0x801B5378 -> 0x801B5414
void zShrapnel_Reset() {
    // Local variables
    struct zFrag * frag; // r31
    struct zFrag * next; // r30
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x805218CC
// total size: 0x8
struct zShrapnelParentList {
    // Members
    unsigned int parentID; // offset 0x0, size 0x4
    struct xModelInstance * parentModel; // offset 0x4, size 0x4
};
// Range: 0x801B546C -> 0x801B57A0
void zShrapnel_DefaultInit(struct zShrapnelAsset * shrap /* r25 */, struct xModelInstance * parent /* r24 */, struct xVec3 * initOffset /* r21 */, struct xVec3 * initVel /* r22 */, void (* cb)(struct zFrag *, struct zFragAsset *) /* r23 */) {
    // Local variables
    struct zShrapnelParentList * plist; // r29
    struct zFrag * frag; // r30
    struct zFragAsset * fasset; // r31
    int i; // r28
    int j; // r27
    int k; // r26
...
static struct zFragProjectileAsset sCinProj; // size: 0x158, address: 0x80D0BAB8
static struct RpAtomic * sCinModel; // size: 0x4, address: 0x80D6A9A8
// Range: 0x801B57A0 -> 0x801B5964
void zShrapnel_GameInit_Cinematic() {
    // Local variables
    int i; // r31

    // References
    // -> static struct zFragProjectileAsset sCinProj;
}

// Range: 0x801B5964 -> 0x801B59A8
void zShrapnel_SceneInit_Cinematic() {
    // Local variables
    const char * dummyModel; // r31

    // References
    // -> static struct RpAtomic * sCinModel;
}

// Range: 0x801B59A8 -> 0x801B5A04
void zFragLoc_Setup(struct zFragLocation * loc /* r31 */, struct xModelInstance * parent /* r30 */) {}

// Range: 0x801B5A04 -> 0x801B5AF4
...
// Range: 0x801B5AF4 -> 0x801B5CF4
void zFragLoc_InitMat(struct zFragLocation * loc /* r30 */, struct xMat4x3 * mat /* r29 */, struct xModelInstance * parent /* r31 */, unsigned int flags /* r1+0x10 */) {
    // Local variables
    int index; // r28
    struct xMat4x3 normalizedMat; // r1+0x20
...
// Range: 0x801B5CF4 -> 0x801B5E74
void zFragLoc_InitVec(struct zFragLocation * loc /* r30 */, struct xVec3 * vec /* r28 */, struct xModelInstance * parent /* r31 */) {
    // Local variables
    struct xMat4x3 tmpMat; // r1+0x20
    int index; // r29
    struct xVec3 offset; // r1+0x10
}

// Range: 0x801B5E74 -> 0x801B5F8C
void zFragLoc_InitDir(struct zFragLocation * loc /* r30 */, struct xVec3 * vec /* r28 */, struct xModelInstance * parent /* r31 */) {
    // Local variables
    int index; // r29
    struct xMat4x3 tmpMat; // r1+0x10

    // References
    // -> static struct xMat4x3 tmpMat;
}

// total size: 0x14
struct ParabolaData {
    // Members
    struct zFragProjectile * proj; // offset 0x0, size 0x4
    struct xVec3 N; // offset 0x4, size 0xC
    float d; // offset 0x10, size 0x4
};
static unsigned char already_asserted; // size: 0x1, address: 0x80D6A9AC
static signed char init; // size: 0x1, address: 0x80D6A9AD
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D638D3
// total size: 0x18
struct analog_data {
    // Members
...
// total size: 0x10
struct /* @class$1873zShrapnel_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x801B5F8C -> 0x801B6CB8
unsigned char zFrag_DefaultInit(struct zFrag * frag /* r31 */, struct zFragAsset * fasset /* r28 */) {
    // Local variables
    struct zFragProjectileAsset * passet; // r29
    struct RpAtomic * m; // r22
    struct zFragProjectile * proj; // r21
    struct zFragLightningAsset * lasset; // r20
    struct zFragParticleAsset * prasset; // r19
    struct zFragSoundAsset * sasset; // r1+0x10
    struct zFragExplosionAsset * easset; // r23
    struct zFragLightAsset * lasset; // r18
    struct zFragSmoke & smokeFrag; // r25
    struct zFragSmokeAsset * prasset; // r26
    struct zFragGooAsset * gasset; // r27
    struct zFragGoo * goo; // r30
    struct RpAtomic * m1; // r17
    struct RpAtomic * m2; // r1+0xC
    struct _zEnv * zen; // r1+0x8
    struct xEnvAsset * easset; // r24

    // References
    // -> static char __FUNCTION__[18];
    // -> static float GOO_FADE_TIME;
...
// total size: 0x890
struct /* @class$2108zShrapnel_cpp */ {
    // Static members
...
// Range: 0x801B6CB8 -> 0x801B6D80
void zFrag_DefaultParticleUpdate(struct zFrag * frag /* r31 */) {
    // Local variables
    struct zFragParticleAsset * passet; // r30

    // References
    // -> static struct zFrag sParticleList;
}

// Range: 0x801B6D80 -> 0x801B6E7C
void zFrag_ParticleManager(float dt /* f31 */) {
    // Local variables
    struct zFrag * frag; // r31
    struct zFragParticle * part; // r29
    struct zFrag * next; // r28
    struct zFragParticleAsset * passet; // r30
...
static char __FUNCTION__[25]; // size: 0x19, address: 0x80521954
// total size: 0xC
struct /* @class$1080zShrapnel_cpp */ {
    // Members
    float t; // offset 0x0, size 0x4
...
// Range: 0x801B6E7C -> 0x801B706C
void zFrag_ProjectileCollData(struct zFrag * frag /* r30 */) {
    // Local variables
    struct zFragProjectile * proj; // r31
    float radius; // f31
    struct xCollis colls; // r1+0x8
...
char __FUNCTION__$localstatic1$zFrag_ProjectileSetupPath__FP5zFragP20zFragProjectileAsset[26]; // size: 0x1A, address: 0x8052196D
// total size: 0x30
struct xMat3x3 {
    // Members
...
// Range: 0x801B706C -> 0x801B7720
void zFrag_ProjectileSetupPath(struct zFrag * frag /* r29 */, struct zFragProjectileAsset * passet /* r30 */) {
    // Local variables
    struct zFragProjectile * proj; // r31
    struct xMat3x3 rotMat; // r1+0xA0
    struct xVec3 rotVals; // r1+0x40
    struct xVec3 baseVel; // r1+0x34
    struct zFragLocation randLoc; // r1+0x74
    struct xVec3 randVel; // r1+0x28
    float len2; // f31
    struct xVec3 back; // r1+0x1C
    struct zFragLocation randLoc; // r1+0x4C
    struct xVec3 randAxis; // r1+0x10

    // References
    // -> char __FUNCTION__$localstatic1$zFrag_ProjectileSetupPath__FP5zFragP20zFragProjectileAsset[26];
}

// Range: 0x801B7720 -> 0x801B7814
void zFrag_DefaultProjectileUpdate(struct zFrag * frag /* r31 */) {
    // Local variables
    struct zFragProjectileAsset * passet; // r30

    // References
    // -> static struct zFrag sProjectileList;
}

static char __FUNCTION__[23]; // size: 0x17, address: 0x80521987
// Range: 0x801B7814 -> 0x801B78F4
void zFrag_DeleteProjectile(struct zFrag * frag /* r31 */) {
    // Local variables
    struct zShrapnelAsset * child; // r30
    struct xVec3 finalVel; // r1+0x8

    // References
    // -> static char __FUNCTION__[23];
}

// Range: 0x801B78F4 -> 0x801B7DA4
void zFrag_ProjectileManager(float dt /* f31 */) {
    // Local variables
    struct zFrag * frag; // r30
    struct zFragProjectile * proj; // r31
    struct zFrag * next; // r28
    int killed; // r29
    float normComp; // f30
    struct xVec3 tanVel; // r1+0x34
    float totalTime; // f27
    float percent; // f26
    float minScale; // f29
    float maxScale; // f25
    float newScale; // f24
    struct xVec3 oldpos; // r1+0x28
    struct xVec3 newAxis; // r1+0x1C
    float len2; // f28
    struct xVec3 back; // r1+0x10
    struct xMat3x3 spin; // r1+0x40

    // References
    // -> static struct zFrag sProjectileList;
}

static char __FUNCTION__[29]; // size: 0x1D, address: 0x8052199E
// Range: 0x801B7DA4 -> 0x801B7F4C
void zFrag_DefaultLightningUpdate(struct zFrag * frag /* r31 */) {
    // Local variables
    struct zFragLightningAsset * lasset; // r30
...
// Range: 0x801B7F4C -> 0x801B8078
void zFrag_LightningManager(float dt /* f31 */) {
    // Local variables
    struct zFrag * frag; // r31
    struct zFragLightning * ligh; // r28
    struct zFragLightningAsset * lasset; // r30
    struct zLightning * inst; // r29
    struct zFrag * next; // r27
    struct xVec3 start; // r1+0x14
    struct xVec3 end; // r1+0x8
...
// Range: 0x801B8078 -> 0x801B8164
void zFrag_DefaultSoundUpdate(struct zFrag * frag /* r31 */) {
    // Local variables
    struct zFragSound * sound; // r30
    struct zFragSoundAsset * sasset; // r29
    struct xVec3 source; // r1+0x8

    // References
    // -> static struct zFrag sSoundList;
}

// Range: 0x801B8164 -> 0x801B822C
void zFrag_SoundManager(float dt /* f31 */) {
    // Local variables
    struct zFrag * frag; // r31
    struct zFragSound * sound; // r30
    struct zFragSoundAsset * sasset; // r1+0x8
    struct zFrag * next; // r29

    // References
    // -> static struct zFrag sSoundList;
}

// Range: 0x801B822C -> 0x801B836C
static void zFrag_DefaultExplosionUpdate(struct zFrag * frag /* r31 */) {
    // Local variables
    struct zFragExplosion * f; // r29
    struct zFragExplosionAsset * a; // r30
...
// Range: 0x801B836C -> 0x801B8448
static void zFrag_ExplosionManager(float dt /* f31 */) {
    // Local variables
    struct zFrag * frag; // r31
    struct zFrag * next; // r29
    struct zFragExplosion * f; // r30
    struct zFragExplosionAsset * a; // r28
    struct xVec3 loc; // r1+0x8
...
// Range: 0x801B8448 -> 0x801B8594
static void zFrag_DefaultDistortionUpdate(struct zFrag * frag /* r31 */) {
    // Local variables
    struct zFragDistortion * f; // r29
    struct zFragDistortionAsset * a; // r30
    int flags; // r28
...
// Range: 0x801B8594 -> 0x801B8660
static void zFrag_DefaultFireUpdate(struct zFrag * frag /* r30 */) {
    // Local variables
    struct zFragFire * f; // r1+0x8
    struct zFragFireAsset * a; // r31
    struct spawn_data spawn; // r1+0xC
}

// Range: 0x801B8660 -> 0x801B871C
static struct xLight * zFrag_InitLight(struct zFragLightInfo & lightInfo /* r31 */, float lifetime /* f31 */) {
    // Local variables
    struct xLight * tempLight; // r30
}

// Range: 0x801B871C -> 0x801B8754
static void zFrag_FreeLight(struct xLight * light /* r1+0x8 */) {}
...
// Range: 0x801B8754 -> 0x801B8868
static void zFrag_UpdateLightIntensity(struct xLight * light /* r1+0x8 */, const struct zFragLightInfo & info /* r31 */, float lifetime /* f29 */) {
    // Local variables
    struct _xFColor col; // r1+0xC
    float percentage; // f31
    float percentage; // f30
}

// Range: 0x801B8868 -> 0x801B8914
static void zFrag_DefaultLightUpdate(struct zFrag * frag /* r31 */, float dt /* r1+0x8 */) {
    // Local variables
    struct zFragLight * lf; // r30
    struct zFragLightAsset * a; // r29
    struct xVec3 loc; // r1+0xC
}

// Range: 0x801B8914 -> 0x801B8A38
static void zFrag_DistortionManager(float dt /* f31 */) {
    // Local variables
    struct zFrag * frag; // r30
    struct zFrag * next; // r28
    struct zFragDistortion * f; // r29
    struct zFragDistortionAsset * a; // r31

    // References
    // -> static struct zFrag sDistortionList;
}

// Range: 0x801B8A38 -> 0x801B8B3C
void zFrag_DefaultSmokeUpdate(struct zFrag * frag /* r30 */, float dt /* f31 */) {
    // Local variables
    struct zFragSmoke & smokeFrag; // r31
    struct zFragSmokeAsset * passet; // r29
    int emitAmount; // r28
}

// Range: 0x801B8B3C -> 0x801B8BB4
static void zFrag_DefaultGooUpdate(struct zFrag * frag /* r3 */) {
    // References
    // -> static struct zFrag sGooList;
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D638E5
...
// Range: 0x801B8BB4 -> 0x801B93A4
static void zFrag_GooManager(float dt /* f31 */) {
    // Local variables
    struct zFrag * frag; // r29
    struct zFrag * next; // r26
    struct zFragGoo * goo; // r31
    struct xModelInstance * model; // r30
    struct xVec3 start; // r1+0x74
    struct xVec3 end; // r1+0x68
    struct xSweptSphere sws; // r1+0x114
    unsigned int previousFlags; // r25
    int result; // r27
    struct xVec3 ourAt; // r1+0x5C
    struct zShrapnelAsset * splatSnd; // r28
    struct xVec3 vel0; // r1+0x50
    struct xVec3 at; // r1+0x44
    float actualFadeTime; // f30
    float actualFadeTime; // f29
    struct xSphere collSphere; // r1+0x34
    struct xCollis collision; // r1+0xC0
    unsigned int previousFlags; // r24
    float perpDot; // f27
    struct xVec3 slideVec; // r1+0x28
    struct xVec3 perpComp; // r1+0x1C
    float decalSize; // f28
    struct xMat4x3 mat; // r1+0x80
    struct xVec3 at; // r1+0x10

    // References
    // -> static float GOO_DECAL_EMIT_TIME;
    // -> static float GOO_DECAL_SCALE;
    // -> static float GOO_MAX_SLIDE_SPEED;
    // -> static float GOO_SLIDE_ACCEL;
    // -> static float GOO_SLIDE_Y_THRESHOLD;
    // -> struct xGlobals * xglobals;
    // -> static float GOO_FADE_DELAY;
    // -> static float GOO_FADE_TIME;
    // -> static char __FUNCTION__[17];
    // -> struct xVec3 g_NZ3;
    // -> static float GOO_STICK_TIME;
    // -> static float GOO_FLY_GRAVITY;
    // -> static struct zFrag sGooList;
}

// Range: 0x801B93A4 -> 0x801B9418
void zFrag_DefaultShrapnelUpdate(struct zFrag * frag /* r30 */) {
    // Local variables
    struct zShrapnelAsset * sasset; // r31
}

// Range: 0x801B9418 -> 0x801B95B4
...
// Range: 0x801B95B4 -> 0x801B96FC
void zFrag_GooRenderer() {
    // Local variables
    struct zFrag * frag; // r29
    int reset; // r1+0x14
    struct zFragGoo * goo; // r31
    struct xModelInstance * currModel; // r28
    struct xMat4x3 * frame; // r30
