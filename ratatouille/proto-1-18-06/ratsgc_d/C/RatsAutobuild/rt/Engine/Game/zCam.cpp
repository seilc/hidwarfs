/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zCam.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x802A486C -> 0x802A6AD4
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
// total size: 0x48
class zTargetBlender {
    // Functions
    struct xVec3 & Update(float dt);

    struct xVec3 & operator const xVec3&() const;

    struct xVec3 & Result() const;

    void SetBlendTime(float time, unsigned char lazy);

    void Reset();

    unsigned char IsBlending();

    void SetGoal(const struct xVec3 & position, int numTargets);

    // Static members
    static float sBlendTime; // size: 0x4

    // Members
    float mBlendTime; // offset 0x0, size 0x4
    unsigned char mLazy; // offset 0x4, size 0x1
    struct xVec3 mPositionIn; // offset 0x8, size 0xC
    int mNumTargetsIn; // offset 0x14, size 0x4
    unsigned char mBlending; // offset 0x18, size 0x1
    unsigned char mFirstTime; // offset 0x19, size 0x1
    struct xVec3 mLastPos; // offset 0x1C, size 0xC
    struct xVec3 mStartPos; // offset 0x28, size 0xC
    struct xVec3 mResult; // offset 0x34, size 0xC
    int mNumTargets; // offset 0x40, size 0x4
    float mTime; // offset 0x44, size 0x4
};
// Range: 0x802A486C -> 0x802A4A50
// this: r31
struct xVec3 & zTargetBlender::Update(float dt /* r1+0x8 */) {
    // Local variables
    float t; // f30
    float s; // f31
}

// Range: 0x802A4A50 -> 0x802A4A78
// this: r1+0x8
struct xVec3 & zTargetBlender::operator const xVec3&() const {}

// Range: 0x802A4A78 -> 0x802A4A80
// this: r3
struct xVec3 & zTargetBlender::Result() const {}

// Range: 0x802A4A80 -> 0x802A4A98
void zTargetBlender::blend(float & o /* r4 */, float a /* f1 */, float b /* f2 */, float s /* f3 */) {}

float sBlendTime; // size: 0x4, address: 0x80D661D0
// Range: 0x802A4A98 -> 0x802A4AA0
float zTargetBlender::BlendTime() {
    // References
    // -> float sBlendTime;
}

// Range: 0x802A4AA0 -> 0x802A4AEC
void zTargetBlender::AddTweaks(const char * prefix /* r1+0x8 */) {
    // References
    // -> float sBlendTime;
}

// total size: 0xC
struct /* @class$832zCam_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$835zCam_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$836zCam_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$837zCam_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$838zCam_cpp */ {
    // Members
...
static unsigned int input_disabled; // size: 0x4, address: 0x80D6B458
static unsigned char have_default_orientation; // size: 0x1, address: 0x80D6B45C
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
static struct xVec3 default_orientation; // size: 0xC, address: 0x80D661D4
static struct zCamPlayer * follow_cam; // size: 0x4, address: 0x80D6B460
// total size: 0x18
struct analog_data {
    // Members
...
static struct xCamGroup global_cam; // size: 0xE0, address: 0x80D2C8D0
...
static struct xCamScreen global_screen; // size: 0x8, address: 0x80D6B464
static class zTargetBlender blendedLookat; // size: 0x48, address: 0x80D2C9B0
static class zTargetBlender blendedMidPoint; // size: 0x48, address: 0x80D2C9F8
static class zTargetBlender blendedFloorMount; // size: 0x48, address: 0x80D2CA40
static unsigned char sYTrackingBelowFloorEnabled; // size: 0x1, address: 0x80D6B46C
static unsigned char firstFrame; // size: 0x1, address: 0x80D661E0
static struct xVec3 lastCameraLookAt; // size: 0xC, address: 0x80D6B470
static unsigned char debug_cam_active; // size: 0x1, address: 0x80D6B47C
static struct xCamGroup debug_cam_group; // size: 0xE0, address: 0x80D2CA90
// total size: 0x70
...
static struct xCamDebug * debug_cam; // size: 0x4, address: 0x80D6B480
static unsigned char debug_cam_triggers; // size: 0x1, address: 0x80D6B484
static unsigned char allow60fpsEmulation; // size: 0x1, address: 0x80D661E1
// total size: 0x10
...
// Range: 0x802A4AEC -> 0x802A4C20
static void init_debug_cam() {
    // References
    // -> static unsigned char debug_cam_triggers;
    // -> static struct xCamDebug * debug_cam;
    // -> struct zGlobals globals;
    // -> static struct xCamGroup debug_cam_group;
    // -> static unsigned char debug_cam_active;
}

// Range: 0x802A4C20 -> 0x802A4C48
static void reset_debug_cam() {
    // References
    // -> static struct xCamGroup debug_cam_group;
}

// Range: 0x802A4C48 -> 0x802A4E04
static unsigned char update_debug_cam(float dt /* f31 */) {
    // References
    // -> static struct xCamGroup debug_cam_group;
    // -> struct zGlobals globals;
    // -> static struct xCamDebug * debug_cam;
...
// Range: 0x802A4E04 -> 0x802A4FD8
void zCamSceneEnter(struct zScene & scene /* r30 */) {
    // References
    // -> struct xVec3 m_Null;
    // -> struct zGlobals globals;
    // -> static unsigned char allow60fpsEmulation;
    // -> static class zTargetBlender blendedFloorMount;
    // -> static class zTargetBlender blendedMidPoint;
    // -> static class zTargetBlender blendedLookat;
    // -> static struct zCamPlayer * follow_cam;
    // -> static struct xCamGroup global_cam;
    // -> unsigned int FB_YRES;
    // -> unsigned int FB_XRES;
    // -> static struct xCamScreen global_screen;
    // -> static unsigned int input_disabled;
    // -> static unsigned char have_default_orientation;
}

// Range: 0x802A5014 -> 0x802A5088
void zCamSceneExit() {
    // References
    // -> static unsigned char firstFrame;
    // -> static struct xCamGroup debug_cam_group;
    // -> struct zGlobals globals;
}
...
// Range: 0x802A5088 -> 0x802A5158
static void zCamStartDefaultCamera() {
    // Local variables
...
// Range: 0x802A5158 -> 0x802A5254
void zCamReset(unsigned char refreshCameraTarget /* r1+0x8 */) {
    // References
    // -> struct xGlobals * xglobals;
    // -> struct zGlobals globals;
    // -> static struct xVec3 lastCameraLookAt;
    // -> static unsigned char firstFrame;
    // -> static class zTargetBlender blendedFloorMount;
    // -> static class zTargetBlender blendedMidPoint;
    // -> static class zTargetBlender blendedLookat;
...
struct xSerial {};
// total size: 0x4
struct /* @class$2192zCam_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2193zCam_cpp */ {
    // Members
...
// Range: 0x802A5254 -> 0x802A5264
// this: r3
void zTargetBlender::SetBlendTime(float time /* f1 */, unsigned char lazy /* r4 */) {
    // References
    // -> float sBlendTime;
}

// Range: 0x802A5264 -> 0x802A52A8
// this: r31
void zTargetBlender::Reset() {}

// Range: 0x802A52A8 -> 0x802A5550
void zCamSetCamera(unsigned int cameraID /* r30 */, unsigned char forceRestart /* r31 */) {
    // Local variables
    float len; // f31
    float len2; // f30
    float len_inv; // f29
...
// Range: 0x802A5550 -> 0x802A5570
static void zCamRestartDefaultCamera() {}

// Range: 0x802A5570 -> 0x802A55A0
void zCamSceneSetup() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x802A55A0 -> 0x802A5794
void zCamHalfAngleIntercept(struct xVec3 & o /* r30 */, const struct xVec3 & A /* r28 */, const struct xVec3 & B /* r31 */, const struct xVec3 & C /* r29 */) {
    // Local variables
    struct xVec3 BC; // r1+0x20
    float lengthBC; // f30
    struct xVec3 BA; // r1+0x14
    struct xVec3 CA; // r1+0x8
    float lengthBA; // f31
    float lengthCA; // f29
    float angleA2; // f28
    float angleB; // f27
    float angleCPrime; // f26
    float lengthBCPrime; // f25
}

// Range: 0x802A5794 -> 0x802A57A4
struct xVec3 & zCamLookPlayer() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x802A57A4 -> 0x802A57B4
struct xVec3 & zCamTransitionTarget() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x802A57B4 -> 0x802A57BC
void zCamSetYTracking(unsigned char enable /* r3 */) {
    // References
    // -> static unsigned char sYTrackingBelowFloorEnabled;
}

// Range: 0x802A57BC -> 0x802A57E4
unsigned char zCamIsChangingFocus() {
    // References
    // -> static class zTargetBlender blendedLookat;
}

// Range: 0x802A57E4 -> 0x802A57EC
// this: r3
unsigned char zTargetBlender::IsBlending() {}

// Range: 0x802A57EC -> 0x802A58C8
static float GetTargetFloorHeight(int whichTarget /* r1+0x8 */) {
    // Local variables
    struct zCommonPlayer * player; // r31
    float mountY; // f31
    float dyFromMount; // f30
...
// Range: 0x802A58C8 -> 0x802A58F4
float zCamFloorMountY() {
    // References
    // -> static class zTargetBlender blendedFloorMount;
}
...
// Range: 0x802A58F4 -> 0x802A5AD4
static void zCamCollideTriggersCamera() {
    // Local variables
    struct xScene & s; // r26
    struct xSphere camSphere; // r1+0x14
    struct xVec3 dummyDir; // r1+0x8
    struct zEntTrigger * * it; // r29
    struct zEntTrigger * * end; // r21
    struct zEntTrigger & trig; // r31
    const struct xTriggerAsset & asset; // r24
    unsigned char want_enter; // r28
    unsigned char want_exit; // r27
    const struct xLinkAsset * link; // r30
    const struct xLinkAsset * end_link; // r20
    unsigned char inside; // r25
...
static char __FUNCTION__[29]; // size: 0x1D, address: 0x805266D0
// total size: 0x24
struct xRay3 {
    // Members
    struct xVec3 origin; // offset 0x0, size 0xC
...
// Range: 0x802A5AD4 -> 0x802A5EFC
static void zCamCollideTriggersCamTarget(struct xScene * sc /* r25 */) {
    // Local variables
    struct xVec3 end_pos; // r1+0x64
    struct xVec3 start_pos; // r1+0x58
    struct xVec3 frame_disp; // r1+0x4C
    struct xVec3 frame_dir; // r1+0x40
    ...
    unsigned char collide; // r27
    struct xVec3 start; // r1+0x34
    struct xVec3 end; // r1+0x28
    struct xSphere sphere; // r1+0x18
    struct zEntTrigger * camVolumeTriggers[3]; // r1+0xC
    int numVolumes; // r22
    int i; // r29
    class xCamTransition * camTransition; // r26
...
// Range: 0x802A5EFC -> 0x802A6260
static void zCamUpdateBlendedTargets(float dt /* f31 */) {
    // Local variables
    int theSingleFocusTarget; // r31
    int dontFocus; // r30
    unsigned char hasFocus; // r29
    struct xVec3 floorMount; // r1+0x68
    struct xVec3 pos; // r1+0x5C
    struct xVec3 floorMount0; // r1+0x50
    struct xVec3 floorMount1; // r1+0x44
    struct xVec3 midPointFM; // r1+0x38
    struct xVec3 Pos0; // r1+0x2C
    struct xVec3 Pos1; // r1+0x20
    struct xVec3 midPoint; // r1+0x14
    struct xVec3 lookAt; // r1+0x8
...
// Range: 0x802A6260 -> 0x802A62A4
// this: r31
void zTargetBlender::SetGoal(const struct xVec3 & position /* r1+0x8 */, int numTargets /* r1+0xC */) {}

// Range: 0x802A62A4 -> 0x802A647C
static void zCamDoAnalogInput() {
    // Local variables
    struct zPlayer * human; // r31
    class zPad * pad; // r29
    class zPadAnalogControl * analog; // r30
...
// Range: 0x802A647C -> 0x802A6644
void zCamUpdate(float dt /* f30 */) {
    // Local variables
    float step; // f29
    float remain; // f31
    unsigned char InDebugCam; // r31
...
// Range: 0x802A6644 -> 0x802A6670
void zCamPrepareRender() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x802A6670 -> 0x802A66A0
void zCamBeginRender() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x802A66A0 -> 0x802A66EC
void zCamEndRender() {
    // References
    // -> struct zGlobals globals;
    // -> static unsigned char debug_cam_active;
...
// Range: 0x802A66EC -> 0x802A66F8
void zCamExitDebugCam() {
    // References
    // -> static unsigned char debug_cam_active;
}

// Range: 0x802A66F8 -> 0x802A6730
void zCamAdd(struct xCam & cam /* r1+0x8 */, unsigned char force_cut /* r1+0xC */) {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x802A6730 -> 0x802A6768
void zCamRemove(struct xCam & cam /* r1+0x8 */, unsigned char force_cut /* r1+0xC */) {
    // References
    // -> struct zGlobals globals;
}

static char __FUNCTION__[24]; // size: 0x18, address: 0x805266ED
// Range: 0x802A6768 -> 0x802A69D4
void zCamSetStartOrientation(unsigned int pointer_id /* r28 */) {
    // Local variables
    unsigned int size; // r1+0x8
    void * a; // r30
    const struct pointer_asset & pa; // r31
...
// Range: 0x802A69D4 -> 0x802A6A34
void zCamSetStartOrientation(float yaw /* f31 */, float pitch /* r1+0x8 */, float roll /* r1+0xC */) {
    // References
    // -> static struct zCamPlayer * follow_cam;
    // -> static struct xVec3 default_orientation;
    // -> static unsigned char have_default_orientation;
}

// Range: 0x802A6A34 -> 0x802A6A48
struct xCam * zCamGetPrimary() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x802A6A48 -> 0x802A6A5C
float zCamGetCameraPlayerAudioBias() {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x802A6A5C -> 0x802A6A90
struct xMat4x3 & zCamGetMatrix() {
    // References
    // -> struct zGlobals globals;
    // -> static struct xCamDebug * debug_cam;
    // -> static unsigned char debug_cam_active;
...
// Range: 0x802A6A90 -> 0x802A6ACC
unsigned char zCamGetDebugMode(struct xMat4x3 * * ppDebugCamMtx /* r3 */) {
    // References
    // -> static unsigned char debug_cam_active;
    // -> static struct xCamDebug * debug_cam;
}

// Range: 0x802A6ACC -> 0x802A6AD4
struct zCamPlayer * zCamGetDefault() {
    // References
    // -> static struct zCamPlayer * follow_cam;
}
