/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zNPCAsset.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8025A0A8 -> 0x8025A9E0
*/
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D65558
// total size: 0x8
struct xBaseAsset {
    // Members
...
// Range: 0x8025A0A8 -> 0x8025A274
static int ValidThief(const struct xDynAsset * dynAsset /* r31 */, unsigned int dynAssetSize /* r29 */) {
    // Local variables
    char fullName[256]; // r1+0x8
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D65563
// Range: 0x8025A274 -> 0x8025A440
static int ValidChef(const struct xDynAsset * dynAsset /* r31 */, unsigned int dynAssetSize /* r29 */) {
    // Local variables
    char fullName[256]; // r1+0x8
    unsigned long expected_asset_size; // r30
...
struct DynNPCInfo {
    // Members
    enum en_npctyp npcType; // offset 0x0, size 0x4
    const char * nam_npcAssetType; // offset 0x4, size 0x4
    int (* fun_dynAssetValidate)(struct xDynAsset *, unsigned int); // offset 0x8, size 0x4
    struct xBase * (* fun_npcCreate)(struct xDynAsset *, unsigned int); // offset 0xC, size 0x4
    unsigned int hash_npcAssetType; // offset 0x10, size 0x4
};
static struct DynNPCInfo g_DynNPCInfo[3]; // size: 0x3C, address: 0x80524080
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D6556D
// total size: 0xC
struct RwV3d {
    // Members
...
// Range: 0x8025A440 -> 0x8025A528
static struct xBase * MakeNPCGeneric(const struct xDynAsset * dynAsset /* r30 */, unsigned int dynAssetSize /* r1+0x8 */) {
    // Local variables
    unsigned long size_expected; // r1+0x10
    struct zNPCAssetIN2 * npcass; // r29
    struct xEntAsset * entass; // r31
    struct xBaseAsset * baseass; // r1+0xC
    struct xBase * npc; // r28

    // References
    // -> static char __FUNCTION__[15];
}

static int needToHash; // size: 0x4, address: 0x80D6B1C8
static signed char init; // size: 0x1, address: 0x80D6B1CC
// Range: 0x8025A528 -> 0x8025A5B0
void zNPCAsset_Startup() {
    // Local variables
    struct DynNPCInfo * npcinf_next; // r31
    struct DynNPCInfo * npcinf; // r30

    // References
    // -> static struct DynNPCInfo g_DynNPCInfo[3];
    // -> static int needToHash;
    // -> static signed char init;
}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80D6557C
// Range: 0x8025A5B0 -> 0x8025A6F0
int zNPCAsset_CountNPCs(int * typeCount /* r25 */) {
    // Local variables
    int count_NPC; // r27
    int num_alldyn; // r24
    int i; // r31
    unsigned int dynAssetSize; // r1+0x8
    const struct xDynAsset * dynAsset; // r30
    int isOurs; // r26
    struct DynNPCInfo * npcinf_next; // r29
    struct DynNPCInfo * npcinf; // r28
    int sum_total; // r23
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x805240BC
// Range: 0x8025A6F0 -> 0x8025A890
void zNPCAsset_CreateNPCs() {
    // Local variables
    int num_alldyn; // r27
    int i; // r29
    unsigned int dynAssetSize; // r1+0x10
    const struct xDynAsset * dynAsset; // r30
    struct DynNPCInfo * npcinf_next; // r28
    struct DynNPCInfo * npcinf; // r31
    int rc; // r1+0xC
    struct xBase * npc; // r1+0x8
...
static char __FUNCTION__[23]; // size: 0x17, address: 0x805240D1
// total size: 0x10
...
// total size: 0xC
struct /* @class$1080zNPCAsset_cpp */ {
    // Members
...
// Range: 0x8025A890 -> 0x8025A96C
void zNPCAsset_RegisterNPCs(int * base_idx /* r28 */) {
    // Local variables
    struct st_XORDEREDARRAY * npclist; // r30
    int num_npc; // r27
    int i; // r29
...
// Range: 0x8025A96C -> 0x8025A9E0
unsigned char zNPCAsset_IsNPC(const struct xDynAsset & asset /* r3 */) {
    // Local variables
    int i; // r31
