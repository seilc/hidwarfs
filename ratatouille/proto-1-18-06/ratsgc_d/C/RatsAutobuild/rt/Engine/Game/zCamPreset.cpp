/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zCamPreset.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x802B2588 -> 0x802B33F4
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x80526900
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// total size: 0x4
struct /* @class$1108zCamPreset_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1264zCamPreset_cpp */ {
    // Members
...
// total size: 0x10
struct /* @class$872zCamPreset_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x802B2588 -> 0x802B26D8
static struct xVec3 GetCheckpointLocation() {
    // Local variables
    struct xVec3 checkpointLocation; // r1+0x8
    int i; // r31
...
// total size: 0x890
struct /* @class$1899zCamPreset_cpp */ {
    // Static members
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D66438
// total size: 0x2
struct debug_info {
    // Members
    unsigned char player_relative; // offset 0x0, size 0x1
    unsigned char snap_to; // offset 0x1, size 0x1
};
// total size: 0x1C
struct zCamPreset : public xBase {
    // Functions
    void load(const struct asset_type & a);

    unsigned char get_checkpoint(struct xVec3 & loc, float & yaw) const;

    unsigned char at_checkpoint() const;

    float distance2(const struct xVec3 & to) const;

    unsigned char get_relative(struct xVec3 & offset, float & yaw) const;

    void get_matrix(struct xMat4x3 & mat) const;
    ...
    // Members
    struct asset_type * asset; // offset 0x10, size 0x4
private:
    int flags; // offset 0x14, size 0x4
    struct debug_info * debug; // offset 0x18, size 0x4
};
// Range: 0x802B26D8 -> 0x802B2884
void zCamPreset::load(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r29 */) {
    // Local variables
...
// total size: 0xC
struct /* @class$761zCamPreset_cpp */ {
    // Members
    float yaw; // offset 0x0, size 0x4
    float pitch; // offset 0x4, size 0x4
    float roll; // offset 0x8, size 0x4
...
struct asset_type : public xDynAsset {
    // Members
    unsigned int flags; // offset 0x10, size 0x4
    unsigned int checkpoint; // offset 0x14, size 0x4
...
// Range: 0x802B2884 -> 0x802B2924
void zCamPreset::update_all() {
    // Local variables
    unsigned char snapped; // r29
    struct zScene & s; // r30
    struct zCamPreset * preset; // r31
    struct zCamPreset * const end_preset; // r28

    // References
    // -> struct zGlobals globals;
}

// Range: 0x802B2924 -> 0x802B2960
void zCamPreset::apply_on_scene_prepare() {
    // Local variables
    struct zCamPreset * preset; // r31
}

// Range: 0x802B2960 -> 0x802B299C
void zCamPreset::apply_on_reset() {
    // Local variables
    struct zCamPreset * preset; // r31
}

// Range: 0x802B299C -> 0x802B2AC8
struct zCamPreset * zCamPreset::find_preset(unsigned char scene_prepare /* r25 */) {
    // Local variables
    struct zCamPreset * best_preset; // r28
    float best_dist2; // f31
    unsigned char best_has_checkpoint; // r27
    struct xVec3 to; // r1+0x14
    struct zScene & s; // r30
    struct zCamPreset * preset; // r31
    struct zCamPreset * const end_preset; // r26
    unsigned char has_checkpoint; // r29
    float dist2; // f30
...
// Range: 0x802B2AC8 -> 0x802B2B44
// this: r31
void zCamPreset::load(const struct asset_type & a /* r1+0x8 */) {}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D6643D
// total size: 0x28
struct PKRAssetType {
    // Members
...
// Range: 0x802B2B44 -> 0x802B2CEC
// this: r31
unsigned char zCamPreset::get_checkpoint(struct xVec3 & loc /* r24 */, float & yaw /* r27 */) const {
    // Local variables
    struct xVec3 check_loc; // r1+0x14
    struct PKRAssetTOCInfo ainfo; // r1+0x20
    int found; // r25
    struct xBaseAsset * checkpoint; // r30
    unsigned int type; // r26
    struct xEntAsset * player; // r29
    struct pointer_asset * pointer; // r28

    // References
    // -> static char __FUNCTION__[15];
}

// Range: 0x802B2CEC -> 0x802B2D44
// this: r1+0x8
unsigned char zCamPreset::at_checkpoint() const {
    // Local variables
    struct xVec3 check_loc; // r1+0x1C
    float check_yaw; // r1+0xC
}
...
// Range: 0x802B2D44 -> 0x802B2DEC
// this: r1+0x10
float zCamPreset::distance2(const struct xVec3 & to /* r31 */) const {
    // Local variables
...
// Range: 0x802B2DEC -> 0x802B2DF0
void zCamPreset::apply() {}

// Range: 0x802B2DF0 -> 0x802B2EBC
// this: r31
unsigned char zCamPreset::get_relative(struct xVec3 & offset /* r28 */, float & yaw /* r29 */) const {
    // Local variables
    const struct xMat4x3 & player_mat; // r30

    // References
    // -> struct xGlobals * xglobals;
}

// Range: 0x802B2EBC -> 0x802B2F78
// this: r31
void zCamPreset::get_matrix(struct xMat4x3 & mat /* r30 */) const {
    // Local variables
    struct xVec3 rel_offset; // r1+0x14
    float rel_yaw; // r1+0x10
    struct xMat4x3 check_mat; // r1+0x20
}

// Range: 0x802B2F78 -> 0x802B2FF4
// this: r31
void zCamPreset::event_handler(unsigned int event /* r1+0x8 */) {}

// Range: 0x802B2FF4 -> 0x802B304C
void zCamPreset::event_handler_callback(struct xBase * from /* r1+0x8 */, struct xBase * to /* r1+0xC */, unsigned int event /* r1+0x10 */, const float * argf /* r1+0x14 */, struct xBase * argw /* r1+0x18 */, unsigned int argwi /* r1+0x1C */) {
    // Local variables
    struct zCamPreset * w; // r31
}

// Range: 0x802B304C -> 0x802B30A0
// this: r31
void zCamPreset::debug_init() {}
...
// total size: 0xC
struct /* @class$808zCamPreset_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$809zCamPreset_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$810zCamPreset_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$811zCamPreset_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$812zCamPreset_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$813zCamPreset_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$814zCamPreset_cpp */ {
    // Members
...
// Range: 0x802B30A0 -> 0x802B33F4
// this: r31
void zCamPreset::add_tweaks() {
    // Local variables
    char prefix[128]; // r1+0x20
    const char * * player_labels; // r1+0x1C
    const char * * pointer_labels; // r1+0x18
    const unsigned int * player_ids; // r1+0x14
    const unsigned int * pointer_ids; // r1+0x10
    unsigned int players; // r30
    unsigned int pointers; // r28
    unsigned int checkpoints; // r29
    char * * checkpoint_labels; // r27
    unsigned int * checkpoint_ids; // r26
