/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zSpotlight.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x802168A4 -> 0x8021A9E0
*/
// total size: 0x4
struct xColor_tag {
    // Members
...
struct xColor_tag warnAuraColor; // size: 0x4, address: 0x80D64918
struct xColor_tag warnLightColor; // size: 0x4, address: 0x80D6491C
static float camera_clip_radius; // size: 0x4, address: 0x80D64920
static float camera_clip_expand; // size: 0x4, address: 0x80D64924
static float player_detect_radius; // size: 0x4, address: 0x80D64928
static unsigned char enable; // size: 0x1, address: 0x80D6AE68
static unsigned char track_camera; // size: 0x1, address: 0x80D6492C
static float radius; // size: 0x4, address: 0x80D64930
static float max_dist; // size: 0x4, address: 0x80D64934
static float view_angle; // size: 0x4, address: 0x80D64938
static unsigned char render_bounds; // size: 0x1, address: 0x80D6AE69
static struct xColor_tag color; // size: 0x4, address: 0x80D6493C
static unsigned char render_flares; // size: 0x1, address: 0x80D64940
static unsigned char render_auras; // size: 0x1, address: 0x80D64941
static unsigned char disable_glow; // size: 0x1, address: 0x80D6AE6A
static unsigned char render_wireframes; // size: 0x1, address: 0x80D6AE6B
static unsigned char render_bounds; // size: 0x1, address: 0x80D6AE6C
static int render_only; // size: 0x4, address: 0x80D64944
// total size: 0x8
struct substr {
    // Members
...
// total size: 0xC
struct /* @class$825zSpotlight_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$826zSpotlight_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$827zSpotlight_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$828zSpotlight_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$829zSpotlight_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$830zSpotlight_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$831zSpotlight_cpp */ {
    // Members
...
// Range: 0x802168A4 -> 0x80216C0C
static void add_tweaks() {
    // Local variables
    const char * prefix; // r31
    const char * * texture_labels; // r1+0x18
    const unsigned int * texture_ids; // r1+0x14
    unsigned int textures; // r1+0x10

    // References
    // -> static int render_only;
    // -> static unsigned char render_bounds;
    // -> static unsigned char render_wireframes;
    // -> static unsigned char disable_glow;
    // -> static unsigned char render_auras;
    // -> static unsigned char render_flares;
    // -> static struct xColor_tag color;
    // -> static unsigned char render_bounds;
    ...
    // -> static float player_detect_radius;
    // -> static float camera_clip_expand;
    // -> static float camera_clip_radius;
}
...
static struct flare_element * flares; // size: 0x4, address: 0x80D6AE70
static int flares_used; // size: 0x4, address: 0x80D6AE74
// total size: 0x4
struct cone_element {
    // Members
    struct zSpotlight * spotlight; // offset 0x0, size 0x4
};
static struct cone_element * cones; // size: 0x4, address: 0x80D6AE78
static int cones_used; // size: 0x4, address: 0x80D6AE7C
static struct RpAtomic * * light_cone_atomics; // size: 0x4, address: 0x80D6AE80
static struct RpAtomic * * aura_cone_atomics; // size: 0x4, address: 0x80D6AE84
static int light_cone_atomics_size; // size: 0x4, address: 0x80D6AE88
static int aura_cone_atomics_size; // size: 0x4, address: 0x80D6AE8C
static struct RpAtomic * test_cone_atomic; // size: 0x4, address: 0x80D6AE90
...
struct cone_attrib {
    ...
    struct zSpotlight * spot; // offset 0xC, size 0x4
};
// Range: 0x80216C0C -> 0x80216D04
static int compare_cones(void * e1 /* r3 */, void * e2 /* r4 */) {
    // Local variables
    const struct cone_attrib & a1; // r31
    const struct cone_attrib & a2; // r30
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80523370
// Range: 0x80216D04 -> 0x80216DA0
static void set_cone_tris(struct RpAtomic * atomic /* r1+0x8 */) {
    // Local variables
    struct RpGeometry * geom; // r27
    struct RpTriangle * tri; // r31
    struct RpTriangle * end_tri; // r26
    int index0; // r29
    int index1; // r30
    int end_index1; // r28
}
...
// Range: 0x80216DA0 -> 0x80216FEC
static void set_cone_verts(struct RpAtomic * atomic /* r1+0x8 */, float radius /* f29 */, float slope /* r1+0xC */, float max_dist /* r1+0x10 */) {
    // Local variables
    struct RpGeometry * geom; // r29
    struct RpMorphTarget * mt; // r30
    struct xVec3 * vert; // r31
    struct xVec3 * end_vert; // r28
    float dz; // f23
    float dradius; // f22
    float dcos; // f21
    float dsin; // f20
    ...
    float dx; // r1+0x18
    float dy; // r1+0x14
    struct xVec3 * end_column_vert; // r27
    float end_radius; // f25
    float hdist; // f24
    float hdist2; // f19
...
// Range: 0x80216FEC -> 0x80217154
static struct RpAtomic * create_cone_atomic() {
    // Local variables
    struct iAsyncLockRWSentry rw_sentry; // r1+0x8
    int vert_size; // r25
    int tri_size; // r26
    unsigned int geom_flags; // r24
...
// Range: 0x80217154 -> 0x8021719C
static void set_cone_atomic_color_solid(struct RpAtomic * atomic /* r1+0x8 */, struct xColor_tag & color /* r1+0xC */) {
    // Local variables
    struct RpGeometry * geom; // r31
}

// Range: 0x8021719C -> 0x802173D8
static void set_cone_atomic_color_faded(struct RpAtomic * atomic /* r1+0x8 */, struct xColor_tag & color /* r30 */) {
    // Local variables
    struct RpGeometry * geom; // r29
    struct xColor_tag * lum; // r31
    struct xColor_tag * end_lum; // r28
    float rstart; // f27
    float gstart; // f26
    float bstart; // f25
    float astart; // f24
    ...
    float b; // f29
    float a; // f28
    struct xColor_tag * end_column_lum; // r27
}
...
// Range: 0x802173D8 -> 0x802174BC
static struct RpMeshHeader * cone_stripper(struct RpBuildMesh * build_mesh /* r20 */) {
    // Local variables
    int total_indices; // r26
    int mesh_size; // r23
    struct RpMeshHeader * mesh_header; // r31
    struct RpMesh * mesh; // r29
    unsigned short * indices; // r30
    int left_index_value; // r25
    int right_index_value; // r28
    unsigned short * end_column; // r22
    int il; // r24
    int ir; // r27
}

// Range: 0x802174BC -> 0x80217518
static void finalize_cone_atomic(struct RpAtomic * atomic /* r1+0x8 */) {
    // Local variables
    struct RpMeshHeader * (* old_callback)(struct RpBuildMesh *, void *); // r1+0x10
    void * old_data; // r1+0xC
    struct RpGeometry * geom; // r31
}

// Range: 0x80217518 -> 0x80217578
static void destroy_atomic(struct RpAtomic * atomic /* r30 */) {
    // Local variables
    struct RwFrame * frame; // r31
}

// Range: 0x80217578 -> 0x802175E0
static void refresh_cone_atomic(struct RpAtomic * atomic /* r30 */, float radius /* r1+0x8 */, float slope /* r1+0xC */, float max_dist /* r1+0x10 */) {
    // Local variables
    struct RpGeometry * geom; // r31
}

// Range: 0x802175E0 -> 0x80217660
static void set_atomic_color(struct RpAtomic * atomic /* r1+0x8 */, struct xColor_tag & color /* r1+0xC */) {
    // Local variables
...
// Range: 0x80217660 -> 0x802176D4
static void move_atomic(struct RpAtomic * atomic /* r31 */, const struct xMat4x3 & mat /* r29 */, float scale /* r1+0x8 */) {
    // Local variables
    struct RwFrame * frame; // r30
}

// Range: 0x802176D4 -> 0x802176D8
static void debug_render() {}

// Range: 0x802176D8 -> 0x80217778
static void debug_init() {
    // Local variables
    float slope; // f31

    // References
    // -> static struct RpAtomic * test_cone_atomic;
    // -> struct xColor_tag g_WHITE;
    // -> static float max_dist;
    // -> static float radius;
    // -> static float view_angle;
...
// total size: 0x4
struct /* @class$884zSpotlight_cpp */ {
    // Members
...
union /* @class$1043zSpotlight_cpp */ {
...
// total size: 0xC
struct /* @class$1042zSpotlight_cpp */ {
    // Members
...
struct debug_data {
    ...
    unsigned char need_atomic_refresh; // offset 0x20, size 0x1
    struct RpAtomic * light_atomic; // offset 0x24, size 0x4
    struct RpAtomic * aura_atomic; // offset 0x28, size 0x4
};
// total size: 0xF0
struct zSpotlight : public xBase {
    // Functions
    void refresh_light_volume();

    void load(const struct zSpotlightAsset & a);
    ...
    void dispatch(unsigned int event);
    ...
    void debug_refresh_atomics();

    void add_tweaks();

    // Static members
    static float warnTime; // size: 0x4

    // Members
    struct zSpotlightAsset * asset; // offset 0x10, size 0x4
    int flags; // offset 0x14, size 0x4
private:
    struct xEnt * attach_ent; // offset 0x18, size 0x4
    struct xEnt * target_ent; // offset 0x1C, size 0x4
    struct xVec3 target_loc; // offset 0x20, size 0xC
    struct xMat4x3 mat; // offset 0x30, size 0x40
    struct xMat4x3 volumeMat; // offset 0x70, size 0x40
    ...
    struct xVec3 end_loc; // offset 0xC0, size 0xC
    float end_radius; // offset 0xCC, size 0x4
    struct RwRaster * raster_flare; // offset 0xD0, size 0x4
    struct RpAtomic * aura_atomic; // offset 0xD4, size 0x4
    unsigned char justActivated; // offset 0xD8, size 0x1
    float warnTimer; // offset 0xDC, size 0x4
    unsigned char enableRender; // offset 0xE0, size 0x1
    float view_angle_last; // offset 0xE4, size 0x4
    class light_volume_type * light_volume; // offset 0xE8, size 0x4
    struct debug_data * debug; // offset 0xEC, size 0x4
};
// Range: 0x8021779C -> 0x802177C0
void zSpotlight::scene_enter() {}

// Range: 0x802177C0 -> 0x80217850
void zSpotlight::scene_exit() {
    // Local variables
    struct RpAtomic * * light; // r31
    struct RpAtomic * * end_light; // r29
    struct RpAtomic * * aura; // r30
    struct RpAtomic * * end_aura; // r28

    // References
    // -> static int aura_cone_atomics_size;
    // -> static struct RpAtomic * * aura_cone_atomics;
    // -> static int light_cone_atomics_size;
    // -> static struct RpAtomic * * light_cone_atomics;
}
...
// total size: 0x10
struct /* @class$1922zSpotlight_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x80217850 -> 0x80217BE8
void zSpotlight::setup_all() {
    // Local variables
    struct zScene & s; // r22
    struct zSpotlight * spots; // r20
    int spots_size; // r27
    struct zSpotlight * spot; // r30
    struct zSpotlight * const end_spot; // r19
    struct cone_attrib * cone_buffer; // r23
    struct cone_attrib * cone; // r31
    struct cone_attrib * end_cone; // r18
    struct RpAtomic * * lights; // r29
    struct RpAtomic * * auras; // r28
    int lights_size; // r26
    int auras_size; // r25
    struct cone_attrib * last_light_cone; // r24
    struct cone_attrib * last_aura_cone; // r17
    ...
    // -> static struct cone_element * cones;
    // -> static struct flare_element * flares;
    // -> struct zGlobals globals;
}

// total size: 0x10
struct {} zSpotlight::light_volume_type::__vtable; // size: 0x10, address: 0x80D64984
// total size: 0x18
class light_volume_type : public xLightVolume {
    // Functions
    light_volume_type(struct RpAtomic * atomic, const struct xMat4x3 & mat);
...
public:
    ...
    unsigned char enableRender; // offset 0x11, size 0x1
    const struct xMat4x3 * mat; // offset 0x14, size 0x4
};
// Range: 0x80217BE8 -> 0x80217C40
// this: r31
light_volume_type::light_volume_type(struct RpAtomic * atomic /* r1+0x8 */, const struct xMat4x3 & mat /* r1+0xC */) {
    // References
    // -> struct [anonymous] zSpotlight::light_volume_type::__vtable;
}

// Range: 0x80217C4C -> 0x80217CC8
void zSpotlight::update_all(float dt /* r1+0x8 */) {
    // Local variables
    struct zScene & s; // r30
    struct zSpotlight * it; // r31
    struct zSpotlight * end; // r29

    // References
    // -> struct zGlobals globals;
}

// Range: 0x80217CC8 -> 0x80217D30
...
// Range: 0x80217D98 -> 0x80218174
// this: r31
void zSpotlight::refresh_light_volume() {
    // Local variables
    float radius; // f30
    float max_dist; // f27
    float view_angle; // f29
    const struct xVec3 & cam_loc; // r30
    struct xMat4x3 scale_mat; // r1+0x160
    struct xMat4x3 expand_mat; // r1+0x120
    float scale; // f31
    struct xVec3 posToCam; // r1+0x10
    float cam_dist; // f28
    float radius_at_camera; // f26
    struct xMat4x3 trans_mat; // r1+0xE0
    struct xMat4x3 itrans_mat; // r1+0xA0
    struct xMat4x3 temp_mat1; // r1+0x60
    struct xMat4x3 temp_mat2; // r1+0x20

    // References
    // -> static float camera_clip_expand;
    // -> static float camera_clip_radius;
    // -> struct xColor_tag warnLightColor;
}
...
void zSpotlight::render_all() {
    ...
    // -> static unsigned char render_flares;
    // -> static unsigned char render_auras;
    // -> static int flares_used;
    // -> static struct flare_element * flares;
    // -> static int cones_used;
    // -> struct zGlobals globals;
}

static char __FUNCTION__[5]; // size: 0x5, address: 0x80D64948
// Range: 0x802182A4 -> 0x80218460
void zSpotlight::load(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r29 */) {
    // Local variables
...
// total size: 0x4
struct /* @class$1446zSpotlight_cpp */ {
    // Members
    struct xColor_tag color; // offset 0x0, size 0x4
};
// total size: 0x4
struct /* @class$1447zSpotlight_cpp */ {
    // Members
    struct xColor_tag color; // offset 0x0, size 0x4
};
// total size: 0x14
struct /* @class$1448zSpotlight_cpp */ {
    // Members
...
// total size: 0x48
struct zSpotlightAsset : public xDynAsset {
    // Members
    unsigned int flags; // offset 0x10, size 0x4
...
// Range: 0x80218460 -> 0x802184D0
// this: r31
void zSpotlight::load(const struct zSpotlightAsset & a /* r1+0x8 */) {}

// Range: 0x802184D0 -> 0x80218530
// this: r31
void zSpotlight::setup() {
    // Local variables
    float radius; // r1+0xC
    float max_dist; // r1+0x8
}

// Range: 0x80218530 -> 0x80218608
// this: r31
void zSpotlight::reset() {}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80D6494D
struct xGlobals * xglobals; // size: 0x4, address: 0x80D63020
unsigned int gThisPlayer; // size: 0x4, address: 0x80D6903C
// Range: 0x80218608 -> 0x8021894C
// this: r31
void zSpotlight::update(float dt /* f31 */) {
    // Local variables
    int i; // r30
    unsigned int playerMask; // r29
    unsigned char hit; // r27
    struct xMat4x3 & model_mat; // r26
...
// total size: 0x890
struct /* @class$2165zSpotlight_cpp */ {
    // Static members
...
// Range: 0x8021894C -> 0x802189C8
// this: r31
void zSpotlight::activate() {}

// Range: 0x802189C8 -> 0x80218A20
// this: r31
void zSpotlight::deactivate() {}

// Range: 0x80218A20 -> 0x80218B0C
// this: r31
void zSpotlight::reload_asset() {}

// Range: 0x80218B0C -> 0x80218B88
// this: r30
void zSpotlight::load_attach_to(unsigned int id /* r1+0x8 */) {
    // Local variables
...
// Range: 0x80218B88 -> 0x80218BF4
// this: r30
void zSpotlight::load_target(unsigned int id /* r1+0x8 */) {
    // Local variables
    struct xBase * obj; // r31
}

// Range: 0x80218BF4 -> 0x80218C9C
// this: r31
void zSpotlight::load_textures() {
    // Local variables
    struct RwTexture * texture_flare; // r30
}

// Range: 0x80218C9C -> 0x80218E10
// this: r31
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D64954
// Range: 0x80218E10 -> 0x80219004
// this: r31
void zSpotlight::add_flare() {
    // Local variables
    struct flare_element * flare; // r30
    float mag; // f31
    float size0; // f30
    float dsize; // f28
    float glow0; // f29
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D6495E
// Range: 0x80219004 -> 0x802190C4
// this: r1+0x8
void zSpotlight::add_aura() {
    // Local variables
    struct cone_element * cone; // r31
...
// Range: 0x802190C4 -> 0x8021913C
// this: r31
void zSpotlight::render_setup() {}
...
// Range: 0x8021913C -> 0x80219844
// this: r31
void zSpotlight::render_aura() {
    // Local variables
    struct xColor_tag color; // r1+0x20
    struct xColor_tag warnColor; // r1+0x1C
    enum RwBlendFunction old_src_blend; // r1+0x18
    enum RwBlendFunction old_dst_blend; // r1+0x14

    // References
    // -> struct xColor_tag g_BLUE;
    // -> static unsigned char render_bounds;
    // -> struct xColor_tag warnAuraColor;
    // -> struct zGlobals globals;
}

// Range: 0x80219844 -> 0x802198C4
// this: r31
void zSpotlight::dispatch(unsigned int event /* r1+0x8 */) {}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64967
// Range: 0x802198C4 -> 0x802199E8
void zSpotlight::cb_dispatch(struct xBase * from /* r1+0x8 */, struct xBase * to /* r30 */, unsigned int event /* r1+0xC */, const float * argf /* r1+0x10 */, struct xBase * argw /* r1+0x14 */, unsigned int argwi /* r1+0x18 */) {
    // Local variables
    struct zSpotlight & e; // r31
...
// Range: 0x802199E8 -> 0x80219DC0
void zSpotlight::render_flares() {
    // Local variables
    struct RxObjSpace3DVertex * vert_buffer; // r28
    struct RxObjSpace3DVertex * vert; // r31
    struct RxObjSpace3DVertex * end_vert; // r26
    struct RwRaster * raster; // r27
    struct flare_element * flare; // r29
    struct flare_element * end_flare; // r25
...
// Range: 0x80219DC0 -> 0x80219E38
static void set_vert(struct RxObjSpace3DVertex & vert /* r3 */, float x /* f1 */, float y /* f2 */, float z /* f3 */, float u /* f4 */, float v /* f5 */, struct xColor_tag & color /* r4 */) {
    // Local variables
    struct RwV3d tmp; // r1+0xC
    struct RwRGBA col; // r1+0x8
}

// Range: 0x80219E9C -> 0x80219F3C
static void flush_textured_strip(struct RxObjSpace3DVertex * verts /* r1+0x8 */, int size /* r31 */) {
    // References
...
// Range: 0x80219F3C -> 0x80219F9C
void zSpotlight::render_auras() {
    // Local variables
    struct cone_element * cone; // r31
    struct cone_element * end_cone; // r30
...
// Range: 0x80219F9C -> 0x80219FF8
// this: r31
void zSpotlight::debug_init() {}

// Range: 0x80219FF8 -> 0x8021A1D8
// this: r31
void zSpotlight::debug_update() {
    // Local variables
    float radius; // f31
    float max_dist; // f30
...
// Range: 0x8021A1D8 -> 0x8021A234
// this: r31
char * zSpotlight::get_tweak_name() {
    // Local variables
    const char * name; // r30
}

// Range: 0x8021A234 -> 0x8021A244
// this: r3
void zSpotlight::debug_refresh_atomics() {}

static struct tweak_callback cb_move_player; // size: 0x28, address: 0x80523388
static signed char init; // size: 0x1, address: 0x80D6AE94
static struct tweak_callback cb_reload; // size: 0x28, address: 0x805233B0
static signed char init; // size: 0x1, address: 0x80D6AE95
static struct tweak_callback cb_reload_degrees; // size: 0x28, address: 0x804BCAB8
static struct tweak_callback cb_rotate_attach_to; // size: 0x28, address: 0x805233D8
static signed char init; // size: 0x1, address: 0x80D6AE96
// Range: 0x8021A244 -> 0x8021A808
// this: r30
void zSpotlight::add_tweaks() {
    // Local variables
    char prefix[64]; // r1+0x98
    struct zSpotlightAsset & a; // r31
    const char * * texture_labels; // r1+0x1C
    const char * * ent_labels; // r1+0x18
    const unsigned int * texture_ids; // r1+0x14
    const unsigned int * ent_ids; // r1+0x10
    unsigned int textures; // r28
    unsigned int ents; // r29

    // References
    // -> static struct tweak_callback cb_reload;
    // -> static struct tweak_callback cb_reload_degrees;
...
enum /* @enum$1445zSpotlight_cpp */ {
    FLAG_ON = 1,
    FLAG_DETECT_PLAYER = 2,
    FLAG_ROTATE_ATTACH_TO = 4,
};
// Range: 0x8021A808 -> 0x8021A84C
void zSpotlight::on_reload(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct zSpotlight & owner; // r31
}

// Range: 0x8021A84C -> 0x8021A8FC
void zSpotlight::on_move_player(const struct tweak_info & info /* r29 */) {
    // Local variables
    struct zSpotlight & owner; // r31
...
// Range: 0x8021A8FC -> 0x8021A958
void zSpotlight::on_rotate_attach_to(const struct tweak_info & info /* r3 */) {
    // Local variables
    struct zSpotlight & owner; // r31
}

// total size: 0x10
struct {} xLightVolume::__vtable; // size: 0x10, address: 0x80D64974
// Range: 0x8021A958 -> 0x8021A9E0
// this: r31
