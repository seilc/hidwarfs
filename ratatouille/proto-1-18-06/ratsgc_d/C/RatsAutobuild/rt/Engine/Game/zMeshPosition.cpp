/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zMeshPosition.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80261914 -> 0x80263200
*/
// total size: 0x8
struct xVec2 {
    // Members
...
// total size: 0x4
struct /* @class$1013zMeshPosition_cpp */ {
    // Members
...
union /* @class$1152zMeshPosition_cpp */ {
...
// total size: 0xC
struct /* @class$1151zMeshPosition_cpp */ {
    // Members
...
class zMeshPosition {
    ...
    unsigned char CheckIntegrity() const;

    unsigned char Valid() const;

    unsigned char IsMeshEnabled() const;

    int GetPortal(const class zMeshPosition & destination, struct xVec2 * portal, float radius) const;

    int GetPortal(const class zMeshPosition & destination, struct xVec3 * portal, float radius) const;

    void GetClosestPosition(class zMeshPosition & result) const;

    void CorrectHeight();

    void AdvanceThroughPortal(int side);

    unsigned char UpdatePositionAutoY(const struct xVec3 & newPos, unsigned short entFilter);

    unsigned char UpdatePositionAutoY(const struct xVec3 & newPos, const struct xEnt & ent);

    unsigned char UpdatePosition(const struct xVec3 & newPos, const float * overrideYLow, const float * overrideYHigh);

    unsigned char CheckInTriangle(const struct xVec3 & position, int subMesh, int triangle, const float * overrideYLow, const float * overrideYHigh);

    unsigned char FindAnyValid(const struct xVec3 & position, const float * yLow, const float * yHigh, unsigned char findClosestValid);

    unsigned char FindAnyValidAutoY(const struct xVec3 & position, unsigned short entFilter, unsigned char findClosestValid);

    unsigned char FindAnyValid(const struct xVec3 & position, const float * overrideYLow, const float * overrideYHigh);

    unsigned char FindNeighborOnPath(const struct xVec3 & startPos, const struct xVec3 & endPos, int subMeshIndex, int triangle, int & neighborTriangle);
...
// Range: 0x80261914 -> 0x80261944
// this: r31
zMeshPosition::zMeshPosition() {}

// Range: 0x80261944 -> 0x80261960
// this: r3
void zMeshPosition::Invalidate() {}
...
// Range: 0x80261960 -> 0x802619DC
// this: r3
unsigned char zMeshPosition::CheckIntegrity() const {}

// Range: 0x802619DC -> 0x80261AC4
// this: r31
unsigned char zMeshPosition::Valid() const {}

// Range: 0x80261AC4 -> 0x80261B1C
// this: r31
unsigned char zMeshPosition::IsMeshEnabled() const {}

// Range: 0x80261B1C -> 0x80261B9C
// this: r1+0x8
int zMeshPosition::GetPortal(const class zMeshPosition & destination /* r1+0xC */, struct xVec2 * portal /* r30 */, float radius /* r1+0x10 */) const {
    // Local variables
    struct xVec3 tempPortal[2]; // r1+0x14
    int value; // r31
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D655E8
struct xVec3 g_O3; // size: 0xC, address: 0x80D6D198
// Range: 0x80261B9C -> 0x802620A8
// this: r31
int zMeshPosition::GetPortal(const class zMeshPosition & destination /* r28 */, struct xVec3 * portal /* r27 */, float radius /* f30 */) const {
    // Local variables
    int exitIndex; // r23
    struct Exit & exit; // r25
    class zMeshPosition exitPosition; // r1+0x20
    class xNavigationMeshAssetSubMesh * subMesh; // r30
    int portalIndex; // r29
    int otherPortalIndex; // r22
    int next; // r24
    struct xVec3 direction; // r1+0x14
    float PORTAL_EDGE_EXTRA_AWAY; // f31
    int i; // r26
    struct xVec3 portalVec; // r1+0x8
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D655F2
// Range: 0x802620A8 -> 0x8026214C
void zMeshPosition::GetClosestPosition(const struct xVec3 & pos /* r1+0x8 */, class zMeshPosition & result /* r31 */) {
    // References
    // -> static char __FUNCTION__[19];
}

// Range: 0x8026214C -> 0x802623B8
// this: r31
void zMeshPosition::GetClosestPosition(class zMeshPosition & result /* r21 */) const {
    // Local variables
    class zMeshPosition meshPosition; // r1+0x14
    float smallest_distance2; // f31
    float Px; // f30
    float Py; // f24
    float Pz; // f29
    int m; // r26
    class xNavigationMeshAssetSubMesh * sub_mesh; // r30
    int i; // r29
    const unsigned char * portal; // r25
    const unsigned char * a; // r28
    const unsigned char * b; // r27
    const unsigned char * end; // r22
    int triangle_index; // r20
    const struct xVec3 & vertex_a; // r24
    const struct xVec3 & vertex_b; // r23
    float portal_point_x; // r1+0x10
    float portal_point_y; // r1+0xC
    float line_t; // r1+0x8
    float CPx; // f28
    float CPy; // f27
    float line_dist2; // f26
    float y_diff; // f25
    float portal_distance2; // f23
}
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D65605
// Range: 0x802623B8 -> 0x8026246C
// this: r31
void zMeshPosition::CorrectHeight() {
    // References
    // -> static char __FUNCTION__[14];
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80524308
// Range: 0x8026246C -> 0x80262544
// this: r31
void zMeshPosition::AdvanceThroughPortal(int side /* r30 */) {
    // References
...
// total size: 0xC
struct /* @class$805zMeshPosition_cpp */ {
    // Members
...
// Range: 0x80262544 -> 0x802625DC
// this: r29
unsigned char zMeshPosition::UpdatePositionAutoY(const struct xVec3 & newPos /* r30 */, unsigned short entFilter /* r1+0x8 */) {
    // Local variables
    struct zEntTrigger * trigger; // r31
    float toleranceHigh; // r1+0x10
    float toleranceLow; // r1+0xC
}

static unsigned char done; // size: 0x1, address: 0x80D6B1F0
static signed char init; // size: 0x1, address: 0x80D6B1F1
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D65613
// Range: 0x802625DC -> 0x802626F0
// this: r29
unsigned char zMeshPosition::UpdatePositionAutoY(const struct xVec3 & newPos /* r30 */, const struct xEnt & ent /* r31 */) {
    // References
...
// Range: 0x802626F0 -> 0x80262860
// this: r31
unsigned char zMeshPosition::UpdatePosition(const struct xVec3 & newPos /* r30 */, const float * overrideYLow /* r28 */, const float * overrideYHigh /* r29 */) {
    // Local variables
    int testSubMesh; // r1+0xC
    int testTriangle; // r1+0x8
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D65627
// Range: 0x80262860 -> 0x80262B1C
// this: r30
unsigned char zMeshPosition::CheckInTriangle(const struct xVec3 & position /* r31 */, int subMesh /* r29 */, int triangle /* r28 */, const float * overrideYLow /* r26 */, const float * overrideYHigh /* r27 */) {
    // Local variables
    float triangleHeight; // f31
    float yLow; // f28
    float yHigh; // f27
    float yLow; // f30
    float yHigh; // f29
...
// Range: 0x80262B1C -> 0x80262BB4
// this: r31
unsigned char zMeshPosition::FindAnyValid(const struct xVec3 & position /* r1+0x8 */, const float * yLow /* r1+0xC */, const float * yHigh /* r1+0x10 */, unsigned char findClosestValid /* r1+0x14 */) {
    // Local variables
    unsigned char found; // r30
}

// Range: 0x80262BB4 -> 0x80262C5C
// this: r27
unsigned char zMeshPosition::FindAnyValidAutoY(const struct xVec3 & position /* r30 */, unsigned short entFilter /* r28 */, unsigned char findClosestValid /* r29 */) {
    // Local variables
    struct zEntTrigger * trigger; // r31
    float toleranceHigh; // r1+0xC
    float toleranceLow; // r1+0x8
}

// Range: 0x80262C5C -> 0x80262D6C
// this: r31
unsigned char zMeshPosition::FindAnyValid(const struct xVec3 & position /* r28 */, const float * overrideYLow /* r1+0x8 */, const float * overrideYHigh /* r1+0xC */) {
    // Local variables
    int count; // r1+0x10
    int subMesh; // r30
    int i; // r29
...
// Range: 0x80262D6C -> 0x80262F9C
unsigned char zMeshPosition::CheckCrossEdge(const struct xVec3 * v0 /* r31 */, const struct xVec3 * v1 /* r28 */, const struct xVec3 & startPos /* r30 */, const struct xVec3 & endPos /* r29 */) {
    // Local variables
    float Ex; // f31
    float Ez; // f30
    float edgeLen; // f27
    float invEdgeLen; // f26
    float Nx; // f25
    float Nz; // f24
    float startDistAbove; // f29
    float endDistAbove; // f28
    float percent; // f23
    float Ix; // f21
    float Iz; // f20
    float along; // f22
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D65637
// Range: 0x80262F9C -> 0x80263200
// this: r29
unsigned char zMeshPosition::FindNeighborOnPath(const struct xVec3 & startPos /* r1+0x8 */, const struct xVec3 & endPos /* r1+0xC */, int subMeshIndex /* r24 */, int triangle /* r25 */, int & neighborTriangle /* r26 */) {
    // Local variables
    const class xNavigationMeshAssetSubMesh & subMesh; // r31
    const struct Triangle & tri; // r30
    struct xVec3 * vertices[3]; // r1+0x10
    int v0; // r27
    int v1; // r28
