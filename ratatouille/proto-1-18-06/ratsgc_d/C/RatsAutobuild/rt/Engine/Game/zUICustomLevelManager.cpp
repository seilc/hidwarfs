/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zUICustomLevelManager.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8029349C -> 0x802940E4
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct Scene scenes[200]; // size: 0x2580, address: 0x80D27660
// total size: 0x14
struct SceneUnlock {
    // Members
...
static struct SceneUnlock scenesUnlock[200]; // size: 0xFA0, address: 0x80D29BE0
static unsigned int sceneCurrent; // size: 0x4, address: 0x80D6B3D8
...
char * allsoaklevels[200]; // size: 0x320, address: 0x80D2AB80
// Range: 0x8029349C -> 0x802934A4
unsigned int zUIIncrediblesGetNumScenes() {
    // References
    // -> int numScenes;
}

static char __FUNCTION__[30]; // size: 0x1E, address: 0x80525570
// Range: 0x802934A4 -> 0x80293530
void zUIIncrediblesSetSceneCurrent(unsigned int value /* r31 */) {
    // References
    // -> static unsigned int sceneCurrent;
    // -> static char __FUNCTION__[30];
}

// Range: 0x80293530 -> 0x80293538
unsigned int zUIIncrediblesGetSceneCurrent() {
    // References
    // -> static unsigned int sceneCurrent;
}

static char __FUNCTION__[36]; // size: 0x24, address: 0x8052558E
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
// total size: 0x4
struct /* @class$924zUICustomLevelManager_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1080zUICustomLevelManager_cpp */ {
    // Members
...
union /* @class$636zUICustomLevelManager_cpp */ {
...
// total size: 0x10
struct /* @class$1630zUICustomLevelManager_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// total size: 0x890
struct /* @class$1873zUICustomLevelManager_cpp */ {
    // Static members
...
// Range: 0x80293538 -> 0x80293690
unsigned int zUIIncrediblesGetCurrentSceneNumber() {
    // Local variables
    int i; // r31
    unsigned int sceneID; // r30

    // References
    // -> static char __FUNCTION__[36];
    // -> int numScenes;
    // -> struct zGlobals globals;
    // -> static struct Scene scenes[200];
}

// Range: 0x80293690 -> 0x80293750
unsigned int zUIGetSceneNumberByID(unsigned int sceneID /* r3 */) {
    // Local variables
    unsigned int i; // r31
    unsigned int id; // r30
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D65EA8
// Range: 0x80293750 -> 0x802937E8
struct Scene * zUIGetSceneByID(unsigned int sceneID /* r30 */) {
    // Local variables
...
// Range: 0x802937E8 -> 0x802937F0
int zUIIncrediblesGetNumStoryScenes() {
    // References
    // -> int numStoryScenes;
}

// Range: 0x802937F0 -> 0x802937F8
int zUIIncrediblesGetNumArenaScenes() {
    // References
    // -> int numArenaScenes;
}

static char __FUNCTION__[23]; // size: 0x17, address: 0x805255B2
// Range: 0x802937F8 -> 0x802938AC
struct Scene * zUIIncrediblesGetScene(unsigned int level /* r30 */) {
    // References
    // -> static struct Scene scenes[200];
    // -> static char __FUNCTION__[23];
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D65EB8
// Range: 0x802938AC -> 0x80293940
struct SceneUnlock * zUIGetSceneUnlock(unsigned int idx /* r31 */) {
    // References
    // -> static struct SceneUnlock scenesUnlock[200];
    // -> static char __FUNCTION__[18];
}

// Range: 0x80293940 -> 0x80293970
struct Scene * zUIIncrediblesFindCurrentScene() {
    // References
    // -> static struct Scene scenes[200];
}

static char __FUNCTION__[31]; // size: 0x1F, address: 0x805255C9
// Range: 0x80293970 -> 0x80293A2C
unsigned char zUIIncrediblesSceneIsCompleted(unsigned int level /* r30 */) {
    // References
    // -> static struct Scene scenes[200];
    // -> static char __FUNCTION__[31];
}

static char __FUNCTION__[30]; // size: 0x1E, address: 0x805255E8
// Range: 0x80293A2C -> 0x80293AE8
unsigned char zUIIncrediblesSceneIsUnlocked(unsigned int level /* r30 */) {
    // References
    // -> static struct Scene scenes[200];
    // -> static char __FUNCTION__[30];
}
...
struct Group {
    // Members
    char code[2]; // offset 0x0, size 0x2
    char pad_unused[2]; // offset 0x2, size 0x2
};
// total size: 0x54
class zUISceneGroups {
    // Functions
    struct Group & operator[](unsigned int index);

    unsigned int NumGroups();

    void AddSceneGroup(unsigned int sceneID);

    void AddSceneGroup(const char * sceneID);

    zUISceneGroups();

    // Members
    unsigned int numSceneGroups; // offset 0x0, size 0x4
    struct Group groups[20]; // offset 0x4, size 0x50
};
class zUISceneGroups sSceneGroups; // size: 0x54, address: 0x80D2AEA0
char __FUNCTION__$localstatic1$__vc__14zUISceneGroupsFUi[5]; // size: 0x5, address: 0x80D65ECA
char __FUNCTION__$localstatic1$AddSceneGroup__14zUISceneGroupsFPCc[14]; // size: 0xE, address: 0x80D65ECF
// total size: 0xC
...
// Range: 0x80293AE8 -> 0x80293BF0
unsigned int zUISceneNumberFromCounter(struct _xCounter * counter /* r31 */) {
    // Local variables
    unsigned int groupIndex; // r29
    const struct Group & group; // r30
    unsigned int tens; // r28
    unsigned int ones; // r27
    unsigned int sceneID; // r26

    // References
    // -> class zUISceneGroups sSceneGroups;
}

// Range: 0x80293BF0 -> 0x80293C90
// this: r31
struct Group & zUISceneGroups::operator[](unsigned int index /* r30 */) {
    // References
    // -> char __FUNCTION__$localstatic1$__vc__14zUISceneGroupsFUi[5];
}

// Range: 0x80293C90 -> 0x80293C98
// this: r3
unsigned int zUISceneGroups::NumGroups() {}

// Range: 0x80293C98 -> 0x80293CCC
unsigned char zUISceneExists(struct _xCounter * mission /* r1+0x8 */) {}

// Range: 0x80293CCC -> 0x80293E48
void zUIAddScene(unsigned int sceneID /* r30 */) {
    // Local variables
    char sceneIDStr[5]; // r1+0x8
    unsigned int number; // r29
    struct Scene & scene; // r31

    // References
    // -> int numScenes;
    // -> char * allsoaklevels[200];
    // -> static struct Scene scenes[200];
    // -> class zUISceneGroups sSceneGroups;
}

// Range: 0x80293E48 -> 0x80293E98
// this: r1+0x8
void zUISceneGroups::AddSceneGroup(unsigned int sceneID /* r31 */) {
    // Local variables
    char code[2]; // r1+0xC
}

// Range: 0x80293E98 -> 0x80293FC0
// this: r31
void zUISceneGroups::AddSceneGroup(const char * sceneID /* r30 */) {
    // Local variables
    unsigned int i; // r29

    // References
    // -> char __FUNCTION__$localstatic1$AddSceneGroup__14zUISceneGroupsFPCc[14];
}

// Range: 0x80293FC0 -> 0x80293FE4
void zUIAllScenesAdded() {
    // References
    // -> int numScenes;
    // -> char * allsoaklevels[200];
}

static unsigned int cMissionCounterID; // size: 0x4, address: 0x80D6B3EC
static signed char init; // size: 0x1, address: 0x80D6B3F0
static unsigned int cMissionValidCounterID; // size: 0x4, address: 0x80D6B3F4
static signed char init; // size: 0x1, address: 0x80D6B3F8
// Range: 0x80293FE4 -> 0x802940B0
void zUILevelsUpdate() {
    // Local variables
    struct _xCounter * mission; // r30
    struct _xCounter * missionValid; // r31

    // References
    // -> static unsigned int cMissionValidCounterID;
    // -> static unsigned int cMissionCounterID;
    // -> static signed char init;
    // -> static signed char init;
}

// Range: 0x802940D8 -> 0x802940E4
// this: r3
zUISceneGroups::zUISceneGroups() {}
