/*
    ...
    Language: C++
    Code range: 0x802F40FC -> 0x802F7620
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
union /* @class$1067zHumanVehiclePlayerActions_cpp */ {
...
// total size: 0x4
struct /* @class$928zHumanVehiclePlayerActions_cpp */ {
    // Members
...
static char * surfaceNames[4]; // size: 0x10, address: 0x80D67028
// total size: 0x10
...
static struct driveSurfaceDescriptor surfaceDescriptors[4]; // size: 0x40, address: 0x80529A48
static unsigned char cameraRestartHack; // size: 0x1, address: 0x80D6B780
static unsigned char altControls; // size: 0x1, address: 0x80D67038
static float IDLE_SPEED; // size: 0x4, address: 0x80D6703C
static float WALK_SPEED; // size: 0x4, address: 0x80D67040
static float GRAVITY; // size: 0x4, address: 0x80D67044
static float FORWARD_ROLL_RETARDING; // size: 0x4, address: 0x80D67048
static float MAX_ACCELERATION; // size: 0x4, address: 0x80D6704C
static float MAX_NORMAL_SPEED; // size: 0x4, address: 0x80D67050
static float MAX_TURN_SPEED; // size: 0x4, address: 0x80D67054
static float MIN_SPEED; // size: 0x4, address: 0x80D6B784
static float ENGINE_BRAKING_DECELERATION; // size: 0x4, address: 0x80D67058
static float TURN_MAX; // size: 0x4, address: 0x80D6705C
static float COLLISION_REFLECTION_VERTICAL; // size: 0x4, address: 0x80D67060
static float COLLISION_REFLECTION_HORIZONTAL; // size: 0x4, address: 0x80D6B788
static float COLLISION_REFLECTION_HORIZONTAL_NPC; // size: 0x4, address: 0x80D67064
static float COLLISION_REFLECTION_HORIZONTAL_DST; // size: 0x4, address: 0x80D67068
static float SECONDARY_DIST; // size: 0x4, address: 0x80D6706C
static float MARGIN_ANGLE; // size: 0x4, address: 0x80D6B78C
static float SKID_FX_VEL_MIN; // size: 0x4, address: 0x80D67070
static float SKID_FX_VEL_MAX; // size: 0x4, address: 0x80D67074
static float TURN_SPEED; // size: 0x4, address: 0x80D67078
static float ACCEL_THRESHOLD; // size: 0x4, address: 0x80D6707C
static unsigned char ACCEL_ON_STICK; // size: 0x1, address: 0x80D6B790
static unsigned char HIT_REACT_ANIM; // size: 0x1, address: 0x80D6B791
static float KNOCKBACK_DIST; // size: 0x4, address: 0x80D67080
static float BIG_EPSILON; // size: 0x4, address: 0x80D67084
// total size: 0x10
struct rest_config {
    // Members
...
static struct rest_config driveRest; // size: 0x10, address: 0x80D67088
float boundYShift; // size: 0x4, address: 0x80D66FF8
// total size: 0x7DC
class zHumanVehiclePlayerDrive : public zPlayerAction {
    ...
    unsigned int SmHitReactStartCheck();

    unsigned int LgHitReactStartCheck();
    ...
    void PreventPoppingThroughWorld(const struct xVec3 * pStartPosition, struct xVec3 * pEndPosition, struct xVec3 * pVelocity);
...
// Range: 0x802F40FC -> 0x802F4534
// this: r30
void zHumanVehiclePlayerDrive::AddTweaks() {
    // Local variables
    ...
    // -> static float COLLISION_REFLECTION_HORIZONTAL_DST;
    // -> static float COLLISION_REFLECTION_HORIZONTAL_NPC;
    // -> static float COLLISION_REFLECTION_HORIZONTAL;
    // -> static float MIN_SPEED;
    // -> static float MAX_TURN_SPEED;
    // -> static float MAX_NORMAL_SPEED;
    // -> static float IDLE_SPEED;
    // -> static unsigned char HIT_REACT_ANIM;
    // -> static struct driveSurfaceDescriptor surfaceDescriptors[4];
    // -> static char * surfaceNames[4];
}

// Range: 0x802F4534 -> 0x802F4540
void zHumanVehiclePlayerDrive::CameraRestartHack() {
    // References
    // -> static unsigned char cameraRestartHack;
}

// Range: 0x802F4540 -> 0x802F46E8
// this: r30
void zHumanVehiclePlayerDrive::AddStates(struct xAnimTable * table /* r31 */) {}

// Range: 0x802F46E8 -> 0x802F46EC
void zHumanVehiclePlayerDrive::AddActionTransitions() {}

// Range: 0x802F46EC -> 0x802F4B04
void zHumanVehiclePlayerDrive::AddInternalTransitions(struct xAnimTable * table /* r31 */) {}

// Range: 0x802F4B04 -> 0x802F4B58
// this: r3
...
// Range: 0x802F4EA4 -> 0x802F4F30
// this: r3
unsigned char zHumanVehiclePlayerDrive::isAnimStateStopped(unsigned int stateID /* r4 */) const {
    // Local variables
    struct xAnimState * astate; // r31
}

// Range: 0x802F4F30 -> 0x802F4F34
void zHumanVehiclePlayerDrive::AddTransitionsFrom() {}

// Range: 0x802F4F34 -> 0x802F4F38
void zHumanVehiclePlayerDrive::Setup() {}

// Range: 0x802F4F38 -> 0x802F4FDC
// this: r31
...
void zHumanVehiclePlayerDrive::ForceCameraOrientation() {
    ...
    // -> static float SECONDARY_DIST;
}

// Range: 0x802F5048 -> 0x802F5150
...
// total size: 0x14
struct /* @class$2270zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0x30
struct /* @class$2271zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$2272zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0x6C
struct /* @class$2273zHumanVehiclePlayerActions_cpp */ {
    // Members
    float dist; // offset 0x0, size 0x4
...
// total size: 0xC
struct /* @class$2274zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0x18
struct /* @class$2275zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0x18
struct /* @class$2277zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0x8
struct /* @class$2278zHumanVehiclePlayerActions_cpp */ {
    // Members
...
struct zHumanVehiclePlayer : public zPlayer {
    ...
    unsigned char isOn; // offset 0x13BC, size 0x1
    float hitTimer; // offset 0x13C0, size 0x4
    float leanLerp; // offset 0x13C4, size 0x4
    float animWalk[3]; // offset 0x13C8, size 0xC
    float animRun[3]; // offset 0x13D4, size 0xC
    unsigned char debugDrawDebugLines; // offset 0x13E0, size 0x1
    float modelYaw; // offset 0x13E4, size 0x4
    struct xVec3 velocity; // offset 0x13E8, size 0xC
    enum iSndGroupHandle soundAsset[2]; // offset 0x13F4, size 0x8
    enum xSndHandle soundSkidLoopHandle; // offset 0x13FC, size 0x4
    class zInteraction * currAction; // offset 0x1400, size 0x4
private:
    unsigned int * atomics_order; // offset 0x1404, size 0x4
    unsigned int atomics_count; // offset 0x1408, size 0x4
    struct xVec3 bone_adjust; // offset 0x140C, size 0xC
    struct xVec3 prev_at; // offset 0x1418, size 0xC
    struct xVec3 prev_bi_normal; // offset 0x1424, size 0xC
    float x_lean; // offset 0x1430, size 0x4
    float y_lean; // offset 0x1434, size 0x4
    float last_sign; // offset 0x1438, size 0x4
    unsigned int modelRender; // offset 0x143C, size 0x4
};
// Range: 0x802F5150 -> 0x802F5380
// this: r31
void zHumanVehiclePlayerDrive::Move(struct xScene * s /* r1+0x8 */, float dt /* f29 */, struct xEntFrame * frame /* r29 */) {
    // Local variables
    struct zHumanVehiclePlayer * hvPlayer; // r30
    float steer; // f30
    float accel; // f31
    ...
    // -> static unsigned char ACCEL_ON_STICK;
    // -> static unsigned char cameraRestartHack;
...
// total size: 0x4
struct /* @class$2250zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0x4
struct /* @class$2032zHumanVehiclePlayerActions_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2033zHumanVehiclePlayerActions_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// total size: 0x4
struct /* @class$2034zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0x8
struct /* @class$2031zHumanVehiclePlayerActions_cpp */ {
    // Members
...
class zSpringboard {};
// total size: 0x2
struct /* @class$2036zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0x10
struct /* @class$1341zHumanVehiclePlayerActions_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// total size: 0x890
struct /* @class$1838zHumanVehiclePlayerActions_cpp */ {
    // Static members
...
static float driveAngle; // size: 0x4, address: 0x80D6B794
static signed char init; // size: 0x1, address: 0x80D6B798
float PLSBOUND_RADIUS; // size: 0x4, address: 0x80D66FF0
// total size: 0x40
...
// Range: 0x802F5380 -> 0x802F5F60
// this: r31
void zHumanVehiclePlayerDrive::Physics(float dt /* f29 */, float steer /* f23 */, float accel /* f26 */, struct xEntFrame * frame /* r29 */) {
    // Local variables
    struct zHumanVehiclePlayer * hvPlayer; // r30
    float lastSpeed; // f20
    float forwardSpeed; // r1+0x60
    float rightSpeed; // f22
    float upSpeed; // r1+0x5C
    float speedPercentage; // f30
    float surfaceSpeedPercentage; // f25
    float steerAngle; // f28
    float absmag; // f21
    float x; // r1+0x58
    float z; // r1+0x54
    float stickCameraAngle; // r1+0x50
    float analogMag; // r1+0x4C
    struct xMat4x3 playerAbsMat; // r1+0xD0
    float camAngle; // r1+0x48
    float angle_difference; // f31
    float steerFactor; // f19
    float accelerationFactor; // f24
    struct xVec3 tempVelocity; // r1+0xB8
    float oldVelMag; // r1+0x44
    struct xVec3 newVelocity; // r1+0xAC
    struct xVec3 acceleration; // r1+0xA0
    struct xVec3 forwardVelocity; // r1+0x94
    struct xVec3 rightVelocity; // r1+0x88
    float velMag; // r1+0x40
    struct xVec3 oldPos; // r1+0x7C
    struct xVec3 newPos; // r1+0x70
    struct xVec3 groundNormal; // r1+0x64
    unsigned int animUserFlag; // r25
    unsigned int blendUserFlag; // r24
    float skidFXIntensity; // f27

    // References
    // -> static float SKID_FX_VEL_MIN;
    // -> static float SKID_FX_VEL_MAX;
    // -> float boundYShift;
    // -> float PLSBOUND_RADIUS;
    // -> static float WALK_SPEED;
    // -> static float IDLE_SPEED;
    // -> static float GRAVITY;
    // -> static float FORWARD_ROLL_RETARDING;
    // -> static float MAX_ACCELERATION;
    // -> static float MIN_SPEED;
    // -> static float ENGINE_BRAKING_DECELERATION;
    // -> static float TURN_MAX;
...
// Range: 0x802F5F60 -> 0x802F5FAC
void zHumanVehiclePlayerDrive::CalculateIdealOrientation(float yaw /* r1+0x10 */, struct xMat3x3 * pIdealOrientation /* r1+0x14 */) {
    // Local variables
    struct xMat3x3 yawMatrix; // r1+0x20
}

static char __FUNCTION__[5]; // size: 0x5, address: 0x80D67098
static unsigned int zCOLLISION_REFLECT_Y; // size: 0x4, address: 0x80D70AC8
// Range: 0x802F5FAC -> 0x802F61F8
// this: r31
unsigned char zHumanVehiclePlayerDrive::operator()(struct xEnt & ent /* r29 */) {
    // Local variables
    struct xCollis * pCollis; // r30
    int collisionsDetected; // r27
    int maxCollisions; // r25
    int f; // r26
    int f; // r28
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D6709D
char __FUNCTION__$localstatic1$GetOwner__22zIComponent<8zNPCBase>Fv[9]; // size: 0x9, address: 0x80D670AD
static unsigned int zCOLLISION_NPC; // size: 0x4, address: 0x80D70AF0
static unsigned int zCOLLISION_DST; // size: 0x4, address: 0x80D70AF4
static unsigned int zCOLLISION_NEEDS_DEPEN_OUTER_BOUND; // size: 0x4, address: 0x80D70AF8
static unsigned int zCOLLISION_NEEDS_REFLECTION; // size: 0x4, address: 0x80D70AFC
static unsigned int zCOLLISION_NEEDS_DEPEN_INNER_BOUND; // size: 0x4, address: 0x80D70B00
float PLSBOUND_MIDRADIUS; // size: 0x4, address: 0x80D66FF4
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x802F61F8 -> 0x802F6DB4
// this: r31
void zHumanVehiclePlayerDrive::SphereCollision(const struct xVec3 * pNewPosition /* r1+0x8 */, const struct xVec3 * pVelocity /* r1+0xC */, unsigned char * pInGroundContact /* r1+0x10 */, struct xVec3 * pGroundNormal /* r1+0x14 */) {
    // Local variables
    int i; // r27
    struct xSphere collisionSphere[5]; // r1+0xC0
    int i; // r23
    int i; // r26
    int numGroundCollisions; // r25
    int tempNumCollisions; // r20
    int i; // r22
    struct xVec3 averageGroundNormal; // r1+0xB4
    int i; // r30
    const struct xSurface * pSurface; // r24
    unsigned char isWallSurfaceType; // r1+0x1C
    struct zSurfaceProps * pSurfaceProps; // r29
    unsigned int surfaceType; // r18
    struct xEnt * pEntity; // r28
    struct xVec3 playerPos; // r1+0xA8
    struct xVec3 npcPos; // r1+0x9C
    struct xVec3 knockback; // r1+0x90
    struct xVec3 delta; // r1+0x84
    struct xVec3 tempVelocity; // r1+0x78
    float scalarVel2; // f26
    class zICompNPCEntity * npcEnt; // r1+0x48
    class zNPCBase * npcOwn; // r1+0x44
    class zICompLogic * logicComponent; // r1+0x40
    class zCompLogicWaiter * waiterLogicComponent; // r1+0x3C
    struct zEntSimpleObj * pSimpleObj; // r17
    float hitParams[4]; // r1+0x68
    struct zPlatform * pPlatform; // r1+0x38
    float hitParams[4]; // r1+0x58
    struct xVec3 downVec; // r1+0x4C
    float collisionDownDist; // f30
    unsigned char isLowCollision; // r1+0x1B
    unsigned char isHighCollision; // r1+0x1A
    float normDotUp; // f31
    float angleFromFlat; // f25
    unsigned char isSteepSurface; // r19
    unsigned char drawLine; // r1+0x19
    float normDot; // f24
    float downDist; // f23
    float distFromMidShockPos; // f22
...
// total size: 0xC
struct /* @class$869zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$870zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$871zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$872zHumanVehiclePlayerActions_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$873zHumanVehiclePlayerActions_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$874zHumanVehiclePlayerActions_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$875zHumanVehiclePlayerActions_cpp */ {
    // Members
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D670B6
enum eCollisionType {
    eCollisionTypeHeadOn = 0,
    eCollisionTypeSideKnock = 1,
    eCollisionTypeDamage = 2,
};
// Range: 0x802F6DB4 -> 0x802F6EBC
// this: r30
void zHumanVehiclePlayerDrive::CollisionFX(enum eCollisionType type /* r1+0x8 */) {
    // Local variables
...
// Range: 0x802F6EBC -> 0x802F72D0
// this: r31
void zHumanVehiclePlayerDrive::ApplyCollisionRebounds(struct xVec3 * pVelocity /* r29 */, struct xEntFrame * frame /* r28 */) {
    // Local variables
    int i; // r30
    float collisionReflectionHorizontal; // f30
    float normDot; // f29
    struct xVec3 initialVel; // r1+0x24
    float vehicleWallAngle; // f22
    float VEHICLE_FRONT_ANGLE_RANGE; // f21
    float forwardSpeed; // f20
    struct xVec3 forwardVel; // r1+0x18
    float wallForce; // f28
    float CRASH_SPEED_THRESHOLD; // f19
    struct xVec3 velocityDelta; // r1+0xC
    float verticalVelocityDelta; // r1+0x8
    float velocityDeltaSize; // f27
    float SIDE_KNOCK_MIN_VEL; // f26
    float SIDE_KNOCK_MAX_VEL; // f18
    float volumeFactor; // f31

    // References
    // -> static float COLLISION_REFLECTION_VERTICAL;
    // -> static float COLLISION_REFLECTION_HORIZONTAL_DST;
    // -> static float COLLISION_REFLECTION_HORIZONTAL_NPC;
    // -> static float COLLISION_REFLECTION_HORIZONTAL;
}

// Range: 0x802F72D0 -> 0x802F7408
// this: r31
void zHumanVehiclePlayerDrive::DepenetrateCollisionList(struct xVec3 * pPosition /* r29 */) {
    // Local variables
    int i; // r30
    float innerBound; // f30
    float depenDepth; // f31
    struct xVec3 depen; // r1+0x8
    float depenScale; // f29
...
// total size: 0xC
struct /* @class$1066zHumanVehiclePlayerActions_cpp */ {
    // Members
...
// Range: 0x802F7408 -> 0x802F7620
// this: r27
void zHumanVehiclePlayerDrive::PreventPoppingThroughWorld(const struct xVec3 * pStartPosition /* r1+0x8 */, struct xVec3 * pEndPosition /* r28 */, struct xVec3 * pVelocity /* r31 */) {
    // Local variables
    struct xVec3 startPos; // r1+0x30
    struct xVec3 endPos; // r1+0x24
    struct xVec3 direction; // r1+0x18
    float rayLength; // f29
    struct xRay3 ray; // r1+0x3C
    struct xCollis collision; // r1+0x60
    int a; // r1+0x14
    float normDot; // f31
    float offset; // f30
...
// total size: 0x54
struct {} zHumanVehiclePlayerDrive::__vtable; // size: 0x54, address: 0x80529A88
// total size: 0xC
