/*
    ...
    Language: C++
    Code range: 0x801CBB30 -> 0x801CCEF4
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static class zUI * uiElements[1024]; // size: 0x1000, address: 0x80D0C288
static unsigned int uiElementCount; // size: 0x4, address: 0x80D6AA60
static unsigned char uiHDREnabled; // size: 0x1, address: 0x80D6AA64
static unsigned char uiAnyActive; // size: 0x1, address: 0x80D63CE0
static unsigned char screenLetterBoxActive; // size: 0x1, address: 0x80D6AA65
static float afterTimer; // size: 0x4, address: 0x80D6AA68
// total size: 0x8
struct substr {
    // Members
...
// total size: 0x1
struct /* @class$833zUIMgr_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$834zUIMgr_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$835zUIMgr_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$836zUIMgr_cpp */ {
    // Members
...
static struct tweak_callback uiCallbacks; // size: 0x28, address: 0x80D0D288
// Range: 0x801CBB30 -> 0x801CBC50
static char * zUIMgrGetTypeName(class zUI * ui /* r3 */) {}
...
// Range: 0x801CBC50 -> 0x801CBCEC
static void OnExpandTweaks(const struct tweak_info & info /* r29 */) {
    // Local variables
    class zUI * ui; // r31
    char name[128]; // r1+0x8
    const struct zUIAsset * pAsset; // r30
...
// Range: 0x801CBCEC -> 0x801CC074
static void OnUpdateTweak() {
    // Local variables
    const struct tweak_info * ti; // r30
    class zUI * ui; // r31

    // References
    // -> unsigned int FB_YRES;
    // -> unsigned int FB_XRES;
    // -> static struct tweak_callback uiCallbacks;
}

// Range: 0x801CC074 -> 0x801CC07C
float zUIGetTimer() {
    // References
    // -> static float afterTimer;
}

// Range: 0x801CC07C -> 0x801CC084
void zUISetTimer(float value /* f1 */) {
    // References
    // -> static float afterTimer;
}

static struct tweak_callback uimotionCallbacks; // size: 0x28, address: 0x80521E70
static signed char init; // size: 0x1, address: 0x80D6AA6C
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D63CE1
// total size: 0x30
class xfont {
    // Members
public:
...
// Range: 0x801CC084 -> 0x801CC710
void zUIMgrInit(struct zScene * s /* r30 */) {
    // Local variables
    class zUIText * ui1; // r1+0x24
    int i; // r24
    class zUIImage * ui2; // r1+0x20
    int i; // r23
    class zUIModel * ui3; // r1+0x1C
    int i; // r22
    class zUIBox * ui4; // r1+0x18
    int i; // r21
    class zUIUserString * ui5; // r1+0x14
    int i; // r20
    class zUIController * ui7; // r1+0x10
    int i; // r19
    int i; // r1+0xC
    struct zUIMotionAsset * asset; // r27
    struct zUIMotionCmdAsset * cmd; // r29
    unsigned int j; // r18
    struct zUIMotionCmdColor * color; // r31
    unsigned int i; // r26
    unsigned int i; // r28
    char name[128]; // r1+0xF8
    int count; // r1+0x8
    int i; // r25
    struct zUIMotionAsset * asset; // r17
    char name[128]; // r1+0x78

    // References
    // -> static struct tweak_callback uimotionCallbacks;
...
// Range: 0x801CC958 -> 0x801CC960
void zUIMgrSetScreenLetterBox(unsigned char value /* r3 */) {
    // References
    // -> static unsigned char screenLetterBoxActive;
}

// Range: 0x801CC960 -> 0x801CC968
...
// Range: 0x801CCB1C -> 0x801CCB24
void zUIMgrEnableHDR(unsigned char enabled /* r3 */) {
    // References
    // -> static unsigned char uiHDREnabled;
}

// Range: 0x801CCB24 -> 0x801CCB78
void zUIMgrSortList(class zUI * * list /* r1+0x8 */, int count /* r1+0xC */) {}
...
// Range: 0x801CCC7C -> 0x801CCE44
void zUIMgrRenderElementHDR(struct zUIMgrHDRContext & context /* r28 */, class zUI * ui /* r31 */) {
    // Local variables
    unsigned char blends; // r30
    struct xColor_tag save; // r1+0x10
    void * oldAlphaTest; // r1+0xC
    void * oldAlphaTestValue; // r1+0x8
}

// Range: 0x801CCE44 -> 0x801CCE50
...
// Range: 0x801CCE50 -> 0x801CCEF4
static int ElementCompare::operator()(void * A /* r1+0x8 */, void * B /* r1+0xC */) {
    // Local variables
    const class zUI * a; // r31
    const class zUI * b; // r30
    unsigned int akey; // r28
    unsigned int bkey; // r27
}
