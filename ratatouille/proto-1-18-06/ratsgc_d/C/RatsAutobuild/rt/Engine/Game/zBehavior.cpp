/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zBehavior.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801E711C -> 0x801EB73C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
unsigned char sDebugBehavior; // size: 0x1, address: 0x80D6ACF0
// Range: 0x801E711C -> 0x801E7124
unsigned int ErrorCB() {}

static long long lastTime; // size: 0x8, address: 0x80D6ACF8
static signed char init; // size: 0x1, address: 0x80D6AD00
static float lastDiff; // size: 0x4, address: 0x80D6AD04
static signed char init; // size: 0x1, address: 0x80D6AD08
// Range: 0x801E7124 -> 0x801E7298
static void BehaviorLog(class behavior * inBehavior /* r28 */, const char * inTxt1 /* r26 */, const char * inTxt2 /* r27 */) {
    // Local variables
    class zNPCCommon * npc; // r31
    long long curTime; // r29
...
// total size: 0xC
struct /* @class$1039zBehavior_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$2286zBehavior_cpp */ {
    // Members
...
class behavior {
    // Functions
    unsigned char JumpTransitionAllowable();

    unsigned char DodgeTransitionAllowable();

    unsigned char inAnimState(const char * state, float look_ahead, struct xModelInstance * model) const;

    unsigned char inAnimStateRange(unsigned int stateIDstart, unsigned int stateIDend, float look_ahead, struct xModelInstance * model) const;

    unsigned char inAnimState(unsigned int stateID, float look_ahead, struct xModelInstance * model) const;

    unsigned char isAnimStateStopped(const char * state, struct xModelInstance * model) const;

    unsigned char isAnimStateStopped(unsigned int stateID, struct xModelInstance * model) const;

    float getAnimDuration();

    float getAnimTime();

    void setAnimState(unsigned int new_stateID, float blend, struct xModelInstance * model);

    void setAnimState(const char * new_state, float blend, struct xModelInstance * model);

    void setAnimState(const char * new_state, float blend, float dest_time, struct xModelInstance * model);

    void setAnimState(unsigned int new_stateID, float blend, float dest_time, struct xModelInstance * model);

    unsigned char hasAnimEnded(struct xModelInstance * model);

    void addTest(class behavior * destBehavior, unsigned char (* inTestFunc)(struct xEnt *, class behavior *, class behavior *), char * description, float waitTime);

    void addTest(class behavior * destBehavior, unsigned char (* inTestFunc)(struct xEnt *, class behavior *, class behavior *));

    void replaceTest(class behavior * oldDestBehavior, unsigned char (* oldTestFunc)(struct xEnt *, class behavior *, class behavior *), class behavior * newDestBehavior, unsigned char (* newTestFunc)(struct xEnt *, class behavior *, class behavior *), char * description);

    void replaceTest(class behavior * oldDestBehavior, unsigned char (* oldTestFunc)(struct xEnt *, class behavior *, class behavior *), class behavior * newDestBehavior, unsigned char (* newTestFunc)(struct xEnt *, class behavior *, class behavior *));

    void insertTest(class behavior * oldDestBehavior1, unsigned char (* oldTestFunc1)(struct xEnt *, class behavior *, class behavior *), class behavior * oldDestBehavior2, unsigned char (* oldTestFunc2)(struct xEnt *, class behavior *, class behavior *), class behavior * newDestBehavior, unsigned char (* newTestFunc)(struct xEnt *, class behavior *, class behavior *), char * description);

    void insertTest(class behavior * oldDestBehavior1, unsigned char (* oldTestFunc1)(struct xEnt *, class behavior *, class behavior *), class behavior * oldDestBehavior2, unsigned char (* oldTestFunc2)(struct xEnt *, class behavior *, class behavior *), class behavior * newDestBehavior, unsigned char (* newTestFunc)(struct xEnt *, class behavior *, class behavior *));

    class behavior * runTests(float seconds);

    void ClearTestWaitTimers();

    behavior();

    unsigned char isAttackState();
...
// total size: 0x70
struct {} zBehaviorManager::__vtable; // size: 0x70, address: 0x805228CC
// total size: 0x14
...
// total size: 0x890
struct /* @class$2051zBehavior_cpp */ {
    // Static members
...
class zBehaviorManager {
    ...
    void createTransientStack(int inMax);
    ...
    void beforeAnimMatrices(struct xAnimPlay * play, struct xQuat * quat, struct xVec3 * tran, int boneCount);

    unsigned char collisionResponse(const struct xVec3 & current_dir, const struct SphereCollisionResults & scene_collide, struct xVec3 & response_dpos);

    void registerCollision(const struct SphereCollisionResults & scene_collide);
    ...
    unsigned char CanBeTargeted();

    unsigned char CanBeInterruptedForIncidental();

    unsigned char IsFrozen();

    void nuke(struct xVec3 * sourcePos, float inTime, float inRadius, float outRadius);

    void RunDamageBehavior(class behavior * dmgBehavior, struct zCombatDamageInfo & damageInfo);

    unsigned char IsInDamageBehavior();

    void damage(struct zCombatDamageInfo & damageInfo);

    unsigned char TestForJumpBehaviorChange();

    unsigned char TestForShieldBehaviorChange();

    unsigned char TestForDeathVolumeBehaviorChange();

    void testForBehaviorChange(float dt);

    unsigned char testForCompleteChange();

    void preupdate();

    void update(float dt);

    void addBehavior(class behavior * new_behavior, int priority);

    void setBehavior(class behavior * new_behavior);
    ...
    void PushTransient(class behavior * pushTransient, struct zTransientNodeData * initData);

    void PopTransient();

    void ResetTransients(unsigned char enterCurBehavior);
...
// Range: 0x801E7298 -> 0x801E73B0
// this: r31
zBehaviorManager::zBehaviorManager() {
    // References
    // -> struct [anonymous] zBehaviorManager::__vtable;
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x805226F8
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x801E73B0 -> 0x801E7488
// this: r31
void zBehaviorManager::createTransientStack(int inMax /* r30 */) {
    // References
    // -> static char __FUNCTION__[21];
    // -> unsigned int gActiveHeap;
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D64118
// Range: 0x801E7488 -> 0x801E7634
// this: r29
unsigned char zBehaviorManager::systemEvent(struct xBase * from /* r23 */, struct xBase * to /* r24 */, unsigned int to_event /* r25 */, const float * to_param /* r26 */, struct xBase * to_param_widget /* r27 */, unsigned int to_param_widget_id /* r28 */) {
    // Local variables
    class behavior * curBehavior; // r30
    struct behavior_node * node; // r31

    // References
    // -> static char __FUNCTION__[12];
}

// Range: 0x801E7634 -> 0x801E76A0
// this: r1+0x8
void zBehaviorManager::beforeAnimMatrices(struct xAnimPlay * play /* r1+0xC */, struct xQuat * quat /* r1+0x10 */, struct xVec3 * tran /* r1+0x14 */, int boneCount /* r1+0x18 */) {
    // Local variables
    class behavior * curBehavior; // r31
}

// Range: 0x801E76A0 -> 0x801E7728
// this: r29
unsigned char zBehaviorManager::collisionResponse(const struct xVec3 & current_dir /* r1+0x8 */, const struct SphereCollisionResults & scene_collide /* r1+0xC */, struct xVec3 & response_dpos /* r1+0x10 */) {
    // Local variables
    unsigned char collide_on; // r30
    struct behavior_node * node; // r31
}

// Range: 0x801E7728 -> 0x801E7788
// this: r30
void zBehaviorManager::registerCollision(const struct SphereCollisionResults & scene_collide /* r1+0x8 */) {
    // Local variables
    struct behavior_node * node; // r31
}

// Range: 0x801E7788 -> 0x801E77E8
// this: r30
void zBehaviorManager::add_states(struct xAnimTable * table /* r1+0x8 */) {
    // Local variables
    struct behavior_node * node; // r31
}

// Range: 0x801E77E8 -> 0x801E7848
// this: r30
void zBehaviorManager::add_transitions(struct xAnimTable * table /* r1+0x8 */) {
    // Local variables
    struct behavior_node * node; // r31
}

// Range: 0x801E7848 -> 0x801E78E0
// this: r30
void zBehaviorManager::staticSetup() {
    // Local variables
    struct behavior_node * node; // r31
...
// Range: 0x801E78E0 -> 0x801E792C
// this: r1+0x8
void zBehaviorManager::debugDisplay() {
    // Local variables
    class behavior * curBehavior; // r31
}

// Range: 0x801E792C -> 0x801E7978
// this: r1+0x8
void zBehaviorManager::debugRender() {
    // Local variables
    class behavior * curBehavior; // r31
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D64124
// Range: 0x801E7978 -> 0x801E7A1C
// this: r1+0x8
unsigned char zBehaviorManager::CanBeTargeted() {
    // Local variables
    class behavior * curBehavior; // r31
...
static char __FUNCTION__[30]; // size: 0x1E, address: 0x8052270D
// Range: 0x801E7A1C -> 0x801E7AC4
// this: r1+0x8
unsigned char zBehaviorManager::CanBeInterruptedForIncidental() {
    // Local variables
    class behavior * curBehavior; // r31
...
// Range: 0x801E7AC4 -> 0x801E7B28
// this: r31
unsigned char zBehaviorManager::IsFrozen() {}
...
// Range: 0x801E7B28 -> 0x801E7DD0
// this: r31
void zBehaviorManager::nuke(struct xVec3 * sourcePos /* r1+0x8 */, float inTime /* f28 */, float inRadius /* f29 */, float outRadius /* f27 */) {
    // Local variables
    class zNPCCommon * npc; // r30
    float outRadius2; // f26
    float inRadius2; // f25
    float dist2; // f30
    float dist; // f24
    float t; // f23
    float at; // f31
    struct zTransientNodeData transientData; // r1+0xC
}
...
// Range: 0x801E7DD0 -> 0x801E7E80
// this: r29
void zBehaviorManager::RunDamageBehavior(class behavior * dmgBehavior /* r30 */, struct zCombatDamageInfo & damageInfo /* r31 */) {
    // Local variables
    unsigned char transit; // r28
    class zNPCCommon * npcOwner; // r27
}
...
// total size: 0x4
struct /* @class$2269zBehavior_cpp */ {
    // Members
...
// total size: 0x4
struct /* @class$2228zBehavior_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2229zBehavior_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// total size: 0x4
struct /* @class$2230zBehavior_cpp */ {
    // Members
...
// total size: 0x8
struct /* @class$2227zBehavior_cpp */ {
    // Members
...
class zNPCBase {};
// total size: 0x2
struct /* @class$2232zBehavior_cpp */ {
    // Members
...
// total size: 0x10
struct /* @class$1797zBehavior_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// total size: 0x1
struct /* @class$845zBehavior_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$846zBehavior_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$847zBehavior_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$848zBehavior_cpp */ {
    // Members
...
// Range: 0x801E7E80 -> 0x801E7EF4
// this: r31
unsigned char zBehaviorManager::IsInDamageBehavior() {
    // Local variables
    class behavior * curBehavior; // r30
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80D64132
enum ezHitReaction {
    zHR_None = 0,
    zHR_Smash = 1,
    zHR_Damage = 2,
...
// Range: 0x801E7EF4 -> 0x801E879C
// this: r30
void zBehaviorManager::damage(struct zCombatDamageInfo & damageInfo /* r31 */) {
    // Local variables
    class zNPCCommon * npcOwner; // r29
    unsigned char blocked; // r28
    enum ezHitReaction hitReaction; // r27
    enum ezHitReaction hitReaction; // r26

    // References
    // -> static char __FUNCTION__[7];
}

// Range: 0x801E879C -> 0x801E87A0
void zBehaviorManager::carry() {}

static unsigned char done; // size: 0x1, address: 0x80D6AD09
static signed char init; // size: 0x1, address: 0x80D6AD0A
static char __FUNCTION__[26]; // size: 0x1A, address: 0x8052276C
// total size: 0x0
...
// Range: 0x801E87A0 -> 0x801E8A0C
// this: r29
unsigned char zBehaviorManager::TestForJumpBehaviorChange() {
    // Local variables
    class zNPCCommon * npc; // r31
    struct xVec3 npcPos; // r1+0x20
    struct xVec3 landPoint; // r1+0x14
    struct zEntTrigger * isectrigger; // r30
    class zMeshPosition localMeshPos; // r1+0x2C
    unsigned char result; // r28
    char tempStr[256]; // r1+0x74
    struct zTransientNodeData tranData; // r1+0x44
    struct xVec3 boxUpperWorld; // r1+0x8
...
// Range: 0x801E8A0C -> 0x801E8AD8
// this: r31
unsigned char zBehaviorManager::TestForShieldBehaviorChange() {
    // Local variables
    struct xVec3 poi; // r1+0x8
    unsigned int shieldDir; // r30
}
...
// Range: 0x801E8AD8 -> 0x801E8C54
// this: r29
unsigned char zBehaviorManager::TestForDeathVolumeBehaviorChange() {
    // Local variables
    class zNPCCommon * npc; // r31
    struct xVec3 npcPos; // r1+0x8
    struct zCombat * npcCombat; // r30
    struct zEntTrigger * trigger; // r28
    struct zCombatDamageInfo dmInfo; // r1+0x14

    // References
    // -> struct xVec3 m_Null;
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x80522786
// Range: 0x801E8C54 -> 0x801E8E10
// this: r31
void zBehaviorManager::testForBehaviorChange(float dt /* f31 */) {
    // Local variables
    class behavior * changeTo; // r29
    class zNPCCommon * npc; // r30
    struct xVec3 * npcPos; // r28

    // References
    // -> static char __FUNCTION__[22];
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x8052279C
// Range: 0x801E8E10 -> 0x801E8F68
// this: r31
unsigned char zBehaviorManager::testForCompleteChange() {
    // Local variables
    class behavior * transient; // r30

    // References
    // -> static char __FUNCTION__[22];
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D64139
// Range: 0x801E8F68 -> 0x801E90DC
// this: r31
void zBehaviorManager::preupdate() {
    // Local variables
    class behavior * transient; // r30
...
static char __FUNCTION__[7]; // size: 0x7, address: 0x80D64143
// Range: 0x801E90DC -> 0x801E92B8
// this: r31
void zBehaviorManager::update(float dt /* f31 */) {
    // Local variables
    class zNPCCommon * npc; // r29
    int originalTotal; // r28
    class behavior * transient; // r30
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6414A
// Range: 0x801E92B8 -> 0x801E9480
// this: r31
void zBehaviorManager::addBehavior(class behavior * new_behavior /* r25 */, int priority /* r27 */) {
    // Local variables
    struct behavior_node * node; // r29
    struct behavior_node * inserted_node; // r30
    struct behavior_node * pre; // r26
    struct behavior_node * post; // r28
...
// Range: 0x801E9480 -> 0x801E9598
// this: r31
void zBehaviorManager::setBehavior(class behavior * new_behavior /* r28 */) {
    // Local variables
    class behavior * prevBehavior; // r30
    class behavior * const previous_behavior; // r29
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D64156
// Range: 0x801E9598 -> 0x801E963C
// this: r31
void zBehaviorManager::preRender() {
    // References
    // -> static char __FUNCTION__[10];
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80D64160
// Range: 0x801E963C -> 0x801E96E0
// this: r31
void zBehaviorManager::render() {
    // References
    // -> static char __FUNCTION__[7];
}

static char __FUNCTION__[6]; // size: 0x6, address: 0x80D64167
// Range: 0x801E96E0 -> 0x801E98CC
// this: r31
void zBehaviorManager::reset() {
    // Local variables
    unsigned short beforeFlags; // r29
    struct behavior_node * node; // r30

    // References
    // -> static char __FUNCTION__[6];
}

// Range: 0x801E98CC -> 0x801E9928
// this: r30
void zBehaviorManager::exit() {
    // Local variables
    class behavior * curBehavior; // r31
}

// Range: 0x801E9928 -> 0x801E992C
void zBehaviorManager::addTweaks() {}

// Range: 0x801E992C -> 0x801E9968
// this: r31
unsigned char behavior::JumpTransitionAllowable() {}

// Range: 0x801E9968 -> 0x801E99A4
// this: r31
unsigned char behavior::DodgeTransitionAllowable() {}

// Range: 0x801E99A4 -> 0x801E99F8
// this: r1+0x8
unsigned char behavior::inAnimState(const char * state /* r1+0xC */, float look_ahead /* r1+0x10 */, struct xModelInstance * model /* r1+0x14 */) const {}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D6416D
// Range: 0x801E99F8 -> 0x801E9B98
// this: r29
unsigned char behavior::inAnimStateRange(unsigned int stateIDstart /* r1+0x8 */, unsigned int stateIDend /* r1+0xC */, float look_ahead /* f30 */, struct xModelInstance * model /* r30 */) const {
    // Local variables
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6417E
// Range: 0x801E9B98 -> 0x801E9D84
// this: r29
unsigned char behavior::inAnimState(unsigned int stateID /* r1+0x8 */, float look_ahead /* f30 */, struct xModelInstance * model /* r30 */) const {
    // Local variables
    struct xAnimState * astate; // r31
    float duration; // f31
...
// Range: 0x801E9D84 -> 0x801E9DD0
// this: r1+0x8
unsigned char behavior::isAnimStateStopped(const char * state /* r1+0xC */, struct xModelInstance * model /* r1+0x10 */) const {}

// Range: 0x801E9DD0 -> 0x801E9E84
// this: r3
unsigned char behavior::isAnimStateStopped(unsigned int stateID /* r4 */, struct xModelInstance * model /* r5 */) const {
    // Local variables
    struct xAnimState * astate; // r31
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D6418A
// Range: 0x801E9E84 -> 0x801E9FA8
// this: r29
float behavior::getAnimDuration() {
    // Local variables
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6419A
// Range: 0x801E9FA8 -> 0x801EA0C0
// this: r29
float behavior::getAnimTime() {
    // Local variables
    struct xModelInstance * model; // r31
    struct xAnimState * astate; // r30
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D641A6
// Range: 0x801EA0C0 -> 0x801EA1E8
// this: r29
void behavior::setAnimState(unsigned int new_stateID /* r28 */, float blend /* f31 */, struct xModelInstance * model /* r30 */) {
    // Local variables
    struct xAnimTransition * transition; // r31

    // References
    // -> static char __FUNCTION__[13];
}

// Range: 0x801EA1E8 -> 0x801EA23C
// this: r1+0x8
void behavior::setAnimState(const char * new_state /* r1+0xC */, float blend /* r1+0x10 */, struct xModelInstance * model /* r1+0x14 */) {}

// Range: 0x801EA23C -> 0x801EA298
// this: r1+0x8
void behavior::setAnimState(const char * new_state /* r1+0xC */, float blend /* r1+0x10 */, float dest_time /* r1+0x14 */, struct xModelInstance * model /* r1+0x18 */) {}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D641B3
// Range: 0x801EA298 -> 0x801EA3CC
// this: r29
void behavior::setAnimState(unsigned int new_stateID /* r28 */, float blend /* f31 */, float dest_time /* r1+0x8 */, struct xModelInstance * model /* r30 */) {
    // Local variables
    struct xAnimTransition * transition; // r31
...
// Range: 0x801EA3CC -> 0x801EA46C
// this: r3
unsigned char behavior::hasAnimEnded(struct xModelInstance * model /* r4 */) {
    // Local variables
    struct xAnimState * astate; // r31
    float duration; // f31
}

static char __FUNCTION__[8]; // size: 0x8, address: 0x80D641C0
// Range: 0x801EA46C -> 0x801EA6A4
// this: r1+0x8
void behavior::addTest(class behavior * destBehavior /* r1+0xC */, unsigned char (* inTestFunc)(struct xEnt *, class behavior *, class behavior *) /* r1+0x10 */, char * description /* r28 */, float waitTime /* r1+0x14 */) {
    // Local variables
    struct testNode * append_node; // r31
    int len; // r29
    struct testNode * node; // r30
...
// Range: 0x801EA6A4 -> 0x801EA6F8
// this: r1+0x8
void behavior::addTest(class behavior * destBehavior /* r1+0xC */, unsigned char (* inTestFunc)(struct xEnt *, class behavior *, class behavior *) /* r1+0x10 */) {}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D641C8
// Range: 0x801EA6F8 -> 0x801EA918
// this: r1+0x8
void behavior::replaceTest(class behavior * oldDestBehavior /* r1+0xC */, unsigned char (* oldTestFunc)(struct xEnt *, class behavior *, class behavior *) /* r1+0x10 */, class behavior * newDestBehavior /* r1+0x14 */, unsigned char (* newTestFunc)(struct xEnt *, class behavior *, class behavior *) /* r1+0x18 */, char * description /* r29 */) {
    // Local variables
    struct testNode * node; // r31
    int len; // r30
...
// Range: 0x801EA918 -> 0x801EA978
// this: r1+0x8
void behavior::replaceTest(class behavior * oldDestBehavior /* r1+0xC */, unsigned char (* oldTestFunc)(struct xEnt *, class behavior *, class behavior *) /* r1+0x10 */, class behavior * newDestBehavior /* r1+0x14 */, unsigned char (* newTestFunc)(struct xEnt *, class behavior *, class behavior *) /* r1+0x18 */) {}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D641D4
// Range: 0x801EA978 -> 0x801EABE4
// this: r1+0x8
void behavior::insertTest(class behavior * oldDestBehavior1 /* r1+0xC */, unsigned char (* oldTestFunc1)(struct xEnt *, class behavior *, class behavior *) /* r1+0x10 */, class behavior * oldDestBehavior2 /* r1+0x14 */, unsigned char (* oldTestFunc2)(struct xEnt *, class behavior *, class behavior *) /* r1+0x18 */, class behavior * newDestBehavior /* r1+0x1C */, unsigned char (* newTestFunc)(struct xEnt *, class behavior *, class behavior *) /* r1+0x20 */, char * description /* r28 */) {
    // Local variables
    struct testNode * node; // r31
    struct testNode * insert_node; // r30
    int len; // r29
...
// Range: 0x801EABE4 -> 0x801EAC54
// this: r1+0x8
void behavior::insertTest(class behavior * oldDestBehavior1 /* r1+0xC */, unsigned char (* oldTestFunc1)(struct xEnt *, class behavior *, class behavior *) /* r1+0x10 */, class behavior * oldDestBehavior2 /* r1+0x14 */, unsigned char (* oldTestFunc2)(struct xEnt *, class behavior *, class behavior *) /* r1+0x18 */, class behavior * newDestBehavior /* r1+0x1C */, unsigned char (* newTestFunc)(struct xEnt *, class behavior *, class behavior *) /* r1+0x20 */) {}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D641DF
// Range: 0x801EAC54 -> 0x801EADA8
// this: r30
class behavior * behavior::runTests(float seconds /* r1+0x8 */) {
    // Local variables
    struct testNode * node; // r31
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D641E8
// Range: 0x801EADA8 -> 0x801EAE54
// this: r30
void behavior::ClearTestWaitTimers() {
    // Local variables
    struct testNode * node; // r31
...
// total size: 0x118
struct {} behavior::__vtable; // size: 0x118, address: 0x805227B4
// Range: 0x801EAE54 -> 0x801EAE74
// this: r3
behavior::behavior() {
    // References
    // -> struct [anonymous] behavior::__vtable;
}

// Range: 0x801EAE74 -> 0x801EAE90
// this: r3
unsigned char behavior::isAttackState() {}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D641FC
// Range: 0x801EAE90 -> 0x801EB0F4
// this: r31
void zBehaviorManager::PushTransient(class behavior * pushTransient /* r29 */, struct zTransientNodeData * initData /* r1+0x8 */) {
    // Local variables
    class behavior * prevBehavior; // r30
    struct zTransientNodeData * contextData; // r28

    // References
    // -> static char __FUNCTION__[14];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D6420A
// Range: 0x801EB0F4 -> 0x801EB484
// this: r31
void zBehaviorManager::PopTransient() {
    // Local variables
    class behavior * poppedTransient; // r30
    struct zTransientNode * tnode; // r28
    class behavior * nextTransient; // r29
    struct zTransientNodeData * contextData; // r27

    // References
    // -> static char __FUNCTION__[13];
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D64217
// Range: 0x801EB484 -> 0x801EB73C
// this: r31
void zBehaviorManager::ResetTransients(unsigned char enterCurBehavior /* r1+0x8 */) {
    // Local variables
    class behavior * prevBehavior; // r30
