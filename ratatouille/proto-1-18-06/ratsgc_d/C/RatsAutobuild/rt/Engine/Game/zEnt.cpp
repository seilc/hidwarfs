/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zEnt.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8014AE64 -> 0x8014E828
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static int g_dbdisp_stdlog; // size: 0x4, address: 0x80D69FA0
static int g_dbdisp_showusage; // size: 0x4, address: 0x80D69FA4
unsigned int checkpoint_collision_hack(zEnt*)::model_id; // size: 0x4, address: 0x80D69FA8
signed char checkpoint_collision_hack(zEnt*)::localstatic4 guard; // size: 0x1, address: 0x80D69FAC
float checkpoint_collision_hack(zEnt*)::xlower; // size: 0x4, address: 0x80D69FB0
signed char checkpoint_collision_hack(zEnt*)::localstatic6 guard; // size: 0x1, address: 0x80D69FB4
float checkpoint_collision_hack(zEnt*)::xupper; // size: 0x4, address: 0x80D69FB8
signed char checkpoint_collision_hack(zEnt*)::localstatic8 guard; // size: 0x1, address: 0x80D69FBC
float checkpoint_collision_hack(zEnt*)::ylower; // size: 0x4, address: 0x80D69FC0
signed char checkpoint_collision_hack(zEnt*)::localstatic10 guard; // size: 0x1, address: 0x80D69FC4
float checkpoint_collision_hack(zEnt*)::yupper; // size: 0x4, address: 0x80D69FC8
signed char checkpoint_collision_hack(zEnt*)::localstatic12 guard; // size: 0x1, address: 0x80D69FCC
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D62650
// total size: 0x30
struct xMat3x3 {
    // Members
...
// total size: 0x1
struct /* @class$845zEnt_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$846zEnt_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$847zEnt_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$848zEnt_cpp */ {
    // Members
...
// Range: 0x8014AE64 -> 0x8014B68C
void zEntInit(struct zEnt * ent /* r31 */, struct xEntAsset * asset /* r30 */, unsigned int type /* r1+0x8 */) {
    // Local variables
    int num_used; // r28
    struct xAnimState * ast; // r27
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D62659
// Range: 0x8014B68C -> 0x8014B71C
void zEntSetup(struct zEnt * ent /* r31 */) {
    // References
    // -> static char __FUNCTION__[10];
}

// Range: 0x8014B71C -> 0x8014B908
void checkpoint_collision_hack(struct zEnt * ent /* r31 */) {
    // Local variables
    struct xBound & bound; // r28
    struct xVec3 & lower; // r30
    struct xVec3 & upper; // r29

    // References
    // -> float checkpoint_collision_hack(zEnt*)::xupper;
    // -> float checkpoint_collision_hack(zEnt*)::yupper;
    // -> float checkpoint_collision_hack(zEnt*)::xlower;
    // -> float checkpoint_collision_hack(zEnt*)::ylower;
    // -> signed char checkpoint_collision_hack(zEnt*)::localstatic12 guard;
    // -> signed char checkpoint_collision_hack(zEnt*)::localstatic10 guard;
    // -> signed char checkpoint_collision_hack(zEnt*)::localstatic8 guard;
    // -> signed char checkpoint_collision_hack(zEnt*)::localstatic6 guard;
    // -> unsigned int checkpoint_collision_hack(zEnt*)::model_id;
    // -> signed char checkpoint_collision_hack(zEnt*)::localstatic4 guard;
}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D62663
// Range: 0x8014B908 -> 0x8014B9F8
void zEntSave(struct zEnt * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[9];
}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D6266C
// Range: 0x8014B9F8 -> 0x8014BAE8
void zEntLoad(struct zEnt * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[9];
}

static unsigned char already_asserted; // size: 0x1, address: 0x80D69FCD
static signed char init; // size: 0x1, address: 0x80D69FCE
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D62675
// Range: 0x8014BAE8 -> 0x8014BD34
void zEntReset(struct zEnt * ent /* r31 */) {
    // Local variables
    struct xAnimState * ast; // r29
...
// Range: 0x8014BD34 -> 0x8014BD6C
void zEntUpdate(struct zEnt * ent /* r1+0x8 */, struct zScene * scene /* r1+0xC */, float elapsedSec /* r1+0x10 */) {}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D6267F
// Range: 0x8014BD6C -> 0x8014BEEC
void zEntEventAll(struct xBase * from /* r1+0x8 */, unsigned int fromEvent /* r1+0xC */, unsigned int toEvent /* r1+0x10 */, float * toParam /* r1+0x14 */) {
    // Local variables
    struct zScene * s; // r31
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D6268C
// Range: 0x8014BEEC -> 0x8014C020
void zEntEventAllOfType(struct xBase * from /* r1+0x8 */, unsigned int fromEvent /* r1+0xC */, unsigned int toEvent /* r1+0x10 */, float * toParam /* r1+0x14 */, unsigned int type /* r1+0x18 */) {
    // Local variables
    struct zScene * s; // r31
    unsigned short i; // r30
...
// Range: 0x8014C020 -> 0x8014C05C
void zEntEventAllOfType(unsigned int toEvent /* r1+0x8 */, unsigned int type /* r1+0xC */) {}

static char __FUNCTION__[21]; // size: 0x15, address: 0x8051D770
// total size: 0x14
...
// Range: 0x8014C05C -> 0x8014C4D8
struct xModelInstance * zEntRecurseModelInfo(void * info /* r1+0x8 */, struct xEnt * ent /* r28 */) {
    // Local variables
    unsigned int i; // r31
    unsigned int bufsize; // r1+0xC
    struct RpAtomic * imodel; // r29
    struct xModelInstance * tempInst[64]; // r1+0x10
    struct xModelAssetInfo * zinfo; // r27
    struct xModelAssetInst * zinst; // r30
    struct xAnimTable * table; // r24
...
// Range: 0x8014C4D8 -> 0x8014C574
void zEntParseModelInfo(struct xEnt * ent /* r31 */, unsigned int assetID /* r29 */) {
    // Local variables
    unsigned int bufsize; // r1+0xC
    void * info; // r30
    const char * cs; // r1+0x8
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x8051D785
// Range: 0x8014C574 -> 0x8014C65C
static void zEntTransitionToState(struct xAnimPlay * play /* r30 */, struct xAnimState * state /* r29 */) {
    // Local variables
...
static int been_here; // size: 0x4, address: 0x80D69FD0
static int been_here; // size: 0x4, address: 0x80D69FD4
static int been_here; // size: 0x4, address: 0x80D69FD8
static int been_here; // size: 0x4, address: 0x80D69FDC
static int been_here; // size: 0x4, address: 0x80D69FE0
static int been_here; // size: 0x4, address: 0x80D69FE4
static int been_here; // size: 0x4, address: 0x80D69FE8
static int been_here; // size: 0x4, address: 0x80D69FEC
static int been_here; // size: 0x4, address: 0x80D69FF0
static int been_here; // size: 0x4, address: 0x80D69FF4
static int been_here; // size: 0x4, address: 0x80D69FF8
static int been_here; // size: 0x4, address: 0x80D69FFC
static int been_here; // size: 0x4, address: 0x80D6A000
static int been_here; // size: 0x4, address: 0x80D6A004
static int been_here; // size: 0x4, address: 0x80D6A008
static int been_here; // size: 0x4, address: 0x80D6A00C
static int been_here; // size: 0x4, address: 0x80D6A010
static int been_here; // size: 0x4, address: 0x80D6A014
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D6269F
// Range: 0x8014C65C -> 0x8014CF1C
void zEntAnimEventCore(struct xModelInstance * model /* r18 */, struct xAnimTable * atbl /* r28 */, unsigned int animEvent /* r19 */, const float * animParam /* r30 */) {
    // Local variables
    struct xAnimPlay * play; // r29
    struct xAnimSingle * single; // r31
    int anum; // r26
    char name[12]; // r1+0x30
    struct xAnimState * ast; // r27
    char name[12]; // r1+0x24
    struct xAnimState * ast; // r20
    int anum1; // r25
    int anum2; // r24
    int anum; // r1+0x8
    char name[12]; // r1+0x18
    struct xAnimState * ast; // r22
    int anum; // r23
    float prob; // f31
    char name[12]; // r1+0xC
    struct xAnimState * ast; // r21
...
// Range: 0x8014CF1C -> 0x8014CF94
void zEntAnimEvent(struct zEnt * ent /* r31 */, unsigned int animEvent /* r29 */, const float * animParam /* r30 */) {}

unsigned int g_hash_xentanim[5]; // size: 0x14, address: 0x80D626B4
static int been_here; // size: 0x4, address: 0x80D6A018
static char __FUNCTION__[23]; // size: 0x17, address: 0x8051D7BC
struct xGlobals * xglobals; // size: 0x4, address: 0x80D63020
// Range: 0x8014CF94 -> 0x8014D878
void zEntAnimEvent_AutoAnim(struct zEnt * ent /* r31 */, unsigned int animEvent /* r17 */, const float * animParam /* r28 */) {
    // Local variables
    struct xAnimPlay * play; // r29
    struct xAnimSingle * single; // r30
    int anum; // r24
    struct xAnimTable * tab; // r20
    struct xAnimState * ast; // r27
    struct xAnimTable * tab; // r19
    struct xAnimState * ast; // r22
    int anum1; // r25
    int anum2; // r26
    int anum; // r21
    struct xAnimTable * tab; // r18
    struct xAnimState * ast; // r23
    float prob; // f31

    // References
    // -> struct xGlobals * xglobals;
    // -> static char __FUNCTION__[23];
    // -> unsigned int g_hash_xentanim[5];
    // -> static int been_here;
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D626C8
// Range: 0x8014D878 -> 0x8014D974
struct xModelAssetParam * zEntGetModelParams(unsigned int assetID /* r1+0x8 */, unsigned short * size /* r30 */) {
    // Local variables
    unsigned int bufsize; // r1+0xC
    unsigned int tempsize; // r29
    void * info; // r28
    struct xModelAssetInfo * minf; // r31

    // References
    // -> static char __FUNCTION__[19];
}

// Range: 0x8014D974 -> 0x8014D9C8
struct xModelAssetParam * zParamGetNext(struct xModelAssetParam * param /* r3 */, unsigned short * size /* r4 */) {}

// Range: 0x8014D9C8 -> 0x8014DA08
char * zParamGetString(struct xModelAssetParam * param /* r1+0x8 */, unsigned int size /* r1+0xC */, const char * tok /* r1+0x10 */, const char * def /* r1+0x14 */) {}

// Range: 0x8014DA08 -> 0x8014DAA4
char * zParamGetString(struct xModelAssetParam * param /* r31 */, unsigned int size /* r30 */, char * tok /* r1+0x8 */, char * def /* r1+0xC */) {
    // Local variables
    unsigned int testhash; // r29
}

// Range: 0x8014DAA4 -> 0x8014DAE4
int zParamGetInt(struct xModelAssetParam * param /* r1+0x8 */, unsigned int size /* r1+0xC */, const char * tok /* r1+0x10 */, int def /* r1+0x14 */) {}

// Range: 0x8014DAE4 -> 0x8014DB6C
int zParamGetInt(struct xModelAssetParam * param /* r1+0x8 */, unsigned int size /* r1+0xC */, char * tok /* r1+0x10 */, int def /* r1+0x14 */) {
    // Local variables
    char * str; // r31
}

// Range: 0x8014DB6C -> 0x8014DBAC
float zParamGetFloat(struct xModelAssetParam * param /* r1+0x8 */, unsigned int size /* r1+0xC */, const char * tok /* r1+0x10 */, float def /* r1+0x14 */) {}

// Range: 0x8014DBAC -> 0x8014DC24
float zParamGetFloat(struct xModelAssetParam * param /* r1+0x8 */, unsigned int size /* r1+0xC */, char * tok /* r1+0x10 */, float def /* r1+0x14 */) {
    // Local variables
    char * str; // r31
}
...
// Range: 0x8014DC24 -> 0x8014DEA4
void zEntGetShadowParams(struct xEnt * ent /* r31 */, struct xVec3 * center /* r26 */, struct xVec3 * radius /* r29 */, float * strength /* r25 */, enum radius_enum rtype /* r24 */) {
    // Local variables
    class zNPCBaseIN2 * npc; // r23
    struct xBox * bbox; // r30
    float cacheFudgeFactor; // f29
    float rad; // f28
    float r; // f31
    struct xVec3 * lower; // r28
...
// Range: 0x8014DEA4 -> 0x8014E1C0
void zEntGetShadowParams(struct xEnt * ent /* r30 */, struct xVec3 * center /* r26 */, float * radius /* r29 */, float * strength /* r25 */, enum radius_enum rtype /* r24 */) {
    // Local variables
    class zNPCBaseIN2 * npc; // r23
    struct xBox * bbox; // r31
    float cacheFudgeFactor; // f28
    float rad; // f27
    float r; // f31
    struct xVec3 * lower; // r28
    struct xVec3 * upper; // r27
...
// Range: 0x8014E1C0 -> 0x8014E3BC
void zEntSetOrientation(const struct xMat4x3 & mat /* r30 */, struct RwMatrixTag * modelmat /* r31 */, unsigned char use_world_up /* r1+0x8 */) {
    // Local variables
    float right_len; // f31
    float up_len; // f30
    float at_len; // f29
    struct xVec3 _up_; // r1+0x24
    struct xVec3 _at_; // r1+0x18
    struct xVec3 _right_; // r1+0xC
}

int zEventLogBufInit; // size: 0x4, address: 0x80D69038
char zEventLogBuf[20][256]; // size: 0x1400, address: 0x80B60318
// Range: 0x8014E3BC -> 0x8014E620
void zEventDebugLog(struct xBase * from /* r30 */, unsigned int fromEvent /* r21 */, struct xBase * to /* r29 */, unsigned int toEvent /* r1+0x8 */) {
    // Local variables
    int i; // r31
    const char * f; // r28
    const char * t; // r27
    ...
    // -> static int g_dbdisp_stdlog;
    // -> int zEventLogBufInit;
}

// Range: 0x8014E620 -> 0x8014E678
void zEventDebugModeCB() {
    // Local variables
    int i; // r31
...
// Range: 0x8014E678 -> 0x8014E824
static void zEventDebugModeUsage() {
    // References
    // -> static int g_dbdisp_stdlog;
    // -> static int g_dbdisp_showusage;
}

// Range: 0x8014E824 -> 0x8014E828
static void zEventDebugModeKeystroke() {}
