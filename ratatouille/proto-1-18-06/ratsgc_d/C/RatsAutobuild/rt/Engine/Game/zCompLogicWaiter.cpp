/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zCompLogicWaiter.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80314264 -> 0x8031618C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static float WAITER_BOUND_RADIUS; // size: 0x4, address: 0x80D67628
static float WAITER_BOUND_MIDRADIUS; // size: 0x4, address: 0x80D6762C
static float WAITER_BOUND_YSHIFT; // size: 0x4, address: 0x80D67630
static float BIG_EPSILON; // size: 0x4, address: 0x80D67634
static float WAITER_COLLISION_REFLECTION_HORIZONTAL; // size: 0x4, address: 0x80D67638
static float WAITER_TURN_SPEED; // size: 0x4, address: 0x80D6763C
static float WAITER_SPEED; // size: 0x4, address: 0x80D67640
static float WAITER_IDLE_SPEED; // size: 0x4, address: 0x80D67644
static float showScalarVel; // size: 0x4, address: 0x80D6B910
static float KNOCKBACK_DIST; // size: 0x4, address: 0x80D67648
static unsigned int WAITER_TORSO; // size: 0x4, address: 0x80D6764C
static unsigned int WAITER_HEAD; // size: 0x4, address: 0x80D67650
static unsigned int WAITER_LHAND; // size: 0x4, address: 0x80D67654
static unsigned int WAITER_RHAND; // size: 0x4, address: 0x80D67658
// total size: 0x28
...
class zCompLogicWaiter : public zICompLogic {
    // Functions
    unsigned char SystemEvent(unsigned int to_event, const float * to_param, struct xBase * to_param_widget, unsigned int to_param_widget_id);

    unsigned char MoveStartCheck(class zICompNPCEntity * compNPCEntity);

    unsigned char MoveStopCheck(class zICompNPCEntity * compNPCEntity);

    unsigned char HitReactStartCheck(class zICompNPCEntity * compNPCEntity);

    unsigned char HitReactStopCheck(class zICompNPCEntity * compNPCEntity);

    void Attached();

    void AllAttached();

    void knockback(struct xVec3 * knockback);

    void Update(float dt);

    unsigned char operator()(struct xEnt & ent);

    void SphereCollision(const struct xVec3 * pNewPosition, const struct xVec3 * pVelocity);
    ...
    unsigned char isAnimStopped() const;
...
// Range: 0x80314264 -> 0x803143D8
void zCompLogicWaiter::AddStaticTweaks() {
    // Local variables
    char * prefix; // r31

    // References
    // -> static float WAITER_BOUND_MIDRADIUS;
    // -> static unsigned int WAITER_RHAND;
    // -> static unsigned int WAITER_LHAND;
    // -> static unsigned int WAITER_HEAD;
    // -> static unsigned int WAITER_TORSO;
    // -> static float WAITER_TURN_SPEED;
    // -> static float WAITER_SPEED;
    // -> static float WAITER_IDLE_SPEED;
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6765C
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// total size: 0x10
struct /* @class$1933zCompLogicWaiter_cpp */ {
    // Members
...
// Range: 0x803143D8 -> 0x8031455C
// this: r31
unsigned char zCompLogicWaiter::SystemEvent(unsigned int to_event /* r1+0x8 */, const float * to_param /* r28 */, struct xBase * to_param_widget /* r30 */, unsigned int to_param_widget_id /* r1+0xC */) {
    // Local variables
    class zICompNPCEntity * compNPCEntity; // r29
    char name[20]; // r1+0x10
...
// Range: 0x8031455C -> 0x803146B8
void zCompLogicWaiter::CreateAnimTable(struct xAnimTable * table /* r31 */) {}
...
// Range: 0x803146B8 -> 0x80314708
// this: r3
unsigned char zCompLogicWaiter::MoveStartCheck(class zICompNPCEntity * compNPCEntity /* r4 */) {
    // References
    // -> static float WAITER_IDLE_SPEED;
}

// Range: 0x80314708 -> 0x803147CC
// this: r3
unsigned char zCompLogicWaiter::MoveStopCheck(class zICompNPCEntity * compNPCEntity /* r4 */) {
    // References
    // -> static float WAITER_IDLE_SPEED;
}

// Range: 0x803147CC -> 0x80314818
// this: r3
unsigned char zCompLogicWaiter::HitReactStartCheck(class zICompNPCEntity * compNPCEntity /* r4 */) {}

// Range: 0x80314818 -> 0x80314888
// this: r1+0x8
unsigned char zCompLogicWaiter::HitReactStopCheck(class zICompNPCEntity * compNPCEntity /* r30 */) {}
...
// total size: 0x68
class zNPCWaiter : public zNPCBase {};
// total size: 0x78
struct zNPCAssetWaiter : public zNPCAssetRATS {
    // Members
    unsigned int firstMovePoint; // offset 0x74, size 0x4
};
// Range: 0x80314888 -> 0x80314974
// this: r31
void zCompLogicWaiter::Attached() {
    // Local variables
    class zNPCWaiter * npcWaiter; // r29
    struct zNPCAssetWaiter * npcassWaiter; // r30

    // References
    // -> struct xVec3 m_Null;
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D67668
// Range: 0x80314974 -> 0x80314A68
// this: r30
void zCompLogicWaiter::AllAttached() {
    // Local variables
...
// Range: 0x80314A68 -> 0x80314AD8
// this: r31
void zCompLogicWaiter::knockback(struct xVec3 * knockback /* r1+0x8 */) {}

// Range: 0x80314AD8 -> 0x80314F58
// this: r31
void zCompLogicWaiter::Update(float dt /* f30 */) {
    // Local variables
    class zICompNPCEntity * compNPCEntity; // r30
    struct xEntFrame * frame; // r29
    struct xVec3 toDest; // r1+0x30
    struct xVec3 newVelocity; // r1+0x24
    struct xVec3 newPos; // r1+0x18
    struct xVec3 tempVelocity; // r1+0xC
    unsigned int animID; // r28
    const struct xVec3 * curPos; // r27
    float mpArriveDistance; // f31
    struct xMovePoint * nextPatrolPoint; // r1+0x8

    // References
    // -> static float WAITER_BOUND_RADIUS;
    // -> static float showScalarVel;
    // -> static float WAITER_SPEED;
    // -> static float WAITER_TURN_SPEED;
    // -> struct xVec3 m_Null;
}
...
// total size: 0x4
struct /* @class$915zCompLogicWaiter_cpp */ {
    // Members
...
union /* @class$1054zCompLogicWaiter_cpp */ {
...
// total size: 0xC
struct /* @class$834zCompLogicWaiter_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$835zCompLogicWaiter_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$836zCompLogicWaiter_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$837zCompLogicWaiter_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$838zCompLogicWaiter_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$839zCompLogicWaiter_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$840zCompLogicWaiter_cpp */ {
    // Members
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D67674
// Range: 0x80314F58 -> 0x80315178
// this: r31
unsigned char zCompLogicWaiter::operator()(struct xEnt & ent /* r29 */) {
    // Local variables
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D67679
unsigned int zCOLLISION_NPC; // size: 0x4, address: 0x80D70E5C
unsigned int zCOLLISION_NEEDS_DEPEN_OUTER_BOUND; // size: 0x4, address: 0x80D70E60
unsigned int zCOLLISION_NEEDS_REFLECTION; // size: 0x4, address: 0x80D70E64
// total size: 0x1C
...
// Range: 0x80315178 -> 0x80315B54
// this: r31
void zCompLogicWaiter::SphereCollision(const struct xVec3 * pNewPosition /* r1+0x8 */, const struct xVec3 * pVelocity /* r1+0xC */) {
    // Local variables
    class zICompNPCEntity * compNPCEntity; // r29
    struct xEntFrame * frame; // r24
    struct xSphere collisionSphere[5]; // r1+0x13C
    int i; // r30
    struct xSphere * extra_spheres[5]; // r1+0xF0
    int num_spheres; // r1+0x30
    struct xSweptSphere sws; // r1+0x18C
    struct xVec3 deltaPos; // r1+0xE4
    struct xVec3 testNewPos; // r1+0xD8
    struct xVec3 testOldPos; // r1+0xCC
    int j; // r27
    struct xVec3 myPos; // r1+0xC0
    struct xVec3 theirPos; // r1+0xB4
    struct xVec3 knockback; // r1+0xA8
    struct xVec3 delta; // r1+0x9C
    struct zCombatDamageInfo combatDamageInfo; // r1+0x104
    unsigned int numGroundCollisions; // r23
    struct xVec3 averageGroundNormal; // r1+0x90
    struct xEnt * pEntity; // r28
    class zICompNPCEntity * npcEnt; // r22
    struct xVec3 myPos; // r1+0x84
    struct xVec3 theirPos; // r1+0x78
    struct xVec3 knockback; // r1+0x6C
    struct xVec3 delta; // r1+0x60
    class zNPCBase * npcOwn; // r1+0x2C
    class zICompLogic * logicComponent; // r1+0x28
    class zCompLogicWaiter * waiterLogicComponent; // r1+0x24
    struct zEntSimpleObj * pSimpleObj; // r21
    float hitParams[4]; // r1+0x50
    struct zPlatform * pPlatform; // r20
    float hitParams[4]; // r1+0x40
    struct xVec3 downVec; // r1+0x34
    float collisionDownDist; // f30
    unsigned char isLowCollision; // r19
    unsigned char isHighCollision; // r1+0x12
    float normDotUp; // f31
    float angleFromFlat; // f26
    unsigned char isSteepSurface; // r25
    unsigned char drawLine; // r1+0x11
    float normDot; // f25
    float downDist; // f24
    float distFromMidShockPos; // f23

    // References
    // -> unsigned int zCOLLISION_NEEDS_DEPEN_OUTER_BOUND;
    // -> static float WAITER_BOUND_YSHIFT;
    // -> struct xColor_tag g_GREEN;
    // -> unsigned int zCOLLISION_NEEDS_REFLECTION;
    // -> struct xColor_tag g_RED;
    // -> struct xVec3 g_Y3;
    // -> static float BIG_EPSILON;
    // -> static float WAITER_BOUND_RADIUS;
    // -> static char __FUNCTION__[16];
    // -> unsigned int zCOLLISION_NPC;
...
// total size: 0x890
struct /* @class$2176zCompLogicWaiter_cpp */ {
    // Static members
...
// Range: 0x80315B54 -> 0x80315E5C
// this: r31
void zCompLogicWaiter::ApplyCollisionRebounds(struct xVec3 * pVelocity /* r30 */, struct xEntFrame * frame /* r28 */) {
    // Local variables
    float collisionReflectionHorizontal; // f29
    int i; // r29
    float normDot; // f30
    struct xVec3 initialVel; // r1+0x24
    float vehicleWallAngle; // f23
    float VEHICLE_FRONT_ANGLE_RANGE; // f22
    float forwardSpeed; // f21
    ...
    float velocityDeltaSize; // f28
    float SIDE_KNOCK_MIN_VEL; // f27
    float SIDE_KNOCK_MAX_VEL; // f18
    float volumeFactor; // f31

    // References
    // -> static float WAITER_COLLISION_REFLECTION_HORIZONTAL;
}

// Range: 0x80315E5C -> 0x80315FC4
// this: r31
void zCompLogicWaiter::DepenetrateCollisionList(struct xVec3 * pPosition /* r29 */) {
    // Local variables
    int i; // r30
...
// total size: 0xC
struct /* @class$1053zCompLogicWaiter_cpp */ {
    // Members
...
// Range: 0x80315FC4 -> 0x803160FC
unsigned char zCompLogicWaiterTurnToFace(struct xEntFrame * frame /* r31 */, const struct xVec3 * target /* r30 */, float speedLimit /* r1+0x8 */, float dt /* r1+0xC */) {
    // Local variables
...
// Range: 0x803160FC -> 0x8031618C
// this: r29
unsigned char zCompLogicWaiter::isAnimStopped() const {
    // Local variables
    class zICompNPCEntity * compNPCEntity; // r31
