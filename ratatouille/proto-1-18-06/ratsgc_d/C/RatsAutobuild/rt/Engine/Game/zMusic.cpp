/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zMusic.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80185304 -> 0x80186828
*/
enum xSndHandle {
};
// total size: 0x24
...
static struct zMusicTrackInfo sMusicTrack[2]; // size: 0x48, address: 0x80D00F40
// total size: 0x8
struct xBaseAsset {
    // Members
...
static struct xSndGroup * spMusicGroup; // size: 0x4, address: 0x80D6A630
// total size: 0x8
struct zMusicState {
    // Members
    unsigned int uMusicDefault; // offset 0x0, size 0x4
    unsigned char bPaused; // offset 0x4, size 0x1
};
static struct zMusicState sMusic; // size: 0x8, address: 0x80D6A634
float gDialogueMusicLevel; // size: 0x4, address: 0x80D63140
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D63144
enum iSndGroupHandle {
};
// Range: 0x80185304 -> 0x8018540C
static void _zMusicStart(unsigned int uWhichSlot /* r31 */, unsigned int uTune /* r28 */) {
    // Local variables
    int uPlayFlags; // r30
    enum iSndGroupHandle hSoundGroup; // r29

    // References
    // -> static struct zMusicTrackInfo sMusicTrack[2];
    // -> static struct xSndGroup * spMusicGroup;
    // -> static char __FUNCTION__[13];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D63151
// Range: 0x8018540C -> 0x801854C8
static void _zMusicPause(unsigned int uWhichSlot /* r31 */, unsigned char bPause /* r1+0x8 */) {
    // References
    // -> static struct zMusicTrackInfo sMusicTrack[2];
    // -> static char __FUNCTION__[13];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6315E
// Range: 0x801854C8 -> 0x80185594
static void _zMusicStop(unsigned int uWhichSlot /* r31 */) {
    // References
    // -> static struct zMusicTrackInfo sMusicTrack[2];
...
// Range: 0x80185594 -> 0x801856D0
void zMusicInit() {
    // Local variables
    int i; // r31
    enum iSndGroupHandle hMusicDefGroup; // r30
    unsigned char uSoundCount; // r29
    unsigned char randSound; // r28

    // References
    // -> float gDialogueMusicLevel;
    // -> static struct zMusicState sMusic;
    // -> struct xGlobals * xglobals;
    // -> static struct xSndGroup * spMusicGroup;
    // -> static struct zMusicTrackInfo sMusicTrack[2];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D6316A
enum eGameMode {
    eGameMode_Boot = 0,
    eGameMode_Intro = 1,
    eGameMode_Title = 2,
...
// Range: 0x801856D0 -> 0x80185CF0
void zMusicUpdate() {
    // Local variables
    float fNowTime; // f30
    unsigned int i; // r31
    float fDeltaTime; // f28
    float fPercentTime; // f31
    float fHowLongPlaying; // f27
    unsigned int nOtherTrack; // r30
...
// Range: 0x80185D3C -> 0x80185D7C
void zMusicKill() {
    // Local variables
    unsigned int i; // r31
}

// Range: 0x80185D7C -> 0x80185DDC
void zMusicPause() {
    // Local variables
    unsigned int i; // r31
...
// Range: 0x80185E3C -> 0x80185EA8
void zMusicSetVolume(float vol /* r1+0x8 */, float delay /* r1+0xC */) {
    // References
    // -> static struct zMusicTrackInfo sMusicTrack[2];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D63177
enum eTrackReason {
    eMusic = 0,
    eNewMusic = 1,
    eTempMusic = 2,
};
// Range: 0x80185EA8 -> 0x80186284
static unsigned int _zMusicCrossFade(float fNewVolume /* f30 */, float fNewVolTime /* f31 */, unsigned int newTrack /* r27 */, enum eTrackReason eReason /* r28 */) {
    // Local variables
    unsigned int i; // r30
    unsigned int nFadeInSlot; // r31
    unsigned int j; // r29
...
// total size: 0x1
struct ZeroVolumeHack {};
// Range: 0x80186284 -> 0x80186338
void ZeroVolumeHack::StopZeroVolume(unsigned int slot /* r31 */) {
    // References
    // -> static struct zMusicTrackInfo sMusicTrack[2];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63188
// Range: 0x80186338 -> 0x80186578
void zMusicNewSongEvent(const float * toParam /* r30 */, unsigned int toParamWidgetID /* r29 */) {
    // Local variables
    float fFadeVolTime; // f31
    float fNewVolTime; // f29
    float fFadeVolume; // f30
    float fNewVolume; // f28
    unsigned int i; // r31
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D6319B
// Range: 0x801865A0 -> 0x801867B0
void zMusicTempSongEvent(const float * toParam /* r30 */, unsigned int toParamWidgetID /* r28 */) {
    // Local variables
    float fFadeVolume; // f31
    float fFadeVolTime; // f30
    float fNewVolTime; // f29
    float fPlayTime; // f28
    float fNewVolume; // f27
    unsigned int i; // r31
    unsigned int nSlot; // r29
