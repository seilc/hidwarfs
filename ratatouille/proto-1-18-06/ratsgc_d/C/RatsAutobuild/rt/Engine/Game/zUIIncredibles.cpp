/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zUIIncredibles.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80221ED8 -> 0x80223374
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static unsigned char unlockedMovies[53]; // size: 0x35, address: 0x80D18408
char sceneBaseName[32]; // size: 0x20, address: 0x80D1843D
char sceneBaseDescription[256]; // size: 0x100, address: 0x80D1845D
static const char * SCENE_STATUS_BONUS; // size: 0x4, address: 0x80D6AF10
static unsigned char unlockCheatOn; // size: 0x1, address: 0x80D6AF14
static unsigned char sceneUnlockAll; // size: 0x1, address: 0x80D6AF15
int numScenes; // size: 0x4, address: 0x80D6B3DC
// total size: 0x20
struct xLinkAsset {
    // Members
...
// Range: 0x80221ED8 -> 0x80222038
static unsigned int GetSceneNumber(const char * str /* r27 */, unsigned int size /* r26 */, unsigned char * isBM01 /* r21 */) {
    // Local variables
    unsigned int sceneCurrent; // r25
    unsigned char isNumber; // r24
    unsigned int i; // r29
    unsigned int sceneNumber; // r31
    unsigned int i; // r28
    unsigned int id; // r20
    struct xBase * base; // r30
    class zUIController * controller; // r23
...
// total size: 0x4
struct /* @class$901zUIIncredibles_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1039zUIIncredibles_cpp */ {
    // Members
...
union /* @class$1040zUIIncredibles_cpp */ {
...
// total size: 0x10
struct /* @class$1288zUIIncredibles_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// total size: 0x890
struct /* @class$1785zUIIncredibles_cpp */ {
    // Static members
...
struct Scene {
    // Members
    const char * levelName; // offset 0x0, size 0x4
    const char * levelDescription; // offset 0x4, size 0x4
    unsigned short levelNameLength; // offset 0x8, size 0x2
    unsigned short levelDescriptionLength; // offset 0xA, size 0x2
    unsigned int imageID; // offset 0xC, size 0x4
    unsigned int expBonus; // offset 0x10, size 0x4
    int goalOffenseMrI; // offset 0x14, size 0x4
    int goalOffenseFrozone; // offset 0x18, size 0x4
    int goalDefenseMrI; // offset 0x1C, size 0x4
    int goalDefenseFrozone; // offset 0x20, size 0x4
    unsigned int flags; // offset 0x24, size 0x4
    unsigned char bonusCount; // offset 0x28, size 0x1
    char levelCode[5]; // offset 0x29, size 0x5
};
// Range: 0x80222038 -> 0x80222178
static void ParseTagSceneName(struct jot & j /* r31 */, const struct split_tag & ti /* r30 */) {
    // Local variables
    unsigned int sceneNumber; // r28
    char full[64]; // r1+0x14
    char sceneStr[8]; // r1+0xC
    unsigned int len; // r1+0x8
    struct Scene * sceneCur; // r29

    // References
    // -> char sceneBaseName[32];
...
// total size: 0x2
struct /* @class$1811zUIIncredibles_cpp */ {
    // Members
...
// Range: 0x80222178 -> 0x80222240
static void ParseTagSceneDescription(struct jot & j /* r30 */, const struct split_tag & ti /* r31 */) {
    // Local variables
    unsigned int sceneNumber; // r28
    struct Scene * sceneCur; // r29
}

static char buffer[48]; // size: 0x30, address: 0x80D1855D
// Range: 0x80222240 -> 0x80222340
static void ParseTagSceneStatus(struct jot & j /* r31 */, const struct split_tag & ti /* r30 */) {
    // Local variables
    unsigned char isBM01; // r1+0x8
    unsigned int sceneNumber; // r27
    struct Scene * sceneCur; // r29
    unsigned int bonusCount; // r28

    // References
    // -> static char buffer[48];
    // -> static const char * SCENE_STATUS_BONUS;
}

unsigned int loadSceneID; // size: 0x4, address: 0x80D6B410
// Range: 0x80222340 -> 0x802223BC
static void ParseTagLoadingSceneName(struct jot & j /* r31 */) {
    // Local variables
...
// Range: 0x802223BC -> 0x80222408
static void ParseTagIgnore(struct jot & j /* r31 */) {}

// Range: 0x80222408 -> 0x8022247C
static void ParseTagSpace(struct jot & j /* r31 */) {}

static struct tag_type newTags[7]; // size: 0x8C, address: 0x80523748
static unsigned int SCENEFLAGS_UNLOCKED; // size: 0x4, address: 0x80D6F200
// total size: 0x14
struct SceneUnlock {
    // Members
    unsigned int sceneID; // offset 0x0, size 0x4
    unsigned int sceneID_Unlock[4]; // offset 0x4, size 0x10
};
// Range: 0x8022247C -> 0x80222540
static void zUIUnlockRelatedStages(unsigned int sceneID /* r1+0x8 */) {
    // Local variables
    int i; // r30
    struct SceneUnlock * scene; // r31
    int j; // r29
    struct Scene * sceneToUnlock; // r28

    // References
    // -> int numScenes;
    // -> static unsigned int SCENEFLAGS_UNLOCKED;
}

// Range: 0x80222540 -> 0x8022258C
unsigned char zUIIncrediblesIsSceneCompleted(int idx /* r1+0x8 */) {}

// Range: 0x8022258C -> 0x802225D8
unsigned char zUIIncrediblesIsSceneUnlocked(int idx /* r1+0x8 */) {}

static char __FUNCTION__[27]; // size: 0x1B, address: 0x805237D4
static unsigned int SCENEFLAGS_COMPLETED; // size: 0x4, address: 0x80D6F204
static unsigned int SCENEFLAGS_WAS_COMPLETED; // size: 0x4, address: 0x80D6F208
// total size: 0x1C
...
int latestScene; // size: 0x4, address: 0x80D6B3E8
// Range: 0x802225D8 -> 0x80222740
unsigned char zUIIncrediblesAdvanceStage() {
    // Local variables
    unsigned char sceneWasCompleted; // r29
    int i; // r30
    struct Scene * scene; // r31
    unsigned int sceneID; // r28

    // References
    // -> static char __FUNCTION__[27];
    // -> int numScenes;
    // -> int latestScene;
    // -> static unsigned int SCENEFLAGS_WAS_COMPLETED;
    // -> static unsigned int SCENEFLAGS_COMPLETED;
    // -> struct zGlobals globals;
}

// Range: 0x80222740 -> 0x802227E0
unsigned char zUIIncrediblesAllGoalsCompleted(unsigned int i /* r1+0x8 */) {
    // Local variables
    struct Scene * scene; // r30
    int j; // r31
}

static char __FUNCTION__[32]; // size: 0x20, address: 0x805237EF
// Range: 0x802227E0 -> 0x8022289C
unsigned char zUIIncrediblesAllGoalsCompleted() {
    // Local variables
    int i; // r31
...
static char __FUNCTION__[38]; // size: 0x26, address: 0x8052380F
// Range: 0x8022289C -> 0x802229B8
int zUIIncrediblesGoalsCompletedInPercent() {
    // Local variables
    int pointsPerGoal; // r26
    int totalGoals; // r29
    int totalPoints; // r30
    int i; // r28
    struct Scene * scene; // r27
    int j; // r31
...
static char __FUNCTION__[39]; // size: 0x27, address: 0x80523835
// Range: 0x802229B8 -> 0x80222A88
int zUIIncrediblesScenesCompletedInPercent() {
    // Local variables
    int pointsPerScene; // r29
    int totalPoints; // r30
    int i; // r31
...
static char __FUNCTION__[33]; // size: 0x21, address: 0x8052385C
// Range: 0x80222A88 -> 0x80222B44
unsigned char zUIIncrediblesAllScenesCompleted() {
    // Local variables
    int i; // r31
...
// Range: 0x80222B44 -> 0x80222B8C
unsigned char zUIIncrediblesAllCompleted() {}

enum UnlockFeature {
    Unlock_Scenes = 0,
};
int numStoryScenes; // size: 0x4, address: 0x80D6B3E0
int numArenaScenes; // size: 0x4, address: 0x80D6B3E4
char * allsoaklevels[]; // size: 0x0, address: 0x80D2AB80
char sceneBaseImage[32]; // size: 0x20, address: 0x80D2AEF8
// total size: 0x8
struct xIniValue {
    // Members
...
// Range: 0x80222B8C -> 0x80222CD0
void zUIIncrediblesInitINI(struct xIniFile * ini /* r30 */) {
    // Local variables
    struct Scene * sceneMain; // r31

    // References
    // -> char sceneBaseImage[32];
    // -> char sceneBaseDescription[256];
    // -> char sceneBaseName[32];
    // -> int numScenes;
    // -> char * allsoaklevels[];
    // -> int numArenaScenes;
    // -> int numStoryScenes;
    // -> static unsigned char sceneUnlockAll;
}

// Range: 0x80222CD0 -> 0x80222D00
void zUIIncrediblesStartup() {
    // References
    // -> static struct tag_type newTags[7];
}
...
// Range: 0x80222D00 -> 0x80222DDC
void zUIIncrediblesResetGame() {
    // Local variables
    int i; // r30
    struct Scene * scene; // r29
    int i; // r31
    unsigned int i; // r28
    ...
    // -> int numScenes;
    // -> static unsigned char sceneUnlockAll;
    // -> static unsigned char unlockedMovies[53];
    // -> static unsigned char unlockCheatOn;
}

// Range: 0x80222DDC -> 0x80222E10
void zUIIncrediblesSceneSetup() {
    // References
    // -> static const char * SCENE_STATUS_BONUS;
}

// Range: 0x80222E10 -> 0x80222E30
unsigned int zUIIncrediblesGetTotalCheckpoints() {}

// Range: 0x80222E30 -> 0x80222E50
unsigned int zUIIncrediblesGetHitCheckpoints() {}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D64B00
// total size: 0xC
...
// Range: 0x80222E50 -> 0x80222EE8
void zUIIncrediblesWarp(struct _xCounter * counter /* r31 */) {
    // References
    // -> static char __FUNCTION__[19];
}

static char __FUNCTION__[25]; // size: 0x19, address: 0x8052387D
// Range: 0x80222EE8 -> 0x80223038
void zUIIncrediblesWarpDirect(unsigned int sceneIndex /* r30 */) {
    // Local variables
    struct Scene * scene; // r31
    unsigned int sceneCode; // r29
...
// Range: 0x80223038 -> 0x80223040
static unsigned char zUIIncrediblesGameIsBeaten() {}
...
static struct _zPortal portal; // size: 0x14, address: 0x80D6AF18
// total size: 0x18
...
static struct xPortalAsset portalAsset; // size: 0x18, address: 0x80D18590
unsigned int SCENE_ID_MNU_START; // size: 0x4, address: 0x80D6A5B0
unsigned char gExitAfterPause; // size: 0x1, address: 0x80D6A1C4
// Range: 0x80223040 -> 0x802231A0
void zUIIncrediblesWarp(unsigned int sceneCode /* r31 */) {
    // References
...
// Range: 0x802231A0 -> 0x802231C8
void zUIIncrediblesHitCheckPoint() {}

// Range: 0x802231C8 -> 0x802231F0
void zUIIncrediblesPlayedMovie(int index /* r3 */) {
    // References
    // -> static unsigned char unlockedMovies[53];
}

static char __FUNCTION__[24]; // size: 0x18, address: 0x80523896
// Range: 0x802231F0 -> 0x802232C0
void zUIIncrediblesOpenBonus(int index /* r31 */) {
    // Local variables
    struct Scene * scene; // r30
...
// Range: 0x802232C0 -> 0x802232D4
void zUIIncrediblesSave(struct xSerial * serial /* r3 */) {
    // Local variables
    int i; // r29
    struct Scene * scene; // r28
    int i; // r30
    unsigned char b; // r23
    unsigned int TOTAL_BONUS_CONST; // r22
    unsigned int INDEX_COUNT; // r25
    int i; // r27
    int i; // r31
    int i; // r26
}
...
static enum ePauseType pause_type; // size: 0x4, address: 0x80D6AF2C
// Range: 0x802232D4 -> 0x802232DC
enum ePauseType zUIIncrediblesGetPauseType() {
    // References
    // -> static enum ePauseType pause_type;
}

// Range: 0x802232DC -> 0x802232E4
void zUIIncrediblesSetPauseType(enum ePauseType type /* r3 */) {
    // References
    // -> static enum ePauseType pause_type;
}

// Range: 0x802232E4 -> 0x802232F8
void zUIIncrediblesLoad(struct xSerial * serial /* r3 */) {
    // Local variables
    int i; // r29
    struct Scene * scene; // r27
    int i; // r30
    unsigned char b; // r24
    unsigned int TOTAL_BONUS_CONST; // r23
    unsigned int INDEX_COUNT; // r26
    int i; // r28
    int i; // r31
    int val; // r25
}

// Range: 0x802232F8 -> 0x8022332C
unsigned int zUIIncrediblesGetGameProgress() {}

// Range: 0x8022332C -> 0x80223374
unsigned int zUIIncrediblesGetAutoSaveStage() {
    // References
    // -> struct zGlobals globals;
}
