/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zParCmd.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8018BD20 -> 0x8018D10C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static class zVolume * sClipVolume[32]; // size: 0x80, address: 0x80D011B8
static int sClipVolumeTotal; // size: 0x4, address: 0x80D6A698
// Range: 0x8018BD20 -> 0x8018BDE0
void zParCmdInit() {}

// Range: 0x8018BDE0 -> 0x8018BE84
void zParCmdFindClipVolumes() {
    // Local variables
    char findname[64]; // r1+0x8
    class zVolume * vol; // r31

    // References
    // -> static int sClipVolumeTotal;
    // -> static class zVolume * sClipVolume[32];
}

static char __FUNCTION__[27]; // size: 0x1B, address: 0x8051F760
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x8018BE84 -> 0x8018C014
void xParCmdKillDistance_Update(struct xParCmd * c /* r27 */, struct xParGroup * ps /* r28 */) {
    // Local variables
    struct xPar * p; // r31
    struct xParCmdKillDistance * cmd; // r30
    struct xVec3 * camera_pos; // r29
...
static char __FUNCTION__[26]; // size: 0x1A, address: 0x8051F77B
// total size: 0x18
...
// Range: 0x8018C014 -> 0x8018C1D8
void xParCmdClipVolumes_Update(struct xParCmd * c /* r26 */, struct xParGroup * ps /* r28 */) {
    // Local variables
    struct xPar * p; // r31
    struct xParCmdAlpha3rdPolyReg * cmd; // r27
    int i; // r30
    class zVolume * vol; // r29
    struct xCollis collis; // r1+0x8
...
// Range: 0x8018C1D8 -> 0x8018C1DC
void xParCmdPlayerCollision_Update() {}

static char __FUNCTION__[30]; // size: 0x1E, address: 0x8051F795
...
// Range: 0x8018C1DC -> 0x8018C42C
void xParCmdAnimalMagentism_Update(struct xParCmd * c /* r28 */, struct xParGroup * ps /* r29 */, float dt /* r1+0x8 */) {
    // Local variables
    struct xPar * p; // r31
    struct xParCmdAnimalMagentism * cmd; // r30
    struct xVec3 pos; // r1+0x18
    float mul; // f30
    struct xVec3 vec; // r1+0xC
    float dist; // f31
...
static struct xCollis collis; // size: 0x54, address: 0x80D01238
// total size: 0x20
struct xQCData {
    // Members
...
static struct xBound bnd_fake; // size: 0x4C, address: 0x8051F7B4
static char __FUNCTION__[27]; // size: 0x1B, address: 0x8051F800
...
// Range: 0x8018C42C -> 0x8018C740
void xParCmdDamagePlayer_Update(struct xParCmd * c /* r23 */, struct xParGroup * ps /* r26 */) {
    // Local variables
    struct xPar * p; // r31
    struct xParCmdDamagePlayer * cmd; // r30
    unsigned long il; // r25
    unsigned long ir; // r24
    struct xBound * pbound; // r28
    int last_idx; // r27
    int hithim; // r1+0x10
    int i; // r29
    ...
    // -> struct xVec3 m_Null;
    // -> struct xGlobals * xglobals;
    // -> static struct xCollis collis;
    // -> static struct xBound bnd_fake;
    // -> static char __FUNCTION__[27];
}
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D632A8
// total size: 0x2C
...
// Range: 0x8018C740 -> 0x8018CA04
void xParCmdJet_Update(struct xParCmd * c /* r28 */, struct xParGroup * ps /* r29 */, float dt /* r1+0x8 */) {
    // Local variables
    struct xPar * p; // r30
    struct xParCmdJet * cmd; // r31
    float mdt; // f27
...
// Range: 0x8018CA04 -> 0x8018CB6C
void xParCmdCustom_Grass_Update(struct xParGroup * ps /* r30 */, float dt /* f29 */) {
    // Local variables
    struct xPar * p; // r31
    float radSqr; // f31
...
static char __FUNCTION__[26]; // size: 0x1A, address: 0x8051F81B
// total size: 0x14
struct xParCmdApplyCamMat : public xParCmdAsset {
    // Members
    struct xVec3 apply; // offset 0x8, size 0xC
};
// Range: 0x8018CB6C -> 0x8018CDA0
void xParCmdApplyCamMat_Update(struct xParCmd * c /* r27 */, struct xParGroup * ps /* r28 */, float dt /* f30 */) {
    // Local variables
    struct xPar * p; // r31
    struct xParCmdApplyCamMat * cmd; // r29
...
float STANK_SLOW_RATE; // size: 0x4, address: 0x80D632BC
static char __FUNCTION__[21]; // size: 0x15, address: 0x8051F835
// total size: 0x1C
...
// Range: 0x8018CDA0 -> 0x8018D10C
void xParCmdCustom_Update(struct xParCmd * c /* r28 */, struct xParGroup * ps /* r29 */, float dt /* f24 */) {
    // Local variables
    struct xPar * p; // r31
    struct xParCmdCustom * cmd; // r30
    float mdt; // f28
    float radSqr; // f27
    float epsilon; // f26
    struct xVec3 center; // r1+0x14
