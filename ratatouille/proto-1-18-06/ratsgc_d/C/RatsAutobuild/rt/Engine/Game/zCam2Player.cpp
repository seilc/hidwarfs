/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zCam2Player.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80289D10 -> 0x8028BF54
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
float enable; // size: 0x4, address: 0x80D65CF8
float mountY; // size: 0x4, address: 0x80D65CFC
float thetaRestore; // size: 0x4, address: 0x80D65D00
float phiRestore; // size: 0x4, address: 0x80D65D04
float distRestore; // size: 0x4, address: 0x80D65D08
float maxspeed_theta; // size: 0x4, address: 0x80D65D0C
float maxspeed_yaw; // size: 0x4, address: 0x80D65D10
float maxspeed_dist; // size: 0x4, address: 0x80D65D14
unsigned char render; // size: 0x1, address: 0x80D6B370
unsigned char lock_enable; // size: 0x1, address: 0x80D65D18
// Range: 0x80289D10 -> 0x80289DB4
void AddTweaks(const char * prefix /* r31 */) {
    // Local variables
    char _buffer[128]; // r1+0x88
    char _buffer[128]; // r1+0x8

    // References
    // -> unsigned char lock_enable;
    // -> unsigned char render;
}

float cPlayerMarginAngleNormal; // size: 0x4, address: 0x80D65D1C
float cPlayerMarginAngleLocking; // size: 0x4, address: 0x80D65D20
float cNearWallMaxPushDistance; // size: 0x4, address: 0x80D65D24
unsigned char lock_yaw_to_theta; // size: 0x1, address: 0x80D6B371
// total size: 0x8
struct substr {
    // Members
...
// total size: 0xC
struct /* @class$768zCam2Player_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$769zCam2Player_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$770zCam2Player_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$771zCam2Player_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$772zCam2Player_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$773zCam2Player_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$774zCam2Player_cpp */ {
    // Members
...
// Range: 0x80289DB4 -> 0x80289FF8
void AddTweaks(const char * prefix /* r31 */) {
    // References
    // -> float maxspeed_dist;
    // -> float maxspeed_yaw;
    // -> float maxspeed_theta;
    // -> float mountY;
    // -> float phiRestore;
    // -> float thetaRestore;
    // -> float distRestore;
    // -> float enable;
    // -> unsigned char lock_yaw_to_theta;
    // -> struct tweak_callback cb_tweak_convert_degrees;
    // -> float cPlayerMarginAngleLocking;
    // -> float cPlayerMarginAngleNormal;
}

// total size: 0x18
struct /* @class$1940zCam2Player_cpp */ {
    // Members
...
class zCam2Player : public xCam {
    // Functions
    void set_secondary_target(const struct xVec3 & targetPos, float radius, float marginAngle);

    void set_secondary_target(const struct xEnt & ent, float radius, float marginAngle);

    void reset_springs();

    void update_springs(float dt);

    void reset_basis();

    void inherit_basis();

    void clamp_secondary();

    void update_target_info(float dt);

    void reset_target_info();

    int yaw_past_limit(float & pastLimit, float guardBandYaw);

    void clamp_players_yawoffset();

    void clamp_players_nearwall();

    void post_collide_adjust_yaw();

    void clamp_players();

    void update_precollide();

    void update_pivot();

    void update_compress();
    ...
    void start_position_aim();

    void stop();

    void update_camera_goals(float dt);
...
// Range: 0x80289FF8 -> 0x8028A01C
void zCam2Player::get_rest_default(struct rest_config & cfg /* r4 */) {}

// Range: 0x8028A01C -> 0x8028A07C
// this: r31
void zCam2Player::set_secondary_target(const struct xVec3 & targetPos /* r1+0x8 */, float radius /* r1+0xC */, float marginAngle /* r1+0x10 */) {}

// Range: 0x8028A07C -> 0x8028A094
// this: r3
void zCam2Player::set_secondary_target(const struct xEnt & ent /* r4 */, float radius /* f1 */, float marginAngle /* f2 */) {}
...
// Range: 0x8028A094 -> 0x8028A0E4
// this: r31
void zCam2Player::reset_springs() {
    // References
    // -> float mountY;
}
...
// Range: 0x8028A0E4 -> 0x8028A198
// this: r31
void zCam2Player::update_springs(float dt /* f31 */) {
    // References
    // -> float maxspeed_dist;
    // -> float maxspeed_theta;
    // -> float maxspeed_yaw;
...
// Range: 0x8028A198 -> 0x8028A328
// this: r31
void zCam2Player::reset_basis() {
    // Local variables
    float yaw; // f31
...
// Range: 0x8028A328 -> 0x8028A3B4
// this: r31
void zCam2Player::inherit_basis() {
    // Local variables
    const class zCam2Player * srcCam; // r30
}
...
// total size: 0x4
struct /* @class$901zCam2Player_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1039zCam2Player_cpp */ {
    // Members
...
union /* @class$1040zCam2Player_cpp */ {
...
// total size: 0x10
struct /* @class$1668zCam2Player_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
static int count; // size: 0x4, address: 0x80D6B374
static signed char init; // size: 0x1, address: 0x80D6B378
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x8028A3B4 -> 0x8028A788
// this: r31
void zCam2Player::clamp_secondary() {
    // Local variables
    const struct xEnt * ent; // r29
    struct xVec3 * center; // r30
    float radius; // f23
    float margin_angle; // r1+0x8
    struct xVec3 camToTarget; // r1+0xC
    float xzDistanceCam; // f22
    float arc; // f31
    float dist2; // f21
    float dx__; // f25
    float dz__; // f24
    float theta; // f30
    float yawFromOrigin; // f20
    float deltaMax; // f29
    float startYaw; // f28
    float deltaYaw; // f27
    float endYaw; // f26
...
// Range: 0x8028A788 -> 0x8028A82C
// this: r31
void zCam2Player::update_target_info(float dt /* r1+0x8 */) {}

// Range: 0x8028A82C -> 0x8028A8C8
// this: r31
void zCam2Player::reset_target_info() {}
...
// total size: 0x890
struct /* @class$1911zCam2Player_cpp */ {
    // Static members
...
// Range: 0x8028A8C8 -> 0x8028ACE0
// this: r30
int zCam2Player::yaw_past_limit(float & pastLimit /* r31 */, float guardBandYaw /* r1+0x8 */) {
    // Local variables
    struct xVec3 camPos; // r1+0x44
    float camGoalYaw; // f19
    const struct zPlayer * player0; // r29
    const struct zPlayer * player1; // r28
    struct xVec3 player0Pos; // r1+0x38
    struct xVec3 camGoalToPlayer0; // r1+0x2C
    float player0Arc; // f30
    float toPlayer0Yaw; // f18
    float dYawFromPlayer0; // f29
    struct xVec3 player1Pos; // r1+0x20
    struct xVec3 camGoalToPlayer1; // r1+0x14
    float player1Arc; // f28
    float toPlayer1Yaw; // r1+0x10
    float dYawFromPlayer1; // f27
    float dYawMax; // f24
    float dAbsYawFromPlayer0; // f22
    float dAbsYawFromPlayer1; // f21
    float adjustedDYawP0; // f26
    float adjustedDYawP1; // f25
    float dYawMostOut; // f23
    float radiusArcOfMostOut; // f20
    float dYawPastLimit; // f31
...
// Range: 0x8028ACE0 -> 0x8028AD70
static struct zPlayer * GetPlayer(int player /* r31 */) {
    // References
...
// Range: 0x8028AD70 -> 0x8028AE60
// this: r31
void zCam2Player::clamp_players_yawoffset() {
    // Local variables
    float overLimit; // r1+0xC
    float newTheta; // f31
    float overLimit; // r1+0x8
    float newYaw; // f30
...
static int i; // size: 0x4, address: 0x80D6B37C
static signed char init; // size: 0x1, address: 0x80D6B380
// Range: 0x8028AE60 -> 0x8028B0E0
// this: r31
void zCam2Player::clamp_players_nearwall() {
    // Local variables
    float camGoalYaw; // f30
    struct xVec3 camPos; // r1+0x20
    struct xVec3 atXZ; // r1+0x14
    float planarDistance; // f27
    float deltaD; // f29
    const struct xVec3 & origin; // r30
    struct xVec3 camGoalToOriginN; // r1+0x8
    float dot; // f28
    float xzDDistToOrigin; // f26
    float cosPhi; // f25
    float finalDist; // f31
    float finalDDist; // f24

    // References
    // -> static int i;
    // -> static signed char init;
    // -> float cNearWallMaxPushDistance;
}

float sNearWall; // size: 0x4, address: 0x80D6B200
// total size: 0x1
...
// Range: 0x8028B0E0 -> 0x8028B148
static float MinPlanarPlayerDistanceXZ(const struct xVec3 & camPos /* r30 */, const struct xVec3 & atXZ /* r31 */) {
    // Local variables
    float planarDistance; // r1+0x8
}

// Range: 0x8028B148 -> 0x8028B254
static void MinPlanarDistanceXZ(float & minDistance /* r28 */, int playerIndex /* r27 */, const struct xVec3 & camPos /* r30 */, const struct xVec3 & atXZ /* r29 */) {
    // Local variables
    const struct xVec3 & playerPos; // r31
    struct xVec3 camToPlayer; // r1+0x8
    float distance; // f31
...
// Range: 0x8028B254 -> 0x8028B2D4
// this: r31
void zCam2Player::post_collide_adjust_yaw() {
    // Local variables
    float guardYaw; // f31
    float overLimit; // r1+0x8
    float newYaw; // f30
...
// Range: 0x8028B2D4 -> 0x8028B368
// this: r31
void zCam2Player::clamp_players() {
    // References
    // -> struct xGlobals * xglobals;
}

// Range: 0x8028B368 -> 0x8028B3B8
// this: r31
void zCam2Player::update_precollide() {}

// Range: 0x8028B3B8 -> 0x8028B424
// this: r31
void zCam2Player::update_pivot() {}

// Range: 0x8028B424 -> 0x8028B490
// this: r31
void zCam2Player::update_compress() {}
...
// Range: 0x8028B490 -> 0x8028B4D8
// this: r31
void zCam2Player::update_phi() {
    // Local variables
    struct OOBExtent extent; // r1+0xC
    unsigned int flags; // r1+0x8
}

// Range: 0x8028B4D8 -> 0x8028B524
// this: r31
void zCam2Player::update_theta() {
    // References
    // -> unsigned char lock_yaw_to_theta;
}

// Range: 0x8028B524 -> 0x8028B528
void zCam2Player::update_distance() {}

// Range: 0x8028B528 -> 0x8028B658
// this: r31
void zCam2Player::apply_basis() {
    // References
    // -> float enable;
}

// Range: 0x8028B658 -> 0x8028B6EC
// this: r31
void zCam2Player::create() {
    // Local variables
...
// Range: 0x8028B6EC -> 0x8028B7C4
// this: r31
void zCam2Player::reset() {
    // References
    // -> struct config cfg;
    // -> unsigned char lock_enable;
}
...
// Range: 0x8028B7C4 -> 0x8028B810
// this: r31
void zCam2Player::start() {
    // Local variables
    struct rest_config cfg; // r1+0x8
}

// Range: 0x8028B810 -> 0x8028B854
// this: r31
void zCam2Player::start_position_aim() {}

// Range: 0x8028B854 -> 0x8028B87C
// this: r1+0x8
void zCam2Player::stop() {}

// Range: 0x8028B87C -> 0x8028B93C
// this: r31
void zCam2Player::update_camera_goals(float dt /* f31 */) {}

static unsigned int i; // size: 0x4, address: 0x80D6B384
static signed char init; // size: 0x1, address: 0x80D6B388
static char __FUNCTION__[7]; // size: 0x7, address: 0x80D65D28
struct xVec3 g_Y3; // size: 0xC, address: 0x80D6D1B0
// Range: 0x8028B93C -> 0x8028BE78
// this: r31
void zCam2Player::update(float dt /* f29 */) {
    // Local variables
    unsigned int savedClumpFilter; // r28
    float pastLimit; // r1+0xC
    float pastLimit; // r1+0x8
    float lockIntensity; // f28
    struct xVec3 origin; // r1+0x4C
    struct basis_traits & start; // r30
    struct basis_traits end; // r1+0x134
    struct basis_traits last_good; // r1+0x58
    struct xVec3 diffOrigin; // r1+0x40
    float lo; // f31
    struct xVec3 loLoc; // r1+0x34
    float hi; // f27
    struct xVec3 hiLoc; // r1+0x28
    struct xVec3 range; // r1+0x1C
    float rangeLen2; // f26
    int i; // r29
    float test; // f30
    struct xVec3 newOrigin; // r1+0x10
...
// Range: 0x8028BE78 -> 0x8028BF54
// this: r31
void zCam2Player::debug_render() {
    // References
    // -> unsigned char render;
}

// total size: 0x4C
struct {} zCam2Player::__vtable; // size: 0x4C, address: 0x80525308
// total size: 0x0
