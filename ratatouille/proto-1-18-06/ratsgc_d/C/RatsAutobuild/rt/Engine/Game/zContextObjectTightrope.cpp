/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zContextObjectTightrope.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x802D5B90 -> 0x802D6B38
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static float MAGNETIZE_DIST; // size: 0x4, address: 0x80D66A88
static float LOOK_AHEAD_TIME; // size: 0x4, address: 0x80D66A8C
static unsigned int LOOK_AHEAD_POINTS; // size: 0x4, address: 0x80D66A90
static float PREDICT_DIST; // size: 0x4, address: 0x80D66A94
static float PREDICT_Y_SNAP; // size: 0x4, address: 0x80D66A98
static float START_CHECK_TIME; // size: 0x4, address: 0x80D66A9C
static unsigned char drawDebugChecks; // size: 0x1, address: 0x80D6B6E8
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D66AA0
// total size: 0x1C
...
class zContextObjectTightrope : public zContextObject {
    ...
    zContextObjectTightrope(struct zContextObjectTightropeAsset * theAsset);

    void Setup();

    unsigned char Advance(float dist, float & u, unsigned char forward) const;

    unsigned char CanMount(struct zCommonPlayer * player, union MountData & mountData, float & priority) const;

    unsigned char Mount(struct zCommonPlayer * player, union MountData & mountData);

    unsigned char ClearChkby() const;

    void RestoreChkby(unsigned char prevValue) const;
...
// Range: 0x802D5B90 -> 0x802D5D74
// this: r31
void zContextObjectTightrope::Draw() const {
    // Local variables
    struct xBox box; // r1+0x14
    struct RwCamera * cam; // r29
    struct RwMatrixTag * rwmatrix; // r28
    struct xVec3 camPos; // r1+0x8
    float d1; // f28
    float d2; // f29
    float lowest; // f30
    float lodDistance; // f27
    float farClip; // f26
    float t; // f31
    int increments; // r30
...
// Range: 0x802D5D74 -> 0x802D5EA0
void zContextObjectTightrope::AddStaticTweaks() {
    // Local variables
    const char * prefix; // r31

    // References
    // -> static unsigned char drawDebugChecks;
    // -> static float PREDICT_DIST;
    // -> static unsigned int LOOK_AHEAD_POINTS;
    // -> static float LOOK_AHEAD_TIME;
    // -> static float MAGNETIZE_DIST;
}

static char __FUNCTION__[5]; // size: 0x5, address: 0x80D66AA5
// total size: 0x14
struct {} zContextObjectTightrope::__vtable; // size: 0x14, address: 0x80D66AC0
// total size: 0x10
struct xVec4 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x802D5EA0 -> 0x802D6018
// this: r31
zContextObjectTightrope::zContextObjectTightrope(struct zContextObjectTightropeAsset * theAsset /* r1+0x8 */) {
    // References
    // -> static float MAGNETIZE_DIST;
    // -> struct xGlobals * xglobals;
    // -> static char __FUNCTION__[5];
    // -> struct [anonymous] zContextObjectTightrope::__vtable;
}

static char __FUNCTION__[5]; // size: 0x5, address: 0x80D66AAA
// Range: 0x802D6024 -> 0x802D61EC
void zContextObjectTightrope::Init(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r28 */) {
    // Local variables
...
static char __FUNCTION__[6]; // size: 0x6, address: 0x80D66AAF
// Range: 0x802D61EC -> 0x802D629C
// this: r31
void zContextObjectTightrope::Setup() {
    // References
    // -> static char __FUNCTION__[6];
}

// Range: 0x802D629C -> 0x802D62FC
// this: r30
unsigned char zContextObjectTightrope::Advance(float dist /* r1+0x8 */, float & u /* r31 */, unsigned char forward /* r1+0xC */) const {}

static struct xColor_tag colors[10]; // size: 0x28, address: 0x80529070
static signed char init; // size: 0x1, address: 0x80D6B6E9
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D66AB5
...
// Range: 0x802D62FC -> 0x802D6A88
// this: r27
unsigned char zContextObjectTightrope::CanMount(struct zCommonPlayer * player /* r31 */, union MountData & mountData /* r25 */, float & priority /* r30 */) const {
    // Local variables
    const struct xVec3 & dpos; // r26
    const struct xVec3 & pos; // r29
    struct xIsect intersectData; // r1+0x50
    float dposLength2; // f26
    float curvePosU; // r1+0x1C
    struct xVec3 curvePos; // r1+0x44
    float magnetizeDistSqr; // f23
    float currDistance2; // f28
    float currPriority; // f30
    float xzVel; // f25
    float a; // r1+0x18
    float b; // r1+0x14
    unsigned char validEstimate; // r24
    float timeAbove; // f27
    float timeBelow; // f29
    float checkTime; // f31
    unsigned int i; // r28
    struct xVec3 predictPos; // r1+0x38
    float currDistance2; // f24

    // References
    // -> static char __FUNCTION__[9];
    // -> static unsigned int LOOK_AHEAD_POINTS;
    // -> static struct xColor_tag colors[10];
    ...
    // -> static unsigned char drawDebugChecks;
    // -> static float PREDICT_Y_SNAP;
    // -> static float PREDICT_DIST;
    // -> static float START_CHECK_TIME;
...
// total size: 0x4
struct /* @class$892zContextObjectTightrope_cpp */ {
    // Members
...
union /* @class$1031zContextObjectTightrope_cpp */ {
...
// total size: 0xC
struct /* @class$1030zContextObjectTightrope_cpp */ {
    // Members
...
// total size: 0x890
struct /* @class$1857zContextObjectTightrope_cpp */ {
    // Static members
...
// total size: 0x54
struct /* @class$2123zContextObjectTightrope_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$833zContextObjectTightrope_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$834zContextObjectTightrope_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$835zContextObjectTightrope_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$836zContextObjectTightrope_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$837zContextObjectTightrope_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$838zContextObjectTightrope_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$839zContextObjectTightrope_cpp */ {
    // Members
...
// total size: 0x4
struct /* @class$1548zContextObjectTightrope_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$1549zContextObjectTightrope_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// total size: 0x4
struct /* @class$2066zContextObjectTightrope_cpp */ {
    // Members
...
// total size: 0x8
struct /* @class$2065zContextObjectTightrope_cpp */ {
    // Members
    float time; // offset 0x0, size 0x4
...
class zNPCBase {};
// total size: 0x2
struct /* @class$2068zContextObjectTightrope_cpp */ {
    // Members
...
// total size: 0x10
struct /* @class$1599zContextObjectTightrope_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x802D6A88 -> 0x802D6AE4
// this: r29
unsigned char zContextObjectTightrope::Mount(struct zCommonPlayer * player /* r30 */, union MountData & mountData /* r31 */) {
    // Local variables
    struct xVec3 pos; // r1+0x8
}

// Range: 0x802D6AE4 -> 0x802D6B20
// this: r3
unsigned char zContextObjectTightrope::ClearChkby() const {
    // Local variables
    unsigned char prevVal; // r31
}

// Range: 0x802D6B20 -> 0x802D6B38
// this: r3
void zContextObjectTightrope::RestoreChkby(unsigned char prevValue /* r4 */) const {}
...
// total size: 0x14
struct {} zContextObject::__vtable; // size: 0x14, address: 0x80D66AD4
