/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zMenu.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801837F0 -> 0x80184D20
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static float time_elapsed; // size: 0x4, address: 0x80D63094
static float time_last; // size: 0x4, address: 0x80D6A600
static float time_current; // size: 0x4, address: 0x80D6A604
static float sAttractMode_timer; // size: 0x4, address: 0x80D6A608
static float sOneLiner_timer; // size: 0x4, address: 0x80D6A60C
enum xSndHandle {
};
static enum xSndHandle frozone_speaks; // size: 0x4, address: 0x80D63098
static int card; // size: 0x4, address: 0x80D6A610
static int sInMenu; // size: 0x4, address: 0x80D6A614
static float ONELINER_WAITTIME; // size: 0x4, address: 0x80D6309C
static float holdTmr; // size: 0x4, address: 0x80D630A0
static unsigned char sAllowAttract; // size: 0x1, address: 0x80D6A618
// Range: 0x801837F0 -> 0x80183814
static void zMenuDecrementIdleTimer() {
    // References
    // -> static float time_elapsed;
    // -> static float sOneLiner_timer;
    // -> static float sAttractMode_timer;
}

// Range: 0x80183814 -> 0x80183828
static void zMenuResetIdleTimer() {
    // References
    // -> static float ONELINER_WAITTIME;
    // -> static float sOneLiner_timer;
    // -> static float sAttractMode_timer;
}

// Range: 0x80183828 -> 0x80183830
int zMenuRunning() {
    // References
    // -> static int sInMenu;
}

enum eFMVFile attractMovies[5]; // size: 0x14, address: 0x80D630A4
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D630B8
// Range: 0x80183830 -> 0x80183968
void zMenuInitLoad(unsigned int theSceneID /* r1+0x8 */) {
    // Local variables
    long long t; // r30
...
// Range: 0x80183968 -> 0x801839C8
void zMenuInitSetMode() {
    // References
    // -> int sFirstBoot;
    // -> struct zGlobals globals;
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D630C6
struct xGlobals * xglobals; // size: 0x4, address: 0x80D63020
// Range: 0x801839C8 -> 0x80183B1C
void zMenuExit() {
    // Local variables
    long long t; // r29
    int i; // r31
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D630D0
...
// Range: 0x80183B1C -> 0x80183E14
void zMenuSetup() {
    // Local variables
    long long t; // r28
    int i; // r31
    struct zUITextAsset * asset; // r30
    ...
    // -> static unsigned char sAllowAttract;
}

// total size: 0x4
struct /* @class$2062zMenu_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2063zMenu_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// total size: 0x1
struct /* @class$833zMenu_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$834zMenu_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$835zMenu_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$836zMenu_cpp */ {
    // Members
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D630DB
...
// Range: 0x80183E14 -> 0x80184984
unsigned int zMenuLoop(int force_draw_black /* r1+0x8 */) {
    // Local variables
    long long t; // r22
    int ostrich_delay; // r26
    unsigned int nextSceneID; // r31
    unsigned int attractsPlayed; // r30
    unsigned int FMVreturn; // r1+0x18
    int draw_black; // r29
    int ss; // r21
    const char * fname; // r20
    unsigned int i; // r25
    ...
    // -> static float time_elapsed;
    // -> static float time_current;
    // -> static float time_last;
    // -> static float sAttractMode_timer;
    // -> enum eFMVFile attractMovies[5];
    // -> int sFirstBoot;
    // -> static float ONELINER_WAITTIME;
    // -> static float sOneLiner_timer;
    // -> static enum xSndHandle frozone_speaks;
    // -> static unsigned char sAllowAttract;
    // -> float SECS_PER_VBLANK;
    // -> struct xStats gStats;
    // -> struct xColor_tag g_CLEAR;
    // -> struct xColor_tag g_BLACK;
    // -> static float holdTmr;
}

// total size: 0x2
struct /* @class$1614zMenu_cpp */ {
    // Members
...
// Range: 0x801849E0 -> 0x80184BCC
static unsigned int zMenuUpdateMode() {
    // Local variables
    unsigned int retVal; // r31
    float elapsed1; // f31
...
// Range: 0x80184BCC -> 0x80184BD8
unsigned int zMenuGetBadCard() {
    // References
    // -> static int card;
}

// Range: 0x80184BD8 -> 0x80184C90
unsigned int zMenuFMVPlay(const char * filename /* r30 */, unsigned int buttons /* r1+0x8 */, float time /* r1+0xC */, unsigned int uSubtitlesAID /* r1+0x10 */, unsigned char skippable /* r1+0x14 */, unsigned char lockController /* r1+0x15 */, unsigned char displayDemoBlink /* r1+0x16 */) {
    // Local variables
    unsigned int rval; // r31
...
// Range: 0x80184C90 -> 0x80184C98
int zMenuIsFirstBoot() {
    // References
    // -> int sFirstBoot;
}

// Range: 0x80184C98 -> 0x80184CA0
void zMenuFirstBootSet(int val /* r3 */) {
    // References
    // -> int sFirstBoot;
}

// Range: 0x80184CA0 -> 0x80184D18
void zMenuPause(unsigned char bPause /* r1+0x8 */) {
    // References
    // -> long long sTimeLast;
    // -> float SECS_PER_VBLANK;
    // -> static float time_last;
...
// Range: 0x80184D18 -> 0x80184D20
void zMenuAllowAtract(unsigned char allowAttract /* r3 */) {
    // References
    // -> static unsigned char sAllowAttract;
}
