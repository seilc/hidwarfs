/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zMiniME.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80304CE0 -> 0x80309AE8
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static char curveCmds[1][16]; // size: 0x10, address: 0x80D67318
static char actionCmds[3][16]; // size: 0x30, address: 0x8052A8B8
static char minigameCmds[1][16]; // size: 0x10, address: 0x80D67328
// total size: 0x8
struct /* @class$1618zMiniME_cpp */ {
    // Members
...
struct /* @class$1618zMiniME_cpp */ {
    ...
    char * string; // offset 0x0, size 0x4
    unsigned int value; // offset 0x4, size 0x4
} inputData[9]; // size: 0x48, address: 0x8052A8E8
// total size: 0x4
struct node {
    // Members
...
// total size: 0x1
struct /* @class$895zMiniME_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$896zMiniME_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$897zMiniME_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$898zMiniME_cpp */ {
    // Members
...
static struct xResponseCurve alphaCurve; // size: 0x14, address: 0x80D6B858
// total size: 0x8
struct /* @class$1628zMiniME_cpp */ {
    // Members
...
struct /* @class$1628zMiniME_cpp */ {
    ...
    unsigned int hash; // offset 0x0, size 0x4
    unsigned int event; // offset 0x4, size 0x4
} eventData[1067]; // size: 0x2158, address: 0x8052A930
// total size: 0x8
struct curve_node {
    // Members
    float t; // offset 0x0, size 0x4
    float val; // offset 0x4, size 0x4
};
// total size: 0x8
struct xIniValue {
    // Members
...
class zMiniME {
    ...
    // Members
public:
    struct xResponseCurve * curves; // offset 0x0, size 0x4
    unsigned int * curveIDs; // offset 0x4, size 0x4
    int curve_count; // offset 0x8, size 0x4
    struct curve_node * curvepoints; // offset 0xC, size 0x4
    class zMinigame * minigames; // offset 0x10, size 0x4
    int minigame_count; // offset 0x14, size 0x4
    union _zMinigameActions * actions; // offset 0x18, size 0x4
    class PadHistory history; // offset 0x1C, size 0x41B0
};
static class zMiniME singleMM; // size: 0x41CC, address: 0x80D2DD60
// Range: 0x80304CE0 -> 0x80304CEC
class zMiniME & zMiniME::GetMgr() {
    // References
    // -> static class zMiniME singleMM;
}

// Range: 0x80304CEC -> 0x80304D0C
void zMiniME::Init() {}

// total size: 0x8
struct /* @class$1643zMiniME_cpp */ {
    // Members
    float t; // offset 0x0, size 0x4
    float a; // offset 0x4, size 0x4
};
static // total size: 0x8
struct /* @class$1643zMiniME_cpp */ {
    // Members
    float t; // offset 0x0, size 0x4
    float a; // offset 0x4, size 0x4
} alphas[4]; // size: 0x20, address: 0x804D9FF8
enum xCamCoordType {
...
// Range: 0x80304D0C -> 0x80304E58
void zMiniME::SceneEnter(struct zScene * zsc /* r1+0x8 */) {
    // Local variables
    class zMiniME & mgr; // r31
    char str[16]; // r1+0x10
    unsigned int size; // r1+0xC
    unsigned int * buf; // r30
...
struct /* @class$1643zMiniME_cpp */ {
    ...
    // -> static struct xResponseCurve alphaCurve;
    // -> struct xGlobals * xglobals;
}

// Range: 0x80304E58 -> 0x80304EC8
static float get_F32(char * in /* r30 */, char * * out /* r31 */) {
    // Local variables
    float val; // f31
}

// Range: 0x80304EC8 -> 0x80304FC0
static unsigned int get_U32(char * in /* r31 */, char * * out /* r28 */) {
    // Local variables
    unsigned int val; // r29
    char * p; // r30
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D67338
// Range: 0x80304FC0 -> 0x803050BC
static unsigned int get_input(char * in /* r30 */, char * * out /* r29 */) {
    // Local variables
    int i; // r31
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D67342
// Range: 0x803050BC -> 0x803051C4
static unsigned int get_event(char * in /* r30 */, char * * out /* r28 */) {
    // Local variables
    unsigned int hash; // r29
    int i; // r31
...
// Range: 0x803051C4 -> 0x80305268
static int get_group_type(char * in /* r31 */, char * * out /* r30 */) {}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D6734C
...
// Range: 0x80305268 -> 0x80306084
void zMiniME::ParseMGD(unsigned int * buffer /* r1+0x8 */, unsigned int size /* r1+0xC */) {
    // Local variables
    class zMiniME & mgr; // r31
    struct xIniFile * ini; // r30
    unsigned char isCurve; // r1+0x15
    unsigned char isMinigame; // r1+0x14
    int numCurves; // r23
    int numCurvepoints; // r18
    int numMinigames; // r27
    int numActions; // r20
    struct xIniSection * isec; // r29
    struct xIniSection * end_sec; // r1+0xA4
    int i; // r1+0xA0
    unsigned int amt; // r1+0x9C
    unsigned char groupStart; // r1+0x13
    int groupType; // r1+0x98
    struct zMinigameInput * lastInput; // r1+0x94
    char * idxs; // r1+0x90
    int * curveIdx; // r1+0x8C
    int * actionIdx; // r1+0x88
    unsigned char isAction; // r1+0x12
    unsigned char isDDR; // r1+0x11
    int i; // r25
    char * s; // r1+0x84
    unsigned char cmd_ok; // r1+0x10
    int j; // r1+0x80
    int j; // r1+0x7C
    char * s; // r1+0x78
    char * t; // r1+0x74
    float time; // f31
    struct zMinigameEvent * event; // r28
    struct zMinigameInput * input; // r26
    struct zMinigameText * text; // r1+0x70
    int j; // r1+0x6C
    char * s; // r1+0x68
    char * s; // r17
    int num; // r1+0x64
    struct xResponseCurve * curve; // r1+0x60
    int num; // r1+0x5C
    char * q; // r1+0x58
    int i; // r1+0x54
    class zMinigame * mg; // r24
    unsigned int j; // r19
    struct zMinigameEvent * event; // r22
    struct zMinigameInput * input; // r21
    char flags[32]; // r1+0xA8
    struct zMinigameText * text; // r1+0x50

    // References
    // -> void * RwEngineInstance;
    // -> static char __FUNCTION__[9];
    // -> static char minigameCmds[1][16];
    // -> static char actionCmds[3][16];
    // -> static char curveCmds[1][16];
    // -> unsigned int _rwMemoryLastAllocLine;
    // -> const char * _rwMemoryLastAllocFile;
    // -> unsigned int gActiveHeap;
}

// Range: 0x80306084 -> 0x803060A8
void zMiniME::SceneExit() {}
...
struct PadValue {
    // Members
    float t; // offset 0x0, size 0x4
    unsigned int on; // offset 0x4, size 0x4
    struct analog_data stick[2]; // offset 0x8, size 0x30
};
// total size: 0x2
...
class PadHistory {
    ...
    void DetectStirring(int stick);

    unsigned char IsStirring(float & frequency, float & duration);

    // Members
    struct PadValue value[300]; // offset 0x0, size 0x41A0
    struct PadValue * input; // offset 0x41A0, size 0x4
    unsigned char isStirring; // offset 0x41A4, size 0x1
    float stirringFrequency; // offset 0x41A8, size 0x4
    float stirringDuration; // offset 0x41AC, size 0x4
};
// Range: 0x803060A8 -> 0x80306108
// this: r31
void PadHistory::Reset() {
    // Local variables
    int i; // r30
}

struct _tagxPad mPad[4]; // size: 0x4E0, address: 0x80B86CF8
// Range: 0x80306108 -> 0x80306204
// this: r31
void PadHistory::Update() {
    // Local variables
    struct _tagxPad * pad; // r30

    // References
    // -> struct _tagxPad mPad[4];
}

// Range: 0x80306204 -> 0x80306238
// this: r3
void PadHistory::Render() {
    // Local variables
    struct PadValue * val; // r31
    float lx; // f31
    float ly; // f30
    int i; // r30
    float x; // f29
...
static float maxRot; // size: 0x4, address: 0x80D6B86C
static signed char init; // size: 0x1, address: 0x80D6B870
float VBLANKS_PER_SEC; // size: 0x4, address: 0x80D63010
// Range: 0x80306238 -> 0x80306624
// this: r30
void PadHistory::DetectStirring(int stick /* r21 */) {
    // Local variables
    float prev_delta[15]; // r1+0xC
    int j; // r25
    float now; // f21
    struct PadValue * val; // r31
    int n; // r29
    struct PadValue * start; // r1+0x8
    unsigned char dumpStirring; // r24
    int maxInc; // r23
    int inc; // r28
    float inc_dur; // f30
    int maxDec; // r22
    ...
    float last; // f20
    float delta; // f31
    float smoothed_delta; // f24
    float ang; // f23
    float sum; // f22
    int j; // r26
    float smoothed_freq; // f19
...
// Range: 0x80306624 -> 0x80306648
// this: r3
unsigned char PadHistory::IsStirring(float & frequency /* r4 */, float & duration /* r5 */) {}

// Range: 0x80306648 -> 0x803066A8
// this: r30
void zMiniME::HandleInput(float dt /* f31 */) {
    // Local variables
...
// Range: 0x803066A8 -> 0x80306740
void zMiniME::Update(float dt /* f31 */) {
    // Local variables
    class zMiniME & mgr; // r30
    class zMinigame * mg; // r31
    class zMinigame * mgend; // r29
}

// Range: 0x80306740 -> 0x803067CC
void zMiniME::Render() {
    // Local variables
    class zMiniME & mgr; // r31
    class zMinigame * mg; // r30
    class zMinigame * mgend; // r29
}

// Range: 0x803067CC -> 0x8030686C
void zMiniME::StartMinigame(unsigned int replyWidgetID /* r1+0x8 */, unsigned int minigameID /* r27 */, const float * params /* r30 */) {
    // Local variables
    class zMiniME & mgr; // r29
    class zMinigame * mg; // r31
    class zMinigame * mgend; // r28
}

// total size: 0xC
struct zMinigameActionBase {
    // Members
    int type; // offset 0x0, size 0x4
...
// total size: 0x24
struct zMinigameEvent : public zMinigameActionBase {
    // Members
    unsigned int toWidgetID; // offset 0xC, size 0x4
    unsigned int event; // offset 0x10, size 0x4
    float params[4]; // offset 0x14, size 0x10
};
// total size: 0x1C
struct zMinigameInput : public zMinigameActionBase {
    // Members
    unsigned int input; // offset 0xC, size 0x4
    float frequency; // offset 0x10, size 0x4
    float duration; // offset 0x14, size 0x4
    unsigned int curveID; // offset 0x18, size 0x4
};
// total size: 0x14
struct zMinigameText : public zMinigameActionBase {
    // Members
    unsigned int hintTextID; // offset 0xC, size 0x4
    unsigned int textBoxID; // offset 0x10, size 0x4
};
union _zMinigameActions {
    struct zMinigameEvent event; // offset 0x0, size 0x24
    struct zMinigameInput input; // offset 0x0, size 0x1C
    struct zMinigameText text; // offset 0x0, size 0x14
};
// total size: 0x2C
class zMinigame {
    // Functions
    ...
    union _zMinigameActions * current_action; // offset 0x8, size 0x4
    unsigned int replyToID; // offset 0xC, size 0x4
    unsigned char groupAnd; // offset 0x10, size 0x1
    unsigned char groupOr; // offset 0x11, size 0x1
    int groupType; // offset 0x14, size 0x4
public:
    unsigned int minigameID; // offset 0x18, size 0x4
    unsigned int active : 1; // offset 0x1C, size 0x4
    unsigned int type : 7; // offset 0x1C, size 0x4
    unsigned int stalled : 1; // offset 0x1C, size 0x4
    unsigned int flashy : 1; // offset 0x1C, size 0x4
    unsigned int padding : 22; // offset 0x1C, size 0x4
    unsigned int cameraID; // offset 0x20, size 0x4
    union _zMinigameActions * actions; // offset 0x24, size 0x4
    unsigned int action_count; // offset 0x28, size 0x4
};
// Range: 0x8030686C -> 0x8030689C
// this: r3
void zMinigame::Start(unsigned int ID /* r4 */) {}

// Range: 0x8030689C -> 0x803068B0
// this: r3
void zMinigame::Stop() {}

static float last_hit[10]; // size: 0x28, address: 0x80D31F2C
// Range: 0x803068B0 -> 0x8030711C
// this: r31
void zMinigame::Update(float dt /* f31 */) {
    // Local variables
    struct _tagxPad * pad; // r29
    class zMiniME & mgr; // r1+0x60
    float frequency; // r1+0x5C
    float duration; // r1+0x58
    unsigned char isStirring; // r1+0x15
    union _zMinigameActions * action; // r26
    union _zMinigameActions * end_action; // r24
    float t; // r1+0x54
    struct zMinigameEvent * event; // r28
    struct zMinigameInput * input; // r30
    unsigned char hit; // r1+0x14
    float delta; // r1+0x50
    unsigned int eventMap[10]; // r1+0x64
    unsigned char done; // r23
    struct zMinigameEvent * event; // r27
    union _zMinigameActions * stall_action; // r1+0x4C
    struct zMinigameInput * input; // r25

    // References
    // -> static float last_hit[10];
    // -> struct _tagxPad mPad[4];
}
...
// Range: 0x8030711C -> 0x80307290
static void render_card(struct RwRaster * raster /* r1+0x8 */, float x /* f28 */, float y /* f29 */, float w /* f26 */, float h /* f27 */, unsigned char alpha /* r31 */) {
    // Local variables
    float rcz; // f31
    float nsz; // f30
    struct rwGameCube2DVertex vert[4]; // r1+0xC
}

// Range: 0x80307290 -> 0x803072C4
static void set_vert(struct rwGameCube2DVertex & vert /* r3 */, float x /* f1 */, float y /* f2 */, float u /* f3 */, float v /* f4 */, float nsz /* f6 */, unsigned char alpha /* r4 */) {}

// Range: 0x803072C4 -> 0x8030733C
static void render_card_centered(struct RwRaster * raster /* r1+0x8 */, float cx /* r1+0xC */, float cy /* r1+0x10 */, float dx /* f30 */, float dy /* f31 */, unsigned char alpha /* r1+0x14 */) {}

// total size: 0x8
struct /* @class$2228zMiniME_cpp */ {
    // Members
    unsigned int id; // offset 0x0, size 0x4
...
struct /* @class$2228zMiniME_cpp */ {
    ...
    unsigned int id; // offset 0x0, size 0x4
    struct RwTexture * texture; // offset 0x4, size 0x4
} textures[5]; // size: 0x28, address: 0x8052CA88
// Range: 0x8030733C -> 0x803076D8
// this: r31
void zMinigame::Render() {
    // Local variables
    int i; // r29
    union _zMinigameActions * action; // r30
    union _zMinigameActions * end_action; // r27
    float total; // f29
    float start; // f31
    float end; // f28
    float pct; // f30
...
// Range: 0x803076D8 -> 0x80307724
static void U8_COLOR_CLAMP(unsigned char & destu8 /* r3 */, float srcf32 /* f1 */) {}

static float GX1; // size: 0x4, address: 0x80D67358
static float GY1; // size: 0x4, address: 0x80D6735C
static float GDX; // size: 0x4, address: 0x80D67360
static float GDY; // size: 0x4, address: 0x80D67364
// total size: 0x4
struct /* @class$2284zMiniME_cpp */ {
    // Members
    float time; // offset 0x0, size 0x4
};
...
struct /* @class$2284zMiniME_cpp */ {
    // Members
    float time; // offset 0x0, size 0x4
} sGoodFX[8]; // size: 0x20, address: 0x80D31F54
static int sNumGoodFX; // size: 0x4, address: 0x80D6B874
static struct RwTexture * sGoodFXTex; // size: 0x4, address: 0x80D6B878
// Range: 0x80307724 -> 0x80307794
void zMiniME::NewGoodFX() {
    // References
    // -> static int sNumGoodFX;
    // -> static // total size: 0x4
struct /* @class$2284zMiniME_cpp */ {
    // Members
    float time; // offset 0x0, size 0x4
} sGoodFX[8];
    // -> static struct RwTexture * sGoodFXTex;
}

// Range: 0x80307794 -> 0x80307988
void zMiniME::RenderGoodFX() {
    // Local variables
    float time; // f30
    int i; // r31
    float frac; // f31
...
static int numDeflectors; // size: 0x4, address: 0x80D6B87C
static class zAnalogInputDeflection * deflectors[16]; // size: 0x40, address: 0x80D31F74
static int numDirectors; // size: 0x4, address: 0x80D6B880
static class zAnalogInputDirection * directors[16]; // size: 0x40, address: 0x80D31FB4
static int numTwiddlers; // size: 0x4, address: 0x80D6B884
static class zAnalogInputTwiddler * twiddlers[16]; // size: 0x40, address: 0x80D31FF4
static int numPours; // size: 0x4, address: 0x80D6B888
static class zCMGPourWidget * pours[16]; // size: 0x40, address: 0x80D32034
// total size: 0x1
class zAnalogInput {};
// Range: 0x80307994 -> 0x80307AD4
void zAnalogInput::Update(float dt /* f31 */) {
    // Local variables
    int i; // r31
    class zAnalogInputDeflection * input; // r27
    int i; // r30
    class zAnalogInputDirection * input; // r26
    int i; // r29
    class zAnalogInputTwiddler * twiddle; // r25
    int i; // r28
    class zCMGPourWidget * pour; // r24

    // References
    // -> static int numPours;
    // -> static class zCMGPourWidget * pours[16];
    // -> static int numTwiddlers;
    // -> static class zAnalogInputTwiddler * twiddlers[16];
    // -> static int numDirectors;
    // -> static class zAnalogInputDirection * directors[16];
    // -> static int numDeflectors;
    // -> static class zAnalogInputDeflection * deflectors[16];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D67368
// Range: 0x80307AD4 -> 0x80307C3C
void zAnalogInput::SceneEnter() {
    // Local variables
    int numDyns; // r29
    int i; // r30
    struct xDynAsset * dyn; // r31

    // References
    // -> static char __FUNCTION__[11];
}

// Range: 0x80307C3C -> 0x80307C60
void zAnalogInput::SceneExit() {
    // References
    // -> static int numPours;
    // -> static int numTwiddlers;
    // -> static int numDirectors;
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D67373
// total size: 0x20
struct zAnalogInputDeflectionAsset : public xDynAsset {
    // Members
    unsigned int stick; // offset 0x10, size 0x4
    unsigned int axis; // offset 0x14, size 0x4
    unsigned int outputID[2]; // offset 0x18, size 0x8
};
// total size: 0x1C
class zAnalogInputDeflection : public xBase {
    // Functions
    zAnalogInputDeflection(struct zAnalogInputDeflectionAsset * theAsset);
    ...
    // Members
    struct zAnalogInputDeflectionAsset * asset; // offset 0x10, size 0x4
    struct xBase * output[2]; // offset 0x14, size 0x8
};
// Range: 0x80307C60 -> 0x80307D50
// this: r31
zAnalogInputDeflection::zAnalogInputDeflection(struct zAnalogInputDeflectionAsset * theAsset /* r30 */) {
    // References
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D67378
// Range: 0x80307D50 -> 0x80307F24
void zAnalogInputDeflection::Init(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r28 */) {
    // Local variables
...
static char __FUNCTION__[8]; // size: 0x8, address: 0x80D6737D
// Range: 0x80307F24 -> 0x80308048
void zAnalogInputDeflection::EventCB(struct xBase * from /* r1+0x8 */, struct xBase * to /* r30 */, unsigned int toEvent /* r1+0xC */, const float * toParam /* r1+0x10 */, struct xBase * toParamWidget /* r1+0x14 */, unsigned int toParamWidgetID /* r1+0x18 */) {
    // Local variables
    class zAnalogInputDeflection * input; // r31

    // References
    // -> static char __FUNCTION__[8];
}

// Range: 0x80308048 -> 0x80308108
// this: r31
void zAnalogInputDeflection::EventCB(unsigned int toEvent /* r1+0x8 */) {
    // Local variables
    int i; // r30
}

// Range: 0x80308108 -> 0x80308234
// this: r31
void zAnalogInputDeflection::Update() {
    // Local variables
    struct _tagxPad * pad; // r27
    float * data; // r29
    float val; // f31
    float deflection; // f29
    unsigned char centered; // r26
    int i; // r30
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D67385
// total size: 0x1C
struct zAnalogInputDirectionAsset : public xDynAsset {
    // Members
    unsigned int stick; // offset 0x10, size 0x4
    unsigned int outputID[2]; // offset 0x14, size 0x8
};
// total size: 0x1C
class zAnalogInputDirection : public xBase {
    // Functions
    zAnalogInputDirection(struct zAnalogInputDirectionAsset * theAsset);
    ...
    // Members
    struct zAnalogInputDirectionAsset * asset; // offset 0x10, size 0x4
    struct xBase * output[2]; // offset 0x14, size 0x8
};
// Range: 0x80308234 -> 0x80308324
// this: r31
zAnalogInputDirection::zAnalogInputDirection(struct zAnalogInputDirectionAsset * theAsset /* r30 */) {
    // References
    // -> static char __FUNCTION__[5];
    // -> static int numDirectors;
    // -> static class zAnalogInputDirection * directors[16];
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D6738A
// Range: 0x80308324 -> 0x803084F8
void zAnalogInputDirection::Init(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r28 */) {
    // Local variables
    unsigned long expected_asset_size; // r29
...
static char __FUNCTION__[8]; // size: 0x8, address: 0x80D6738F
// Range: 0x803084F8 -> 0x8030861C
void zAnalogInputDirection::EventCB(struct xBase * from /* r1+0x8 */, struct xBase * to /* r30 */, unsigned int toEvent /* r1+0xC */, const float * toParam /* r1+0x10 */, struct xBase * toParamWidget /* r1+0x14 */, unsigned int toParamWidgetID /* r1+0x18 */) {
    // Local variables
    class zAnalogInputDirection * input; // r31

    // References
    // -> static char __FUNCTION__[8];
}

// Range: 0x8030861C -> 0x803086DC
// this: r31
void zAnalogInputDirection::EventCB(unsigned int toEvent /* r1+0x8 */) {
    // Local variables
    int i; // r30
}

// Range: 0x803086DC -> 0x80308850
// this: r31
void zAnalogInputDirection::Update() {
    // Local variables
    struct _tagxPad * pad; // r29
    float ang; // f29
    unsigned char centered; // r28
    float direction; // f31
    int i; // r30
    float params[4]; // r1+0x8
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D67397
// total size: 0x30
struct zAnalogInputTwiddlerAsset : public xDynAsset {
    // Members
    unsigned int twiddleeID; // offset 0x10, size 0x4
    unsigned int type; // offset 0x14, size 0x4
    unsigned int axis; // offset 0x18, size 0x4
    float inputMin; // offset 0x1C, size 0x4
    float inputMax; // offset 0x20, size 0x4
    float outputMin; // offset 0x24, size 0x4
    float outputMax; // offset 0x28, size 0x4
    float defaultValue; // offset 0x2C, size 0x4
};
// total size: 0x20
class zAnalogInputTwiddler : public xBase {
    // Functions
    zAnalogInputTwiddler(struct zAnalogInputTwiddlerAsset * theAsset);
    ...
    // Members
    struct zAnalogInputTwiddlerAsset * asset; // offset 0x10, size 0x4
    float value; // offset 0x14, size 0x4
    struct xEnt * twiddlee; // offset 0x18, size 0x4
    float inputNorm; // offset 0x1C, size 0x4
};
// Range: 0x80308850 -> 0x80308A0C
// this: r31
zAnalogInputTwiddler::zAnalogInputTwiddler(struct zAnalogInputTwiddlerAsset * theAsset /* r30 */) {
    // References
    // -> static char __FUNCTION__[5];
    // -> static int numTwiddlers;
    // -> static class zAnalogInputTwiddler * twiddlers[16];
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D6739C
// Range: 0x80308A0C -> 0x80308BE0
void zAnalogInputTwiddler::Init(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r28 */) {
    // Local variables
    unsigned long expected_asset_size; // r29
...
static char __FUNCTION__[8]; // size: 0x8, address: 0x80D673A1
// Range: 0x80308BE0 -> 0x80308D04
void zAnalogInputTwiddler::EventCB(struct xBase * from /* r1+0x8 */, struct xBase * to /* r30 */, unsigned int toEvent /* r1+0xC */, const float * toParam /* r1+0x10 */, struct xBase * toParamWidget /* r1+0x14 */, unsigned int toParamWidgetID /* r1+0x18 */) {
    // Local variables
    class zAnalogInputTwiddler * input; // r31

    // References
    // -> static char __FUNCTION__[8];
}

// Range: 0x80308D04 -> 0x80308DB4
// this: r31
void zAnalogInputTwiddler::EventCB(unsigned int toEvent /* r1+0x8 */, const float * toParam /* r30 */) {}

// Range: 0x80308DB4 -> 0x80308F6C
// this: r31
void zAnalogInputTwiddler::Update() {
    // Local variables
    float val; // f31
}

static char __FUNCTION__[5]; // size: 0x5, address: 0x80D673A9
// total size: 0x50
struct zCMGPourWidgetAsset : public xDynAsset {
    // Members
    float volume; // offset 0x10, size 0x4
    unsigned int fillInputID; // offset 0x14, size 0x4
    unsigned int fillInputType; // offset 0x18, size 0x4
    float fillInputMin; // offset 0x1C, size 0x4
    float fillInputMax; // offset 0x20, size 0x4
    float fillFlowRate; // offset 0x24, size 0x4
    unsigned int tiltInputID; // offset 0x28, size 0x4
    unsigned int tiltInputType; // offset 0x2C, size 0x4
    float tiltInputMin; // offset 0x30, size 0x4
    float tiltInputMax; // offset 0x34, size 0x4
    float fillWeight; // offset 0x38, size 0x4
    float spillWeight; // offset 0x3C, size 0x4
    float foamWeight; // offset 0x40, size 0x4
    unsigned int scoreCounterID; // offset 0x44, size 0x4
    unsigned int flowOutputID; // offset 0x48, size 0x4
    unsigned int fillOutputID; // offset 0x4C, size 0x4
};
// total size: 0x3C
class zCMGPourWidget : public xBase {
    // Functions
    zCMGPourWidget(struct zCMGPourWidgetAsset * theAsset);

    void EventCB(struct xBase * from, unsigned int toEvent, const float * toParam);
    ...
    // Members
    struct zCMGPourWidgetAsset * asset; // offset 0x10, size 0x4
    unsigned char started; // offset 0x14, size 0x1
    unsigned char pouring; // offset 0x15, size 0x1
    float currentVolume; // offset 0x18, size 0x4
    float fillInput; // offset 0x1C, size 0x4
    float tiltInput; // offset 0x20, size 0x4
    float fill; // offset 0x24, size 0x4
    float spill; // offset 0x28, size 0x4
    float foam; // offset 0x2C, size 0x4
    float score; // offset 0x30, size 0x4
    struct xBase * flowOutput; // offset 0x34, size 0x4
    struct xBase * fillOutput; // offset 0x38, size 0x4
};
// Range: 0x80308F6C -> 0x80309078
// this: r31
zCMGPourWidget::zCMGPourWidget(struct zCMGPourWidgetAsset * theAsset /* r30 */) {
    // References
    // -> static char __FUNCTION__[5];
    // -> static int numPours;
    // -> static class zCMGPourWidget * pours[16];
...
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D673AE
// Range: 0x80309078 -> 0x8030924C
void zCMGPourWidget::Init(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r28 */) {
    // Local variables
    unsigned long expected_asset_size; // r29
...
static char __FUNCTION__[8]; // size: 0x8, address: 0x80D673B3
// Range: 0x8030924C -> 0x80309370
void zCMGPourWidget::EventCB(struct xBase * from /* r1+0x8 */, struct xBase * to /* r30 */, unsigned int toEvent /* r1+0xC */, const float * toParam /* r1+0x10 */, struct xBase * toParamWidget /* r1+0x14 */, unsigned int toParamWidgetID /* r1+0x18 */) {
    // Local variables
    class zCMGPourWidget * pour; // r31

    // References
    // -> static char __FUNCTION__[8];
}

// Range: 0x80309370 -> 0x80309558
// this: r31
void zCMGPourWidget::EventCB(struct xBase * from /* r30 */, unsigned int toEvent /* r1+0x8 */, const float * toParam /* r29 */) {
    // Local variables
    float params[4]; // r1+0x1C
    float params[4]; // r1+0xC
}

// Range: 0x80309558 -> 0x8030988C
// this: r31
void zCMGPourWidget::Update(float dt /* r1+0x8 */) {
    // Local variables
    signed short count; // r30
    float rate; // f31
    float params[4]; // r1+0x1C
    float spillage; // f30
    float amt; // f29
...
unsigned int pourBarTexID; // size: 0x4, address: 0x80D673BC
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x8030988C -> 0x80309A7C
// this: r30
void zCMGPourWidget::Render() {
    // Local variables
    struct RwTexture * tex; // r31
    ...
    // -> unsigned int FB_YRES;
    // -> unsigned int FB_XRES;
    // -> unsigned int pourBarTexID;
}

// Range: 0x80309A7C -> 0x80309AE8
void zCMGPourWidget::RenderAll() {
    // Local variables
    int i; // r31
    class zCMGPourWidget * pour; // r30
