/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zReticle.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801F3AAC -> 0x801F57BC
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct xFuncPiece sFadeSize[2]; // size: 0x40, address: 0x80D17FC0
// total size: 0x20
struct xFuncPiece {
    // Members
...
static struct xFuncPiece sDistSize; // size: 0x20, address: 0x80D18000
static float sAlpha; // size: 0x4, address: 0x80D642C0
static float sFadeInSpeed; // size: 0x4, address: 0x80D642C4
static float sFadeOutSpeed; // size: 0x4, address: 0x80D642C8
static float sMinDist; // size: 0x4, address: 0x80D642CC
static float sPulseAmp; // size: 0x4, address: 0x80D642D0
static float sPulseSpeed; // size: 0x4, address: 0x80D642D4
static float sRotSpeed; // size: 0x4, address: 0x80D642D8
static float sSwirlMaxU; // size: 0x4, address: 0x80D642DC
static float sSwirlMinU; // size: 0x4, address: 0x80D642E0
static float sSwirlMaxV; // size: 0x4, address: 0x80D642E4
static float sSwirlMinV; // size: 0x4, address: 0x80D642E8
static unsigned char sSwirlRed; // size: 0x1, address: 0x80D642EC
static unsigned char sSwirlGreen; // size: 0x1, address: 0x80D642ED
static unsigned char sSwirlBlue; // size: 0x1, address: 0x80D642EE
static unsigned char sSwirlRedSwitchLever1; // size: 0x1, address: 0x80D642EF
static unsigned char sSwirlGreenSwitchLever1; // size: 0x1, address: 0x80D6AD50
static unsigned char sSwirlBlueSwitchLever1; // size: 0x1, address: 0x80D642F0
static unsigned char sSwirlRedSwitchLever2; // size: 0x1, address: 0x80D642F1
static unsigned char sSwirlGreenSwitchLever2; // size: 0x1, address: 0x80D642F2
static unsigned char sSwirlBlueSwitchLever2; // size: 0x1, address: 0x80D642F3
static float sLetterMaxU; // size: 0x4, address: 0x80D642F4
static float sLetterMinU; // size: 0x4, address: 0x80D642F8
static float sLetterMaxV; // size: 0x4, address: 0x80D642FC
static float sLetterMinV; // size: 0x4, address: 0x80D64300
static unsigned char sLetterRed; // size: 0x1, address: 0x80D64304
static unsigned char sLetterGreen; // size: 0x1, address: 0x80D64305
static unsigned char sLetterBlue; // size: 0x1, address: 0x80D64306
static float sXMult; // size: 0x4, address: 0x80D64308
static float sZBias; // size: 0x4, address: 0x80D6430C
// total size: 0x24
struct zReticle {
    // Members
    int flags; // offset 0x0, size 0x4
    float currAngle; // offset 0x4, size 0x4
    float currFade; // offset 0x8, size 0x4
    float currPulse; // offset 0xC, size 0x4
    struct xVec3 pos; // offset 0x10, size 0xC
    float radius; // offset 0x1C, size 0x4
    struct xColor_tag color; // offset 0x20, size 0x4
};
static struct zReticle reticle[4]; // size: 0x90, address: 0x80D18020
// total size: 0x34
struct RwRaster {
    // Members
...
static struct RwRaster * reticleRast; // size: 0x4, address: 0x80D6AD54
// total size: 0x8
struct RwObject {
    // Members
...
void zReticle_Init() {
    ...
    // References
    // -> static struct xFuncPiece sDistSize;
    // -> static struct xFuncPiece sFadeSize[2];
    // -> static struct RwRaster * reticleRast;
    // -> static struct zReticle reticle[4];
}

// Range: 0x801F3BFC -> 0x801F3C4C
float zReticle_RotateSwirl(float angle /* f1 */, float dt /* f2 */) {
    // Local variables
    float result; // f31

    // References
    // -> static float sRotSpeed;
}

// Range: 0x801F3C4C -> 0x801F3D4C
static void UpdateReticle(struct zReticle * ret /* r31 */, float dt /* f31 */) {
    // References
    // -> static float sPulseSpeed;
    // -> static float sFadeOutSpeed;
    // -> static float sFadeInSpeed;
}

// Range: 0x801F3D4C -> 0x801F3DBC
void zReticle_Update(float dt /* r1+0x8 */) {
    // Local variables
    int i; // r31
...
// Range: 0x801F3DBC -> 0x801F3DD8
void zReticle_GetSwirlColor(struct xColor_tag * color /* r3 */) {
    // References
    // -> static unsigned char sSwirlBlue;
    // -> static unsigned char sSwirlGreen;
    // -> static unsigned char sSwirlRed;
}

// Range: 0x801F3DD8 -> 0x801F3DF4
void zReticle_GetLetterColor(struct xColor_tag * color /* r3 */) {
    // References
    // -> static unsigned char sLetterBlue;
    // -> static unsigned char sLetterGreen;
    // -> static unsigned char sLetterRed;
}
...
// Range: 0x801F3DF4 -> 0x801F42FC
void zReticle_RenderSwirl(struct xMat4x3 * mat /* r1+0x8 */, float angle /* f27 */, struct xColor_tag & color1 /* r1+0xC */, struct xColor_tag & color2 /* r1+0x10 */) {
    // Local variables
    ...
    struct RwRGBA col; // r1+0x18
    struct RwV3d tmp; // r1+0x24
    struct RwRGBA col; // r1+0x14
    unsigned short * indx; // r28

    // References
    // -> struct tagiRenderArrays gRenderArr;
    // -> static float sSwirlMaxV;
    // -> static float sSwirlMaxU;
    // -> static float sSwirlMinU;
    // -> static float sSwirlMinV;
    // -> static float sXMult;
    // -> static struct RwRaster * reticleRast;
}
...
// Range: 0x801F42FC -> 0x801F47A0
void zReticle_RenderLetter(struct xMat4x3 * mat /* r1+0x8 */, struct xColor_tag & color /* r29 */) {
    // Local variables
    struct RxObjSpace3DVertex * vert; // r31
    int currVert; // r30
    float x; // f30
    ...
    float ang; // f31
    float aspect; // f28
    struct RwV3d tmp; // r1+0x40
    struct RwRGBA col; // r1+0x18
    struct RwV3d tmp; // r1+0x34
    struct RwRGBA col; // r1+0x14
    struct RwV3d tmp; // r1+0x28
    struct RwRGBA col; // r1+0x10
    struct RwV3d tmp; // r1+0x1C
    ...
    // References
    // -> struct tagiRenderArrays gRenderArr;
    // -> static float sLetterMinV;
    // -> static float sLetterMaxU;
    // -> static float sLetterMaxV;
    // -> static float sLetterMinU;
    // -> static struct RwRaster * reticleRast;
}
...
// total size: 0x1
struct /* @class$858zReticle_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$859zReticle_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$860zReticle_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$861zReticle_cpp */ {
    // Members
...
// Range: 0x801F47A0 -> 0x801F4A30
static void RenderReticle(struct zReticle * ret /* r31 */) {
    // Local variables
    struct xMat4x3 * cam; // r30
    struct xVec3 disp; // r1+0x24
    float dist; // f30
    struct xVec3 adjPos; // r1+0x18
    float atDist; // f29
    struct xMat4x3 objToWorld; // r1+0x30
    float scale; // f31

    // References
    // -> static float sAlpha;
    // -> static float sPulseAmp;
    // -> static struct xFuncPiece sFadeSize[2];
    // -> static float sMinDist;
    // -> static struct xFuncPiece sDistSize;
    // -> struct zGlobals globals;
}

// Range: 0x801F4A30 -> 0x801F4AD4
...
// Range: 0x801F4AD4 -> 0x801F4E88
void zReticle_AddTweaks() {
    // References
    // -> static float sPulseSpeed;
    // -> static float sPulseAmp;
    // -> static float sRotSpeed;
...
static float sAlphaUpRate; // size: 0x4, address: 0x80D64310
static float sAlphaDownRate; // size: 0x4, address: 0x80D64314
static float sMinDrawDist2; // size: 0x4, address: 0x80D64318
static float sMaxDrawAlpha; // size: 0x4, address: 0x80D6431C
static float sIconAngle; // size: 0x4, address: 0x80D6AD58
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D64320
// Range: 0x801F4E88 -> 0x801F5040
void zIcon_Init(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r29 */) {
    // Local variables
...
// total size: 0x30
struct zIconAsset : public xDynAsset {
    // Members
...
// total size: 0x70
struct zIcon : public xBase {
    // Members
    struct zIconAsset * iconAsset; // offset 0x10, size 0x4
    struct xMat4x3 mat; // offset 0x20, size 0x40
    float currAlpha; // offset 0x60, size 0x4
    unsigned int runFlags; // offset 0x64, size 0x4
};
// Range: 0x801F5040 -> 0x801F5124
void zIcon_Init(struct zIcon * icon /* r31 */, struct zIconAsset * asset /* r30 */) {
    // Local variables
    float yaw; // f31
    float pitch; // f30
    float roll; // f29
...
// Range: 0x801F5124 -> 0x801F5194
void zIcon_Reset() {
    // Local variables
    struct zScene * zsc; // r30
    int count; // r28
    struct zIcon * iconList; // r29
    int i; // r31
...
// Range: 0x801F5194 -> 0x801F5230
void zIcon_Update(float dt /* f31 */) {
    // Local variables
    struct zScene * zsc; // r30
    int count; // r27
    struct zIcon * iconList; // r26
    int i; // r29
    struct zIcon * icon; // r31
    struct zIconAsset * asset; // r28

    // References
    // -> struct zGlobals globals;
    // -> static float sIconAngle;
}

// Range: 0x801F5230 -> 0x801F53B8
float zIcon_GetNewAlpha(float currAlpha /* f31 */, float dt /* f23 */, const struct xVec3 & pos /* r1+0x8 */, unsigned char active /* r1+0xC */, float radius /* f24 */) {
    // Local variables
    float newAlpha; // f29
    ...
    // -> static float sMinDrawDist2;
    // -> static float sMaxDrawAlpha;
    // -> struct zGlobals globals;
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D6432B
// Range: 0x801F53B8 -> 0x801F54F0
void zIcon_EventCB(struct xBase * to /* r30 */, unsigned int toEvent /* r1+0x8 */) {
    // Local variables
    struct zIcon * icon; // r31
    struct zIconAsset * asset; // r1+0xC

    // References
    // -> static char __FUNCTION__[14];
}

// Range: 0x801F54F0 -> 0x801F5550
void zIcon_Render() {
    // Local variables
    struct zScene * zsc; // r31
    int count; // r28
    struct zIcon * iconList; // r27
    int i; // r30
    struct zIcon * icon; // r29
...
// Range: 0x801F5550 -> 0x801F5600
void zIcon_Render(const struct xMat4x3 & mat /* r1+0x8 */, float alpha /* f31 */, enum zInteractionType type /* r1+0xC */) {
    // Local variables
    struct xColor_tag color1; // r1+0x14
    struct xColor_tag color2; // r1+0x10

    // References
    // -> static unsigned char sSwirlBlueSwitchLever2;
    // -> static unsigned char sSwirlGreenSwitchLever2;
    // -> static unsigned char sSwirlRedSwitchLever2;
    // -> static unsigned char sSwirlBlueSwitchLever1;
    // -> static unsigned char sSwirlGreenSwitchLever1;
    // -> static unsigned char sSwirlRedSwitchLever1;
}

// Range: 0x801F5600 -> 0x801F5680
void zIcon_Render(const struct xMat4x3 & mat /* r1+0x8 */, float alpha /* f31 */) {
    // Local variables
...
// Range: 0x801F5680 -> 0x801F57BC
void zIcon_Render(const struct xMat4x3 & mat /* r26 */, struct xColor_tag & color1 /* r31 */, struct xColor_tag & color2 /* r27 */) {
    // Local variables
    struct xMat4x3 tempMat; // r1+0x20
