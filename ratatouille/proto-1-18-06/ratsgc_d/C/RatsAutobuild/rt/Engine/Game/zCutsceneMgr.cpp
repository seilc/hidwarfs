/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zCutsceneMgr.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80140FF4 -> 0x80142B34
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
unsigned int demoDisableHackTable[1]; // size: 0x4, address: 0x80D62478
// total size: 0x8
struct RwObject {
    // Members
...
struct zCutsceneHack cutsceneHackTable[15]; // size: 0xC30, address: 0x8051C870
// Range: 0x80140FF4 -> 0x80140FFC
int zCutsceneMgrGetHackTableSize() {}

// Range: 0x80140FFC -> 0x80141010
struct zCutsceneHack & zCutsceneMgrGetHackTableDataByIdx(int idx /* r3 */) {
    // References
    // -> struct zCutsceneHack cutsceneHackTable[15];
}

static int gCutsceneSkipOK; // size: 0x4, address: 0x80D69ED0
static unsigned char ents_hidden; // size: 0x1, address: 0x80D69ED4
// Range: 0x80141010 -> 0x80141040
void zCutsceneMgrInit(void * b /* r1+0x8 */, void * tasset /* r1+0xC */) {}
...
// Range: 0x80141040 -> 0x801410D8
void zCutsceneMgrInit(struct xBase * b /* r29 */, struct xCutsceneMgrAsset * tasset /* r30 */) {
    // Local variables
    struct zCutsceneMgr * t; // r31

    // References
    // -> static unsigned char ents_hidden;
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D6247C
// Range: 0x801410D8 -> 0x801411F0
void zCutsceneMgrReset(struct zCutsceneMgr * t /* r31 */) {
    // References
    // -> static unsigned char ents_hidden;
    // -> static char __FUNCTION__[18];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D6248E
// total size: 0x0
...
// Range: 0x801411F0 -> 0x80141274
void zCutsceneMgrSave(struct xSerial * s /* r1+0x8 */) {
    // References
    // -> static char __FUNCTION__[17];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D6249F
// Range: 0x80141274 -> 0x801412F8
void zCutsceneMgrLoad(struct xSerial * s /* r1+0x8 */) {
    // References
    // -> static char __FUNCTION__[17];
}

// Range: 0x801412F8 -> 0x80141338
static struct RpAtomic * NextAtomicCallback(struct RpAtomic * atomic /* r3 */, void * data /* r4 */) {
    // Local variables
    struct RpAtomic * * nextModel; // r31
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x8051D4A0
// Range: 0x80141338 -> 0x80141764
static void zCutsceneMgrPlayStart(struct zCutsceneMgr * t /* r31 */) {
    // Local variables
    unsigned int i; // r30
    unsigned int j; // r29
    unsigned int hackTableElements; // r22
    unsigned int cinid; // r21
    unsigned int hackid; // r24
    unsigned char hacked; // r23
    unsigned int counter; // r28
    struct RpAtomic * tmpModel; // r27
    struct xCutsceneData * mphdata; // r26
    unsigned int mphIndex; // r25
    struct RpClump * clump; // r20
    struct RpAtomic * nextModel; // r1+0x8

    // References
    // -> struct zCutsceneHack cutsceneHackTable[15];
    // -> static char __FUNCTION__[22];
    // -> static int gCutsceneSkipOK;
}

static int been_here; // size: 0x4, address: 0x80D69ED8
static int been_here; // size: 0x4, address: 0x80D69EDC
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D624B0
// total size: 0x30
struct xMat3x3 {
    // Members
...
// total size: 0x4
struct /* @class$889zCutsceneMgr_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1053zCutsceneMgr_cpp */ {
    // Members
...
union /* @class$601zCutsceneMgr_cpp */ {
...
// total size: 0x10
struct /* @class$1678zCutsceneMgr_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x80141764 -> 0x80141BCC
void zCutsceneMgrEventCB(struct xBase * to /* r30 */, unsigned int toEvent /* r26 */, const float * toParam /* r27 */) {
    // Local variables
    struct zCutsceneMgr * t; // r31
    unsigned char disableCutscene; // r29
    unsigned int i; // r28
    int rc; // r25
    ...
    // -> static int been_here;
    // -> unsigned int demoDisableHackTable[1];
}

float csDebug_Speed; // size: 0x4, address: 0x80D624C4
int csDebug_KeepSpeed; // size: 0x4, address: 0x80D69EE0
unsigned char thisShouldAlwaysBeFalseItsJustToFoolTheStupidCompiler; // size: 0x1, address: 0x80D69EE4
static char __FUNCTION__[23]; // size: 0x17, address: 0x8051D4B6
// Range: 0x80141BCC -> 0x80141D20
void zCutsceneMgrFinishLoad(struct xBase * to /* r30 */) {
    // Local variables
    struct zCutsceneMgr * t; // r31

    // References
    // -> static char __FUNCTION__[23];
    // -> unsigned char thisShouldAlwaysBeFalseItsJustToFoolTheStupidCompiler;
}

static char __FUNCTION__[23]; // size: 0x17, address: 0x8051D4CD
struct xGlobals * xglobals; // size: 0x4, address: 0x80D63020
// Range: 0x80141D20 -> 0x80141EE0
void zCutsceneMgrFinishExit(struct xBase * to /* r29 */) {
    // Local variables
    struct zCutsceneMgr * t; // r31
    int i; // r30
...
// total size: 0x890
struct /* @class$1921zCutsceneMgr_cpp */ {
    // Static members
...
// total size: 0xC
struct /* @class$830zCutsceneMgr_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$831zCutsceneMgr_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$832zCutsceneMgr_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$833zCutsceneMgr_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$834zCutsceneMgr_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$835zCutsceneMgr_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$836zCutsceneMgr_cpp */ {
    // Members
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D624C8
// total size: 0x14
struct xPECircle {
    // Members
...
// Range: 0x80141EE0 -> 0x80141FE8
static void zCutsceneMgrKillFX(struct zCutsceneMgr * t /* r30 */) {
    // Local variables
    struct xCutsceneMgrAsset * a; // r31
    int i; // r30
    struct zParEmitter * e; // r0

    // References
    // -> static char __FUNCTION__[19];
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x8051D4E4
// Range: 0x80141FE8 -> 0x801421A0
static void zCutsceneMgrUpdateFX(struct zCutsceneMgr * t /* r29 */) {
    // Local variables
    struct xCutsceneMgrAsset * a; // r31
    struct xCutscene * csn; // r30
    int i; // r29
    struct zParEmitter * e; // r0
...
static int doCineFX; // size: 0x4, address: 0x80D69EE8
static signed char init; // size: 0x1, address: 0x80D69EEC
static int roller; // size: 0x4, address: 0x80D69EF0
static signed char init; // size: 0x1, address: 0x80D69EF4
static int inker; // size: 0x4, address: 0x80D69EF8
static signed char init; // size: 0x1, address: 0x80D69EFC
static float scl_fasterMin; // size: 0x4, address: 0x80D69F00
static signed char init; // size: 0x1, address: 0x80D69F04
static float scl_fasterMax; // size: 0x4, address: 0x80D69F08
static signed char init; // size: 0x1, address: 0x80D69F0C
static float scl_faster; // size: 0x4, address: 0x80D69F10
static signed char init; // size: 0x1, address: 0x80D69F14
static float makeFastFaster; // size: 0x4, address: 0x80D69F18
static signed char init; // size: 0x1, address: 0x80D69F1C
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D624DC
struct _tagxPad mPad[4]; // size: 0x4E0, address: 0x80B86CF8
float VBLANKS_PER_SEC; // size: 0x4, address: 0x80D63010
// Range: 0x801421A0 -> 0x80142998
void zCutsceneMgrUpdate(struct xBase * to /* r29 */, struct xScene * sc /* r27 */, float dt /* f31 */) {
    // Local variables
    struct zCutsceneMgr * t; // r29
    int listenIfHeld; // r28
    int i; // r1+0x14
    int i; // r1+0x10
    float y; // f30
    int i; // r1+0xC
    ...
    // -> struct xGlobals * xglobals;
    // -> float csDebug_Speed;
    // -> static float scl_faster;
    // -> static float scl_fasterMin;
    // -> static float makeFastFaster;
    // -> static float scl_fasterMax;
    // -> static signed char init;
    // -> static signed char init;
    // -> static signed char init;
    // -> static signed char init;
    // -> struct _tagxPad mPad[4];
    // -> int csDebug_KeepSpeed;
    // -> static int inker;
    // -> static int roller;
    // -> static signed char init;
    // -> static signed char init;
...
// Range: 0x80142998 -> 0x80142A38
void zCutsceneMgr_Started() {
    // Local variables
    int i; // r31
...
// Range: 0x80142A38 -> 0x80142B34
static void check_hide_entities() {
    // Local variables
    unsigned char hide; // r3
