/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zCamMarker.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x802AACCC -> 0x802ACB74
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static unsigned char activated; // size: 0x1, address: 0x80D6B4E8
static struct zCamWidget * widget_cam; // size: 0x4, address: 0x80D6B4EC
enum _tagTransType {
    eTransType_None = 0,
...
static struct xCamAsset static_cam_asset; // size: 0x88, address: 0x80D2CE30
// Range: 0x802AACCC -> 0x802AAD58
static void activate_camera(const struct xCamAsset & a /* r1+0x8 */, struct xEnt * ent /* r1+0xC */) {
    // References
    // -> static struct zCamWidget * widget_cam;
    // -> static unsigned char activated;
}

// total size: 0x210
struct zCamWidget : public xCam {
    // Functions
    static void transition(const struct xCamAsset * asset, float time, struct xEnt * target_ent);

    static void start_transition();

    static void finish_transition();

    static void refresh_end_mat();
    ...
    // Members
    unsigned char transitioning; // offset 0x140, size 0x1
    unsigned char restore_original; // offset 0x141, size 0x1
    const struct xCamAsset * asset; // offset 0x144, size 0x4
    float time; // offset 0x148, size 0x4
    float end_time; // offset 0x14C, size 0x4
    float start_fov; // offset 0x150, size 0x4
    float end_fov; // offset 0x154, size 0x4
    struct xVec3 start_loc; // offset 0x158, size 0xC
    struct xQuat start_dir; // offset 0x164, size 0x10
    struct xQuat end_dir; // offset 0x174, size 0x10
    float last_s; // offset 0x184, size 0x4
    struct xEnt * target_ent; // offset 0x188, size 0x4
    struct xMat4x3 end_mat; // offset 0x190, size 0x40
    struct xMat4x3 original_mat; // offset 0x1D0, size 0x40
};
// Range: 0x802AAD58 -> 0x802AAE14
// this: r31
static void zCamWidget::transition(const struct xCamAsset * asset /* r30 */, float time /* f30 */, struct xEnt * target_ent /* r1+0x8 */) {}

// Range: 0x802AAE14 -> 0x802AAE90
// this: r31
static void zCamWidget::start_transition() {}

// Range: 0x802AAE90 -> 0x802AAEF4
// this: r31
static void zCamWidget::finish_transition() {}
...
// total size: 0x10
struct /* @class$877zCamMarker_cpp */ {
    // Members
...
// Range: 0x802AAEF4 -> 0x802AAFF0
// this: r31
static void zCamWidget::refresh_end_mat() {
    // Local variables
    const struct xVec3 & target; // r30
    struct xVec3 at; // r1+0x8
}

// Range: 0x802AAFF0 -> 0x802AB09C
static void deactivate_camera(unsigned char force_cut /* r31 */, unsigned char restore_original /* r1+0x8 */) {
    // References
    // -> static struct zCamWidget * widget_cam;
    // -> static unsigned char activated;
}

// Range: 0x802AB09C -> 0x802AB11C
static void refresh_active_camera(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xCamAsset & a; // r31

    // References
    // -> static struct zCamWidget * widget_cam;
}

// Range: 0x802AB11C -> 0x802AB188
static void on_change_active(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xCamAsset & a; // r31
...
// Range: 0x802AB1B0 -> 0x802AB304
static void on_change_direction(const struct tweak_info & info /* r29 */) {
    // Local variables
    struct xCamAsset & a; // r30
    struct xVec3 & rdir; // r31
    struct xVec3 dir; // r1+0x1C
    struct xVec3 olddir; // r1+0x10
    struct xMat3x3 mat; // r1+0x30
}

// total size: 0xC
struct /* @class$813zCamMarker_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$814zCamMarker_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$815zCamMarker_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$816zCamMarker_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$817zCamMarker_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$818zCamMarker_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$819zCamMarker_cpp */ {
    // Members
...
static struct tweak_callback cb_active; // size: 0x28, address: 0x804CEE08
static struct tweak_callback cb_refresh; // size: 0x28, address: 0x804CEE30
static struct tweak_callback cb_direction; // size: 0x28, address: 0x804CEE58
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D662E0
// Range: 0x802AB304 -> 0x802AB570
static void add_tweaks(const char * prefix /* r29 */, struct xCamAsset & a /* r30 */) {
    // Local variables
    unsigned long il; // r28
    unsigned long ir; // r27
    struct xMat3x3 mat; // r1+0x10
    struct xVec3 & dir; // r31

    // References
    // -> static struct tweak_callback cb_direction;
    // -> static struct tweak_callback cb_refresh;
    // -> static struct tweak_callback cb_active;
    // -> static char __FUNCTION__[11];
}

// Range: 0x802AB570 -> 0x802AB5DC
static void on_explode_tweaks(const struct tweak_info & info /* r31 */) {
    // Local variables
    struct xCamAsset & a; // r30
    char prefix[128]; // r1+0x8
}

static struct tweak_callback cb_explode; // size: 0x28, address: 0x805267C8
static signed char init; // size: 0x1, address: 0x80D6B4F0
// Range: 0x802AB5DC -> 0x802AB6D4
static void add_tweak_stub(struct xCamAsset & a /* r31 */) {
    // Local variables
...
static char active_camera_name[64]; // size: 0x40, address: 0x80D2CEB8
static struct tweak_callback cb_deactivate; // size: 0x28, address: 0x804CEE80
// Range: 0x802AB6D4 -> 0x802AB754
static void add_global_tweaks() {
    // References
    // -> static struct tweak_callback cb_deactivate;
    // -> static char active_camera_name[64];
}
...
// Range: 0x802AB7DC -> 0x802AB80C
void zCamMarkerActivateCamera(const struct xCamAsset & a /* r1+0x8 */, struct xEnt * ent /* r1+0xC */) {}

// Range: 0x802AB80C -> 0x802AB87C
...
// Range: 0x802AB87C -> 0x802AB8E0
// this: r31
static void zCamWidget::create() {}

// Range: 0x802AB91C -> 0x802AB958
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D662EB
// Range: 0x802AB958 -> 0x802ABA1C
void zCamMarkerInit(struct xBase * b /* r31 */, struct xCamAsset * asset /* r30 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D662FA
// total size: 0x14
...
// total size: 0x0
struct xSerial {};
// Range: 0x802ABA1C -> 0x802ABB14
void zCamMarkerSave(struct zCamMarker * m /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D66309
// Range: 0x802ABB14 -> 0x802ABC0C
void zCamMarkerLoad(struct zCamMarker * m /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static unsigned char done; // size: 0x1, address: 0x80D6B4F1
static signed char init; // size: 0x1, address: 0x80D6B4F2
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D66318
// Range: 0x802ABC0C -> 0x802ABD6C
void zCamMarkerEventCB(struct xBase * to /* r31 */, unsigned int toEvent /* r1+0x8 */, struct xBase * toParamWidget /* r1+0xC */) {
    // References
...
// Range: 0x802ABD6C -> 0x802ABD9C
void zStaticCameraAsset_Init(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r1+0xC */) {}

// total size: 0x2C
struct zStaticCameraAsset : public xDynAsset {
    // Members
...
// total size: 0x14
struct zStaticCameraObject : public xBase {
    // Members
    struct zStaticCameraAsset * asset; // offset 0x10, size 0x4
};
// Range: 0x802ABD9C -> 0x802ABDE4
void zStaticCameraAsset_Init(struct zStaticCameraObject * object /* r31 */, struct zStaticCameraAsset * asset /* r30 */) {}

// Range: 0x802ABDE4 -> 0x802ABEA8
void zStaticCameraAsset_EventCB(struct xBase * to /* r1+0x8 */, unsigned int toEvent /* r1+0xC */, struct xBase * toParamWidget /* r1+0x10 */) {
    // Local variables
    const struct zStaticCameraObject & cam; // r30
    const struct zStaticCameraAsset & asset; // r31

    // References
    // -> static struct xCamAsset static_cam_asset;
}

unsigned char zCamPlayerPOI::ApplyAssetSettings(const xCamBinaryAsset&)::already_asserted; // size: 0x1, address: 0x80D6B4F3
signed char zCamPlayerPOI::ApplyAssetSettings(const xCamBinaryAsset&)::localstatic4 guard; // size: 0x1, address: 0x80D6B4F4
char __FUNCTION__$localstatic1$ApplyAssetSettings__13zCamPlayerPOIFRC15xCamBinaryAsset[19]; // size: 0x13, address: 0x80D6632A
// total size: 0x18
class zCamPool : public zCamPoolBase {};
static class zCamPool POICamPool; // size: 0x18, address: 0x80D2CEF8
// total size: 0x14
struct zCamBinaryPOI : public xBase {
    // Functions
    void Init(const struct xCamBinaryAsset & asset);

    unsigned char Activate(const struct xCamTransitionParams * transitionParams, unsigned char forceRestart);

    // Members
    const struct xCamBinaryAsset * mAsset; // offset 0x10, size 0x4
};
// Range: 0x802ABEA8 -> 0x802ABF0C
// this: r31
void zCamBinaryPOI::Init(const struct xCamBinaryAsset & asset /* r30 */) {}

// total size: 0x34
struct xCamBinaryAsset : public xDynAsset {
    // Members
    unsigned short mVersion; // offset 0x10, size 0x2
    unsigned short mFlags; // offset 0x12, size 0x2
    unsigned int mTargetID; // offset 0x14, size 0x4
    float mTargetMarginAngle; // offset 0x18, size 0x4
    float mTargetRadiusScale; // offset 0x1C, size 0x4
    float mDistanceScale; // offset 0x20, size 0x4
    float mPhiOffset; // offset 0x24, size 0x4
    float mYawOffset; // offset 0x28, size 0x4
    float mNearWallScale; // offset 0x2C, size 0x4
    float mFarWallScale; // offset 0x30, size 0x4
};
// Range: 0x802ABF0C -> 0x802ABF98
void zCamBinaryPOI::Init(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r1+0xC */) {
    // References
    // -> static class zCamPool POICamPool;
}
...
// Range: 0x802ABF98 -> 0x802ABFC0
void zCamBinaryPOI::SceneExit() {
    // References
    // -> static class zCamPool POICamPool;
}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D6633D
...
// Range: 0x802ABFC0 -> 0x802AC164
// this: r30
unsigned char zCamBinaryPOI::Activate(const struct xCamTransitionParams * transitionParams /* r1+0x8 */, unsigned char forceRestart /* r1+0xC */) {
    // Local variables
    struct xBase * target; // r29
    struct zCamPlayerPOI * cam; // r31
...
// Range: 0x802AC164 -> 0x802AC1DC
void zCamBinaryPOI::EventCB(struct xBase * to /* r1+0x8 */, unsigned int toEvent /* r1+0xC */, const float * toParam /* r30 */) {}

unsigned char mDebugDraw; // size: 0x1, address: 0x80D6B4F5
unsigned char sTweakedAssetActive; // size: 0x1, address: 0x80D6B4F6
struct xCamBinaryAsset sTweakedAsset; // size: 0x34, address: 0x80D2CF10
// total size: 0x4D0
struct zCamPlayerPOI : public zCam2Player {
    // Functions
    void update(struct xScene & scene, float dt);

    void ApplyAssetSettings(const struct xCamBinaryAsset & asset);

    void start();

    // Static members
    static unsigned char mDebugDraw; // size: 0x1
    static struct xCamBinaryAsset sTweakedAsset; // size: 0x34
    static unsigned char sTweakedAssetActive; // size: 0x1

    // Members
    const struct xCamBinaryAsset * mAsset; // offset 0x4C0, size 0x4
    struct xBase * mTarget; // offset 0x4C4, size 0x4
};
// Range: 0x802AC1DC -> 0x802AC1E8
void zCamPlayerPOI::asset_tweak_open() {
    // References
    // -> unsigned char sTweakedAssetActive;
}

// Range: 0x802AC1E8 -> 0x802AC1F4
void zCamPlayerPOI::asset_tweak_close() {
    // References
    // -> unsigned char sTweakedAssetActive;
}

static unsigned char tweaksAdded; // size: 0x1, address: 0x80D6B4F7
static signed char init; // size: 0x1, address: 0x80D6B4F8
static struct tweak_callback asset_tweak_cb; // size: 0x28, address: 0x805267F0
static signed char init; // size: 0x1, address: 0x80D6B4F9
static char * prefix; // size: 0x4, address: 0x80D6B4FC
static signed char init; // size: 0x1, address: 0x80D6B500
// Range: 0x802AC1F4 -> 0x802AC4B4
void zCamPlayerPOI::AddTweaks() {
    // Local variables
    struct xCamBinaryAsset & BinaryPOI; // r31

    // References
    // -> unsigned char mDebugDraw;
    // -> static char * prefix;
    // -> struct xCamBinaryAsset sTweakedAsset;
    // -> static signed char init;
    // -> static struct tweak_callback asset_tweak_cb;
    // -> static signed char init;
    // -> static unsigned char tweaksAdded;
    // -> static signed char init;
}

// total size: 0x4C
struct {} zCamPlayerPOI::__vtable; // size: 0x4C, address: 0x80526818
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x802AC4B4 -> 0x802AC564
// this: r31
void zCamPlayerPOI::update(struct xScene & scene /* r1+0x8 */, float dt /* r1+0xC */) {
    // References
...
// Range: 0x802AC564 -> 0x802AC864
// this: r31
void zCamPlayerPOI::ApplyAssetSettings(const struct xCamBinaryAsset & asset /* r30 */) {
    // Local variables
    struct xSphere entSphere; // r1+0x30
    class zNPCBaseIN2 * NPCBase; // r29
    struct xSphere entSphere; // r1+0x20
    struct rest_config newRest; // r1+0x10
    float nearWall; // r1+0xC
    float farWall; // r1+0x8

    // References
    // -> char __FUNCTION__$localstatic1$ApplyAssetSettings__13zCamPlayerPOIFRC15xCamBinaryAsset[19];
    // -> unsigned char zCamPlayerPOI::ApplyAssetSettings(const xCamBinaryAsset&)::already_asserted;
    // -> signed char zCamPlayerPOI::ApplyAssetSettings(const xCamBinaryAsset&)::localstatic4 guard;
}
...
// total size: 0x4
struct /* @class$1104zCamMarker_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1242zCamMarker_cpp */ {
    // Members
...
// total size: 0x18
struct /* @class$1842zCamMarker_cpp */ {
    // Members
...
// Range: 0x802AC864 -> 0x802AC8BC
// this: r31
void zCamPlayerPOI::start() {
    // References
    // -> struct xCamBinaryAsset sTweakedAsset;
}

// Range: 0x802AC948 -> 0x802AC968
void zCamPlayerPOI::add_tweaks() {}
...
static // total size: 0x4C
struct {} @unnamed@zCamMarker_cpp@::zCamWidget::__vtable; // size: 0x4C, address: 0x80526864
// Range: 0x802AC968 -> 0x802AC970
static float zCamWidget::getCameraPlayerAudioBias() {}

// Range: 0x802AC970 -> 0x802AC9EC
// this: r31
static void zCamWidget::start() {
    // References
    // -> static unsigned char activated;
}

// Range: 0x802AC9EC -> 0x802ACA48
// this: r31
static void zCamWidget::stop() {
    // References
    // -> static unsigned char activated;
}

// Range: 0x802ACA48 -> 0x802ACB74
// this: r31
static void zCamWidget::update(float dt /* r1+0x8 */) {
    // Local variables
    float t; // f30
    float s; // f31
    float qs; // f29
