/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zScene.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801A7760 -> 0x801B333C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
char __FUNCTION__$localstatic1$Bead__17zCameraCurveAssetCFi[5]; // size: 0x5, address: 0x80D636C0
char __FUNCTION__$localstatic1$Validate__17zCameraCurveAssetCFv[9]; // size: 0x9, address: 0x80D636C5
char __FUNCTION__$localstatic1$Validate__15xCamBinaryAssetCFv[9]; // size: 0x9, address: 0x80D636CE
// total size: 0x4
struct RwRGBA {
    // Members
...
char __FUNCTION__$localstatic1$render_immediate_pass__12xLightVolumeF10xColor_tagbb[22]; // size: 0x16, address: 0x805204B0
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
unsigned char DRAW_SAFE_AREA; // size: 0x1, address: 0x80D6A858
static int bytesNeeded; // size: 0x4, address: 0x80D6A85C
static int availOnDisk; // size: 0x4, address: 0x80D6A860
static int neededFiles; // size: 0x4, address: 0x80D6A864
static unsigned int enableScreenAdj; // size: 0x4, address: 0x80D6A868
static float offsetx; // size: 0x4, address: 0x80D6A86C
static float offsety; // size: 0x4, address: 0x80D6A870
static float oldOffsetx; // size: 0x4, address: 0x80D6A874
static float oldOffsety; // size: 0x4, address: 0x80D6A878
static float menuOldOffsetx; // size: 0x4, address: 0x80D6A87C
static float menuOldOffsety; // size: 0x4, address: 0x80D6A880
int g_idx_reloadTweaks; // size: 0x4, address: 0x80D636D8
static int sMemDepthSceneStart; // size: 0x4, address: 0x80D636DC
static int sMemDepthJustHIPStart; // size: 0x4, address: 0x80D636E0
static unsigned char sLoadingPackFile; // size: 0x1, address: 0x80D6A884
// Range: 0x801A7760 -> 0x801A7768
unsigned char zSceneIsLoadingPackFile() {
    // References
    // -> static unsigned char sLoadingPackFile;
}

static unsigned int sDebugSceneSwitchCount; // size: 0x4, address: 0x80D6A888
// total size: 0x20
struct xLinkAsset {
    // Members
...
static struct zSceneObjectInstanceDesc sInitTable[32]; // size: 0x380, address: 0x805204C8
// total size: 0x14
...
static struct dynamic_type_data dynamic_types[92]; // size: 0x730, address: 0x804B03D8
// Range: 0x801A7768 -> 0x801A77F4
static unsigned int count_dynamic_types(const char * name /* r1+0x8 */) {
    // Local variables
    unsigned int type; // r28
    int dyn_size; // r27
    unsigned int total; // r30
    int i; // r31
    unsigned int asset_size; // r1+0xC
    const struct xDynAsset * a; // r29
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D636E4
// Range: 0x801A77F4 -> 0x801A78FC
static void add_dynamic_types(struct zScene & s /* r29 */) {
    // Local variables
    unsigned long i; // r30
    const struct dynamic_type_data & d; // r31
    int count; // r28
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D636F6
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x801A78FC -> 0x801A7BAC
static unsigned int init_dynamic_type(struct zScene & s /* r30 */, unsigned int index /* r26 */, const struct dynamic_type_data & d /* r31 */) {
    // Local variables
    int count; // r24
    unsigned int type; // r23
    int dyn_size; // r22
    int i; // r27
    int cnt; // r25
    unsigned int asset_size; // r1+0x8
    struct xDynAsset * a; // r29
    struct xBase * b; // r28
...
// Range: 0x801A7BAC -> 0x801A7C30
static unsigned int init_dynamic_types(struct zScene & s /* r1+0x8 */, unsigned int index /* r29 */) {
    // Local variables
    unsigned long i; // r31
    const struct dynamic_type_data & d; // r30
...
static char __FUNCTION__[26]; // size: 0x1A, address: 0x80520848
// Range: 0x801A7C30 -> 0x801A7EA8
static unsigned int zSceneInitFunc_DefaultEnt(struct zScene * s /* r28 */, struct zSceneObjectInstanceDesc * desc /* r31 */, unsigned int base_idx /* r25 */) {
    // Local variables
    unsigned char * block; // r27
    int count; // r26
...
static char __FUNCTION__[23]; // size: 0x17, address: 0x80520862
// Range: 0x801A7EA8 -> 0x801A8170
static unsigned int zSceneInitFunc_Default(struct zScene * s /* r25 */, struct zSceneObjectInstanceDesc * desc /* r31 */, unsigned int base_idx /* r26 */) {
    // Local variables
    unsigned char * block; // r28
    int count; // r27
    unsigned int assetSize; // r1+0x8
    unsigned int offset; // r24
    struct xBase * b; // r30
...
// Range: 0x801A8170 -> 0x801A8248
static unsigned int zSceneInitFunc_MovePoint(struct zScene * s /* r26 */, struct zSceneObjectInstanceDesc * desc /* r27 */, unsigned int base_idx /* r28 */) {
    // Local variables
    int count; // r29
    unsigned int assetSize; // r1+0x8
    struct zMovePoint * movpBlock; // r25
    int idx; // r31
    struct xBase * b; // r30
    struct xBaseAsset * basset; // r24
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x80520879
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x801A8248 -> 0x801A8454
static unsigned int zSceneInitFunc_Player(struct zScene * s /* r30 */, struct zSceneObjectInstanceDesc * desc /* r25 */, unsigned int base_idx /* r27 */) {
    // Local variables
    int count; // r29
    struct zEnt * entBlock; // r24
    int idx; // r28
    struct xBase * b; // r31
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x8052088F
enum _tagTransType {
    eTransType_None = 0,
...
// Range: 0x801A8454 -> 0x801A8544
static unsigned int zSceneInitFunc_Camera(struct zScene * s /* r26 */, struct zSceneObjectInstanceDesc * desc /* r27 */, unsigned int base_idx /* r28 */) {
    // Local variables
    int count; // r29
    struct zCamMarker * camBlock; // r25
    int idx; // r31
    struct xBase * b; // r30
    struct xCamAsset * assetCam; // r24
...
// Range: 0x801A8544 -> 0x801A85E4
static unsigned int zSceneInitFunc_Surface(struct zScene * s /* r26 */, struct zSceneObjectInstanceDesc * desc /* r27 */, unsigned int base_idx /* r28 */) {
    // Local variables
    int count; // r29
    int idx; // r31
    struct xBase * b; // r30
...
// Range: 0x801A85E4 -> 0x801A8688
static unsigned int zSceneInitFunc_Volume(struct zScene * s /* r26 */, struct zSceneObjectInstanceDesc * desc /* r27 */, unsigned int base_idx /* r28 */) {
    // Local variables
    int count; // r29
    int idx; // r31
    struct xBase * b; // r30
...
// Range: 0x801A8688 -> 0x801A8690
static unsigned int zSceneInitFunc_LobMaster(unsigned int base_idx /* r5 */) {}

// total size: 0x0
struct st_ZDISPATCH_DATA {};
// Range: 0x801A8690 -> 0x801A8760
static unsigned int zSceneInitFunc_Dispatcher(struct zScene * s /* r25 */, struct zSceneObjectInstanceDesc * desc /* r26 */, unsigned int base_idx /* r27 */) {
    // Local variables
    int count; // r29
    struct st_ZDISPATCH_DATA * dpat_pool; // r28
    int idx; // r31
    struct xBase * b; // r30
    struct xEntAsset * asset; // r24
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D63708
// total size: 0x1C
...
static unsigned int sPipeSceneAssetID; // size: 0x4, address: 0x80D6A890
int xModelLookupCount; // size: 0x4, address: 0x80D692F4
// Range: 0x801A8828 -> 0x801A8874
static void PipeCountStuffCB(struct RpAtomic * data /* r1+0x8 */, struct xModelPipe & pipe /* r31 */) {
    // References
...
// Range: 0x801A8874 -> 0x801A88D4
static void PipeAddStuffCB(struct RpAtomic * data /* r1+0x8 */, struct xModelPipe & pipe /* r1+0xC */) {
    // References
    // -> int xModelLookupCount;
    // -> struct xModelPipeLookup * xModelLookupList;
}

static unsigned int modelToCheck; // size: 0x4, address: 0x80D6A894
static signed char init; // size: 0x1, address: 0x80D6A898
static char __FUNCTION__[22]; // size: 0x16, address: 0x805208A5
// total size: 0x10
...
struct xModelPipeTable * xModelPipeData[]; // size: 0x0, address: 0x80B865B4
int xModelPipeCount[]; // size: 0x0, address: 0x80B86574
...
// Range: 0x801A88D4 -> 0x801A8CB4
static void PipeForAllSceneModels(void (* pipeCB)(struct RpAtomic *, struct xModelPipe &, unsigned int) /* r22 */) {
    // Local variables
    int i; // r29
    int j; // r31
    int k; // r30
    int numModels; // r23
    struct RpAtomic * model; // r28
    struct PKRAssetTOCInfo ainfo; // r1+0xC
    unsigned int numSubObjects; // r26
    unsigned int remainSubObjBits; // r27
    unsigned int currSubObjBits; // r25
    struct RpAtomic * tempmodel; // r24
    int asdf; // r1+0x8
...
unsigned int gTransitionSceneID; // size: 0x4, address: 0x80D6A89C
// Range: 0x801A8CB4 -> 0x801A8E4C
static unsigned int BaseTypeNeedsUpdate(unsigned char baseType /* r3 */) {}

unsigned char gInSceneResetOrInit; // size: 0x1, address: 0x80D6A8A0
static int been_here; // size: 0x4, address: 0x80D6A8A4
static unsigned int num_ribbons; // size: 0x4, address: 0x80D6A8A8
static signed char init; // size: 0x1, address: 0x80D6A8AC
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D63712
...
// Range: 0x801A8E4C -> 0x801AA740
void zSceneInit(unsigned int theSceneID /* r1+0x8 */, int reloadInProgress /* r1+0xC */) {
    // Local variables
    char scenename[8]; // r1+0x78
    long long t; // r1+0x70
    float pdone; // f31
    struct zScene * s; // r31
    int i; // r30
    unsigned char rgba_bkgrd[4]; // r1+0x64
    char * pSceneId; // r29
    float percentageInc; // f30
    unsigned char loaded; // r1+0x14
    long long time; // r1+0x68
    int numDynAsset; // r1+0x60
    unsigned int asset_size; // r1+0x5C
    struct xDynAsset * asset; // r22
    unsigned long expected_asset_size; // r17
    unsigned int idleWrite; // r28
    unsigned int j; // r25
    struct iAnimFile * raw; // r21
    unsigned int j; // r24
    struct iAnimFile * raw; // r20
    int i; // r27
    void * data; // r19
    int typeCount; // r18
    int j; // r23
    int total_npcs; // r26
    int total_new_npcs; // r1+0x58
    int total_old_npcs; // r1+0x54
    int base_idx; // r1+0x50
    int curidx; // r1+0x4C
    int nobj; // r1+0x48
    int maxPolys; // r1+0x44
    int maxCaches; // r1+0x40
    int lkitCount; // r1+0x3C
    void * lkitData; // r1+0x38
    struct zPlatform * pobj; // r1+0x34
    int navMeshCount; // r1+0x30
    class xNavigationMeshAsset * navMesh; // r1+0x2C
    ...
    // -> static unsigned int sDebugSceneSwitchCount;
    // -> int gOccludeCount;
...
// Range: 0x801AA740 -> 0x801AA750
static void IncrementTweakReloadIndex() {
    // References
    // -> int g_idx_reloadTweaks;
}

// Range: 0x801AA750 -> 0x801AA8F8
void zScene_SetMinVSync() {
    // References
    // -> struct zGlobals globals;
}

static int dump_watermark; // size: 0x4, address: 0x80D6A8B0
static signed char init; // size: 0x1, address: 0x80D6A8B4
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D6371D
// total size: 0x8
struct /* @class$2867zScene_cpp */ {
    // Members
    float player; // offset 0x0, size 0x4
    float other; // offset 0x4, size 0x4
};
// total size: 0x1C
struct /* @class$2868zScene_cpp */ {
    // Members
...
// total size: 0x4
struct /* @class$2882zScene_cpp */ {
    // Members
    float phase; // offset 0x0, size 0x4
};
// total size: 0x1
struct /* @class$2883zScene_cpp */ {};
union /* @class$2881zScene_cpp */ {
    // total size: 0x4
    struct /* @class$2882zScene_cpp */ {
        // Members
        float phase; // offset 0x0, size 0x4
    ...
    // total size: 0x1
    struct /* @class$2883zScene_cpp */ {} flicker; // offset 0x0, size 0x1
};
// total size: 0x3C
struct motion_data {
    // Members
...
unsigned int SCENE_ID_MNU_START; // size: 0x4, address: 0x80D6A5B0
// total size: 0xA08
class zTimeHandlerMgr : public zQueue {};
class zTimeHandlerMgr timehandler_mgr; // size: 0xA08, address: 0x80D175B8
// Range: 0x801AA8F8 -> 0x801AAE4C
void zSceneExit(int beginReload /* r26 */) {
    // Local variables
    long long t; // r28
    int i; // r30
    struct zScene * s; // r31
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D63728
...
// Range: 0x801AAE4C -> 0x801AB46C
void zSceneSwitch(struct _zPortal * p /* r1+0x8 */, int forceSameScene /* r1+0xC */, unsigned char sameScene /* r23 */, int playerNum /* r31 */) {
    // Local variables
    long long t; // r19
    struct xPortalAsset * passet; // r30
    unsigned int nextSceneID; // r17
    unsigned int PlayerMarkerStartID; // r26
    unsigned int PlayerMarkerStartCamID; // r1+0x18
    float PlayerStartAngle; // f31
    unsigned char teleportPlayer; // r25
    struct pointer_asset * pointer; // r22
    struct xMarkerAsset * marker; // r18
    struct xEnt & player; // r24
    struct xVec3 & loc; // r1+0x14
    struct pointer_asset * pointer; // r27
    struct xEntFrame * frame; // r29
    unsigned int size; // r1+0x10
    struct xMarkerAsset * m; // r21
    struct xEntFrame * frame; // r28
...
int dumpBuffer; // size: 0x4, address: 0x80D6A8B8
int logTally; // size: 0x4, address: 0x80D6A8BC
int logEachItem; // size: 0x4, address: 0x80D6A8C0
int logSwitches; // size: 0x4, address: 0x80D6A8C4
static int cnt_saver[195]; // size: 0x30C, address: 0x805208BC
enum en_ZBASETYPE {
    eBaseTypeUnknown = 0,
    eBaseTypeTrigger = 1,
    eBaseTypeVillain = 2,
...
static enum en_ZBASETYPE typ_lastBase; // size: 0x4, address: 0x80D6A8C8
static int indyTallyLast; // size: 0x4, address: 0x80D6A8CC
int DBG_Persist::LogFinalStats(const xSerial*, unsigned int, int)::skipEmpties; // size: 0x4, address: 0x80D6A8D0
signed char DBG_Persist::LogFinalStats(const xSerial*, unsigned int, int)::localstatic4 guard; // size: 0x1, address: 0x80D6A8D4
int needToAddTweaks; // size: 0x4, address: 0x80D63738
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D6373C
// total size: 0x0
...
// Range: 0x801AB46C -> 0x801ABCE8
void zSceneSave(struct zScene * ent /* r26 */) {
    // Local variables
    long long t; // r24
    struct xSerial xser; // r1+0x8
    struct xSerial * s; // r31
    int i; // r29
    unsigned int serialSceneID; // r27
    int i; // r28
    struct xBase * b; // r30
...
// Range: 0x801ABCE8 -> 0x801ABE94
void LogFinalStats(const struct xSerial * xser /* r27 */, unsigned int serialSceneID /* r1+0x8 */, int wasSaving /* r28 */) {
    // Local variables
    char nam_scn[16]; // r1+0xC
    int i; // r31
...
// Range: 0x801ABE94 -> 0x801ABF38
void LogEachItem(const struct xSerial * xser /* r31 */, unsigned int aid_item /* r1+0x8 */) {
    // Local variables
    int diff; // r30
...
// Range: 0x801ABF38 -> 0x801ABF68
void IncTypeCount(const struct xSerial * xser /* r1+0x8 */, int bt /* r1+0xC */) {}

// Range: 0x801ABF68 -> 0x801ABFBC
void IncTypeCount(const struct xSerial * xser /* r1+0x8 */, enum en_ZBASETYPE bt /* r31 */) {
    // References
    // -> static int cnt_saver[195];
}

// Range: 0x801ABFBC -> 0x801AC088
void LogChkTypeSwitch(const struct xSerial * xser /* r1+0x8 */, enum en_ZBASETYPE bt /* r31 */) {
    // References
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D63747
// Range: 0x801AC20C -> 0x801ACA74
void zSceneLoad(struct zScene * ent /* r24 */) {
    // Local variables
    long long t; // r25
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80521150
// Range: 0x801ACA74 -> 0x801ACB08
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D63752
// Range: 0x801ACB08 -> 0x801AD0A0
void zSceneReset() {
    // Local variables
    long long t; // r26
    struct zScene * s; // r31
...
// Range: 0x801AD0A0 -> 0x801AD168
void zSceneStoreCheckPoint(unsigned int initialCameraID /* r1+0x8 */) {
    // Local variables
    struct zPlayerCheckPoint checkPoints[4]; // r1+0xC
    int i; // r31
...
// Range: 0x801AD168 -> 0x801AD19C
void zSceneStoreCheckPoint(struct zPlayerCheckPoint * checkPoints /* r1+0x8 */, unsigned int initialCameraID /* r1+0xC */) {}

// Range: 0x801AD19C -> 0x801AD1F0
static void ActivateCB(struct xBase * base /* r31 */) {}
...
// total size: 0x4
struct /* @class$2357zScene_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2358zScene_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// total size: 0x1
struct /* @class$873zScene_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$874zScene_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$875zScene_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$876zScene_cpp */ {
    // Members
...
// Range: 0x801AD1F0 -> 0x801AD244
static void DeactivateCB(struct xBase * base /* r31 */) {}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6375E
enum eGameMode {
    eGameMode_Boot = 0,
    eGameMode_Intro = 1,
    eGameMode_Title = 2,
...
// Range: 0x801AD244 -> 0x801AE3A4
void zSceneSetup() {
    // Local variables
    long long t; // r1+0xA0
    struct zScene * s; // r31
    unsigned int dontcaresize; // r1+0x9C
    int CutsceneTocCount; // r1+0x98
    int i; // r1+0x94
    int i; // r1+0x90
    struct xBase * b; // r29
    int i; // r1+0x8C
    int i; // r1+0x88
    int max_drivensort_iters; // r1+0x84
    unsigned int driven_swapped; // r1+0x80
    unsigned int i; // r19
    unsigned int j; // r1+0x7C
    struct xEnt * bdriven; // r1+0x78
    struct xBase * btmp; // r1+0x74
    struct zEntSimpleObj * * entList; // r1+0x70
    unsigned int entCount; // r1+0x6C
    struct xEnt * * entList; // r1+0x68
    int entCount; // r1+0x64
    int i; // r30
    int j; // r27
    unsigned int k; // r1+0x60
    unsigned int numPrimeMovers; // r26
    unsigned int numDriven; // r1+0x5C
    struct xEnt * ent; // r1+0x58
    int driverCount; // r1+0x54
    struct xEnt * ent; // r25
    unsigned int id; // r1+0x50
    unsigned int numGroups; // r18
    struct xGroup * grp; // r21
    unsigned int gcnt; // r1+0x4C
    struct xBase * gbase; // r1+0x48
    struct xEnt * gent; // r1+0x44
    struct xGroup * driveGroupList; // r28
    unsigned int currMover; // r1+0x40
    unsigned int allocsize; // r1+0x3C
    struct xGroupAsset * grpAssetList; // r1+0x38
    struct xBase * * grpBaseList; // r24
    unsigned int i; // r17
    int i; // r1+0x34
    struct xEnt * ent; // r1+0x30
    struct xGroupAsset * gasset; // r1+0x2C
    struct xEnt * other; // r23
    struct xGroup * * tempGrpList; // r1+0x28
    struct xGroup * * tempGrpCurr; // r20
    struct xGroup * grp; // r1+0x24
    unsigned int i; // r1+0x20
    union FloatAndVoid defaultDist; // r1+0x1C
    union FloatAndVoid lodDist; // r1+0x18
    struct zLODTable * lod; // r22
    struct xEnt * ent; // r1+0x14
...
// Range: 0x801AE3A4 -> 0x801AE460
void zSceneSetObjectLightKit(struct xLightKit * objLightKit /* r28 */) {
    // Local variables
    int f; // r30
    struct zScene * zsc; // r31
    struct xEnt * tgtent; // r29

    // References
    // -> struct zGlobals globals;
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80521504
// Range: 0x801AE460 -> 0x801AE5F8
void zSceneResetLightKits() {
    // Local variables
    int i; // r29
    struct _zEnv * zen; // r24
    struct xEnvAsset * easset; // r31
    struct xLightKit * objLightKit; // r23
    int lkitCount; // r22
    struct xLightKit * lkit; // r30
    struct xGroup * group; // r25
    unsigned int j; // r28
    unsigned int nitam; // r21
    struct xBase * itamz; // r27
    struct xEnt * entitam; // r26
...
// Range: 0x801AE5F8 -> 0x801AE6B4
int zSceneSetup_serialTraverseCB(unsigned int clientID /* r1+0x8 */, struct xSerial * xser /* r1+0xC */) {
    // Local variables
    char uiName[16]; // r1+0x14
    int val; // r1+0x10
    struct xBase * UIptr; // r31
}

float gSceneUpdateTime; // size: 0x4, address: 0x80D6A8D8
static struct xVec3 sOldPosPlayer; // size: 0xC, address: 0x80D6A8DC
static struct xVec3 sOldPosCamera; // size: 0xC, address: 0x80D6A8E8
static unsigned int sSuddenMove; // size: 0x4, address: 0x80D6A8F4
static unsigned int sUITimers[10]; // size: 0x28, address: 0x80D01678
static unsigned int init; // size: 0x4, address: 0x80D6A8F8
static signed char init; // size: 0x1, address: 0x80D6A8FC
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D6376A
...
class xFunctionGenerator : public xBase {
    ...
    const char * graphName; // offset 0x18, size 0x4
    float currentCycleWidth; // offset 0x1C, size 0x4
    float currentPulseWidth; // offset 0x20, size 0x4
    float currentTime; // offset 0x24, size 0x4
    float lastUpTime; // offset 0x28, size 0x4
    float lastDownTime; // offset 0x2C, size 0x4
};
// Range: 0x801AE6B4 -> 0x801AF44C
void zSceneUpdate(float elapsedSec /* f31 */) {
    // Local variables
    long long tsu; // r19
    int i; // r27
    int isPaused; // r28
    ...
    float apady; // f29
    int i; // r29
    struct zScene * pScene; // r30
    struct xBase * * ppBaseObjs; // r23
    struct xBase * bob; // r31
    struct xEnt * ent; // r26
    int timer; // r25
    char str[32]; // r1+0x8
    int timer; // r24
    class xFunctionGenerator * fg; // r18
...
unsigned int zbuffer_debug_lines; // size: 0x4, address: 0x80D63778
static void (* zSceneSpecialRender_curFunc[2])(); // size: 0x8, address: 0x80D6377C
static int zSceneSpecialRender_numFuncs; // size: 0x4, address: 0x80D6A900
// Range: 0x801AF44C -> 0x801AF4B0
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D63784
int gGCwaterDisable; // size: 0x4, address: 0x80D6AFB0
float sTimeElapsed; // size: 0x4, address: 0x80D6A1A4
// Range: 0x801AF4B0 -> 0x801AFD04
void zSceneRenderPreFX() {
    // Local variables
    struct zScene * s; // r31
    struct xScene * sc; // r29
    unsigned int shadowHackCase; // r23
    struct xEnt * * entptr; // r28
    struct xEnt * * entlast; // r21
    struct xEnt * ent; // r30
    int i; // r27
    struct xLightKit * objLightKit; // r24
    struct _zEnv * zen; // r22
    struct xEnvAsset * easset; // r26
    int i; // r25
...
static struct IDBasePair * scobj_idbps; // size: 0x4, address: 0x80D6A904
static int scobj_size; // size: 0x4, address: 0x80D63798
static int nidbps; // size: 0x4, address: 0x80D6379C
static char __FUNCTION__[23]; // size: 0x17, address: 0x805217C4
// Range: 0x801B037C -> 0x801B04BC
...
static char __FUNCTION__[23]; // size: 0x17, address: 0x805217DB
// Range: 0x801B04BC -> 0x801B05C0
...
// Range: 0x801B05C0 -> 0x801B05C8
static int zSceneObjHashtableUsage() {
    // References
    // -> static int nidbps;
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x805217F2
// Range: 0x801B05C8 -> 0x801B06F0
void zSceneObjHashtableAdd(unsigned int id /* r28 */, struct xBase * base /* r1+0x8 */) {
    // Local variables
    int haha; // r1+0xC
    int k; // r30
    int chkd; // r29
    struct IDBasePair * idbp; // r31
...
// Range: 0x801B06F0 -> 0x801B0784
static struct xBase * zSceneObjHashtableGet(unsigned int id /* r3 */) {
    // Local variables
    int k; // r30
    int chkd; // r29
    struct IDBasePair * idbp; // r31
...
// Range: 0x801B0784 -> 0x801B07AC
struct xBase * zSceneFindObject(unsigned int gameID /* r1+0x8 */) {}

// Range: 0x801B07AC -> 0x801B0820
char * zSceneGetName(unsigned int gameID /* r29 */) {
    // Local variables
    struct xBase * b; // r31
    const char * n; // r30
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D637A0
// Range: 0x801B0820 -> 0x801B08D8
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D637AE
// Range: 0x801B08D8 -> 0x801B0A34
void zSceneForAllBase(struct xBase * (* func)(struct xBase *, struct zScene *, void *) /* r29 */, void * data /* r1+0x8 */) {
    // Local variables
    struct zScene * s; // r31
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D637BF
// Range: 0x801B0A34 -> 0x801B0BB0
void zSceneForAllBase(struct xBase * (* func)(struct xBase *, struct zScene *, void *) /* r29 */, int baseType /* r1+0x8 */, void * data /* r1+0xC */) {
    // Local variables
    struct zScene * s; // r31
    unsigned short i; // r30
...
static int show_loadtime; // size: 0x4, address: 0x80D6A908
static signed char init; // size: 0x1, address: 0x80D6A90C
static int show_allscene; // size: 0x4, address: 0x80D6A910
static signed char init; // size: 0x1, address: 0x80D6A914
// Range: 0x801B0BB0 -> 0x801B0E84
void zSceneDebugModeCB() {
    // Local variables
    unsigned int scnid; // r30
    char * sceneStr; // r31
    ...
    // -> static int sMemDepthJustHIPStart;
    // -> static int sMemDepthSceneStart;
    // -> static int show_loadtime;
    // -> static int show_allscene;
    // -> struct zGlobals globals;
    // -> static signed char init;
    // -> static signed char init;
}

// Range: 0x801B0E84 -> 0x801B0EB4
static struct xBase * zSceneExitSoundIteratorCB(struct xBase * b /* r31 */) {}

// Range: 0x801B0EB4 -> 0x801B0EB8
void zSceneMemLvlChkCB() {}

// Range: 0x801B0EB8 -> 0x801B0FC0
...
// Range: 0x801B1064 -> 0x801B10FC
char * zSceneGetAreaName(unsigned int sceneID /* r1+0x8 */) {
    // Local variables
    int world; // r1+0x10
    int task; // r1+0xC
...
// Range: 0x801B115C -> 0x801B117C
unsigned int zSceneCalcProgress() {}

// Range: 0x801B117C -> 0x801B11A4
void zScene_UpdateFlyToInterface(float dt /* r1+0x8 */) {}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80D637D0
// Range: 0x801B11A4 -> 0x801B1354
void zSceneEnableVisited(struct zScene * s /* r31 */) {
    // Local variables
    unsigned int uiNameID; // r28
    char uiName[64]; // r1+0x8
    const char * sceneName; // r29
    int i; // r30
...
// Range: 0x801B1354 -> 0x801B135C
void zSceneEnableScreenAdj(unsigned int enable /* r3 */) {
    // References
    // -> static unsigned int enableScreenAdj;
}

// Range: 0x801B135C -> 0x801B1370
...
// Range: 0x801B13E0 -> 0x801B1400
void zSceneResetScreenAdj() {}

// Range: 0x801B1400 -> 0x801B1408
unsigned int zScene_ScreenAdjustMode() {
    // References
    // -> static unsigned int enableScreenAdj;
}

static int been_here; // size: 0x4, address: 0x80D6A918
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D637E4
// Range: 0x801B1408 -> 0x801B16A4
unsigned char zSceneLoadPackFile(unsigned int id /* r31 */, int options /* r30 */, unsigned char noLocalized /* r1+0x8 */, const char * filename /* r29 */) {
    // Local variables
    int versionNumber; // r28
    float pdone; // r1+0x10
    int versionNumber; // r27
...
// Range: 0x801B16A4 -> 0x801B17F8
unsigned char zSceneUpdateAsset(struct xBase & sceneObject /* r30 */, void * asset /* r29 */, unsigned int assetSize /* r1+0x8 */) {
    // Local variables
    int i; // r31
...
const char * * all_entity_labels; // size: 0x4, address: 0x80D6A928
unsigned int * all_entity_ids; // size: 0x4, address: 0x80D6A92C
unsigned int all_entity_count; // size: 0x4, address: 0x80D6A930
const char * * all_models_labels; // size: 0x4, address: 0x80D6A934
unsigned int * all_models_ids; // size: 0x4, address: 0x80D6A938
struct RpAtomic * * all_models_pointers; // size: 0x4, address: 0x80D6A93C
int all_models_count; // size: 0x4, address: 0x80D6A940
// Range: 0x801B17F8 -> 0x801B1830
static int icompare_strings(void * e1 /* r1+0x8 */, void * e2 /* r1+0xC */) {}
...
struct TexKAndL * texKAndL; // size: 0x4, address: 0x80D6A944
// total size: 0x28
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D637F7
// Range: 0x801B1830 -> 0x801B1BA4
static void add_texture_tweaks() {
    // Local variables
    char * label_buffer; // r31
    char * tweak_buffer; // r27
    char * * labels; // r25
    unsigned int * ids; // r23
    int size; // r29
    int i; // r24
...
// Range: 0x801B1BA4 -> 0x801B1F8C
static void add_entity_tweaks() {
    // Local variables
    struct zSceneObjectInstanceDesc * it; // r27
    const struct dynamic_type_data * it; // r26
    const struct dynamic_type_data * end; // r1+0x1C
    char * label_buffer; // r30
    const char * * labels; // r25
    unsigned int * ids; // r21
    int index; // r28
    struct zSceneObjectInstanceDesc * it; // r31
    int i; // r20
    int end; // r1+0x18
    struct PKRAssetTOCInfo info; // r1+0x48
    const char * name; // r19
    int i; // r22
    int end; // r1+0x14
    unsigned char is_ent; // r18
    const struct xDynAsset * a; // r17
    const struct dynamic_type_data * it; // r29
    const struct dynamic_type_data * myend; // r1+0x10
    struct PKRAssetTOCInfo info; // r1+0x28
    const char * name; // r1+0xC
    int i; // r23
    char * s; // r24
...
// Range: 0x801B1F8C -> 0x801B1FDC
static void convert_model_to_visibility(const struct tweak_info & info /* r29 */, void * value /* r1+0x8 */) {
    // Local variables
    struct RpAtomic * input; // r30
    unsigned char * output; // r31
}

// Range: 0x801B1FDC -> 0x801B2050
static void convert_visibility_to_model(const struct tweak_info & info /* r28 */, void * value /* r1+0x8 */) {
    // Local variables
    unsigned char * input; // r30
    struct RpAtomic * output; // r29
    unsigned int flags; // r31
}

// Range: 0x801B2050 -> 0x801B20CC
static void disable_all_models() {
    // Local variables
    int i; // r31
    struct RpAtomic * model; // r30
...
static struct tweak_callback cb; // size: 0x28, address: 0x80521808
static signed char init; // size: 0x1, address: 0x80D6A948
static unsigned char dummy; // size: 0x1, address: 0x80D6A949
static signed char init; // size: 0x1, address: 0x80D6A94A
static struct tweak_callback cb_disable_all; // size: 0x28, address: 0x804B0B08
static struct tweak_callback cb_enable_all; // size: 0x28, address: 0x804B0B30
static struct tweak_callback cb_toggle_all; // size: 0x28, address: 0x804B0B58
// total size: 0xC
...
// Range: 0x801B21C4 -> 0x801B252C
static void prepare_model_list() {
    // Local variables
    int modelCount; // r28
    struct ModelTweakInit * tweaks; // r31
    unsigned int runningCount; // r30
    int i; // r27
...
// Range: 0x801B252C -> 0x801B2530
static void model_poly_count_helper() {}

static char * * asset_map_labels[195]; // size: 0x30C, address: 0x80D016A0
static unsigned int * asset_map_ids[195]; // size: 0x30C, address: 0x80D019AC
static unsigned int asset_map_size[195]; // size: 0x30C, address: 0x80D01CB8
static const char * none_label; // size: 0x4, address: 0x80D6A94C
static signed char init; // size: 0x1, address: 0x80D6A950
static const char * * empty_labels; // size: 0x4, address: 0x80D6A954
static signed char init; // size: 0x1, address: 0x80D6A958
static const unsigned int * empty_ids; // size: 0x4, address: 0x80D6A95C
static signed char init; // size: 0x1, address: 0x80D6A960
static unsigned int none_id; // size: 0x4, address: 0x80D6E6F8
// Range: 0x801B2530 -> 0x801B2974
static void create_widget_maps() {
    ...
    char * label; // r26
    char * * labels; // r29
    struct zSceneObjectInstanceDesc * it; // r25
    struct PKRAssetTOCInfo info; // r1+0x30
    unsigned int id; // r17
    const struct dynamic_type_data * it; // r24
    const struct dynamic_type_data * c_dtd_end; // r1+0xC
    const struct xDynAsset * a; // r20
    const struct dynamic_type_data * it; // r27
    const struct dynamic_type_data * myend; // r19
    struct PKRAssetTOCInfo info; // r1+0x10
    unsigned int & msize; // r22
    unsigned int * ids; // r23
...
static struct raster_pair * raster_map; // size: 0x4, address: 0x80D6A964
static int raster_map_size; // size: 0x4, address: 0x80D6A968
// Range: 0x801B2974 -> 0x801B2A58
...
static struct asset_type_tweak_map * asset_type_maps; // size: 0x4, address: 0x80D6A96C
static int asset_type_maps_size; // size: 0x4, address: 0x80D6A970
// total size: 0xC
...
// Range: 0x801B2A58 -> 0x801B2AA8
static int compare_temp_asset_info(void * e1 /* r3 */, void * e2 /* r4 */) {
    // Local variables
    const struct temp_asset_info & t1; // r31
    const struct temp_asset_info & t2; // r30
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x80521830
// Range: 0x801B2AA8 -> 0x801B3030
static void create_asset_type_map() {
    // Local variables
    unsigned int * static_types; // r27
    int static_types_size; // r1+0x30
    int total_assets; // r24
    int i; // r18
    struct temp_asset_info * assinfo_buffer; // r29
    struct temp_asset_info * assinfo; // r30
    int i; // r21
    int type_assets; // r1+0x2C
    int j; // r20
    struct xDynAsset * dynass; // r1+0x28
    struct PKRAssetTOCInfo ainfo; // r1+0x34
    unsigned int last_type; // r1+0x24
    unsigned int last_dyn_type; // r1+0x20
    int i; // r17
    const struct temp_asset_info & info; // r26
    int type_index; // r28
    int i; // r19
    const struct temp_asset_info & info; // r25
    int i; // r1+0x1C
    struct asset_type_tweak_map & atype; // r31
    int temp_index; // r1+0x18
    int j; // r23
    const struct temp_asset_info & info; // r1+0x14
    char * label; // r1+0x10
    int label_size; // r1+0xC
    int j; // r22
    const char * label; // r1+0x8
...
// Range: 0x801B3030 -> 0x801B3064
static void add_scene_tweaks() {}

// Range: 0x801B3064 -> 0x801B307C
unsigned int zSceneGetTextureTweakMap(const char * * & labels /* r3 */, const unsigned int * & ids /* r4 */) {
    // References
    // -> unsigned int all_texture_count;
    // -> unsigned int * all_texture_ids;
    // -> const char * * all_texture_labels;
...
// Range: 0x801B307C -> 0x801B3094
unsigned int zSceneGetModelTweakMap(const char * * & labels /* r3 */, const unsigned int * & ids /* r4 */) {
    // References
    // -> int all_models_count;
    // -> unsigned int * all_models_ids;
    // -> const char * * all_models_labels;
...
// Range: 0x801B3094 -> 0x801B30AC
unsigned int zSceneGetEntityTweakMap(const char * * & labels /* r3 */, const unsigned int * & ids /* r4 */) {
    // References
    // -> unsigned int all_entity_count;
    // -> unsigned int * all_entity_ids;
    // -> const char * * all_entity_labels;
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x80521846
// Range: 0x801B30AC -> 0x801B3198
...
static const char * empty_label; // size: 0x4, address: 0x80D6A978
static signed char init; // size: 0x1, address: 0x80D6A97C
static unsigned int empty_id; // size: 0x4, address: 0x80D6E6FC
// total size: 0x14
...
// Range: 0x801B3198 -> 0x801B3270
unsigned int zSceneGetAssetTweakMap(const char * * & labels /* r29 */, const unsigned int * & ids /* r30 */, unsigned int asset_type /* r1+0x8 */, unsigned int dyn_type /* r1+0xC */) {
    // Local variables
    struct asset_type_tweak_map key; // r1+0x14
    struct asset_type_tweak_map * end; // r28
    struct asset_type_tweak_map * found; // r31
...
// Range: 0x801B3270 -> 0x801B32F8
unsigned int zSceneGetTextureIDFromRaster(struct RwRaster * raster /* r1+0x8 */) {
    // Local variables
    struct raster_pair * raster_map_end; // r30
    struct raster_pair * found; // r31
...
static unsigned int gLastSceneID; // size: 0x4, address: 0x80D6A980
// Range: 0x801B32F8 -> 0x801B3300
void zSceneSetLastSceneID(unsigned int value /* r3 */) {
    // References
...
// Range: 0x801B3300 -> 0x801B3330
static int compare_asset_type_tweak_map::operator()(const struct asset_type_tweak_map & t1 /* r4 */, const struct asset_type_tweak_map & t2 /* r5 */) {}

// total size: 0x1
struct compare_raster {};
// Range: 0x801B3330 -> 0x801B333C
static int compare_raster::operator()(struct RwRaster * raster /* r4 */, const struct raster_pair & b /* r5 */) {}
