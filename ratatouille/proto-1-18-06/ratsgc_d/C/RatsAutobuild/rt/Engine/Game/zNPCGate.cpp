/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zNPCGate.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x803388C0 -> 0x80339A54
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
class zNPCGateMgr {
    // Functions
    void Assign(class zNPCGate * npcGate);

    // Members
    int numberOfGates; // offset 0x0, size 0x4
    class zNPCGate * * npcGates; // offset 0x4, size 0x4
    int curGate; // offset 0x8, size 0x4
};
static class zNPCGateMgr gNPCGateMgr; // size: 0xC, address: 0x80D6BB18
// Range: 0x803388C0 -> 0x80338908
void zNPCGateMgr::SceneSetup() {
    // References
    // -> static class zNPCGateMgr gNPCGateMgr;
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80D67B98
// Range: 0x80338908 -> 0x80338A1C
// this: r31
void zNPCGateMgr::Assign(class zNPCGate * npcGate /* r1+0x8 */) {
    // References
    // -> static char __FUNCTION__[7];
}

// Range: 0x80338A1C -> 0x80338A6C
void zNPCGateMgr::SceneUpdate() {
    // Local variables
    int i; // r31
...
// Range: 0x80338A6C -> 0x80338ADC
void zNPCGateMgr::SceneExit() {
    // Local variables
    int i; // r31
...
// Range: 0x80338ADC -> 0x80338B2C
void zNPCGateMgr::HandlePlayerCollisions() {
    // Local variables
    int i; // r31
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D67B9F
// total size: 0x20
struct xLinkAsset {
    // Members
...
// total size: 0x20
struct zNPCGateAsset : public xDynAsset {
    // Members
    unsigned int gateTrigger; // offset 0x10, size 0x4
    unsigned int gateFacing; // offset 0x14, size 0x4
    unsigned int npcsGroup; // offset 0x18, size 0x4
...
enum eNPCGateDir {
    ...
    eNPCGateDir_Right = 1,
    eNPCGateDir_AtM = 2,
    eNPCGateDir_RightM = 3,
};
// total size: 0x58
class zNPCGate : public xBase {
    // Functions
    zNPCGate(struct zNPCGateAsset * asset);

    void Setup();

    void SetupAttachedNPCs();

    void SetupPhysicalData();

    void Update();

    float ConvertWorldToGateLocalZ(const struct xVec3 & iWorld);

    void ConvertWorldToBoxLocal(const struct xVec3 & iWorld, struct xVec3 & oLocal);

    void ConvertBoxLocalToWorld(const struct xVec3 & iLocal, struct xVec3 & oWorld);
    ...
    // Members
    struct zNPCGateAsset * npcGateAsset; // offset 0x10, size 0x4
    int numberOfNPCs; // offset 0x14, size 0x4
    class zNPCBase * * npcPointers; // offset 0x18, size 0x4
    struct zEntTrigger * gateTrigger; // offset 0x1C, size 0x4
    struct xBox gateBox; // offset 0x20, size 0x18
    struct xMat4x3 * gateMat; // offset 0x38, size 0x4
    enum eNPCGateDir gateFacing; // offset 0x3C, size 0x4
    struct xVec3 gateDir; // offset 0x40, size 0xC
    float gateMax; // offset 0x4C, size 0x4
    float gateMin; // offset 0x50, size 0x4
    unsigned char gateIsOn; // offset 0x54, size 0x1
    unsigned char gateIsEnabled; // offset 0x55, size 0x1
};
// Range: 0x80338B2C -> 0x80338D0C
void zNPCGate::InitDynamic(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r28 */) {
    // Local variables
...
// Range: 0x80338D0C -> 0x80338D80
// this: r31
zNPCGate::zNPCGate(struct zNPCGateAsset * asset /* r30 */) {}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D67BAB
// Range: 0x80338D80 -> 0x80338E4C
void zNPCGate::EventHandler(struct xBase * to /* r30 */, unsigned int toEvent /* r1+0x8 */) {
    // Local variables
    class zNPCGate * npcGate; // r31

    // References
    // -> static char __FUNCTION__[13];
}

// Range: 0x80338E4C -> 0x80338E8C
// this: r31
void zNPCGate::Setup() {
    // References
    // -> static class zNPCGateMgr gNPCGateMgr;
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D67BB8
// total size: 0xC
...
// Range: 0x80338E8C -> 0x80339114
// this: r31
void zNPCGate::SetupAttachedNPCs() {
    // Local variables
    struct xBase * b; // r30
    struct xGroup * npcsGroup; // r23
    int numberInGroup; // r22
    int i; // r29
    struct xBase * b; // r28
    class zNPCBase * npc; // r20
    int curNPC; // r27
    int i; // r26
    struct xBase * b; // r25
    class zNPCBase * npc; // r21
    class zNPCBase * children; // r1+0xC
    int numChildren; // r1+0x8
    int j; // r24
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D67BCA
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x80339114 -> 0x80339464
// this: r31
void zNPCGate::SetupPhysicalData() {
    // Local variables
    struct xBase * b; // r30
    struct xSphere * player_spheres[5]; // r1+0xC
    int num_spheres; // r1+0x8
    float playerBound; // f30
    float trigSizeInc; // f31
...
// Range: 0x80339464 -> 0x80339610
// this: r31
void zNPCGate::Update() {
    // Local variables
    struct xVec3 playerPos; // r1+0x8
    float gatePos; // f31
    int i; // r30
    const struct xVec3 * npcPos; // r28
    class zICompNPCEntity * npcEnt; // r29
    float npcGatePos; // f30
...
// total size: 0x1
struct /* @class$1025zNPCGate_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$1026zNPCGate_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$1027zNPCGate_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$1028zNPCGate_cpp */ {
    // Members
...
// Range: 0x80339610 -> 0x80339668
// this: r31
float zNPCGate::ConvertWorldToGateLocalZ(const struct xVec3 & iWorld /* r1+0x8 */) {
    // Local variables
    struct xVec3 diff; // r1+0xC
}

// Range: 0x80339668 -> 0x80339700
// this: r29
void zNPCGate::ConvertWorldToBoxLocal(const struct xVec3 & iWorld /* r1+0x8 */, struct xVec3 & oLocal /* r30 */) {
    // Local variables
    struct xVec3 diff; // r1+0xC
    struct xVec3 * matAt; // r31
}

// Range: 0x80339700 -> 0x8033979C
// this: r28
void zNPCGate::ConvertBoxLocalToWorld(const struct xVec3 & iLocal /* r31 */, struct xVec3 & oWorld /* r29 */) {
    // Local variables
    struct xVec3 * matAt; // r30
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x8052E8E0
// Range: 0x8033979C -> 0x80339A04
// this: r31
void zNPCGate::HandlePlayerCollision() {
    // Local variables
    struct xVec3 * playerOldPos; // r28
    struct xVec3 * playerDPos; // r27
    struct xVec3 playerNewPos; // r1+0x24
    struct xVec3 localPlayerNewPos; // r1+0x18
    float wallDistances[4]; // r1+0x8
    int closestWall; // r29
    float closestWallDist; // f31
    int i; // r30
...
// Range: 0x80339A04 -> 0x80339A30
// this: r3
void zNPCGate::Destroy() {}
