/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zContextObjectClimbableRope.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x802DFBFC -> 0x802E169C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static float ROPE_RADIUS; // size: 0x4, address: 0x80D66C28
static int ROPE_SEGMENTS; // size: 0x4, address: 0x80D66C2C
static float SPEED_Y_MIN; // size: 0x4, address: 0x80D66C30
static float SPEED_XZ_MIN; // size: 0x4, address: 0x80D66C34
static float MOUNT_RADIUS; // size: 0x4, address: 0x80D66C38
static float DIR_COS; // size: 0x4, address: 0x80D66C3C
static float Y_ABOVE; // size: 0x4, address: 0x80D66C40
static float Y_BELOW; // size: 0x4, address: 0x80D66C44
static float SWING_DIST; // size: 0x4, address: 0x80D66C48
static float SPEED_UP; // size: 0x4, address: 0x80D66C4C
static float SPEED_DOWN; // size: 0x4, address: 0x80D66C50
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
static struct xVec3 GRAB_POINT_UP; // size: 0xC, address: 0x80D66C54
static struct xVec3 GRAB_POINT_DOWN; // size: 0xC, address: 0x80D66C60
static float SWING_PERIOD; // size: 0x4, address: 0x80D66C6C
static float SWING_ANGULAR_FREQ; // size: 0x4, address: 0x80D6B700
static float SWING_ANGLE_MAX; // size: 0x4, address: 0x80D66C70
static float SWING_INPUT_ACCEL; // size: 0x4, address: 0x80D66C74
static float SWING_DAMPEN_K; // size: 0x4, address: 0x80D66C78
static float SWING_DAMPEN_K_FREE; // size: 0x4, address: 0x80D66C7C
// Range: 0x802DFBFC -> 0x802DFC10
static void RecalcAngFreq() {
    // References
    // -> static float SWING_PERIOD;
    // -> static float SWING_ANGULAR_FREQ;
}
...
// total size: 0x4
struct /* @class$940zContextObjectClimbableRope_cpp */ {
    // Members
...
union /* @class$1079zContextObjectClimbableRope_cpp */ {
...
// total size: 0xC
struct /* @class$1078zContextObjectClimbableRope_cpp */ {
    // Members
...
class zCollidable : public xEnt {
    ...
    void SetupCollision(const struct xBox & box);

    void UpdateCollision(const struct xMat4x3 & mat);

    void UpdateCollision(const struct xVec3 & pos);

    // Members
protected:
    struct xMat4x3 collisionMat; // offset 0xE0, size 0x40
};
// Range: 0x802DFC10 -> 0x802DFC60
// this: r31
zCollidable::zCollidable() {}
...
// Range: 0x802DFC60 -> 0x802DFD38
// this: r31
void zCollidable::SetupCollision(const struct xBox & box /* r30 */) {}

// Range: 0x802DFD38 -> 0x802DFD80
// this: r30
void zCollidable::UpdateCollision(const struct xMat4x3 & mat /* r31 */) {}

// Range: 0x802DFD80 -> 0x802DFDF4
// this: r31
void zCollidable::UpdateCollision(const struct xVec3 & pos /* r30 */) {
    // Local variables
...
// total size: 0xC
struct /* @class$881zContextObjectClimbableRope_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$882zContextObjectClimbableRope_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$883zContextObjectClimbableRope_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$884zContextObjectClimbableRope_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$885zContextObjectClimbableRope_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$886zContextObjectClimbableRope_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$887zContextObjectClimbableRope_cpp */ {
    // Members
...
static struct tweak_callback recalcAngFreqCB; // size: 0x28, address: 0x804D56A8
struct tweak_callback cb_tweak_convert_cos; // size: 0x28, address: 0x804780B8
// total size: 0x24
struct Asset : public xDynAsset {
    // Members
    unsigned int mountObjectID; // offset 0x10, size 0x4
    float length; // offset 0x14, size 0x4
    float yaw; // offset 0x18, size 0x4
    unsigned int textureID; // offset 0x1C, size 0x4
    float textureLength; // offset 0x20, size 0x4
};
// total size: 0x2D0
class zContextObjectClimbableRope : public zContextObject {
    // Functions
    unsigned char Ascend(float dt);

    unsigned char Descend(float dt);

    void SwingInput(float stickMagnitude);

    void UpdateClimberFrame();

    void Update(float dt);

    void Render() const;

    zContextObjectClimbableRope(struct Asset * theAsset);

    void Setup();

    struct xVec3 & GetPos() const;

    unsigned char CanMount(struct zCommonPlayer * player, float & priority) const;

    unsigned char Mount(struct zCommonPlayer * player, float dt);

    void Reset();

    void ClearCollisionFlags();

    void RestoreCollisionFlags();

    // Members
protected:
    class zCollidable collidables[2]; // offset 0x20, size 0x240
    struct Asset * asset; // offset 0x260, size 0x4
    struct RwRaster * raster; // offset 0x264, size 0x4
    union { // inferred
        struct xEnt * ent; // offset 0x268, size 0x4
        struct xVec3 pos; // offset 0x268, size 0xC
    };
    float swingAngle; // offset 0x274, size 0x4
    float swingAngleVel; // offset 0x278, size 0x4
    float swingInput; // offset 0x27C, size 0x4
    struct xMat3x3 orientation; // offset 0x280, size 0x30
    struct xEnt * climberEnt; // offset 0x2B0, size 0x4
    float climberRopePos; // offset 0x2B4, size 0x4
    struct xVec3 climberHandPos; // offset 0x2B8, size 0xC
    unsigned char attachEnt; // offset 0x2C4, size 0x1
};
// Range: 0x802DFDF4 -> 0x802E0138
void zContextObjectClimbableRope::AddStaticTweaks() {
    // Local variables
    const char * prefix; // r31

    // References
    // -> static struct tweak_callback recalcAngFreqCB;
    // -> static float SWING_PERIOD;
    // -> static float SWING_DAMPEN_K_FREE;
    // -> static float SWING_DAMPEN_K;
    // -> static float SWING_INPUT_ACCEL;
    // -> static struct xVec3 GRAB_POINT_DOWN;
    // -> static struct xVec3 GRAB_POINT_UP;
    // -> static float SPEED_DOWN;
    // -> static float SPEED_UP;
    // -> static float Y_BELOW;
    // -> static float Y_ABOVE;
    // -> struct tweak_callback cb_tweak_convert_cos;
    // -> static float DIR_COS;
    // -> static float MOUNT_RADIUS;
    // -> static float SPEED_XZ_MIN;
    // -> static float SPEED_Y_MIN;
    // -> static int ROPE_SEGMENTS;
    // -> static float ROPE_RADIUS;
}

// Range: 0x802E0138 -> 0x802E0174
// this: r3
unsigned char zContextObjectClimbableRope::Ascend(float dt /* f1 */) {
    // References
    // -> static float SPEED_UP;
}

// Range: 0x802E0174 -> 0x802E01E0
// this: r31
unsigned char zContextObjectClimbableRope::Descend(float dt /* r1+0x8 */) {
    // References
    // -> static float SPEED_DOWN;
}

// Range: 0x802E01E0 -> 0x802E01E8
// this: r3
void zContextObjectClimbableRope::SwingInput(float stickMagnitude /* f1 */) {}

// Range: 0x802E01E8 -> 0x802E0254
// this: r31
void zContextObjectClimbableRope::UpdateClimberFrame() {
    // Local variables
...
// Range: 0x802E0254 -> 0x802E070C
// this: r31
void zContextObjectClimbableRope::Update(float dt /* f28 */) {
    // Local variables
    float swingAngleMax; // f30
    float gravityAcceleration; // f27
    float acceleration; // f21
    float newVel; // f31
    float maxVel; // f29
    float prevSwingAngle; // r1+0x14
    struct xVec3 ropeUp; // r1+0x18
    struct xBBox & topBox; // r30
    struct xMat4x3 topMat; // r1+0x30
    struct xBBox & bottomBox; // r29
    int i; // r28

    // References
    // -> struct xColor_tag g_RED;
    // -> static float SWING_ANGLE_MAX;
    // -> static float SWING_DAMPEN_K_FREE;
    // -> static float SWING_DAMPEN_K;
    // -> static float SWING_ANGULAR_FREQ;
    // -> static float SWING_INPUT_ACCEL;
    // -> static float SWING_DIST;
}
...
// total size: 0xC
class xIMVertexIterator {
    // Members
public:
    struct xVec3 * pos; // offset 0x0, size 0x4
    struct xVec2 * uv; // offset 0x4, size 0x4
    struct xColor_tag * color; // offset 0x8, size 0x4
};
// Range: 0x802E070C -> 0x802E0C34
// this: r31
void zContextObjectClimbableRope::Render() const {
    // Local variables
    int vertexCount; // r28
    struct xIMLockContext lock; // r1+0x78
    class xIMVertexIterator currentVertex; // r1+0x6C
    float swingAngleCos; // f22
    float swingAngleSin; // f21
    float swingAngleTan; // r1+0x8
    struct xVec3 topUp; // r1+0x60
    struct xVec3 topRight; // r1+0x54
    float bottomAngle; // f29
    float bottomAngleCos; // f20
    float bottomAngleSin; // f19
    struct xVec3 bottomUp; // r1+0x48
    struct xVec3 bottomRight; // r1+0x3C
    int i; // r30
    float fraction; // f31
    float angle; // f28
    float sinRadius; // f27
    float cosRadius; // f26
    struct xVec3 topPos; // r1+0x30
    struct xVec3 bottomPos; // r1+0x24
    struct xVec3 endPos; // r1+0x18
    int i; // r29
    float fraction; // f30
    float angle; // f25
    float sinRadius; // f24
    float cosRadius; // f23
    struct xVec3 bottomPos; // r1+0xC
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D66C80
...
// Range: 0x802E0C34 -> 0x802E0D40
void zContextObjectClimbableRope::RenderAll() {
    // Local variables
    unsigned int count; // r27
    class zContextObjectClimbableRope * ropeList; // r30
    unsigned char * rendered; // r29
    unsigned int i; // r31
    unsigned int j; // r28
...
// total size: 0x14
struct {} zContextObjectClimbableRope::__vtable; // size: 0x14, address: 0x80D66C90
struct xVec3 m_UnitAxisY; // size: 0xC, address: 0x80D6D944
// Range: 0x802E0D40 -> 0x802E0DD8
// this: r31
zContextObjectClimbableRope::zContextObjectClimbableRope(struct Asset * theAsset /* r1+0x8 */) {
    // References
    // -> struct xVec3 m_UnitAxisY;
    // -> struct [anonymous] zContextObjectClimbableRope::__vtable;
}

static char __FUNCTION__[5]; // size: 0x5, address: 0x80D66C8A
// Range: 0x802E0DD8 -> 0x802E0FAC
void zContextObjectClimbableRope::Init(struct xBase & data /* r1+0x8 */, struct xDynAsset & asset /* r31 */, unsigned long asset_size /* r28 */) {
    // Local variables
...
// Range: 0x802E0FAC -> 0x802E1108
// this: r31
void zContextObjectClimbableRope::Setup() {
    // Local variables
    unsigned int size; // r1+0xC
    void * posAsset; // r30
    struct RwTexture * texture; // r29
    struct RwTexture * texture; // r1+0x8
...
// Range: 0x802E1108 -> 0x802E114C
// this: r31
struct xVec3 & zContextObjectClimbableRope::GetPos() const {}

// Range: 0x802E114C -> 0x802E12E0
// this: r28
unsigned char zContextObjectClimbableRope::CanMount(struct zCommonPlayer * player /* r29 */, float & priority /* r30 */) const {
    // Local variables
    struct xVec3 playerPos; // r1+0x14
    const struct xVec3 & ropePos; // r31
    struct xVec3 dir; // r1+0x8
    float length2; // f31
    float currPriority; // f30
...
// total size: 0x890
struct /* @class$1903zContextObjectClimbableRope_cpp */ {
    // Static members
...
// total size: 0x4
struct /* @class$1508zContextObjectClimbableRope_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$1509zContextObjectClimbableRope_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// total size: 0x4
struct /* @class$2050zContextObjectClimbableRope_cpp */ {
    // Members
...
// total size: 0x8
struct /* @class$2049zContextObjectClimbableRope_cpp */ {
    // Members
    float time; // offset 0x0, size 0x4
...
class zNPCBase {};
// total size: 0x2
struct /* @class$2052zContextObjectClimbableRope_cpp */ {
    // Members
...
// total size: 0x10
struct /* @class$1660zContextObjectClimbableRope_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x802E12E0 -> 0x802E1584
// this: r30
unsigned char zContextObjectClimbableRope::Mount(struct zCommonPlayer * player /* r29 */, float dt /* f27 */) {
    // Local variables
    unsigned char match; // r27
    struct xVec3 playerPos; // r1+0x18
    const struct xVec3 & ropePos; // r28
    struct xVec3 & dpos; // r31
    struct xVec3 diff; // r1+0xC
    float minUpDpos; // f28
    float diffXZLength2; // f31
    float dposXZLength2; // f30
    float minTranslation2; // f29
...
// Range: 0x802E1584 -> 0x802E15FC
// this: r31
void zContextObjectClimbableRope::Reset() {}

// Range: 0x802E15FC -> 0x802E164C
// this: r1+0x8
void zContextObjectClimbableRope::ClearCollisionFlags() {
    // Local variables
    int i; // r31
}

// Range: 0x802E164C -> 0x802E169C
// this: r1+0x8
void zContextObjectClimbableRope::RestoreCollisionFlags() {
    // Local variables
    int i; // r31
}
