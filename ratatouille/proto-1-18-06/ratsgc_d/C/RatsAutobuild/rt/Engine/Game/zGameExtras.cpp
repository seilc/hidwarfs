/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zGameExtras.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801724B8 -> 0x8017306C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static int g_enableGameExtras; // size: 0x4, address: 0x80D6A4D0
static int g_currDay; // size: 0x4, address: 0x80D6A4D4
static int g_currMonth; // size: 0x4, address: 0x80D6A4D8
static int g_gameExtraFlags; // size: 0x4, address: 0x80D6A4DC
// total size: 0x4
struct EGGMisc {
    // Members
...
struct EGGData {
    // Members
    union { // inferred
        int placeholder; // offset 0x0, size 0x4
        struct EGGMisc misc; // offset 0x0, size 0x4
    };
};
// total size: 0x10
struct EGGItem {
    // Members
    int (* fun_check)(struct EGGItem *); // offset 0x0, size 0x4
    struct EGGItemFuncs * funcs; // offset 0x4, size 0x4
    int enabled; // offset 0x8, size 0x4
    struct EGGData eggdata; // offset 0xC, size 0x4
};
// total size: 0x10
struct EGGItemFuncs {
    // Members
    void (* fun_update)(float, struct EGGItem *); // offset 0x0, size 0x4
    void (* fun_init)(struct EGGItem *); // offset 0x4, size 0x4
    void (* fun_reset)(struct EGGItem *); // offset 0x8, size 0x4
    void (* fun_done)(struct EGGItem *); // offset 0xC, size 0x4
};
static struct EGGItemFuncs EGGEmpty; // size: 0x10, address: 0x80D62D00
static struct EGGItem g_eggBasket[2]; // size: 0x20, address: 0x8051EF18
static char __FUNCTION__[22]; // size: 0x16, address: 0x8051EF38
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// total size: 0x4
struct /* @class$889zGameExtras_cpp */ {
    // Members
...
union /* @class$808zGameExtras_cpp */ {
...
// total size: 0xC
struct /* @class$1053zGameExtras_cpp */ {
    // Members
...
// Range: 0x801724B8 -> 0x801725EC
void zGameExtras_SceneInit() {
    // Local variables
    int somethingIsEnabled; // r29
    struct EGGItem * egg_next; // r30
    struct EGGItem * egg; // r31

    // References
    // -> static int g_enableGameExtras;
    // -> static struct EGGItem g_eggBasket[2];
    // -> static int g_currMonth;
    // -> static int g_currDay;
    // -> static char __FUNCTION__[22];
    // -> struct zGlobals globals;
}

// Range: 0x801725EC -> 0x80172674
...
// Range: 0x8017273C -> 0x801727F8
void zGameExtras_SceneUpdate(float dt /* f31 */) {
    // Local variables
    struct EGGItem * egg_next; // r30
    struct EGGItem * egg; // r31
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x8051EF4E
// Range: 0x801727F8 -> 0x80172A54
static int EGG_check_ExtrasFlags() {
    // References
    // -> static char __FUNCTION__[22];
    // -> static int g_gameExtraFlags;
    // -> static int g_currDay;
    // -> static int g_currMonth;
}

static int g_flg_chEnabled; // size: 0x4, address: 0x80D6A4E0
// Range: 0x80172A54 -> 0x80172A60
void zGameExtras_NewGameReset() {
    // References
    // -> static int g_flg_chEnabled;
}
...
// Range: 0x80172A60 -> 0x80172A9C
void zGameExtras_Save(struct xSerial * xser /* r1+0x8 */) {
    // Local variables
    int keepers; // r31
...
// Range: 0x80172A9C -> 0x80172AE0
void zGameExtras_Load(struct xSerial * xser /* r1+0x8 */) {
    // Local variables
    int keepers; // r1+0xC
...
static float k_CHEATTIMER_START; // size: 0x4, address: 0x80D62D10
static float sCheatTimer; // size: 0x4, address: 0x80D6A4E4
static int sCheatInputCount; // size: 0x4, address: 0x80D6A4E8
unsigned int sCheatTestCheats[16]; // size: 0x40, address: 0x8051EF98
unsigned int sCheatSwapCCLR[16]; // size: 0x40, address: 0x8051EFD8
unsigned int sCheatSwapCCUD[16]; // size: 0x40, address: 0x8051F018
// total size: 0x10
struct GECheat {
    // Members
    unsigned int * key_code; // offset 0x0, size 0x4
    void (* fun_cheat)(); // offset 0x4, size 0x4
    int flg_keep; // offset 0x8, size 0x4
    int flg_mode; // offset 0xC, size 0x4
};
static struct GECheat cheatList[4]; // size: 0x40, address: 0x8051F058
unsigned int sCheatPressed[16]; // size: 0x40, address: 0x8051F098
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D62D14
// Range: 0x80172AE0 -> 0x80172BD8
static int TestCheat(unsigned int * cheat_info /* r30 */) {
    // Local variables
    int i; // r31
    int enable_cheat; // r29

    // References
    // -> unsigned int sCheatPressed[16];
    // -> static char __FUNCTION__[10];
}

// Range: 0x80172BD8 -> 0x80172C38
static void AddToCheatPressed(unsigned int button /* r3 */) {
    // Local variables
    int i; // r31
...
static int displayPresses; // size: 0x4, address: 0x80D6A4EC
static signed char init; // size: 0x1, address: 0x80D6A4F0
...
// Range: 0x80172C38 -> 0x80172F6C
void zGameCheatsUpdate(float dt /* r1+0x8 */) {
    // Local variables
    int i; // r28
    int startover; // r30
    int i; // r1+0x10
    int i; // r1+0xC
    int match; // r27
    struct GECheat * rec_next; // r29
    struct GECheat * rec_curr; // r31
    int rc; // r26

    // References
    // -> unsigned int sCheatPressed[16];
    // -> static float sCheatTimer;
    // -> static struct GECheat cheatList[4];
    // -> static float k_CHEATTIMER_START;
    // -> struct _tagxPad mPad[4];
    // -> static int sCheatInputCount;
    // -> static int displayPresses;
    // -> static signed char init;
    // -> int GCN_DEMO_PARAM;
}

// Range: 0x80172F6C -> 0x80172F7C
void GEC_CheatFlagAdd(int flg /* r3 */) {
    // References
    // -> static int g_flg_chEnabled;
}

// Range: 0x80172F7C -> 0x80172F8C
void GEC_CheatFlagToggle(int flg /* r3 */) {
    // References
    // -> static int g_flg_chEnabled;
}

static unsigned int aid_sndList[7]; // size: 0x1C, address: 0x8051F0D8
static int been_here; // size: 0x4, address: 0x80D6A4F4
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D62D1E
// Range: 0x80172F8C -> 0x80172FD0
static void GEC_dfltSound() {
    // References
    // -> static unsigned int aid_sndList[7];
}

// Range: 0x80172FD0 -> 0x80173004
static void GEC_cb_TestCheats() {}

// Range: 0x80173004 -> 0x80173038
static void GEC_cb_SwapCCLR() {}

// Range: 0x80173038 -> 0x8017306C
