/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zFXGoo.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8027A688 -> 0x8027C2B4
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static float defaultGooTimes[4]; // size: 0x10, address: 0x80D65A78
static float defaultGooWarbc[4]; // size: 0x10, address: 0x80D65A88
// total size: 0x8
struct RwObject {
    // Members
...
struct zFXGooInstance zFXGooInstances[24]; // size: 0xBA0, address: 0x80D258A8
float min2; // size: 0x4, address: 0x80D65A98
float max2; // size: 0x4, address: 0x80D65A9C
float goo_freeze_coeff; // size: 0x4, address: 0x80D65AA0
float goo_melt_coeff; // size: 0x4, address: 0x80D65AA4
float wpow; // size: 0x4, address: 0x80D65AA8
static struct RwTexture * g_txtr_gooFrozen; // size: 0x4, address: 0x80D6B2B0
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D65AAC
...
// Range: 0x8027A688 -> 0x8027ABCC
void zFXGooEnable(struct xModelInstance * gooModel /* r1+0x8 */, int freezeGroup /* r1+0xC */) {
    // Local variables
    int i; // r29
    ...
    int numverts; // r27
    int numtris; // r1+0x34
    struct RpMorphTarget * mtp2; // r25
    struct RwV3d * ov; // r22
    struct RwV3d * nv; // r21
    struct RwV3d * on; // r20
    struct RwV3d * nn; // r19
    struct RwRGBA * nc; // r28
    struct RwTexCoords * ouv; // r18
    struct RwTexCoords * nuv; // r17
    struct RpTriangle * ot; // r24
    struct RpTriangle * nt; // r23
    unsigned short v1; // r1+0x14
    unsigned short v2; // r1+0x12
    unsigned short v3; // r1+0x10
    struct RpMaterial * mp; // r1+0x30
    struct RwSphere boundingSphere; // r1+0x38
    struct xVec3 * orig_verts; // r1+0x2C
    struct RwRGBA * orig_colors; // r1+0x28
    float * orig_uvs; // r1+0x24
...
unsigned int no_modify_goo; // size: 0x4, address: 0x80D6B2B4
static char __FUNCTION__[21]; // size: 0x15, address: 0x80524D38
enum zFXGooState {
    zFXGooStateNormal = 0,
...
// ERROR: Failed to emit tag 1D814E5 (GlobalSubroutine)
// Range: 0x8027B208 -> 0x8027B27C
void zFXGooUpdate(float dt /* r1+0x8 */) {
    // Local variables
    int i; // r30
    struct zFXGooInstance * goo; // r31
...
static int doDebugDraw; // size: 0x4, address: 0x80D6B2B8
static signed char init; // size: 0x1, address: 0x80D6B2BC
static float tad; // size: 0x4, address: 0x80D6B2C0
static signed char init; // size: 0x1, address: 0x80D6B2C4
static int addWorldOffset; // size: 0x4, address: 0x80D6B2C8
static signed char init; // size: 0x1, address: 0x80D6B2CC
static int addParentOffset; // size: 0x4, address: 0x80D6B2D0
static signed char init; // size: 0x1, address: 0x80D6B2D4
static int initTweaks; // size: 0x4, address: 0x80D6B2D8
static signed char init; // size: 0x1, address: 0x80D6B2DC
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D65AB9
...
// Range: 0x8027B27C -> 0x8027BCAC
struct RpAtomic * zFXGooRenderAtomic(struct RpAtomic * atomic /* r17 */) {
    // Local variables
    int i; // r20
    int j; // r26
    struct zFXGooInstance * goo; // r30
    struct xVec3 vec_parMotion; // r1+0xC0
    struct RxObjSpace3DVertex * Im3DBuffer; // r19
    unsigned int Im3DBufferPos; // r24
    struct RxObjSpace3DVertex * imv; // r31
    struct RwRaster * rast; // r1+0x20
    struct RpGeometry * geom; // r27
    struct xVec3 * vert; // r23
    struct xVec3 * normals; // r18
    struct RwRGBA * color; // r28
    float A; // f27
    float B; // f26
    unsigned char * alpha; // r25
    struct xVec3 vec; // r1+0xB4
    float d2; // f31
    struct RpTriangle * tri; // r29
    struct xVec3 v0; // r1+0xA8
    struct xVec3 v1; // r1+0x9C
    struct xVec3 v2; // r1+0x90
    struct xVec3 n0; // r1+0x84
    struct xVec3 n1; // r1+0x78
    struct xVec3 n2; // r1+0x6C
    struct xVec3 resultV0; // r1+0x60
    struct xVec3 resultV1; // r1+0x54
    struct xVec3 resultV2; // r1+0x48
    ...
    char str_buf[128]; // r1+0xCC
    struct zFXGooInstance * goo; // r22
    int a; // r21
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D65ACC
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x8027BCAC -> 0x8027BE6C
int zFXGooIs(struct xEnt * obj /* r29 */, float & depth /* r26 */, unsigned int playerCheck /* r1+0x8 */) {
    // Local variables
    int i; // r30
    struct zFXGooInstance * goo; // r31
...
// total size: 0x1
struct /* @class$833zFXGoo_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$834zFXGoo_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$835zFXGoo_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$836zFXGoo_cpp */ {
    // Members
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D65AD5
// Range: 0x8027BE6C -> 0x8027BFD4
void zFXGooEventSetWarb(struct xEnt * ent /* r29 */, const float * warb /* r1+0x8 */) {
    // Local variables
    int i; // r30
    struct zFXGooInstance * goo; // r31
...
static char __FUNCTION__[29]; // size: 0x1D, address: 0x80524D4D
// Range: 0x8027BFD4 -> 0x8027C14C
void zFXGooEventSetFreezeDuration(struct xEnt * ent /* r29 */, float duration /* r1+0x8 */) {
    // Local variables
    int i; // r30
    struct zFXGooInstance * goo; // r31
    int freezeGroup; // r28
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D65AE8
// Range: 0x8027C14C -> 0x8027C2B4
void zFXGooEventMelt(struct xEnt * ent /* r29 */) {
    // Local variables
    int i; // r30
    struct zFXGooInstance * goo; // r31
    int freezeGroup; // r28
