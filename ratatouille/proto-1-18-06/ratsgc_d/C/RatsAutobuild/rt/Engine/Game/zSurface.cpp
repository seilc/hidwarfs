/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Game\zSurface.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801B994C -> 0x801BBE9C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct zSurfaceProps * zsps; // size: 0x4, address: 0x80D6A9B0
static int sMapperCount; // size: 0x4, address: 0x80D6A9B4
// total size: 0x8
...
static struct zMaterialMapAsset * sMapper[20]; // size: 0x50, address: 0x805219C0
// total size: 0x50
...
static struct xSurface sDefaultSurface; // size: 0x28, address: 0x80D0BC10
// total size: 0x20
struct xLinkAsset {
    // Members
...
static struct zSurfaceProps sDefaultSurfaceProps; // size: 0x118, address: 0x80D0BC38
// total size: 0x8
struct xBaseAsset {
    // Members
    unsigned int id; // offset 0x0, size 0x4
...
static struct zSurfAssetIN sDefaultSurfaceAsset; // size: 0x1B8, address: 0x80D0BD50
static struct xSurface * pDefaultSurface; // size: 0x4, address: 0x80D6A9B8
static int been_here; // size: 0x4, address: 0x80D6A9BC
static int been_here; // size: 0x4, address: 0x80D6A9C0
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D638F8
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x801B994C -> 0x801B9E2C
void zSurfaceInit() {
    // Local variables
    unsigned int size; // r1+0x8
    unsigned short nsurfs; // r26
    struct zSurfAssetIN * defaultAsset; // r24
    unsigned short i; // r27
    struct zSurfAssetIN * asset; // r29
    struct xSurface * surf; // r28
    struct zSurfaceProps * moprops; // r31
    int j; // r30

    // References
    // -> static struct xSurface sDefaultSurface;
    // -> static struct xSurface * pDefaultSurface;
    // -> static struct zSurfaceProps * zsps;
...
// Range: 0x801B9E2C -> 0x801B9F40
static void zSurfaceInitDefaultSurface() {
    // References
    // -> static struct zSurfAssetIN sDefaultSurfaceAsset;
    // -> static struct zSurfaceProps sDefaultSurfaceProps;
    // -> static struct xSurface sDefaultSurface;
}

static int been_here; // size: 0x4, address: 0x80D6A9C4
static int been_here; // size: 0x4, address: 0x80D6A9C8
static char __FUNCTION__[23]; // size: 0x17, address: 0x80521A10
// Range: 0x801B9F40 -> 0x801BA0E0
void zSurfaceRegisterMapper(unsigned int assetId /* r31 */) {
    // References
    // -> static int sMapperCount;
    // -> static struct zMaterialMapAsset * sMapper[20];
    // -> static int been_here;
    // -> static char __FUNCTION__[23];
    // -> static int been_here;
}

// Range: 0x801BA0E0 -> 0x801BA108
void zSurfaceExit() {
    // References
    // -> static int sMapperCount;
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80521A27
// Range: 0x801BA108 -> 0x801BA1A4
void zSurfaceResetSurface(struct xSurface * surf /* r31 */) {
    // References
    // -> static char __FUNCTION__[21];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63905
// total size: 0x8
...
// Range: 0x801BA1A4 -> 0x801BA2FC
struct xSurface * zSurfaceGetSurface(unsigned int mat_id /* r1+0x8 */) {
    // Local variables
    int map; // r30
    struct zMaterialMapAsset * mapper; // r29
    unsigned short i; // r28
    struct zMaterialMapEntry * entry; // r31
    unsigned short nsurfs; // r25
...
// Range: 0x801BA2FC -> 0x801BA374
struct xSurface * zSurfaceGetSurface(unsigned int oid /* r1+0x8 */, void * optr /* r1+0xC */, struct xModelInstance * mptr /* r30 */) {
    // Local variables
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63918
...
// Range: 0x801BA374 -> 0x801BA48C
struct xSurface * zSurfaceGetSurface(const struct xSweptSphere * sphere /* r31 */) {
    // Local variables
    struct xSurface * surf; // r30
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D6392B
// Range: 0x801BA48C -> 0x801BA598
struct xSurface * zSurfaceGetSurface(const struct xCollis * coll /* r31 */) {
    // Local variables
    struct xSurface * surf; // r30
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D6393E
// Range: 0x801BA598 -> 0x801BA648
unsigned char zSurfaceOutOfBounds(const struct xSurface & s /* r30 */) {
    // Local variables
    struct zSurfAssetIN * a; // r31

    // References
    // -> static char __FUNCTION__[20];
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x80521A3C
// Range: 0x801BA648 -> 0x801BA754
int zSurfaceGetDamageType(const struct xSurface * surf /* r31 */) {
    // Local variables
    struct zSurfAssetIN * sass; // r30

    // References
    // -> static char __FUNCTION__[22];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63952
// Range: 0x801BA754 -> 0x801BA864
unsigned int zSurfaceGetStandOn(const struct xSurface * surf /* r31 */) {
    // Local variables
    struct zSurfAssetIN * sass; // r30
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D63965
// total size: 0x0
struct xSerial {};
// Range: 0x801BA864 -> 0x801BA954
void zSurfaceSave(struct xSurface * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[13];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D63972
// Range: 0x801BA954 -> 0x801BAA44
void zSurfaceLoad(struct xSurface * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[13];
}

char __FUNCTION__$localstatic1$CacheTextureRefs__FP19zSurfacePropTexAnim[17]; // size: 0x11, address: 0x80D6397F
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D63990
// Range: 0x801BAA44 -> 0x801BABF4
void zSurfaceSetup(struct xSurface * s /* r29 */) {
    // Local variables
...
// Range: 0x801BABF4 -> 0x801BAD3C
void CacheTextureRefs(struct zSurfacePropTexAnim * texanim /* r31 */) {
    // Local variables
    struct xGroup * grp; // r28
    unsigned int num_items; // r27
    unsigned int amt; // r26
    void * mem; // r25
    int i; // r30
    unsigned int aid_item; // r29
    void * memptr; // r24
    struct RwTexture * txtr; // r23

    // References
    // -> struct xGlobals * xglobals;
    // -> char __FUNCTION__$localstatic1$CacheTextureRefs__FP19zSurfacePropTexAnim[17];
    // -> unsigned int gActiveHeap;
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D6399E
// Range: 0x801BAD3C -> 0x801BAEA8
void zSurfaceUpdate(struct xBase * to /* r29 */, struct xScene * sc /* r1+0x8 */, float dt /* r1+0xC */) {
    // Local variables
    struct xSurface * t; // r31
    struct zSurfaceProps * moprops; // r30

    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D639AD
// Range: 0x801BAEA8 -> 0x801BB4C4
static void zSurfaceEventCB(struct xBase * to /* r22 */, unsigned int toEvent /* r1+0x8 */, const float * toParam /* r30 */, struct xBase * toParamWidget /* r25 */) {
    // Local variables
    struct xSurface * t; // r31
    struct zSurfaceProps * p; // r24
    struct zSurfaceProps * p; // r27
    struct zSurfaceProps * p; // r21
    struct zSurfaceProps * p; // r28
    struct zSurfaceProps * p; // r29
    int idx; // r26
    struct zSurfaceProps * p; // r23

    // References
    // -> static char __FUNCTION__[16];
}

// Range: 0x801BB4C4 -> 0x801BB4CC
struct xSurface & zSurfaceGetDefault() {
    // References
    // -> static struct xSurface * pDefaultSurface;
}

char __FUNCTION__$localstatic1$zSurfaceUpdateTextureAnim__FfP8xSurfaceP13zSurfaceProps[26]; // size: 0x1A, address: 0x80521A52
...
// Range: 0x801BB4CC -> 0x801BBE9C
void zSurfaceUpdateTextureAnim(float dt /* f31 */, struct zSurfaceProps * moprops /* r31 */) {
    // Local variables
    float timestep_forUV; // f28
    int j; // r30
    struct xVec3 d; // r1+0x8
    struct zSurfacePropUVFX & sfx; // r29
    float uTime; // f30
    int uTimeInt; // r27
    float vTime; // f29
    int vTimeInt; // r26
    struct xGroup * g; // r25
    unsigned int max; // r28

    // References
    // -> char __FUNCTION__$localstatic1$zSurfaceUpdateTextureAnim__FfP8xSurfaceP13zSurfaceProps[26];
