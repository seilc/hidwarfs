/*
    ...
    Language: C++
    Code range: 0x8010FA74 -> 0x8010FEE0
*/
// total size: 0x20
struct xLinkAsset {
    // Members
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80516B48
...
// Range: 0x8010FA74 -> 0x8010FDF8
static void * _iGCUVRenderCallback(void * object /* r19 */, struct RxGameCubePipeData * pipeData /* r31 */) {
    // Local variables
    unsigned int numMeshes; // r26
    struct RxGameCubeVertexBuffer * vbHeader; // r28
    struct RxGameCubeDisplayList * dList; // r29
    struct RpMesh * mesh; // r30
    struct RwMatrixTag * ltm; // r24
    void (* matFunc)(struct RwRGBAReal *, struct RwRGBA *, float); // r25
    struct RpAtomic * atomic; // r23
    float tmtx[3][4]; // r1+0x8
    struct RwTexture * texture; // r27
    struct RwGameCubeRasterExtension * rasExt; // r22
