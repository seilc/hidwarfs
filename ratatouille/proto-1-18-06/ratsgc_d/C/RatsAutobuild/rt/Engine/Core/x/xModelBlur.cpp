/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xModelBlur.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80233F78 -> 0x802367FC
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static unsigned char sModelBlurEnabled; // size: 0x1, address: 0x80D64D90
static unsigned char render; // size: 0x1, address: 0x80D64D91
static unsigned char render_texture; // size: 0x1, address: 0x80D6AF98
static unsigned char render_opaque; // size: 0x1, address: 0x80D6AF99
static unsigned char render_card_spheres; // size: 0x1, address: 0x80D6AF9A
static int alpha_compare; // size: 0x4, address: 0x80D64D94
static int min_alpha; // size: 0x4, address: 0x80D64D98
// Range: 0x80233F78 -> 0x802340DC
static void add_tweaks() {
    ...
    // -> static unsigned char render_card_spheres;
    // -> static unsigned char render_opaque;
    // -> static unsigned char render_texture;
...
// total size: 0xC
struct /* @class$734xModelBlur_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$735xModelBlur_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$736xModelBlur_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$737xModelBlur_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$738xModelBlur_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$739xModelBlur_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$740xModelBlur_cpp */ {
    // Members
...
class xFXCameraTexture {
    // Members
public:
    struct RwCamera * cam; // offset 0x0, size 0x4
    struct RwRaster * raster; // offset 0x4, size 0x4
    struct RwRaster * bgraster; // offset 0x8, size 0x4
    struct RpWorld * world; // offset 0xC, size 0x4
    struct RwTexture * texture; // offset 0x10, size 0x4
    unsigned long vert_buffer_used; // offset 0x14, size 0x4
    struct xColor_tag bgcolor; // offset 0x18, size 0x4
private:
    struct RwCamera * oldcam; // offset 0x1C, size 0x4
    struct RpWorld * oldworld; // offset 0x20, size 0x4
    float rcz; // offset 0x24, size 0x4
    float w; // offset 0x28, size 0x4
    float h; // offset 0x2C, size 0x4
    struct RwRaster * zraster; // offset 0x30, size 0x4
    int max_tw; // offset 0x34, size 0x4
    int max_th; // offset 0x38, size 0x4
    float old_near_plane; // offset 0x3C, size 0x4
    float old_far_plane; // offset 0x40, size 0x4
};
static class xFXCameraTexture camtex; // size: 0x44, address: 0x80D185E0
...
// total size: 0x4
struct /* @class$884xModelBlur_cpp */ {
    // Members
...
union /* @class$1066xModelBlur_cpp */ {
...
// total size: 0xC
struct /* @class$1065xModelBlur_cpp */ {
    // Members
...
// Range: 0x802340DC -> 0x80234320
static void render_model_now(struct xModelInstance * model /* r31 */) {
    // Local variables
    unsigned char reset; // r30
    struct xMat3x3 tmpmat; // r1+0x10
...
struct path_node {
    // Members
    unsigned char just_added; // offset 0x0, size 0x1
    float age; // offset 0x4, size 0x4
...
struct activity_data {
    ...
    class xModelBlur * owner; // offset 0x4, size 0x4
    const struct config_data * cfg; // offset 0x8, size 0x4
    struct xModelInstance * model; // offset 0xC, size 0x4
    struct xVec3 center; // offset 0x10, size 0xC
    struct path_node path[32]; // offset 0x1C, size 0x280
    struct card_data cards[32]; // offset 0x29C, size 0x200
    int path_start; // offset 0x49C, size 0x4
    int path_size; // offset 0x4A0, size 0x4
    int cards_size; // offset 0x4A4, size 0x4
    struct interpolator lifetime; // offset 0x4A8, size 0x14
    struct interpolator alpha; // offset 0x4BC, size 0x14
};
struct activity_data * activity_buffer; // size: 0x4, address: 0x80D6AF9C
struct activity_data * activities[6]; // size: 0x18, address: 0x80D18624
int activities_used; // size: 0x4, address: 0x80D6AFA0
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
class xModelBlur {
    ...
    unsigned char activate(float lifetime, float alpha, const struct config_data & cfg, struct xModelInstance * model);

    void deactivate();

    void move(const struct xVec3 & center);

    void set_config(const struct config_data & cfg);

    unsigned char visible() const;

    void set_lifetime(float value, float time);

    void set_alpha(float value, float time);

    unsigned char update(float dt);

    void render();

    void render_cards(const struct xFRect & src_rect, float xoff0, float yoff0, float xoff1, float yoff1);
...
// Range: 0x80234320 -> 0x80234410
void xModelBlur::scene_enter() {
    // Local variables
    int i; // r31
    struct activity_data * activity; // r30

    // References
    // -> int activities_used;
    // -> struct activity_data * activities[6];
    // -> struct activity_data * activity_buffer;
    // -> static class xFXCameraTexture camtex;
    // -> struct xColor_tag g_CLEAR;
}

// Range: 0x80234410 -> 0x80234438
void xModelBlur::scene_exit() {
    // References
    // -> static class xFXCameraTexture camtex;
}

// Range: 0x80234438 -> 0x802344FC
void xModelBlur::reset_all() {
    // Local variables
    int i; // r30
    int i; // r31
    struct activity_data * activity; // r29
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D64D9C
// Range: 0x802344FC -> 0x802345F4
void xModelBlur::update_all(float dt /* r1+0x8 */) {
    // Local variables
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D64DA7
// Range: 0x802345F4 -> 0x802346D8
void xModelBlur::render_all() {
    ...
    // -> int activities_used;
    // -> struct activity_data * activities[6];
    // -> static unsigned char sModelBlurEnabled;
}
...
// Range: 0x802346D8 -> 0x802346E4
// this: r3
void xModelBlur::create() {}

// Range: 0x802346E4 -> 0x802347D8
// this: r31
unsigned char xModelBlur::activate(float lifetime /* f30 */, float alpha /* f31 */, const struct config_data & cfg /* r1+0x8 */, struct xModelInstance * model /* r1+0xC */) {
    // References
    // -> int activities_used;
}
...
struct interpolator {
    // Functions
    static void reset(float value);

    static void go(float value, float time);
    ...
    float trate; // offset 0x10, size 0x4
};
// Range: 0x802347D8 -> 0x802347F4
// this: r3
static void interpolator::reset(float value /* f1 */) {}

// Range: 0x802347F4 -> 0x80234844
// this: r31
void xModelBlur::deactivate() {}

static char __FUNCTION__[5]; // size: 0x5, address: 0x80D64DB2
// Range: 0x80234844 -> 0x80234A64
// this: r31
void xModelBlur::move(const struct xVec3 & center /* r26 */) {
    // Local variables
    int second_index; // r27
    const struct xVec3 & A; // r30
    ...
    float min_dist; // f28
    float min_dist2; // f26
    struct path_node & p; // r28

    // References
    // -> static char __FUNCTION__[5];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D64DB7
// Range: 0x80234A64 -> 0x80234B00
// this: r31
void xModelBlur::set_config(const struct config_data & cfg /* r1+0x8 */) {
    // References
    // -> static char __FUNCTION__[11];
}

// Range: 0x80234B00 -> 0x80234BC4
// this: r3
unsigned char xModelBlur::visible() const {}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D64DC2
// Range: 0x80234BC4 -> 0x80234C6C
// this: r31
void xModelBlur::set_lifetime(float value /* r1+0x8 */, float time /* r1+0xC */) {
    // References
    // -> static char __FUNCTION__[13];
}

// Range: 0x80234C6C -> 0x80234D08
// this: r31
static void interpolator::go(float value /* f31 */, float time /* f30 */) {}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D64DCF
// Range: 0x80234D08 -> 0x80234DB0
// this: r31
void xModelBlur::set_alpha(float value /* r1+0x8 */, float time /* r1+0xC */) {
    // References
    // -> static char __FUNCTION__[10];
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80D64DD9
// Range: 0x80234DB0 -> 0x802352E0
// this: r31
unsigned char xModelBlur::update(float dt /* f21 */) {
    // Local variables
    int i; // r30
    float lifetime; // f30
    struct path_node & p; // r28
    struct path_node & pp; // r22
    float card_dist; // f27
    int max_card_renders; // r21
    int end; // r1+0x18
    float alpha_scale; // f20
    struct card_data * cards; // r19
    int cards_size; // r26
    float path_dist; // f31
    struct path_node & p0; // r24
    struct path_node & p1; // r23
    const struct xVec3 & A; // r29
    const struct xVec3 & B; // r25
    float ABx; // f26
    float ABy; // f25
    float ABz; // f24
    float segment_dist; // f29
    float idist; // f19
    float age0; // f18
    float dage; // r1+0x14
    float dist; // f23
    float t; // f28
    float age; // f22
    struct card_data & card; // r27
    unsigned int alpha; // r18
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D64DE0
// Range: 0x80235370 -> 0x8023541C
struct activity_data * xModelBlur::alloc_activity() {
    // Local variables
    struct activity_data * activity; // r31
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D64DEF
// Range: 0x8023541C -> 0x80235580
void xModelBlur::free_activity(struct activity_data * activity /* r1+0x8 */) {
    // Local variables
    struct activity_data * * tail_activity; // r31
    struct activity_data * temp; // r30
...
static unsigned int * FBSaveMem; // size: 0x4, address: 0x80D6AFA4
static unsigned int * textureMem; // size: 0x4, address: 0x80D6AFA8
static char __FUNCTION__[7]; // size: 0x7, address: 0x80D64DFD
// total size: 0x1C
...
// Range: 0x80235580 -> 0x80235E70
// this: r31
void xModelBlur::render() {
    // Local variables
    unsigned int oldFBMSK; // r26
    struct RwRaster * draw_raster; // r25
    const struct xVec3 & center; // r24
    struct xVec3 view_center; // r1+0x38
    struct RwCamera * world_cam; // r27
    const struct xMat4x3 & view_mat; // r23
    float rz; // f31
    float screen_x; // f30
    float screen_y; // f29
    float screen_radius; // f28
    const struct xVec2 & world_view_window; // r28
    struct xFRect scr_rect; // r1+0x28
    struct xFRect tex_rect; // r1+0x18
    float old_projection[7]; // r1+0x70
    void * FBSaveMemPopAddr; // r22
    void * textureMemPopAddr; // r21
    struct xModelInstance * m; // r30
    float oldalpha; // f25
    struct xModelInstance * m; // r29
    float oldalpha; // f24
    float mProjection[4][4]; // r1+0xCC
    float mIdentity[4][4]; // r1+0x8C
    struct _GXColor colorAmbient; // r1+0x14
    ...
    // -> static class xFXCameraTexture camtex;
    // -> static unsigned char render_texture;
    // -> static unsigned int * FBSaveMem;
    // -> static unsigned int * textureMem;
...
class xFXFastRaster {
    // Members
    struct RwRaster * src; // offset 0x0, size 0x4
    struct RwRaster * dst; // offset 0x4, size 0x4
...
// Range: 0x80235F0C -> 0x802367FC
// this: r31
void xModelBlur::render_cards(const struct xFRect & src_rect /* r29 */, float xoff0 /* r1+0x10 */, float yoff0 /* r1+0x14 */, float xoff1 /* r1+0x18 */, float yoff1 /* r1+0x1C */) {
    // Local variables
    struct RwRaster * draw_raster; // r20
    class xFXFastRaster fr; // r1+0xC0
    int maxu; // r1+0x38
    int maxv; // r1+0x34
    float radius; // r1+0x30
    struct RwCamera * world_cam; // r30
    const struct xMat4x3 & view_mat; // r19
    const struct xVec2 & vw; // r1+0x2C
    struct _xFColor color; // r1+0x68
    float max_component; // f31
    float zscale; // f28
    float zbias; // f23
    int i; // r28
    const struct card_data & card; // r23
    struct xVec3 sp; // r1+0x5C
    float rz; // f30
    float blitz; // r1+0x28
    float cx; // f27
    float cy; // f26
    ...
    float y0; // f24
    float y1; // f21
    struct xFRect clip_src_rect; // r1+0x4C
    struct xFRect clip_dst_rect; // r1+0x3C
    struct _GXTexObj screenBack; // r1+0xA0
    struct _GXColor colorAmbient; // r1+0x24
    float projVector[7]; // r1+0x78
    float posMatrix[3][4]; // r1+0xD4
    unsigned char r; // r27
    unsigned char g; // r26
    unsigned char b; // r25
    unsigned char a; // r24
