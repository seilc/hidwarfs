/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xSubTitles.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800FC638 -> 0x800FCBD0
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct basic_rect screen_bounds; // size: 0x10, address: 0x80D6D878
static float subtitleFontWidth; // size: 0x4, address: 0x80D612C8
static float subtitleFontHeight; // size: 0x4, address: 0x80D612CC
static float subtitleSize; // size: 0x4, address: 0x80D699C8
// total size: 0x8
struct xBaseAsset {
    // Members
...
// total size: 0xC
struct xSubtitlesAsset : public xBaseAsset {
    // Members
    unsigned short m_uAssetNumLines; // offset 0x8, size 0x2
    unsigned short m_uAssetByteCount; // offset 0xA, size 0x2
    struct xSubtitleLine m_paSubtitleLines[]; // offset 0xC, size 0x0
};
static struct xSubtitlesAsset * m_pCurrentAsset; // size: 0x4, address: 0x80D699CC
static unsigned int m_uLastTimeKey; // size: 0x4, address: 0x80D699D0
// Range: 0x800FC638 -> 0x800FC6FC
void xSubTitlesInit() {
    // References
    // -> static float subtitleFontHeight;
    // -> static float subtitleFontWidth;
    // -> static float subtitleSize;
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D612D0
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x800FC6FC -> 0x800FC784
void * xSubTitlesRead(unsigned int aid /* r1+0x8 */, void * indata /* r1+0xC */, unsigned int insize /* r30 */, unsigned int * outsize /* r29 */) {
    // Local variables
    struct xSubtitlesAsset * pSubTitleAsset; // r31
...
// Range: 0x800FC784 -> 0x800FC794
void xSubTitlesUnloaded(void * userdata /* r3 */) {
    // Local variables
    struct xSubtitlesAsset * pSubTitleAsset; // r1+0x8
}

static unsigned int last_failed_aid; // size: 0x4, address: 0x80D699D4
static signed char init; // size: 0x1, address: 0x80D699D8
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D612DF
// total size: 0x4
struct RwRGBA {
    // Members
...
// Range: 0x800FC794 -> 0x800FCBC0
unsigned char xSubTitlesRender(unsigned int aid /* r30 */, float fTime /* f31 */) {
    // Local variables
    unsigned int uSearchIndex; // r31
    char * pszDisplay; // r29
    struct xColor_tag cText; // r1+0x14
    struct xColor_tag cBackGnd; // r1+0x10
    class xtextbox tbSubtitles; // r1+0xBC
    ...
    // -> static unsigned int m_uLastTimeKey;
    // -> static struct xSubtitlesAsset * m_pCurrentAsset;
    // -> static char __FUNCTION__[17];
    // -> static unsigned int last_failed_aid;
...
// total size: 0x2
struct /* @class$792xSubTitles_cpp */ {
    // Members
...
// Range: 0x800FCBC0 -> 0x800FCBC8
void xSubTitlesSetSize(float size /* f1 */) {
    // References
    // -> static float subtitleSize;
}

// Range: 0x800FCBC8 -> 0x800FCBD0
float xSubTitlesGetSize() {
    // References
    // -> static float subtitleSize;
}
