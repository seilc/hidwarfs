/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xCutscene.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8002BD7C -> 0x8002F11C
*/
char __FUNCTION__$localstatic1$Set__13i2dQuadBufferFP7i2dRectP7i2dRect10xColor_tag[4]; // size: 0x4, address: 0x80D5EA78
// total size: 0x80
...
struct xCutscene sActiveCutscene; // size: 0x1A8, address: 0x8055BD48
static unsigned int sCutTocCount[16]; // size: 0x40, address: 0x804F10F8
static struct xCutsceneInfo * sCutTocInfo[16]; // size: 0x40, address: 0x804F1138
static int sCutTocMax; // size: 0x4, address: 0x80D68C98
unsigned char gCutsceneDrawBounds; // size: 0x1, address: 0x80D68C9C
unsigned char gCutsceneDrawLineToOrigin; // size: 0x1, address: 0x80D68C9D
// total size: 0xC
struct xModelPool {
    // Members
...
static struct xModelInstance * sCutsceneFakeModel; // size: 0x4, address: 0x80D68CA0
static struct xCutsceneModelHack * sCutsceneModelHackBuf; // size: 0x4, address: 0x80D68CA4
// Range: 0x8002BD7C -> 0x8002BE1C
void xCutscene_InitBegin() {
    // References
    // -> unsigned char gCutsceneDrawLineToOrigin;
    // -> unsigned char gCutsceneDrawBounds;
    // -> static int sCutTocMax;
    // -> static struct xCutsceneInfo * sCutTocInfo[16];
    // -> static unsigned int sCutTocCount[16];
    // -> struct xCutscene sActiveCutscene;
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5EA7C
// Range: 0x8002BE1C -> 0x8002BEFC
void xCutscene_InitTOC(void * toc /* r31 */) {
    // References
    // -> static int sCutTocMax;
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5EA8E
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x8002BEFC -> 0x8002C164
void xCutscene_InitEnd() {
    // Local variables
    int i; // r31
    int maxModels; // r29
    int tocnum; // r28
    struct xCutsceneInfo * cnfo; // r30
    unsigned int cnum; // r27

    // References
    // -> static char __FUNCTION__[18];
    // -> unsigned int gActiveHeap;
    // -> static struct xCutsceneModelHack * sCutsceneModelHackBuf;
    // -> static int sCutTocMax;
    // -> static unsigned int sCutTocCount[16];
    // -> static struct xCutsceneInfo * sCutTocInfo[16];
    // -> static struct xModelInstance * sCutsceneFakeModel;
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5EAA0
...
// Range: 0x8002C164 -> 0x8002C3FC
struct xCutscene * xCutscene_Create(unsigned int csnID /* r1+0x8 */, unsigned int subtitlesID /* r1+0xC */) {
    // Local variables
    struct xCutscene * csn; // r31
    int tocnum; // r29
    struct xCutsceneInfo * cnfo; // r30
    unsigned int i; // r28
    unsigned int maxload; // r27
...
static int been_here; // size: 0x4, address: 0x80D68CA8
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5EAB1
// Range: 0x8002C3FC -> 0x8002C5FC
int xCutscene_Destroy(struct xCutscene * csn /* r31 */) {
    // Local variables
    unsigned int i; // r30
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D5EAC3
// Range: 0x8002C5FC -> 0x8002C6AC
int xCutscene_LoadStart(struct xCutscene * csn /* r31 */) {
    // References
    // -> static char __FUNCTION__[20];
}

// Range: 0x8002C6AC -> 0x8002C730
static float xCutsceneConvertBreak(float time /* f1 */, struct xCutsceneBreak * breaklist /* r3 */, unsigned int breakcount /* r4 */, int idx /* r5 */) {
    // Local variables
    unsigned int i; // r31
    float tdiff; // f31
}

static int sRepeatFrames; // size: 0x4, address: 0x80D68CAC
static signed char init; // size: 0x1, address: 0x80D68CB0
static unsigned char sLastFrame; // size: 0x1, address: 0x80D68CB1
static signed char init; // size: 0x1, address: 0x80D68CB2
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5EAD7
unsigned int gSoak; // size: 0x4, address: 0x80D6A1E0
// Range: 0x8002C730 -> 0x8002CA54
int xCutscene_Update(struct xCutscene * csn /* r31 */, float dt /* r1+0x8 */) {
    // Local variables
    struct xCutsceneTime * oldChunk; // r30

    // References
    // -> static unsigned char sLastFrame;
    // -> unsigned int gSoak;
    // -> static int sRepeatFrames;
    // -> static signed char init;
    // -> static signed char init;
    // -> static char __FUNCTION__[17];
}

// Range: 0x8002CA54 -> 0x8002CA88
void xCutscene_SetSpeed(struct xCutscene * csn /* r3 */, float speed /* f1 */) {}
...
// Range: 0x8002CA88 -> 0x8002CCCC
static void xcsCalcAnimMatrices(struct RwMatrixTag * animMat /* r31 */, struct RpAtomic * model /* r26 */, struct xCutsceneAnimHdr * ahdr /* r29 */, float time /* r1+0x10 */, unsigned int tworoot /* r1+0x14 */) {
    // Local variables
    struct xQuat quatresult[101]; // r1+0x55C
    struct xVec3 tranresult[101]; // r1+0xA0
    void * afile; // r25
    struct xMat4x3 m1; // r1+0x60
    struct xMat4x3 m2; // r1+0x20
    unsigned int numbone; // r24
    unsigned int boneidx; // r28
    struct xQuat * qqq; // r27
    struct xVec3 * ttt; // r30
}

struct xMat4x3 g_I3; // size: 0x40, address: 0x80B7FA80
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D5EAE8
// total size: 0x8
struct JDeltaRun {
    // Members
    int VertIdx; // offset 0x0, size 0x4
    int VertCt; // offset 0x4, size 0x4
};
// total size: 0x18
struct JDeltaTarget {
    // Members
    int skipSize; // offset 0x0, size 0x4
    int numRuns; // offset 0x4, size 0x4
    int numVerts; // offset 0x8, size 0x4
    float scale; // offset 0xC, size 0x4
    struct JDeltaRun runs[1]; // offset 0x10, size 0x8
};
// Range: 0x8002CCCC -> 0x8002D484
static void JDeltaEval(struct RpAtomic * model /* r1+0x8 */, void * deltaModel /* r27 */, void * deltaAnim /* r17 */, float time /* f28 */) {
    // Local variables
    float outweight[128]; // r1+0x30
    float * currweight; // r25
    int i; // r30
    int numFrames; // r1+0x2C
    int numWeights; // r26
    float * times; // r28
    float * weights; // r24
    float lerp; // f27
    float invlerp; // f26
    struct RwV3d * outverts; // r29
    int numRun; // r1+0x28
    int numTargets; // r1+0x24
    struct JDeltaTarget * dtgt; // r31
    float scale; // f31
    signed short * svert; // r23
    int j; // r20
    int cmpval; // r1+0x20
    struct RwV3d * vert; // r1+0x1C
    int j; // r1+0x18
    int cmpval; // r1+0x14
    float scale; // f30
    signed short * svert; // r22
    int j; // r19
    int cmpval; // r1+0x10
    float scale; // f29
...
// Range: 0x8002D484 -> 0x8002D4F4
static void CutsceneShadowRender(struct CutsceneShadowModel * smod /* r31 */) {
    // Local variables
    struct RpAtomic * model; // r30
    unsigned int bits; // r29
}

static unsigned int DRAW_SKIP_MASK; // size: 0x4, address: 0x80D68CB4
static signed char init; // size: 0x1, address: 0x80D68CB8
static unsigned char done; // size: 0x1, address: 0x80D68CB9
static signed char init; // size: 0x1, address: 0x80D68CBA
static unsigned char done; // size: 0x1, address: 0x80D68CBB
static signed char init; // size: 0x1, address: 0x80D68CBC
// total size: 0xC
struct xVec3 {
    // Static members
...
static struct xVec3 shadVec; // size: 0xC, address: 0x80D5EAF4
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5EB00
// total size: 0x18
...
struct xLightKitBlendInfo gLastLightKit; // size: 0xC, address: 0x80D69288
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
struct zCutsceneHack {
    ...
    struct RpAtomic * (* renderCB)(struct RpAtomic *); // offset 0x10, size 0x4
    struct xModelPipe pipe[8]; // offset 0x14, size 0x40
    struct xVec3 scale; // offset 0x54, size 0xC
    void * dataPtr; // offset 0x60, size 0x4
    unsigned int numAtomics; // offset 0x64, size 0x4
    void * morph_dataPtr[8]; // offset 0x68, size 0x20
    void * atomic_dataPtr[8]; // offset 0x88, size 0x20
    unsigned int atomic_Idx[8]; // offset 0xA8, size 0x20
    unsigned int cinnameID; // offset 0xC8, size 0x4
    unsigned int modelnameID; // offset 0xCC, size 0x4
};
// total size: 0x8
...
struct CutsceneShadowModel {
    ...
    struct RwMatrixTag * animMat; // offset 0x4, size 0x4
...
// Range: 0x8002D4F4 -> 0x8002EBC0
void xCutscene_Render(struct xCutscene * csn /* r31 */) {
    // Local variables
    unsigned int i; // r28
    unsigned int dataIndex; // r1+0xF4
    unsigned int animIndex; // r1+0xF0
    unsigned int mphIndex; // r1+0xEC
    unsigned int visFlags; // r1+0xE8
    unsigned int visIdx; // r18
    unsigned int fakeCount; // r29
    unsigned int backface; // r1+0xE4
    unsigned int localModel; // r1+0xE0
    unsigned int tworoot; // r1+0xDC
    unsigned int noshadow; // r1+0xD8
    struct xCutsceneData * data; // r27
    struct xCutsceneData * mphdata; // r25
    struct RpAtomic * model; // r30
    struct RpAtomic * shadowModel; // r1+0xD4
    struct RwMatrixTag animMat[101]; // r1+0x214
    struct xVec3 * camVec; // r1+0xD0
    struct xModelPipe * pipeCurr; // r1+0xCC
    unsigned int idx; // r1+0xC8
    unsigned int hacked_model; // r17
    int tableIdx; // r1+0xC4
    struct XCSNNosey * nosey; // r19
    float breakTime; // f31
    struct xBox combinedAnimBound; // r1+0x170
    unsigned int tempSize; // r1+0xC0
    struct xVec3 * scaleVec; // r26
    int table_size; // r1+0xBC
    int h; // r1+0xB8
    struct RpAtomic * tmpModel; // r1+0xB4
    struct zCutsceneHack & cutsceneHackTableData; // r22
    struct zCutsceneHack & cutsceneHackTableData2; // r1+0xB0
    unsigned int viscnt; // r1+0xAC
    unsigned int * currvis; // r24
    unsigned int objIndex; // r1+0xA8
    unsigned int subIndex; // r1+0xA4
    unsigned int frameMin; // r1+0xA0
    unsigned int frameMax; // r1+0x9C
    unsigned int frameIndex; // r1+0x98
    unsigned int flag; // r1+0x94
    unsigned int shadowBits; // r1+0x90
    unsigned int atomicNumber; // r1+0x8C
    struct zCutsceneHack & cutsceneHackTableData2; // r1+0x88
    unsigned int morphAnimIndex; // r1+0x84
    unsigned int morphModelIndex; // r1+0x80
    unsigned int numFrame; // r1+0x7C
    unsigned int numRun; // r1+0x78
    struct xCutsceneMphFrame * mphFrame; // r20
    struct xCutsceneMphRun * mphRun; // r21
    struct xMorphTargetFile * mphFile; // r23
    signed short * v_array[4]; // r1+0x130
    signed short weight[4]; // r1+0xF8
    unsigned int skipsize; // r1+0x74
    struct xVec3 * csnTmpArray; // r1+0x70
    struct xVec3 * currtmp; // r1+0x6C
    struct xVec3 * outv; // r1+0x68
    unsigned int j; // r1+0x64
    unsigned int cmpval; // r1+0x60
    void * deltaAnim; // r1+0x5C
    void * deltaModel; // r1+0x58
    int culled; // r1+0x54
    struct xBox worldbox; // r1+0x158
    int culled; // r1+0x50
    struct xBox worldbox; // r1+0x140
    struct zCutsceneHack & cutsceneHackTableData2; // r1+0x4C
    struct CutsceneShadowModel smod; // r1+0x124
    struct xShadowCache scache; // r1+0x188
    struct xVec3 center; // r1+0x118
    struct xVec3 radius; // r1+0x10C
    float fillCacheRadius; // f30
    struct xLightKitBlendInfo disabledLightKit; // r1+0x100

    // References
    // -> struct xGlobals * xglobals;
    // -> static struct xVec3 shadVec;
    // -> struct xLightKitBlendInfo gLastLightKit;
    // -> static char __FUNCTION__[17];
    ...
    // -> static unsigned int DRAW_SKIP_MASK;
    // -> static signed char init;
}
...
// Range: 0x8002EBC0 -> 0x8002EBCC
struct xCutscene * xCutscene_CurrentCutscene() {
    // References
    // -> struct xCutscene sActiveCutscene;
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80D5EB11
static struct xCamCutscene * cutscene_cam; // size: 0x4, address: 0x80D68CC0
// Range: 0x8002EBCC -> 0x8002EC30
static void init_cutscene_cam() {
    // References
    // -> static struct xCamCutscene * cutscene_cam;
}

// total size: 0x1A0
struct xCamCutscene : public xCam {
    // Functions
    static void create();
    ...
    static struct xCam * get_next();

    static void update();

    // Members
    struct xCutscene * csn; // offset 0x140, size 0x4
    struct xMat4x3 original_mat; // offset 0x150, size 0x40
    struct xCam * previous_cam; // offset 0x190, size 0x4
};
// Range: 0x8002EC30 -> 0x8002EC88
// this: r31
static void xCamCutscene::create() {}

// Range: 0x8002ECC4 -> 0x8002ED18
static void cutscene_started(struct xCutscene * csn /* r31 */) {
    // References
    // -> static struct xCamCutscene * cutscene_cam;
}

// Range: 0x8002ED18 -> 0x8002ED60
static void cutscene_stopped(struct xCutscene * csn /* r31 */) {
    // References
    // -> static struct xCamCutscene * cutscene_cam;
}

static // total size: 0x4C
struct {} @unnamed@xCutscene_cpp@::xCamCutscene::__vtable; // size: 0x4C, address: 0x804F1178
// Range: 0x8002ED60 -> 0x8002EDB0
// this: r31
...
// total size: 0xC
struct /* @class$1106xCutscene_cpp */ {
    // Members
...
// total size: 0x10
struct /* @class$1383xCutscene_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x8002EDB0 -> 0x8002EDE8
// this: r31
static void xCamCutscene::stop() {}

// Range: 0x8002EDE8 -> 0x8002EE08
// this: r3
static struct xCam * xCamCutscene::get_next() {}

// Range: 0x8002EE08 -> 0x8002F11C
// this: r30
static void xCamCutscene::update() {
    // Local variables
    struct xCutsceneData * data; // r28
    unsigned int i; // r29
    unsigned int dataIndex; // r27
    struct xMat3x3 tmpMat; // r1+0x30
    struct xQuat quats[2]; // r1+0x10
    float invlerp; // f30
    float lerp; // f31
    float tmpXap; // f29
    float tmpFoc; // f28
    int frame; // r26
    unsigned int count; // r25
    struct zFlyKey * keys; // r31
