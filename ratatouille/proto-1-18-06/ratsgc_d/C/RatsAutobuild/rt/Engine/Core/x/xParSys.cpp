/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xParSys.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800B86B4 -> 0x800B9B40
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static int sBlendTable[11]; // size: 0x2C, address: 0x80483B80
...
static enum ptank_render_mode_enum ptank_render_mode; // size: 0x4, address: 0x80D60420
static unsigned char ptank_render_mode_registered; // size: 0x1, address: 0x80D69410
static char * ptank_render_mode_labels[3]; // size: 0xC, address: 0x80D60424
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
static struct xVec3 par_offset_right; // size: 0xC, address: 0x80D69414
static struct xVec3 par_offset_up; // size: 0xC, address: 0x80D69420
// total size: 0x30
struct xMat3x3 {
    // Members
...
// Range: 0x800B86B4 -> 0x800B86FC
static void par_sprite_begin() {
    // References
    // -> struct xGlobals * xglobals;
    // -> static struct xVec3 par_offset_up;
    // -> static struct xVec3 par_offset_right;
}
...
// Range: 0x800B86FC -> 0x800B8A4C
static void par_sprite_update(struct xParSys & sys /* r29 */, struct xParGroup & group /* r26 */) {
    // Local variables
    unsigned int pivot; // r28
    struct xVec3 offset_right; // r1+0x24
    struct xVec3 offset_up; // r1+0x18
    struct ptank_pool__pos_color_size_uv2 pool; // r1+0x30
    int min_alpha; // r27
    struct xParCmdTex * tex; // r30
    struct xPar * p; // r31
    struct RwSphere worldsph; // r1+0x8
    struct xVec3 & loc; // r25
...
// Range: 0x800B8A4C -> 0x800B8B04
unsigned char using_ptank_render(const struct xParSysAsset & sys /* r29 */) {
    // References
    // -> static enum ptank_render_mode_enum ptank_render_mode;
    // -> static char * ptank_render_mode_labels[3];
    // -> static unsigned char ptank_render_mode_registered;
}

// Range: 0x800B8B04 -> 0x800B8B4C
static void render_par_sprite(void * data /* r31 */, struct xParGroup * ps /* r1+0x8 */) {}

// total size: 0x8
struct xParSysInfo {
    // Members
    int type; // offset 0x0, size 0x4
    void (* func)(void *, struct xParGroup *); // offset 0x4, size 0x4
};
static struct xParSysInfo sParSysInfo[7]; // size: 0x38, address: 0x804F6D08
static char __FUNCTION__[25]; // size: 0x19, address: 0x804F6D40
// Range: 0x800B8B4C -> 0x800B8BF0
void (* xParSysGetRenderFunction(int which /* r31 */))(void *, struct xParGroup *) {
    // References
    // -> static struct xParSysInfo sParSysInfo[7];
    // -> static char __FUNCTION__[25];
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D60430
// Range: 0x800B8BF0 -> 0x800B8E18
void xParCmdTexInit(struct xParCmdTex * cmd /* r31 */) {
    // References
    // -> static char __FUNCTION__[15];
}

// Range: 0x800B8E18 -> 0x800B8E48
void xParSysInit(void * b /* r1+0x8 */, void * tasset /* r1+0xC */) {}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6043F
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x800B8E48 -> 0x800B91F0
void xParSysInit(struct xBase * b /* r20 */, struct xParSysAsset * tasset /* r30 */) {
    // Local variables
    struct xParSys * t; // r31
    unsigned int size; // r22
    unsigned int i; // r29
    unsigned char * cmdPtr; // r25
    unsigned int size; // r21
    unsigned int i; // r28
    unsigned int i; // r26
    struct xParCmdMove & cmd; // r27
...
// Range: 0x800B91F0 -> 0x800B925C
void xParSysSetup(struct xParSys * t /* r31 */) {}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D6044B
// Range: 0x800B925C -> 0x800B93B8
void xParSysReset(struct xParSys * t /* r31 */) {
    // References
    // -> static char __FUNCTION__[13];
}

// Range: 0x800B93B8 -> 0x800B93E8
struct xBase * xParSysExit(struct xBase * b /* r31 */) {}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D60458
// Range: 0x800B93E8 -> 0x800B9488
void xParSysExit(struct xParSys * t /* r31 */) {
    // References
    // -> static char __FUNCTION__[12];
}
...
// total size: 0x0
struct xSerial {};
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D60464
// Range: 0x800B9488 -> 0x800B95F0
void xParSysEventCB(struct xBase * to /* r30 */, unsigned int toEvent /* r1+0x8 */) {
    // Local variables
    struct xParSys * t; // r31
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D60473
// Range: 0x800B95F0 -> 0x800B979C
void xParSysUpdate(struct xBase * to /* r28 */, float dt /* f31 */) {
    // Local variables
    struct xParSys * s; // r30
    struct xParSys * parent; // r29
    struct xParGroup * g; // r31

    // References
    // -> static char __FUNCTION__[14];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60481
// Range: 0x800B979C -> 0x800B9A38
static void xParGroupUpdateR(struct xParSys * s /* r31 */, struct xParGroup * g /* r27 */, float dt /* f31 */) {
    // Local variables
    unsigned int i; // r29
    struct xParCmd * cmd; // r30
    void (* func)(struct xParCmd *, struct xParGroup *, float); // r28

    // References
    // -> static char __FUNCTION__[17];
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D60492
// Range: 0x800B9A38 -> 0x800B9B40
void xParGroupUpdate(struct xParSys * s /* r29 */, struct xParGroup * g /* r1+0x8 */, float dt /* r1+0xC */) {
    // Local variables
    unsigned int i; // r30
    struct xParCmd * cmd; // r31
    void (* func)(struct xParCmd *, struct xParGroup *, float); // r28
