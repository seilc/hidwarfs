/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xParEmitter.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800AFE20 -> 0x800B420C
*/
// total size: 0x8
struct xBaseAsset {
    // Members
...
static struct xParEmitterAsset sSaveEmmiterSettings; // size: 0x54, address: 0x80B94C98
// total size: 0x138
...
static struct xParEmitterPropsAsset sSaveEmmiterPropSettings; // size: 0x138, address: 0x80B94CEC
static struct xParEmitterPropsAsset sDummyProp; // size: 0x138, address: 0x80B94E24
// Range: 0x800AFE20 -> 0x800AFEE4
int xParInterpConvertInterpMode(struct xParInterp * p /* r3 */) {}

// Range: 0x800AFEE4 -> 0x800AFF14
void xParEmitterInit(void * b /* r1+0x8 */, void * tasset /* r1+0xC */) {}
...
// Range: 0x800AFF14 -> 0x800B01B0
void xParEmitterInit(struct xBase * b /* r26 */, struct xParEmitterAsset * pea /* r29 */) {
    // Local variables
    struct xParEmitterPropsAsset * prop; // r30
    struct xParEmitter * t; // r31
    int i; // r28
    float mag2; // f31

    // References
    // -> static struct xParEmitterPropsAsset sDummyProp;
}
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60258
// total size: 0x50
...
// total size: 0x4
struct /* @class$944xParEmitter_cpp */ {
    // Members
...
union /* @class$1083xParEmitter_cpp */ {
...
// total size: 0xC
struct /* @class$1082xParEmitter_cpp */ {
    // Members
...
// Range: 0x800B01B0 -> 0x800B036C
void xParEmitterSetup(struct xParEmitter * t /* r31 */) {
    // Local variables
    struct xEnt * ent; // r30
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60269
// Range: 0x800B036C -> 0x800B0464
void xParEmitterReset(struct xParEmitter * t /* r31 */) {
    // References
    // -> static char __FUNCTION__[17];
}
...
// total size: 0x0
struct xSerial {};
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D6027A
// total size: 0x184
...
// Range: 0x800B0464 -> 0x800B0580
void xParEmitterEventCB(struct xBase * to /* r30 */, unsigned int toEvent /* r1+0x8 */) {
    // Local variables
    struct xParEmitter * t; // r31
    struct xParEmitterCustomSettings fake_settings; // r1+0xC

    // References
    // -> static char __FUNCTION__[19];
}

static char __FUNCTION__[22]; // size: 0x16, address: 0x804F6768
// Range: 0x800B0580 -> 0x800B0920
struct xPar * xParEmitterEmitCustom(struct xParEmitter * p /* r31 */, float dt /* f31 */, struct xParEmitterCustomSettings * info /* r30 */) {
    // Local variables
    unsigned int flags; // r29
    struct xParEmitterAsset * pe; // r28
    struct xPar * emitted_par; // r27

    // References
    // -> static struct xParEmitterPropsAsset sSaveEmmiterPropSettings;
    // -> static struct xParEmitterAsset sSaveEmmiterSettings;
    // -> static char __FUNCTION__[22];
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D6028D
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// total size: 0x10
struct /* @class$1347xParEmitter_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x800B0920 -> 0x800B0A9C
static int xParEmitterCull(struct xParEmitter * pe /* r30 */, struct xPar * p /* r29 */) {
    // Local variables
    struct xParEmitterAsset * a; // r31
    struct xVec3 camera_pos; // r1+0x14
    struct xVec3 d; // r1+0x8
    float dmag; // f31
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D6029D
// Range: 0x800B0A9C -> 0x800B0C84
float xParInterpCompute(int interp_mode /* r1+0x8 */, struct xParInterp * r /* r31 */, float time /* f30 */, int time_has_elapsed /* r1+0xC */, float lastVal /* r1+0x10 */) {
    // Local variables
    float val; // f31
...
static char __FUNCTION__[26]; // size: 0x1A, address: 0x804F67DC
// Range: 0x800B0C84 -> 0x800B0E70
struct xPar * xParEmitterEmitSetTexIdxs(struct xPar * p /* r31 */, const struct xParSys * ps /* r29 */) {
    // Local variables
...
static int doDebugDraw; // size: 0x4, address: 0x80D693D0
static signed char init; // size: 0x1, address: 0x80D693D4
static float rad_cullDebugDraw; // size: 0x4, address: 0x80D693D8
static signed char init; // size: 0x1, address: 0x80D693DC
// total size: 0x70
struct xPar {
    // Members
...
static struct xParGroup temp_group; // size: 0x24, address: 0x80B94F5C
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D602AF
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x800B0E70 -> 0x800B1DC0
struct xPar * xParEmitterEmit(struct xParEmitter * pe /* r30 */, float emit_dt /* f27 */, float par_dt /* f31 */) {
    // Local variables
    struct xPar * last_p; // r1+0x50
    struct xParEmitterAsset * pea; // r1+0x4C
    struct xParEmitterAsset * a; // r27
    struct xParEmitterPropsAsset * prop; // r29
    int rate_has_elapsed; // r1+0x48
    float rate; // f25
    struct xParInterp * r; // r1+0x44
    int count; // r23
    struct xParSys * ps; // r28
    struct xParGroup * g; // r17
    struct xPar * p; // r31
    float life; // f29
    float size_birth; // f26
    float size_death; // f24
    struct xVec3 emitPosition; // r1+0x90
    struct xBase * attachObject; // r19
    unsigned int attachGroupIndex; // r21
    unsigned int attachGroupTotal; // r1+0x40
    int emitAgain; // r18
    int emitterIsAttachedToObject; // r1+0x3C
    struct xBase * emitObj; // r26
    int marker; // r1+0x38
    struct xGroup * g; // r22
    unsigned int get_rnd_group_idx; // r1+0x34
    const struct xEnt * attach_ent; // r25
    const struct xMat4x3 * bone_mat; // r1+0x30
    struct xVec3 bone_vel; // r1+0x84
    struct xVec3 add_vel; // r1+0x78
    struct xVec3 diff; // r1+0x6C
    float ds2; // f21
    float last_emit_time; // f30
    const struct xVec3 & camPos; // r1+0x2C
    int i; // r1+0x28
    int c; // r24
    float fc1; // f23
    float fc2; // f20
    struct xBase * obj; // r20
    unsigned char tempGroupInitialized; // r1+0x8
    struct xPar * save_next; // r1+0x24
    struct xPar * save_prev; // r1+0x20
    float u; // f28
    struct xVec3 pos_diff; // r1+0x60
    float initial_length; // f22
    struct xVec3 diff; // r1+0x54
    float ds2; // f19

    // References
    // -> struct xGlobals * xglobals;
    // -> struct xColor_tag g_PINK;
    // -> static float rad_cullDebugDraw;
    // -> static int doDebugDraw;
    // -> static struct xParGroup temp_group;
    // -> static char __FUNCTION__[16];
...
// total size: 0x890
struct /* @class$1890xParEmitter_cpp */ {
    // Static members
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D602BF
// Range: 0x800B1DC0 -> 0x800B1ED0
void xParEmitterUpdate(struct xBase * to /* r1+0x8 */, float dt /* r1+0xC */) {
    // Local variables
    struct xParEmitter * pe; // r31
...
static char * region_type_names[17]; // size: 0x44, address: 0x804F683C
static char * interp_type_names[8]; // size: 0x20, address: 0x804F6880
static char * render_type_names[7]; // size: 0x1C, address: 0x804F68A0
static char buffer[16]; // size: 0x10, address: 0x80D693DD
// Range: 0x800B1ED0 -> 0x800B1F40
static char * get_asset_name(struct xParEmitterAsset * asset /* r30 */) {
    // Local variables
    const char * name; // r31
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x804F68BC
// total size: 0xC
struct /* @class$756xParEmitter_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$757xParEmitter_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$758xParEmitter_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$759xParEmitter_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$760xParEmitter_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$761xParEmitter_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$762xParEmitter_cpp */ {
    // Members
...
// Range: 0x800B1F40 -> 0x800B2030
static void on_change_circle_dir(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xParEmitterAsset & asset; // r31
    float mag2; // f31
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x804F68D1
// Range: 0x800B2030 -> 0x800B2190
static void on_change_volume_entity(const struct tweak_info & info /* r28 */) {
    // Local variables
    struct xParEmitter & emit; // r30
    unsigned int id; // r27
    struct xEnt * ent; // r31
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x804F68E9
// Range: 0x800B2190 -> 0x800B2268
static void on_change_bone_index(const struct tweak_info & info /* r28 */) {
    // Local variables
    struct xParEmitter & emit; // r30
    struct xParEmitterAsset & asset; // r27
    struct xPEEntBone & region; // r29
    struct xEnt * ent; // r31
...
static struct tweak_callback cb_volume_entity; // size: 0x28, address: 0x804F6900
static signed char init; // size: 0x1, address: 0x80D693ED
static struct tweak_callback cb_circle_dir; // size: 0x28, address: 0x804F6928
static signed char init; // size: 0x1, address: 0x80D693EE
static char * type_labels[11]; // size: 0x2C, address: 0x804F6950
static struct tweak_callback cb_bone_index; // size: 0x28, address: 0x804F697C
static signed char init; // size: 0x1, address: 0x80D693EF
static char * type_labels[2]; // size: 0x8, address: 0x80D602D4
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D602DC
...
// Range: 0x800B2268 -> 0x800B2DD8
static void add_region_tweaks(struct xParEmitter & emit /* r29 */, unsigned char reset /* r30 */) {
    // Local variables
    struct xParEmitterAsset & asset; // r31
    char prefix[128]; // r1+0xE90
    char _buffer[128]; // r1+0xE10
    char _buffer[128]; // r1+0xD90
    char _buffer[128]; // r1+0xD10
    char _buffer[128]; // r1+0xC90
    char _buffer[128]; // r1+0xC10
    char _buffer[128]; // r1+0xB90
    char _buffer[128]; // r1+0xB10
    char _buffer[128]; // r1+0xA90
    char _buffer[128]; // r1+0xA10
    char _buffer[128]; // r1+0x990
    char _buffer[128]; // r1+0x910
    char _buffer[128]; // r1+0x890
    char _buffer[128]; // r1+0x810
    char _buffer[128]; // r1+0x790
    char _buffer[128]; // r1+0x710
    char _buffer[128]; // r1+0x690
    char _buffer[128]; // r1+0x610
    char _buffer[128]; // r1+0x590
    char _buffer[128]; // r1+0x510
    char _buffer[128]; // r1+0x490
    char _buffer[128]; // r1+0x410
    char _buffer[128]; // r1+0x390
    char _buffer[128]; // r1+0x310
    char _buffer[128]; // r1+0x290
    char _buffer[128]; // r1+0x210
    char _buffer[128]; // r1+0x190
    char _buffer[128]; // r1+0x110
    char _buffer[128]; // r1+0x90

    // References
    // -> static char __FUNCTION__[18];
    // -> static char * type_labels[2];
    // -> static char * type_labels[11];
    // -> static struct tweak_callback cb_bone_index;
    // -> static signed char init;
    // -> static struct tweak_callback cb_circle_dir;
    // -> static signed char init;
    // -> static struct tweak_callback cb_volume_entity;
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x804F69E8
// Range: 0x800B2DD8 -> 0x800B2E78
static void on_change_region_type(const struct tweak_info & info /* r31 */) {
    // Local variables
    struct xParEmitter & emit; // r30
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D602EE
// Range: 0x800B2E78 -> 0x800B2FF8
static void on_change_cmd_count(const struct tweak_info & info /* r26 */) {
    // Local variables
    struct xParEmitter & emit; // r29
    struct xParSys & sys; // r31
    char prefix[128]; // r1+0x8
    char * end; // r25
    const struct tweak_info * parent; // r24
    unsigned int size; // r28
    unsigned int i; // r30
    struct xParCmd & cmd; // r23
    unsigned int i; // r27
...
// Range: 0x800B2FF8 -> 0x800B3150
static void add_interp_tweak(const char * cat /* r1+0x8 */, const char * title /* r1+0xC */, struct xParInterp & pi /* r31 */) {
    // Local variables
    char prefix[128]; // r1+0x210
    char _buffer[128]; // r1+0x190
    char _buffer[128]; // r1+0x110
    char _buffer[128]; // r1+0x90
    ...
    // References
    // -> static char * interp_type_names[8];
}

// Range: 0x800B3150 -> 0x800B3174
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x804F69FE
// Range: 0x800B3174 -> 0x800B3220
static void on_change_render_type(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xParSys & sys; // r31
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D60302
// Range: 0x800B3220 -> 0x800B32E4
static void on_change_attach_to(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xParEmitter & emit; // r31
    unsigned int id; // r29
...
// Range: 0x800B32E4 -> 0x800B334C
static void on_change_texture(const struct tweak_info & info /* r29 */) {
    // Local variables
    struct xParSys & sys; // r31
    struct xParSysAsset & asys; // r30
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D60316
// Range: 0x800B334C -> 0x800B33E4
static void on_expand_emitter(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xParEmitter * emit; // r31
...
static struct tweak_callback cb_explode; // size: 0x28, address: 0x804F6A14
static signed char init; // size: 0x1, address: 0x80D693F0
// Range: 0x800B33E4 -> 0x800B34D4
static void add_tweaks(struct xParEmitter & emit /* r31 */) {
    // Local variables
...
static struct tweak_callback cb_emit_type; // size: 0x28, address: 0x804F6A3C
static signed char init; // size: 0x1, address: 0x80D693F1
static struct tweak_callback cb_attach_to; // size: 0x28, address: 0x804F6A64
static signed char init; // size: 0x1, address: 0x80D693F2
static struct tweak_callback cb_render_type; // size: 0x28, address: 0x804F6A8C
static signed char init; // size: 0x1, address: 0x80D693F3
static struct tweak_callback cb_change_texture; // size: 0x28, address: 0x804F6AB4
static signed char init; // size: 0x1, address: 0x80D693F4
static struct tweak_callback cb_cmd_count; // size: 0x28, address: 0x804F6ADC
static signed char init; // size: 0x1, address: 0x80D693F5
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D60328
...
// Range: 0x800B34D4 -> 0x800B420C
static void explode_tweaks(struct xParEmitter & emit /* r31 */) {
    // Local variables
    const char * asset_name; // r23
    char prefix[128]; // r1+0xF60
    char * end; // r27
    char _buffer[128]; // r1+0xEE0
    char _buffer[128]; // r1+0xE60
    char _buffer[128]; // r1+0xDE0
    char _buffer[128]; // r1+0xD60
    char _buffer[128]; // r1+0xCE0
    char _buffer[128]; // r1+0xC60
    char _buffer[128]; // r1+0xBE0
    char _buffer[128]; // r1+0xB60
    char _buffer[128]; // r1+0xAE0
    char _buffer[128]; // r1+0xA60
    struct xParEmitterAsset & asset; // r26
    char _buffer[128]; // r1+0x9E0
    char _buffer[128]; // r1+0x960
    char _buffer[128]; // r1+0x8E0
    char _buffer[128]; // r1+0x860
    char _buffer[128]; // r1+0x7E0
    struct xParEmitterPropsAsset & props; // r29
    struct xParSys & sys; // r30
    struct xParSysAsset & asys; // r28
    char _buffer[128]; // r1+0x760
    char _buffer[128]; // r1+0x6E0
    char _buffer[128]; // r1+0x660
    char _buffer[128]; // r1+0x5E0
    char _buffer[128]; // r1+0x560
    char _buffer[128]; // r1+0x4E0
    char _buffer[128]; // r1+0x460
    char _buffer[128]; // r1+0x3E0
    char _buffer[128]; // r1+0x360
    char _buffer[128]; // r1+0x2E0
    char _buffer[128]; // r1+0x260
    char _buffer[128]; // r1+0x1E0
    char _buffer[128]; // r1+0x160
    unsigned int max_commands; // r22
    char _buffer[128]; // r1+0xE0
    unsigned int i; // r25
    struct xParCmd & cmd; // r21

    // References
    // -> static struct tweak_callback cb_cmd_count;
    // -> static signed char init;
    ...
    // -> static signed char init;
    // -> static struct tweak_callback cb_render_type;
    // -> static char * render_type_names[7];
    // -> static signed char init;
    // -> char * tweak_blend_mode_labels[11];
    // -> static struct tweak_callback cb_attach_to;
    // -> unsigned int all_entity_count;
    // -> unsigned int * all_entity_ids;
    // -> const char * * all_entity_labels;
    // -> static struct tweak_callback cb_emit_type;
    // -> static char * region_type_names[17];
    // -> static signed char init;
    // -> static signed char init;
