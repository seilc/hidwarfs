/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\gc\iWater.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80236940 -> 0x80238544
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
struct RpLight * iWaterDirectionalLight; // size: 0x4, address: 0x80D6AFB4
struct FxParam * iWaterEMBM; // size: 0x4, address: 0x80D6AFB8
// total size: 0x20
struct xLinkAsset {
    // Members
...
struct xEnt * waterObjects[8]; // size: 0x20, address: 0x80D18640
unsigned int numWaterObjects; // size: 0x4, address: 0x80D6AFBC
struct xEnt * water2; // size: 0x4, address: 0x80D6AFC0
struct xEnt * water3; // size: 0x4, address: 0x80D6AFC4
struct xEnt * water4; // size: 0x4, address: 0x80D6AFC8
struct xEnt * water5; // size: 0x4, address: 0x80D6AFCC
struct xEnt * water7; // size: 0x4, address: 0x80D6AFD0
float frameCountScale; // size: 0x4, address: 0x80D64E08
int bump_mode; // size: 0x4, address: 0x80D64E0C
unsigned char dff_alpha; // size: 0x1, address: 0x80D64E10
int env_clamp; // size: 0x4, address: 0x80D6AFD4
int override_width; // size: 0x4, address: 0x80D64E14
int water_frame; // size: 0x4, address: 0x80D64E18
float yscale; // size: 0x4, address: 0x80D64E1C
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
struct xVec3 camscale; // size: 0xC, address: 0x80D64E20
static struct RwTexture * saveBump; // size: 0x4, address: 0x80D6AFD8
// total size: 0x18
struct bump_layer {
    // Members
    float u; // offset 0x0, size 0x4
    float v; // offset 0x4, size 0x4
    float us; // offset 0x8, size 0x4
    float vs; // offset 0xC, size 0x4
    float scale; // offset 0x10, size 0x4
    unsigned char alpha; // offset 0x14, size 0x1
    unsigned char pad[3]; // offset 0x15, size 0x3
};
struct bump_layer bump_layers[4]; // size: 0x60, address: 0x80523DA0
int num_bump_layers; // size: 0x4, address: 0x80D64E2C
static struct RwTexture * bumpTex; // size: 0x4, address: 0x80D6AFDC
static struct RwRaster * bumpRaster; // size: 0x4, address: 0x80D6AFE0
// total size: 0x10
struct RwRGBAReal {
    // Members
...
// Range: 0x80236940 -> 0x802369CC
static struct RpLight * CreateDirectionalLight() {
    // Local variables
    struct RpLight * light; // r31
    struct RwFrame * frame; // r30
    struct RwRGBAReal color; // r1+0x8
}
...
static struct RwCamera * saveCamera; // size: 0x4, address: 0x80D6AFE4
static struct RwCamera * bumpCamera; // size: 0x4, address: 0x80D6AFE8
static struct RwCamera * clonedShadowCamera; // size: 0x4, address: 0x80D6AFEC
// total size: 0x18
...
// Range: 0x802369CC -> 0x80236BC0
static void iWaterBumpGenRenderLayer(struct RwCamera * camera /* r29 */, struct RwTexture * texture /* r1+0x8 */, float u /* f29 */, float v /* f30 */, unsigned char alpha /* r31 */, float scale /* f31 */) {
    // Local variables
    struct rwGameCube2DVertex vx[4]; // r1+0x10
    float crw; // f28
    float crh; // f27
    struct RwRaster * camRas; // r30
    float recipZ; // r1+0xC
}

// Range: 0x80236BC0 -> 0x80236C4C
static int iWaterBumpGenBegin(struct RwCamera * bumpCamera /* r30 */, struct RwRGBA * bgColor /* r31 */) {
    // References
    // -> static struct RwCamera * saveCamera;
}
...
// Range: 0x80236C4C -> 0x80236CB0
static void iWaterBumpGenEnd() {
    // References
    // -> struct xGlobals * xglobals;
    // -> static struct RwCamera * saveCamera;
    // -> static struct RwCamera * bumpCamera;
}

static float foo; // size: 0x4, address: 0x80D6AFF0
...
// Range: 0x80236CB0 -> 0x80236E94
static struct RwTexture * iWaterBumpGen(struct RwTexture * bump /* r24 */, struct RwTexture * src /* r25 */) {
    // Local variables
    unsigned char genNeeded; // r27
    int i; // r30
    struct xModelInstance * modelInst; // r29
    struct RwRGBA black; // r1+0x8
    struct bump_layer * bp; // r31
    int i; // r28
    ...
    // -> static float foo;
    // -> static struct RwCamera * bumpCamera;
    // -> struct bump_layer bump_layers[4];
    // -> static struct RwCamera * clonedShadowCamera;
    // -> static struct RwRaster * bumpRaster;
    // -> struct RwCamera * ShadowCamera;
    // -> float frameCountScale;
    // -> float sTimeElapsed;
    // -> unsigned int numWaterObjects;
    // -> struct xEnt * waterObjects[8];
}

struct RwTexture * iWaterBumpTex; // size: 0x4, address: 0x80D6B060
// Range: 0x80236E94 -> 0x80236EC8
void iWaterBumpGen() {
    // References
    // -> struct RwTexture * iWaterBumpTex;
    // -> static struct RwTexture * bumpTex;
    // -> int bump_mode;
}

// Range: 0x80236EC8 -> 0x80236EF4
void iWaterCreateMTEffect() {
    // References
    // -> int gGCwaterDisable;
}

static struct xLightKit * waterLightKit; // size: 0x4, address: 0x80D6AFF4
static struct RwRGBAReal waterHackColor; // size: 0x10, address: 0x80D64E30
// Range: 0x80236EF4 -> 0x80236F9C
static void iWaterObjectRender(struct xEnt * ent /* r31 */) {
    // References
    // -> static struct RwRGBAReal waterHackColor;
    // -> static struct xLightKit * waterLightKit;
}

float bw_factor; // size: 0x4, address: 0x80D64E40
static struct RpAtomic * (* defaultRenderCB)(struct RpAtomic *); // size: 0x4, address: 0x80D6AFF8
// Range: 0x80236F9C -> 0x80236FCC
static struct RpAtomic * iWaterAtomicRender(struct RpAtomic * atomic /* r1+0x8 */) {
    // References
    // -> static struct RpAtomic * (* defaultRenderCB)(struct RpAtomic *);
}

unsigned int waterTexID; // size: 0x4, address: 0x80D64E44
// total size: 0x28
struct PKRAssetType {
    // Members
...
// Range: 0x80236FCC -> 0x80237098
static void iWaterObjectSetup(struct xEnt * ent /* r25 */) {
    // Local variables
    struct RpAtomic * atomic; // r26
    struct RpGeometry * geom; // r30
    struct RwTexture * newtex; // r29
    int i; // r31
...
float BumpWidth; // size: 0x4, address: 0x80D64E54
float BumpHeight; // size: 0x4, address: 0x80D64E50
float Shininess; // size: 0x4, address: 0x80D64E58
int EnvMode; // size: 0x4, address: 0x80D6B034
// total size: 0x0
struct asset_type {};
// total size: 0x0
struct debug_info {};
...
// total size: 0x1
struct /* @class$771iWater_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$772iWater_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$773iWater_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$774iWater_cpp */ {
    // Members
...
// total size: 0x3C
class zWaterBody : private xBase {
    // Members
public:
    int motion_type; // offset 0x10, size 0x4
private:
    struct asset_type * asset; // offset 0x14, size 0x4
    struct xEnt * body_ent; // offset 0x18, size 0x4
    struct xEnt * * refract_ents; // offset 0x1C, size 0x4
    int refract_ents_size; // offset 0x20, size 0x4
    struct xEnt * * reflect_ents; // offset 0x24, size 0x4
    int reflect_ents_size; // offset 0x28, size 0x4
    int body_id; // offset 0x2C, size 0x4
    unsigned char visible; // offset 0x30, size 0x1
    int motion_cfg_id; // offset 0x34, size 0x4
...
// Range: 0x80237098 -> 0x802377F4
void iWaterSceneEnter() {
    // Local variables
    struct zScene & scene; // r27
    class zWaterBody * it; // r28
    class zWaterBody * end; // r23
    unsigned int body; // r30
    struct zEntSimpleObj * ent; // r24
    int flags; // r22
    int i; // r26
    struct xEnt * ent; // r31
    struct RwMatrixTag newMat; // r1+0x88
    char pfx[64]; // r1+0x48
    char str[64]; // r1+0x8
    struct bump_layer * bp; // r29
    int i; // r25
    ...
    // -> int num_bump_layers;
    // -> struct bump_layer bump_layers[4];
    // -> struct xVec3 camscale;
    // -> float yscale;
    // -> int override_width;
    // -> int env_clamp;
    // -> unsigned char dff_alpha;
    // -> float frameCountScale;
    // -> int bump_mode;
    // -> float bw_factor;
    // -> int EnvMode;
    // -> unsigned int numWaterObjects;
    // -> struct xEnt * waterObjects[8];
    // -> struct RpLight * iWaterDirectionalLight;
    // -> static struct RwTexture * saveBump;
    // -> static struct RwRaster * bumpRaster;
    // -> static struct RwTexture * bumpTex;
    // -> int gGCwaterDisable;
}

// Range: 0x802377F4 -> 0x802378BC
void iWaterSceneExit() {
    // Local variables
...
static unsigned int override_bump; // size: 0x4, address: 0x80D6AFFC
static signed char init; // size: 0x1, address: 0x80D6B000
static unsigned int override_env; // size: 0x4, address: 0x80D6B004
static signed char init; // size: 0x1, address: 0x80D6B008
static float uvscale; // size: 0x4, address: 0x80D6B00C
static signed char init; // size: 0x1, address: 0x80D6B010
static float range_scale; // size: 0x4, address: 0x80D6B014
static signed char init; // size: 0x1, address: 0x80D6B018
static unsigned char render_texture; // size: 0x1, address: 0x80D6B019
static signed char init; // size: 0x1, address: 0x80D6B01A
static unsigned char _temp_tweaks_registered; // size: 0x1, address: 0x80D6B01B
static signed char init; // size: 0x1, address: 0x80D6B01C
struct RwTexture * iWaterEnvTex; // size: 0x4, address: 0x80D6B064
...
struct FxParam EMBM; // size: 0x14, address: 0x80D6B020
struct RwFrame * FxFrameArray[2]; // size: 0x8, address: 0x80D64E60
...
// Range: 0x802378BC -> 0x80237F58
static int iWaterUpdateEMBM(int check /* r1+0x10 */) {
    // Local variables
    struct RpMultiTexture * mt; // r26
    struct RpGameCubeMTConfig * config; // r31
    unsigned int rasSize; // r25
    float scale; // f31
    struct RwTexture * bump; // r30
    struct RwTexture * env; // r28
    ...
    // -> static struct RwTexture * saveBump;
    // -> static struct RwTexture * bumpTex;
    // -> static float range_scale;
    // -> static unsigned int override_env;
    // -> static unsigned int override_bump;
...
unsigned int enableWater; // size: 0x4, address: 0x80D64E48
// Range: 0x80237F58 -> 0x80237FB4
void iWaterEnable(struct RpAtomic * atomic /* r31 */) {
    // References
    // -> struct FxParam * iWaterEMBM;
    // -> unsigned int enableWater;
    // -> int gGCwaterDisable;
}

// Range: 0x80237FB4 -> 0x802382C4
void iWaterUpdate() {
    // Local variables
    struct xVec3 playerPos; // r1+0x8
    float dist2; // f30
    float dist3; // f29
    float dist4; // f28
    float dist5; // f27
    float dist7; // f26
    int closestWater; // r30
    float closestDist; // f31
    int i; // r31
...
// Range: 0x802382C4 -> 0x802383F8
void iWaterReset() {
    // Local variables
    int i; // r30
    struct zEntSimpleObj * ent; // r31
    struct zScene * s; // r1+0x8
...
// Range: 0x802383F8 -> 0x80238544
void CalcNBT(struct _rwDlNBTDataTag * nbtData /* r3 */) {}
