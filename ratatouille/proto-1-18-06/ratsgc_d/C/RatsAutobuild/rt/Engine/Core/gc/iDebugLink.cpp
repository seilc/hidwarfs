/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\gc\iDebugLink.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8028412C -> 0x8028483C
*/
static long exiChan; // size: 0x4, address: 0x80D6B340
unsigned long mailbox; // size: 0x4, address: 0x80D6B344
// Range: 0x8028412C -> 0x80284170
int hioEnumCallback(long chan /* r31 */) {
    // References
    // -> static long exiChan;
}

// Range: 0x80284170 -> 0x80284190
void hioCallback() {}

// Range: 0x80284190 -> 0x802841D0
...
// total size: 0x4
struct /* @class$892iDebugLink_cpp */ {
    // Members
...
union /* @class$1031iDebugLink_cpp */ {
...
// total size: 0xC
struct /* @class$1030iDebugLink_cpp */ {
    // Members
...
// total size: 0x83D8
class iDebugLink : public xDebugLink {
    // Functions
    enum eError Initialize();

    enum eError ReadyForAssetData();
};
// Range: 0x802841D0 -> 0x80284234
void iDebugLink::Allocate() {
    // References
...
// Range: 0x80284270 -> 0x802843F0
// this: r31
enum eError iDebugLink::Initialize() {
    // References
    // -> static long exiChan;
}

// Range: 0x802843F0 -> 0x80284594
// this: r31
enum eError iDebugLink::ReadyForAssetData() {
    // Local variables
    unsigned char hdrBuffer[32]; // r1+0x20
    unsigned int assetSize; // r30
    const char * assetName; // r27
    int roundedAssetSize; // r28
    unsigned char * buffer; // r29
    enum eError retval; // r26
}

// Range: 0x80284594 -> 0x80284700
enum eError iDebugLink::SendEntEvent() {
    // Local variables
    unsigned char hdrBuffer[32]; // r1+0x40
    float params[4]; // r1+0x20
    unsigned int widgetID; // r31
    unsigned int event; // r30
    unsigned int paramWidgetID; // r29
    struct xBase * sceneObject; // r28
}

// Range: 0x80284700 -> 0x80284820
void iDebugLink::Update() {
    // References
    // -> unsigned long mailbox;
    // -> struct xGlobals * xglobals;
}

// Range: 0x80284820 -> 0x8028483C
enum eError iDebugLink::HeartBeatResponse(char * * argv /* r4 */, int argc /* r5 */) {}

// total size: 0x30
struct {} iDebugLink::__vtable; // size: 0x30, address: 0x80525110
