/*
    ...
    Language: C++
    Code range: 0x80055CF8 -> 0x8005EFC8
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static float nsn_angle; // size: 0x4, address: 0x80D5F1B0
// total size: 0x18
struct xBox {
    // Members
...
static struct xBox all_ents_box; // size: 0x18, address: 0x80B5EA40
static int all_ents_box_init; // size: 0x4, address: 0x80D68F50
// total size: 0x20
struct xLinkAsset {
    // Members
...
static struct xEnt * * dbg_xents; // size: 0x4, address: 0x80D68F54
static unsigned short dbg_num; // size: 0x2, address: 0x80D68F58
static signed short dbg_idx; // size: 0x2, address: 0x80D5F1B4
static int dbg_mode; // size: 0x4, address: 0x80D68F5C
static int dbg_holding_inc; // size: 0x4, address: 0x80D68F60
static int dbg_holding_dec; // size: 0x4, address: 0x80D68F64
static float dbg_tmr; // size: 0x4, address: 0x80D68F68
static float dbg_ltm; // size: 0x4, address: 0x80D68F6C
// total size: 0x20
struct xModCollRec {
    // Members
    struct xEnt * owner; // offset 0x0, size 0x4
    unsigned int nusing; // offset 0x4, size 0x4
    long long ctime; // offset 0x8, size 0x8
    long long ltime; // offset 0x10, size 0x8
    float frac; // offset 0x18, size 0x4
};
static struct xModCollRec * mcrecs; // size: 0x4, address: 0x80D68F70
static int num_mcrecs; // size: 0x4, address: 0x80D68F74
static long long mctime; // size: 0x8, address: 0x80D68F78
static struct xModCollRec smcrecs[150]; // size: 0x12C0, address: 0x80B5EA58
enum eModCollMode {
    eModCollMode_None = 0,
    eModCollMode_CTime = 1,
    eModCollMode_LTime = 2,
};
static enum eModCollMode mcmode; // size: 0x4, address: 0x80D68F80
static char __FUNCTION__[6]; // size: 0x6, address: 0x80D5F1B6
...
// Range: 0x80055CF8 -> 0x80056084
static void reset(struct xEnt & ent /* r31 */) {
    // Local variables
...
static char __FUNCTION__[8]; // size: 0x8, address: 0x80D5F1BC
// Range: 0x80056084 -> 0x800562FC
static void refresh(struct xEnt & ent /* r31 */) {
    // Local variables
    struct xModelInstance & model; // r30
    struct xMat4x3 & mat; // r26
    struct anim_coll_data & ac; // r28
    struct xMat4x3 & bone_mat; // r29
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D5F1C4
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5F1CD
// Range: 0x800562FC -> 0x80056324
void xEntSceneInit() {
    // References
    // -> static int all_ents_box_init;
}

// Range: 0x80056324 -> 0x80056344
void xEntSceneExit() {}

// Range: 0x80056344 -> 0x800564C0
static void xEntAddHittableFlag(struct xEnt * ent /* r3 */) {
    // Local variables
    unsigned int i; // r31
}

static unsigned int receive_models[29]; // size: 0x74, address: 0x804F2C58
// Range: 0x800564C0 -> 0x8005653C
static void hack_receive_shadow(struct xEnt * ent /* r3 */) {
    // Local variables
    const unsigned int * end_receive_models; // r30
...
// Range: 0x8005653C -> 0x800565FC
static void xEntAddShadowRecFlag(struct xEnt * ent /* r31 */) {}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D5F1D7
// Range: 0x800565FC -> 0x800567D4
void xEntInit(struct xEnt * ent /* r31 */, struct xEntAsset * asset /* r30 */) {
    // References
    // -> static char __FUNCTION__[9];
}

// Range: 0x800567D4 -> 0x8005683C
void xEntGrowGlobalBoundingBox(const struct xVec3 * pos /* r31 */) {
    // References
    // -> static struct xBox all_ents_box;
    // -> static int all_ents_box_init;
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5F1E0
// Range: 0x8005683C -> 0x80056ADC
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5F1F0
// Range: 0x80056ADC -> 0x80056C18
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5F201
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x80056C18 -> 0x80056E90
static unsigned char collide_downward(struct xVec3 & loc /* r28 */, struct xEnt * & hit /* r27 */, struct xScene & s /* r1+0x8 */, struct xEnt & ent /* r31 */, float max_dist /* r1+0xC */) {
    // Local variables
    float lower_bound_extent; // f31
    struct xVec3 & center; // r30
    struct xRay3 ray; // r1+0x24
    unsigned char old_bound_type; // r25
    float old_bound_radius; // f30
    struct xCollis coll; // r1+0x94
    struct xSphere * spheres[5]; // r1+0x10
    int count; // r26
    int i; // r29
    struct xBound b; // r1+0x48
...
// total size: 0x1
struct /* @class$846xEnt_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$847xEnt_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$848xEnt_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$849xEnt_cpp */ {
    // Members
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x804F2D10
// Range: 0x80056E90 -> 0x80056F20
static void stacked_owner_destroyed(void * context /* r31 */) {
    // References
...
// Range: 0x80056F20 -> 0x80056FDC
static void mount_stacked_entity(struct xEnt & ent /* r30 */, struct xEnt & driver /* r31 */) {
    // Local variables
    struct zDestructible * durst; // r29
}

static char __FUNCTION__[27]; // size: 0x1B, address: 0x804F2D28
// Range: 0x80056FDC -> 0x800570F0
static void drop_driven_stacked_entity(struct xEnt & ent /* r31 */) {
    // References
    // -> static char __FUNCTION__[27];
}

// Range: 0x800570F0 -> 0x80057188
static void dismount_stacked_entity(struct xEnt & ent /* r31 */) {
    // Local variables
    struct zDestructible * durst; // r30
}

// Range: 0x80057188 -> 0x80057194
static void setup_stacked_entity(struct xEnt & ent /* r3 */) {}

// Range: 0x80057194 -> 0x800571C8
static void drop_stacked_entity(struct xEnt & ent /* r31 */) {}

// Range: 0x800571C8 -> 0x800571D4
static void stop_stacked_entity(struct xEnt & ent /* r3 */) {}

// Range: 0x800571D4 -> 0x80057398
static void update_stacked_entity(struct xScene & s /* r29 */, struct xEnt & ent /* r31 */, float dt /* f30 */) {
    // Local variables
    float dist; // f31
    struct xVec3 hitloc; // r1+0x24
    struct xEnt * hitent; // r1+0x8
    struct xVec3 initialObjEuler; // r1+0x18
    struct xMat4x3 * objMat; // r30
    struct xVec3 destObjEuler; // r1+0xC
}

// Range: 0x80057398 -> 0x800573C0
void xEntDrop(struct xEnt * ent /* r1+0x8 */) {}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5F212
// total size: 0x1C
...
// Range: 0x800573C0 -> 0x800578C0
void xEntSetup(struct xEnt * ent /* r31 */) {
    // Local variables
    struct xSurface * surf; // r29
    struct zSurfaceProps * pp; // r27
    struct xModelInstance * minst; // r30
    struct xModelInstance * collmod; // r26
    struct PKRAssetTOCInfo ainfo; // r1+0xC
    int i; // r28
    int numModel; // r25
    const char * assetname; // r1+0x8
...
// Range: 0x800578C0 -> 0x80057944
static void HackHoldableTurret(struct xEnt * ent /* r31 */) {}

static int been_here; // size: 0x4, address: 0x80D68F84
static int been_here; // size: 0x4, address: 0x80D68F88
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5F21C
// Range: 0x80057944 -> 0x80057BBC
void xEntDrivenBySetup(struct xEnt * ent /* r31 */) {
    // Local variables
    int i; // r28
    const struct xLinkAsset * la; // r29
    struct xEnt * dent; // r30
...
static int been_here; // size: 0x4, address: 0x80D68F8C
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5F22E
// Range: 0x80057BBC -> 0x80057CD4
void xEntDrivenByReset(struct xEnt * ent /* r31 */) {
    // Local variables
    int i; // r30
    const struct xLinkAsset * la; // r29
    struct xEnt * dent; // r28
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D5F240
// Range: 0x80057CD4 -> 0x80057DF4
void xEntSave(struct xEnt * ent /* r30 */, struct xSerial * s /* r31 */) {
    // References
    // -> static char __FUNCTION__[9];
}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D5F249
// Range: 0x80057DF4 -> 0x80057F20
void xEntLoad(struct xEnt * ent /* r31 */, struct xSerial * s /* r30 */) {
    // Local variables
    int b; // r1+0x8
...
// Range: 0x80057F20 -> 0x80057F5C
void xEntReset(struct xEnt * ent /* r31 */) {}

static int been_here; // size: 0x4, address: 0x80D68F90
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5F252
unsigned char gInSceneResetOrInit; // size: 0x1, address: 0x80D6A8A0
struct xVec3 g_O3; // size: 0xC, address: 0x80D6D198
// Range: 0x80057F5C -> 0x80058660
void xEntReset(struct xEnt * ent /* r31 */, const struct xVec3 & pos /* r1+0x10 */, const struct xVec3 & ang /* r29 */) {
    // Local variables
    struct xMat4x3 frame; // r1+0x20
    struct xModelInstance * minst; // r30
    struct xBox * combinedAnimBound; // r28
    unsigned short old_flags; // r25
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5F25C
// Range: 0x80058660 -> 0x80058804
struct xModelInstance * xEntLoadModel(struct xEnt * ent /* r28 */, struct RpAtomic * imodel /* r29 */, struct xModelInstance * reuseInstance /* r31 */) {
    // Local variables
    struct xModelInstance * nextModel; // r27
    struct xModelInstance * model; // r30
...
// Range: 0x80058804 -> 0x80058840
static void xEntAddToPos(struct xEnt * ent /* r31 */, const struct xVec3 * dpos /* r1+0x8 */) {}

static int setMaterialTextureRestore; // size: 0x4, address: 0x80D68F94
int LocateSurfTexture_TexAnim(zSurfaceProps*, int)::useGroups; // size: 0x4, address: 0x80D68F98
signed char LocateSurfTexture_TexAnim(zSurfaceProps*, int)::localstatic4 guard; // size: 0x1, address: 0x80D68F9C
char __FUNCTION__$localstatic1$LocateSurfTexture_TexAnim__FP13zSurfacePropsi[26]; // size: 0x1A, address: 0x804F2D43
// Range: 0x80058840 -> 0x80058874
void xEntSetupPipeline(struct xModelInstance * model /* r31 */) {}

unsigned char ReportUVProb; // size: 0x1, address: 0x80D5F8A8
// Range: 0x80058874 -> 0x80058A54
void xEntSetupPipeline(struct xSurface * surf /* r30 */, struct RpAtomic * model /* r23 */) {
    // Local variables
    int setPipeline; // r29
    struct zSurfaceProps * pp; // r31
    int tdx; // r25
    struct RwTexture * texptr; // r27
    struct RwTexture * texptr; // r28
    int tdx; // r1+0x8
    unsigned char dualPass; // r24
    char err[1024]; // r1+0xC

    // References
    // -> unsigned char ReportUVProb;
    // -> static int setMaterialTextureRestore;
}

// Range: 0x80058A54 -> 0x80058B78
struct RwTexture * LocateSurfTexture_TexAnim(struct zSurfaceProps * surfProp /* r31 */, int idx_pass /* r28 */) {
    // Local variables
    struct RwTexture * txtr; // r30
    unsigned int idx_grpItem; // r27
    struct xGroup * grp; // r29

    // References
    // -> char __FUNCTION__$localstatic1$LocateSurfTexture_TexAnim__FP13zSurfacePropsi[26];
    // -> int LocateSurfTexture_TexAnim(zSurfaceProps*, int)::useGroups;
    // -> signed char LocateSurfTexture_TexAnim(zSurfaceProps*, int)::localstatic4 guard;
}

// Range: 0x80058B78 -> 0x80058BAC
void xEntRestorePipeline(struct xModelInstance * model /* r31 */) {}

// Range: 0x80058BAC -> 0x80058BE8
void xEntRestorePipeline(struct RpAtomic * model /* r1+0x8 */) {
    // References
    // -> static int setMaterialTextureRestore;
}

struct xEnt * gEnt_Debug; // size: 0x4, address: 0x80D68FA0
int gEnt_ActiveModel; // size: 0x4, address: 0x80D68FA4
int gEnt_ShowBone; // size: 0x4, address: 0x80D5F26C
int gEnt_ShowVertex; // size: 0x4, address: 0x80D5F270
struct xVec3 gEnt_ShowBoneLoc; // size: 0xC, address: 0x80D68FA8
struct xVec3 gEnt_ShowVertexLoc; // size: 0xC, address: 0x80D68FB4
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D5F274
int gDrawInvisibleWalls; // size: 0x4, address: 0x80D6A5C0
// Range: 0x80058BE8 -> 0x80058E34
void xEntRender(struct xEnt * ent /* r31 */) {
    ...
    // References
    // -> int gDrawInvisibleWalls;
    // -> static struct xModCollRec smcrecs[150];
...
// Range: 0x80058E34 -> 0x80059198
static void xEntRender_DebugInfo(struct xEnt * ent /* r30 */) {
    // Local variables
    struct xModelInstance * active_model; // r31
    int i; // r29
    struct xMat4x3 mat_bone; // r1+0x30

    // References
    // -> struct xVec3 gEnt_ShowVertexLoc;
    // -> struct xGlobals * xglobals;
    // -> struct xColor_tag g_YELLOW;
    // -> struct xColor_tag g_RED;
    // -> int gEnt_ShowVertex;
    // -> struct xVec3 gEnt_ShowBoneLoc;
    // -> struct xColor_tag g_MAGENTA;
    // -> int gEnt_ShowBone;
    // -> struct xColor_tag g_NEON_GREEN;
    // -> int gEnt_ActiveModel;
    // -> struct xEnt * gEnt_Debug;
    // -> struct xColor_tag g_ORANGE;
    // -> struct xColor_tag g_PIMP_GOLD;
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D5F27F
// total size: 0x118
struct xStats {
    // Members
...
// Range: 0x80059198 -> 0x800593E0
void xEntUpdate(struct xEnt * ent /* r31 */, struct xScene * sc /* r30 */, float dt /* f31 */) {
    // Local variables
    long long t0; // r28
    long long tec; // r26
    long long t1; // r24
...
// Range: 0x800593E0 -> 0x80059498
void xEntBeginUpdate(struct xEnt * ent /* r31 */, float dt /* r1+0x8 */) {}

// Range: 0x80059498 -> 0x80059608
void xEntEndUpdate(struct xEnt * ent /* r31 */, struct xScene * sc /* r1+0x8 */, float dt /* r1+0xC */) {
    // Local variables
    struct xVec3 * mpos; // r30
    struct xVec3 * upos; // r29
}

// Range: 0x80059608 -> 0x80059698
void xEntDefaultBoundUpdate(struct xEnt * ent /* r30 */, struct xVec3 * pos /* r1+0x8 */) {
    // Local variables
    struct xBound * bound; // r31
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x804F2D5D
// Range: 0x80059698 -> 0x80059858
void xEntDefaultTranslate(struct xEnt * ent /* r31 */, struct xVec3 * dpos /* r29 */, struct xMat4x3 * dmat /* r30 */) {
    // References
    // -> static char __FUNCTION__[21];
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x804F2D72
// Range: 0x80059858 -> 0x8005996C
static void xEntRotationToMatrix(struct xEntFrame * frame /* r31 */) {
    // References
    // -> static char __FUNCTION__[21];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D5F28A
// Range: 0x8005996C -> 0x80059B2C
void xEntMotionToMatrix(struct xEnt * ent /* r30 */, struct xEntFrame * frame /* r31 */) {
    // References
    // -> static char __FUNCTION__[19];
}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D5F29D
// Range: 0x80059B2C -> 0x80059E7C
void xEntMove(struct xEnt * ent /* r31 */, struct xScene * sc /* r1+0x10 */, float dt /* r1+0x14 */) {
    // Local variables
    struct xEntFrame * dframe; // r30
    struct xVec3 dpos; // r1+0x30
    struct xMat4x3 newMat; // r1+0xC0
    struct xMat4x3 invOldmat; // r1+0x80
    struct xMat4x3 deltaMat; // r1+0x40
    struct xVec3 startPos; // r1+0x24
    struct xVec3 dpos; // r1+0x18

    // References
    // -> static char __FUNCTION__[9];
}

// Range: 0x80059E7C -> 0x8005A0E4
static void post_move(struct xEnt & ent /* r31 */) {
    // Local variables
    struct xModelInstance & model; // r30
    struct xMat4x3 & mat; // r28
    struct anim_coll_data & ac; // r29

    // References
    // -> static char __FUNCTION__[10];
}

// Range: 0x8005A0E4 -> 0x8005A33C
static void pre_move(struct xEnt & ent /* r31 */) {
    // Local variables
    struct xModelInstance & model; // r30
    struct xMat4x3 & mat; // r28
    struct anim_coll_data & ac; // r27

    // References
    // -> static char __FUNCTION__[9];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5F2A6
// Range: 0x8005A33C -> 0x8005A62C
void xEntApplyPhysics(struct xEnt * ent /* r31 */, struct xScene * sc /* r30 */, float dt /* f31 */) {
    // Local variables
...
int xent_nupdates; // size: 0x4, address: 0x80D5F2B8
int xent_entent_nchks; // size: 0x4, address: 0x80D5F2BC
float xent_ee_per_upd; // size: 0x4, address: 0x80D5F2C0
int xent_nentcoll; // size: 0x4, address: 0x80D5F2C4
float xent_ec_per_upd; // size: 0x4, address: 0x80D5F2C8
float xent_ee_per_ec; // size: 0x4, address: 0x80D5F2CC
int xent_nqcrejects; // size: 0x4, address: 0x80D5F2D0
float xent_qcr_per_upd; // size: 0x4, address: 0x80D5F2D4
float xent_qcr_per_ec; // size: 0x4, address: 0x80D5F2D8
float xent_qcr_per_ee; // size: 0x4, address: 0x80D5F2DC
// Range: 0x8005A62C -> 0x8005A7FC
void xEntDebugUpdate() {
    // References
    // -> int xent_entent_nchks;
    // -> int xent_nqcrejects;
    // -> float xent_qcr_per_ee;
    // -> int xent_nentcoll;
    // -> float xent_qcr_per_ec;
    // -> int xent_nupdates;
    // -> float xent_qcr_per_upd;
    // -> float xent_ee_per_ec;
    // -> float xent_ec_per_upd;
    // -> float xent_ee_per_upd;
}

// Range: 0x8005A7FC -> 0x8005AB14
void xEntCollide(struct xEnt * ent /* r31 */, struct xScene * sc /* r29 */, float dt /* f28 */) {
    // Local variables
    long long t2; // r27
    float sbr; // f30
    struct xCollis * coll; // r30
    float h_dot_n; // f29
    float depen_len; // f31
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5F2E0
struct xColor_tag g_NEON_BLUE; // size: 0x4, address: 0x80D6C6C0
struct xColor_tag g_NEON_RED; // size: 0x4, address: 0x80D6C6B8
// Range: 0x8005AC28 -> 0x8005AE9C
void xEntEndCollide(struct xEnt * ent /* r31 */, struct xScene * sc /* r1+0x8 */, float dt /* r1+0xC */) {
    // Local variables
    float sbr; // f30
    struct xVec3 start; // r1+0x28
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5F2EF
// Range: 0x8005AE9C -> 0x8005B07C
void xEntCollCheckEnv(struct xEnt * p /* r31 */, struct xScene * sc /* r29 */) {
    // Local variables
    struct xCollis * coll; // r30
    unsigned char ncolls; // r28

    // References
    // -> static char __FUNCTION__[17];
}

static int offs[4][3][2]; // size: 0x60, address: 0x804F2D88
static int k; // size: 0x4, address: 0x80D68FC0
// total size: 0xC
...
// Range: 0x8005B07C -> 0x8005B390
static void xEntCollCheckOneGrid(struct xEnt * p /* r28 */, struct xScene * sc /* r24 */, struct xEnt * (* hitIt)(struct xEnt *, struct xScene *, void *) /* r25 */, struct xGrid * grid /* r30 */) {
    // Local variables
    struct xGridIterator it; // r1+0x10
    struct xEnt * ent; // r27
    struct xVec3 * pcenter; // r29
    int px; // r1+0xC
    int pz; // r1+0x8
    struct xGridBound * cell; // r31
    struct xBound * cellbound; // r23
    struct xBox clbox; // r1+0x1C
    float clcenterx; // f30
    float clcenterz; // f31
    int i; // r26
    int _x; // r22
    int _z; // r21
    struct xBound * cellbound; // r20
    struct xBound * cellbound; // r19
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D5F300
// total size: 0x38
struct xGrid {
    // Members
...
// Range: 0x8005B390 -> 0x8005B4A4
void xEntCollCheckByGrid(struct xEnt * p /* r31 */, struct xScene * sc /* r29 */, struct xEnt * (* hitIt)(struct xEnt *, struct xScene *, void *) /* r30 */) {
    // References
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x804F2DE8
struct xGrid npcs_grid; // size: 0x38, address: 0x80CF49B0
// Range: 0x8005B4A4 -> 0x8005B574
void xEntCollCheckNPCsByGrid(struct xEnt * p /* r31 */, struct xScene * sc /* r1+0x8 */, struct xEnt * (* hitIt)(struct xEnt *, struct xScene *, void *) /* r1+0xC */) {
    // References
    // -> struct xGrid npcs_grid;
    // -> static char __FUNCTION__[24];
}

int xent_entent; // size: 0x4, address: 0x80D68FC4
static char __FUNCTION__[27]; // size: 0x1B, address: 0x804F2E00
// Range: 0x8005B574 -> 0x8005BAD0
struct xEnt * xEntCollCheckOneEntNoDepen(struct xEnt * ent /* r31 */, void * data /* r1+0x8 */) {
    // Local variables
    struct xEnt * p; // r29
    struct xCollis * coll; // r30
    unsigned int modl_coll; // r24
    struct xBound tmp; // r1+0x18
    struct xBound * bptr; // r28
    long long t0; // r19
    unsigned char ncolls; // r23
    struct xModelInstance * collModel; // r18
    struct xVec3 * upper; // r27
    struct xVec3 * lower; // r26
    struct xModelInstance * collModel; // r17
    long long t1; // r1+0x10
    unsigned char idx; // r25
    float rsum; // f31
    float dx; // f30
    float dy; // f27
    float dz; // f29
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5F314
// Range: 0x8005BAD0 -> 0x8005BF3C
void xEntCollideFloor(struct xEnt * p /* r31 */, struct xScene * sc /* r24 */, float dt /* f31 */) {
    // Local variables
    struct xCollis * coll; // r26
    unsigned char idx; // r27
    struct xCollis * ml; // r29
    struct xVec3 motion; // r1+0x8
    float mlen; // f28
    int stepping; // r25
    float sbr; // f30
    struct xCollis * mf; // r30
    struct xEnt * fent; // r28
    float flr_dist; // f27
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D5F325
// Range: 0x8005BF3C -> 0x8005C24C
void xEntCollideCeiling(struct xEnt * p /* r31 */, struct xScene * sc /* r23 */, float dt /* r1+0x8 */) {
    // Local variables
    struct xCollis * coll; // r26
    unsigned char idx; // r27
    struct xCollis * ml; // r30
    float sbr; // f31
    struct xCollis * mf; // r29
    struct xEnt * fent; // r28
    float ceil_dist; // f30
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5F338
// Range: 0x8005C24C -> 0x8005C7A0
void xEntCollideWalls(struct xEnt * p /* r31 */, struct xScene * sc /* r25 */, float dt /* f29 */) {
    // Local variables
    struct xCollis * coll; // r30
    struct xEnt * cent; // r28
    unsigned char sidx; // r27
    unsigned char eidx; // r26
    unsigned char idx; // r29
    float sbr; // f31
...
// Range: 0x8005C7A0 -> 0x8005C7AC
struct xBox * xEntGetAllEntsBox() {
    // References
    // -> static struct xBox all_ents_box;
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5F349
// Range: 0x8005C7AC -> 0x8005C840
void xEntDebugInit() {
    // Local variables
    unsigned int num_xents; // r31

    // References
    // -> static int dbg_holding_dec;
    // -> static int dbg_holding_inc;
    // -> static int dbg_mode;
    // -> static signed short dbg_idx;
    // -> static char __FUNCTION__[14];
    // -> unsigned int gActiveHeap;
    // -> static struct xEnt * * dbg_xents;
    // -> static unsigned short dbg_num;
}

// Range: 0x8005C840 -> 0x8005C880
void xEntDebugExit() {
    // References
    // -> static int dbg_mode;
    // -> static signed short dbg_idx;
...
// Range: 0x8005C880 -> 0x8005C8F4
void xEntDebugAdd(struct xEnt * xent /* r31 */) {
    // References
    // -> static unsigned short dbg_num;
    // -> static struct xEnt * * dbg_xents;
}

static int sBoneIndex; // size: 0x4, address: 0x80D68FC8
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D5F357
// Range: 0x8005C8F4 -> 0x8005CA84
static void xEntDebugCB() {
    // Local variables
    struct xEnt * xent; // r31
...
// Range: 0x8005CA84 -> 0x8005CB60
static char * xEntCollTypeStrSingle(unsigned char type /* r3 */) {}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D5F363
// Range: 0x8005CB60 -> 0x8005CCC0
static char * xEntCollTypeStrAll(char * str /* r31 */, unsigned char type /* r30 */) {
    // References
    // -> static char __FUNCTION__[19];
}

// Range: 0x8005CCC0 -> 0x8005CD68
static char * xEntCollLevStr(unsigned char lev /* r3 */) {}

// Range: 0x8005CD68 -> 0x8005CDF8
static char * xEntBoundTypeStr(unsigned char type /* r3 */) {}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5F376
// Range: 0x8005CDF8 -> 0x8005CFB0
static char * xEntBoundValStr(char * str /* r30 */, const struct xBound * b /* r31 */) {
    // References
    // -> static char __FUNCTION__[16];
}

static char scoll_name[6][6]; // size: 0x24, address: 0x804F2E1B
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5F386
// total size: 0x30
struct xMat3x3 {
    // Members
...
// Range: 0x8005CFB0 -> 0x8005D7B4
static void xEntDebugWrite(const struct xEnt * xent /* r31 */) {
    // Local variables
    char str[40]; // r1+0x80
    const struct xMat3x3 * mat; // r26
    const struct xVec3 * pos; // r29
    struct xVec3 dpos; // r1+0x40
    struct xVec3 * vel; // r28
    struct xVec3 * ovel; // r21
    struct xVec3 dvel; // r1+0x34
    struct xVec3 scale; // r1+0x28
    struct xMat3x3 mat_norm; // r1+0x50
    struct xVec3 ang; // r1+0x1C
    struct xCollis * scoll; // r30
    int i; // r27
    struct xVec3 lhdng; // r1+0x10

    // References
    // -> static int sBoneIndex;
    // -> struct zGlobals globals;
    // -> static char scoll_name[6][6];
    // -> static char __FUNCTION__[15];
    // -> static int dbg_mode;
}

// Range: 0x8005D7B4 -> 0x8005D908
static void xEntDebugDraw(const struct xEnt * xent /* r31 */) {
    // Local variables
    struct xVec3 v; // r1+0x14
    struct xMat4x3 t; // r1+0x20
...
// Range: 0x8005D908 -> 0x8005DC10
static void xEntDebugIPad(struct xEnt * xent /* r31 */) {
    // Local variables
    float now; // f31
    float dt; // f30
    ...
    // -> static int dbg_mode;
    // -> static unsigned short dbg_num;
    // -> static float dbg_tmr;
    // -> static int sBoneIndex;
    // -> static int dbg_holding_dec;
    // -> static int dbg_holding_inc;
    // -> static float dbg_ltm;
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D5F395
// Range: 0x8005DC10 -> 0x8005DCC0
static void xModCollInit() {
    // Local variables
    int i; // r31

    // References
    // -> static struct xModCollRec smcrecs[150];
    // -> static enum eModCollMode mcmode;
    // -> static long long mctime;
    // -> static int num_mcrecs;
    // -> static char __FUNCTION__[13];
    // -> unsigned int gActiveHeap;
    // -> static struct xModCollRec * mcrecs;
}

// Range: 0x8005DCC0 -> 0x8005DD20
static void xModCollExit() {
    // Local variables
    int i; // r31
...
static int been_here; // size: 0x4, address: 0x80D68FCC
static int been_here; // size: 0x4, address: 0x80D68FD0
static int been_here; // size: 0x4, address: 0x80D68FD4
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D5F3A2
// Range: 0x8005DD20 -> 0x8005DF28
static void xModCollAdd(struct xEnt * xent /* r30 */) {
    // Local variables
    struct xModCollRec * end; // r29
    struct xModCollRec * mc; // r31
...
static int been_here; // size: 0x4, address: 0x80D68FD8
static int been_here; // size: 0x4, address: 0x80D68FDC
static int been_here; // size: 0x4, address: 0x80D68FE0
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5F3AE
// Range: 0x8005DF28 -> 0x8005E104
void xModCollAddSample(struct xEnt * xent /* r30 */, long long elapsed /* r28 */) {
    // Local variables
    struct xModCollRec * end; // r27
    struct xModCollRec * mc; // r31
...
// Range: 0x8005E104 -> 0x8005E194
static int CmpModCollCtime(void * a /* r3 */, void * b /* r4 */) {
    // Local variables
    struct xModCollRec * mca; // r31
    struct xModCollRec * mcb; // r30
}

// Range: 0x8005E194 -> 0x8005E28C
static int CmpModCollLtime(void * a /* r3 */, void * b /* r4 */) {
    // Local variables
    struct xModCollRec * mca; // r31
    struct xModCollRec * mcb; // r30
}

// Range: 0x8005E28C -> 0x8005E5D8
static void xModCollCB() {
    // Local variables
    struct xModCollRec * mc; // r31
    int i; // r30
    float poo; // f30
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x804F2E3F
// Range: 0x8005E5D8 -> 0x8005E6A8
void xEntAnimateCollision(struct xEnt & ent /* r31 */, unsigned char on /* r30 */) {
    // References
    // -> static char __FUNCTION__[21];
    // -> unsigned int gActiveHeap;
}

// Range: 0x8005E6A8 -> 0x8005E9DC
unsigned char xEntValidType(unsigned char type /* r3 */) {}

// Range: 0x8005E9DC -> 0x8005EB28
unsigned char xEntTurnToFace(struct xEnt * ent /* r31 */, const struct xVec3 * target /* r30 */, float speedLimit /* r1+0x8 */, float dt /* r1+0xC */) {
    // Local variables
    struct xVec3 currentFacing; // r1+0x1C
    float dot; // f30
    struct xVec3 axis; // r1+0x10
    float angle; // f31
    float maxAngle; // f29
}
...
// Range: 0x8005EB28 -> 0x8005EB74
void xEntOpacityInit(struct xEntOpacity * opacity /* r3 */, struct xEnt * ent /* r4 */) {}

// Range: 0x8005EB74 -> 0x8005EDDC
void xEntOpacityUpdate(struct xEntOpacity * opacity /* r3 */, struct xEnt * ent /* r4 */, float dt /* f1 */) {}

// Range: 0x8005EDDC -> 0x8005EDE0
void xEntLoadCheckPoint() {}

// Range: 0x8005EDE0 -> 0x8005EDE4
void xEntStoreCheckPoint() {}

// Range: 0x8005EDE4 -> 0x8005EDF0
void on_select_debug_me(const struct tweak_info & info /* r3 */) {
    // References
    // -> struct xEnt * gEnt_Debug;
}

static char __FUNCTION__[23]; // size: 0x17, address: 0x804F2E54
// Range: 0x8005EDF0 -> 0x8005EED8
void on_change_active_model(const struct tweak_info & info /* r28 */) {
    // Local variables
    struct xEnt * this_ent; // r29
    struct xModelInstance * active_model; // r31
    int i; // r30
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x804F2E6B
// Range: 0x8005EED8 -> 0x8005EFC8
void on_change_show_vertex(const struct tweak_info & info /* r27 */) {
    // Local variables
    struct xEnt * this_ent; // r30
    struct xModelInstance * active_model; // r31
    int i; // r29
    signed short verts; // r28
