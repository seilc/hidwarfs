/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\gc\iParMgr.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8011B0D0 -> 0x8011DB5C
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static int gColorTableInit; // size: 0x4, address: 0x80D69B80
static float gColorTable[256]; // size: 0x400, address: 0x80CA8F90
// total size: 0x8
struct RwObject {
    // Members
...
// Range: 0x8011B0D0 -> 0x8011B144
void iParMgrInit() {
    // Local variables
    int i; // r31

    // References
    // -> static int gColorTableInit;
    // -> static float gColorTable[256];
}

// Range: 0x8011B144 -> 0x8011B148
void iParMgrUpdate() {}

static char __FUNCTION__[27]; // size: 0x1B, address: 0x80516C80
// total size: 0x18
...
// Range: 0x8011B148 -> 0x8011B810
void iParMgrRenderParSys_Sprite(void * data /* r1+0x8 */, struct xParGroup * ps /* r20 */) {
    // Local variables
    struct xPar * idx; // r25
    struct zParSys * s; // r19
    struct RwTexture * texture; // r18
    struct RwRaster * raster; // r17
    int indexCount; // r26
    int vertexCount; // r27
    unsigned short * i3d; // r28
    struct RxObjSpace3DVertex * v3d; // r31
    struct xParCmdTex * tex; // r29
    struct xPar * p; // r30
    float size; // f31
    struct xVec3 h; // r1+0x5C
    struct xVec3 v; // r1+0x50
    unsigned char r; // r24
    unsigned char g; // r23
    unsigned char b; // r22
    unsigned char a; // r21
    struct xVec3 vert[4]; // r1+0x68
    struct RwRGBA col; // r1+0x1A
    struct RwRGBA col; // r1+0x16
    struct RwRGBA col; // r1+0x12
    struct RwRGBA col; // r1+0xE
...
static char __FUNCTION__[27]; // size: 0x1B, address: 0x80516C9B
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x8011B810 -> 0x8011B930
void iRenderSetCameraViewMatrix(struct xMat4x3 * m /* r31 */) {
    // Local variables
...
static struct RxObjSpace3DVertex v3d[4]; // size: 0x90, address: 0x80CA9390
static unsigned short i3d[6]; // size: 0xC, address: 0x80D61A08
// Range: 0x8011B930 -> 0x8011C0EC
void iRenderPushQuadStreak(struct xPar * p /* r31 */, struct xParCmdTex * tex /* r30 */) {
    // Local variables
    void * vertices; // r20
    unsigned short * indices; // r19
    struct xVec3 h; // r1+0x48
    int vertexCount; // r25
    int indexCount; // r24
    unsigned char r; // r29
    unsigned char g; // r28
    ...
    float v1; // f28
    float v2; // f27
    unsigned short * src; // r22
    unsigned short * dst; // r21
    int i; // r23

    // References
    // -> struct tagiRenderInput gRenderBuffer;
    // -> static struct RxObjSpace3DVertex v3d[4];
    // -> static unsigned short i3d[6];
}

static struct RxObjSpace3DVertex v3d[4]; // size: 0x90, address: 0x80CA9420
static unsigned short i3d[6]; // size: 0xC, address: 0x80D61A14
// total size: 0x30
struct xMat3x3 {
    // Members
...
// Range: 0x8011C0EC -> 0x8011C8DC
static void iRenderPushFlat(struct xPar * p /* r31 */, struct xParCmdTex * tex /* r30 */) {
    // Local variables
    void * vertices; // r20
    unsigned short * indices; // r19
    int vertexCount; // r25
    ...
    unsigned char a; // r26
    float size; // f31
    struct xVec3 vert[4]; // r1+0xB0
    struct xMat3x3 groundmat; // r1+0x80
    float yaw; // f30
    struct xVec3 zdir; // r1+0x68
    struct xVec3 xdir; // r1+0x5C
    struct xVec3 centre; // r1+0x50
...
// Range: 0x8011C8DC -> 0x8011C978
void iRenderFlush() {
    // References
    // -> struct tagiRenderInput gRenderBuffer;
}

// Range: 0x8011C978 -> 0x8011C9F8
void iRenderTrianglesImmediate(void * vertices /* r1+0x8 */, int vertexCount /* r1+0xC */, unsigned short * indices /* r1+0x10 */, int indexCount /* r30 */) {
    // Local variables
    unsigned int transformFlags; // r31
}

// Range: 0x8011C9F8 -> 0x8011CD64
void iParMgrRenderParSys_Streak(void * data /* r1+0x8 */, struct xParGroup * ps /* r20 */) {
    // Local variables
    unsigned int transformFlags; // r1+0x1C
    struct xPar * idx; // r29
    struct zParSys * s; // r24
    struct RwTexture * texture; // r23
    struct RwRaster * raster; // r22
    struct RxObjSpace3DVertex * v3d; // r31
    struct xPar * p; // r30
    void * vertices; // r1+0x18
    int vertexCount; // r21
    struct xVec3 h; // r1+0x44
    float size; // f31
    struct xVec3 vert[3]; // r1+0x50
    struct RwV3d tmp; // r1+0x38
    struct RwV3d tmp; // r1+0x2C
    struct RwV3d tmp; // r1+0x20
    unsigned char r; // r28
...
// Range: 0x8011CD64 -> 0x8011D0C4
void iParMgrRenderParSys_InvStreak(void * data /* r1+0x8 */, struct xParGroup * ps /* r20 */) {
    // Local variables
    unsigned int transformFlags; // r1+0x1C
    struct xPar * idx; // r29
    struct zParSys * s; // r24
...
// Range: 0x8011D0C4 -> 0x8011D16C
void iParMgrRenderParSys_QuadStreak(void * data /* r1+0x8 */, struct xParGroup * ps /* r30 */) {
    // Local variables
    unsigned int transformFlags; // r1+0xC
    struct xPar * idx; // r31
    struct zParSys * s; // r29
    struct RwTexture * texture; // r28
    struct RwRaster * raster; // r27
}

// Range: 0x8011D16C -> 0x8011D170
void iParMgrRenderParSys_Static() {}

static struct RxObjSpace3DVertex v3d[4]; // size: 0x90, address: 0x80CA94B0
static unsigned short i3d[6]; // size: 0xC, address: 0x80D61A20
// Range: 0x8011D170 -> 0x8011DAC4
void iParMgrRenderParSys_Ground(void * data /* r1+0x10 */, struct xParGroup * ps /* r21 */) {
    // Local variables
    struct xPar * idx; // r29
    struct zParSys * s; // r20
    struct RwTexture * texture; // r19
    struct RwRaster * raster; // r18
    struct xParCmdTex * tex; // r30
    struct xPar * p; // r31
    void * vertices; // r1+0x2C
    unsigned short * indices; // r1+0x28
    int vertexCount; // r24
    int indexCount; // r23
    float size; // f31
    struct xVec3 vert[4]; // r1+0xC0
    unsigned char r; // r28
    unsigned char g; // r27
    unsigned char b; // r26
    unsigned char a; // r25
    struct xMat3x3 groundmat; // r1+0x90
    float angx; // f30
    float angy; // f29
    float angz; // f28
    struct xVec3 zdir; // r1+0x78
    struct xVec3 xdir; // r1+0x6C
    struct xVec3 centre; // r1+0x60
...
// Range: 0x8011DAC4 -> 0x8011DB5C
void iParMgrRenderParSys_Flat(void * data /* r1+0x8 */, struct xParGroup * ps /* r30 */) {
    // Local variables
    unsigned int transformFlags; // r1+0xC
    struct xPar * idx; // r31
    struct zParSys * s; // r29
