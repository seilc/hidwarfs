/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\gc\iMorph.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80119914 -> 0x8011A450
*/
// total size: 0x8
struct RwObject {
    // Members
...
static struct RpGeometry * s_geom; // size: 0x4, address: 0x80D69B30
static struct RpMorphTarget * s_tgt; // size: 0x4, address: 0x80D69B34
static float * s_alloc; // size: 0x4, address: 0x80D69B38
static float * s_vTemp; // size: 0x4, address: 0x80D69B3C
static float * s_nTemp; // size: 0x4, address: 0x80D69B40
static int s_numV; // size: 0x4, address: 0x80D69B44
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D619D8
// total size: 0x14
...
// Range: 0x80119914 -> 0x80119FD8
static void MorphCommon(struct RpAtomic * model /* r20 */, struct RwMatrixTag * mat /* r1+0x8 */, signed short * * v_array /* r21 */, signed short * weight /* r22 */, unsigned int normals /* r28 */, float scale /* f31 */, int dorender /* r1+0xC */) {
    // Local variables
    unsigned int i; // r31
    unsigned int a; // r29
    signed short wa[4]; // r1+0x14
    signed short * va[4]; // r1+0x1C
    int wsum; // r25
    struct RwV3d * vold; // r24
    struct RwV3d * nold; // r26
    int lockMode; // r19
    void * temp_scratch; // r27
    struct RpUserDataArray * usr; // r23
    struct DirtyMorph * dm; // r30

    // References
    // -> static struct RpMorphTarget * s_tgt;
    // -> static struct RpGeometry * s_geom;
    // -> static float * s_nTemp;
    // -> static float * s_vTemp;
    // -> static int s_numV;
    // -> static float * s_alloc;
    // -> static char __FUNCTION__[12];
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D619E4
// Range: 0x80119FD8 -> 0x8011A0D8
void iMorphOptimize(struct RpAtomic * model /* r1+0x8 */, int normals /* r28 */) {
    // Local variables
    struct RpGeometry * geom; // r31
    int numV; // r27
    int usridx; // r30
    struct RpUserDataArray * usr; // r29

    // References
    // -> static char __FUNCTION__[15];
}

// Range: 0x8011A0D8 -> 0x8011A148
void iMorphRender(struct RpAtomic * model /* r1+0x8 */, struct RwMatrixTag * mat /* r1+0xC */, signed short * * v_array /* r1+0x10 */, signed short * weight /* r1+0x14 */, unsigned int normals /* r1+0x18 */, float scale /* r1+0x1C */) {
    // References
    // -> static float * s_alloc;
}

// Range: 0x8011A148 -> 0x8011A1A0
void FastS16unpack(float * dest /* r3 */, signed short * v /* r4 */, int count /* r5 */, float scale /* f1 */) {
    // Local variables
    int i; // r31
}

// Range: 0x8011A1A0 -> 0x8011A2A0
void FastS16weight2(float * dest /* r3 */, signed short * * v_array /* r4 */, signed short * weight /* r5 */, int count /* r6 */, float scale /* f1 */) {
    // Local variables
    int i; // r31
    signed short * v0; // r30
    signed short * v1; // r29
...
// Range: 0x8011A2A0 -> 0x8011A450
void FastS16weight4(float * dest /* r3 */, signed short * * v_array /* r4 */, signed short * weight /* r5 */, int count /* r6 */, float scale /* f1 */) {
    // Local variables
    int i; // r31
    signed short * v0; // r30
    signed short * v1; // r29
