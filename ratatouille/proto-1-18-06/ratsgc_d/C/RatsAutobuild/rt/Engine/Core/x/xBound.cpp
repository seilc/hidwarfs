/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xBound.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8000EC78 -> 0x80010C00
*/
// total size: 0x20
struct xQCData {
    // Members
...
// Range: 0x8000EC78 -> 0x8000ED2C
void xBoundUpdate(struct xBound * b /* r31 */) {}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D5E580
// Range: 0x8000ED2C -> 0x8000EED4
void xBoundGetBox(struct xBox & box /* r28 */, const struct xBound & bound /* r29 */) {
    // Local variables
    const struct xSphere & o; // r31
    const struct xCylinder & c; // r30
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5E58D
// Range: 0x8000EED4 -> 0x8000F09C
void xBoundGetCylinder(struct xCylinder & o /* r30 */, const struct xBound & bound /* r31 */) {
    // Local variables
    struct xVec3 diff; // r1+0x14
    const struct xMat4x3 & mat; // r29
    struct xVec3 v; // r1+0x8
    float r2; // f31
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5E59F
// Range: 0x8000F0C8 -> 0x8000F250
void xBoundGetSphere(struct xSphere & o /* r30 */, const struct xBound & bound /* r31 */) {
    // Local variables
    const struct xMat4x3 & mat; // r29
    struct xVec3 v; // r1+0x8
    float r2; // f31
...
// Range: 0x8000F250 -> 0x8000F290
void xBoundSphereHitsOBB(const struct xSphere * s /* r1+0x8 */, const struct xBox * b /* r1+0xC */, const struct xMat4x3 * m /* r1+0x10 */, struct xCollis * coll /* r1+0x14 */) {}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5E5AF
int xent_entent; // size: 0x4, address: 0x80D68FC4
int xent_nqcrejects; // size: 0x4, address: 0x80D5F2D0
// Range: 0x8000F290 -> 0x8000F54C
void xBoundHitsBound(const struct xBound * a /* r31 */, const struct xBound * b /* r30 */, struct xCollis * c /* r29 */) {
    // References
    // -> int xent_nqcrejects;
    // -> int xent_entent;
    // -> static char __FUNCTION__[16];
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5E5BF
// Range: 0x8000F54C -> 0x8000F764
void xBoundHitsModel(const struct xBound * b /* r31 */, const struct xModelInstance * m /* r30 */, struct xCollis * colls /* r29 */, unsigned char ncolls /* r1+0x8 */, float sth /* r1+0xC */) {
    // References
    // -> static char __FUNCTION__[16];
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5E5CF
// total size: 0x24
struct xRay3 {
    // Members
...
// Range: 0x8000F764 -> 0x8000FD00
static void xBoundOBBIsectRay(const struct xBox * b /* r30 */, const struct xMat4x3 * m /* r31 */, const struct xRay3 * r /* r29 */, struct xIsect * isect /* r28 */) {
    // Local variables
    struct xRay3 xfr; // r1+0x80
    struct xBox sbox; // r1+0x1C
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5E5E1
// total size: 0x2C
struct xIsect {
    // Members
    unsigned int flags; // offset 0x0, size 0x4
...
// Range: 0x8000FD00 -> 0x8000FF98
void xRayHitsBound(const struct xRay3 * r /* r29 */, const struct xBound * b /* r31 */, struct xCollis * c /* r30 */) {
    // Local variables
    struct xIsect isect; // r1+0x8

    // References
    // -> static char __FUNCTION__[14];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5E5EF
// Range: 0x8000FF98 -> 0x80010170
void xSphereHitsBound(const struct xSphere * o /* r29 */, const struct xBound * b /* r31 */, struct xCollis * c /* r30 */) {
    // References
    // -> static char __FUNCTION__[17];
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5E600
// Range: 0x80010170 -> 0x800103EC
void xVecHitsBound(const struct xVec3 * v /* r29 */, const struct xBound * b /* r31 */, struct xCollis * c /* r30 */) {
    // Local variables
    struct xIsect isect; // r1+0x14
...
// Range: 0x800103EC -> 0x80010488
void xBoundDraw(const struct xBound * b /* r31 */) {}

struct xMat4x3 g_I3; // size: 0x40, address: 0x80B7FA80
// Range: 0x80010488 -> 0x800105CC
unsigned char xBoundHitsBound(const struct xBound & a /* r30 */, const struct xBound & b /* r31 */) {
    // References
    // -> struct xMat4x3 g_I3;
}

// Range: 0x800105CC -> 0x800106A4
unsigned char xSphereHitsBound(const struct xVec3 & c /* r30 */, float r /* f31 */, const struct xBound & b /* r31 */) {}

// Range: 0x800106A4 -> 0x80010760
unsigned char xOBBHitsBound(const struct xBox & box /* r29 */, const struct xMat4x3 & mat /* r30 */, const struct xBound & b /* r31 */) {
    // References
    // -> struct xMat4x3 g_I3;
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5E60E
// Range: 0x80010760 -> 0x80010844
void xBoundGetCenter(const struct xBound * b /* r31 */, struct xVec3 * center /* r30 */) {
    // References
    // -> static char __FUNCTION__[16];
}

// Range: 0x80010844 -> 0x80010C00
unsigned char xOBBHitsTriangle(const struct xBox & box /* r27 */, const struct xMat4x3 & mat /* r31 */, const struct xVec3 * vertices /* r30 */) {
    // Local variables
    struct xVec3 dist; // r1+0x3C
    struct xVec3 center; // r1+0x30
    struct xVec3 v2MinusV1; // r1+0x24
    struct xVec3 v1MinusV0; // r1+0x18
    struct xVec3 v0MinusV2; // r1+0xC
    struct xVec3 seperationAxes[13]; // r1+0x48
    int i; // r28
    struct xVec3 & s; // r29
    float boxDist; // f28
    float centerDotS; // f27
    float minBoxPos; // f20
    float maxBoxPos; // f19
    float v0DotS; // f31
    float v1DotS; // f30
