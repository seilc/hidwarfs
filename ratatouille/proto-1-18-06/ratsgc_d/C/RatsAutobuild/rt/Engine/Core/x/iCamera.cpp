/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\iCamera.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80258B38 -> 0x8025A0A0
*/
float sCameraNearClip; // size: 0x4, address: 0x80D654D8
float giCameraOONearClip; // size: 0x4, address: 0x80D654DC
float sCameraFarClip; // size: 0x4, address: 0x80D654E0
// total size: 0x8
struct RwObject {
    // Members
...
static struct RwCamera * sMainGameCamera; // size: 0x4, address: 0x80D6B1B4
static float tweakFarClip; // size: 0x4, address: 0x80D6B1B8
// Range: 0x80258B38 -> 0x80258BC0
void iCameraSetOrthoProjection(struct RwCamera * cam /* r29 */, unsigned char ortho /* r1+0x8 */) {
    // Local variables
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D654E4
// total size: 0x1
...
// Range: 0x80258BC0 -> 0x80258E90
struct RwCamera * iCameraCreate(int width /* r25 */, int height /* r26 */, int mainGameCamera /* r1+0x8 */) {
    // Local variables
    struct iAsyncLockRWSentry rw_sentry; // r1+0xC
    struct RwV2d vw; // r1+0x10
    struct RwCamera * camera; // r31
    struct RwFrame * cameraFrame; // r30
    struct RwRaster * cameraRaster; // r29
    struct RwRaster * zbuffer; // r28

    // References
    // -> static struct RwCamera * sMainGameCamera;
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D654F2
...
// Range: 0x80258E90 -> 0x80259008
void iCameraDestroy(struct RwCamera * camera /* r31 */) {
    // Local variables
    struct iAsyncLockRWSentry rw_sentry; // r1+0x8
    struct RpWorld * pWorld; // r29
...
float iCameraClearTime; // size: 0x4, address: 0x80D6B1BC
float iCameraClearTimeLast; // size: 0x4, address: 0x80D6B1C0
// total size: 0x4
struct RwRGBA {
    // Members
...
static struct RwRGBA hotPink; // size: 0x4, address: 0x80D65501
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D65505
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x80259008 -> 0x80259108
void iCameraBegin(struct RwCamera * cam /* r31 */) {
    // Local variables
    long long timeA; // r29
    long long timeB; // r27
    ...
    // References
    // -> float sCameraNearClip;
    // -> float iCameraClearTime;
    // -> float iCameraClearTimeLast;
    // -> static struct RwRGBA hotPink;
    // -> struct xGlobals * xglobals;
    // -> static char __FUNCTION__[13];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D65512
// Range: 0x80259108 -> 0x8025919C
void iCameraEnd(struct RwCamera * cam /* r31 */) {
    // References
    // -> static char __FUNCTION__[11];
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D6551D
// Range: 0x8025919C -> 0x80259230
void iCameraShowRaster(struct RwCamera * cam /* r31 */) {
    // References
    // -> static char __FUNCTION__[18];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D6552F
// Range: 0x80259230 -> 0x80259490
void iCameraUpdatePos(struct RwCamera * cam /* r28 */, struct xMat4x3 * pos /* r29 */) {
    // Local variables
    struct RwFrame * f; // r31
    struct RwMatrixTag * m; // r30

    // References
    // -> static char __FUNCTION__[17];
}

// Range: 0x80259490 -> 0x80259498
float iCameraGetAspectRatio() {}

// Range: 0x80259498 -> 0x80259518
void iCameraSetFOV(struct RwCamera * cam /* r1+0x8 */, float fov /* r1+0xC */) {
    // Local variables
    float view_window; // f31
    float fAspectRatio; // f30
    struct RwV2d vw; // r1+0x10
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D65540
// Range: 0x80259518 -> 0x80259638
void iCameraAssignEnv(struct RwCamera * camera /* r30 */, struct iEnv * env_geom /* r31 */) {
    // Local variables
    struct iAsyncLockRWSentry rw_sentry; // r1+0x8
...
// Range: 0x80259638 -> 0x802596E8
void iCamGetViewMatrix(struct RwCamera * camera /* r1+0x8 */, struct xMat4x3 * view_matrix /* r30 */) {
    // Local variables
    struct RwMatrixTag * rw_view; // r31
}

// Range: 0x802596E8 -> 0x80259754
void iCameraSetNearFarClip(float nearPlane /* f1 */, float farPlane /* f2 */) {
    // References
    // -> float sCameraFarClip;
    // -> float sCameraNearClip;
}

// Range: 0x80259754 -> 0x80259768
void iCameraGetNearFarClip(float & nearPlane /* r3 */, float & farPlane /* r4 */) {
    // References
    // -> float sCameraFarClip;
    // -> float sCameraNearClip;
}

// Range: 0x80259768 -> 0x802598A8
void iCameraSetFogParams(struct iFogParams * fp /* r31 */, float time /* f31 */) {
    // References
    // -> struct xGlobals * xglobals;
}

// Range: 0x80259914 -> 0x80259E08
void iCameraUpdateFog(long long t /* r27 */) {
    // Local variables
    struct iFogParams * fp; // r30
    struct iFogParams * fa; // r31
    struct iFogParams * fb; // r29
    float a; // f31
    struct RwRGBA ca; // r1+0x10
    struct RwRGBA cb; // r1+0xC
...
// Range: 0x80259E08 -> 0x80259F08
void iCameraSetFogRenderStates() {
    // Local variables
    struct RwCamera * pCamera; // r30
    struct iFogParams * pFogParams; // r31
    unsigned int bite_me; // r29
...
// Range: 0x80259F08 -> 0x80259F48
static float iCameraFogFarClip() {
    // References
    // -> struct xGlobals * xglobals;
    // -> static float tweakFarClip;
}

// Range: 0x80259F48 -> 0x80259F70
static void iCameraFogOffWrapper() {}

// Range: 0x80259F70 -> 0x80259FEC
void iCameraSetFog(unsigned char enable /* r1+0x8 */) {
    // Local variables
    struct iFogParams * pFogParams; // r31
...
// Range: 0x80259FEC -> 0x8025A01C
struct RwCamera * RwCameraBeginUpdateWrapper(struct RwCamera * camera /* r1+0x8 */) {
    // Local variables
    struct RwCamera * retval; // r31
}

// Range: 0x8025A01C -> 0x8025A020
void UpdateGlobalFrustumPlanes() {}
...
// Range: 0x8025A020 -> 0x8025A0A0
void iCameraPerformRenderWareHack(struct RwCamera * camera /* r1+0x8 */) {
    // Local variables
    unsigned int memSize; // r30
    struct RpWorldSector * * newFrustumSectors; // r29
    int cameraExtOffset; // r28
