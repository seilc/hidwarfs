/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xScene.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800D194C -> 0x800D5978
*/
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D60B40
...
// Range: 0x800D194C -> 0x800D1DE4
void xSceneInit(struct xScene * sc /* r31 */, unsigned short numTrigs /* r26 */, unsigned short numSpecialTrigs /* r27 */, unsigned short numStats /* r28 */, unsigned short numDyns /* r29 */, unsigned short numNpcs /* r30 */) {
    // References
    // -> static char __FUNCTION__[11];
    // -> unsigned int gActiveHeap;
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D60B4B
// Range: 0x800D1DE4 -> 0x800D1E64
void xSceneExit(struct xScene * sc /* r1+0x8 */) {
    // References
    // -> static char __FUNCTION__[11];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D60B56
// total size: 0x0
struct xSerial {};
// Range: 0x800D1E64 -> 0x800D1F48
void xSceneSave(struct xScene * ent /* r1+0x8 */, struct xSerial * s /* r1+0xC */) {
    // References
    // -> static char __FUNCTION__[11];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D60B61
// Range: 0x800D1F48 -> 0x800D202C
void xSceneLoad(struct xScene * ent /* r1+0x8 */, struct xSerial * s /* r1+0xC */) {
    // References
    // -> static char __FUNCTION__[11];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D60B6C
// Range: 0x800D202C -> 0x800D20B4
void xSceneSetup(struct xScene * sc /* r31 */) {
    // References
    // -> static char __FUNCTION__[12];
}

static unsigned char done; // size: 0x1, address: 0x80D696C0
static signed char init; // size: 0x1, address: 0x80D696C1
static unsigned char done; // size: 0x1, address: 0x80D696C2
static signed char init; // size: 0x1, address: 0x80D696C3
static unsigned char done; // size: 0x1, address: 0x80D696C4
static signed char init; // size: 0x1, address: 0x80D696C5
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D60B78
// total size: 0xC
struct /* @class$1419xScene_cpp */ {
    // Members
...
// Range: 0x800D20B4 -> 0x800D262C
void xSceneAddEnt(struct xScene * sc /* r31 */, struct xEnt * ent /* r30 */) {
    // Local variables
    struct xTriggerAsset * triggerAsset; // r29
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D60B85
// Range: 0x800D262C -> 0x800D26D4
struct xBase * xSceneResolvID(struct xScene * sc /* r31 */, unsigned int id /* r1+0x8 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D60B94
// Range: 0x800D26D4 -> 0x800D2794
char * xSceneID2Name(struct xScene * sc /* r31 */, unsigned int id /* r1+0x8 */) {
    // References
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60BA2
// Range: 0x800D2794 -> 0x800D28CC
void xSceneForAllEnts(struct xScene * sc /* r31 */, struct xEnt * (* func)(struct xEnt *, struct xScene *, void *) /* r29 */, void * data /* r1+0x8 */) {
    // Local variables
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D60BB3
// total size: 0x24
struct xRay3 {
    // Members
...
// Range: 0x800D28CC -> 0x800D3B4C
void xRayHitsGrid(struct xGrid * grid /* r31 */, struct xScene * sc /* r1+0x8 */, struct xRay3 * r /* r30 */, void (* rentcb)(struct xScene *, struct xRay3 *, struct xQCData *, struct xEnt *, void *) /* r1+0xC */, struct xQCData * qcr /* r25 */, void * data /* r1+0x10 */) {
    // Local variables
    struct xLine3 ln; // r1+0x134
    struct xGridIterator it; // r1+0x128
    struct xVec3 delta; // r1+0x11C
    float len; // f18
    int sclgx; // r1+0x118
    int sclgz; // r1+0x114
    int eclgx; // r1+0x110
    int eclgz; // r1+0x10C
    float sclcx; // r1+0x108
    float sclcz; // f29
    float eclcx; // r1+0x104
    float eclcz; // f28
    float halfclx; // f25
    float halfclz; // f31
    float quatrclx; // f30
    float quatrclz; // r1+0x100
    int sz; // r27
    int ez; // r26
    int x; // r29
    int z; // r28
    ...
    int ax; // r24
    int bx; // r23
    struct xGridBound * cell; // r20
    struct xBound * cellbound; // r1+0xCC
    struct xGridBound * cell; // r19
    struct xBound * cellbound; // r1+0xC8
    float cz; // r1+0xC4
    float lz; // r1+0xC0
    float rz; // r1+0xBC
    float lx; // f21
    ...
    int ax; // r22
    int bx; // r21
    struct xGridBound * cell; // r18
    struct xBound * cellbound; // r1+0x98
    struct xGridBound * cell; // r17
    struct xBound * cellbound; // r1+0x94
    struct xGridBound * cell; // r1+0x90
    struct xBound * cellbound; // r1+0x8C

    // References
    // -> static char __FUNCTION__[13];
}

int xscRayentNchks; // size: 0x4, address: 0x80D60BC0
int xscNupdates; // size: 0x4, address: 0x80D60BC4
float xscRePerUpd; // size: 0x4, address: 0x80D60BC8
int xscRayscNchks; // size: 0x4, address: 0x80D60BCC
int xscRaycamscNchks; // size: 0x4, address: 0x80D60BD0
int xscRayscfNchks; // size: 0x4, address: 0x80D60BD4
float xscRsPerUpd; // size: 0x4, address: 0x80D60BD8
float xscRcamsPerUpd; // size: 0x4, address: 0x80D60BDC
float xscRsfPerUpd; // size: 0x4, address: 0x80D60BE0
float xscRePerRs; // size: 0x4, address: 0x80D60BE4
int xscNqcrejects; // size: 0x4, address: 0x80D60BE8
float xscQcrPerUpd; // size: 0x4, address: 0x80D60BEC
float xscQcrPerRs; // size: 0x4, address: 0x80D60BF0
float xscQcrPerRe; // size: 0x4, address: 0x80D60BF4
// Range: 0x800D3B4C -> 0x800D3DAC
void xSceneDebugUpdate() {
    // References
    // -> int xscRayentNchks;
    // -> int xscNqcrejects;
    // -> float xscQcrPerRe;
    ...
    // -> int xscRayscfNchks;
    // -> float xscRsfPerUpd;
    // -> int xscRaycamscNchks;
    // -> float xscRcamsPerUpd;
    // -> float xscRsPerUpd;
    // -> float xscRePerUpd;
}

// total size: 0xC
struct /* @class$1039xScene_cpp */ {
    // Members
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D60BF8
// Range: 0x800D3DAC -> 0x800D3ED8
void xRayHitsEnt(struct xRay3 * r /* r1+0x8 */, struct xQCData * qcr /* r29 */, struct xEnt * ent /* r30 */, void * colldata /* r1+0xC */) {
    // Local variables
    struct xCollis * coll; // r31
    struct xCollis c; // r1+0x10
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D60C04
// Range: 0x800D3ED8 -> 0x800D41CC
void xRayHitsScene(struct xScene * sc /* r29 */, struct xRay3 * r /* r30 */, struct xCollis * coll /* r31 */) {
    // Local variables
    struct xQCData q; // r1+0x8
    struct xCollis c; // r1+0x28
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60C12
// Range: 0x800D41CC -> 0x800D4330
void xRayHitsEntFlags(struct xRay3 * r /* r29 */, struct xCollis * coll /* r30 */, struct xQCData * qcr /* r28 */, struct xEnt * ent /* r31 */, unsigned char collType /* r1+0x8 */) {
    // Local variables
    struct xCollis c; // r1+0xC

    // References
    // -> static char __FUNCTION__[17];
}

static unsigned char already_asserted; // size: 0x1, address: 0x80D696C6
static signed char init; // size: 0x1, address: 0x80D696C7
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D60C23
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D60C28
// total size: 0x10
struct cbRayHitsEnt {
    // Functions
    static cbRayHitsEnt(const struct xRay3 & ray, struct xCollis & coll, unsigned char chkby, unsigned char collType);

    static unsigned char operator()(struct xEnt & ent);

    // Members
    const struct xRay3 * ray; // offset 0x0, size 0x4
    struct xCollis * coll; // offset 0x4, size 0x4
    unsigned char chkby; // offset 0x8, size 0x1
    unsigned char collType; // offset 0x9, size 0x1
    int hitCalc; // offset 0xC, size 0x4
};
// total size: 0x10
struct xSphere {
    // Members
...
// Range: 0x800D4330 -> 0x800D44FC
void xRayHitsSceneFlags(struct xScene * sc /* r28 */, const struct xRay3 * r /* r30 */, struct xCollis * coll /* r31 */, unsigned char collType /* r1+0x8 */, unsigned char chk /* r29 */) {
    // Local variables
    struct cbRayHitsEnt cb; // r1+0xC
    struct xBound bound; // r1+0x70
    struct xCollis tempColl; // r1+0x1C
...
// Range: 0x800D44FC -> 0x800D451C
// this: r3
static cbRayHitsEnt::cbRayHitsEnt(const struct xRay3 & ray /* r4 */, struct xCollis & coll /* r5 */, unsigned char chkby /* r6 */, unsigned char collType /* r7 */) {}

// total size: 0x10
struct cbSphereHitsEnt {
    // Members
...
// Range: 0x800D451C -> 0x800D45F8
static void ProjectTriangle(struct xVec3 * rkD /* r3 */, struct xVec3 * apkTri /* r4 */, float * rfMin /* r5 */, float * rfMax /* r6 */) {
    // Local variables
    float fDot; // f31
}

// Range: 0x800D45F8 -> 0x800D46DC
static void ProjectBox(struct xVec3 * rkD /* r3 */, struct xBox * rkBox /* r4 */, float * rfMin /* r5 */, float * rfMax /* r6 */) {
    // Local variables
    float fDdC; // f31
    float fR; // f30
}

// Range: 0x800D46DC -> 0x800D4ADC
static unsigned int Mgc_TriBoxTest(struct xVec3 * apkTri /* r31 */, struct xBox * rkBox /* r29 */) {
    // Local variables
    float fMin0; // r1+0x14
    float fMax0; // r1+0x10
    float fMin1; // r1+0xC
    float fMax1; // r1+0x8
    struct xVec3 kD; // r1+0x24
    struct xVec3 akE[3]; // r1+0x30
    struct xVec3 baxis; // r1+0x18
    unsigned int andflag; // r25
    unsigned int curflag; // r30
    int i; // r28
    int i0; // r27
    int i1; // r26
}

static struct xMat4x3 * sBoxTestMat; // size: 0x4, address: 0x80D696C8
static unsigned char sUseJSP; // size: 0x1, address: 0x80D696CC
static void * sTestOPtr; // size: 0x4, address: 0x80D696D0
static struct xModelInstance * sTestMPtr; // size: 0x4, address: 0x80D696D4
// total size: 0x28
...
// Range: 0x800D4ADC -> 0x800D4FD4
static struct RpCollisionTriangle * nearestFloorCB(struct RpCollisionTriangle * collTriangle /* r30 */, void * data /* r1+0x8 */) {
    // Local variables
    struct xNearFloorPoly * nfpoly; // r31
    float currnear; // f31
    ...
    float neardz; // f25
    float nearmag; // f24
    struct xVec3 edgevec; // r1+0x24
    struct xVec3 centvec; // r1+0x18
    struct xVec3 testpt; // r1+0xC

    // References
    // -> static struct xModelInstance * sTestMPtr;
    // -> static void * sTestOPtr;
    // -> static struct xMat4x3 * sBoxTestMat;
    // -> static unsigned char sUseJSP;
}

// Range: 0x800D4FD4 -> 0x800D50B0
static struct RpCollisionTriangle * boxNearestFloorCB(struct RpIntersection * intersection /* r1+0x8 */, struct RpCollisionTriangle * collTriangle /* r31 */, float distance /* r1+0xC */, void * data /* r29 */) {
    // Local variables
    struct xNearFloorPoly * nfpoly; // r30
    struct xVec3 xformVert[3]; // r1+0x10

    // References
    // -> static struct xMat4x3 * sBoxTestMat;
}

static struct xBound sNearestBound; // size: 0x4C, address: 0x80B96308
static unsigned char sNearestChk; // size: 0x1, address: 0x80D696D8
static unsigned char sNearestCollType; // size: 0x1, address: 0x80D696D9
// total size: 0x18
struct RwLine {
    // Members
...
static struct RpIntersection sSphereIsx; // size: 0x1C, address: 0x80B96354
// Range: 0x800D50B0 -> 0x800D50F0
static struct RpCollisionTriangle * sectorNearestFloorCB(struct RpIntersection * intersection /* r1+0x8 */, struct RpCollisionTriangle * collTriangle /* r1+0xC */, float distance /* r1+0x10 */, void * data /* r1+0x14 */) {}

static int been_here; // size: 0x4, address: 0x80D696DC
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D60C3B
// total size: 0x18
struct zSurfMatFX {
    // Members
    unsigned int flags; // offset 0x0, size 0x4
...
// Range: 0x800D50F0 -> 0x800D52B0
static int gridNearestFloorCB(struct xEnt * ent /* r31 */, void * cbdata /* r1+0x8 */) {
    // Local variables
    struct xNearFloorPoly * nfpoly; // r28
    struct zSurfaceProps * props; // r30
    unsigned char flag; // r27
    struct xCollis tempC; // r1+0xC
    struct RwFrame * frame; // r26

    // References
    // -> static int been_here;
    // -> static char __FUNCTION__[19];
    // -> static struct RpIntersection sSphereIsx;
    // -> static struct xModelInstance * sTestMPtr;
    // -> static struct xMat4x3 * sBoxTestMat;
    // -> static void * sTestOPtr;
    // -> static struct xBound sNearestBound;
    // -> static unsigned char sNearestCollType;
    // -> static unsigned char sNearestChk;
}

static char __FUNCTION__[23]; // size: 0x17, address: 0x80513A44
struct xGrid grabbable_grid; // size: 0x38, address: 0x80CF49E8
// Range: 0x800D52B0 -> 0x800D56C8
unsigned int xSceneNearestFloorPoly(struct xScene * sc /* r30 */, struct xNearFloorPoly * nfpoly /* r31 */, unsigned char collType /* r1+0x8 */, unsigned char chk /* r28 */) {
    // Local variables
...
// Range: 0x800D56C8 -> 0x800D5978
// this: r30
static unsigned char cbRayHitsEnt::operator()(struct xEnt & ent /* r31 */) {
    // Local variables
    struct zSurfaceProps * prop; // r29
    struct xCollis tempColl; // r1+0x8
    struct xModelInstance * collModel; // r28
