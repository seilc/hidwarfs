/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xCamSupport.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80016AF4 -> 0x8001B010
*/
static char __FUNCTION__[7]; // size: 0x7, address: 0x80D5E7B0
...
struct debug_context {
    // Members
    char weight_text[256]; // offset 0x0, size 0x100
    int max_nodes_used; // offset 0x100, size 0x4
};
// total size: 0xB0
class xCamSupportPath {
    // Functions
    void create(int max_size, int debug_max_size);

    void reset(const struct xVec3 & start_loc);

    void refresh_config();

    int polar_subdivide(const struct xCamCoordPolar & start, const struct xCamCoordPolar & end);

    void prune(int start, int size);

    void smooth(int start, int size);

    float traverse(struct xVec3 & rloc, float velmag, float dt);

    void refresh_dist();

    void debug_init(int debug_max_size);

    void debug_refresh_config();

    void add_tweaks(const char * prefix, const char * debug_prefix);
...
// Range: 0x80016AF4 -> 0x80016C5C
// this: r31
void xCamSupportPath::create(int max_size /* r30 */, int debug_max_size /* r1+0x8 */) {
    // Local variables
    unsigned int alloc_size; // r29
...
// Range: 0x80016C5C -> 0x80016CE8
// this: r31
void xCamSupportPath::reset(const struct xVec3 & start_loc /* r1+0x8 */) {
    // Local variables
    struct path_node * p; // r30
    struct path_node * endp; // r29
}

// Range: 0x80016CE8 -> 0x80016D28
// this: r31
...
// Range: 0x80016D28 -> 0x80016E44
// this: r31
int xCamSupportPath::polar_subdivide(const struct xCamCoordPolar & start /* r1+0x8 */, const struct xCamCoordPolar & end /* r1+0xC */) {
    // Local variables
    int path_end; // r28
    int vert_used; // r30
    int subdiv_end; // r29
...
static char __FUNCTION__[6]; // size: 0x6, address: 0x80D5E7B7
// Range: 0x80016E44 -> 0x8001709C
// this: r31
void xCamSupportPath::prune(int start /* r23 */, int size /* r1+0x8 */) {
    // Local variables
    int old_used; // r1+0xC
    int i0; // r28
    int i1; // r26
    int i2; // r25
    int end_i2; // r24
    const struct path_node & A; // r27
    struct path_node & B; // r30
    const struct path_node & C; // r29
    float dx; // f31
    float dy; // f30
    float dz; // f29
    float ABmag; // f28
...
static struct path_node prev_path[33]; // size: 0x18C, address: 0x8055B760
static char __FUNCTION__[7]; // size: 0x7, address: 0x80D5E7BD
// Range: 0x800170B8 -> 0x800173D4
// this: r31
void xCamSupportPath::smooth(int start /* r21 */, int size /* r22 */) {
    // Local variables
    const float * w; // r30
    struct path_node * pp; // r28
    struct path_node * end_pp; // r17
    int r; // r24
    int end_r; // r1+0x18
    int i; // r27
    int end_i; // r1+0x14
    ...
    float Ry; // f30
    float Rz; // f29
    struct path_node & node; // r19
    const struct path_node & B; // r26
    const struct path_node & B; // r25

    // References
    // -> static struct path_node prev_path[33];
    // -> static char __FUNCTION__[7];
}
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D5E7C4
// Range: 0x800173D4 -> 0x800177C8
// this: r31
float xCamSupportPath::traverse(struct xVec3 & rloc /* r26 */, float velmag /* f29 */, float dt /* f21 */) {
    // Local variables
    int i; // r28
    int end; // r25
    const struct path_node & p0; // r29
    const struct path_node & p1; // r30
    float dx; // f28
    float dy; // f27
    float dz; // f26
    float segdist; // f31
    float t; // f20
    const struct path_node & p2; // r27
    float ndx; // f25
    float ndy; // f24
    float ndz; // f23
    float next_subdist; // r1+0x8
    float d; // f19
    float damp; // f22
    float s; // f30
...
static float increase_factor; // size: 0x4, address: 0x80D68C10
static signed char init; // size: 0x1, address: 0x80D68C14
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5E7CD
// total size: 0xC
struct path_node {
    // Members
    struct xVec3 loc; // offset 0x0, size 0xC
};
// Range: 0x800178EC -> 0x80017DE0
int xCamSupportPath::polar_subdivide(struct path_node * vert /* r24 */, int vert_size /* r25 */, const struct xCamCoordPolar & start /* r28 */, const struct xCamCoordPolar & end /* r29 */, float thresh /* r1+0x8 */) {
    // Local variables
    struct path_node head; // r1+0x48
    struct path_node tail; // r1+0x3C
    int vert_used; // r30
    float ABx; // f23
    float ABy; // f22
    float ABz; // f21
    float best_total; // f20
    float x; // f26
    float y; // f25
    float z; // f24
    float theta; // f29
    ...
    float dy; // r1+0x34
    float dz; // r1+0x30
    float dtheta; // r1+0x2C
    float dphi; // r1+0x28
    float ddist; // r1+0x24
    float total; // f30
    struct path_node * v; // r31
    struct path_node * endv; // r22
    float cp; // f19
    const struct xVec3 & A; // r27
    float ABx; // f18
    float ABy; // r1+0x20
    float ABz; // r1+0x1C
    const struct xVec3 & A; // r26
    float ABx; // r1+0x18
    float ABy; // r1+0x14
    float ABz; // r1+0x10
    float dtotal; // r1+0xC
    int subdiv_increase; // r21

    // References
    // -> static float increase_factor;
...
// total size: 0x4
struct /* @class$1070xCamSupport_cpp */ {
    // Members
...
union /* @class$819xCamSupport_cpp */ {
...
// total size: 0x10
struct /* @class$832xCamSupport_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5E7DD
// Range: 0x80017DE0 -> 0x80018094
void xCamSupportPath::compute_weights(float * w /* r31 */, int n /* r27 */, float s /* f21 */) {
    // Local variables
    float start_efrac; // f31
    int i; // r29
    float efrac; // f30
    int j; // r30
    float spow; // f28
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D5E7ED
// Range: 0x80018094 -> 0x800181F0
// this: r31
void xCamSupportPath::debug_init(int debug_max_size /* r30 */) {
    // Local variables
    unsigned int alloc_size; // r29
...
// Range: 0x800181F0 -> 0x800182AC
// this: r30
void xCamSupportPath::debug_refresh_config() {
    // Local variables
    char * s; // r31
    int i; // r29
}
...
// total size: 0xC
struct /* @class$768xCamSupport_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$769xCamSupport_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$770xCamSupport_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$771xCamSupport_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$772xCamSupport_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$773xCamSupport_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$774xCamSupport_cpp */ {
    // Members
...
// Range: 0x800182AC -> 0x800182E4
static void on_change_config(const struct tweak_info & info /* r30 */) {
    // Local variables
    class xCamSupportPath * path; // r31
}
...
static struct tweak_callback cb_config; // size: 0x28, address: 0x80474BA0
// Range: 0x800182E4 -> 0x80018484
// this: r31
void xCamSupportPath::add_tweaks(const char * prefix /* r30 */, const char * debug_prefix /* r29 */) {
    // References
    // -> static struct tweak_callback cb_config;
}

enum /* @enum$956xCamSupport_cpp */ {
    DEBUG_FLAG_RENDER = 1,
};
// total size: 0x4
struct xColor_tag {
...
// Range: 0x80018484 -> 0x80018738
// this: r31
void xCamSupportPath::debug_render() {
    // Local variables
    int i; // r27
    int end; // r25
    struct xColor_tag color; // r1+0x14
    const struct path_node & p0; // r24
    const struct path_node & p1; // r23
    int i; // r28
    int end; // r26
    float red; // f31
    float dred; // f30
    const struct path_node & p0; // r30
    const struct path_node & p1; // r29
    float asum0; // f29
    float asum1; // f28
...
static unsigned char enable_collision; // size: 0x1, address: 0x80D5E7F8
static unsigned char enable_coll_entity; // size: 0x1, address: 0x80D5E7F9
static unsigned char render_coll_cache_ent; // size: 0x1, address: 0x80D68C15
static unsigned char render_coll_cache_tri; // size: 0x1, address: 0x80D68C16
static unsigned char render_coll_pivot; // size: 0x1, address: 0x80D68C17
static unsigned char render_coll_compress; // size: 0x1, address: 0x80D68C18
// Range: 0x80018738 -> 0x80018894
static void add_tweaks(const char * prefix /* r31 */) {
    // References
    // -> static unsigned char render_coll_compress;
    // -> static unsigned char render_coll_pivot;
    // -> static unsigned char render_coll_cache_tri;
    // -> static unsigned char render_coll_cache_ent;
    // -> static unsigned char enable_coll_entity;
    // -> static unsigned char enable_collision;
}
...
static struct ent_cache_entry ent_cache[256]; // size: 0x400, address: 0x8055B904
static int ent_cache_size; // size: 0x4, address: 0x80D68C1C
static char __FUNCTION__[5]; // size: 0x5, address: 0x80D5E7FA
// Range: 0x80018894 -> 0x80018AF0
static float line_point_dist(const struct xVec3 & A /* r29 */, const struct xVec3 & B /* r30 */, const struct xVec3 & P /* r31 */) {
    // Local variables
    float ABx; // f31
    float ABy; // f30
    float ABz; // f29
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D5E7FF
// Range: 0x80018AF0 -> 0x80018CF4
static float arc_sphere_to_plane(const struct xMat4x3 & circle_mat /* r30 */, float Rc /* r1+0x8 */, float Rs /* r1+0xC */, const struct xVec3 & Nplane /* r31 */, float Dplane /* f27 */) {
    // Local variables
    struct xVec3 N; // r1+0x1C
    struct xVec3 point_on_plane; // r1+0x10
    float D; // f26
    float D_Rc; // f30
    float Nx2_Ny2; // f28
    float det; // f31
...
static int been_here; // size: 0x4, address: 0x80D68C20
static int been_here; // size: 0x4, address: 0x80D68C24
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5E813
...
// total size: 0x890
struct /* @class$1808xCamSupport_cpp */ {
    // Static members
...
// total size: 0xC
struct /* @class$1208xCamSupport_cpp */ {
    // Members
...
// Range: 0x80018CF4 -> 0x80018E88
static unsigned char capsule_hits_scene(const struct xCapsule & cap /* r31 */, struct xCollis & coll /* r30 */) {
    // Local variables
    struct xBox box; // r1+0x28
    struct xQCData qcd; // r1+0x8
    struct xRay3 ray; // r1+0x40
    struct ent_cache_entry * ite; // r29
    struct ent_cache_entry * end_ite; // r27
    struct xEnt & ent; // r28
    struct xCollis temp_coll; // r1+0x64

    // References
    // -> static int ent_cache_size;
    // -> static struct ent_cache_entry ent_cache[256];
    // -> struct xGlobals * xglobals;
}

// Range: 0x80018F34 -> 0x80019224
static unsigned char ray_hits_bound(const struct xRay3 & ray /* r30 */, const struct xBound & bound /* r31 */, float expand /* f31 */) {
    // Local variables
    float oldr; // f28
    unsigned char result; // r29
    struct xBox box; // r1+0x28
    struct xMat3x3 mat; // r1+0x70
    struct xRay3 local_ray; // r1+0x40
    float scale; // f30
    float iscale; // f29
    struct xBox box; // r1+0x10
...
// Range: 0x80019224 -> 0x800193C4
static unsigned char swept_sphere_hits_scene(struct xSweptSphere & sws /* r31 */, const struct xVec3 & start /* r30 */, const struct xVec3 & end /* r29 */, float radius /* f31 */) {
    // Local variables
    struct xBox box; // r1+0x28
    struct xQCData qcd; // r1+0x8
    struct xRay3 ray; // r1+0x40
    struct ent_cache_entry * ite; // r28
    struct ent_cache_entry * end_ite; // r25
    struct xEnt & ent; // r27
    struct xModelInstance & model; // r26
...
// Range: 0x800193C4 -> 0x800194FC
static unsigned char sphere_hits_scene(const struct xSphere & o /* r29 */, struct xCollis & coll /* r31 */) {
    // Local variables
    struct xQCData qcd; // r1+0x8
    struct ent_cache_entry * ite; // r30
    struct ent_cache_entry * end_ite; // r27
    struct xEnt & ent; // r28
    struct xCollis temp_coll; // r1+0x28
...
// Range: 0x800194FC -> 0x800196B4
static void draw_arc(const struct xMat4x3 & circle_mat /* r29 */, float radius /* f29 */, float min_ang /* f25 */, float max_ang /* r1+0x8 */, int segments /* r30 */) {
    // Local variables
    float dang; // f30
    float ang; // f31
    int i; // r31
    struct xVec3 loc0; // r1+0x18
    struct xVec3 loc1; // r1+0xC
}

// total size: 0x38
class xCamSupportLOSBar {
    // Functions
    void reset(const struct xVec3 & origin, const struct xVec3 & stern_loc);

    void begin_update(const struct xVec3 & origin);

    void precollide();

    unsigned char collide_pivot();

    unsigned char collide_compress();
...
// Range: 0x800196B4 -> 0x80019704
// this: r31
void xCamSupportLOSBar::reset(const struct xVec3 & origin /* r1+0x8 */, const struct xVec3 & stern_loc /* r1+0xC */) {
    // References
    // -> struct config cfg;
}

// Range: 0x80019704 -> 0x800197A4
// this: r31
void xCamSupportLOSBar::begin_update(const struct xVec3 & origin /* r30 */) {
    // Local variables
    float dy; // f31
...
struct cache_entity_callback {
    // Functions
    static cache_entity_callback(const struct xVec3 & center, float radius);

    static unsigned char operator()(struct xEnt & ent);

    // Members
    struct ent_cache_entry * cache; // offset 0x0, size 0x4
    struct xVec3 center; // offset 0x4, size 0xC
    float radius; // offset 0x10, size 0x4
};
// Range: 0x800197A4 -> 0x800199AC
// this: r31
void xCamSupportLOSBar::precollide() {
    ...
    float yupper; // f26
    float ycenter; // f25
    struct xBound bound; // r1+0x1C
    struct cache_entity_callback cb; // r1+0x8
...
// Range: 0x800199AC -> 0x800199D8
// this: r3
static cache_entity_callback::cache_entity_callback(const struct xVec3 & center /* r4 */, float radius /* f1 */) {
    // References
    // -> static struct ent_cache_entry ent_cache[256];
}

static unsigned char done; // size: 0x1, address: 0x80D68C28
static signed char init; // size: 0x1, address: 0x80D68C29
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5E822
...
// Range: 0x800199D8 -> 0x8001A98C
// this: r31
unsigned char xCamSupportLOSBar::collide_pivot() {
    // Local variables
    struct xCollis coll; // r1+0x2C0
    struct xCapsule to_cap; // r1+0x1F0
    float to_t; // f30
    float to_dist; // f26
    struct xVec3 to_tohit; // r1+0x174
    struct xVec3 to_heading; // r1+0x168
    struct xVec3 to_impact; // r1+0x15C
    float new_rail_yoffset; // f28
    struct xCapsule new_cap; // r1+0x1D4
    struct xVec3 N; // r1+0x150
    float D; // r1+0x80
    struct xMat4x3 vcircle_mat; // r1+0x280
    float bar_mag; // f19
    float Rc; // r1+0x7C
    float vrot; // f29
    float oldpitch; // r1+0x78
    float newpitch; // r1+0x74
    float dx; // f18
    float dz; // r1+0x70
    float xzdist; // r1+0x6C
    float newy; // r1+0x68
    struct xCapsule from_cap; // r1+0x1B8
    float from_t; // f31
    float from_dist; // f24
    struct xVec3 from_tohit; // r1+0x144
    float start_s; // f23
    float end_s; // f22
    int tries; // r26
    float s; // f25
    struct xCapsule cap; // r1+0x19C
    struct xVec3 cap_offset; // r1+0x138
    struct xVec3 impact; // r1+0x12C
    struct xVec3 N; // r1+0x120
    float D; // r1+0x64
    float min_from_t; // r1+0x60
    const struct xVec3 & C; // r28
    struct xVec3 P; // r1+0x114
    struct xVec3 CP; // r1+0x108
    struct xVec3 uCP; // r1+0xFC
    float Rc; // f21
    struct xMat4x3 circle_mat; // r1+0x240
    float max_rot; // r1+0x5C
    struct xMat3x3 rot_mat; // r1+0x210
    struct xVec3 tohit0; // r1+0xF0
    struct xVec3 tohit1; // r1+0xE4
    float depen_mul; // r1+0x58
    struct xVec3 depen; // r1+0xD8
    const struct xVec3 & P; // r30
    const struct xVec3 & A; // r27
    const struct xVec3 & B; // r29
    struct xVec3 AP; // r1+0xCC
    struct xVec3 BP; // r1+0xC0
    struct xVec3 rot_axis; // r1+0xB4
    float R; // r1+0x54
    float R2; // r1+0x50
    struct xVec3 PV; // r1+0xA8
    float det; // f20
    struct xVec3 V; // r1+0x9C
    struct xVec3 BV; // r1+0x90
    float offset_mag; // r1+0x4C
    struct xCapsule test_cap; // r1+0x180
    float old_dist; // r1+0x48
    struct xVec3 test_impact; // r1+0x84
    float new_dist; // f27
    int asdf; // r1+0x44
...
// Range: 0x8001A98C -> 0x8001ACA8
// this: r31
unsigned char xCamSupportLOSBar::collide_compress() {
    // Local variables
    struct xSweptSphere sws; // r1+0x9C
    struct xVec3 start; // r1+0x3C
    struct xVec3 end; // r1+0x30
    struct xVec3 dir; // r1+0x24
    struct xSphere o; // r1+0x14
...
// Range: 0x8001ACA8 -> 0x8001AE4C
void xCamSupportLOSBar::add_tweaks(const char * prefix /* r31 */, const char * debug_prefix /* r30 */) {
    // References
    // -> struct config cfg;
...
// Range: 0x8001AE4C -> 0x8001AEE8
void xCamSupportLOSBar::debug_render() {
    // Local variables
    struct ent_cache_entry * e; // r31
    struct ent_cache_entry * ende; // r30
...
// Range: 0x8001AEE8 -> 0x8001B010
// this: r30
static unsigned char cache_entity_callback::operator()(struct xEnt & ent /* r31 */) {
    // References
    // -> static int ent_cache_size;
