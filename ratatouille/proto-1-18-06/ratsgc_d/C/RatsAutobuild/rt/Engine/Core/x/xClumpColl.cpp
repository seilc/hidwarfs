/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xClumpColl.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8001B978 -> 0x8001E514
*/
unsigned char xClumpColl_FilterFlags; // size: 0x1, address: 0x80D5E830
unsigned char xClumpColl_FilterFlagsAllMask; // size: 0x1, address: 0x80D68C30
unsigned char xClumpColl_FilterFlagsAllEqual; // size: 0x1, address: 0x80D5E831
// Range: 0x8001B978 -> 0x8001B9B8
unsigned int xClumpColl_SetFilterFlags(unsigned char filterFlags /* r3 */, unsigned char allMask /* r4 */, unsigned char allEqual /* r5 */) {
    // Local variables
    unsigned int previousFlags; // r31
...
// Range: 0x8001B9B8 -> 0x8001B9D0
void xClumpColl_RestoreFilterFlags(unsigned int previousFlags /* r3 */) {
    // References
    // -> unsigned char xClumpColl_FilterFlagsAllEqual;
    // -> unsigned char xClumpColl_FilterFlagsAllMask;
    // -> unsigned char xClumpColl_FilterFlags;
...
static char __FUNCTION__[28]; // size: 0x1C, address: 0x804F0F38
// Range: 0x8001B9D0 -> 0x8001BAEC
struct xClumpCollBSPTree * xClumpColl_StaticBufferInit(void * data /* r1+0x8 */) {
    // Local variables
    unsigned int * header; // r30
    unsigned int numBranchNodes; // r29
    unsigned int numTriangles; // r27
    struct xClumpCollBSPTree * tree; // r31
    unsigned int magicTag; // r28
...
static char __FUNCTION__[42]; // size: 0x2A, address: 0x804F0F54
// total size: 0x18
struct RwBBox {
    // Members
...
// Range: 0x8001BB04 -> 0x8001BD94
struct xClumpCollBSPTree * xClumpColl_ForAllBoxLeafNodeIntersections(struct xClumpCollBSPTree * tree /* r29 */, struct RwBBox * box /* r27 */, int (* callBack)(struct xClumpCollBSPTriangle *, void *) /* r25 */, void * data /* r1+0x8 */) {
    // Local variables
    int nStack; // r30
    struct nodeInfo nodeStack[33]; // r1+0x14
    struct nodeInfo node; // r1+0xC
    struct xClumpCollBSPTriangle * tris; // r26
    struct xClumpCollBSPBranchNode * branch; // r31

    // References
    // -> static char __FUNCTION__[42];
}

static char __FUNCTION__[43]; // size: 0x2B, address: 0x804F0F7E
// total size: 0x18
...
// Range: 0x8001BD94 -> 0x8001C970
struct xClumpCollBSPTree * xClumpColl_ForAllLineLeafNodeIntersections(struct xClumpCollBSPTree * tree /* r28 */, struct RwLine * line /* r24 */, struct xClumpCollV3dGradient * grad /* r29 */, int (* callBack)(struct xClumpCollBSPTriangle *, void *) /* r25 */, void * data /* r1+0x8 */) {
    // Local variables
    int nStack; // r30
    struct nodeInfo nodeStack[33]; // r1+0x6C
    struct nodeInfo node; // r1+0x1C
    struct RwLine lineStack[33]; // r1+0x174
    struct RwLine currLine; // r1+0x54
    struct xClumpCollBSPTriangle * tris; // r26
    union RwSplitBits lStart; // r1+0x18
    union RwSplitBits lEnd; // r1+0x14
    union RwSplitBits rStart; // r1+0x10
    union RwSplitBits rEnd; // r1+0xC
    struct xClumpCollBSPBranchNode * branch; // r31
    struct RwV3d vTmp; // r1+0x48
    float delta; // f31
    struct RwV3d vTmp; // r1+0x3C
    float delta; // f30
    struct RwV3d vLeft; // r1+0x30
    struct RwV3d vRight; // r1+0x24
    float delta; // f29
    float delta; // f28
...
static char __FUNCTION__[46]; // size: 0x2E, address: 0x804F0FA9
// Range: 0x8001C9C0 -> 0x8001D5C8
struct xClumpCollBSPTree * xClumpColl_ForAllCapsuleLeafNodeIntersections(struct xClumpCollBSPTree * tree /* r28 */, struct RwLine * line /* r24 */, float radius /* f27 */, struct xClumpCollV3dGradient * grad /* r29 */, int (* callBack)(struct xClumpCollBSPTriangle *, void *) /* r25 */, void * data /* r1+0x8 */) {
    // Local variables
    int nStack; // r30
    struct nodeInfo nodeStack[33]; // r1+0x6C
    struct nodeInfo node; // r1+0x1C
...
static char __FUNCTION__[26]; // size: 0x1A, address: 0x804F0FD7
// total size: 0x28
...
// Range: 0x8001D5C8 -> 0x8001DC24
static int LeafNodeLinePolyIntersect(struct xClumpCollBSPTriangle * triangles /* r29 */, void * data /* r21 */) {
    // Local variables
    struct PolyLineTestParam * isData; // r31
    struct CallBackParam * cbParam; // r25
    struct RwV3d * v0; // r30
    struct RwV3d * v1; // r27
    struct RwV3d * v2; // r26
    float distance; // f31
    ...
    float det; // f30
    struct RwV3d edgetmp; // r1+0x20
    float lo; // f29
    float hi; // f28
    float u; // f27
    float v; // f25
    struct RpCollisionTriangle collisionTri; // r1+0x68
...
static char __FUNCTION__[28]; // size: 0x1C, address: 0x804F0FF1
// Range: 0x8001DC24 -> 0x8001DDEC
static int LeafNodeSpherePolyIntersect(struct xClumpCollBSPTriangle * triangles /* r31 */, void * data /* r24 */) {
    // Local variables
    struct PolyTestParam * isData; // r29
    struct CallBackParam * cbParam; // r30
...
static char __FUNCTION__[25]; // size: 0x19, address: 0x804F100D
// Range: 0x8001DDEC -> 0x8001E0A8
static int LeafNodeBoxPolyIntersect(struct xClumpCollBSPTriangle * triangles /* r31 */, void * data /* r25 */) {
    // Local variables
    struct PolyTestParam * isData; // r26
...
static char __FUNCTION__[31]; // size: 0x1F, address: 0x804F1026
// total size: 0x10
struct RwSphere {
    // Members
...
// Range: 0x8001E0A8 -> 0x8001E4E0
struct xClumpCollBSPTree * xClumpColl_ForAllIntersections(struct xClumpCollBSPTree * tree /* r30 */, struct RpIntersection * intersection /* r31 */, struct RpCollisionTriangle * (* callBack)(struct RpIntersection *, struct RpWorldSector *, struct RpCollisionTriangle *, float, void *) /* r28 */, void * data /* r1+0x8 */) {
    // Local variables
    struct CallBackParam cbParam; // r1+0x14
    struct PolyLineTestParam isData; // r1+0x68
    struct RwLine * line; // r29
    float recip; // f31
    struct PolyTestParam isData; // r1+0x44
    struct TestSphere testSphere; // r1+0xC
    struct PolyTestParam isData; // r1+0x20
