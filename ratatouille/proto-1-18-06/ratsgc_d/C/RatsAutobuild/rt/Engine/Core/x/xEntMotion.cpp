/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xEntMotion.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800649B0 -> 0x80069400
*/
char __FUNCTION__$localstatic1$xSpline3_ArcTotal__FP8xSpline3[18]; // size: 0x12, address: 0x80D5F480
// total size: 0x10
struct basic_rect {
    // Static members
...
// total size: 0x4
struct /* @class$901xEntMotion_cpp */ {
    // Members
...
union /* @class$1040xEntMotion_cpp */ {
...
// total size: 0xC
struct /* @class$1039xEntMotion_cpp */ {
    // Members
...
static struct xEntMotion * * dbg_xems; // size: 0x4, address: 0x80D69018
static unsigned short ent_motion_dbg_num; // size: 0x2, address: 0x80D6901C
static unsigned short ent_motion_dbg_num_allocd; // size: 0x2, address: 0x80D6901E
static signed short ent_motion_dbg_idx; // size: 0x2, address: 0x80D5F492
static int been_here; // size: 0x4, address: 0x80D69020
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5F494
// Range: 0x800649B0 -> 0x80064DA0
void xEntMotionInit(struct xEntMotion * motion /* r30 */, struct xEnt * owner /* r1+0x8 */, struct xEntMotionAsset * asset /* r29 */) {
    // Local variables
    struct xEntMotionMechData * mkasst; // r31
...
// Range: 0x80064DA0 -> 0x80064DBC
static void xEntSpeedDataReset(struct xEntSpeedData * speed_data /* r3 */) {}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5F4A3
// Range: 0x80064DBC -> 0x80064FD8
void xEntMotionSetup(struct xEntMotion * motion /* r31 */) {
    // Local variables
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5F4B3
// total size: 0x30
struct xMat3x3 {
    // Members
...
// Range: 0x80064FD8 -> 0x80065808
void xEntMotionReset(struct xEntMotion * motion /* r31 */, struct xScene * sc /* r24 */) {
    // Local variables
    struct xEntMotionPenData * aspen; // r27
    float dangle; // f21
    struct xMat3x3 pshrot; // r1+0x40
    struct xVec3 totop; // r1+0x28
    struct xVec3 pshtotop; // r1+0x1C
    struct xVec3 pshdelta; // r1+0x10
    struct xMat3x3 * modlrot; // r26
    struct xVec3 * modlpos; // r25
    struct xEnt * ownr; // r28
    struct xEntMotionMechData * mkasst; // r30
    struct xEntMechData * mech; // r29
    float scale; // f29
    float model_scale; // f31
    float asset_scale; // f30
    float drot; // f28
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5F4C3
// Range: 0x80065808 -> 0x80065A14
void xEntMotionMove(struct xEntMotion * motion /* r31 */, struct xScene * sc /* r29 */, float dt /* f31 */, struct xEntFrame * frame /* r30 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D5F4D2
// Range: 0x80065A14 -> 0x80065D20
static void xEntERMove(struct xEntMotion * motion /* r31 */, float dt /* f31 */, struct xEntFrame * frame /* r30 */) {
    // Local variables
    float newt; // f28
    float rem; // f30
    float scale; // f25
...
// Range: 0x80065D20 -> 0x80065EC4
static void xEntOrbitMove(struct xEntMotion * motion /* r31 */, float dt /* f25 */, struct xEntFrame * frame /* r30 */) {
    // Local variables
    float th; // f30
    float tf; // f26
    float u; // f29
...
// Range: 0x80065EC4 -> 0x80066030
static void xEntMPGetNext(struct xEntMotion * motion /* r30 */, struct xMovePoint * prev /* r1+0x10 */) {
    // Local variables
    struct xEntMPData * mp; // r31
    struct xVec3 tempdir; // r1+0x14
    struct xMat3x3 mat; // r1+0x20
}

// Range: 0x80066030 -> 0x800660C4
static void xEntSpeedDataUpdate(struct xEntSpeedData * speed_data /* r3 */, float dt /* f1 */) {}
...
// Range: 0x800660C4 -> 0x800665E8
static void xEntMPMove(struct xEntMotion * motion /* r29 */, struct xScene * sc /* r28 */, float dt /* f29 */, struct xEntFrame * frame /* r30 */) {
    // Local variables
    struct xEntMPData * mp; // r31
    float newdist; // f28
    struct xMovePoint * prev; // r27
    float qdot; // f31
    struct xVec3 tgt; // r1+0x60
    struct xVec3 direction; // r1+0x54
    struct xVec3 bank; // r1+0x48
    struct xQuat quat; // r1+0x38
    struct xQuat qold; // r1+0x28
    struct xMat3x3 tmpmat; // r1+0x70
    struct xVec3 scale; // r1+0x1C
    float u; // f30
    struct xVec3 gravity; // r1+0x10
}

struct xVec3 g_O3; // size: 0xC, address: 0x80D6D198
// Range: 0x800665E8 -> 0x80066894
static void xEntSplineMove(struct xEntMotion * motion /* r29 */, float dt /* f31 */, struct xEntFrame * frame /* r30 */) {
    // Local variables
    struct xEntSplineData * spline_data; // r31
    unsigned char still_on_spline; // r28
    struct xVec3 target_position; // r1+0x5C
    struct xVec3 new_position; // r1+0x50
    struct xVec3 scale; // r1+0x44
    struct xVec3 dir; // r1+0x38
    struct xMat3x3 mat; // r1+0x70
    struct xQuat target_orient; // r1+0x28

    // References
    // -> struct xVec3 g_O3;
}

// Range: 0x80066894 -> 0x80066A4C
static void xEntPenMove(struct xEntMotion * motion /* r31 */, float dt /* f24 */, struct xEntFrame * frame /* r30 */) {
    // Local variables
    struct xEntPenData * pen; // r28
    struct xEntMotionPenData * aspen; // r29
    float th; // f30
    float tf; // f28
    float u; // f27
    float v; // f26
    ...
    float newt; // f31
    float rem; // f29
    struct xMat4x3 delta_mat; // r1+0x10
}

// Range: 0x80066A4C -> 0x80066EDC
static void xEntMechMove(struct xEntMotion * motion /* r29 */, struct xScene * sc /* r26 */, float dt /* f31 */, struct xEntFrame * frame /* r27 */) {
    // Local variables
    struct xEntMechData * mech; // r31
    struct xEntMotionMechData * mkasst; // r30
    unsigned int last; // r28
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5F4DD
// Range: 0x80066EDC -> 0x80067230
static unsigned int xEntScale(struct xEntMotion * motion /* r29 */, float dt /* r1+0x8 */, struct xEntFrame * frame /* r30 */) {
    // Local variables
    struct xEntMechData * mech; // r27
    struct xModelInstance * model; // r31
    struct xEntMotionMechData * mkasst; // r28
    unsigned int last; // r26
    float scale; // f29
    float u; // f30
    float scale_adjust; // f31

    // References
    // -> static char __FUNCTION__[10];
}

// Range: 0x80067230 -> 0x80067634
static unsigned int xEntSldMove(struct xEntMotion * motion /* r29 */, float dt /* f28 */, struct xEntFrame * frame /* r28 */) {
    // Local variables
    struct xEntMechData * mech; // r30
    struct xEntMotionMechData * mkasst; // r31
    unsigned int last; // r27
    float tmradj; // f30
    float rem; // f29
    float speed; // f31
}

// Range: 0x80067634 -> 0x80067898
static unsigned int xEntRotMove(struct xEntMotion * motion /* r28 */, float dt /* f28 */, struct xEntFrame * frame /* r29 */) {
    // Local variables
    struct xEntMechData * mech; // r31
    struct xEntMotionMechData * mkasst; // r30
    unsigned int last; // r25
    float tmradj; // f31
    float rem; // f29
    float speed; // f30
    struct xEnt * ownr; // r26
    struct xEntAsset * easst; // r27
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5F4E7
// Range: 0x80067898 -> 0x80067B7C
void xEntMechForward(struct xEntMotion * motion /* r30 */) {
    // Local variables
    struct xEntMechData * mech; // r31
    struct xEntMotionMechData * mkasst; // r29

    // References
    // -> static char __FUNCTION__[16];
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5F4F7
// Range: 0x80067B7C -> 0x80067EC4
void xEntMechReverse(struct xEntMotion * motion /* r29 */) {
    // Local variables
    struct xEntMechData * mech; // r31
    struct xEntMotionMechData * mkasst; // r30
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D5F507
// Range: 0x80067EC4 -> 0x8006816C
void xEntMotionTranslate(struct xEntMotion * motion /* r31 */, const struct xVec3 * dpos /* r29 */, struct xMat4x3 * dmat /* r30 */) {
    // References
    // -> static char __FUNCTION__[20];
}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80D5F51B
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x8006816C -> 0x80068254
void xEntMotionDebugInit(unsigned short num_xems /* r31 */) {
    // References
    // -> static signed short ent_motion_dbg_idx;
    // -> static unsigned short ent_motion_dbg_num_allocd;
    // -> static char __FUNCTION__[20];
    // -> static struct xEntMotion * * dbg_xems;
    // -> unsigned int gActiveHeap;
    // -> static unsigned short ent_motion_dbg_num;
}

// Range: 0x80068254 -> 0x80068278
...
// Range: 0x80068278 -> 0x800682A4
static void xEntMotionDebugAdd(struct xEntMotion * xem /* r3 */) {
    // References
    // -> static unsigned short ent_motion_dbg_num;
    // -> static struct xEntMotion * * dbg_xems;
    // -> static unsigned short ent_motion_dbg_num_allocd;
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5F52F
// Range: 0x800682A4 -> 0x800683CC
static void xEntMotionDebugCB() {
    // Local variables
    struct xEntMotion * xem; // r31
...
// Range: 0x800683CC -> 0x80068FE0
static void xEntMotionDebugWrite(const struct xEntMotion * xem /* r31 */) {
    // Local variables
    char * gps; // r30
...
// Range: 0x80068FE0 -> 0x800692F4
static void xEntMotionDebugDraw(const struct xEntMotion * xem /* r31 */) {
    // Local variables
    struct xMovePoint * xmp; // r30
    unsigned short idx; // r28
    struct xMovePoint * omp; // r29
    unsigned short jdx; // r27
    struct xMovePoint * pmp; // r26
...
// Range: 0x800692F4 -> 0x80069400
static void xEntMotionDebugIPad(struct xEntMotion * xem /* r31 */) {
    // References
    // -> struct zGlobals globals;
    // -> static unsigned short ent_motion_dbg_num;
    // -> static signed short ent_motion_dbg_idx;
...
// total size: 0x10
struct /* @class$1373xEntMotion_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
