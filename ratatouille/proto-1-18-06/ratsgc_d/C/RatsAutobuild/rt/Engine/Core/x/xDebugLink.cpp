/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xDebugLink.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8028483C -> 0x802852E0
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
char mBuffer[2048]; // size: 0x800, address: 0x80D264E8
// total size: 0x30
struct {} xDebugLink::__vtable; // size: 0x30, address: 0x80525140
// total size: 0x804
struct Command {
    // Members
...
class xDebugLink {
    ...
    enum eError UpdateSceneObject(char * receivedBuffer);

    enum eError Connect(char * * argv, int argc);

    enum eError Disconnect(char * * argv, int argc);

    char * GetDevkitName() const;

    char * GetConnectedComputerName() const;
...
// Range: 0x8028483C -> 0x802848B4
// this: r31
xDebugLink::xDebugLink() {
    // References
    // -> struct [anonymous] xDebugLink::__vtable;
}
...
// total size: 0x4
struct /* @class$918xDebugLink_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1082xDebugLink_cpp */ {
    // Members
...
// Range: 0x802848B4 -> 0x80284C78
// this: r31
enum eError xDebugLink::UpdateSceneObject(char * receivedBuffer /* r1+0x8 */) {
    // Local variables
    struct xBase * sceneObject; // r27
    char * asset; // r29
    unsigned int oldAssetSize; // r1+0x30
    char * oldAsset; // r26
    char * tempAllocation; // r25
    struct st_PACKER_ATOC_NODE * assetNode; // r30
    const struct xBaseAsset * baseAsset; // r21
    int numberOfCameraCurvesInScene; // r1+0x2C
    int count; // r28
    struct zCameraCurveAsset * cameraCurveAsset; // r22
    unsigned int i; // r24
    struct zEntTrigger & trigger; // r20
    const struct xTriggerAsset & triggerAsset; // r23
...
// total size: 0xC
struct /* @class$1354xDebugLink_cpp */ {
    // Members
...
// Range: 0x80284C78 -> 0x80284DA8
enum eError xDebugLink::SendEntEvent(char * * argv /* r31 */, int argc /* r1+0x8 */) {
    // Local variables
    float params[4]; // r1+0x18
    int i; // r30
    unsigned int event; // r1+0x14
    unsigned int widgetID; // r1+0x10
    unsigned int paramWidgetID; // r1+0xC
    struct xBase * sceneObject; // r29
}

// Range: 0x80284DA8 -> 0x80284E14
// this: r29
enum eError xDebugLink::Connect(char * * argv /* r30 */, int argc /* r1+0x8 */) {}

// Range: 0x80284E14 -> 0x80284E84
// this: r31
enum eError xDebugLink::Disconnect(char * * argv /* r1+0x8 */, int argc /* r1+0xC */) {}

// Range: 0x80284E84 -> 0x80284FDC
int xDebugLink::CmdToArgv(char * str /* r1+0x8 */, char * * argv /* r29 */, int maxargs /* r1+0xC */) {
    // Local variables
    int argc; // r28
    int argcT; // r30
    char * strNil; // r23
...
// Range: 0x80284FDC -> 0x80284FE8
// this: r3
char * xDebugLink::GetDevkitName() const {}

// Range: 0x80284FE8 -> 0x80284FF4
// this: r3
char * xDebugLink::GetConnectedComputerName() const {}

// Range: 0x80284FF4 -> 0x80285000
// this: r3
unsigned char xDebugLink::IsConnected() const {}

// Range: 0x80285000 -> 0x802852E0
// this: r31
void xDebugLink::Update() {
    // Local variables
    char command[12]; // r1+0x8
    char responseString[256]; // r1+0x13C
    char * argv[10]; // r1+0x14
    char args[256]; // r1+0x3C
    int i; // r30
    int argc; // r28
    unsigned char knownCommand; // r27
    enum eError result; // r29
    unsigned int cmdPrefixLen; // r26
