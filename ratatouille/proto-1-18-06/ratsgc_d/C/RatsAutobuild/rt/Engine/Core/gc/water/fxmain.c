/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\gc\water\fxmain.c
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80238D48 -> 0x80239058
*/
// total size: 0x8
struct RwObject {
    // Members
...
static struct FxParam * (* EffectOpenFunctions[1])(); // size: 0x4, address: 0x80D64E68
static char * EffectNames[1]; // size: 0x4, address: 0x80D6B038
static struct FxParam * Effects[1]; // size: 0x4, address: 0x80D6B03C
static int EffectIndex; // size: 0x4, address: 0x80D6B040
// total size: 0x0
...
// Range: 0x80238D48 -> 0x80238E54
static struct RpMaterial * MaterialSetupEffect(struct RpMaterial * material /* r27 */, void * data /* r1+0x8 */) {
    // Local variables
    struct RpMaterial * srcMaterial; // r26
    struct RpMultiTexture * srcMT; // r29
    struct RpMultiTexture * dstMT; // r31
    unsigned int numTextures; // r28
    unsigned int i; // r30
    struct RpMaterial * (* preRender)(struct RpMaterial *, void *, struct RxGameCubePipeData *, void *); // r1+0x14
    struct RpMaterial * (* postRender)(struct RpMaterial *, void *, struct RxGameCubePipeData *, void *); // r1+0x10
    void * cbData; // r1+0xC
}
...
// Range: 0x80238E54 -> 0x80238EE8
struct RpAtomic * AtomicSetupEffect(struct RpAtomic * atomic /* r31 */, void * data /* r1+0x8 */) {
    // Local variables
...
int FxOpen() {
    ...
    // -> static struct FxParam * Effects[1];
    // -> static char * EffectNames[1];
    // -> struct FxParam * iWaterEMBM;
    // -> static struct FxParam * (* EffectOpenFunctions[1])();
    // -> struct RwFrame * FxFrameArray[2];
    // -> struct RpLight * iWaterDirectionalLight;
}

// Range: 0x80238FEC -> 0x80239058
void FxClose() {
    // Local variables
    int i; // r31
