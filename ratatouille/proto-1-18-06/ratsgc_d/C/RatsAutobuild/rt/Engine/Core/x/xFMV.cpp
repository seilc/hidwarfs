/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xFMV.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8006B88C -> 0x8006BD84
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static float sFMVVolumeScale; // size: 0x4, address: 0x80D5F610
static void (* sxFMVPauseSoundCB)(unsigned char); // size: 0x4, address: 0x80D5F614
// Range: 0x8006B88C -> 0x8006B8D0
void xFMVPauseSound(unsigned char doPause /* r31 */) {
    // References
    // -> static void (* sxFMVPauseSoundCB)(unsigned char);
}
...
// Range: 0x8006B8D0 -> 0x8006B98C
static void DefaultFMVPauseSoundCB(unsigned char doPause /* r31 */) {
    // References
    // -> struct zGlobals globals;
}

// Range: 0x8006B98C -> 0x8006B9D4
int xFMVGetBinkCompliantVolume() {
    // Local variables
    int volume; // r31

    // References
    // -> static float sFMVVolumeScale;
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5F618
// total size: 0x10
struct BINKRECT {
    // Members
...
// Range: 0x8006B9D4 -> 0x8006BD84
struct RwTexture * xFMVGetTexture(struct MOVIE * pMovie /* r31 */, unsigned short looping /* r1+0x8 */, unsigned short freezeframed /* r21 */, unsigned int startframe /* r26 */, unsigned int endframe /* r29 */) {
    // Local variables
    int frameSkipped; // r25
    struct RwRaster * pBinkRaster; // r30
    unsigned char * pBits; // r28
    int format; // r24
    int pixformat; // r1+0xC
    int nDestPitch; // r23
    int bFailed; // r27
    struct RwRaster * pNullOnErr; // r22
