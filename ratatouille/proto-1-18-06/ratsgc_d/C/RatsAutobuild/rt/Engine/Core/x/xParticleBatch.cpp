/*
    ...
    Language: C++
    Code range: 0x801D789C -> 0x801DA7A8
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct internal_system systems[128]; // size: 0x2400, address: 0x80D0DB88
static struct system_bucket buckets[6][128]; // size: 0x3C00, address: 0x80D0FF88
static struct block_info * head_free_block; // size: 0x4, address: 0x80D6AC28
static int free_block_count; // size: 0x4, address: 0x80D6AC2C
static unsigned char * block_buffer; // size: 0x4, address: 0x80D6AC30
static struct block_info block_info_buffer[32]; // size: 0x180, address: 0x80D13B88
static int systems_size; // size: 0x4, address: 0x80D6AC34
static int buckets_size[6]; // size: 0x18, address: 0x80D13D08
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D63E98
// Range: 0x801D78B4 -> 0x801D795C
static struct block_info * alloc_block() {
    // Local variables
    struct block_info * b; // r31
...
// Range: 0x801D795C -> 0x801D7978
static void free_block(struct block_info * b /* r3 */) {
    // References
    // -> static int free_block_count;
    // -> static struct block_info * head_free_block;
}

// Range: 0x801D7978 -> 0x801D79F8
static void create_system(struct internal_system & isys /* r31 */, const struct xParticleBatchSystem & sys /* r1+0x8 */) {}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D63EA4
// total size: 0x14
...
// Range: 0x801D7A4C -> 0x801D7C3C
static void add_to_bucket(struct internal_system & isys /* r31 */) {
    // Local variables
    int & bsize; // r29
    struct system_bucket * const begin_bucket; // r28
    struct system_bucket * const end_bucket; // r27
    struct system_bucket key; // r1+0xC
    struct system_bucket * bucket; // r30
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63EB2
// Range: 0x801D7C68 -> 0x801D7E3C
static void remove_from_bucket(struct internal_system & isys /* r31 */) {
    // Local variables
    int & bsize; // r28
    struct system_bucket * const begin_bucket; // r25
    struct system_bucket * const end_bucket; // r27
    struct system_bucket key; // r1+0xC
    struct system_bucket * bucket; // r30
    struct internal_system * it; // r29
    struct internal_system * * itp; // r26
...
// Range: 0x801D7E3C -> 0x801D7EB4
static void clear_system(struct internal_system & system /* r31 */) {
    // Local variables
    struct block_info * block; // r30
    struct block_info * next_block; // r29
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D63EC5
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D63ED5
// Range: 0x801D7EB4 -> 0x801D8034
static void merge_blocks(struct internal_system & system /* r29 */) {
    // Local variables
    struct block_info * first; // r27
    struct block_info * second; // r26
    struct block_info * s0; // r31
    struct block_info * s1; // r30
    struct block_info * * ps0; // r25
    struct block_info * b; // r28
    struct block_info * * pb; // r24
    int total; // r23
    unsigned char * tail; // r22

    // References
    // -> static char __FUNCTION__[13];
}

// Range: 0x801D8034 -> 0x801D80D0
static void refresh_blocks(struct internal_system & system /* r31 */) {
    // Local variables
    struct block_info * b; // r30
    struct block_info * * pb; // r29
}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D63EE2
// Range: 0x801D80D0 -> 0x801D84EC
static void validate_system(const struct internal_system & isys /* r31 */) {
    // Local variables
    int blocks_used; // r29
    int units_used; // r28
    const struct block_info * b; // r30

    // References
    // -> static char __FUNCTION__[16];
}

static char __FUNCTION__[25]; // size: 0x19, address: 0x80522570
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x801D84EC -> 0x801D85D0
...
// Range: 0x801D85D0 -> 0x801D85D4
static void adjust_batch_size() {}

// Range: 0x801D85D4 -> 0x801D85D8
void xParticleBatchSceneExit() {}

// Range: 0x801D85D8 -> 0x801D8680
void xParticleBatchReset() {
    // Local variables
    struct internal_system * sys; // r31
    struct internal_system * end_sys; // r27
    struct block_info * b; // r30
    struct block_info * endb; // r29
    unsigned char * mem; // r28
...
// total size: 0x4
struct /* @class$1150xParticleBatch_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1288xParticleBatch_cpp */ {
    // Members
...
union /* @class$819xParticleBatch_cpp */ {
...
// total size: 0x10
struct /* @class$832xParticleBatch_cpp */ {
    // Members
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x80522589
// Range: 0x801D87E0 -> 0x801D88D0
int xParticleBatchAddSystem(const struct xParticleBatchSystem & system /* r1+0x8 */, const char * dbgFile /* r1+0xC */, int dbgLine /* r1+0x10 */) {
    // Local variables
    int system_id; // r30
...
static char __FUNCTION__[28]; // size: 0x1C, address: 0x805225A1
// Range: 0x801D88D0 -> 0x801D89D4
void xParticleBatchReplaceSystem(int system_id /* r29 */, const struct xParticleBatchSystem & system /* r30 */) {
    // Local variables
    struct internal_system & isys; // r31
...
float SCALE_FREE_PERCENT; // size: 0x4, address: 0x80D63EF4
float SCALE_SYSTEM_FREE_PERCENT; // size: 0x4, address: 0x80D63EF8
static char __FUNCTION__[22]; // size: 0x16, address: 0x805225BD
// Range: 0x801D89D4 -> 0x801D8C88
float xParticleBatchRequest(int system_id /* r29 */, float amount /* f30 */, int max_blocks /* r30 */) {
    // Local variables
    float throttledAmount; // f31
    struct internal_system & s; // r31
    float newBlockUsage; // f28
    float systemFreePercent; // f29
    float free_percent; // f27
    float newAmount; // f26
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63EFC
// Range: 0x801D8C88 -> 0x801D8DFC
int xParticleBatchEmit(int system_id /* r27 */, int amount /* r29 */, unsigned char * * mem /* r26 */) {
    // Local variables
    struct internal_system & isys; // r31
    struct block_info * block; // r30
    int remain; // r28
...
// Range: 0x801D8F50 -> 0x801D8F98
// this: r31
void xParticleBatchGroup::create() {}

static char __FUNCTION__[5]; // size: 0x5, address: 0x80D63F0F
static char __FUNCTION__[31]; // size: 0x1F, address: 0x805225D3
// Range: 0x801D8F98 -> 0x801D909C
static int compare_group_systems_function(void * e1 /* r1+0x8 */, void * e2 /* r1+0xC */) {
    // Local variables
    int sysid1; // r31
    int sysid2; // r30
    const struct xParticleBatchSystem & isys1; // r1+0x14
    const struct xParticleBatchSystem & isys2; // r1+0x10
...
// Range: 0x801D909C -> 0x801D91D8
static int compare_systems(const struct xParticleBatchSystem & sys1 /* r3 */, const struct xParticleBatchSystem & sys2 /* r4 */) {}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D63F14
// Range: 0x801D91D8 -> 0x801D9300
// this: r31
int xParticleBatchGroup::create_system(const struct xParticleBatchSystem & system /* r26 */, const char * dbgFile /* r1+0x8 */, int dbgLine /* r1+0xC */) {
    // Local variables
    const unsigned char * end_elements; // r29
    const unsigned char * found; // r30
    int system_id; // r28
...
static struct tweak_callback cb_reload; // size: 0x28, address: 0x804B5E80
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D63F22
...
// Range: 0x801D9300 -> 0x801D9630
// this: r1+0x10
void xParticleBatchGroup::add_tweaks(int * system_id /* r29 */, const char * prefix /* r27 */) {
    // Local variables
    int stack_pointer[4]; // r1+0x1C
    struct xParticleBatchSystem & isys; // r30
    const char * * texture_labels; // r1+0x18
    const unsigned int * texture_ids; // r1+0x14
    unsigned int textures; // r25
...
enum /* @enum$955xParticleBatch_cpp */ {
    FLAG_NOZTEST = 1,
    FLAG_ZWRITE = 2,
    FLAG_ORDERED = 4,
    FLAG_BRIGHT = 8,
...
// Range: 0x801D9630 -> 0x801D9640
// this: r3
static reload_context::reload_context(class xParticleBatchGroup * group /* r4 */, int * system_id /* r5 */, unsigned char privatized /* r6 */) {}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D63F2D
// total size: 0x8
struct substr {
    // Members
...
// total size: 0xC
struct /* @class$768xParticleBatch_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$769xParticleBatch_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$770xParticleBatch_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$771xParticleBatch_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$772xParticleBatch_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$773xParticleBatch_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$774xParticleBatch_cpp */ {
    // Members
...
// Range: 0x801D9640 -> 0x801D9858
void xParticleBatchGroup::on_reload(const struct tweak_info & info /* r27 */) {
    // Local variables
    struct reload_context & context; // r31
    int * system_id; // r30
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x805225F2
// Range: 0x801D9858 -> 0x801D9A74
void xParticleBatchDebugCB() {
    // Local variables
    int numFreeBlocks; // r30
    struct block_info * block; // r29
    float free_percent; // f31
    int i; // r28
    struct internal_system & s; // r31
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63F37
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63F4A
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63F5D
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63F70
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63F83
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D63F96
// total size: 0x1
struct compare_buckets {
    // Functions
    static int operator()(void * e1, void * e2) const;
};
// Range: 0x801D9A74 -> 0x801D9B34
static int compare_buckets::operator()(const struct system_bucket & b1 /* r4 */, const struct system_bucket & b2 /* r5 */) {
    // Local variables
    int ordered1; // r31
    int ordered2; // r30
}
...
// Range: 0x801D9B34 -> 0x801D9BF0
static int compare_group_systems_functor::operator()(const struct xParticleBatchSystem & system /* r1+0x8 */, const unsigned char & system_id /* r31 */) {
    // Local variables
    const struct xParticleBatchSystem & isys; // r30
...
// Range: 0x801D9BF0 -> 0x801D9C38
// this: r1+0x8
static int compare_buckets::operator()(void * e1 /* r30 */, void * e2 /* r31 */) const {
    // Local variables
    const struct system_bucket & b1; // r1+0x10
    const struct system_bucket & b2; // r1+0xC
}
...
// Range: 0x801D9C38 -> 0x801D9E20
static void pre_render_buckets(struct system_bucket * first /* r1+0x8 */, struct system_bucket * last /* r1+0xC */, float dt /* r1+0x10 */) {
    // Local variables
    struct ptank_pool__color_mat pool; // r1+0x14
    struct system_bucket * bucket; // r29
    struct internal_system * system; // r31
    unsigned char started; // r27
    unsigned char lost_any; // r26
    struct block_info * block; // r30
    int newused; // r28
...
// Range: 0x801D9E20 -> 0x801DA008
static void pre_render_buckets(struct system_bucket * first /* r1+0x8 */, struct system_bucket * last /* r1+0xC */, float dt /* r1+0x10 */) {
    // Local variables
    struct ptank_pool__color_mat_uv2 pool; // r1+0x14
    struct system_bucket * bucket; // r29
    struct internal_system * system; // r31
    unsigned char started; // r27
    unsigned char lost_any; // r26
...
// Range: 0x801DA008 -> 0x801DA1F0
static void pre_render_buckets(struct system_bucket * first /* r1+0x8 */, struct system_bucket * last /* r1+0xC */, float dt /* r1+0x10 */) {
    // Local variables
    struct ptank_pool__pos_color_size pool; // r1+0x14
    struct system_bucket * bucket; // r29
    struct internal_system * system; // r31
    unsigned char started; // r27
    unsigned char lost_any; // r26
...
// Range: 0x801DA1F0 -> 0x801DA3D8
static void pre_render_buckets(struct system_bucket * first /* r1+0x8 */, struct system_bucket * last /* r1+0xC */, float dt /* r1+0x10 */) {
    // Local variables
    struct ptank_pool__pos_color_size_uv2 pool; // r1+0x14
    struct system_bucket * bucket; // r29
    struct internal_system * system; // r31
    unsigned char started; // r27
    unsigned char lost_any; // r26
...
// Range: 0x801DA3D8 -> 0x801DA5C0
static void pre_render_buckets(struct system_bucket * first /* r1+0x8 */, struct system_bucket * last /* r1+0xC */, float dt /* r1+0x10 */) {
    // Local variables
    struct ptank_pool__pos_color_size_rot pool; // r1+0x14
    struct system_bucket * bucket; // r29
    struct internal_system * system; // r31
    unsigned char started; // r27
    unsigned char lost_any; // r26
...
// Range: 0x801DA5C0 -> 0x801DA7A8
static void pre_render_buckets(struct system_bucket * first /* r1+0x8 */, struct system_bucket * last /* r1+0xC */, float dt /* r1+0x10 */) {
    // Local variables
    struct ptank_pool__pos_color_size_rot_uv2 pool; // r1+0x14
    struct system_bucket * bucket; // r29
    struct internal_system * system; // r31
    unsigned char started; // r27
    unsigned char lost_any; // r26
