/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xGrid.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8007EA54 -> 0x8008110C
*/
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5F988
// total size: 0x38
struct xGrid {
    // Members
...
// Range: 0x8007EA54 -> 0x8007EB24
void xGridBoundInit(struct xGridBound * gridb /* r31 */, void * data /* r30 */) {
    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5F997
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x8007EB24 -> 0x8007F0C0
void xGridInit(struct xGrid * grid /* r31 */, const struct xBox * bounds /* r30 */, unsigned short nx /* r28 */, unsigned short nz /* r29 */, unsigned char ingrid_id /* r27 */) {
    // Local variables
    float gsizex; // f31
    float gsizez; // f30
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5F9A1
// Range: 0x8007F0C0 -> 0x8007F158
void xGridKill(struct xGrid * grid /* r31 */) {
    // References
    // -> static char __FUNCTION__[10];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D5F9AB
// Range: 0x8007F158 -> 0x8007F2E0
void xGridEmpty(struct xGrid * grid /* r30 */) {
    // Local variables
    struct xGridBound * * head; // r29
    struct xGridBound * curr; // r31
    int x; // r28
    int z; // r27
    struct xGridBound * currnext; // r26
    struct xGridBound * currnext; // r25

    // References
    // -> static char __FUNCTION__[11];
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5F9B6
// ERROR: Failed to emit tag 31600E (GlobalSubroutine)
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D5F9C5
// Range: 0x8007F478 -> 0x8007F59C
static int xGridAdd(struct xGrid * grid /* r31 */, struct xGridBound * gridb /* r1+0x8 */, int x /* r1+0xC */, int z /* r1+0x10 */) {
    // Local variables
    struct xGridBound * * cell; // r30

    // References
    // -> static char __FUNCTION__[9];
}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D5F9CE
// total size: 0x20
struct xLinkAsset {
    // Members
...
// Range: 0x8007F59C -> 0x8007FD74
int xGridAdd(struct xGrid * grid /* r30 */, struct xEnt * ent /* r27 */) {
    // Local variables
    struct xBound * bound; // r31
    struct xVec3 * center; // r26
    float maxr; // f31
    struct xSphere * sph; // r25
    int r; // r24
    struct xBBox * bbox; // r29
    float rx; // f28
    float ry; // f27
    float rz; // f26
    float len2; // f25
    int r; // r23
    struct xBBox * bbox; // r28
    float rx; // f24
    float rz; // f23
    float len2; // f22
    int r; // r22
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D5F9D7
// ERROR: Failed to emit tag 321022 (GlobalSubroutine)
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D5F9E3
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x8007FEFC -> 0x800800DC
void xGridUpdate(struct xGrid * grid /* r27 */, struct xEnt * ent /* r26 */) {
    // Local variables
    int dx; // r1+0xC
    int dz; // r1+0x8
    struct xGridBound * gridb; // r30
    struct xGridBound * cur; // r31
    struct xGridBound * * prev; // r29
    struct xGridBound * * cell; // r28
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D5F9EF
// Range: 0x800800DC -> 0x80080210
static struct xGridBound * * xGridGetCell(struct xGrid * grid /* r30 */, const struct xEnt * ent /* r26 */, int & grx /* r28 */, int & grz /* r29 */) {
    // Local variables
    const struct xBound * bound; // r27
    const struct xVec3 * center; // r31

    // References
    // -> static char __FUNCTION__[13];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D5F9FC
// Range: 0x80080210 -> 0x80080508
void xGridGetCell(struct xGrid * grid /* r31 */, float posx /* r1+0x8 */, float posz /* r1+0xC */, int & grx /* r29 */, int & grz /* r30 */) {
    // Local variables
    float pgridx; // f31
    float pgridz; // f30
...
// Range: 0x80080508 -> 0x8008057C
struct xGridBound * xGridIterFirstCell(struct xGrid * grid /* r29 */, float posx /* r1+0x8 */, float posy /* r1+0xC */, float posz /* r1+0x10 */, int & grx /* r30 */, int & grz /* r31 */, struct xGridIterator & it /* r1+0x14 */) {}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5FA09
// Range: 0x8008057C -> 0x80080920
int xGridEntIsTooBig(struct xGrid * grid /* r27 */, const struct xEnt * ent /* r26 */) {
    // Local variables
    const struct xBound * bound; // r31
    float maxr; // f31
    const struct xSphere * sph; // r28
    const struct xBBox * bbox; // r30
    float rx; // f30
    float ry; // f29
    float rz; // f28
    float len2; // f25
    const struct xBBox * bbox; // r29
    float rx; // f27
    float rz; // f26
    float len2; // f24
...
static int offs[4][3][2]; // size: 0x60, address: 0x804F4E78
static int k; // size: 0x4, address: 0x80D69158
// total size: 0xC
...
// Range: 0x80080920 -> 0x80080C7C
void xGridCheckPosition(struct xGrid * grid /* r31 */, struct xVec3 * pos /* r29 */, struct xQCData * qcd /* r22 */, int (* hitCB)(struct xEnt *, void *) /* r23 */, void * cbdata /* r24 */) {
    // Local variables
...
// Range: 0x80080C7C -> 0x80080CC8
struct xGridBound * xGridIterFirstCell(struct xGridBound * * head /* r3 */, struct xGridIterator & it /* r4 */) {
    // Local variables
    struct xGridBound * curcell; // r31
}

// Range: 0x80080CC8 -> 0x80080D64
struct xGridBound * xGridIterFirstCell(struct xGrid * grid /* r31 */, int grx /* r29 */, int grz /* r30 */, struct xGridIterator & it /* r1+0x8 */) {}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5FA1A
// Range: 0x80080D64 -> 0x80080F34
void xGridIterClose(struct xGridIterator & it /* r30 */) {
    // Local variables
    int & iter_active; // r29
    struct xGridBound * cur; // r31
    struct xGridBound * * prev; // r28

    // References
    // -> static char __FUNCTION__[15];
}

// Range: 0x80080F34 -> 0x80081030
void xGridCheckBoundSetup(struct xBox & box /* r31 */, int & lower_x /* r26 */, int & lower_z /* r27 */, int & upper_x /* r28 */, int & upper_z /* r29 */, struct xGrid & grid /* r30 */, const struct xBound & bound /* r1+0x8 */) {
    // Local variables
    float ex; // f31
    float ez; // f30
}

// Range: 0x80081030 -> 0x8008110C
static void get_grid_index(int & ox /* r3 */, int & oz /* r4 */, const struct xGrid & grid /* r5 */, float x /* f1 */, float z /* f2 */) {
    // Local variables
    int ix; // r31
    int iz; // r30
    int xmax; // r29
