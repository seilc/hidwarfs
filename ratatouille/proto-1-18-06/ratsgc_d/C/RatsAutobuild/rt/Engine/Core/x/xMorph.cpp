/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xMorph.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800A63C0 -> 0x800A6A14
*/
// total size: 0xC
struct RwV3d {
    // Members
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D60048
// total size: 0x8
struct RwObject {
    // Members
...
// Range: 0x800A63C0 -> 0x800A67A8
struct xMorphSeqFile * xMorphSeqSetup(void * data /* r21 */, void * (* FindAssetCB)(unsigned int, char *) /* r1+0x8 */) {
    // Local variables
    int i; // r31
    int j; // r26
    struct RpAtomic * model; // r24
    struct xMorphTargetFile * mphtgt; // r27
    struct xMorphSeqFile * header; // r30
    float * timelist; // r20
    struct xMorphFrame * framelist; // r29
    unsigned int * assetlist; // r28
    char * namelist; // r25
    void * assetPtr; // r22
    unsigned int skipsize; // r23
...
// Range: 0x800A67A8 -> 0x800A69FC
void xMorphRender(struct xMorphSeqFile * seq /* r28 */, struct RwMatrixTag * mat /* r24 */, float time /* f31 */) {
    // Local variables
    float lerp; // f30
    unsigned int tidx; // r29
    unsigned int tcount; // r27
    signed short weight[4]; // r1+0x8
    signed short * wptr; // r26
    float * times; // r30
    struct xMorphFrame * frame; // r31
}

// Range: 0x800A69FC -> 0x800A6A14
float xMorphSeqDuration(struct xMorphSeqFile * seq /* r3 */) {}
