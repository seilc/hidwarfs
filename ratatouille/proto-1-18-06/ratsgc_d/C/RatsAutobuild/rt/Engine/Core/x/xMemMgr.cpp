/*
    ...
    Language: C++
    Code range: 0x8009B004 -> 0x8009EE5C
*/
// total size: 0xC
struct xMemArea {
    // Members
...
void (* sMemBaseNotifyFunc)(); // size: 0x4, address: 0x80D692AC
int gMemReporting; // size: 0x4, address: 0x80D692B0
unsigned int gMemReportingThreshold; // size: 0x4, address: 0x80D5FDF0
// total size: 0x8
struct allocIdentifierData {
    // Members
    int bytesAllocated; // offset 0x0, size 0x4
    unsigned int assetID; // offset 0x4, size 0x4
};
static struct allocIdentifierData allocList[4]; // size: 0x20, address: 0x80B81EBC
// Range: 0x8009B004 -> 0x8009B0A0
static void xMemDebugModeCB() {
    // Local variables
...
static struct tag_xFile DBSSoakLogFile; // size: 0x114, address: 0x80B81EDC
static unsigned char DBSSoakLogOpen; // size: 0x1, address: 0x80D692B4
static int DBSSceneMemFree; // size: 0x4, address: 0x80D5FDF4
static unsigned int DBSLastSceneId; // size: 0x4, address: 0x80D692B8
static int soakcnt; // size: 0x4, address: 0x80D692BC
static signed char init; // size: 0x1, address: 0x80D692C0
static int soakidx; // size: 0x4, address: 0x80D692C4
static signed char init; // size: 0x1, address: 0x80D692C8
char * * soaklevels; // size: 0x4, address: 0x80D6A20C
// Range: 0x8009B0A0 -> 0x8009B2F8
void xMemDebug_SoakLog(unsigned int SceneId /* r28 */) {
    // Local variables
    const char * DBSFileName; // r31
    char soakbuf[128]; // r1+0x88
    char dfname[128]; // r1+0x8
    int tmemfree; // r30
...
// Range: 0x8009B2F8 -> 0x8009B428
void xMem_DB_DispStatSum() {
    // Local variables
    struct xHeapState * sp; // r31
    struct xMemHeap * heap; // r30
...
// Range: 0x8009B428 -> 0x8009B564
void xMemInit() {
    // Local variables
    int i; // r31

    // References
    // -> static struct allocIdentifierData allocList[4];
    // -> unsigned int gActiveHeap;
    // -> struct xMemHeap gxHeap[2];
    // -> struct xMemInfo gMemInfo;
}

// Range: 0x8009B564 -> 0x8009B588
void xMemExit() {}

static int sMemMgrTrckLogDisplayDumpedDelay; // size: 0x4, address: 0x80D692CC
static int sMemMgrUsageTrackerTopLine; // size: 0x4, address: 0x80D692D0
static struct tag_xFile sMemMgrTrckLog; // size: 0x114, address: 0x80B81FF0
static unsigned char sMemMgrTrckLogIsOpen; // size: 0x1, address: 0x80D692D4
static unsigned int sMemMgrGenericMallocTag[8]; // size: 0x20, address: 0x80B82104
static unsigned int sMemMgrUsageTrackerPackerMallocMem[90]; // size: 0x168, address: 0x804F5720
static unsigned int sMemMgrUsageTrackerPackerApproxRWMallocMem[90]; // size: 0x168, address: 0x804F5888
static unsigned int sMemMgrUsageTrackerGenericMallocMem[90]; // size: 0x168, address: 0x804F59F0
static struct xMemMgrUsageTrackerLookup xMemMgrUsageTrackerLookupTable[90]; // size: 0x438, address: 0x804F5B58
struct xMemMgrDeptLookup xMemMgrDeptLookupTable[4]; // size: 0x40, address: 0x804F5F90
static char __FUNCTION__[21]; // size: 0x15, address: 0x804F5FD0
// Range: 0x8009B588 -> 0x8009B694
void xMemMgrSetDeptMaxMem(unsigned int deptID /* r27 */, unsigned int deptMaxMem /* r1+0x8 */) {
    // Local variables
    unsigned int heapID; // r29
    struct xMemHeap * heap; // r30
    struct xHeapState * sp; // r28
    unsigned int i; // r31

    // References
    // -> struct xMemMgrDeptLookup xMemMgrDeptLookupTable[4];
    // -> struct xMemHeap gxHeap[2];
...
enum eMemMgrDeptTag {
    eMemMgrDeptTag_Anim = 0,
    eMemMgrDeptTag_Art = 1,
    eMemMgrDeptTag_Design = 2,
    eMemMgrDeptTag_Prog = 3,
    eMemMgrDeptTag_NumDepts = 4,
};
// total size: 0xC
struct xMemMgrUsageTrackerLookup {
    // Members
    enum eMemMgrTag tagID; // offset 0x0, size 0x4
    enum eMemMgrDeptTag deptID; // offset 0x4, size 0x4
    char * tagNameString; // offset 0x8, size 0x4
};
// total size: 0x10
struct xMemMgrDeptLookup {
    // Members
    enum eMemMgrDeptTag deptID; // offset 0x0, size 0x4
    char * deptNameString; // offset 0x4, size 0x4
    unsigned int maxMem; // offset 0x8, size 0x4
    unsigned int throwAssert; // offset 0xC, size 0x4
};
// Range: 0x8009B694 -> 0x8009B874
void xMemMgrUsageTrackerInit() {
    // Local variables
    int i; // r31
    struct xMemMgrUsageTrackerLookup tagswapstorage; // r1+0x18
    int j; // r30
    struct xMemMgrDeptLookup deptswapstorage; // r1+0x8
    int k; // r29

    // References
    // -> struct xGlobals * xglobals;
    // -> static unsigned int sMemMgrGenericMallocTag[8];
    // -> static struct xMemMgrUsageTrackerLookup xMemMgrUsageTrackerLookupTable[90];
    // -> struct xMemMgrDeptLookup xMemMgrDeptLookupTable[4];
}

// Range: 0x8009B8B4 -> 0x8009B8E8
void xMemMgrUsageTrackerShutdown() {
    // References
    // -> static struct tag_xFile sMemMgrTrckLog;
    // -> static unsigned char sMemMgrTrckLogIsOpen;
}

// Range: 0x8009B8E8 -> 0x8009B90C
void xMemMgrUsageTrackerCB() {}

// Range: 0x8009B90C -> 0x8009B998
static void xMemMgrUsageTrackerGetDeptTotals(unsigned int * totArray /* r30 */) {
    // Local variables
    unsigned int i; // r31
...
// Range: 0x8009B998 -> 0x8009BC10
static void xMemMgrUsageTrackerPrintTotals() {
    // Local variables
    int curline; // r31
    unsigned int deptTotals[4]; // r1+0x8
    int deptit; // r28
    int j; // r30
    signed char k; // r27
    int i; // r29
    ...
    // References
    // -> static int sMemMgrUsageTrackerTopLine;
    // -> static struct xMemMgrUsageTrackerLookup xMemMgrUsageTrackerLookupTable[90];
    // -> struct xMemMgrDeptLookup xMemMgrDeptLookupTable[4];
    // -> static int sMemMgrTrckLogDisplayDumpedDelay;
}

// Range: 0x8009BC10 -> 0x8009BCBC
static void xMemMgrUsageTrackerGetInput() {
    // References
    // -> static int sMemMgrTrckLogDisplayDumpedDelay;
    // -> static int sMemMgrUsageTrackerTopLine;
}

// Range: 0x8009BCBC -> 0x8009BCEC
void xMemMgrUsageTrackerSetDumpFile(char * filename /* r1+0x8 */) {
    // References
    // -> struct xGlobals * xglobals;
}
...
// Range: 0x8009BCEC -> 0x8009C074
void xMemMgrUsageTrackerDumpLog(char * fmt /* r27 */) {
    // Local variables
    char logfilebuf[2048]; // r1+0x84
    struct __va_list_struct marker[1]; // r1+0x78
    char * lfbufloc; // r31
    unsigned int deptTotals[4]; // r1+0x68
    int deptit; // r28
    int j; // r30
    int i; // r29
...
// Range: 0x8009C074 -> 0x8009C0A4
void xMemMgrUsageTrackerCloseLog() {
    // References
    // -> static unsigned char sMemMgrTrckLogIsOpen;
    // -> static struct tag_xFile sMemMgrTrckLog;
}

// Range: 0x8009C0A4 -> 0x8009C0A8
void xMemMgrPackerStaticTally() {}

// Range: 0x8009C0A8 -> 0x8009C0AC
void xMemMgrPackerStaticRemove() {}

// Range: 0x8009C0AC -> 0x8009C0CC
void xMemMgrPackerMallocTally(unsigned int size /* r3 */, unsigned int tag /* r4 */) {
    // References
    // -> static unsigned int sMemMgrUsageTrackerPackerMallocMem[90];
}

// Range: 0x8009C0CC -> 0x8009C0EC
void xMemMgrPackerMallocRemove(unsigned int size /* r3 */, unsigned int tag /* r4 */) {
    // References
    // -> static unsigned int sMemMgrUsageTrackerPackerMallocMem[90];
}

// Range: 0x8009C0EC -> 0x8009C10C
void xMemMgrPackerApproxRWMallocTally(unsigned int size /* r3 */, unsigned int tag /* r4 */) {
    // References
    // -> static unsigned int sMemMgrUsageTrackerPackerApproxRWMallocMem[90];
}

// Range: 0x8009C10C -> 0x8009C12C
void xMemMgrPackerApproxRWMallocRemove(unsigned int size /* r3 */, unsigned int tag /* r4 */) {
    // References
    // -> static unsigned int sMemMgrUsageTrackerPackerApproxRWMallocMem[90];
}

// Range: 0x8009C12C -> 0x8009C14C
void xMemMgrGenericMallocTally(unsigned int size /* r3 */, unsigned int tag /* r4 */) {
    // References
    // -> static unsigned int sMemMgrUsageTrackerGenericMallocMem[90];
}

// Range: 0x8009C14C -> 0x8009C1A4
void xMemMgrGenericMallocRemove(unsigned int size /* r3 */, unsigned int tag /* r4 */) {
    // References
    // -> static unsigned int sMemMgrUsageTrackerGenericMallocMem[90];
}

static char __FUNCTION__[28]; // size: 0x1C, address: 0x804F5FE5
// Range: 0x8009C1A4 -> 0x8009C274
void xMemMgrGenericMallocPushTag(unsigned int tag /* r1+0x8 */) {
    // Local variables
    unsigned char i; // r31
...
// Range: 0x8009C274 -> 0x8009C2F0
unsigned int xMemMgrGenericMallocGetTag() {
    // Local variables
    unsigned char i; // r31
...
static char __FUNCTION__[27]; // size: 0x1B, address: 0x804F6001
// Range: 0x8009C2F0 -> 0x8009C3E4
void xMemMgrGenericMallocPopTag() {
    // Local variables
    unsigned char i; // r31
...
// Range: 0x8009C3E4 -> 0x8009C404
unsigned char xMemMgrGenericMallocVerifyClearTag() {
    // References
    // -> static unsigned int sMemMgrGenericMallocTag[8];
}
...
class xMemMgrUsageTracker {
    ...
    void TrackAllocation(unsigned int size, unsigned int tag);

    unsigned int GetMemUsageByTag(unsigned int tag) const;

    // Members
    unsigned int usageByTag[90]; // offset 0x0, size 0x168
};
// Range: 0x8009C404 -> 0x8009C434
// this: r1+0x8
void xMemMgrUsageTracker::Init() {}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5FDF8
// Range: 0x8009C434 -> 0x8009C514
// this: r30
void xMemMgrUsageTracker::TrackAllocation(unsigned int size /* r1+0x8 */, unsigned int tag /* r31 */) {
    // References
    // -> static struct xMemMgrUsageTrackerLookup xMemMgrUsageTrackerLookupTable[90];
    // -> struct xGlobals * xglobals;
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5FE08
// Range: 0x8009C514 -> 0x8009C5F4
// this: r30
unsigned int xMemMgrUsageTracker::GetMemUsageByTag(unsigned int tag /* r31 */) const {
    // References
    // -> static unsigned int sMemMgrUsageTrackerPackerApproxRWMallocMem[90];
    // -> static unsigned int sMemMgrUsageTrackerPackerMallocMem[90];
    // -> static unsigned int sMemMgrUsageTrackerGenericMallocMem[90];
...
// Range: 0x8009C5F4 -> 0x8009C63C
unsigned int xMemMgrGetTotalMemUsageByTag(unsigned int tag /* r1+0x8 */) {
    // References
    // -> struct xMemHeap gxHeap[2];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D5FE19
// Range: 0x8009C63C -> 0x8009C940
void xMemInitHeap(struct xMemHeap * heap /* r31 */, unsigned int base /* r29 */, unsigned int size /* r27 */, unsigned int flags /* r25 */) {
    // Local variables
    unsigned int old_base; // r26
    int align; // r28
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5FE26
// Range: 0x8009C940 -> 0x8009CC64
static unsigned int xMemGetBlockInfo(struct xMemHeap * heap /* r29 */, unsigned int size /* r23 */, int align /* r24 */, struct xMemBlkInfo * info /* r30 */) {
    // Local variables
    int guard_size; // r27
    int header_size; // r25
    int total; // r28
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5FE37
// Range: 0x8009CC64 -> 0x8009CFD4
void * xMemGrowAlloc(unsigned int heapID /* r23 */, unsigned int size /* r28 */, unsigned int tag /* r1+0x8 */) {
    // Local variables
    int align; // r1+0xC
    unsigned int oldalignsize; // r25
    unsigned int newalignsize; // r26
    struct xMemHeap * heap; // r30
    struct xMemBlock * hdr; // r31
    struct xHeapState * sp; // r29
    void * memptr; // r24
    unsigned int guard; // r27
...
static int been_here; // size: 0x4, address: 0x80D692D8
static unsigned int ignoredAllocations[4]; // size: 0x10, address: 0x80D5FE48
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5FE58
// total size: 0x1C
struct xMemBlkInfo {
    // Members
...
// Range: 0x8009CFD4 -> 0x8009D96C
void * xMemAlloc(unsigned int heapID /* r1+0x8 */, unsigned int size /* r28 */, int align /* r21 */, unsigned int tag /* r1+0xC */, unsigned int assetID /* r22 */, const char * file /* r1+0x10 */, const char * func /* r1+0x14 */, int line /* r1+0x18 */) {
    // Local variables
    struct xMemHeap * heap; // r31
    struct xMemBlock * hdr; // r29
    struct xHeapState * sp; // r30
    struct xMemBlkInfo info; // r1+0x30
    unsigned int i; // r20
    unsigned int total; // r27
    signed short idx; // r19
    struct xMemHeap * other; // r26
    struct xHeapState * op; // r25
    unsigned int collision; // r1+0x2C
    unsigned char tallied; // r18
    int i; // r24
    int i; // r23
    unsigned char ignore; // r1+0x1D
    unsigned int i; // r17
    int a; // r1+0x28

    // References
    // -> static unsigned int ignoredAllocations[4];
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5FE62
// Range: 0x8009D96C -> 0x8009E0F8
static unsigned int xMemCheckHeap(struct xMemHeap * heap /* r31 */, unsigned int dump /* r25 */) {
    // Local variables
    struct xMemBlock * hdr; // r29
    struct xMemBlkInfo info; // r1+0x10
    struct xHeapState * sp; // r30
    unsigned int j; // r28
    unsigned int total; // r1+0xC
    unsigned int align; // r1+0x8
    unsigned int save_curr; // r24
    unsigned int heapID; // r27
...
// Range: 0x8009E0F8 -> 0x8009E174
unsigned int xMemCheck() {
    // Local variables
    unsigned int i; // r31
    struct xMemHeap * heap; // r30
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D5FE70
// Range: 0x8009E174 -> 0x8009E328
int xMemPushBase(unsigned int heapID /* r29 */) {
    // Local variables
    struct xMemHeap * heap; // r31
    struct xHeapState * sp; // r30
...
// Range: 0x8009E380 -> 0x8009E3A4
int xMemPushBase() {
    // References
    // -> unsigned int gActiveHeap;
}

// Range: 0x8009E3A4 -> 0x8009E404
void * xMemAllocFreeable(void * alloced_memory /* r3 */, int asked_for_size /* r4 */, void (* func)(void *) /* r5 */, void * param /* r6 */) {
    // Local variables
    struct xMemFreeLink * link; // r30
    struct xMemHeap * heap; // r31
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D5FE7D
// Range: 0x8009E404 -> 0x8009E5F4
int xMemPopBase(unsigned int heapID /* r26 */, int depth /* r27 */) {
    // Local variables
    struct xMemHeap * heap; // r31
    struct xHeapState * sp; // r1+0x8
    int new_idx; // r30
    int popped; // r28
    struct xMemFreeLink * list; // r29
...
// Range: 0x8009E5F4 -> 0x8009E620
int xMemPopBase(int depth /* r1+0x8 */) {
    // References
    // -> unsigned int gActiveHeap;
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D5FE89
// Range: 0x8009E620 -> 0x8009E6B8
int xMemGetBase(unsigned int heapID /* r31 */) {
    // References
    // -> struct xMemHeap gxHeap[2];
    // -> static char __FUNCTION__[12];
}

// Range: 0x8009E6B8 -> 0x8009E6C0
void xMemRegisterBaseNotifyFunc(void (* func)() /* r3 */) {
    // References
    // -> void (* sMemBaseNotifyFunc)();
}

// Range: 0x8009E6C0 -> 0x8009E6E4
int xMemGetBase() {
    // References
    // -> unsigned int gActiveHeap;
}

// Range: 0x8009E6E4 -> 0x8009E798
static void xMemPoolAddElements(struct xMemPool * pool /* r31 */, void * buffer /* r24 */, unsigned int count /* r25 */) {
    // Local variables
    int i; // r28
    void * curr; // r30
    void (* initCB)(struct xMemPool *, void *); // r29
    unsigned int next; // r27
    unsigned int size; // r26
}

// Range: 0x8009E798 -> 0x8009E844
void xMemPoolSetup(struct xMemPool * pool /* r31 */, void * buffer /* r30 */, unsigned int nextOffset /* r1+0x8 */, unsigned int flags /* r1+0xC */, void (* initCB)(struct xMemPool *, void *) /* r1+0x10 */, unsigned int size /* r1+0x14 */, unsigned int count /* r1+0x18 */, unsigned int numRealloc /* r1+0x1C */) {}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5FE95
// Range: 0x8009E844 -> 0x8009E974
void * xMemPoolAlloc(struct xMemPool * pool /* r31 */, unsigned int memtag /* r1+0x8 */) {
    // Local variables
    void * retval; // r30
    unsigned int next; // r29
    unsigned int flags; // r28
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D5FEA3
// Range: 0x8009E974 -> 0x8009EAD0
void xMemPoolFree(struct xMemPool * pool /* r31 */, void * data /* r27 */) {
    // Local variables
    void * freeList; // r25
    unsigned int next; // r29
    unsigned int flags; // r24
    void * prev; // r26
    void * curr; // r30
    void * freeListCheck; // r28

    // References
    // -> static char __FUNCTION__[13];
}

unsigned int scratch_mem_used; // size: 0x4, address: 0x80D692DC
static unsigned short scratch_stack[128]; // size: 0x100, address: 0x80B82124
static unsigned char scratch_buffer[16384]; // size: 0x4000, address: 0x80B82230
static int scratch_allocations; // size: 0x4, address: 0x80D692E0
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5FEB0
// Range: 0x8009EAD0 -> 0x8009ECA0
void * xMemScratchPush(unsigned int size /* r25 */, unsigned int align /* r29 */) {
    // Local variables
    unsigned char rendering; // r1+0x8
    unsigned char * scratch; // r28
    unsigned int align_mask; // r31
    unsigned char * mem; // r27
    unsigned int used; // r30
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5FEC0
// Range: 0x8009ECA0 -> 0x8009EE5C
void xMemScratchPop(void * mem /* r30 */) {
    // Local variables
    unsigned char * scratch; // r31
    unsigned short last_size; // r29
    unsigned char * last_mem; // r28
