/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xPtankPool.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800C5774 -> 0x800C69A4
*/
static int MAX_PTANKS_COLOR_MAT; // size: 0x4, address: 0x80D60768
static int MAX_PTANKS_COLOR_MAT_UV2; // size: 0x4, address: 0x80D6076C
static int MAX_PTANKS_POS_COLOR_SIZE; // size: 0x4, address: 0x80D60770
static int MAX_PTANKS_POS_COLOR_SIZE_UV2; // size: 0x4, address: 0x80D60774
static int MAX_PTANKS_POS_COLOR_SIZE_ROT; // size: 0x4, address: 0x80D60778
static int MAX_PTANKS_POS_COLOR_SIZE_ROT_UV2; // size: 0x4, address: 0x80D6077C
static int MAX_PTANKS; // size: 0x4, address: 0x80D69650
static unsigned char inited; // size: 0x1, address: 0x80D69654
static unsigned int current_sequence_index; // size: 0x4, address: 0x80D69658
// total size: 0x8
struct RwObject {
    // Members
...
struct ptank_context {
    // Members
    struct ptank_context * next; // offset 0x0, size 0x4
    struct RpAtomic * ptank; // offset 0x4, size 0x4
...
struct group_data {
    // Members
    int max_size; // offset 0x0, size 0x4
    unsigned int create_flags; // offset 0x4, size 0x4
    struct ptank_context * ptanks; // offset 0x8, size 0x4
    struct ptank_context * * buckets; // offset 0xC, size 0x4
    int size; // offset 0x10, size 0x4
    int used; // offset 0x14, size 0x4
    int buckets_used; // offset 0x18, size 0x4
};
static struct group_data groups[6]; // size: 0xA8, address: 0x80511488
static int blocks_used[6]; // size: 0x18, address: 0x80B953E0
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D60780
// Range: 0x800C5774 -> 0x800C58EC
static void sort_buckets(struct group_data & group /* r29 */) {
    // Local variables
    struct ptank_context * it; // r31
    struct ptank_context * endp; // r25
    const struct RwTexture * texture; // r27
    struct ptank_context * * bucket; // r30
    struct ptank_context * * end; // r26
    struct ptank_context * front; // r28
    const struct RwTexture * btexture; // r24
...
// total size: 0x4
struct /* @class$1150xPtankPool_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$1288xPtankPool_cpp */ {
    // Members
...
// total size: 0x10
struct /* @class$832xPtankPool_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x800C58EC -> 0x800C59C4
static void adjust_block_sizes() {
    // References
    // -> static int MAX_PTANKS_COLOR_MAT_UV2;
    // -> static int MAX_PTANKS_COLOR_MAT;
    // -> static int MAX_PTANKS_POS_COLOR_SIZE;
    // -> static int MAX_PTANKS_POS_COLOR_SIZE_UV2;
    // -> static int MAX_PTANKS_POS_COLOR_SIZE_ROT;
    // -> static int MAX_PTANKS_POS_COLOR_SIZE_ROT_UV2;
    // -> static int MAX_PTANKS;
    // -> struct xGlobals * xglobals;
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6078D
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x800C59C4 -> 0x800C5BC4
static void init_groups() {
    // Local variables
    int total; // r28
    struct group_data * it; // r30
    struct group_data * end; // r27
    unsigned long ptanks_size; // r26
    unsigned long buckets_size; // r25
    unsigned char * mem; // r29
    struct group_data * it; // r31
    struct group_data * end; // r24

    // References
    // -> static struct group_data groups[6];
...
// Range: 0x800C5BC4 -> 0x800C5CC0
static struct RpAtomic * create_ptank(unsigned int flags /* r29 */) {
    // Local variables
    struct iAsyncLockRWSentry rw_sentry; // r1+0x8
    unsigned int plat_flags; // r27
    struct RpAtomic * ptank; // r31
    struct RwFrame * frame; // r30
    struct RwTexCoords uv[2]; // r1+0xC
}
...
// Range: 0x800C5CC0 -> 0x800C5D28
static void destroy_ptank(struct RpAtomic * ptank /* r31 */) {
    // Local variables
    struct RwFrame * frame; // r30
}

// Range: 0x800C5D28 -> 0x800C5DEC
static void render_ptank(const struct ptank_context & context /* r31 */, int & render_flags /* r30 */) {
    // Local variables
    int xor_flags; // r29
    int noztest; // r28
    int zwrite; // r27
    unsigned int oldtest; // r26
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D60799
// Range: 0x800C5DEC -> 0x800C5EEC
static int compare_ptanks(void * e1 /* r1+0x8 */, void * e2 /* r1+0xC */) {
    // Local variables
    const struct ptank_context * p1; // r31
    const struct ptank_context * p2; // r30
    const struct RwTexture * tex1; // r29
    const struct RwTexture * tex2; // r28
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80511530
// Range: 0x800C5EEC -> 0x800C60BC
void xPTankPoolSceneEnter() {
    // Local variables
    struct group_data * it; // r31
    struct group_data * end; // r28
    int want; // r29
    int size; // r27
    ...
    // References
    // -> static int blocks_used[6];
    // -> static char __FUNCTION__[21];
    // -> static struct group_data groups[6];
    // -> static unsigned int current_sequence_index;
...
// total size: 0xC
struct /* @class$768xPtankPool_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$769xPtankPool_cpp */ {
    // Members
...
// total size: 0xC
struct /* @class$770xPtankPool_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$771xPtankPool_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$772xPtankPool_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$773xPtankPool_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$774xPtankPool_cpp */ {
    // Members
...
// Range: 0x800C60BC -> 0x800C61D8
static int create_ptanks(struct group_data & group /* r31 */, int want /* r28 */) {
    // Local variables
    int old_group_size; // r27
    struct ptank_context * head; // r29
    struct ptank_context * end; // r26
    struct ptank_context * it; // r30

    // References
    // -> static char __FUNCTION__[14];
}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80D607A7
// Range: 0x800C61D8 -> 0x800C62C0
void xPTankPoolSceneExit() {
    // Local variables
    struct group_data * g; // r31
    struct group_data * endg; // r29
    struct ptank_context * p; // r30
    struct ptank_context * endp; // r28
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D607BB
// Range: 0x800C62C0 -> 0x800C65DC
void xPTankPoolRender() {
    // Local variables
    const char * prefix; // r1+0x10
    int i; // r27
    char name[128]; // r1+0x14
    int i; // r26
    struct ptank_context * ptanks; // r28
    struct ptank_context * ptank; // r29
    struct group_data * g; // r31
    struct group_data * endg; // r24
    struct ptank_context * p; // r30
    struct ptank_context * endp; // r23
    int used; // r1+0xC
    int render_flags; // r1+0x8
    struct ptank_context * end_ptank; // r25
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D607CC
// total size: 0x10
struct render_state {
    // Members
    struct RwTexture * texture; // offset 0x0, size 0x4
...
struct ptank_pool {
    // Functions
    void grab_block(enum ptank_group_type type);
...
// Range: 0x800C65DC -> 0x800C68F8
// this: r31
void ptank_pool::grab_block(enum ptank_group_type type /* r1+0x8 */) {
    // Local variables
    struct group_data & group; // r28
    struct ptank_context * * end; // r25
    struct ptank_context * * it; // r27
    struct ptank_context * & context; // r30
    const struct RwTexture * texture; // r24
    struct ptank_context * * it; // r26
    struct ptank_context * & context; // r29
