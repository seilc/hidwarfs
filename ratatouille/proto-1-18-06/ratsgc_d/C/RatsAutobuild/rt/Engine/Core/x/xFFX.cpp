/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xFFX.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8006AD6C -> 0x8006B824
*/
static unsigned int psize; // size: 0x4, address: 0x80D69050
// total size: 0x20
struct xLinkAsset {
    // Members
...
static struct xFFX * pool; // size: 0x4, address: 0x80D69054
static struct xFFX * alist; // size: 0x4, address: 0x80D69058
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D5F5A0
unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x8006AD6C -> 0x8006AE84
void xFFXPoolInit(unsigned int num_ffx /* r30 */) {
    // Local variables
    unsigned int i; // r31

    // References
    // -> static unsigned int psize;
    // -> static struct xFFX * pool;
    // -> static struct xFFX * alist;
    // -> static char __FUNCTION__[13];
    // -> unsigned int gActiveHeap;
}

// Range: 0x8006AE84 -> 0x8006AEC0
struct xFFX * xFFXAlloc() {
    // Local variables
    struct xFFX * f; // r31
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D5F5AD
// Range: 0x8006AEC0 -> 0x8006AF4C
void xFFXFree(struct xFFX * f /* r31 */) {
    // References
    // -> static struct xFFX * alist;
    // -> static char __FUNCTION__[9];
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5F5B6
// Range: 0x8006AF4C -> 0x8006B05C
signed short xFFXAddEffect(struct xEnt * ent /* r31 */, struct xFFX * f /* r30 */) {
    // References
    // -> static char __FUNCTION__[14];
}

static char __FUNCTION__[24]; // size: 0x18, address: 0x804F3060
// Range: 0x8006B05C -> 0x8006B150
unsigned int xFFXRemoveEffectByFData(struct xEnt * ent /* r30 */, void * fdata /* r1+0x8 */) {
    // Local variables
    struct xFFX * * ffxh; // r31
    struct xFFX * next; // r29

    // References
    // -> static char __FUNCTION__[24];
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D5F5C4
// Range: 0x8006B150 -> 0x8006B23C
static void xFFXApplyOne(struct xFFX * ffx /* r31 */, struct xEnt * ent /* r29 */, struct xScene * sc /* r30 */, float dt /* f31 */) {
    // References
    // -> static char __FUNCTION__[13];
}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5F5D1
// Range: 0x8006B23C -> 0x8006B2E4
void xFFXApply(struct xEnt * ent /* r31 */, struct xScene * sc /* r1+0x8 */, float dt /* r1+0xC */) {
    // References
    // -> static char __FUNCTION__[10];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D5F5DB
// total size: 0x24
...
// Range: 0x8006B2E4 -> 0x8006B524
void xFFXShakeUpdateEnt(struct xEnt * ent /* r30 */, struct xScene * sc /* r1+0x8 */, float dt /* r1+0xC */, void * fdata /* r29 */) {
    // Local variables
...
static unsigned int shake_psize; // size: 0x4, address: 0x80D6905C
static struct xFFXShakeState * shake_pool; // size: 0x4, address: 0x80D69060
static struct xFFXShakeState * shake_alist; // size: 0x4, address: 0x80D69064
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5F5EE
// Range: 0x8006B524 -> 0x8006B63C
void xFFXShakePoolInit(unsigned int num /* r30 */) {
    // Local variables
    unsigned int i; // r31

    // References
    // -> static unsigned int shake_psize;
    // -> static struct xFFXShakeState * shake_pool;
    // -> static struct xFFXShakeState * shake_alist;
    // -> static char __FUNCTION__[18];
    // -> unsigned int gActiveHeap;
}

// Range: 0x8006B63C -> 0x8006B678
struct xFFXShakeState * xFFXShakeAlloc() {
    // Local variables
    struct xFFXShakeState * s; // r31

    // References
    // -> static struct xFFXShakeState * shake_alist;
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5F600
// Range: 0x8006B678 -> 0x8006B704
void xFFXShakeFree(struct xFFXShakeState * s /* r31 */) {
    // References
    // -> static struct xFFXShakeState * shake_alist;
    // -> static char __FUNCTION__[14];
}
...
struct xFFXRotMatchState {
    // Members
    int lgrounded; // offset 0x0, size 0x4
    struct xVec3 lfup; // offset 0x4, size 0xC
    struct xVec3 lfat; // offset 0x10, size 0xC
    struct xVec3 plfat; // offset 0x1C, size 0xC
    ...
    struct xFFXRotMatchState * next; // offset 0x40, size 0x4
};
// total size: 0x30
struct xMat3x3 {
    // Members
...
static unsigned int rot_match_psize; // size: 0x4, address: 0x80D69068
static struct xFFXRotMatchState * rot_match_pool; // size: 0x4, address: 0x80D6906C
static struct xFFXRotMatchState * rot_match_alist; // size: 0x4, address: 0x80D69070
static char __FUNCTION__[21]; // size: 0x15, address: 0x804F3078
// Range: 0x8006B704 -> 0x8006B824
void xFFXRotMatchPoolInit(unsigned int num /* r30 */) {
    // Local variables
    unsigned int i; // r31

    // References
    // -> static unsigned int rot_match_psize;
    // -> static struct xFFXRotMatchState * rot_match_pool;
    // -> static struct xFFXRotMatchState * rot_match_alist;
