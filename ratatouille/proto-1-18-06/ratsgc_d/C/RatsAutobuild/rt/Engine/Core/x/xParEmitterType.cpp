/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xParEmitterType.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800B45C4 -> 0x800B700C
*/
static char __FUNCTION__[21]; // size: 0x15, address: 0x804F6B08
// total size: 0x8
struct xBaseAsset {
    // Members
...
// Range: 0x800B45C4 -> 0x800B46CC
void xParEmitterEmitPoint(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r31 */) {
    // References
...
// Range: 0x800B46CC -> 0x800B4814
static void xParEmitterAngleVariation(struct xPar * p /* r29 */, struct xParEmitterAsset * a /* r31 */) {
    // Local variables
    struct xMat3x3 mat_rot; // r1+0x20
    float ang_debrisCone; // f31
    float ang[3]; // r1+0x10
}

static char __FUNCTION__[26]; // size: 0x1A, address: 0x804F6B1D
// total size: 0x10
struct _xMat2x2 {
    // Members
...
// Range: 0x800B4814 -> 0x800B4A6C
void xParEmitterEmitCircleEdge(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r31 */) {
    // Local variables
    float rot_amount; // f31
    float ct; // f30
    float st; // f29
    struct _xMat2x2 rot_mat; // r1+0x18
    struct xVec2 vec2; // r1+0x10
    struct xVec2 new_vec; // r1+0x8
    float dt_radius; // f28
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x804F6B37
// Range: 0x800B4A6C -> 0x800B4CD0
void xParEmitterEmitCircle(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r31 */) {
    // Local variables
    float rot_amount; // f30
    float ct; // f29
    float st; // f28
    struct _xMat2x2 rot_mat; // r1+0x18
    struct xVec2 vec2; // r1+0x10
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D60338
// Range: 0x800B4CD0 -> 0x800B4E18
void xParEmitterEmitRect(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r31 */) {
    // References
    // -> static char __FUNCTION__[20];
}

static char __FUNCTION__[24]; // size: 0x18, address: 0x804F6B4D
// Range: 0x800B4E18 -> 0x800B4FD4
void xParEmitterEmitRectEdge(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r31 */) {
    // Local variables
    float rand; // f31
    float val; // f30
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D6034C
// total size: 0xC
struct RwV3d {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x800B4FD4 -> 0x800B5280
void xParEmitterEmitLine(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r31 */) {
    // Local variables
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x804F6B65
// Range: 0x800B5280 -> 0x800B5434
void xParEmitterEmitSphere(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r31 */) {
    // Local variables
    float yaw; // f31
    float pitch; // f30
    float sy; // f28
...
static char __FUNCTION__[26]; // size: 0x1A, address: 0x804F6B7B
// Range: 0x800B5434 -> 0x800B5738
void xParEmitterEmitSphereEdge(struct xPar * p /* r31 */, struct xParEmitterAsset * a /* r30 */, int subtype /* r1+0x10 */) {
    // Local variables
    struct xVec3 dirvec; // r1+0x20
    struct xVec3 offset; // r1+0x14
    struct xMat3x3 mat_rot; // r1+0x30
    float mag; // f31
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x804F6B95
// total size: 0x20
struct xLinkAsset {
    // Members
...
// Range: 0x800B5738 -> 0x800B5990
void xParEmitterEmitVolume(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r29 */, class xVolume * vol /* r28 */) {
    // Local variables
    const struct xBound * b; // r31
...
static char __FUNCTION__[22]; // size: 0x16, address: 0x804F6BAB
// total size: 0x50
...
// total size: 0x4
struct /* @class$901xParEmitterType_cpp */ {
    // Members
...
union /* @class$1040xParEmitterType_cpp */ {
...
// total size: 0xC
struct /* @class$1039xParEmitterType_cpp */ {
    // Members
...
// Range: 0x800B5990 -> 0x800B5BCC
void xParEmitterEmitEntity(struct xPar * p /* r23 */, struct xParEmitterAsset * a /* r27 */, struct xEnt * ent /* r29 */) {
    // Local variables
    unsigned int size; // r26
    struct xModelInstance * model; // r30
    struct RpGeometry * geom; // r25
    struct xVec3 loc; // r1+0x8
    unsigned int which; // r28
...
static char __FUNCTION__[27]; // size: 0x1B, address: 0x804F6BC1
// total size: 0x8
struct xParCmdAsset {
    // Members
    unsigned int type; // offset 0x0, size 0x4
...
// Range: 0x800B5BCC -> 0x800B5D38
void xParEmitterEmitOffsetPoint(struct xParEmitter * pe /* r1+0x8 */, struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r29 */, struct xEnt * ent /* r31 */) {
    // Local variables
...
// Range: 0x800B5D38 -> 0x800B5E4C
void xParEmitterEmitVCylEdge(struct xPar * p /* r31 */, struct xParEmitterAsset * a /* r30 */) {
    // Local variables
    float ang; // f31
    struct xVec2 dir; // r1+0x10
    struct xVec2 offset; // r1+0x8
    float deflect; // f30
}

static char __FUNCTION__[27]; // size: 0x1B, address: 0x804F6BDC
// Range: 0x800B5E4C -> 0x800B5F54
void xParEmitterEmitOCircleEdge(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r31 */, float dt /* r1+0x8 */) {
    // References
    // -> static char __FUNCTION__[27];
}

// Range: 0x800B5F54 -> 0x800B60A4
static void ocircle_emit(struct xPar & p /* r30 */, struct xParEmitterAsset & a /* r31 */, float radius /* f27 */) {
    // Local variables
...
static char __FUNCTION__[23]; // size: 0x17, address: 0x804F6BF7
// Range: 0x800B60A4 -> 0x800B61D0
void xParEmitterEmitOCircle(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r31 */, float dt /* r1+0x8 */) {
    // Local variables
    float rr; // f31
...
static struct xMat4x3 buffer_mat; // size: 0x40, address: 0x80B94F80
static char __FUNCTION__[28]; // size: 0x1C, address: 0x804F6C0E
// Range: 0x800B61D0 -> 0x800B62D0
struct xMat4x3 * xParEmitterTransformEntBone(struct xVec3 & loc /* r1+0x8 */, struct xVec3 & vel /* r1+0xC */, const struct xParEmitterAsset & a /* r25 */, const struct xEnt & ent /* r26 */) {
    // Local variables
    const struct xPEEntBone & region; // r31
    const struct xModelInstance & model; // r30
    const struct xMat4x3 & root_mat; // r28
    const struct xMat4x3 * mat; // r29
    const struct xMat4x3 & anim_mat; // r27

    // References
    // -> static struct xMat4x3 buffer_mat;
    // -> static char __FUNCTION__[28];
}

// Range: 0x800B62D0 -> 0x800B63EC
static void transform_ent_bone(struct xVec3 & loc /* r30 */, struct xVec3 & vel /* r27 */, const struct xParEmitterAsset & a /* r28 */, const struct xMat4x3 & mat /* r29 */) {
    // Local variables
    const struct xPEEntBone & region; // r31
    struct xVec3 offset; // r1+0x8
}

// Range: 0x800B63EC -> 0x800B642C
void xParEmitterTransformEntBone(struct xVec3 & loc /* r1+0x8 */, struct xVec3 & vel /* r1+0xC */, const struct xParEmitterAsset & a /* r1+0x10 */, const struct xMat4x3 & mat /* r1+0x14 */) {}

// Range: 0x800B642C -> 0x800B64D0
void xParEmitterEmitEntBone(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r29 */, const struct xMat4x3 & mat /* r1+0x8 */) {
    // Local variables
    const struct xPEEntBone & region; // r31
    struct xVec3 offset; // r1+0x18
}

// Range: 0x800B64D0 -> 0x800B69CC
static struct xVec3 get_random_offset(const struct xPEEntBone & region /* r31 */, const struct xMat4x3 & mat /* r30 */) {
    // Local variables
...
// Range: 0x800B69CC -> 0x800B6A9C
void xParEmitterEmitEntBound(struct xPar * p /* r30 */, struct xParEmitterAsset * a /* r29 */, const struct xEnt * ent /* r26 */) {
    // Local variables
    const struct xPEEntBound & region; // r31
    const struct xModelInstance & model; // r28
    const struct xMat4x3 & mat; // r27
    struct xVec3 offset; // r1+0x14
}

// Range: 0x800B6A9C -> 0x800B700C
static struct xVec3 get_random_offset(const struct xBound & bound /* r31 */, float expand /* f30 */, unsigned int vtype /* r1+0x8 */) {
    // Local variables
    struct xVec3 offset; // r1+0x30
    struct xVec3 expandVec; // r1+0x24
    float ang; // f25
    float uz; // f29
    float r; // f24
    float rr; // f31
    ...
    float uz; // f28
    float r; // f22
    unsigned int pick; // r30
    unsigned int side; // r29
    float sign; // f27
    struct xVec3 scale; // r1+0x18
    unsigned int pick; // r28
    unsigned int side; // r27
    float sign; // f26
    struct xVec3 scale; // r1+0xC
}
