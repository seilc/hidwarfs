/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xFX.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80078944 -> 0x8007D150
*/
// total size: 0x8
struct RwObject {
    // Members
...
struct RpAtomic * (* gAtomicRenderCallBack)(struct RpAtomic *); // size: 0x4, address: 0x80D69108
// total size: 0x10
struct RwRGBAReal {
    // Members
...
struct RpLight * MainLight; // size: 0x4, address: 0x80D6910C
unsigned char ReportUVProb; // size: 0x1, address: 0x80D5F8A8
// total size: 0x8
struct fxAtomic {
    // Members
    struct RpAtomic * atomic; // offset 0x0, size 0x4
    float * originalUVs; // offset 0x4, size 0x4
};
static unsigned int num_fx_atomics; // size: 0x4, address: 0x80D69110
// total size: 0x40
struct RwMatrixTag {
    // Members
...
static struct RwFrame * g_matfx_frame; // size: 0x4, address: 0x80D69114
static int sAuraRequests; // size: 0x4, address: 0x80D69118
unsigned int gFXSurfaceFlags; // size: 0x4, address: 0x80D6A180
// Range: 0x80078944 -> 0x8007894C
void xFXDebugSetSurfaceFlags(unsigned int flags /* r3 */) {
    // References
    // -> unsigned int gFXSurfaceFlags;
}

// Range: 0x8007894C -> 0x80078954
unsigned int xFXDebugGetSurfaceFlags() {
    // References
    // -> unsigned int gFXSurfaceFlags;
}

unsigned int xfx_initted; // size: 0x4, address: 0x80D6911C
// Range: 0x80078954 -> 0x80078A0C
void xFXInit() {
    // Local variables
    ...
    // References
    // -> static int sAuraRequests;
    // -> struct RpLight * MainLight;
    // -> static struct RwFrame * g_matfx_frame;
...
struct xFXRing {
    ...
    struct xColor_tag ring_color; // offset 0x30, size 0x4
    ...
    struct xFXRing * * parent; // offset 0x38, size 0x4
    struct RwRaster * rast_ring; // offset 0x3C, size 0x4
};
struct xFXRing ringlist[8]; // size: 0x200, address: 0x80B7C248
// total size: 0x24
...
// Range: 0x80078A0C -> 0x80078A70
void xFXRingInit() {
    // Local variables
    int i; // r30
    struct xFXRing * ringdata; // r31

    // References
    // -> struct xFXRing ringlist[8];
}

// Range: 0x80078A70 -> 0x80078AD8
struct RwRaster * xFXRingFindRWRaster(unsigned int aid_texture /* r29 */) {
    // Local variables
    struct RwTexture * txtr; // r31
    struct RwRaster * rast; // r30
}
...
// Range: 0x80078AD8 -> 0x80078BD0
struct xFXRing * xFXRingCreate(const struct xVec3 * pos /* r27 */, const struct xFXRing * params /* r28 */) {
    // Local variables
    struct xFXRing * ring; // r31
    int i; // r30
...
// Range: 0x80078BD0 -> 0x80078CD0
static void xFXRingUpdate(float dt /* f1 */) {
    // Local variables
    struct xFXRing * ring; // r31
    int i; // r30
    float lifetime; // f30
...
static struct xFXBubbleParams defaultBFX[1]; // size: 0x18, address: 0x804F4D30
// total size: 0x18
...
static struct xFXBubbleParams * BFX; // size: 0x4, address: 0x80D5F8AC
// Range: 0x80078CD0 -> 0x80078CE0
static void xFXReadBubbleParams() {
    // References
    // -> static struct xFXBubbleParams defaultBFX[1];
    // -> static struct xFXBubbleParams * BFX;
}
...
float shiny_factor; // size: 0x4, address: 0x80D5F8B0
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5F8B4
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// Range: 0x80078CE0 -> 0x80079120
void xFX_SceneEnter(struct RpWorld * world /* r1+0x8 */) {
    // Local variables
    int i; // r29
    int num; // r18
    const struct xSurface * sp; // r23
    struct zSurfaceProps * pp; // r27
    struct zSurfMatFX * fxp; // r31
    struct RpMaterial * mp; // r28
    struct RwTexture * mptex; // r17
    struct RwTexture * env; // r22
    struct RwTexture * bump; // r21
    struct RwTexture * env; // r25
    struct RwTexture * bump; // r24
    struct xScene * sc; // r26
    struct xEnt * ent; // r30
    struct RpAtomic * (* tmp)(struct RpAtomic *); // r1+0xC

    // References
    // -> static unsigned int num_fx_atomics;
    // -> struct RpAtomic * (* gAtomicRenderCallBack)(struct RpAtomic *);
...
// Range: 0x80079120 -> 0x80079124
void xFX_SceneExit() {}

// Range: 0x80079124 -> 0x80079158
void xFXUpdate(float dt /* f31 */) {}

// Range: 0x80079158 -> 0x80079200
unsigned int xFXEnvMap(struct RpMaterial * mat /* r29 */, unsigned int envmapID /* r30 */, float coeff /* r1+0x8 */) {
    // Local variables
    struct RwTexture * env; // r31
}

// Range: 0x80079200 -> 0x800792A8
unsigned int xFXBumpMap(struct RpMaterial * mat /* r29 */, unsigned int bumpmapID /* r30 */, float coeff /* r1+0x8 */) {
    // Local variables
    struct RwTexture * bump; // r31
}

// Range: 0x800792A8 -> 0x80079330
static void LightResetFrame(struct RpLight * light /* r1+0x8 */) {
    // Local variables
    struct RwFrame * frame; // r31
    struct RwV3d xAxis; // r1+0x18
    struct RwV3d yAxis; // r1+0xC
}

// Range: 0x80079330 -> 0x80079364
static struct RpMaterial * MaterialDisableMatFX(struct RpMaterial * material /* r31 */) {}

// Range: 0x80079364 -> 0x800793C0
struct RpAtomic * AtomicDisableMatFX(struct RpAtomic * atomic /* r30 */) {
    // Local variables
...
// Range: 0x800793C0 -> 0x800794D4
static struct RpMaterial * MaterialSetEnvMap(struct RpMaterial * material /* r31 */, void * data /* r27 */) {
    // Local variables
    struct RwTexture * matTexture; // r28
    char * textureName; // r1+0x8
...
// total size: 0x0
struct RpSkin {};
// Range: 0x800794D4 -> 0x80079578
static struct RpMaterial * MaterialSetBumpMap(struct RpMaterial * material /* r31 */, void * data /* r27 */) {
    // Local variables
    struct RwTexture * matTexture; // r29
    struct RwTexture * texture; // r30
...
// Range: 0x80079578 -> 0x80079670
static struct RpMaterial * MaterialSetBumpEnvMap(struct RpMaterial * material /* r30 */, struct RwTexture * env /* r28 */, float env_coeff /* r1+0x8 */, struct RwTexture * bump /* r29 */, float bump_coeff /* r1+0xC */) {
    // Local variables
    struct RwFrame * frame; // r31
...
static struct RxPipeline * xFXanimUVPipeline; // size: 0x4, address: 0x80D69120
float xFXanimUVRotMat0[2]; // size: 0x8, address: 0x80D5F8C4
float xFXanimUVRotMat1[2]; // size: 0x8, address: 0x80D5F8CC
float xFXanimUVTrans[2]; // size: 0x8, address: 0x80D5F8D4
float xFXanimUVScale[2]; // size: 0x8, address: 0x80D5F8DC
// Range: 0x80079670 -> 0x80079688
void xFXanimUVSetTranslation(const struct xVec3 * trans /* r3 */) {
    // References
    // -> float xFXanimUVTrans[2];
}

// Range: 0x80079688 -> 0x800796A0
void xFXanimUVSetScale(const struct xVec3 * scale /* r3 */) {
    // References
    // -> float xFXanimUVScale[2];
}

// Range: 0x800796A0 -> 0x80079708
void xFXanimUVSetAngle(float angle /* f29 */) {
    // Local variables
    float s; // f31
    float c; // f30

    // References
    // -> float xFXanimUVRotMat1[2];
    // -> float xFXanimUVRotMat0[2];
}

float xFXanimUV2PRotMat0[2]; // size: 0x8, address: 0x80D5F8E4
float xFXanimUV2PRotMat1[2]; // size: 0x8, address: 0x80D5F8EC
float xFXanimUV2PTrans[2]; // size: 0x8, address: 0x80D5F8F4
float xFXanimUV2PScale[2]; // size: 0x8, address: 0x80D5F8FC
struct RwTexture * xFXanimUV2PTexture; // size: 0x4, address: 0x80D69124
// Range: 0x80079708 -> 0x80079720
void xFXanimUV2PSetTranslation(const struct xVec3 * trans /* r3 */) {
    // References
    // -> float xFXanimUV2PTrans[2];
}

// Range: 0x80079720 -> 0x80079738
void xFXanimUV2PSetScale(const struct xVec3 * scale /* r3 */) {
    // References
    // -> float xFXanimUV2PScale[2];
}

// Range: 0x80079738 -> 0x800797A0
void xFXanimUV2PSetAngle(float angle /* f29 */) {
    // Local variables
    float s; // f31
    float c; // f30

    // References
    // -> float xFXanimUV2PRotMat1[2];
    // -> float xFXanimUV2PRotMat0[2];
}

// Range: 0x800797A0 -> 0x800797A8
void xFXanimUV2PSetTexture(struct RwTexture * texture /* r3 */) {
    // References
    // -> struct RwTexture * xFXanimUV2PTexture;
}

static struct RwMatrixTag gFXUVs[2]; // size: 0x80, address: 0x80B7C448
static unsigned char done; // size: 0x1, address: 0x80D69128
static signed char init; // size: 0x1, address: 0x80D69129
static char __FUNCTION__[21]; // size: 0x15, address: 0x804F4D48
// Range: 0x800797A8 -> 0x80079B64
struct RpAtomic * xFXanimUVAtomicSetup(struct RpAtomic * atomic /* r29 */, unsigned char dualPass /* r1+0x8 */) {
    // Local variables
    ...
    float px; // f27
    float py; // f26
    char text[256]; // r1+0x9

    // References
    // -> static struct RwMatrixTag gFXUVs[2];
...
// Range: 0x80079B64 -> 0x80079BB8
int xFXanimUVCreate() {
    // References
    // -> static struct RxPipeline * xFXanimUVPipeline;
}
...
class tier_queue_allocator joint_alloc; // size: 0x20, address: 0x80B7C4C8
struct activity_data * activities[32]; // size: 0x80, address: 0x80B7C4E8
int activities_used; // size: 0x4, address: 0x80D6912C
unsigned char need_sort; // size: 0x1, address: 0x80D69130
// total size: 0x48
...
// total size: 0x38
struct /* @class$2828xFX_cpp */ {
    // Members
    unsigned char active; // offset 0x0, size 0x1
    unsigned char repeat; // offset 0x1, size 0x1
    unsigned char bind_to_player; // offset 0x2, size 0x1
    float time; // offset 0x4, size 0x4
    struct xVec3 center; // offset 0x8, size 0xC
    struct xVec3 offset; // offset 0x14, size 0xC
    float radius_inner; // offset 0x20, size 0x4
    float radius_outer; // offset 0x24, size 0x4
    float freq; // offset 0x28, size 0x4
    float duration; // offset 0x2C, size 0x4
    float orient; // offset 0x30, size 0x4
    float random_orient_increase; // offset 0x34, size 0x4
};
// total size: 0x288
struct debug_info {
    // Members
    char tweak_prefix[128]; // offset 0x0, size 0x80
    char name[32]; // offset 0x80, size 0x20
    struct curve_node curve[32]; // offset 0xA0, size 0x180
    unsigned int texture; // offset 0x220, size 0x4
    int active_joints; // offset 0x224, size 0x4
    struct config_type cfg; // offset 0x228, size 0x20
    void (* tweaked)(struct xFXRibbon &, void *); // offset 0x248, size 0x4
    void * context; // offset 0x24C, size 0x4
    // total size: 0x38
    struct /* @class$2828xFX_cpp */ {
        // Members
        unsigned char active; // offset 0x0, size 0x1
        ...
        float orient; // offset 0x30, size 0x4
        float random_orient_increase; // offset 0x34, size 0x4
    } spiral; // offset 0x250, size 0x38
};
// total size: 0x14
struct xFXRibbon {
    // Functions
    void create(int disableAutomaticDebugTweaks);

    void clear();

    void set_config(const struct config_type * cfg);

    void insert(const struct xVec3 & loc, const struct xVec3 & norm, float scale, float alpha, int flags);
    ...
    void refresh_joint(struct joint_data & joint, const class iterator & it);

    void eval_joint(const struct joint_data & joint, struct xColor_tag & color, float & width);

    void render_strip(struct RxObjSpace3DVertex * verts, class iterator & first, unsigned long size);

    void copy_parameters(struct xFXRibbon * ribbon) const;

    void add_tweaks(const char * debug_name, const char * tweak_prefix, void (* tweaked)(struct xFXRibbon &, void *), void * context);

    void debug_init();

    void debug_update(float dt);

    void debug_refresh_config();
    ...
    unsigned char debug_need_update() const;

    unsigned char debug_tweakhelp_enabled() const;

    void create_test_curve(int size);
...
// Range: 0x80079BB8 -> 0x80079C24
// this: r31
void xFXRibbon::create(int disableAutomaticDebugTweaks /* r1+0x8 */) {}

// Range: 0x80079C24 -> 0x80079C6C
// this: r31
...
static struct curve_node default_curve[2]; // size: 0x18, address: 0x8047CEC0
// Range: 0x80079C6C -> 0x80079CE4
void xFXRibbon::load_default_config(struct config_type & cfg /* r31 */) {
    // References
    // -> static struct curve_node default_curve[2];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D5F904
// Range: 0x80079CE4 -> 0x80079EA8
// this: r30
void xFXRibbon::set_config(const struct config_type * cfg /* r31 */) {
    // References
    // -> unsigned char need_sort;
    // -> static char __FUNCTION__[11];
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80D5F90F
char __FUNCTION__$localstatic1$pop_back__36tier_queue<Q29xFXRibbon10joint_data>Fv[9]; // size: 0x9, address: 0x80D5F916
char __FUNCTION__$localstatic1$push_front__36tier_queue<Q29xFXRibbon10joint_data>FUi[11]; // size: 0xB, address: 0x80D5F91F
char __FUNCTION__$localstatic1$front__36tier_queue<Q29xFXRibbon10joint_data>Fv[6]; // size: 0x6, address: 0x80D5F92A
// Range: 0x80079EA8 -> 0x8007A010
// this: r31
void xFXRibbon::insert(const struct xVec3 & loc /* r1+0x8 */, const struct xVec3 & norm /* r1+0xC */, float scale /* r1+0x10 */, float alpha /* r1+0x14 */, int flags /* r1+0x18 */) {
    // Local variables
    struct joint_data & joint; // r30
...
// Range: 0x8007A010 -> 0x8007A098
// this: r31
void xFXRibbon::activate() {
    // References
    // -> int activities_used;
    // -> struct activity_data * activities[32];
}

// Range: 0x8007A098 -> 0x8007A128
// this: r3
void xFXRibbon::deactivate() {
    // Local variables
    struct activity_data * * move_act_position; // r31
    struct activity_data * move_act; // r29
    struct activity_data * * tail_act_position; // r30
    struct activity_data * tail_act; // r28
...
static char __FUNCTION__[7]; // size: 0x7, address: 0x80D5F930
char __FUNCTION__$localstatic1$back__36tier_queue<Q29xFXRibbon10joint_data>Fv[5]; // size: 0x5, address: 0x80D5F937
// Range: 0x8007A128 -> 0x8007A30C
// this: r31
void xFXRibbon::update(float dt /* f31 */) {
    // Local variables
    struct joint_data & joint; // r30
    unsigned int age; // r28
    const struct joint_data & joint2; // r29
    unsigned int age2; // r27

    // References
    // -> static char __FUNCTION__[7];
}

static char __FUNCTION__[7]; // size: 0x7, address: 0x80D5F93C
char __FUNCTION__$localstatic1$__vc__36tier_queue<Q29xFXRibbon10joint_data>Fi[5]; // size: 0x5, address: 0x80D5F943
// Range: 0x8007A30C -> 0x8007A4B4
// this: r31
void xFXRibbon::render() {
    // Local variables
    struct RxObjSpace3DVertex * verts; // r25
    int size; // r24
    int it; // r30
    int subsize; // r29
    int next_it; // r27
...
// Range: 0x8007A4B4 -> 0x8007A4E0
struct RwRaster * xFXRibbon::get_raster(const char * name /* r1+0x8 */) {}

// Range: 0x8007A4E0 -> 0x8007A550
struct RwRaster * xFXRibbon::get_raster(unsigned int id /* r29 */) {
    // Local variables
    struct RwTexture * texture; // r31
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D5F948
// Range: 0x8007A550 -> 0x8007A874
void xFXRibbon::get_normal(struct xVec3 & norm /* r30 */, const struct xVec3 & dir /* r31 */, float orient /* f26 */) {
    // Local variables
    float a; // f31
    float b; // f30
    float ax; // f29
...
// Range: 0x8007A874 -> 0x8007AA54
// this: r30
void xFXRibbon::refresh_joint(struct joint_data & joint /* r31 */, const class iterator & it /* r29 */) {
    // Local variables
    struct xVec3 * p[2]; // r1+0x30
    struct xVec3 offset; // r1+0x44
    float mag2; // f31
    struct xVec3 dir; // r1+0x38
}

// Range: 0x8007AA54 -> 0x8007AC64
// this: r31
void xFXRibbon::eval_joint(const struct joint_data & joint /* r26 */, struct xColor_tag & color /* r27 */, float & width /* r25 */) {
    // Local variables
    float frac; // f31
    const struct curve_node * curve; // r28
    const struct curve_node & node0; // r30
    const struct curve_node & node1; // r29
    float ifrac; // f29
    float subfrac; // f30
    float alpha; // r1+0x8
}

// Range: 0x8007AC64 -> 0x8007AC74
void lerp(float & v /* r3 */, float frac /* f1 */, float v0 /* f2 */, float v1 /* f3 */) {}

// Range: 0x8007AC74 -> 0x8007ACF4
void lerp(unsigned char & v /* r3 */, float frac /* f1 */, unsigned char v0 /* r4 */, unsigned char v1 /* r5 */) {}
...
// Range: 0x8007ACF4 -> 0x8007B15C
// this: r30
void xFXRibbon::render_strip(struct RxObjSpace3DVertex * verts /* r24 */, class iterator & first /* r27 */, unsigned long size /* r22 */) {
    // Local variables
    struct RxObjSpace3DVertex * v; // r31
    class iterator it; // r1+0x28
    class iterator last; // r1+0x20
    ...
    float ny; // f23
    float nz; // f22
    struct RxObjSpace3DVertex * endv; // r28
    float ulookup[2]; // r1+0x18
    int back; // r26
    float u; // f21
    float du; // f19
    float u; // f31
...
// Range: 0x8007B15C -> 0x8007B168
static void set_vert_uv(struct RxObjSpace3DVertex & vert /* r3 */, float u /* f1 */, float v /* f2 */) {}

// Range: 0x8007B168 -> 0x8007B1D8
static void set_vert(struct RxObjSpace3DVertex & vert /* r3 */, float x /* f1 */, float y /* f2 */, float z /* f3 */, struct xColor_tag & color /* r4 */) {
    // Local variables
    struct RwV3d tmp; // r1+0xC
    struct RwRGBA col; // r1+0x8
}

// Range: 0x8007B1D8 -> 0x8007B220
// this: r30
void xFXRibbon::copy_parameters(struct xFXRibbon * ribbon /* r31 */) const {}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5F953
// Range: 0x8007B220 -> 0x8007B428
int xFXRibbon::compare_ribbons(void * e1 /* r28 */, void * e2 /* r29 */) {
    // Local variables
    const struct xFXRibbon * r1; // r31
    const struct xFXRibbon * r2; // r30

    // References
    // -> static char __FUNCTION__[16];
}

// Range: 0x8007B428 -> 0x8007B4C0
void xFXRibbon::sort_active_ribbons() {
    // Local variables
    struct activity_data * * act; // r31
    struct activity_data * * end_act; // r30
...
static struct raster_pair * raster_map; // size: 0x4, address: 0x80D69134
static int raster_map_size; // size: 0x4, address: 0x80D69138
static struct xFXRibbon dummy_ribbon; // size: 0x14, address: 0x80D6913C
// total size: 0xC
struct curve_node {
    // Members
...
static struct config_type dummy_cfg; // size: 0x20, address: 0x80B7C568
// Range: 0x8007B4C0 -> 0x8007B5F4
void xFXRibbon::debug_scene_enter() {
    // Local variables
    ...
    // References
    // -> static struct xFXRibbon dummy_ribbon;
    // -> static struct config_type dummy_cfg;
...
static struct tweak_callback cb_config; // size: 0x28, address: 0x804F4D60
static signed char init; // size: 0x1, address: 0x80D69150
static struct tweak_callback cb_change_texture; // size: 0x28, address: 0x804F4D88
static signed char init; // size: 0x1, address: 0x80D69151
static struct tweak_callback cb_curve; // size: 0x28, address: 0x804F4DB0
static signed char init; // size: 0x1, address: 0x80D69152
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D5F963
...
// Range: 0x8007B5F4 -> 0x8007BBCC
// this: r31
void xFXRibbon::add_tweaks(const char * debug_name /* r29 */, const char * tweak_prefix /* r28 */, void (* tweaked)(struct xFXRibbon &, void *) /* r1+0x8 */, void * context /* r1+0xC */) {
    // Local variables
...
// Range: 0x8007BBCC -> 0x8007BCC0
// this: r31
void xFXRibbon::debug_init() {}

// Range: 0x8007BCC0 -> 0x8007C010
// this: r31
void xFXRibbon::debug_update(float dt /* f22 */) {
    // Local variables
    int insert_flags; // r29
    float frac; // f30
    float ang; // f29
    float radius; // f26
    struct xVec3 norm; // r1+0x20
    struct xVec3 loc; // r1+0x14
    float old_orient; // f28
    float orient; // f31
    float so; // f25
    float co; // f27
    struct xVec3 onorm; // r1+0x8
}

// Range: 0x8007C010 -> 0x8007C0E4
// this: r31
void xFXRibbon::debug_refresh_config() {
    // Local variables
    struct raster_pair * raster_map_end; // r29
    struct raster_pair * found; // r30
...
// Range: 0x8007C128 -> 0x8007C208
// this: r31
void xFXRibbon::debug_update_curve() {}

static struct tweak_callback cb_spiral; // size: 0x28, address: 0x804F4DD8
static signed char init; // size: 0x1, address: 0x80D69154
static struct tweak_callback cb_activate; // size: 0x28, address: 0x804F4E00
static signed char init; // size: 0x1, address: 0x80D69155
// Range: 0x8007C208 -> 0x8007C63C
// this: r31
void xFXRibbon::add_spiral_tweaks() {
    // Local variables
    char prefix[128]; // r1+0x5D8
    char _buffer[128]; // r1+0x558
    char _buffer[128]; // r1+0x4D8
    char _buffer[128]; // r1+0x458
    char _buffer[128]; // r1+0x3D8
    char _buffer[128]; // r1+0x358
    char _buffer[128]; // r1+0x2D8
    char _buffer[128]; // r1+0x258
    char _buffer[128]; // r1+0x1D8
    char _buffer[128]; // r1+0x158
    char _buffer[128]; // r1+0xD8
    char _buffer[128]; // r1+0x58

    // References
    // -> static struct tweak_callback cb_activate;
    // -> static signed char init;
    // -> static struct tweak_callback cb_spiral;
    // -> static signed char init;
}

static struct tweak_callback cb_node_time; // size: 0x28, address: 0x804F4E28
static signed char init; // size: 0x1, address: 0x80D69156
static struct tweak_callback cb_node_value; // size: 0x28, address: 0x804F4E50
static signed char init; // size: 0x1, address: 0x80D69157
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D5F96E
// Range: 0x8007C63C -> 0x8007C96C
// this: r31
void xFXRibbon::update_curve_tweaks() {
    // Local variables
    char prefix[128]; // r1+0x1D8
    char * tail; // r28
    ...
    // References
    // -> static struct tweak_callback cb_node_value;
...
// Range: 0x8007C96C -> 0x8007C9D0
// this: r3
unsigned char xFXRibbon::debug_need_update() const {}

// Range: 0x8007C9D0 -> 0x8007C9E4
// this: r3
unsigned char xFXRibbon::debug_tweakhelp_enabled() const {}

struct xColor_tag g_WHITE; // size: 0x4, address: 0x80D6C6A4
// Range: 0x8007C9E4 -> 0x8007CB14
// this: r28
void xFXRibbon::create_test_curve(int size /* r29 */) {
    // Local variables
...
// Range: 0x8007CB14 -> 0x8007CB74
// this: r31
void xFXRibbon::move_spiral_to_player() {
    // Local variables
...
// Range: 0x8007CB74 -> 0x8007CBE4
void xFXRibbon::on_start_spiral(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// Range: 0x8007CBE4 -> 0x8007CC3C
void xFXRibbon::on_activate_spiral(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// Range: 0x8007CC3C -> 0x8007CCAC
void xFXRibbon::on_change_curve(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// Range: 0x8007CCAC -> 0x8007CD18
void xFXRibbon::on_change_config(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// Range: 0x8007CD18 -> 0x8007CE5C
void xFXRibbon::on_change_node_time(const struct tweak_info & info /* r28 */) {
    // Local variables
    struct xFXRibbon & owner; // r30
    char * found; // r27
    int i; // r29
    struct curve_node * curve; // r31
    int curve_size; // r26
...
// Range: 0x8007CE5C -> 0x8007CEBC
void xFXRibbon::on_change_node_value(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// total size: 0xC
struct /* @class$800xFX_cpp */ {
    // Members
...
// total size: 0x1
struct /* @class$803xFX_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$804xFX_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$805xFX_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$806xFX_cpp */ {
    // Members
...
// Range: 0x8007CEBC -> 0x8007CF30
void xFXRibbon::on_change_texture(const struct tweak_info & info /* r30 */) {
    // Local variables
    struct xFXRibbon & owner; // r31
}

// Range: 0x8007CF30 -> 0x8007CFD4
void xFXRibbon::scene_enter() {
    ...
    // References
    // -> class tier_queue_allocator joint_alloc;
    // -> struct activity_data * activities[32];
    // -> int activities_used;
}

// Range: 0x8007CFD4 -> 0x8007D040
void xFXRibbon::update_all(float dt /* r1+0x8 */) {
    // Local variables
    struct activity_data * * act; // r31
    struct activity_data * * end_act; // r30
    struct xFXRibbon * ribbon; // r29
...
// Range: 0x8007D040 -> 0x8007D144
void xFXRibbon::render_all() {
    // Local variables
    struct RwRaster * raster; // r27
    unsigned int blend_src; // r29
    unsigned int blend_dst; // r28
    struct activity_data * * act; // r30
    struct activity_data * * end_act; // r26
    struct xFXRibbon * ribbon; // r31
