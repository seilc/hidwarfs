/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xModelBucket.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800A2C14 -> 0x800A5FB0
*/
// total size: 0x8
struct RwObject {
    // Members
...
static struct RpWorld * sBucketDummyWorld; // size: 0x4, address: 0x80D69330
// total size: 0x14
...
static struct RwCamera * sBucketDummyCamera; // size: 0x4, address: 0x80D69334
static int sBucketCurr; // size: 0x4, address: 0x80D69338
static int sBucketCount; // size: 0x4, address: 0x80D6933C
// total size: 0x10
struct RwSphere {
    // Members
...
// total size: 0x4
struct /* @class$884xModelBucket_cpp */ {
    // Members
...
static struct xModelBucket * sBucketList; // size: 0x4, address: 0x80D69340
static int sBucketClipCullCurr; // size: 0x4, address: 0x80D69344
static int sBucketClipCullCount; // size: 0x4, address: 0x80D69348
static struct xModelBucket * * sBucketClipCullList; // size: 0x4, address: 0x80D6934C
static int sAlphaCurr; // size: 0x4, address: 0x80D69350
static int sAlphaStart; // size: 0x4, address: 0x80D69354
static int sAlphaCount; // size: 0x4, address: 0x80D69358
// total size: 0x14
struct xModelAlphaBucket {
    // Members
    struct RpAtomic * Data; // offset 0x0, size 0x4
    struct xModelInstance * MInst; // offset 0x4, size 0x4
    float AlphaFade; // offset 0x8, size 0x4
    float SortValue; // offset 0xC, size 0x4
    unsigned int Layer; // offset 0x10, size 0x4
};
static struct xModelAlphaBucket * sAlphaList; // size: 0x4, address: 0x80D6935C
unsigned char sRenderModels; // size: 0x1, address: 0x80D60010
unsigned char sRenderAlpha; // size: 0x1, address: 0x80D60011
unsigned char sRenderClones; // size: 0x1, address: 0x80D60012
static int sBucketOpaqueRenderStart; // size: 0x4, address: 0x80D69360
// Range: 0x800A2C14 -> 0x800A2CF4
static int compare_opaque_buckets(void * e1 /* r3 */, void * e2 /* r4 */) {
    // Local variables
    const struct xModelBucket & b1; // r31
    const struct xModelBucket & b2; // r30
    int use_layer1; // r29
    int use_layer2; // r28
}

static char __FUNCTION__[27]; // size: 0x1B, address: 0x804F60B8
// total size: 0x30
struct xMat3x3 {
    // Members
...
// total size: 0xC
struct /* @class$1040xModelBucket_cpp */ {
    // Members
...
union /* @class$1041xModelBucket_cpp */ {
...
// total size: 0x10
struct /* @class$1305xModelBucket_cpp */ {
    // Members
    int flags; // offset 0x0, size 0x4
...
// Range: 0x800A2CF4 -> 0x800A2EC0
unsigned int xModelBucket_HackPipeFlags(struct RpAtomic * data /* r29 */, unsigned int pipeFlags /* r31 */, unsigned int aid /* r30 */) {
    // Local variables
    int rc; // r28
...
// Range: 0x800A2EC0 -> 0x800A2EC8
int DBG_SkipCloneHelperComment() {}

// Range: 0x800A2EC8 -> 0x800A2F1C
void xModelBucket_PreCountReset() {
    // References
    // -> static int sAlphaStart;
    // -> static struct xModelAlphaBucket * sAlphaList;
    // -> static int sAlphaCount;
    // -> static int sAlphaCurr;
    // -> static struct xModelBucket * * sBucketClipCullList;
    // -> static int sBucketClipCullCount;
    // -> static int sBucketClipCullCurr;
    // -> static struct xModelBucket * sBucketList;
    // -> static int sBucketCount;
    // -> static int sBucketCurr;
}

// Range: 0x800A2F1C -> 0x800A2FFC
void xModelBucket_PreCountBucket(struct RpAtomic * data /* r29 */, struct xModelPipe & pipe /* r26 */, unsigned int subObjects /* r28 */) {
    // Local variables
    unsigned int pipeFlags; // r27
    int numbuckets; // r31
    int modelcount; // r30
...
static char __FUNCTION__[27]; // size: 0x1B, address: 0x804F60D3
...
// Range: 0x800A2FFC -> 0x800A31F4
void xModelBucket_PreCountAlloc(int maxAlphaModels /* r1+0x8 */) {
    // Local variables
    struct RwBBox bbox; // r1+0xC

    // References
    // -> static struct RwCamera * sBucketDummyCamera;
    // -> static struct RpWorld * sBucketDummyWorld;
    // -> static char __FUNCTION__[27];
    // -> static int sAlphaCount;
...
static struct RpMaterial * s_pMaterial; // size: 0x4, address: 0x80D69364
// Range: 0x800A31F4 -> 0x800A3230
static struct RpMaterial * GrabMaterial(struct RpMaterial * material /* r31 */) {
    // References
    // -> static struct RpMaterial * s_pMaterial;
}

// Range: 0x800A3230 -> 0x800A3238
static struct RpAtomic * GimmeAtomicCallback(struct RpAtomic * atomic /* r3 */, void * tmp /* r4 */) {}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D60013
...
// total size: 0x0
struct RpSkin {};
// Range: 0x800A3238 -> 0x800A3AAC
void FullAtomicDupe(struct RpAtomic * atomic /* r28 */, int count /* r22 */, struct RpAtomic * * output /* r29 */) {
    // Local variables
    int i; // r30
    struct RwStream * stream; // r31
    struct RwMemory rwmemA; // r1+0x24
    struct RpAtomic * tempAtom; // r1+0x8
    unsigned int origFlags; // r21
    unsigned char hasEffects; // r26
    float origShineValue; // f31
    struct RpClump * clump; // r27
    struct RwLinkList backupClump_atomicList; // r1+0x1C
    struct RwLLLink backupAtomic_inClumpLink; // r1+0x14
    int result; // r25
    struct RpClump * tempClump; // r24
    struct RwMemory rwmemB; // r1+0xC
    struct RpClump * outClump; // r20
    struct RpHAnimHierarchy * pHier; // r23
    struct RpSkin * pSkin; // r19
...
// Range: 0x800A3AD4 -> 0x800A3BB8
void xModelBucket_SetMaterialFX(struct RpAtomic * model /* r31 */, struct xModelPipe & pipe /* r28 */) {
    // Local variables
    struct RpGeometry * geom; // r29
    unsigned char skinned; // r26
    int numMaterials; // r25
    int i; // r30
    struct RpMaterial * material; // r27
}

static char __FUNCTION__[26]; // size: 0x1A, address: 0x804F60EE
// Range: 0x800A3BB8 -> 0x800A3F40
void xModelBucket_InsertBucket(struct RpAtomic * data /* r31 */, struct xModelPipe & pipe /* r29 */, unsigned int subObjects /* r28 */) {
    // Local variables
    unsigned int pipeFlags; // r27
    int numbuckets; // r30
    int pipeSetting[2]; // r1+0xC
    struct RpAtomic * extraData; // r1+0x8
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D60022
// Range: 0x800A3F40 -> 0x800A4150
void xModelBucket_Init() {
    // Local variables
    int i; // r29
    struct xModelBucket * bucket; // r30
    struct xModelBucket * * backref; // r31

    // References
    // -> unsigned char sRenderClones;
    // -> unsigned char sRenderAlpha;
    // -> unsigned char sRenderModels;
    // -> static int sBucketCount;
    // -> static struct xModelBucket * * sBucketClipCullList;
    // -> static struct xModelBucket * sBucketList;
    // -> static struct RpWorld * sBucketDummyWorld;
...
// Range: 0x800A4150 -> 0x800A41A8
struct xModelBucket * * xModelBucket_GetBuckets(struct RpAtomic * data /* r3 */) {
    // Local variables
    int i; // r31
...
struct RpAtomic * gxModelBucketProfileModel; // size: 0x4, address: 0x80D69368
unsigned int gxModelBucketProfileCount; // size: 0x4, address: 0x80D6936C
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60034
enum xScreenDump {
    exSD_NONE = 0,
    exSD_NATIVE = 1,
    exSD_MASSIVE = 2,
...
// Range: 0x800A41F0 -> 0x800A4674
void xModelBucket_Add(struct xModelInstance * minst /* r31 */) {
    // Local variables
    struct RpAtomic * model; // r29
    struct xMat4x3 * cammat; // r30
    float camdist2; // f30
    struct xModelBucket * bucket; // r28
    float camdot; // f29
    float alphaFade; // f31
    unsigned char alphaFlags; // r27
    unsigned char alphaLOD; // r25
    unsigned char alphaModel; // r26
    unsigned char alphaProg; // r23
    ...
    // -> unsigned char sRenderModels;
    // -> unsigned int gxModelBucketProfileCount;
    // -> struct RpAtomic * gxModelBucketProfileModel;
}

static char __FUNCTION__[26]; // size: 0x1A, address: 0x804F6108
// Range: 0x800A4674 -> 0x800A4AA0
void xModelBucket_AddAlphaFade(struct xModelInstance * minst /* r31 */, float alphaFade /* f30 */) {
    // Local variables
    struct RpAtomic * model; // r29
    struct xMat4x3 * cammat; // r30
    float camdist2; // f29
...
// Range: 0x800A4AA0 -> 0x800A4AAC
void xModelBucket_RenderOpaqueBegin() {
    // References
    // -> static int sBucketOpaqueRenderStart;
}

static unsigned int check_model; // size: 0x4, address: 0x80D69370
static signed char init; // size: 0x1, address: 0x80D69374
static unsigned int check_ent; // size: 0x4, address: 0x80D69378
static signed char init; // size: 0x1, address: 0x80D6937C
static char __FUNCTION__[26]; // size: 0x1A, address: 0x804F6122
// Range: 0x800A4AAC -> 0x800A4D34
void xModelBucket_RenderOpaque(int maxLayer /* r1+0x8 */) {
    // Local variables
    int i; // r28
    struct xModelBucket * bucket; // r30
    struct RpAtomic * data; // r27
    struct xModelInstance * minst; // r31
    const char * model_name; // r26
    int asdf; // r1+0x10
    int asdf; // r1+0xC
    struct RpAtomic * oldmodel; // r25
    int flags; // r29
...
void xModelBucket_RenderAlphaBegin() {
    ...
    // -> static int sAlphaStart;
    // -> unsigned char sRenderAlpha;
    // -> int xModelAlphaBucketEnabled;
    // -> int xModelBucketEnabled;
}

static int enableValidation; // size: 0x4, address: 0x80D69380
static signed char init; // size: 0x1, address: 0x80D69384
static const struct xModelInstance * lastModel; // size: 0x4, address: 0x80D69388
static signed char init; // size: 0x1, address: 0x80D6938C
static char __FUNCTION__[32]; // size: 0x20, address: 0x804F613C
unsigned char HDR_brightening; // size: 0x1, address: 0x80D6AD78
// total size: 0x28
struct PKRAssetType {
    // Members
...
// Range: 0x800A4DBC -> 0x800A52F0
void ValidateRenderStateExpectations(const struct xModelInstance * model /* r29 */, unsigned int pipeFlags /* r30 */) {
    // Local variables
    int isProblem; // r31
    struct PKRAssetTOCInfo info_curr; // r1+0x28
    enum en_pkrfindmyass quality; // r28
    struct PKRAssetTOCInfo info_last; // r1+0x8
    enum en_pkrfindmyass quality; // r27

    // References
    // -> static const struct xModelInstance * lastModel;
    // -> struct xGlobals * xglobals;
    // -> unsigned char HDR_brightening;
    // -> static char __FUNCTION__[32];
    // -> static signed char init;
    // -> static int enableValidation;
    // -> static signed char init;
}

static unsigned int check_model; // size: 0x4, address: 0x80D69390
static signed char init; // size: 0x1, address: 0x80D69394
static unsigned int check_ent; // size: 0x4, address: 0x80D69398
static signed char init; // size: 0x1, address: 0x80D6939C
static char __FUNCTION__[38]; // size: 0x26, address: 0x804F615C
enum RwFogType {
...
// Range: 0x800A52F0 -> 0x800A59BC
static void xModelBucket_RenderAlphaLayerInternal(int maxLayer /* r1+0x8 */) {
    // Local variables
    unsigned int pipeMask; // r24
    unsigned int curPipeFlags; // r30
    unsigned int lastPipeFlags; // r29
    unsigned char curAlphaDiscard; // r25
    unsigned char lastAlphaDiscard; // r20
    enum RwFogType oldfogtype; // r19
    int layer; // r18
    struct xModelInstance * minst; // r31
    const char * model_name; // r23
    int asdf; // r1+0x14
    int asdf; // r1+0x10
    struct RpAtomic * oldmodel; // r17
    float oldAlpha; // f31
    float fade; // f30
    unsigned int xorPipeFlags; // r28
    unsigned char fadeChanged; // r1+0xC
    enum RwBlendFunction srcBlend; // r27
    enum RwBlendFunction destBlend; // r26
...
// total size: 0x10
struct xModelBadLayer {
    // Members
    unsigned int modelID; // offset 0x0, size 0x4
    unsigned int asserted; // offset 0x4, size 0x4
    int curLayer; // offset 0x8, size 0x4
    int modelLayer; // offset 0xC, size 0x4
};
static struct xModelBadLayer xModelBucket_BadSetModels[16]; // size: 0x100, address: 0x80B865F8
static int currentCountBadSetModels; // size: 0x4, address: 0x80D693A0
// Range: 0x800A59BC -> 0x800A5A5C
static void xModelBucket_CheckModel(unsigned int modelID /* r3 */, int curLayer /* r4 */, int modelLayer /* r5 */) {
    // Local variables
    int i; // r30
    struct xModelBadLayer & model; // r29
    struct xModelBadLayer & tempModel; // r31

    // References
    // -> static int currentCountBadSetModels;
    // -> static struct xModelBadLayer xModelBucket_BadSetModels[16];
}

static char __FUNCTION__[33]; // size: 0x21, address: 0x804F6182
// Range: 0x800A5A5C -> 0x800A5B70
static void xModelBucket_DoGCAssertsInternal() {
    // Local variables
    int i; // r30
    struct xModelBadLayer & model; // r31
...
// Range: 0x800A5B70 -> 0x800A5BD0
static void xModelBucket_ClearModelsArray() {
    // Local variables
    int i; // r30
    struct xModelBadLayer & model; // r31
...
// Range: 0x800A5BD0 -> 0x800A5BF8
void xModelBucket_InitBadSetModelsCheck() {
    // References
    // -> static int currentCountBadSetModels;
}

// Range: 0x800A5BF8 -> 0x800A5C18
void xModelBucket_DoGCAsserts() {}

// Range: 0x800A5C18 -> 0x800A5CE4
void xModelBucket_RenderAlphaLayer(int maxLayer /* r30 */) {
    // Local variables
    int lastLayer; // r29
    int layer; // r31
...
// Range: 0x800A5CE4 -> 0x800A5D64
unsigned char xModelBucket_RenderAny(int maxLayer /* r3 */) {
    // References
    // -> static int sBucketOpaqueRenderStart;
    // -> static struct xModelBucket * sBucketList;
    // -> static int sBucketCount;
...
// Range: 0x800A5DF0 -> 0x800A5EAC
static void xModelBucket_SetBlendAndZModes(int flags /* r29 */) {
    // Local variables
    enum RwBlendFunction srcBlend; // r31
    enum RwBlendFunction destBlend; // r30
}

// Range: 0x800A5EAC -> 0x800A5F38
static void xModelBucket_ResetBlendAndZModes(int flags /* r31 */) {}

// total size: 0x1
class CmpAlphaBucket {};
// Range: 0x800A5F38 -> 0x800A5FB0
static int CmpAlphaBucket::operator()(void * _a /* r4 */, void * _b /* r5 */) {
    // Local variables
    const struct xModelAlphaBucket * a; // r31
    const struct xModelAlphaBucket * b; // r30
}
