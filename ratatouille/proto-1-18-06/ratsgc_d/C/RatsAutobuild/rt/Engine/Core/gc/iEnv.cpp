/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\gc\iEnv.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8010A798 -> 0x8010C3F0
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static int sBeginDrawFX; // size: 0x4, address: 0x80D69A78
...
static struct RpWorld * sPipeWorld; // size: 0x4, address: 0x80D69A80
// total size: 0x14
...
static struct RwCamera * sPipeCamera; // size: 0x4, address: 0x80D69A84
...
static struct RpGameCubeTevOp sTevOpAddLightmap; // size: 0x50, address: 0x80516A60
static struct RpGameCubeTevOp sTevOpModulate; // size: 0x50, address: 0x80516AB0
enum RpGXTexGenType {
    rpGX_TG_MTX3x4 = 0,
    rpGX_TG_MTX2x4 = 1,
    rpGX_TG_BUMP0 = 2,
...
static struct RpGameCubeTexGen sTexGenIdentity; // size: 0x18, address: 0x80516B00
enum RwPlatformID {
    rwID_PCD3D7 = 1,
    rwID_PCOGL = 2,
    rwID_MAC = 3,
...
static struct RpMTEffect * sEffectLightMap; // size: 0x4, address: 0x80D69A88
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D61650
// total size: 0x10
struct RwSphere {
    // Members
...
// Range: 0x8010A798 -> 0x8010AA64
static struct RpAtomic * SetPipelineCB(struct RpAtomic * atomic /* r25 */) {
    // Local variables
    struct RpMaterialList * matList; // r31
    int i; // r30
    unsigned int matEffect; // r23
    struct RpGeometry * geom; // r26
    int vert_size; // r24
    struct RwRGBA * vert_colors; // r29
    int j; // r28
    struct RwTexture * baseTexture; // r22
    struct RwTexture * lightmapTexture; // r21
    struct RpMaterial * material; // r20
    struct RpMultiTexture * multiTexture; // r27
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D6165E
...
// Range: 0x8010AC50 -> 0x8010AECC
void iEnvLoadBegin(struct iEnv * env /* r31 */, int dataType /* r1+0x8 */, int count /* r30 */) {
    // Local variables
    struct RwBBox tmpbbox; // r1+0xC
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D6166C
// Range: 0x8010AECC -> 0x8010B0AC
unsigned char iEnvLoadJSP(struct iEnv * env /* r31 */, unsigned int aid /* r1+0x8 */, void * data /* r28 */, int dataType /* r1+0xC */, int index /* r29 */) {
    // Local variables
    struct xJSPHeader * jsp; // r30

    // References
    // -> static char __FUNCTION__[12];
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80516B18
// Range: 0x8010B0AC -> 0x8010B1A8
void iEnvJSPVisibilityInc(struct iEnv * env /* r31 */, struct xJSPHeader * jsp /* r1+0x8 */) {
    // Local variables
    int i; // r30
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80516B2D
// Range: 0x8010B1A8 -> 0x8010B2A4
void iEnvJSPVisibilityDec(struct iEnv * env /* r31 */, struct xJSPHeader * jsp /* r1+0x8 */) {
    // Local variables
    int i; // r30
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D61678
// Range: 0x8010B2A4 -> 0x8010B398
void iEnvJSPOn(struct iEnv * env /* r31 */, struct xJSPHeader * jsp /* r1+0x8 */) {
    // Local variables
    int i; // r30
...
static char __FUNCTION__[11]; // size: 0xB, address: 0x80D61682
// Range: 0x8010B398 -> 0x8010B48C
void iEnvJSPOff(struct iEnv * env /* r31 */, struct xJSPHeader * jsp /* r1+0x8 */) {
    // Local variables
    int i; // r30
...
// Range: 0x8010B48C -> 0x8010B51C
void iBuildJSPBound(const struct xClumpCollBSPTree * colltree /* r29 */, struct xBox & box /* r28 */) {
    // Local variables
    int j; // r31
    int k; // r30
}

// Range: 0x8010B51C -> 0x8010B588
void iEnvLoadEnd(struct iEnv * env /* r31 */, int dataType /* r1+0x8 */) {
    // References
    // -> static struct RwCamera * sPipeCamera;
    // -> static struct RpWorld * sPipeWorld;
}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D6168D
// Range: 0x8010B588 -> 0x8010B8AC
...
// Range: 0x8010B8AC -> 0x8010B8B0
void iEnvDefaultLighting() {}

// Range: 0x8010B8B0 -> 0x8010B8B4
...
static struct iEnv * lastEnv; // size: 0x4, address: 0x80D69A8C
// Range: 0x8010B8B4 -> 0x8010B8BC
void iEnvSetup(struct iEnv * env /* r3 */) {
    // References
    // -> static struct iEnv * lastEnv;
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D61696
// Range: 0x8010B8BC -> 0x8010B9D4
void iEnvRender(struct iEnv * env /* r31 */, unsigned char alpha /* r1+0x8 */) {
    // References
    // -> static struct iEnv * lastEnv;
    // -> static char __FUNCTION__[11];
}

struct RwCamera * globalCamera; // size: 0x4, address: 0x80D6B1B0
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D616A1
// Range: 0x8010B9D4 -> 0x8010BAA8
void iEnvEndRenderFX(struct iEnv * env /* r31 */) {
    // References
...
static struct iEnv * sCmpEnv; // size: 0x4, address: 0x80D69A90
static int sColorCompare; // size: 0x4, address: 0x80D69A94
...
// Range: 0x8010BAA8 -> 0x8010BD9C
static int CmpMatOrder(void * a /* r1+0x8 */, void * b /* r1+0xC */) {
    // Local variables
    const struct iEnvMatOrder * ael; // r26
    const struct iEnvMatOrder * bel; // r25
    struct xJSPNodeInfo * anod; // r29
    struct xJSPNodeInfo * bnod; // r28
    int decalcmp; // r23
    struct RpMaterial * amat; // r31
    struct RpMaterial * bmat; // r30
    enum RpMatFXMaterialFlags matfx1; // r24
    enum RpMatFXMaterialFlags matfx2; // r22
    int sidecmp; // r21
    char * atex; // r20
    char * btex; // r19
    int namecmp; // r27
    struct RwTexture * dual1; // r18
    struct RwTexture * dual2; // r17
    char * tname1; // r1+0x1C
    char * tname2; // r1+0x18
    unsigned int acol; // r1+0x14
    unsigned int bcol; // r1+0x10

    // References
    // -> static int sColorCompare;
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D616B1
// Range: 0x8010BD9C -> 0x8010C090
static void iEnvMatOrderSetup(struct iEnv * env /* r31 */) {
    // Local variables
    int i; // r30
    int currJSPNode; // r29
    int currMatGroup; // r25
    int nodeIndex; // r27
    struct RwLLLink * cur; // r26
    struct RwLLLink * end; // r23
    struct RpClump * clump; // r24
    struct RpAtomic * apAtom; // r22
    int i; // r28
...
// Range: 0x8010C090 -> 0x8010C324
static void iEnvMatOrderRenderGC(struct iEnv * env /* r29 */, unsigned char alpha /* r1+0x8 */) {
    // Local variables
    int i; // r21
    int backcullon; // r24
    int zbufferon; // r23
    int matOrderCount; // r1+0x14
    struct iEnvMatOrder * currMatOrder; // r31
    int currMatGroup; // r26
    int matOrderEnd; // r17
    struct RpAtomic * apAtom; // r28
    struct xJSPNodeInfo * nodeInfo; // r30
    struct RwFrame * frame; // r1+0x10
    unsigned int flagValue; // r20
    unsigned char alphaDiscard; // r27
    int i; // r22
    struct RwTexture * texture; // r25
    struct RwGameCubeRasterExtension * rasExt; // r19
}

// Range: 0x8010C324 -> 0x8010C398
void iEnvLightPreRender(struct iEnv * env /* r30 */, struct RpLight * light /* r1+0x8 */) {
    // Local variables
    int i; // r31
}

// Range: 0x8010C398 -> 0x8010C3F0
void iEnvLightPostRender(struct iEnv * env /* r30 */) {
    // Local variables
    int i; // r31
}
