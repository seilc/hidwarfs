/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\gc\iCutscene.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801087C8 -> 0x801096A0
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D615C8
// total size: 0x80
...
// Range: 0x801087C8 -> 0x80108A1C
void iCSSoundSetup(struct xCutscene * csn /* r31 */) {
    // Local variables
    unsigned int dataIndex; // r26
    struct xCutsceneData * data; // r30
    unsigned int numData; // r23
    enum xRegion eMyRegion; // r25
    int i; // r29
    unsigned char bLeftFound; // r24
    unsigned int uDefaultLeftTrack; // r27
    unsigned int uDefaultRightTrack; // r1+0x8
    unsigned int uRegion; // r28

    // References
    // -> static char __FUNCTION__[14];
}

// Range: 0x80108A1C -> 0x80108C10
void * iCSSoundGetData(unsigned int assetID /* r1+0x8 */, unsigned int * len /* r24 */) {
    // Local variables
    ...
    unsigned int dataIndex; // r26
    unsigned int numData; // r23
    unsigned int foundChannel; // r25
    int channel; // r29
    int chunk; // r22
    int i; // r27
}

static char __FUNCTION__[15]; // size: 0xF, address: 0x80D615D6
...
// Range: 0x80108C10 -> 0x80108CE0
static void iCSAsyncReadCB(struct tag_xFile * txf /* r31 */) {
    // Local variables
    int bytes; // r1+0x8
    int key; // r29
    enum IFILE_READSECTOR_STATUS cdstat; // r28
    struct xCutscene * xc; // r30

    // References
    // -> static char __FUNCTION__[15];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D615E5
// total size: 0x28
struct PKRAssetType {
    // Members
...
// Range: 0x80108CE0 -> 0x80108E88
unsigned int iCSFileOpen(struct xCutscene * csn /* r31 */) {
    // Local variables
    unsigned int headerskip; // r28
    unsigned int filesize; // r1+0x8
    struct PKRAssetTOCInfo ainfo; // r1+0xC
    const char * filename; // r29
    int uOffsetBytes; // r30

    // References
    // -> static char __FUNCTION__[12];
}

unsigned int read_sizzze; // size: 0x4, address: 0x80D69A40
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D615F1
// Range: 0x80108E88 -> 0x80108FE0
void iCSFileAsyncRead(struct xCutscene * csn /* r29 */, void * dest /* r27 */, unsigned int size /* r31 */) {
    // Local variables
    unsigned int * tp; // r28
    int i; // r30
    ...
    // -> unsigned int read_sizzze;
}

// Range: 0x80108FE0 -> 0x80109024
void iCSFileAsyncSkip(struct xCutscene * csn /* r31 */, unsigned int size /* r1+0x8 */) {}

// Range: 0x80109024 -> 0x80109058
void iCSFileClose(struct xCutscene * csn /* r31 */) {}

static int been_here; // size: 0x4, address: 0x80D69A44
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D61602
...
// Range: 0x80109058 -> 0x801096A0
int iCSLoadStep(struct xCutscene * csn /* r31 */) {
    // Local variables
    unsigned int skipAccum; // r30
    unsigned int tmpSize; // r1+0x8
    void * foundModel; // r25
    struct xSndGroup * pLeftGroup; // r29
    enum iSndGroupHandle leftSoundGroup; // r24
    struct xSndGroup * pRightGroup; // r28
    enum iSndGroupHandle rightSoundGroup; // r23
    struct xSndGroup * pLeftGroup; // r27
    enum iSndGroupHandle leftSoundGroup; // r22
    unsigned int i; // r26
