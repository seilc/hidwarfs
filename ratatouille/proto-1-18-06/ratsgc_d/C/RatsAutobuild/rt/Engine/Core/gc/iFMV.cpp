/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\gc\iFMV.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8010DBB8 -> 0x8010FA6C
*/
// total size: 0x10
struct BINKRECT {
    // Members
...
struct BINKIO {
    ...
    struct BINK * bink; // offset 0x18, size 0x4
    ...
    void (* suspend_callback)(struct BINKIO *); // offset 0xEC, size 0x4
    int (* try_suspend_callback)(struct BINKIO *); // offset 0xF0, size 0x4
    void (* resume_callback)(struct BINKIO *); // offset 0xF4, size 0x4
    void (* idle_on_callback)(struct BINKIO *); // offset 0xF8, size 0x4
...
struct BINK {
    ...
    struct BINKIO bio; // offset 0x110, size 0x13C
    ...
    int rtindex; // offset 0x2C8, size 0x4
    ...
    struct BINKSND * bsnd; // offset 0x308, size 0x4
    ...
    struct BUNDLEPOINTERS bunp; // offset 0x318, size 0x24
...
struct BINK * hBinkCurrMovie; // size: 0x4, address: 0x80D69AA8
static unsigned char sARAMtmp; // size: 0x1, address: 0x80D69AAC
static unsigned int amt; // size: 0x4, address: 0x80D69AB0
...
// Range: 0x8010DBB8 -> 0x8010DCD0
static void * BigMovieAlloc(unsigned int uNumBytes /* r30 */) {
    // Local variables
    void * addr; // r31
    ...
    // -> static unsigned int amt;
    // -> static unsigned char sARAMtmp;
}

// Range: 0x8010DCD0 -> 0x8010DD38
static void BigMovieFree(void * pMemToFree /* r31 */) {
    // References
    // -> void * RwEngineInstance;
}

// Range: 0x8010DD38 -> 0x8010DD6C
unsigned char iFMVSystemInit() {}

static char __FUNCTION__[9]; // size: 0x9, address: 0x80D61768
enum eGameMode {
    eGameMode_Boot = 0,
    eGameMode_Intro = 1,
    eGameMode_Title = 2,
...
// Range: 0x8010DD6C -> 0x8010E08C
static struct BINK * iFMVOpen(const char * fullFile /* r29 */, unsigned int uBinkFlags /* r1+0x8 */) {
    // Local variables
    struct RADARAMCALLBACKS OurGCCallbacks; // r1+0xC
    struct BINK * hBinkMovie; // r31
    int numRetries; // r30
    struct tag_xFile tfile; // r1+0x14
    enum xRegion reg; // r28
...
// Range: 0x8010E08C -> 0x8010E0E8
static void iFMVClose(struct BINK * hMovie /* r1+0x8 */) {
    // References
    // -> static unsigned char sARAMtmp;
}
...
static struct MOVIE * movie_array; // size: 0x4, address: 0x80D69AB4
static int movie_asset_count; // size: 0x4, address: 0x80D69AB8
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D61771
...
// Range: 0x8010E0E8 -> 0x8010E368
void iFMVScenePostInit() {
    // Local variables
    struct PKRAssetTOCInfo temp_asset_info; // r1+0x8
    int i; // r31
    const char * hipname; // r30
    char fullname[255]; // r1+0x28
    int bFileOpenOk; // r29

    // References
    // -> static int movie_asset_count;
    // -> static struct MOVIE * movie_array;
    // -> static char __FUNCTION__[18];
    // -> unsigned int gActiveHeap;
}

// Range: 0x8010E368 -> 0x8010E3C8
void iFMVSceneExit() {
    // Local variables
    int i; // r31
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D61783
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x8010E3C8 -> 0x8010E74C
struct MOVIE * iFMVOpen(struct PKRAssetTOCInfo & rAssetInfo /* r30 */) {
    // Local variables
    struct RADARAMCALLBACKS OurGCCallbacks; // r1+0xC
    struct MOVIE * pMovie; // r1+0x8
    int i; // r31
    int width; // r27
    int height; // r26
    struct RwRaster * pBinkRaster; // r29
    struct RwRaster * pNullOnErr; // r28
...
// Range: 0x8010E74C -> 0x8010E778
int NextPow2(int in /* r3 */) {
    // Local variables
    int ret; // r31
}

// Range: 0x8010E778 -> 0x8010E81C
void iFMVClose(struct MOVIE * & pMovie /* r31 */) {
    // References
    // -> static unsigned char sARAMtmp;
}
...
// Range: 0x8010E81C -> 0x8010E9B0
unsigned int iFMVPlay(const char * filename /* r24 */, unsigned int buttons /* r1+0x8 */, float time /* r1+0xC */, unsigned int uSubtitlesAID /* r1+0x10 */, unsigned char skippable /* r1+0x14 */, unsigned char lockController /* r1+0x15 */, unsigned char displayDemoBlink /* r1+0x16 */) {
    // Local variables
    char fullname[64]; // r1+0x21
    char BIK[5]; // r1+0x1C
    char BIP[5]; // r1+0x17
    const char * extension; // r31
    enum _GameOstrich old; // r27
    enum xRegion localRegion; // r26
    unsigned int ret; // r25
...
void iFMVSuspend() {
    ...
    // -> static int movie_asset_count;
    // -> static struct MOVIE * movie_array;
    // -> struct BINK * hBinkCurrMovie;
}

// Range: 0x8010EA94 -> 0x8010EBA4
void iFMVResume() {
    // Local variables
    int i; // r31
    struct BINK * hBink; // r30
...
static char __FUNCTION__[9]; // size: 0x9, address: 0x80D6178C
// total size: 0x2
...
// Range: 0x8010EBA4 -> 0x8010F944
static unsigned int iFMVPlay(const char * filename /* r1+0x18 */, unsigned int buttons /* r1+0x1C */, unsigned int uSubtitlesAID /* r1+0x20 */, unsigned char skippable /* r1+0x24 */, unsigned char displayDemoBlink /* r1+0x25 */, int uAudioTrack /* r1+0x28 */) {
    // Local variables
    int j; // r23
    char fullFile[256]; // r1+0x98
    unsigned int uBinkFlags; // r22
    struct BINK * hBinkMovie; // r31
    long long delay; // r1+0x80
    long long now; // r1+0x78
    unsigned char bTrackFound; // r1+0x2F
    int track; // r17
    struct RwRaster * pCamRaster; // r1+0x5C
    int iBackBufWidth; // r21
    int iBackBufHeight; // r1+0x58
    int iSafeWidth; // r20
    int iSafeHeight; // r28
    struct RwRect MovieRect; // r1+0x88
    float aspect_ratio_mult; // f30
    int iHorizPad; // r1+0x54
    int iVertPad; // r1+0x50
    struct RwCamera * cam; // r1+0x4C
    struct RwRGBA bg; // r1+0x48
    long long fmvAbortDelay; // r1+0x70
    unsigned char aborted; // r1+0x2E
    long long dwTimeStart; // r1+0x68
    long long dwTimeNow; // r1+0x60
    void * xfbDisp; // r1+0x44
    void * xfbCopy; // r1+0x40
    void * xfbDest; // r26
    unsigned char bUseYUVPath; // r1+0x2D
    struct RwRaster * pBinkRaster; // r30
    struct RwGameCubeRasterExtension * pRasterExt; // r19
    int iBinkMovieHalfHeight; // r27
    unsigned char waiting; // r1+0x2C
    int i; // r25
    struct _tagxPad * pad; // r29
    float time; // f29
    int oneIfSkip; // r1+0x3C
    int offset; // r1+0x38
    void * newDest; // r1+0x34
    struct RwRaster * pNullOnErr; // r24
    unsigned char * pBits; // r18
    float fTime; // f31
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D61795
// Range: 0x8010F944 -> 0x8010F9A0
void iFMVResetCallback() {
    // References
    // -> static char __FUNCTION__[18];
}

// Range: 0x8010F9A0 -> 0x8010FA6C
void iFMVKill() {
    // Local variables
    int i; // r31
