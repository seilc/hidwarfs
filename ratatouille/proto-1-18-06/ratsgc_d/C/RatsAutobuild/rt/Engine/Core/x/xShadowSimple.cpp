/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xShadowSimple.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800E6258 -> 0x800E858C
*/
// total size: 0x20
struct xLinkAsset {
    // Members
...
// total size: 0x4
struct /* @class$889xShadowSimple_cpp */ {
    // Members
...
union /* @class$601xShadowSimple_cpp */ {
...
// total size: 0xC
struct /* @class$1036xShadowSimple_cpp */ {
    // Members
...
struct xShadowSimpleQueue {
    // Members
    struct xShadowSimpleCache * cache; // offset 0x0, size 0x4
    unsigned int priority; // offset 0x4, size 0x4
    const struct xMat4x3 * modelMatrix; // offset 0x8, size 0x4
    int hasMatCopy; // offset 0xC, size 0x4
    float radius; // offset 0x10, size 0x4
    float height; // offset 0x14, size 0x4
    float ecc; // offset 0x18, size 0x4
};
static struct xShadowSimpleQueue sCollQueue[2]; // size: 0x38, address: 0x80B99B00
static struct xMat4x3 sCollModelMatrices[2]; // size: 0x80, address: 0x80B99B40
enum RwShadeMode {
...
static struct RxRenderStateVector xrsv; // size: 0x2C, address: 0x80B99BC0
// total size: 0x24
...
static struct RxObjSpace3DVertex sShadVert[384]; // size: 0x3600, address: 0x80B99BEC
static struct RwRaster * sShadRasters[64]; // size: 0x100, address: 0x80B9D1EC
static struct RwRaster * sShadRaster; // size: 0x4, address: 0x80D698D8
static unsigned int sShadVertCount; // size: 0x4, address: 0x80D698DC
static struct RwMatrixTag * sModelMat; // size: 0x4, address: 0x80D698E0
// total size: 0x28
...
// Range: 0x800E6258 -> 0x800E6350
static struct RpCollisionTriangle * shadowRayCB(struct RpCollisionTriangle * tri /* r31 */, float dist /* f31 */, void * data /* r1+0x8 */) {
    // Local variables
    struct xVec3 xformnorm; // r1+0xC
    struct xVec3 * norm; // r28
    float * testdist; // r29
    struct xShadowSimpleCache * cache; // r30

    // References
    // -> static struct RwMatrixTag * sModelMat;
}

// Range: 0x800E6350 -> 0x800E6394
static struct RpCollisionTriangle * shadowRayModelCB(struct RpIntersection * isx /* r1+0x8 */, struct RpCollisionTriangle * tri /* r1+0xC */, float dist /* r1+0x10 */, void * data /* r1+0x14 */) {}

// Range: 0x800E6394 -> 0x800E64AC
static int shadowRayEntCB(struct xEnt * ent /* r29 */, void * cbdata /* r1+0x8 */) {
    // Local variables
    struct ShadowSimpleCBParam * cbparam; // r31
...
static char __FUNCTION__[27]; // size: 0x1B, address: 0x80513D70
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x800E64AC -> 0x800E6730
static void xShadowSimple_SceneCollide(struct xShadowSimpleCache * cache /* r31 */, const struct xVec3 * pos /* r30 */, float depth /* r1+0x8 */) {
    // Local variables
    struct xEnv * env; // r29
    struct RpIntersection isx; // r1+0x34
    int i; // r28
    struct ShadowSimpleCBParam cbparam; // r1+0xC
...
static char __FUNCTION__[26]; // size: 0x1A, address: 0x80513D8B
// Range: 0x800E6730 -> 0x800E6A84
static void xShadowSimple_CalcCorners(struct xShadowSimpleCache * cache /* r31 */, const struct xMat4x3 * mat /* r30 */, float radius /* f31 */, float ecc /* f30 */) {
    // Local variables
    struct xVec3 tempnorm; // r1+0x20
    struct xVec3 shadowRight; // r1+0x14
    struct xVec3 shadowAt; // r1+0x8
...
// Range: 0x800E6A84 -> 0x800E7160
static void xShadowSimple_AddVerts(struct xShadowSimpleCache * cache /* r3 */) {
    // Local variables
    struct RwV3d tmp; // r1+0x5C
    struct RwV3d tmp; // r1+0x50
    struct RwV3d tmp; // r1+0x44
    struct RwV3d tmp; // r1+0x38
    struct RwV3d tmp; // r1+0x2C
    struct RwV3d tmp; // r1+0x20
    ...
    // References
    // -> static unsigned int sShadVertCount;
    // -> static struct RwRaster * sShadRasters[64];
    // -> static struct RxObjSpace3DVertex sShadVert[384];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D60E78
// Range: 0x800E7160 -> 0x800E75DC
void xShadowSimple_Init() {
    // Local variables
    struct RwTexture * tex; // r30
    unsigned int i; // r31
    struct RwV3d packed; // r1+0x44
    struct RwV3d packed; // r1+0x38
    struct RwV3d packed; // r1+0x2C
    struct RwV3d packed; // r1+0x20
    struct RwV3d packed; // r1+0x14
    struct RwV3d packed; // r1+0x8

    // References
    // -> static struct RxObjSpace3DVertex sShadVert[384];
    // -> static char __FUNCTION__[19];
    // -> static struct RwRaster * sShadRaster;
    // -> static struct xMat4x3 sCollModelMatrices[2];
    // -> static struct xShadowSimpleQueue sCollQueue[2];
}

// Range: 0x800E75DC -> 0x800E7620
...
static char __FUNCTION__[24]; // size: 0x18, address: 0x80513DA5
// total size: 0x4
...
struct zSimpleShadow {
    ...
    unsigned int shadowID; // offset 0x4, size 0x4
    unsigned int flags; // offset 0x8, size 0x4
};
// Range: 0x800E7620 -> 0x800E7808
void xShadowSimple_CacheInit(struct xShadowSimpleCache * cache /* r31 */, struct xEnt * ent /* r29 */, unsigned char alpha /* r1+0x8 */) {
    // Local variables
    int i; // r25
    int n; // r22
    unsigned int j; // r28
    struct zSimpleShadowTableHeader * sst; // r26
    struct RwRaster * raster; // r30
    unsigned int flags; // r24
    unsigned int size; // r1+0xC
    struct zSimpleShadow * table; // r27
    struct RwTexture * tex; // r23
...
// Range: 0x800E7808 -> 0x800E7870
void xShadowSimple_CacheInit(struct xShadowSimpleCache * cache /* r31 */, unsigned char alpha /* r1+0x8 */) {
    // References
    // -> static struct RwRaster * sShadRaster;
}

// Range: 0x800E7870 -> 0x800E78CC
void xShadowSimple_Add(struct xShadowSimpleCache * cache /* r31 */, struct xEnt * ent /* r30 */, float radius /* r1+0x8 */, float ecc /* r1+0xC */) {}

// Range: 0x800E78CC -> 0x800E791C
void xShadowSimple_Add(struct xShadowSimpleCache * cache /* r31 */, struct xMat4x3 * entMat /* r1+0x8 */, float radius /* r1+0xC */, float ecc /* r1+0x10 */) {}

// Range: 0x800E791C -> 0x800E7BA0
void xShadowSimple_Render() {
    // Local variables
    int qnum; // r31
    struct xShadowSimpleCache * cache; // r28
    unsigned int i; // r30
    unsigned int j; // r29
    ...
    // References
    // -> static unsigned int sShadVertCount;
    // -> static struct RxRenderStateVector xrsv;
    // -> static struct RxObjSpace3DVertex sShadVert[384];
    // -> static struct RwRaster * sShadRasters[64];
    // -> static struct xShadowSimpleQueue sCollQueue[2];
}
...
static char __FUNCTION__[25]; // size: 0x19, address: 0x80513DBD
// Range: 0x800E7BA0 -> 0x800E7F28
void xShadowSimple_Add_Expert(struct xShadowSimpleCache * cache /* r31 */, const struct xMat4x3 * mat_owner /* r30 */, unsigned char alpha /* r1+0x8 */, float radius /* f31 */, float height /* f28 */, float ecc /* f30 */, unsigned char copyMat /* r1+0x9 */) {
    // Local variables
    unsigned int ownerMoved; // r29
...
// Range: 0x800E7F28 -> 0x800E82E4
int XSSC_Add_ShoveIntoQueue(struct xShadowSimpleCache * cache /* r27 */, const struct xMat4x3 * mat_owner /* r28 */, float radius /* f29 */, float height /* f30 */, float ecc /* f31 */, unsigned char copyMat /* r29 */) {
    // Local variables
    int i; // r31
    int j; // r30
...
// Range: 0x800E8320 -> 0x800E858C
void XSSC_Add_NudgeItAndFudgeIt(struct xShadowSimpleCache * cache /* r31 */, const struct xMat4x3 * mat_owner /* r27 */) {
    // Local variables
    struct xVec3 * vert; // r30
    struct xVec3 xformvert[3]; // r1+0x14
    struct xVec3 xformnorm; // r1+0x8
    struct xEnt * castOnEnt; // r29
    float poo; // f31
    float nx; // f29
    float nz; // f28
    float pdot; // f27
