/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xCM.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x8001E564 -> 0x80020234
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
static struct basic_rect screen_bounds; // size: 0x10, address: 0x80D6C5D0
static const char * KEY; // size: 0x4, address: 0x80D5E838
// Range: 0x8001E564 -> 0x8001E5DC
static void Decrypt(void * buffer_ /* r3 */, unsigned int size /* r4 */, void * key_ /* r5 */, unsigned int keySize /* r6 */) {
    // Local variables
    unsigned char * buffer; // r31
    const unsigned char * key; // r29
    unsigned char last; // r28
    unsigned int keyCurrent; // r30
}

static char __FUNCTION__[8]; // size: 0x8, address: 0x80D5E83C
// total size: 0x4
struct xCreditsData {
    // Members
...
struct xCMcredits {
    ...
    struct sxy in; // offset 0xC, size 0x8
    struct sxy out; // offset 0x14, size 0x8
    float scroll_rate; // offset 0x1C, size 0x4
    float lifetime; // offset 0x20, size 0x4
    struct fade fin; // offset 0x24, size 0x8
    struct fade fout; // offset 0x2C, size 0x8
...
struct xCMtextbox {
    ...
    struct sxy char_size; // offset 0x8, size 0x8
    struct sxy char_spacing; // offset 0x10, size 0x8
    struct sxy box; // offset 0x18, size 0x8
};
// total size: 0x4C
struct xCMpreset {
    // Members
    ...
    struct xCMtextbox box[2]; // offset 0xC, size 0x40
};
// total size: 0x18
struct xCMhunk {
    // Members
    ...
    const char * text1; // offset 0x10, size 0x4
    const char * text2; // offset 0x14, size 0x4
};
// Range: 0x8001E5DC -> 0x8001E7E8
static void xCMprep(struct xCreditsData * data /* r27 */) {
    // Local variables
    struct xCMheader * hdr; // r29
    char * dp; // r26
    struct xCMcredits * cp; // r28
    struct xCMpreset * pp; // r25
    struct xCMhunk * hp; // r31
    struct xCMpreset * preset; // r30
    unsigned int newState; // r23

    // References
    // -> static char __FUNCTION__[8];
}

static float destX; // size: 0x4, address: 0x80D68C38
static float destY; // size: 0x4, address: 0x80D68C3C
static float destWidth; // size: 0x4, address: 0x80D5E844
static float destHeight; // size: 0x4, address: 0x80D5E848
// Range: 0x8001E7E8 -> 0x8001E834
static void xCMScaleBounds(struct basic_rect & r /* r3 */) {
    // References
    // -> static float destHeight;
    // -> static float destWidth;
    // -> static float destY;
    // -> static float destX;
}

// Range: 0x8001E834 -> 0x8001E888
static void xCMScaleBounds(float & x0 /* r3 */, float & y0 /* r4 */, float & x1 /* r5 */, float & y1 /* r6 */) {
    // References
    // -> static float destHeight;
    // -> static float destY;
    // -> static float destWidth;
...
// Range: 0x8001E888 -> 0x8001EA04
static struct xColor_tag xCMcolor_scale(struct xColor_tag & color /* r31 */, float scale /* r1+0x8 */) {
    // Local variables
    struct xColor_tag tmp; // r1+0x10
    struct xColorUnpack tmp2; // r1+0xC
    float r; // f30
    float g; // f29
    float b; // f28
    float a; // f31
...
unsigned int dumpit; // size: 0x4, address: 0x80D68C40
// Range: 0x8001EA04 -> 0x8001EAA4
static void xCMdump_header(struct xCMheader * hdr /* r31 */) {}

// Range: 0x8001EAA4 -> 0x8001EBCC
static void xCMdump_credits(struct xCMcredits * credits /* r31 */) {}

// Range: 0x8001EBCC -> 0x8001EC78
static void xCMdump_textbox(struct xCMtextbox * textbox /* r31 */) {}
...
struct xCMtexture {
    ...
    struct RwTexture * texture; // offset 0x18, size 0x4
    unsigned int pad; // offset 0x1C, size 0x4
};
// Range: 0x8001EC78 -> 0x8001ED50
static void xCMdump_texture(struct xCMtexture * tex /* r31 */) {}

static char * preset_names[5]; // size: 0x14, address: 0x80D5E84C
// Range: 0x8001ED50 -> 0x8001EE24
static void xCMdump_preset(struct xCMpreset * pp /* r31 */) {
    // References
    // -> static char * preset_names[5];
}

// Range: 0x8001EE24 -> 0x8001EEE0
static void xCMdump_hunk(struct xCMhunk * hunk /* r31 */) {}

static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5E860
...
// Range: 0x8001EEE0 -> 0x8001FEA8
static unsigned int xCMrender(float time /* f27 */, struct xCreditsData * data /* r20 */) {
    // Local variables
    struct xCMheader * hdr; // r27
    char * dp; // r23
    struct xCMcredits * cp; // r30
    struct xCMpreset * pp; // r22
    unsigned int i; // r26
    struct xCMhunk * hp; // r29
    float a; // f29
    struct xCMpreset * preset; // r31
    float nx; // f31
    float ny; // f30
    float x0; // r1+0x88
    float x1; // r1+0x84
    ...
    float y1; // r1+0x7C
    float ca; // f28
    struct xCMtexture * tex; // r28
    struct RwRaster * raster; // r19
    enum RwBlendFunction oldSrcBlend; // r1+0x78
    enum RwBlendFunction oldDestBlend; // r1+0x74
    struct xColor_tag color; // r1+0x70
    struct basic_rect bounds; // r1+0xBC
    class xtextbox tb; // r1+0x4B8
    enum RwBlendFunction oldSrcBlend; // r1+0x6C
    enum RwBlendFunction oldDestBlend; // r1+0x68
    struct xColor_tag color; // r1+0x64
    struct basic_rect bounds; // r1+0xAC
    class xtextbox tb; // r1+0x444
    enum RwBlendFunction oldSrcBlend; // r1+0x60
    enum RwBlendFunction oldDestBlend; // r1+0x5C
    unsigned int alignL; // r25
    unsigned int alignR; // r24
    struct xColor_tag colorL; // r1+0x58
    struct basic_rect bounds; // r1+0x9C
    class xtextbox tb; // r1+0x3D0
    struct xColor_tag colorR; // r1+0x54
    struct basic_rect bounds; // r1+0x8C
    class xtextbox tb; // r1+0x35C
...
float dtscale; // size: 0x4, address: 0x80D5E86C
static float credits_time; // size: 0x4, address: 0x80D5E870
static struct xCreditsData * credits_data; // size: 0x4, address: 0x80D68C44
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5E874
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x8001FEA8 -> 0x800200C8
void xCMupdate(float dt /* r1+0x8 */) {
    // Local variables
    float scale; // f31
    int i; // r30
    int port; // r31
    ...
    // References
    // -> static float credits_time;
    // -> static struct xCreditsData * credits_data;
...
static unsigned int credits_parentID; // size: 0x4, address: 0x80D68C48
// Range: 0x800200F0 -> 0x80020184
void xCMstart(struct xCreditsData * data /* r1+0x8 */, struct xBase * parent /* r30 */) {
    // Local variables
    struct xCMheader * hdr; // r31

    // References
    // -> static unsigned int credits_parentID;
    // -> static float credits_time;
    // -> static const char * KEY;
    // -> static struct xCreditsData * credits_data;
}

// Range: 0x80020184 -> 0x8002020C
void xCMstop() {
    // References
    // -> static unsigned int credits_parentID;
    // -> struct zGlobals globals;
...
// Range: 0x8002020C -> 0x80020220
void xCMsetDest(float x /* f1 */, float y /* f2 */, float width /* f3 */, float height /* f4 */) {
    // References
    // -> static float destHeight;
    // -> static float destWidth;
    // -> static float destY;
...
// Range: 0x80020220 -> 0x80020234
unsigned char xCMisRunning() {
    // References
    // -> static struct xCreditsData * credits_data;
}
