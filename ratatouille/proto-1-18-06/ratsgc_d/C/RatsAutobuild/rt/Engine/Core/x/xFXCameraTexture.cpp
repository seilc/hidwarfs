/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xFXCameraTexture.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x801FC274 -> 0x801FD3B8
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
class xFXCameraTexture {
    // Functions
    unsigned char create(int w, int h, unsigned char need_zwrite, int bpp, int zbpp);

    void destroy();

    void begin_render(int clear_flags, unsigned int fbmsk_write, float near_plane, float far_plane);

    void end_render();

    void render_background();

    void setup_subview(struct RwCamera * src_cam, const struct xFRect & src_rect, const struct xFRect & dst_rect);

    void get_subview(struct xMat4x3 & ltm, struct xVec2 & view_window, struct xVec2 & view_offset, struct RwCamera * src_cam, const struct xFRect & src_rect, const struct xFRect & dst_rect) const;
...
// Range: 0x801FC274 -> 0x801FC580
// this: r31
unsigned char xFXCameraTexture::create(int w /* r28 */, int h /* r29 */, unsigned char need_zwrite /* r1+0x8 */, int bpp /* r1+0xC */, int zbpp /* r1+0x10 */) {
    // Local variables
    struct iAsyncLockRWSentry rw_sentry; // r1+0x14
    struct RwBBox bbox; // r1+0x20
    struct RwV2d vw; // r1+0x18
    unsigned int rasterFlags; // r30
}
...
// Range: 0x801FC580 -> 0x801FC664
// this: r31
void xFXCameraTexture::destroy() {
    // Local variables
    struct RwFrame * tmpframe; // r30
}

static char __FUNCTION__[13]; // size: 0xD, address: 0x80D64410
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// total size: 0x4
struct /* @class$912xFXCameraTexture_cpp */ {
    // Members
...
union /* @class$658xFXCameraTexture_cpp */ {
...
// total size: 0xC
struct /* @class$1076xFXCameraTexture_cpp */ {
    // Members
...
// Range: 0x801FC664 -> 0x801FC8A0
// this: r31
void xFXCameraTexture::begin_render(int clear_flags /* r29 */, unsigned int fbmsk_write /* r30 */, float near_plane /* f30 */, float far_plane /* f31 */) {
    // References
    // -> struct zGlobals globals;
    // -> static char __FUNCTION__[13];
}

// Range: 0x801FC8A0 -> 0x801FC940
// this: r31
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D6441D
// Range: 0x801FC940 -> 0x801FCA44
// this: r31
...
// Range: 0x801FCA44 -> 0x801FCB20
// this: r31
void xFXCameraTexture::setup_subview(struct RwCamera * src_cam /* r30 */, const struct xFRect & src_rect /* r1+0x10 */, const struct xFRect & dst_rect /* r1+0x14 */) {
    // Local variables
    struct xMat4x3 ltm; // r1+0x30
    struct xVec2 view_window; // r1+0x20
    struct xVec2 view_offset; // r1+0x18
}
...
// Range: 0x801FCB20 -> 0x801FCF28
// this: r23
void xFXCameraTexture::get_subview(struct xMat4x3 & ltm /* r30 */, struct xVec2 & view_window /* r1+0x8 */, struct xVec2 & view_offset /* r1+0xC */, struct RwCamera * src_cam /* r24 */, const struct xFRect & src_rect /* r31 */, const struct xFRect & dst_rect /* r27 */) const {
    // Local variables
    struct RwRaster * src_raster; // r26
    const struct xMat4x3 & src_ltm; // r28
    const struct xVec2 & src_view_window; // r29
    const struct xVec2 & src_view_offset; // r25
    float src_width; // f23
    float src_height; // f22
    float dst_width; // f21
    float dst_height; // f20
    float src_iwidth; // f31
    float src_iheight; // f30
    float dst_iwidth; // f25
    float dst_iheight; // f24
    float sx; // f19
    float sy; // f18
    float sw; // r1+0x54
    float sh; // r1+0x50
    ...
    float rx; // r1+0x2C
    float ry; // r1+0x28
    float view_center_x; // r1+0x24
    float view_center_y; // r1+0x20
    float mx_correct; // r1+0x1C
    float my_correct; // r1+0x18
    float rw_correct; // r1+0x14
    float rh_correct; // r1+0x10
    float mx; // f29
    float my; // f28
}
...
class xFXFastRaster {
    // Functions
    void set_destination(struct RwRaster * dst, unsigned int fbmsk);
...
// Range: 0x801FCF28 -> 0x801FD0E4
unsigned char xFXFastRaster::clip_rect(struct xFRect & dst_rect /* r31 */, struct RwRaster * dst_raster /* r30 */) {
    // Local variables
    int width; // r25
    int height; // r24
    int dst_x; // r29
...
// Range: 0x801FD0E4 -> 0x801FD1CC
static unsigned char clip_rect(int & x /* r3 */, int & y /* r4 */, int & w /* r5 */, int & h /* r6 */, int wclip /* r7 */, int hclip /* r8 */) {}

// Range: 0x801FD1CC -> 0x801FD310
unsigned char xFXFastRaster::clip_rect(struct xFRect & dst_rect /* r31 */, struct RwRaster * dst_raster /* r1+0x8 */, struct xFRect & src_rect /* r30 */) {
    // Local variables
    float oldx; // f29
    float oldw; // f31
    float oldy; // f28
...
// Range: 0x801FD310 -> 0x801FD314
void xFXFastRaster::begin_render() {}

// Range: 0x801FD314 -> 0x801FD318
void xFXFastRaster::end_render() {}

// Range: 0x801FD318 -> 0x801FD31C
void xFXFastRaster::set_source() {}

static char __FUNCTION__[16]; // size: 0x10, address: 0x80D6442F
// Range: 0x801FD31C -> 0x801FD3B8
// this: r31
void xFXFastRaster::set_destination(struct RwRaster * dst /* r30 */, unsigned int fbmsk /* r1+0x8 */) {
    // References
