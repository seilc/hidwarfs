/*
    ...
    Language: C++
    Code range: 0x80020530 -> 0x8002A6B8
*/
static int been_here; // size: 0x4, address: 0x80D68C50
static int been_here; // size: 0x4, address: 0x80D68C54
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D5E880
enum _xCollsIdx {
...
// Range: 0x80020530 -> 0x8002086C
enum _xCollsIdx xCollideGetCollsIdx(const struct xCollis * coll /* r29 */, const struct xVec3 * tohit /* r31 */, const struct xMat3x3 * mat /* r30 */) {
    // Local variables
    float local_x; // f30
    float local_z; // f31
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5E894
// total size: 0xC
struct xVec3 {
    // Static members
...
// Range: 0x8002086C -> 0x80020B28
unsigned int xSphereHitsSphere(const struct xSphere * a /* r30 */, const struct xSphere * b /* r29 */, struct xCollis * coll /* r31 */) {
    // Local variables
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5E8A6
// Range: 0x80020B28 -> 0x80020D94
unsigned int xSphereHitsBox(const struct xSphere * a /* r29 */, const struct xBox * b /* r30 */, struct xCollis * coll /* r31 */) {
    // Local variables
    struct xIsect isx; // r1+0x8
    float scale; // f31
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5E8B5
// total size: 0x40
...
// Range: 0x80020D94 -> 0x800213D8
unsigned int xSphereHitsOBB_nu(const struct xSphere * s /* r29 */, const struct xBox * b /* r28 */, const struct xMat4x3 * m /* r31 */, struct xCollis * coll /* r30 */) {
    // Local variables
    struct xSphere xfs; // r1+0x1C
    struct xVec3 scale; // r1+0x10
    struct xMat4x3 mnormal; // r1+0x50
    float len2; // f31
    float len_inv; // f30
    float len2; // f29
    float len_inv; // f28
...
// Range: 0x800213D8 -> 0x800217B4
static int PointWithinTriangle(struct xVec3 * _pt /* r3 */, struct xVec3 * * _tri /* r4 */, struct xVec3 * _normal /* r5 */) {
    // Local variables
    struct RwV3d * pt; // r30
    struct RwV3d * * tri; // r31
    struct RwV3d * normal; // r26
    enum Dimension dimension; // r22
    float absX; // f31
    float absY; // f30
    float absZ; // f29
    int inside; // r27
...
// Range: 0x800217B4 -> 0x800218C4
static void FindNearestPointOnLine(struct xVec3 * result /* r1+0x8 */, struct xVec3 * point /* r1+0xC */, struct xVec3 * start /* r31 */, struct xVec3 * end /* r30 */) {
    // Local variables
    float mu; // f31
    struct RwV3d line; // r1+0x1C
    struct RwV3d candidate; // r1+0x10
    float lineLength2; // f30
}

// Range: 0x800218C4 -> 0x80021A84
static void properSphereIsectTri(const struct xVec3 * center /* r28 */, float radius /* f26 */, struct xVec3 * tohit /* r27 */, float * dist_ptr /* r29 */, struct RpCollisionTriangle * tri /* r31 */) {
    // Local variables
    struct xVec3 projPoint; // r1+0x20
    float dist2plane; // f28
    float dist; // f31
    float dist2; // f30
    float radius2; // f27
    unsigned int i; // r30
...
static int sCollidingJSP; // size: 0x4, address: 0x80D68C58
// total size: 0x4
...
// Range: 0x80021A84 -> 0x80021CA0
static struct RpCollisionTriangle * sphereHitsEnvCB(struct RpIntersection * isx /* r28 */, struct RpWorldSector * sector /* r29 */, struct RpCollisionTriangle * tri /* r30 */, float dist /* r1+0x8 */, void * data /* r1+0xC */) {
    // Local variables
    struct xCollis * coll; // r31
    float radius; // f31
    struct xVec3 * center; // r27
    struct xVec3 tohit; // r1+0x10
    float scale; // f30
...
// Range: 0x80021CA0 -> 0x80021CEC
static struct RpCollisionTriangle * sphereHitsModelCB(struct RpCollisionTriangle * tri /* r1+0x8 */, float dist /* r1+0xC */, void * data /* r1+0x10 */) {
    // Local variables
    struct xSphereHitsModel_context * context; // r31
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5E8C7
// total size: 0x18
struct RwLine {
    // Members
...
// Range: 0x80021CEC -> 0x8002201C
unsigned int xSphereHitsModel(const struct xSphere * b /* r28 */, const struct xModelInstance * m /* r30 */, struct xCollis * coll /* r31 */) {
    // Local variables
    struct RpIntersection isx; // r1+0x28
    struct xSphereHitsModel_context context; // r1+0x8
    struct xMat4x3 * mat; // r29
...
// Range: 0x8002201C -> 0x800220A0
unsigned int xSphereHitsEntModel(const struct xSphere * b /* r28 */, const struct xEnt * ent /* r31 */, struct xCollis * coll /* r29 */) {
    // Local variables
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5E8D8
// Range: 0x80022124 -> 0x800227E0
static int xParabolaEnvCB(struct xClumpCollBSPTriangle * triangles /* r29 */, void * data /* r23 */) {
    // Local variables
    struct ParabolaCBData * pd; // r30
    struct xParabola * p; // r31
    struct xCollis * colls; // r26
    struct xVec3 * v0; // r27
    struct xVec3 * v1; // r25
    struct xVec3 * v2; // r24
    unsigned char tester; // r28
    float a; // f27
    float b; // f29
    float c; // f18
    float det; // f20
    struct xVec3 v1p; // r1+0x48
    struct xVec3 v2p; // r1+0x3C
    struct xVec3 N; // r1+0x30
    float pdist; // r1+0x8
    float t1; // f30
    float t2; // f28
    struct xVec3 s1; // r1+0x24
    struct xVec3 s2; // r1+0x18
    float c1; // f24
    float c2; // f23
    float c0; // f19
    float d11; // f26
...
// Range: 0x800227E0 -> 0x80022B6C
int xParabolaHitsEnv(struct xParabola * p /* r31 */, const struct xEnv * env /* r28 */, struct xCollis * colls /* r29 */) {
    // Local variables
...
static const struct xCapsule * cb_capsule; // size: 0x4, address: 0x80D68C5C
int CapsuleEnvClosestToLineCB_trifilter; // size: 0x4, address: 0x80D5E8E8
int CapsuleEnvClosestToLineCB_triindex; // size: 0x4, address: 0x80D68C60
static char __FUNCTION__[26]; // size: 0x1A, address: 0x804F1048
// total size: 0x4
struct xColor_tag {
    // Members
    union { // inferred
...
// Range: 0x80022B6C -> 0x800230D8
static struct RpCollisionTriangle * CapsuleEnvClosestToLineCB(struct RpCollisionTriangle * tri /* r29 */, void * data /* r1+0x8 */) {
    // Local variables
    struct xCollis * colls; // r31
    const struct xVec3 * p1; // r26
    const struct xVec3 * p2; // r25
    const struct xVec3 * v0; // r30
    ...
    // -> int CapsuleEnvClosestToLineCB_trifilter;
    // -> int CapsuleEnvClosestToLineCB_triindex;
    // -> static char __FUNCTION__[26];
}

// Range: 0x800230D8 -> 0x80023118
static struct RpCollisionTriangle * CapsuleModelClosestToLineCB(struct RpCollisionTriangle * tri /* r1+0x8 */, float dist /* r1+0xC */, void * data /* r1+0x10 */) {}

static char __FUNCTION__[25]; // size: 0x19, address: 0x804F1062
// total size: 0x28
...
// Range: 0x80023118 -> 0x8002342C
static int LeafNodeCapsuleIntersect(struct xClumpCollBSPTriangle * triangles /* r31 */, void * data /* r26 */) {
    // Local variables
    struct CallBackParam * cbParam; // r27
    struct RwV3d * v0; // r30
    struct RwV3d * v1; // r29
    struct RwV3d * v2; // r28
    struct RpCollisionTriangle collisionTri; // r1+0x20
    struct RwV3d vTmp; // r1+0x14
    struct RwV3d vTmp2; // r1+0x8
    float recipLength; // f31
    float lengthSq; // f30
...
static char __FUNCTION__[33]; // size: 0x21, address: 0x804F107B
// total size: 0x1C
struct xCapsule {
    // Members
...
// Range: 0x8002342C -> 0x80023A70
int xCapsuleIsectsModelClosestToLine(const struct xCapsule * c /* r30 */, const struct xModelInstance * model /* r29 */, struct xCollis * colls /* r31 */) {
    // Local variables
...
static unsigned char registered; // size: 0x1, address: 0x80D68C64
static signed char init; // size: 0x1, address: 0x80D68C65
static char __FUNCTION__[31]; // size: 0x1F, address: 0x804F109C
// total size: 0x18
...
// Range: 0x80023A70 -> 0x80024014
int xCapsuleIsectsEnvClosestToLine(const struct xCapsule * c /* r30 */, const struct xEnv * env /* r29 */, struct xCollis * colls /* r31 */) {
    // Local variables
    struct RwV3d delta; // r1+0x14
    struct xClumpCollV3dGradient grad; // r1+0x20
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5E8EC
// Range: 0x80024014 -> 0x80024280
unsigned int xBoxHitsSphere(const struct xBox * a /* r29 */, const struct xSphere * b /* r30 */, struct xCollis * coll /* r31 */) {
    // Local variables
    struct xIsect isx; // r1+0x8
    float scale; // f31
...
// Range: 0x80024280 -> 0x80025088
static unsigned int Mgc_BoxBoxTest(const struct xBox * a /* r30 */, const struct xMat4x3 * matA /* r28 */, const struct xBox * b /* r31 */, const struct xMat4x3 * matB /* r29 */) {
    // Local variables
    struct xVec3 akA[3]; // r1+0xBC
    struct xVec3 akB[3]; // r1+0x98
    float afEA[3]; // r1+0x44
    float afEB[3]; // r1+0x38
    float _mag; // f27
    float _mag; // f26
    float _mag; // f25
    float _mag; // f24
    float _mag; // f23
    float _mag; // f22
    struct xVec3 centA; // r1+0x2C
    struct xVec3 centB; // r1+0x20
    struct xVec3 kD; // r1+0x14
    float aafC[3][3]; // r1+0x74
    float aafAbsC[3][3]; // r1+0x50
    float afAD[3]; // r1+0x8
    float fR0; // f29
    float fR1; // f28
    float fR; // f31
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D5E8FB
// Range: 0x80025088 -> 0x800252DC
unsigned int xObbHitsObb(const struct xBox * a /* r27 */, const struct xMat4x3 * a_mat /* r28 */, const struct xBox * b /* r29 */, const struct xMat4x3 * b_mat /* r30 */, struct xCollis * coll /* r31 */) {
    // References
    // -> static char __FUNCTION__[12];
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D5E907
struct xMat4x3 g_I3; // size: 0x40, address: 0x80B7FA80
// Range: 0x800252DC -> 0x800254E8
unsigned int xBoxHitsObb(const struct xBox * a /* r28 */, const struct xBox * b /* r29 */, const struct xMat4x3 * mat /* r30 */, struct xCollis * coll /* r31 */) {
    // References
    // -> struct xMat4x3 g_I3;
    // -> static char __FUNCTION__[12];
}

static char __FUNCTION__[11]; // size: 0xB, address: 0x80D5E913
struct xColor_tag g_NEON_RED; // size: 0x4, address: 0x80D6C6B8
struct xColor_tag g_NEON_GREEN; // size: 0x4, address: 0x80D6C6BC
// Range: 0x800254E8 -> 0x800258AC
static void render_tri(struct tri_data & tri /* r30 */, const struct xModelInstance & model /* r31 */) {
    // Local variables
    const struct xMat4x3 & m; // r23
    struct xVec3 v[3]; // r1+0x34
    struct RpGeometry * geom; // r26
    const struct xVec3 * verts; // r25
    const struct RpMorphTarget * mt; // r22
    const struct RpTriangle & t; // r24
    int i; // r29
    int i; // r28
    int i; // r27
    struct xVec3 r[2]; // r1+0x1C
    struct xVec3 d; // r1+0x10
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5E91E
// total size: 0x10
struct /* @class$3177xCollide_cpp */ {
    // Members
    struct xVec3 norm; // offset 0x0, size 0xC
    float D; // offset 0xC, size 0x4
};
// Range: 0x800258AC -> 0x80025FE0
void xCollideCalcTri(struct tri_data & tri /* r31 */, const struct xModelInstance & model /* r30 */, const struct xVec3 & center /* r17 */, const struct xVec3 & heading /* r18 */) {
    // Local variables
    struct xVec3 v[3]; // r1+0xC0
    struct RpGeometry * geom; // r28
    const struct xVec3 * verts; // r27
    const struct RpMorphTarget * mt; // r19
    const struct RpTriangle & t; // r26
    int i; // r29
...
static char __FUNCTION__[14]; // size: 0xE, address: 0x80D5E92E
// Range: 0x80025FE0 -> 0x800263A8
struct xVec3 xCollisTriHit(const struct tri_data & tri /* r30 */, const struct xModelInstance & model /* r31 */) {
    // Local variables
    const struct xMat4x3 & m; // r25
...
// Range: 0x800263A8 -> 0x800265EC
unsigned char xSphereHitsBox(const struct xVec3 & c /* r3 */, float r /* f1 */, const struct xBox & b /* r4 */) {
    // Local variables
    float ex; // f25
    float ey; // f26
    float ez; // f28
...
// Range: 0x800265EC -> 0x80026658
unsigned char xOBBHitsOBB(const struct xBox & a /* r29 */, const struct xMat4x3 & amat /* r28 */, const struct xBox & b /* r30 */, const struct xMat4x3 & bmat /* r31 */) {
    // Local variables
    float ax; // f28
    float ay; // f27
    float az; // f26
...
// Range: 0x80026658 -> 0x80026820
unsigned char xSphereHitsVCylinder(const struct xVec3 & sc /* r30 */, float sr /* f30 */, const struct xVec3 & cc /* r31 */, float cr /* r1+0x8 */, float ch /* f29 */) {
    // Local variables
    float ydist; // f31
    struct xVec2 xzloc1; // r1+0x1C
    struct xVec2 xzloc2; // r1+0x14
    float xzdist2; // f27
    float max_xzdist; // f28
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x804F10BB
struct xVec3 m_Null; // size: 0xC, address: 0x80D6D920
// Range: 0x80026820 -> 0x80026B6C
...
static char __FUNCTION__[20]; // size: 0x14, address: 0x80D5E93C
// Range: 0x80026B6C -> 0x80026E9C
unsigned int xCylinderHitsSphere(const struct xCylinder * cyl /* r30 */, const struct xSphere * sph /* r29 */, struct xCollis * coll /* r31 */) {
    // Local variables
    float sr; // f30
    float ydist; // f31
    struct xVec2 xzloc1; // r1+0x18
...
// Range: 0x80026E9C -> 0x80027000
unsigned char xSphereHitsVCircle(const struct xVec3 & sc /* r30 */, float sr /* f31 */, const struct xVec3 & cc /* r31 */, float cr /* r1+0x8 */) {
    // Local variables
    float ydist; // f30
    struct xVec2 xzloc1; // r1+0x1C
    struct xVec2 xzloc2; // r1+0x14
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5E950
// Range: 0x80027000 -> 0x8002736C
unsigned char xSphereHitsCone(const struct xVec3 & center /* r30 */, float radius /* r1+0x10 */, const struct xVec3 & kv0 /* r31 */, float kr0 /* r1+0x14 */, const struct xVec3 & kv1 /* r1+0x18 */, float kr1 /* r1+0x1C */) {
    // Local variables
    struct xMat4x3 mat; // r1+0x40
    float dist; // f25
    struct xVec3 C; // r1+0x34
    float R; // f30
    float Cx; // f21
    float Cy; // f29
    float Ax; // f24
...
// Range: 0x8002736C -> 0x8002749C
void xBoxForTriangle(struct xBox & box /* r3 */, const struct xVec3 & p0 /* r4 */, const struct xVec3 & p1 /* r5 */, const struct xVec3 & p2 /* r6 */) {}

// Range: 0x8002749C -> 0x800274FC
void xBoxForModel(struct xBox & o /* r30 */, const struct xModelInstance * m /* r31 */, unsigned char childModels /* r1+0x8 */) {}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D5E960
// Range: 0x800274FC -> 0x80027738
void xBoxForIModelLocal(struct xBox & o /* r29 */, const struct RpAtomic * imodel /* r31 */, unsigned char initBox /* r26 */) {
    // Local variables
    struct RpGeometry * geom; // r30
    struct RpMorphTarget * mtgt; // r27
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5E973
// Range: 0x80027738 -> 0x800278CC
void xBoxForModelLocal(struct xBox & o /* r31 */, const struct xModelInstance * m /* r29 */, unsigned char childModels /* r1+0x8 */) {
    // Local variables
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5E985
// Range: 0x800278CC -> 0x80027B2C
void xSphereForModel(struct xSphere & o /* r27 */, const struct xModelInstance * model /* r25 */, unsigned char childModels /* r1+0x8 */) {
    // Local variables
    struct RpAtomic * imodel; // r31
    struct RpGeometry * geom; // r30
    struct RpMorphTarget * mtgt; // r26
    unsigned short idx; // r28
    const struct xModelInstance * cur_model; // r29
    struct xSphere s; // r1+0xC

    // References
    // -> static char __FUNCTION__[16];
}

static struct xRay3 cbray; // size: 0x24, address: 0x8055BD08
static unsigned char cbnumcs; // size: 0x1, address: 0x80D68C66
static unsigned char cbmaxcs; // size: 0x1, address: 0x80D68C67
static unsigned char FLOOR; // size: 0x1, address: 0x80D68C68
static unsigned char NEXT2; // size: 0x1, address: 0x80D68C69
static unsigned char OTHER; // size: 0x1, address: 0x80D68C6A
static struct RpIntersection cbisx_local; // size: 0x1C, address: 0x8055BD2C
static float cbath; // size: 0x4, address: 0x80D68C6C
// Range: 0x80027B2C -> 0x80027BD8
static struct RpCollisionTriangle * rayHitsEnvBackwardCB(struct RpWorldSector * sector /* r4 */, struct RpCollisionTriangle * tri /* r5 */, float dist /* f1 */, void * data /* r6 */) {
    // Local variables
    struct xCollis * coll; // r31
...
// Range: 0x80027BD8 -> 0x80027C1C
static struct RpCollisionTriangle * rayHitsModelBackwardCB(struct RpIntersection * isx /* r1+0x8 */, struct RpCollisionTriangle * tri /* r1+0xC */, float dist /* r1+0x10 */, void * data /* r1+0x14 */) {}

// Range: 0x80027C1C -> 0x80027D68
static struct RpCollisionTriangle * rayHitsEnvCB(struct RpWorldSector * sector /* r28 */, struct RpCollisionTriangle * tri /* r30 */, float dist /* f31 */, void * data /* r1+0x8 */) {
    // Local variables
    struct xCollis * coll; // r31
    unsigned char flags; // r29
...
// Range: 0x80027D68 -> 0x80027DAC
static struct RpCollisionTriangle * rayHitsModelCB(struct RpIntersection * isx /* r1+0x8 */, struct RpCollisionTriangle * tri /* r1+0xC */, float dist /* r1+0x10 */, void * data /* r1+0x14 */) {}

static char __FUNCTION__[20]; // size: 0x14, address: 0x80D5E995
// Range: 0x80027DAC -> 0x8002826C
unsigned int xRayHitsModelSingle(const struct xRay3 * r /* r31 */, const struct xModelInstance * m /* r29 */, struct xCollis * coll /* r30 */) {
    // Local variables
    struct RpIntersection isx; // r1+0x38
    struct xVec3 delta; // r1+0x2C
    float len; // f31
    struct xMat4x3 * mat; // r28
    struct RwFrame * frame; // r27
    float mscale; // f30
...
// Range: 0x800282B8 -> 0x80028364
unsigned int xRayHitsModel(const struct xRay3 * r /* r28 */, const struct xModelInstance * m /* r29 */, struct xCollis * coll /* r31 */) {
    // Local variables
    struct xModelInstance * model; // r30
    struct xCollis temp_coll; // r1+0x8
}

static char __FUNCTION__[12]; // size: 0xC, address: 0x80D5E9A9
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x80028364 -> 0x80028814
unsigned int xRayHitsJSP(const struct xRay3 * r /* r31 */, const struct xJSPHeader * jsp /* r27 */, struct xCollis * coll /* r30 */) {
    // Local variables
    struct RpIntersection isx; // r1+0x14
    struct xVec3 delta; // r1+0x8
    float len; // f31
    const struct xEnv * env; // r29
    int count; // r25
    struct xJSPHeader * * jsplist; // r26
    int index; // r28
    struct xIsect iBoxIsect; // r1+0x30
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D5E9B5
// Range: 0x80028814 -> 0x80028CE8
unsigned int xRayHitsEnv(const struct xRay3 * r /* r31 */, const struct xEnv * env /* r29 */, struct xCollis * coll /* r30 */) {
    // Local variables
    struct RpIntersection isx; // r1+0x14
    struct xVec3 delta; // r1+0x8
    float len; // f31
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D5E9C1
// Range: 0x80028CE8 -> 0x800294F8
static struct RpCollisionTriangle * sphereHitsEnv3CB(struct RpIntersection * isx /* r27 */, struct RpWorldSector * sector /* r28 */, struct RpCollisionTriangle * tri /* r29 */, float dist /* r1+0x8 */, void * data /* r1+0xC */) {
    // Local variables
    struct xCollis * colls; // r31
    float radius; // f31
    struct xVec3 * center; // r26
    struct xVec3 tohit; // r1+0x1C
    struct xVec3 hdng; // r1+0x10
...
// Range: 0x800294F8 -> 0x8002953C
static struct RpCollisionTriangle * sphereHitsModel3CB(struct RpCollisionTriangle * tri /* r1+0x8 */, float dist /* r1+0xC */, void * data /* r1+0x10 */) {
    // References
    // -> static struct RpIntersection cbisx_local;
}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5E9D2
// Range: 0x8002953C -> 0x80029A28
int xSphereHitsModel3(const struct xSphere * b /* r26 */, const struct xModelInstance * m /* r29 */, struct xCollis * colls /* r31 */, unsigned char ncolls /* r25 */, float sth /* r1+0x10 */) {
    // Local variables
    struct RpIntersection isx; // r1+0x14
    unsigned char idx; // r30
    unsigned char i; // r27
    struct xMat4x3 * mat; // r28
    struct RwFrame * frame; // r24
    float mscale; // f31
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D5E9E4
// Range: 0x80029A28 -> 0x80029E08
int xSphereHitsEnv3(const struct xSphere * b /* r26 */, const struct xEnv * env /* r29 */, struct xCollis * colls /* r30 */, unsigned char ncolls /* r27 */, float sth /* r1+0x8 */) {
    // Local variables
    struct RpIntersection isx; // r1+0xC
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D5E9F4
// Range: 0x80029E08 -> 0x8002A070
unsigned int xSphereHitsEnv(const struct xSphere * b /* r28 */, const struct xEnv * env /* r30 */, struct xCollis * coll /* r31 */) {
    // Local variables
...
// Range: 0x8002A070 -> 0x8002A140
unsigned char xOBBHitsEnt(const struct xBox & box /* r29 */, const struct xMat4x3 & mat /* r30 */, const struct xEnt * ent /* r31 */, const struct xQCData * quickCullData /* r28 */) {}

// Range: 0x8002A140 -> 0x8002A254
static struct RpCollisionTriangle * OBBHitsEnvCB(struct RpCollisionTriangle * tri /* r30 */, void * data /* r1+0x8 */) {
    // Local variables
    struct OBBCallbackData * cbData; // r31
    struct xVec3 verts[3]; // r1+0xC
}

// Range: 0x8002A254 -> 0x8002A298
static struct RpCollisionTriangle * OBBHitsModelCB(struct RpIntersection * isx /* r1+0x8 */, struct RpCollisionTriangle * tri /* r1+0xC */, float dist /* r1+0x10 */, void * data /* r1+0x14 */) {}
...
// Range: 0x8002A298 -> 0x8002A3D4
unsigned char xOBBHitsScene(const struct xBox & box /* r28 */, const struct xMat4x3 & mat /* r29 */, struct xScene * sc /* r30 */, const struct xEnt * object /* r1+0x10 */, unsigned char collType /* r1+0x14 */, unsigned char chk /* r31 */) {
    // Local variables
    struct xMat4x3 tempMat; // r1+0x30
    struct xBound bound; // r1+0x70
    unsigned char hit; // r1+0x15
    struct OBBHitsEntCB cb; // r1+0x18
...
// Range: 0x8002A3D4 -> 0x8002A3EC
// this: r3
static OBBHitsEntCB::OBBHitsEntCB(const struct xBound & bound /* r4 */, unsigned char & hit /* r5 */, const struct xEnt * object /* r6 */, unsigned char chkby /* r7 */, unsigned char collType /* r8 */) {}
...
// Range: 0x8002A3EC -> 0x8002A4E0
unsigned char xOBBHitsEnv(const struct xBox & box /* r27 */, const struct xMat4x3 & mat /* r28 */, const struct xEnv * env /* r30 */) {
    // Local variables
...
// Range: 0x8002A4E0 -> 0x8002A604
unsigned char xOBBHitsModel(const struct xBox & box /* r26 */, const struct xMat4x3 & mat /* r27 */, const struct xModelInstance * model /* r31 */) {
    // Local variables
    struct RpIntersection intersection; // r1+0x1C
    struct RwFrame * frame; // r30
    struct RwMatrixTag * ltm; // r29
...
// Range: 0x8002A604 -> 0x8002A6B8
// this: r31
static unsigned char OBBHitsEntCB::operator()(struct xEnt & ent /* r30 */) {}
