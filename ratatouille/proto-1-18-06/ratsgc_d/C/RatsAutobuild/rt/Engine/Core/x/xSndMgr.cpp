/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xSndMgr.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x800E8E0C -> 0x800EE604
*/
// total size: 0x10
struct basic_rect {
    // Static members
...
// total size: 0x10
struct SndMixerFadersEntry {
    // Members
    enum xSndMixerFaderName fader; // offset 0x0, size 0x4
    float volume; // offset 0x4, size 0x4
    float dampVolume; // offset 0x8, size 0x4
    enum xSndMixerFaderName groupFader; // offset 0xC, size 0x4
};
struct SndMixerFadersEntry g_SndMixerFaders[9]; // size: 0x90, address: 0x80513DD8
...
enum xSndEffect m_eCurrentEffect; // size: 0x4, address: 0x80D698F0
// total size: 0x8
struct SndListenerSpringTableEntry {
    // Members
    enum eSndListenerSpringType type; // offset 0x0, size 0x4
    float stiffness; // offset 0x4, size 0x4
};
struct SndListenerSpringTableEntry g_SndListenerSpringTable[3]; // size: 0x18, address: 0x80513E68
unsigned char gShowSoundMessages; // size: 0x1, address: 0x80D698F4
unsigned char gDrawSoundDebug; // size: 0x1, address: 0x80D698F5
// total size: 0xC
struct SoundControlList {
    // Members
    char * string; // offset 0x0, size 0x4
    unsigned int count; // offset 0x4, size 0x4
    unsigned int * ids; // offset 0x8, size 0x4
};
struct SoundControlList IniList[4]; // size: 0x30, address: 0x80513E80
// Range: 0x800E8E0C -> 0x800E8E70
static unsigned char InIniList(unsigned int id /* r3 */, int whichList /* r4 */) {
    // Local variables
    struct SoundControlList * list; // r31
    unsigned int i; // r30

    // References
    // -> struct SoundControlList IniList[4];
}

static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60EBC
...
// Range: 0x800E8E70 -> 0x800E90F4
void xSndMgr_ParseINI(struct xIniFile * ini /* r1+0x8 */) {
    // Local variables
    struct SoundControlList * ctl; // r31
    int i; // r28
    const char * str; // r27
    char local_str[1024]; // r1+0xC
    char * tok; // r29
    int n; // r30
...
struct sound_fader {
    // Members
    enum xSndHandle handle; // offset 0x0, size 0x4
    float volume; // offset 0x4, size 0x4
...
class linear_pool {
    ...
    struct sound_fader * _buffer; // offset 0x8, size 0x4
};
static class linear_pool faders; // size: 0xC, address: 0x80D698F8
// Range: 0x800E90F4 -> 0x800E9120
static void setup_faders() {
    // References
    // -> static class linear_pool faders;
}

unsigned int gActiveHeap; // size: 0x4, address: 0x80D692A8
// Range: 0x800E9120 -> 0x800E9144
static void reset_faders() {
    // References
    // -> static class linear_pool faders;
}

static char __FUNCTION__[14]; // size: 0xE, address: 0x80D60ECD
char __FUNCTION__$localstatic1$erase__51linear_pool<Q221@unnamed@xSndMgr_cpp@11sound_fader>FPQ221@unnamed@xSndMgr_cpp@11sound_fader[6]; // size: 0x6, address: 0x80D60EDB
// Range: 0x800E9144 -> 0x800E92B0
static void update_faders(float dt /* r1+0x8 */) {
    // Local variables
    struct sound_fader * it; // r31
    struct sound_fader * end; // r30
...
static char __FUNCTION__[16]; // size: 0x10, address: 0x80D60EE1
// Range: 0x800E92B0 -> 0x800E9428
static struct xSndVoiceInfo * xSndMgrGetVoice(enum xSndHandle & xHandle /* r1+0x8 */) {
    // Local variables
    struct iSndHandle * iHandle; // r28
    int iVoiceRef; // r31
    int iVoiceIndex; // r30
    struct xSndVoiceInfo * pVoice; // r29
...
// Range: 0x800E9428 -> 0x800E949C
static struct sound_fader * find_fader(enum xSndHandle handle /* r1+0x8 */) {
    // Local variables
    struct sound_fader * it; // r31
    struct sound_fader * end; // r30
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x80513EB0
// Range: 0x800E949C -> 0x800E9610
int xSndMgrGetVoiceIndex(enum xSndHandle & xHandle /* r1+0x8 */) {
    // Local variables
    struct iSndHandle * iHandle; // r29
    int iVoiceRef; // r31
    int iVoiceIndex; // r30
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D60EF1
// Range: 0x800E9610 -> 0x800E9734
struct xSndGroup * xSndMgr_AllocGroup(unsigned char nSoundAssets /* r28 */) {
    // Local variables
    unsigned int datasize; // r29
    void * pAllocated; // r27
    struct xSndGroup * pRetGroup; // r31
    unsigned int i; // r30
...
// Range: 0x800E9734 -> 0x800E9BD4
void xSndMgrInit() {
    // Local variables
    int i; // r31

    // References
    // -> struct SndMixerFadersEntry g_SndMixerFaders[9];
    // -> unsigned char gDrawSoundDebug;
    // -> float speedOfSound;
    // -> enum xSndEffect m_eCurrentEffect;
    // -> struct xSndGlobals gSnd;
}
...
// Range: 0x800E9BD4 -> 0x800E9C3C
void xSndMgrSetListenerSpringType(enum eSndListenerSpringType type /* r31 */) {
    // Local variables
    float stiffness; // f31

    // References
    // -> struct xSndGlobals gSnd;
    // -> struct SndListenerSpringTableEntry g_SndListenerSpringTable[3];
}
...
// total size: 0x1
struct /* @class$794xSndMgr_cpp */ {
    // Members
    unsigned char value_def; // offset 0x0, size 0x1
};
// total size: 0x10
struct /* @class$795xSndMgr_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
...
// total size: 0x8
struct /* @class$796xSndMgr_cpp */ {
    // Members
    unsigned int value_def; // offset 0x0, size 0x4
    unsigned int mask; // offset 0x4, size 0x4
};
// total size: 0x10
struct /* @class$797xSndMgr_cpp */ {
    // Members
...
// Range: 0x800E9C3C -> 0x800E9C4C
void xSndMgrSelectListenerMode(enum xSndListener Mode /* r3 */) {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800E9C4C -> 0x800E9C8C
void xSndMgrSetEffect(enum xSndEffect Effect /* r31 */) {
    // References
    // -> enum xSndEffect m_eCurrentEffect;
}

// Range: 0x800E9C8C -> 0x800E9C94
...
// Range: 0x800E9C94 -> 0x800E9E38
void xSndMgrProcessSoundPosition(const struct xVec3 * pActual /* r31 */, struct xVec3 * pProcessed /* r30 */) {
    // Local variables
    struct xVec3 playerDelta; // r1+0x20
    struct xVec3 cameraDelta; // r1+0x14
    float playerDist; // f31
    float cameraDist; // f29
    float inwardShift; // f30
    float factor; // f28
    struct xVec3 fudgeAwayFromPlayer; // r1+0x8
...
// Range: 0x800E9E38 -> 0x800E9F98
void xSndMgr_UpdateListenerPosition(float dt /* r1+0x8 */) {
    // Local variables
    struct xMat4x3 * pMat; // r31
...
// Range: 0x800E9F98 -> 0x800EA070
void xSndMgr_InitListenerPosition() {
    // Local variables
    struct xMat4x3 * pMat; // r31
...
static unsigned char done; // size: 0x1, address: 0x80D69904
static signed char init; // size: 0x1, address: 0x80D69905
static unsigned char done; // size: 0x1, address: 0x80D69906
static signed char init; // size: 0x1, address: 0x80D69907
static char __FUNCTION__[28]; // size: 0x1C, address: 0x80513EC5
// total size: 0x8
struct xVec2 {
    // Members
    float x; // offset 0x0, size 0x4
...
// total size: 0x4
struct /* @class$2465xSndMgr_cpp */ {
    // Members
    float u; // offset 0x0, size 0x4
};
// total size: 0x10
struct /* @class$2466xSndMgr_cpp */ {
    // Members
    struct xVec3 pos; // offset 0x0, size 0xC
...
// Range: 0x800EA070 -> 0x800EA740
unsigned char xSndMgr_UpdateVoicePosition(struct xSndVoiceInfo * pVoice /* r31 */) {
    // Local variables
    struct xVec3 vel; // r1+0x34
    struct xEnt * ent; // r30
    struct xVec3 vel; // r1+0x28
    class zNPCCommon * npc; // r1+0xC
    struct xVec3 soundDir; // r1+0x1C
    float length2; // f31
    struct xVec3 * ptr; // r1+0x8
    float velListener; // f30
    float velSource; // f29
    float newDoppler; // f28
    struct xVec3 soundDirectionXYZ; // r1+0x10
...
// Range: 0x800EA740 -> 0x800EA7DC
struct xSndVoiceInfo * xSndMgr_GetFreeVirtualVoiceInfo(unsigned int priority /* r1+0x8 */, int flags /* r1+0xC */, unsigned char virtualOnly /* r1+0x10 */) {
    // Local variables
    struct xSndVoiceInfo * pVoice; // r1+0x14
    int i; // r31
...
// Range: 0x800EA7DC -> 0x800EA860
void xSndMgr_Stop(struct xSndVoiceInfo * pVoice /* r31 */, unsigned char bFreeRef /* r29 */) {
    // Local variables
    int voiceId; // r30
...
// Range: 0x800EA860 -> 0x800EA8F8
void xSndMgr_Pause(struct xSndVoiceInfo * pVoice /* r31 */, unsigned char bPaused /* r29 */) {
    // Local variables
    int voiceId; // r30
...
// Range: 0x800EA8F8 -> 0x800EA978
void xSndMgr_CutscenePause(struct xSndVoiceInfo * pVoice /* r31 */, unsigned char bPaused /* r30 */) {}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80513EE1
// Range: 0x800EA978 -> 0x800EABC4
unsigned char xSndMgr_SetInaudible(struct xSndVoiceInfo * pVoice /* r31 */) {
    // Local variables
    struct xSndVoiceInfo * pVoiceStorage; // r30
    int xVoiceId; // r29
    int xNewVoiceId; // r28

    // References
    // -> unsigned char gShowSoundMessages;
    // -> struct xSndGlobals gSnd;
    // -> static char __FUNCTION__[21];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D60F04
// Range: 0x800EACB0 -> 0x800EADE4
unsigned char xSndMgr_SetAudible(struct xSndVoiceInfo * pVoiceStorage /* r30 */) {
    // Local variables
    struct xSndVoiceInfo * vp; // r31
    unsigned char bOk; // r29
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60F17
// Range: 0x800EADE4 -> 0x800EAF9C
...
// Range: 0x800EAF9C -> 0x800EAFE4
void xSndMgr_SetInitFlag(unsigned char bIsInit /* r3 */) {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800EAFE4 -> 0x800EAFF8
unsigned char xSndMgr_IsInited() {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800EAFF8 -> 0x800EB09C
void xSndMgrUpdatePreScene() {
    // Local variables
    int i; // r31
...
// Range: 0x800EB09C -> 0x800EB158
void xSndMgrUpdate(float dt /* r1+0x8 */) {
    // Local variables
    unsigned int i; // r31
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60F28
// Range: 0x800EB158 -> 0x800EB2A0
void xSndMgrSceneExit() {
    // Local variables
    unsigned int i; // r31
    struct xSndVoiceInfo * pVoice; // r30
...
// Range: 0x800EB2A0 -> 0x800EB2C0
void xSndMgrExit() {}

// Range: 0x800EB2C0 -> 0x800EB33C
unsigned char xSndMgrDoesEffects(signed short eSoundCategory /* r3 */) {}

static char __FUNCTION__[18]; // size: 0x12, address: 0x80D60F39
float SECS_PER_VBLANK; // size: 0x4, address: 0x80D6A5A8
// Range: 0x800EB33C -> 0x800EB5C0
void xSndMgrStopSounds(signed short eSoundCategory /* r26 */, unsigned char bSkipPaused /* r27 */, unsigned char bInReset /* r1+0x8 */) {
    // Local variables
...
// Range: 0x800EB5C0 -> 0x800EB7E8
void xSndMgrStopPausedSounds(signed short eSoundCategory /* r28 */) {
    // Local variables
    int i; // r29
    int i; // r31
    int i; // r30
...
// Range: 0x800EB7E8 -> 0x800EB974
void xSndMgrPauseSounds(signed short eSoundCategory /* r27 */, unsigned char bPaused /* r28 */, unsigned char bPauseFutureSoundsOfThisType /* r1+0x8 */) {
    // Local variables
    int i; // r29
    int i; // r31
    int i; // r30
...
// Range: 0x800EB974 -> 0x800EBB00
void xSndMgrCutscenePause(signed short eSoundCategory /* r27 */, unsigned char bPaused /* r28 */, unsigned char bPauseFutureSoundsOfThisType /* r1+0x8 */) {
    // Local variables
    int i; // r29
    int i; // r31
    int i; // r30
...
static char __FUNCTION__[19]; // size: 0x13, address: 0x80D60F4B
enum xSndMixerFaderName {
    xSndMixerFader_NONE = -1,
    xSndMixerFader_CATEGORY_FADERS_START = 0,
    xSndMixerFader_GAME_3D = 0,
    xSndMixerFader_GAME_2D = 1,
    xSndMixerFader_DIALOG = 2,
    xSndMixerFader_DIALOG_3D = 3,
    xSndMixerFader_MUSIC = 4,
    xSndMixerFader_CUTSCENE = 5,
    xSndMixerFader_UI = 6,
    xSndMixerFader_GROUP_FADERS_START = 7,
    xSndMixerFader_MUSIC_GROUP = 7,
    xSndMixerFader_SFX_GROUP = 8,
    xSndMixerFader_FADERS_END = 9,
};
// Range: 0x800EBB00 -> 0x800EBC2C
void xSndMixerSetVolume(enum xSndMixerFaderName eMixerFader /* r31 */, float fVolume /* f31 */) {
    // References
    // -> struct SndMixerFadersEntry g_SndMixerFaders[9];
    // -> static char __FUNCTION__[19];
}

static char __FUNCTION__[19]; // size: 0x13, address: 0x80D60F5E
// Range: 0x800EBC2C -> 0x800EBD6C
float xSndMixerGetVolume(enum xSndMixerFaderName eMixerFader /* r31 */) {
    // References
    // -> struct SndMixerFadersEntry g_SndMixerFaders[9];
    // -> static char __FUNCTION__[19];
}

static char __FUNCTION__[23]; // size: 0x17, address: 0x80513EF6
// Range: 0x800EBD6C -> 0x800EBE18
void xSndMixerSetDampVolume(enum xSndMixerFaderName eMixerFader /* r31 */, float fVolume /* r1+0x8 */) {
    // References
    // -> struct SndMixerFadersEntry g_SndMixerFaders[9];
    // -> static char __FUNCTION__[23];
}

// Range: 0x800EBE18 -> 0x800EBE58
void xSndMixerResetDampVolumes() {
    // Local variables
    int i; // r31
...
static char __FUNCTION__[25]; // size: 0x19, address: 0x80513F0D
// Range: 0x800EBE58 -> 0x800EC178
float xSndMixerGetOutputVolume(signed short eSoundCategory /* r29 */) {
    // Local variables
    enum xSndMixerFaderName eMixerFader; // r30
    struct SndMixerFadersEntry * CurFader; // r31
    float outputVolume; // f31
    float CurFaderVolume; // f30
...
// Range: 0x800EC178 -> 0x800EC1C4
unsigned char xSndMgrGetSoundCount(enum iSndGroupHandle hSound /* r30 */) {
    // Local variables
    struct xSndGroup * pSndGroup; // r31
}

// Range: 0x800EC1C4 -> 0x800EC23C
unsigned int xSndMgrGetSoundId(enum iSndGroupHandle hSound /* r29 */, unsigned int uIndexOfSound /* r30 */) {
    // Local variables
    struct xSndGroup * pSndGroup; // r31
}

// Range: 0x800EC23C -> 0x800EC2F8
enum iSndGroupHandle xSndMgrGetSoundGroup(unsigned int uSoundGroupNameHash /* r30 */) {
    // Local variables
    enum iSndGroupHandle retHandle; // r31
    struct xSndGroup * pSndGroup; // r29

    // References
    // -> unsigned char gShowSoundMessages;
}

// Range: 0x800EC2F8 -> 0x800EC3B0
unsigned char xSndMgrIsPlaying(enum iSndGroupHandle hSoundGroup /* r30 */) {
    // Local variables
    int i; // r31
...
// Range: 0x800EC3B0 -> 0x800EC448
unsigned char xSndMgrGetPriority(enum iSndGroupHandle hSound /* r29 */, unsigned char bStreaming /* r1+0x8 */, unsigned char bLooping /* r1+0x9 */) {
    // Local variables
    struct xSndGroupHeader * pSnd; // r30
    unsigned int uPriority; // r31
}

static char __FUNCTION__[21]; // size: 0x15, address: 0x80513F26
// Range: 0x800EC448 -> 0x800EC724
struct xSndVoiceInfo * xSndGetFreeVoiceInfo(const struct xSndGroupHeader * pSnd /* r30 */, int iFlags /* r24 */) {
    // Local variables
    float dstFurthest; // f31
    int idxFurthest; // r28
    unsigned int ageOldest; // r26
    int idxOldest; // r27
    unsigned char uInstanceCount; // r29
    int i; // r31
    struct xSndVoiceInfo * vp; // r25
...
static char __FUNCTION__[12]; // size: 0xC, address: 0x80D60F71
// total size: 0x1C
...
// Range: 0x800EC724 -> 0x800ED0E8
enum xSndHandle xSndMgrPlay(enum iSndGroupHandle hSoundToPlay /* r25 */, int uFlags /* r24 */, const struct xVec3 * pPos_WS /* r23 */, const struct xVec3 * pVel /* r1+0x8 */, struct xEnt * pParentEnt /* r26 */, const float * pfOverrideVolume /* r1+0xC */, const float * pfOverridePitch /* r1+0x10 */) {
    // Local variables
    int iFlags; // r28
    struct xSndGroupHeader * pSnd; // r30
    unsigned int uInfoIndex; // r22
    struct xSndGroupInfo * pSndInfo; // r29
    struct iSndFileInfo * ip; // r27
    struct xSndVoiceInfo * vp; // r31
    unsigned int uniqueID; // r21
    int voiceIndex; // r1+0x18
    int voiceRef; // r20
    char buf[128]; // r1+0x1C
    unsigned char bOKtoPlay; // r19
    unsigned char bPlaying; // r18
...
// Range: 0x800ED0E8 -> 0x800ED168
unsigned int xSndMgrGetSoundId(enum xSndHandle & hSound /* r30 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r31

    // References
    // -> unsigned char gShowSoundMessages;
}

// Range: 0x800ED168 -> 0x800ED1E8
unsigned char xSndMgrIsPlaying(enum xSndHandle & hSound /* r31 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r30

    // References
    // -> unsigned char gShowSoundMessages;
}

// Range: 0x800ED1E8 -> 0x800ED26C
unsigned char xSndMgrIsLooping(enum xSndHandle & hSound /* r30 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r31
...
// Range: 0x800ED26C -> 0x800ED2F0
unsigned char xSndMgrIsStreamed(enum xSndHandle & hSound /* r30 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r31
...
// Range: 0x800ED2F0 -> 0x800ED358
unsigned int xSndMgrIsReady(enum xSndHandle & hSound /* r31 */) {
    // References
    // -> unsigned char gShowSoundMessages;
}

// Range: 0x800ED358 -> 0x800ED3D8
void xSndMgrPause(enum xSndHandle & hSound /* r30 */, unsigned char bPause /* r1+0x8 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r31
...
// Range: 0x800ED3D8 -> 0x800ED45C
void xSndMgrStop(enum xSndHandle & hSound /* r31 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r30
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60F7D
// Range: 0x800ED45C -> 0x800ED630
void xSndMgrSetVolume(enum xSndHandle & hSound /* r29 */, float fVolume /* f30 */, unsigned char assetRelative /* r1+0x8 */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r31
    float vol; // f31
    int voiceId; // r30
...
// Range: 0x800ED630 -> 0x800ED660
void xSndMgrStartStereo(enum xSndHandle hSnd00 /* r1+0x8 */, enum xSndHandle hSnd01 /* r1+0xC */) {}

// Range: 0x800ED660 -> 0x800ED6A4
void xSndMgrChangeAttachedPosition(enum xSndHandle & hSound /* r1+0x8 */, const struct xVec3 * pVector /* r1+0xC */) {
    // Local variables
    struct xSndVoiceInfo * voice; // r31
}

// Range: 0x800ED6A4 -> 0x800ED734
void xSndMgrStopChildren(struct xEnt * pParentEnt /* r1+0x8 */) {
    // Local variables
    unsigned int i; // r31
...
char __FUNCTION__$localstatic1$push_back__51linear_pool<Q221@unnamed@xSndMgr_cpp@11sound_fader>Fv[10]; // size: 0xA, address: 0x80D60F8E
char __FUNCTION__$localstatic1$back__51linear_pool<Q221@unnamed@xSndMgr_cpp@11sound_fader>Fv[5]; // size: 0x5, address: 0x80D60F98
// Range: 0x800ED734 -> 0x800ED80C
void xSndMgrFade(enum xSndHandle & hSound /* r30 */, float fadeTime /* f30 */, float startVolume /* r1+0x8 */, float destVolume /* f31 */) {
    // Local variables
    struct sound_fader * fader; // r31

    // References
    // -> static class linear_pool faders;
}

// Range: 0x800ED80C -> 0x800ED8B8
void xSndMgrFade(enum xSndHandle & hSound /* r31 */, float fadeTime /* f30 */, float destVolume /* f31 */) {
    // Local variables
    struct xSndVoiceInfo * vp; // r30
}

unsigned int gCurrSoundMemoryUsed; // size: 0x4, address: 0x80D69908
unsigned int gHighwaterSoundMemoryUsed; // size: 0x4, address: 0x80D6990C
unsigned int gNumStreamsOpened; // size: 0x4, address: 0x80D69910
unsigned int gNumStreamsClosed; // size: 0x4, address: 0x80D69914
static char streamPrefix[5]; // size: 0x5, address: 0x80D60F9D
static char memoryPrefix[5]; // size: 0x5, address: 0x80D60FA2
static char externalPrefix[5]; // size: 0x5, address: 0x80D60FA7
int the_heap; // size: 0x4, address: 0x80D69AD4
// Range: 0x800ED8B8 -> 0x800EDEAC
void xSndMgrDebugRender() {
    // Local variables
    int size; // r19
    char channelStats[256]; // r1+0x72
    int i; // r30
    struct iSndFileInfo * pSndFileInfo; // r24
    int i; // r28
    char buf[26]; // r1+0x8
    char msg[80]; // r1+0x22
    unsigned char stars; // r23
    int i; // r29
    const struct xSndVoiceInfo * vp; // r31
    struct iSndFileInfo * pSndFileInfo; // r22
    char * prefix; // r27
    char pausedState; // r26
    const char * name; // r25

    // References
    // -> unsigned char gDrawSoundDebug;
    // -> static char externalPrefix[5];
    // -> static char streamPrefix[5];
    // -> static char memoryPrefix[5];
    // -> struct xSndGlobals gSnd;
    // -> unsigned int gNumStreamsClosed;
    // -> unsigned int gNumStreamsOpened;
    // -> int the_heap;
}

// Range: 0x800EDEAC -> 0x800EDEB0
void xSndMgrDebugRenderWorld() {}

// Range: 0x800EDEB0 -> 0x800EE0E4
void xSndMgr_SetPaused(signed short whichCat /* r3 */, unsigned char bPause /* r4 */) {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800EE0E4 -> 0x800EE1E0
unsigned char xSndMgrIsPaused(signed short eSoundCategory /* r3 */) {
    // References
    // -> struct xSndGlobals gSnd;
}

// Range: 0x800EE1E0 -> 0x800EE2E4
int xSndMgrAllocRef(unsigned int uniqueID /* r1+0x8 */, int voiceIndex /* r29 */) {
    // Local variables
    int i; // r31
    int refIndex; // r30
...
static char __FUNCTION__[15]; // size: 0xF, address: 0x80D60FAC
// Range: 0x800EE2E4 -> 0x800EE414
void xSndMgrFreeRef(enum xSndHandle & xHandle /* r1+0x8 */) {
    // Local variables
    struct iSndHandle * iHandle; // r30
    int iVoiceRef; // r31
...
static char __FUNCTION__[17]; // size: 0x11, address: 0x80D60FBB
// Range: 0x800EE414 -> 0x800EE5A8
void xSndMgrUpdateRef(enum xSndHandle & xHandle /* r1+0x8 */, int voiceIndex /* r28 */) {
    // Local variables
    int i; // r31
    struct iSndHandle * iHandle; // r29
    int iVoiceRef; // r30
