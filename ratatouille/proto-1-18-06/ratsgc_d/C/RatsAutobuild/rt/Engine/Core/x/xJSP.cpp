/*
    Compile unit: C:\RatsAutobuild\rt\Engine\Core\x\xJSP.cpp
    Producer: MW EABI PPC C-Compiler
    Language: C++
    Code range: 0x80089CA4 -> 0x8008C198
*/
static struct RwV3d * sCurrVert; // size: 0x4, address: 0x80D69260
static unsigned int sAtomicStartCount; // size: 0x4, address: 0x80D69264
static struct RwV3d * * sAtomicStartVert; // size: 0x4, address: 0x80D69268
// total size: 0x8
struct RwObject {
    // Members
...
// Range: 0x80089CA4 -> 0x80089CCC
static struct RpAtomic * CountAtomicCB(struct RpAtomic * atomic /* r3 */, void * data /* r4 */) {
    // References
...
// Range: 0x80089CCC -> 0x80089D54
static struct RpMesh * AddMeshCB(struct RpMesh * mesh /* r29 */, void * pData /* r1+0x8 */) {
    // Local variables
    struct RwV3d * * stripVert; // r31
    unsigned int i; // r30
...
// Range: 0x80089D54 -> 0x80089DD0
static struct RpAtomic * AddAtomicCB(struct RpAtomic * atomic /* r31 */, void * data /* r30 */) {
    // References
    // -> static struct RwV3d * sCurrVert;
    // -> static unsigned int sAtomicStartCount;
...
// Range: 0x80089DD0 -> 0x80089E0C
static struct RpAtomic * AddAtomicPrecalcedVertCB(struct RpAtomic * atomic /* r3 */, void * data /* r4 */) {
    // References
    // -> static unsigned int sAtomicStartCount;
    // -> static struct RwV3d * * sAtomicStartVert;
}

// Range: 0x80089E0C -> 0x80089E38
static struct RpAtomic * ListAtomicCB(struct RpAtomic * atomic /* r3 */, void * data /* r4 */) {
    // Local variables
    struct RpAtomic * * * aList; // r31
}
...
static char __FUNCTION__[10]; // size: 0xA, address: 0x80D5FC30
// total size: 0x0
struct xCamGroup {};
// total size: 0x0
struct xCamScreen {};
...
// Range: 0x80089E38 -> 0x80089FD8
unsigned int get_matid(const struct xJSPHeader * jsp /* r24 */, unsigned int old_mat_id /* r25 */, unsigned int jspAssetID /* r26 */) {
    // Local variables
    char num[100]; // r1+0x8
    unsigned int matid; // r30
    int count; // r27
    const struct xJSPHeader * * jsplist; // r28
    int i; // r31
    const unsigned int * aidlist; // r29
...
static char __FUNCTION__[21]; // size: 0x15, address: 0x804F5600
// total size: 0xC
struct __rwMark {
    // Members
    unsigned int type; // offset 0x0, size 0x4
...
// Range: 0x80089FD8 -> 0x8008A948
void xJSP_MultiStreamRead(void * data /* r27 */, unsigned int size /* r21 */, struct xJSPHeader * * jsp /* r24 */, unsigned int assetID /* r1+0x8 */) {
    // Local variables
    int i; // r28
    struct RpClump * clump; // r22
    struct xClumpCollBSPTree * colltree; // r29
    int rwresult; // r1+0x38
    struct xJSPHeaderEx * hdr; // r31
    struct __rwMark mark; // r1+0x50
    struct __rwMark * mp; // r26
    unsigned int _size; // r17
    struct xJSPHeader * tmphdr; // r30
    struct RwV3d * currVec; // r1+0x34
    struct RwV3d * currVec; // r1+0x30
    struct xJSPNodeTree * tmpNodeTree; // r1+0x2C
    unsigned int * stripvecdata; // r20
    struct __rwMark * mp2; // r1+0x28
    struct __rwMark mark2; // r1+0x44
    struct RwMemory rwmem; // r1+0x3C
    struct RwStream * stream; // r19
    int i; // r23
    int atomCount; // r18
    struct RpAtomic * * atomList; // r25
    struct RpAtomic * * atomCurr; // r1+0x24
...
static char __FUNCTION__[13]; // size: 0xD, address: 0x80D5FC3A
// Range: 0x8008A988 -> 0x8008AB78
void xJSP_Destroy(struct xJSPHeader * jsp /* r31 */) {
    // Local variables
    int i; // r28
    struct RwLLLink * cur; // r29
    struct RwLLLink * end; // r26
    struct RpAtomic * apAtom; // r30
    unsigned int * tp; // r27
...
// Range: 0x8008AB78 -> 0x8008AC64
static void xJSPNodeLightBuild(struct xJSPHeader * jsp /* r30 */) {
    // Local variables
    struct xJSPNodeLight * nlight; // r31
    struct RwLLLink * cur; // r28
    struct RwLLLink * end; // r26
    struct RpClump * clump; // r27
    struct RpAtomic * apAtom; // r29
...
static struct TempSplit * sSplitList; // size: 0x4, address: 0x80D6926C
static struct TempSplit * sSplitCurr; // size: 0x4, address: 0x80D69270
static struct xJSPNodeTreeLeaf * sSplitLeaf; // size: 0x4, address: 0x80D69274
// total size: 0x18
struct RwBBox {
    // Members
...
// Range: 0x8008AC64 -> 0x8008B0C4
static float GetSplitVolumeSum(struct xJSPNodeTreeLeaf * tleaf /* r31 */, int count /* r26 */, int axis /* r27 */, float coord /* f19 */) {
    // Local variables
    int i; // r30
    int numleft; // r29
    int numright; // r28
...
// Range: 0x8008B0C4 -> 0x8008B35C
static int ChooseSplit_MeanMinSum(struct xJSPNodeTreeLeaf * tleaf /* r30 */, int count /* r28 */, int & bestaxis /* r26 */, float & bestcoord /* r27 */) {
    // Local variables
    float bestvolume; // f30
    int i; // r31
    int axis; // r29
...
// Range: 0x8008B35C -> 0x8008B630
static void RecurseSplitJSPNode(struct xJSPNodeTreeLeaf * tleaf /* r30 */, int count /* r27 */, struct TempSplitChild * childptr /* r29 */) {
    // Local variables
    int i; // r31
    int bestaxis; // r1+0xC
    int lastright; // r28
    ...
    struct TempSplit * thisSplit; // r26
...
static char __FUNCTION__[18]; // size: 0x12, address: 0x80D5FC47
// Range: 0x8008B674 -> 0x8008BC88
struct xJSPNodeTree * xJSPNodeTreeBuild(struct xJSPHeader * jsp /* r17 */) {
    // Local variables
    ...
    struct xJSPNodeTreeLeaf * tleaf; // r31
    struct RpGeometry * geom; // r24
    int numVert; // r22
    struct RwV3d * vert; // r28
    struct xJSPNodeTree * jntree; // r29
...
static char __FUNCTION__[36]; // size: 0x24, address: 0x804F5615
// total size: 0x8
struct nodeInfo {
    // Members
    unsigned int type; // offset 0x0, size 0x4
...
// Range: 0x8008BC88 -> 0x8008BF08
struct xJSPHeader * xJSP_ForAllBoxLeafNodeIntersections(struct xJSPHeader * hdr /* r27 */, struct RwBBox * box /* r28 */, int (* callBack)(struct xJSPHeader *, struct xJSPNodeTreeLeaf *, struct RwBBox *, void *) /* r24 */, void * data /* r1+0x8 */) {
    // Local variables
    int nStack; // r30
    struct nodeInfo nodeStack[64]; // r1+0x14
    struct nodeInfo node; // r1+0xC
    struct xJSPNodeTree * tree; // r29
    struct xJSPNodeTreeLeaf * leafy; // r25
    struct xJSPNodeTreeBranch * branch; // r31

    // References
    // -> static char __FUNCTION__[36];
}

// Range: 0x8008BF08 -> 0x8008C080
static int JSPApplyLightCB(struct xJSPHeader * jsp /* r3 */, struct xJSPNodeTreeLeaf * leaf /* r4 */, struct RwBBox * box /* r5 */, void * data /* r6 */) {
    // Local variables
    int i; // r30
    struct xJSPNodeLight * nlight; // r31
}

// Range: 0x8008C080 -> 0x8008C13C
void xJSP_ApplyLocalLight(struct xJSPHeader * jsp /* r1+0x8 */, struct RpLight * light /* r30 */) {
    // Local variables
    struct RwFrame * lframe; // r31
    float lradius; // f31
    struct RwBBox lbox; // r1+0xC
}

// Range: 0x8008C13C -> 0x8008C198
void xJSP_ClearLocalLight(struct xJSPHeader * jsp /* r3 */) {
    // Local variables
    unsigned int i; // r30
    struct xJSPNodeLight * nlight; // r31
}
