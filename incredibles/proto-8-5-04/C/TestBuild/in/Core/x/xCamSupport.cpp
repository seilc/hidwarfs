/*
    Compile unit: C:\TestBuild\in\Core\x\xCamSupport.cpp
    Producer: MW MIPS C Compiler
    Language: C++
*/
static class ent_cache_entry ent_cache[128]; // size: 0x200, address: 0x69C3C0
static signed int ent_cache_size; // size: 0x4, address: 0x6090E8
...
// total size: 0x14
class cache_entity_callback {
    // Members
public:
    class ent_cache_entry * cache; // offset 0x0, size 0x4
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031C920 -> 0x0031CB44
*/
// Range: 0x31C920 -> 0x31CB44
// this: r16
unsigned char xCamSupportLOSBar::collide_compress() {
    /* anonymous block */ {
        // Range: 0x31C920 -> 0x31CB44
        class xSweptSphere sws; // r29+0xA0
        class xVec3 start; // r29+0x180
        class xVec3 end; // r29+0x170
        class xSphere o; // r29+0x90
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031CB50 -> 0x0031E29C
*/
// Range: 0x31CB50 -> 0x31E29C
// this: r21
unsigned char xCamSupportLOSBar::collide_pivot() {
    /* anonymous block */ {
        // Range: 0x31CB50 -> 0x31E29C
        class xCollis coll; // r29+0x1F0
        class xCapsule to_cap; // r29+0x1D0
        float to_t; // r23
        float to_dist; // r22
        class xVec3 to_tohit; // r29+0x480
        class xVec3 to_heading; // r29+0x470
        class xVec3 to_impact; // r29+0x460
        float new_rail_yoffset; // r24
        class xCapsule new_cap; // r29+0x1B0
        class xVec3 N; // r29+0x450
        float D; // r29+0x490
        class xMat4x3 vcircle_mat; // r29+0x170
        float bar_mag; // r29+0x490
        float Rc; // r29+0x490
        float vrot; // r29+0x48C
        float oldpitch; // r29+0x490
        float newpitch; // r29+0x490
        float dx; // r29+0x490
        float dz; // r29+0x490
        float xzdist; // r29+0x490
        float newy; // r29+0x490
        class xCapsule from_cap; // r29+0x150
        float from_t; // r21
        float from_dist; // r20
        class xVec3 from_tohit; // r29+0x440
        float start_s; // r24
        float end_s; // r23
        signed int tries; // r20
        float s; // r22
        class xCapsule cap; // r29+0x130
        class xVec3 cap_offset; // r29+0x430
        class xVec3 impact; // r29+0x420
        class xVec3 N; // r29+0x410
        float D; // r29+0x490
        float min_from_t; // r29+0x490
        class xVec3 & C; // r17
        class xVec3 P; // r29+0x400
        class xVec3 CP; // r29+0x3F0
        class xVec3 uCP; // r29+0x3E0
        float Rc; // r29+0x490
        class xMat4x3 circle_mat; // r29+0xF0
        float max_rot; // r29+0x490
        class xMat3x3 rot_mat; // r29+0xC0
        float depen_mul; // r29+0x490
        class xVec3 depen; // r29+0x3D0
        class xVec3 AP; // r29+0x3C0
        class xVec3 BP; // r29+0x3B0
        class xVec3 rot_axis; // r29+0x3A0
        float R; // r29+0x490
        float R2; // r29+0x490
        class xVec3 PV; // r29+0x390
        class xVec3 V; // r29+0x380
        class xVec3 BV; // r29+0x370
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031E2A0 -> 0x0031E414
*/
// Range: 0x31E2A0 -> 0x31E414
// this: r2
void xCamSupportLOSBar::precollide() {
    /* anonymous block */ {
        ...
        float dx; // r29+0xC0
        float dz; // r29+0xC0
        float xzdist2; // r29+0xC0
        float dylower; // r29+0xC0
        float dyupper; // r29+0xC0
        float yupper; // r29+0xC0
        float ycenter; // r29+0xC0
        class xBound bound; // r29+0x70
        class cache_entity_callback cb; // r29+0x50
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031E420 -> 0x0031E4B4
*/
// Range: 0x31E420 -> 0x31E4B4
// this: r16
void xCamSupportLOSBar::begin_update(class xVec3 & origin /* r2 */) {
    /* anonymous block */ {
        // Range: 0x31E420 -> 0x31E4B4
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031E4C0 -> 0x0031E4FC
*/
// Range: 0x31E4C0 -> 0x31E4FC
// this: r2
void xCamSupportLOSBar::reset(class xVec3 & origin /* r2 */, class xVec3 & stern_loc /* r2 */) {
    /* anonymous block */ {
        // Range: 0x31E4C0 -> 0x31E4FC
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031E500 -> 0x0031E770
*/
// Range: 0x31E500 -> 0x31E770
static unsigned char sphere_hits_scene(class xSphere & o /* r18 */, class xCollis & coll /* r17 */) {
    /* anonymous block */ {
        // Range: 0x31E500 -> 0x31E770
        class xQCData qcd; // r29+0x110
        class ent_cache_entry * ite; // r16
        class ent_cache_entry * end_ite; // r2
        class xEnt & ent; // r2
        class xCollis temp_coll; // r29+0xB0
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031E770 -> 0x0031EC58
*/
// Range: 0x31E770 -> 0x31EC58
static unsigned char swept_sphere_hits_scene(class xSweptSphere & sws /* r19 */, class xVec3 & start /* r20 */, class xVec3 & end /* r18 */, float radius /* r21 */) {
    /* anonymous block */ {
        // Range: 0x31E770 -> 0x31EC58
        class xBox box; // r29+0x1A0
        class xQCData qcd; // r29+0x180
        class xRay3 ray; // r29+0x150
        class ent_cache_entry * ite; // r18
        class ent_cache_entry * end_ite; // r2
        class xEnt & ent; // r2
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031EC60 -> 0x0031F258
*/
// Range: 0x31EC60 -> 0x31F258
static unsigned char capsule_hits_scene(class xCapsule & cap /* r18 */, class xCollis & coll /* r17 */) {
    /* anonymous block */ {
        // Range: 0x31EC60 -> 0x31F258
        class xBox box; // r29+0x210
        class xQCData qcd; // r29+0x1F0
        class xRay3 ray; // r29+0x1C0
        class ent_cache_entry * ite; // r16
        class ent_cache_entry * end_ite; // r2
        class xEnt & ent; // r2
        class xCollis temp_coll; // r29+0x160
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031F260 -> 0x0031F54C
*/
// Range: 0x31F260 -> 0x31F54C
void compute_weights(float * w /* r23 */, signed int n /* r21 */, float s /* r24 */) {
    /* anonymous block */ {
        // Range: 0x31F260 -> 0x31F54C
        float start_efrac; // r23
        signed int i; // r20
        float efrac; // r22
        signed int j; // r19
        float sum; // r6
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031F550 -> 0x0031FA54
*/
// Range: 0x31F550 -> 0x31FA54
signed int polar_subdivide(class path_node * vert /* r21 */, signed int vert_size /* r20 */, class xCamCoordPolar & start /* r19 */, class xCamCoordPolar & end /* r18 */, float thresh /* r31 */) {
    /* anonymous block */ {
        // Range: 0x31F550 -> 0x31FA54
        class path_node head; // r29+0xD0
        class path_node tail; // r29+0xC0
        signed int vert_used; // r17
        float ABx; // r29+0xE0
        float ABy; // r29+0xE0
        float ABz; // r29+0xE0
        float best_total; // r30
        float x; // r29
        float y; // r28
        float z; // r27
        float theta; // r26
        ...
        float dy; // r21
        float dz; // r29+0xBC
        float dtheta; // r29+0xB8
        float dphi; // r29+0xB4
        float ddist; // r29+0xB0
        float total; // r20
        class path_node * v; // r16
        class path_node * endv; // r2
        float cp; // r23
        float ABx; // r29+0xE0
        float ABy; // r29+0xE0
        float ABz; // r29+0xE0
        ...
        float ABy; // r29+0xE0
        float ABz; // r29+0xE0
        float dtotal; // r29+0xE0
        signed int subdiv_increase; // r2
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031FA60 -> 0x0031FB18
*/
// Range: 0x31FA60 -> 0x31FB18
// this: r2
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031FB20 -> 0x0031FED8
*/
// Range: 0x31FB20 -> 0x31FED8
// this: r2
float xCamSupportPath::traverse(class xVec3 & rloc /* r2 */, float velmag /* r29 */, float dt /* r29 */) {
    /* anonymous block */ {
        // Range: 0x31FB20 -> 0x31FED8
        signed int i; // r3
        signed int end; // r2
        class path_node & p0; // r7
        class path_node & p1; // r9
        float dx; // r1
        float dy; // r16
        float dz; // r15
        class path_node & p2; // r2
        float ndx; // r29
        float ndy; // r29
        float ndz; // r29
        float d; // r29
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x0031FEE0 -> 0x0032041C
*/
// Range: 0x31FEE0 -> 0x32041C
// this: r2
void xCamSupportPath::smooth(signed int start /* r2 */, signed int size /* r2 */) {
    /* anonymous block */ {
        // Range: 0x31FEE0 -> 0x32041C
        float * w; // r22
        class path_node * pp; // r12
        class path_node * end_pp; // r2
        signed int r; // r11
        signed int end_r; // r2
        signed int i; // r21
        signed int end_i; // r2
        signed int path_end; // r2
        signed int j; // r20
        signed int end_j; // r2
        float Rx; // r7
        float Ry; // r10
        float Rz; // r9
        class path_node & node; // r2
        class path_node & B; // r2
        class path_node & B; // r2
        class path_node prev_path[33]; // @ 0x0069C230
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x00320420 -> 0x00320598
*/
// Range: 0x320420 -> 0x320598
// this: r2
void xCamSupportPath::prune(signed int start /* r2 */) {
    /* anonymous block */ {
        // Range: 0x320420 -> 0x320598
        signed int i0; // r8
        signed int i1; // r7
        signed int i2; // r6
        signed int end_i2; // r2
        class path_node & A; // r2
        class path_node & B; // r2
        class path_node & C; // r2
        float dx; // r29
        float dy; // r29
        float dz; // r29
    }
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x003205A0 -> 0x003206A0
*/
// Range: 0x3205A0 -> 0x3206A0
// this: r18
signed int xCamSupportPath::polar_subdivide(class xCamCoordPolar & start /* r5 */, class xCamCoordPolar & end /* r2 */) {
    /* anonymous block */ {
        // Range: 0x3205A0 -> 0x3206A0
        signed int vert_used; // r17
        signed int subdiv_end; // r2
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x003206A0 -> 0x003206B4
*/
// Range: 0x3206A0 -> 0x3206B4
// this: r2
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x003206C0 -> 0x00320728
*/
// Range: 0x3206C0 -> 0x320728
// this: r2
void xCamSupportPath::reset(class xVec3 & start_loc /* r2 */) {
    /* anonymous block */ {
        // Range: 0x3206C0 -> 0x320728
        class path_node * p; // r6
        class path_node * endp; // r2
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x00320730 -> 0x003207AC
*/
// Range: 0x320730 -> 0x3207AC
// this: r16
void xCamSupportPath::create(signed int max_size /* r2 */) {
    /* anonymous block */ {
        // Range: 0x320730 -> 0x3207AC
    }
}
