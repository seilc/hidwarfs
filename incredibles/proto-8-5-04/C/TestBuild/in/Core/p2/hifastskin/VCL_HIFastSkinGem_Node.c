/*
    Compile unit: C:\TestBuild\in\Core\p2\hifastskin\VCL_HIFastSkinGem_Node.c
    Producer: MW MIPS C Compiler
    Language: C++
*/
void * rwPDS_VCL_HIFastSkinGem_VU1Transforms[2]; // size: 0x8, address: 0x607C70
class RpPDSSkyVU1CodeTemplate rwPDS_VCL_HIFastSkinGem_VU1Code; // size: 0x8, address: 0x607C78
class RpPDSSkyMatTemplate rwPDS_VCL_HIFastSkinGem_MatPipe; // size: 0x84, address: 0x50E930
...
public:
    ...
    class RwV4dTag * uv2; // offset 0xC, size 0x4
    unsigned int * rgba; // offset 0x10, size 0x4
    class RwV4dTag * skin; // offset 0x14, size 0x4
    signed int vuoffset; // offset 0x18, size 0x4
};
// total size: 0x18
class RwResEntry {
    // Members
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x002FBE50 -> 0x002FBE88
*/
// Range: 0x2FBE50 -> 0x2FBE88
static signed int HIFastSkinGemInstanceCallBack(class RxPS2AllPipeData * ps2AllPipeData /* r16 */, void * * clusterData /* r2 */, unsigned int numClusters /* r2 */) {
    /* anonymous block */ {
        // Range: 0x2FBE50 -> 0x2FBE88
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x002FBE90 -> 0x002FC084
*/
// Range: 0x2FBE90 -> 0x2FC084
static void HIFastSkinGemInterleaveNormalReprocessData(class RxPS2AllPipeData * ps2AllPipeData /* r2 */) {
    /* anonymous block */ {
        // Range: 0x2FBE90 -> 0x2FC084
        signed int i; // r21
        signed int j; // r5
        signed int totwtct[4]; // r29+0xC0
        class BatchData batchcurr; // r29+0xA0
        class RpMesh * mesh; // r20
        class rwPS2AllResEntryHeader * hdr; // r19
        unsigned char * data; // r18
        signed int numbatches; // r2
        class RwV3d * normals; // r2
        class RwV3d * curnorm; // r2
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x002FC090 -> 0x002FD068
*/
// Range: 0x2FC090 -> 0x2FD068
static void FastSkinGemReprocessData(class RxPS2AllPipeData * ps2AllPipeData /* r2 */) {
    /* anonymous block */ {
        // Range: 0x2FC090 -> 0x2FD068
        signed int i; // r29+0x100
        signed int j; // r8
        signed int k; // r3
        signed int totwtct[4]; // r29+0x1160
        class BatchData batchlast; // r29+0x1140
        class BatchData batchcurr; // r29+0x1120
        class RwV4dTag overlapRestore[256]; // r29+0x120
        class RpMesh * mesh; // r29+0xF0
        class rwPS2AllResEntryHeader * hdr; // r29+0xE0
        unsigned char * data; // r29+0xD0
        signed int numbatches; // r2
        signed int repcount; // r21
        signed int repstart; // r6
        signed int repend; // r2
        signed int repemit; // r8
        signed int lastemit; // r23
        signed int overlapCount; // r20
        class RwV4dTag * currOverlapRestore; // r22
        signed int origindex; // r2
        signed int smallidx; // r2
        signed int smallval; // r8
        signed int gotBadOverlap; // r18
        signed int overIdx; // r17
        signed int overNum; // r10
        signed int uniqIdx; // r13
        signed int uniqcurr; // r7
        signed int zval; // r2
        signed int wval; // r2
        signed int weightct[4]; // r29+0x110
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x002FD070 -> 0x002FE474
*/
// Range: 0x2FD070 -> 0x2FE474
static void MoveBatchElement(class BatchData * batch /* r2 */, signed int srcidx /* r2 */, signed int dstidx /* r2 */) {
    /* anonymous block */ {
        // Range: 0x2FD070 -> 0x2FE474
        signed int i; // r8
        class RwV4dTag swap_xyzw; // r29+0x80
        class RwV4dTag swap_uv2; // r29+0x70
        unsigned int swap_rgba; // r2
        class RwV4dTag swap_skin; // r29+0x60
        class RwV4dTag swap_xyzw; // r29+0x50
        class RwV4dTag swap_uv2; // r29+0x40
        unsigned int swap_rgba; // r2
        class RwV4dTag swap_skin; // r29+0x30
    }
}
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x002FE480 -> 0x002FE590
*/
// Range: 0x2FE480 -> 0x2FE590
static void HIFastSkinGemGetBatchData(class BatchData * batch /* r2 */, signed int batchindex /* r2 */, class rwPS2AllResEntryHeader * hdr /* r2 */, class RpMesh * mesh /* r2 */, unsigned char * data /* r2 */) {
    /* anonymous block */ {
        // Range: 0x2FE480 -> 0x2FE590
...
/*
    ...
    Producer: MW MIPS C Compiler
    Language: C++
    Code range: 0x002FE590 -> 0x002FF494
*/
// Range: 0x2FE590 -> 0x2FF494
static signed int HIFastSkinGemMatBridgeCallBack(class RxPS2AllPipeData * ps2AllPipeData /* r20 */) {
    /* anonymous block */ {
        // Range: 0x2FE590 -> 0x2FF494
        __int128 * _rwDMAPktPtr'708; // r15
        unsigned int skyUserSwitch1'707; // r4
        class RpMaterial * material; // r2
        class rpMatFXMaterialData * materialData; // r19
        class RwTexture * baseTexture; // r18
        class RwTexture * dualTexture; // r17
        unsigned long alpha_2; // r21
        unsigned long clamp_2; // r22
        unsigned long fogcol_2; // r16
        unsigned long test_2; // r23
        unsigned long tex1_2; // r29+0x148
        signed long zbuf_2; // r2
        class RwMatrixTag envMapMatrix; // r29+0xF0
        __int128 ltmp; // r2
        unsigned long tmp1; // r2
        signed int oldTSClipperMode; // r2
        class RwMatrixTag tmpMat; // r29+0xB0
        class RwMatrixTag * atomicLtm; // r2
        class RpAtomic * atomic; // r2
        class RwFrame * envMapFrame; // r2
        unsigned int alphaFix; // r2
        unsigned long primSwitch; // r2
        unsigned long fogSwitch; // r5
        unsigned long rastex1; // r29+0x140
        unsigned long rastex2; // r29+0x138
        signed int fogVal; // r19
        signed int fogRestoreFlag; // r30
        class RwFrame * frame; // r2
        class RwCamera * camera; // r2
        class RwFrame * camframe; // r2
        float fogDist; // r29+0x150
        signed long prmTmp; // r4
